...
function createTempHome(): string {
  return makeTempDir(tempDirs, "joopo-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-joopo.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.joopo/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.joopo/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".joopo", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".joopo", "external-plugins", "glueclaw", "joopo.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".joopo", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.JOOPO_LIVE_TEST_QUIET = "1";
    process.env.JOOPO_CONFIG_PATH = "~/custom-joopo.json5";
    process.env.JOOPO_TEST_HOME = priorIsolatedHome;
    process.env.JOOPO_STATE_DIR = path.join(priorIsolatedHome, ".joopo");
    ...
    expect(process.env.JOOPO_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".joopo", "joopo.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".joopo", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".joopo", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.JOOPO_LIVE_TEST = "1";
    process.env.JOOPO_LIVE_USE_REAL_HOME = "1";
    process.env.JOOPO_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.JOOPO_LIVE_TEST;
    delete process.env.JOOPO_LIVE_GATEWAY;
    delete process.env.JOOPO_LIVE_USE_REAL_HOME;
    delete process.env.JOOPO_LIVE_TEST_QUIET;
