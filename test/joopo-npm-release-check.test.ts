...
} from "../scripts/joopo-npm-release-check.ts";
import {
...
describe("shouldSkipPackedTarballValidation", () => {
  ...
  it("accepts truthy values for metadata-only validation", () => {
    expect(
      shouldSkipPackedTarballValidation({
        JOOPO_NPM_RELEASE_SKIP_PACK_CHECK: "1",
      }),
    ).toBe(true);
  });
  ...
  it("treats false-like values as disabled", () => {
    expect(
      shouldSkipPackedTarballValidation({
        JOOPO_NPM_RELEASE_SKIP_PACK_CHECK: "false",
      }),
    ).toBe(false);
  });
});
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"joopo.tgz","files":[]}]')).toEqual([
      { filename: "joopo.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> joopo@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"joopo.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> joopo@2026.3.23 prepack")).toBeNull();
  });
});
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "joopo-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "joopo-pack-inventory-"));
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/joopo/joopo.git" },
        bin: { joopo: "joopo.mjs" },
      }),
    ).toEqual([]);
  });
  ...
  it("rejects local fs-safe dependency specs for npm release", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        repository: { url: "git+https://github.com/joopo/joopo.git" },
        bin: { joopo: "joopo.mjs" },
        dependencies: { "@joopo/fs-safe": "link:../fs-safe" },
      }),
    ).toContain(
      'package.json dependencies["@joopo/fs-safe"] must use a published semver range before npm release; found "link:../fs-safe".',
    );
  });
