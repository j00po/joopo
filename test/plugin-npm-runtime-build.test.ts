...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of plans) {
      ...
      expect(plan?.packagePeerMetadata.peerDependencies.joopo).toBe(
        plan?.packageJson.joopo.compat.pluginApi,
      );
      expect(plan?.packagePeerMetadata.peerDependenciesMeta.joopo.optional).toBe(true);
    }
  });
