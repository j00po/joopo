...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.JOOPO_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "JOOPO_TEST_FAST", value: process.env.JOOPO_TEST_FAST },
    {
      key: "JOOPO_STRICT_FAST_REPLY_CONFIG",
      value: process.env.JOOPO_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "JOOPO_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.JOOPO_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "JOOPO_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.JOOPO_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "JOOPO_STATE_DIR", value: process.env.JOOPO_STATE_DIR },
    { key: "JOOPO_CONFIG_PATH", value: process.env.JOOPO_CONFIG_PATH },
    { key: "JOOPO_GATEWAY_PORT", value: process.env.JOOPO_GATEWAY_PORT },
    { key: "JOOPO_BRIDGE_ENABLED", value: process.env.JOOPO_BRIDGE_ENABLED },
    { key: "JOOPO_BRIDGE_HOST", value: process.env.JOOPO_BRIDGE_HOST },
    { key: "JOOPO_BRIDGE_PORT", value: process.env.JOOPO_BRIDGE_PORT },
    { key: "JOOPO_CANVAS_HOST_PORT", value: process.env.JOOPO_CANVAS_HOST_PORT },
    { key: "JOOPO_TEST_HOME", value: process.env.JOOPO_TEST_HOME },
    { key: "JOOPO_AGENT_DIR", value: process.env.JOOPO_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.JOOPO_TEST_HOME = tempHome;
  process.env.JOOPO_TEST_FAST = "1";
  process.env.JOOPO_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.JOOPO_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.JOOPO_BRIDGE_ENABLED;
  delete process.env.JOOPO_BRIDGE_HOST;
  delete process.env.JOOPO_BRIDGE_PORT;
  delete process.env.JOOPO_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.JOOPO_STATE_DIR = path.join(tempHome, ".joopo");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.JOOPO_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.JOOPO_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".joopo");
  const priorIsolatedHome = params.env.JOOPO_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".joopo")
  ) {
    realStateDir = path.join(params.realHome, ".joopo");
  }
  const tempStateDir = path.join(params.tempHome, ".joopo");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.JOOPO_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.JOOPO_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "joopo.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "joopo.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.JOOPO_LIVE_TEST === "1" ||
    process.env.JOOPO_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.JOOPO_LIVE_USE_REAL_HOME);
