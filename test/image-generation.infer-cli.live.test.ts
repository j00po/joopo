...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.JOOPO_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through joopo infer", () => {
    const outputBase = path.join(os.tmpdir(), `joopo-infer-image-${process.pid}.png`);
