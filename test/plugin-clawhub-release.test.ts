...
import {
  ...
  collectClawHubJoopoOwnerErrors,
...
describe("collectClawHubJoopoOwnerErrors", () => {
  it("requires Joopo-scoped release candidates to already belong to the Joopo publisher", async () => {
    ...
    expect(errors).toEqual([
      "@joopo/demo-plugin: ClawHub package owner must be @joopo; got @steipete.",
      "@joopo/missing-plugin: ClawHub package row must already exist under @joopo before Joopo release publish.",
    ]);
  });
...
function createTempPluginRepo(
  ...
  const repoDir = makeTempRepoRoot(tempDirs, "joopo-clawhub-release-");
