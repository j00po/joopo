...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/joopo-pr-maintainer/scripts/github-activity.sh",
);
...
describe("joopo-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("joopo/joopo last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/joopo\/joopo\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/joopo\/joopo\/commits/g)).toHaveLength(1);
