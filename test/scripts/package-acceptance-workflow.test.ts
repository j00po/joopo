...
const LIVE_E2E_WORKFLOW = ".github/workflows/joopo-live-and-e2e-checks-reusable.yml";
const NPM_TELEGRAM_WORKFLOW = ".github/workflows/npm-telegram-beta-e2e.yml";
const PACKAGE_JSON = "package.json";
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/joopo-release-checks.yml";
...
describe("package acceptance workflow", () => {
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-joopo-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/joopo-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: joopo@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || 'joopo@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("JOOPO_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "JOOPO_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "JOOPO_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run Joopo Docker E2E package");
    expect(workflow).toContain("Download previous-run Joopo Docker E2E package");
    ...
    expect(workflow).toContain("JOOPO_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toMatchObject({
      JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      JOOPO_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "JOOPO_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain('["JOOPO_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["JOOPO_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("JOOPO_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("JOOPO_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("JOOPO_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u JOOPO_GATEWAY_TOKEN -u JOOPO_GATEWAY_PASSWORD joopo",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("joopo@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-joopo"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("JOOPO_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("JOOPO_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${JOOPO_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${JOOPO_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain(
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("JOOPO_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("JOOPO_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("JOOPO_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("JOOPO_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("JOOPO_LIVE_CLI_BACKEND_MODEL=codex-cli/gpt-5.4");
    expect(workflow).toContain("JOOPO_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).toContain("JOOPO_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect((workflow.match(/service_tier=\\"priority\\"/g) ?? []).length).toBeGreaterThanOrEqual(2);
    expect(workflow).not.toContain(
      'JOOPO_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: JOOPO_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).not.toContain(
      "JOOPO_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("JOOPO_LIVE_GATEWAY_MAX_MODELS=2");
    ...
    expect(workflow).toContain("JOOPO_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    ...
    expect(workflow).toContain("image: ghcr.io/joopo/joopo-live-media-runner:ubuntu-24.04");
    ...
    expect(workflow).toContain("JOOPO_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("JOOPO_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("JOOPO_LIVE_COMMAND_ATTEMPTS:-2");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: JOOPO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: JOOPO_LIVE_GATEWAY_PROVIDERS=openai JOOPO_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: JOOPO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: JOOPO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: JOOPO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    ...
    expect(build).toContain('ROOT_DIR="${JOOPO_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    expect(build).toContain('source "$SCRIPT_ROOT_DIR/scripts/lib/docker-build.sh"');
    expect(stage).toContain(
      'local scripts_dir="${JOOPO_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("JOOPO_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "JOOPO_QA_CONVEX_SITE_URL: ${{ secrets.JOOPO_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "JOOPO_QA_CONVEX_SECRET_CI: ${{ secrets.JOOPO_QA_CONVEX_SECRET_CI }}",
    );
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm joopo qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm joopo qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.JOOPO_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      expect(releaseWorkflow).toContain(`qa_live_${lower}_enabled="$qa_live_${lower}_ci_enabled"`);
      expect(releaseWorkflow).toContain(
        `vars.JOOPO_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`JOOPO_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("runs full release children from the trusted workflow ref", () => {
    ...
    expectTextToIncludeAll(
      ...
      [
        "scripts/resolve-joopo-package-candidate.mjs",
        "--source ref",
    ...
    expectTextToIncludeAll(dispatchStep.run, [
      ...
      'args=(-f package_spec="${PACKAGE_SPEC:-joopo@beta}"',
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(runStep.run, [
      'export JOOPO_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
