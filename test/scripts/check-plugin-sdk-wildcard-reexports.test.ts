  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "joopo/plugin-sdk/foo";',
          'export type * from "joopo/plugin-sdk/bar";',
          'export { named } from "joopo/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "joopo/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "joopo/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "joopo/plugin-sdk/foo";',
          'export type { Named } from "joopo/plugin-sdk/foo";',
