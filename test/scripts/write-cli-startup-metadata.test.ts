...
describe("write-cli-startup-metadata", () => {
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("joopo-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: joopo\n",
      renderSourceBrowserHelpText: () => "Usage: joopo browser\n",
    });
    ...
    expect(written.browserHelpText).toContain("joopo browser");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("joopo");
  });
});
