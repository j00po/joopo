...
describe("install.sh", () => {
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "joopo-install-nvm-"));
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "joopo-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g joopo@latest");
    expect(result?.stdout).toContain("using this user prefix");
    expect(result?.stdout).not.toContain("has been saved");
  });

  it("uses a quoted absolute joopo path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "joopo-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const joopoBin = join(npmBin, "joopo");
    writeFileSync(joopoBin, "#!/bin/sh\nexit 0\n");
    chmodSync(joopoBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(joopo_command_for_user "${joopoBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${joopoBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=joopo");
  });
});
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "joopo-install-sh-gum-"));
    try {
...
describe("install.sh duplicate Joopo install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/joopo" "$root/fnm/joopo"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/joopo/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/joopo/package.json"
      collect_joopo_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      JOOPO_BIN="$root/fnm/.bin/joopo"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_joopo_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Joopo global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/joopo");
    expect(result.stdout).toContain("/fnm/joopo");
    expect(result.stdout).toContain("Active joopo:");
    expect(result.stdout).toContain("npm uninstall -g joopo");
  });

  it("stays quiet when only one Joopo npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/joopo"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/joopo/package.json"
      collect_joopo_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_joopo_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Joopo global installs detected");
  });
});
