...
describe("scripts/lib/vitest-shard-timings.mjs", () => {
  ...
  it("uses the CI shard name for include-pattern timing keys", () => {
    expect(
      resolveShardTimingKey({
        ...
        env: { JOOPO_VITEST_SHARD_NAME: "auto-reply/reply agent dispatch" },
  ...
  it("persists include-pattern timing metadata", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-shard-timings-"));
    tempDirs.push(tempDir);
    const env = {
      JOOPO_TEST_PROJECTS_TIMINGS_PATH: path.join(tempDir, "timings.json"),
      JOOPO_VITEST_SHARD_NAME: "auto-reply-reply-agent-runner",
    };
    ...
    expect(
      JSON.parse(fs.readFileSync(env.JOOPO_TEST_PROJECTS_TIMINGS_PATH, "utf8")).configs[
