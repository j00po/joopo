...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and JoopoHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual(
      expect.objectContaining({
        command: "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@joopo/kitchen-sink@latest");
    ...
    expect(sweepScript).toContain("JOOPO_KITCHEN_SINK_PERSONALITY");
    ...
    expect(assertionsScript).toContain("record.joopopackSha256");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual(
      expect.objectContaining({
        command: "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
        name: "plugins",
        resources: expect.arrayContaining(["npm"]),
        stateScenario: "empty",
      }),
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/joopo-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "joopo-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, JOOPO_DISCORD_TOKEN)");
