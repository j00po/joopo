...
describe("happy path prompt snapshots", () => {
  ...
  it("deletes stale generated snapshot artifacts", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-prompt-snapshot-stale-"));
    try {
  ...
  it("keeps heartbeat guidance in heartbeat collaboration mode only", async () => {
    ...
    expect(heartbeat).toContain('"developer_instructions": "This is an Joopo heartbeat turn.');
    const joopoRuntimeInstructions = renderedPromptSection(
      heartbeat,
    ...
    expect(joopoRuntimeInstructions).not.toContain(heartbeatPhrase);
  ...
  it("finds the first available default Codex model catalog source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-codex-catalog-"));
    try {
  ...
  it("skips Codex model prompt fixture sync when no default catalog exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-codex-catalog-missing-"));
    const chunks: string[] = [];
    try {
