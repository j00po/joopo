...
const CHECK_SCRIPT = "scripts/check-joopo-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "joopo-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "joopo", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><joopo-app></joopo-app>",
    ...
    const tarball = join(root, "joopo.tgz");
...
describe("check-joopo-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Joopo package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
