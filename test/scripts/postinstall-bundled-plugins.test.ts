...
async function createExtensionsDir() {
  const root = await createTempDirAsync("joopo-postinstall-");
...
describe("bundled plugin postinstall", () => {
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/joopo/scripts/postinstall-bundled-plugins.mjs",
        modulePath: "/private/var/folders/tmp/joopo/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    const configuredBase = path.join("/tmp", "joopo-cache");
    ...
    expect(removed).not.toContain(path.join(configuredBase, "joopo"));
  ...
  it("keeps pruning sibling compile cache dirs after one removal fails", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] could not prune Joopo compile cache: Error: locked",
    );
  });

  it("does not warn when compile-cache pruning hits EACCES or EPERM (shared caches)", () => {
    const base = path.join("/tmp", "joopo-shared-compile-cache");
  ...
  it("does not warn when the compile-cache base directory cannot be listed (EACCES)", () => {
    const base = path.join("/tmp", "joopo-compile-cache-no-list");
    const warn = vi.fn();
    const rmSync = vi.fn();
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("joopo-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("joopo-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("joopo-source-checkout-state-skip-");
    const home = await createTempDirAsync("joopo-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".joopo", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("joopo-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { JOOPO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("joopo-postinstall-registry-");
  ...
  it("surfaces deprecated plugin registry migration break-glass warnings", async () => {
    ...
    const migratePluginRegistryForInstall = vi.fn(async () => ({
      ...
      migrated: false,
      preflight: {
        deprecationWarnings: ["JOOPO_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated"],
      },
    }));
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] JOOPO_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated",
    );
  });
  ...
  it("honors plugin registry postinstall migration disable env", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { JOOPO_DISABLE_PLUGIN_REGISTRY_MIGRATION: "1" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("does not disable plugin registry migration for falsey env flag strings", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { JOOPO_DISABLE_PLUGIN_REGISTRY_MIGRATION: "0" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-");
  ...
  it("prunes legacy plugin runtime deps state during packaged postinstall", async () => {
    const prefix = await createTempDirAsync("joopo-packaged-prefix-");
    const packageRoot = path.join(prefix, "lib", "node_modules", "joopo");
    const nodeModulesRoot = path.dirname(packageRoot);
    const home = await createTempDirAsync("joopo-packaged-home-");
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "[postinstall] could not prune legacy plugin runtime deps /home/alice/.joopo/plugin-runtime-deps: Error: locked",
      ),
    );
  });

  it("resolves legacy plugin runtime deps roots from Joopo state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          JOOPO_HOME: "/srv/joopo-home",
          JOOPO_CONFIG_PATH: "~/profile/joopo.json",
          JOOPO_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/joopo",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      ...
      "/var/lib/joopo/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-import-");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expect(
      fs.stat(path.join(packageRoot, "dist", "extensions", "qa-channel", "joopo.plugin.json")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("joopo-packaged-install-dist-prune-");
