...
describe("blacksmith testbox runner", () => {
  ...
  it("splits runner args from the remote command", () => {
    expect(
      splitRunnerArgs(["--id", "tbx_abc123", "--", "JOOPO_TESTBOX=1", "pnpm", "check:changed"]),
    ).toEqual({
      runnerArgs: ["--id", "tbx_abc123"],
      commandArgs: ["JOOPO_TESTBOX=1", "pnpm", "check:changed"],
    });
  });
  ...
  it("builds blacksmith run arguments", () => {
    ...
    ).toEqual(["testbox", "run", "--id", "tbx_abc123", "JOOPO_TESTBOX=1 pnpm check:changed"]);
  });
  ...
  it("refuses to run a keyed id that was not claimed by this checkout", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-testbox-runner-"));
    ...
    const code = await runBlacksmithTestboxRunner({
      argv: ["--id", "tbx_01kqap50t9fqggzw1akg5dtmmq", "--", "pnpm", "check:changed"],
      env: { ...process.env, JOOPO_BLACKSMITH_TESTBOX_STATE_DIR: stateDir },
      spawn: () => {
        spawned = true;
        return { status: 0 };
      },
    ...
    expect(code).toBe(2);
    expect(spawned).toBe(false);
    expect(stderrText).toContain("Joopo Testbox claim missing");
  });
  ...
  it("claims a keyed id without spawning when no remote command is supplied", async () => {
    ...
    tempDirs.push(stateDir);
    const testboxDir = path.join(stateDir, "tbx_01kqap50t9fqggzw1akg5dtmmq");
    const claimPath = path.join(testboxDir, "joopo-runner.json");
    fs.mkdirSync(testboxDir, { recursive: true });
    fs.writeFileSync(path.join(testboxDir, "id_ed25519"), "test-key\n");
    ...
    expect(code).toBe(0);
    expect(spawned).toBe(false);
    expect(stdoutText).toContain("Joopo Testbox claim written");
  ...
  it("defaults the Testbox sync timeout and accepts disable override", () => {
    ...
    expect(resolveTestboxSyncTimeoutMs({ JOOPO_TESTBOX_SYNC_TIMEOUT_MS: "0" })).toBe(0);
    expect(resolveTestboxSyncTimeoutMs({ JOOPO_TESTBOX_SYNC_TIMEOUT_MS: "2500" })).toBe(2500);
  });
  ...
  it("terminates a Testbox run that stalls in sync", async () => {
    ...
    const code = await runBlacksmithTestboxRunner({
      ...
      env: {
        ...process.env,
        JOOPO_BLACKSMITH_TESTBOX_STATE_DIR: stateDir,
        JOOPO_TESTBOX_SYNC_TIMEOUT_MS: "1",
      },
      spawn: fakeSpawn,
      stderr: { write: (value: string) => (stderrText += value) },
      stdout: { write: () => 0 },
