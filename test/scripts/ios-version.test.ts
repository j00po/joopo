...
describe("renderIosVersionXcconfig", () => {
  it("renders checked-in defaults from the pinned iOS version", () => {
    ...
    expect(renderIosVersionXcconfig(version)).toContain("JOOPO_IOS_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("JOOPO_MARKETING_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("JOOPO_BUILD_VERSION = 1");
  });
});
