...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "Joopo installation failed with exit code/);
  });
  ...
  it("runs npm capture commands from a writable installer temp directory", () => {
    ...
    const npmInstallBody = extractFunctionBody(source, "Install-JoopoNpm");
    const mainBody = extractFunctionBody(source, "Main");
  ...
  runIfPowerShell("creates a temp npm working directory", () => {
    const tempDir = harness.createTempDir("joopo-install-ps1-");
    ...
    writeFileSync(
      ...
      [
        ...
        'if ($result -notmatch "joopo-installer") { throw "unexpected $result" }',
        "",
      ].join("\n"),
    );
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("caught=Joopo installation failed with exit code 1.");
