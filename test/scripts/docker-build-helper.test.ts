...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("JOOPO_DOCKER_BUILD_RETRIES");
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("JOOPO_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("JOOPO_TESTBOX");
    ...
    expect(liveCliBackend).toContain("direct Codex CLI probe failed before Joopo gateway smoke");
    expect(liveCliBackend).toContain("==> Direct Codex CLI probe ok");
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (JOOPO_SKIP_DOCKER_BUILD=1)"',
    );
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("JOOPO_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(wrapper).toContain("JOOPO_INSTALL_E2E_AGENT_TURNS_PARALLEL");
  });
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(scripts.join("\n")).toContain("JOOPO_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("JOOPO_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("JOOPO_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("JOOPO_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runner).toContain("scripts/e2e/lib/bundled-plugin-install-uninstall/sweep.sh");
    expect(probe).toContain('"joopo.plugin.json"');
    expect(runtimeSmoke).toContain("process.env.JOOPO_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runtimeSmoke).toContain("900000");
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$JOOPO_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$JOOPO_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("covers plugin install/update sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('plugins install "npm:@joopo/demo-plugin-npm@0.0.1"');
