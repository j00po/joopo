...
describe("scripts/lib/docker-e2e-plan", () => {
    ...
    expect(packageUpdateCore.lanes).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          command: "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor",
        ...
        expect.objectContaining({
          ...
          command: "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth",
          stateScenario: "upgrade-survivor",
        }),
      ]),
    );
  ...
  it("expands the published upgrade survivor lane across deduped baselines", () => {
    const plan = planFor({
      ...
      upgradeSurvivorBaselines:
        "joopo@2026.4.29 2026.4.23 joopo@2026.4.23 joopo@2026.3.13-1",
    });
    ...
    expect(plan.lanes).toEqual([
      expect.objectContaining({
        command: expect.stringContaining(
          "JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC='joopo@2026.4.29'",
        ),
        imageKind: "bare",
        stateScenario: "upgrade-survivor",
      }),
      expect.objectContaining({
        command: expect.stringContaining(
          "JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC='joopo@2026.4.23'",
        ),
      }),
      expect.objectContaining({
        command: expect.stringContaining(
          "JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC='joopo@2026.3.13-1'",
        ),
      }),
    ]);
    expect(plan.lanes[0]?.command).toContain(
      'JOOPO_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/published-upgrade-survivor-2026.4.29"',
    );
  });
  ...
  it("expands the published upgrade survivor lane across scenarios", () => {
    ...
    expect(plan.lanes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          command: expect.stringContaining("JOOPO_UPGRADE_SURVIVOR_SCENARIO='feishu-channel'"),
        }),
        expect.objectContaining({
          command: expect.stringContaining("JOOPO_UPGRADE_SURVIVOR_SCENARIO='tilde-log-path'"),
        }),
      ]),
    );
    expect(
      ...
      'JOOPO_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/published-upgrade-survivor-2026.4.29-tilde-log-path"',
    );
  });
  ...
  it("expands update migration across baselines and cleanup scenarios", () => {
    ...
    expect(plan.lanes).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          command: expect.stringContaining(
            "JOOPO_UPGRADE_SURVIVOR_SCENARIO='plugin-deps-cleanup'",
          ),
        }),
      ]),
    );
  ...
  it("plans the Codex npm plugin live lane as package-backed OpenAI proof", () => {
    ...
    expect(plan.lanes).toEqual([
      expect.objectContaining({
        command: "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-codex-npm-plugin",
        imageKind: "bare",
        live: true,
  ...
  it("maps installer E2E to provider-specific package install lanes", () => {
    ...
    expect(plan.lanes).toEqual([
      expect.objectContaining({
        command: expect.stringContaining("JOOPO_E2E_MODELS=openai"),
        name: "install-e2e-openai",
      }),
      expect.objectContaining({
        command: expect.stringContaining("JOOPO_E2E_MODELS=anthropic"),
  ...
  it("maps bundled plugin install/uninstall to package-backed shards", () => {
    ...
    expect(plan.lanes[0]).toEqual(
      expect.objectContaining({
        command: expect.stringContaining("JOOPO_BUNDLED_PLUGIN_SWEEP_INDEX=0"),
        imageKind: "functional",
        live: false,
        name: "bundled-plugin-install-uninstall-0",
    ...
    expect(plan.lanes[23]).toEqual(
      expect.objectContaining({
        command: expect.stringContaining("JOOPO_BUNDLED_PLUGIN_SWEEP_INDEX=23"),
        imageKind: "functional",
        live: false,
  ...
  it("rejects unknown selected lanes with the available lane names", () => {
    expect(() => planFor({ selectedLaneNames: ["missing-lane"] })).toThrow(
      /JOOPO_DOCKER_ALL_LANES unknown lane\(s\): missing-lane/u,
    );
  });
});
