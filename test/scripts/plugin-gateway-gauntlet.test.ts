...
describe("plugin gateway gauntlet helpers", () => {
  ...
  it("discovers bundled plugin manifests into lifecycle matrix rows", async () => {
    ...
    expect(matrix[0]).toMatchObject({
      ...
      manifestPath: path.join("extensions", "alpha", "joopo.plugin.json"),
      enabledByDefault: true,
      providers: ["openai"],
      authMethods: ["oauth"],
  ...
  it("skips source-only plugin dirs that are excluded from the built runtime", async () => {
    await writeManifest("qqbot", "joopo.plugin.json", JSON.stringify({ id: "qqbot" }));
    await writeManifest("telegram", "joopo.plugin.json", JSON.stringify({ id: "telegram" }));
