...
import { createJoopoTestInstance, type JoopoTestInstance } from "./joopo-test-instance.js";
...
export type GatewayInstance = JoopoTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createJoopoTestInstance({ name });
  try {
