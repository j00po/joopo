...
const piEmbeddedMocks = getSharedMocks("joopo.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("joopo.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("joopo.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("joopo.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    joopoHome: process.env.JOOPO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("JOOPO_HOME", snapshot.joopoHome);
  restoreKey("JOOPO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.JOOPO_STATE_DIR = join(home, ".joopo");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "joopo-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".joopo", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): JoopoConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "joopo"),
