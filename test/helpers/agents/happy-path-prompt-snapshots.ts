...
import type { JoopoConfig } from "../../../src/config/types.joopo.js";
import type {
  AnyAgentTool,
...
import { createJoopoCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/joopo-happy-path/workspace";
const AGENT_DIR = "/tmp/joopo-happy-path/agent";
const SESSION_FILE = "/tmp/joopo-happy-path/session.jsonl";
...
type CodexPromptSnapshotApi = {
  ...
  createCodexDynamicToolSpecsForPromptSnapshot: (params: {
    tools: AnyAgentTool[];
    pluginConfig?: { codexDynamicToolsProfile?: "native-first" | "joopo-compat" };
...
const CODEX_WORKSPACE_BOOTSTRAP_INSTRUCTIONS = [
  "Joopo loaded these user-editable workspace files. Treat them as project/user context. Codex loads AGENTS.md natively, so AGENTS.md is not repeated here.",
  "",
  "# Project Context",
  "",
...
const dynamicToolsConfig: JoopoConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "Joopo maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "Joopo",
    ConversationLabel: "Joopo/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@Joopo please verify the Codex happy path too.",
      },
    ],
    Body: "@Joopo can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to Joopo/Codex.",
      ],
      trigger: "user",
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const joopoDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack Joopo can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, simulated Joopo workspace bootstrap config instructions, Joopo developer instructions, turn-scoped collaboration-mode instructions when Joopo provides them, turn input, and the Joopo dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
    ...
    markdownFence(
      ...
      stableJson({
        ...
        joopoDeveloperInstructions: textStats(joopoDeveloperInstructions),
    ...
    "",
    "### User: Codex Config Instructions (Joopo Workspace Bootstrap Context)",
    "",
    markdownFence("text", codexConfigInstructions),
    "",
    "### Developer: Joopo Runtime Instructions",
    "",
    markdownFence("text", joopoDeveloperInstructions),
    "",
...
function renderScenarioSnapshot(scenario: PromptScenario): string {
  ...
  return [
    ...
    "- This captures the Joopo-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, simulated Joopo workspace bootstrap config instructions, Joopo developer instructions, user turn input, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy `SOUL.md`, `TOOLS.md`, and `HEARTBEAT.md` contents so prompt reviewers can see how those Joopo project/user context files are forwarded to Codex. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path Joopo tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after Joopo sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
