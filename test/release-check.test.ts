...
import { bundledDistPluginFile, bundledPluginFile } from "joopo/plugin-sdk/test-fixtures";
...
import { collectInstalledRootDependencyManifestErrors } from "../scripts/joopo-npm-postpublish-verify.ts";
import {
...
describe("packed CLI smoke", () => {
  ...
  it("builds a packed CLI smoke env with packaged-install guardrails", () => {
    expect(
      createPackedCliSmokeEnv(
        {
          ...
          JOOPO_CONFIG_PATH: "/tmp/leaky-config.json",
        },
        { HOME: "/tmp/smoke-home", JOOPO_STATE_DIR: "/tmp/smoke-state" },
      ),
    ).toEqual({
      PATH:
      ...
      TMPDIR: "/tmp/original-tmp",
      SystemRoot: "C:\\Windows",
      JOOPO_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK: "1",
      JOOPO_NO_ONBOARD: "1",
      JOOPO_SERVICE_REPAIR_POLICY: "external",
      JOOPO_SUPPRESS_NOTES: "1",
      JOOPO_STATE_DIR: "/tmp/smoke-state",
    });
  });
  ...
  it("skips plugin command discovery during packed completion cache smoke", () => {
    expect(
      createPackedCompletionSmokeEnv(
        {
          PATH: "/usr/bin",
          JOOPO_COMPLETION_SKIP_PLUGIN_COMMANDS: "0",
        },
        {
          HOME: "/tmp/smoke-home",
          JOOPO_STATE_DIR: "/tmp/smoke-state",
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | joopo.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            joopo: {
              install: { npmSpec: "@joopo/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | joopo.install.minHostVersion must use a semver floor in the form \">=x.y.z[-prerelease][+build]\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | joopo.install must be an object"]);
  });
});
...
describe("bundled plugin package dependency checks", () => {
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "joopo-root-owned-installed-"));

    try {
      mkdirSync(join(tempRoot, "dist", "extensions", "memory-lancedb"), { recursive: true });
      writeFileSync(
        join(tempRoot, "package.json"),
        `{"name":"joopo","dependencies":{}}\n`,
        "utf8",
      );
      writeFileSync(
        join(tempRoot, "dist", "extensions", "memory-lancedb", "package.json"),
        `{"name":"@joopo/memory-lancedb","dependencies":{"root-owned-test-dep":"^1.0.0"}}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("still requires root deps for root-owned installed chunks", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "joopo-root-owned-installed-missing-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        "dist/index.js",
        "dist/extensions/browser/.Joopo-Install-Stage/package.json",
        "dist/extensions/codex/.joopo-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.joopo-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/browser/.Joopo-Install-Stage/package.json",
      "dist/extensions/codex/.joopo-runtime-deps-backup-node_modules-old/zod/index.js",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "joopo-release-private-qa-"));
  ...
  it("blocks private QA paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "joopo-release-inventory-"));
  ...
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    expect(missing).toEqual(
      expect.arrayContaining([
        ...
        bundledDistPluginFile("slack", "joopo.plugin.json"),
        bundledDistPluginFile("slack", "package.json"),
        bundledDistPluginFile("telegram", "runtime-api.js"),
        bundledDistPluginFile("telegram", "joopo.plugin.json"),
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("joopo-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("joopo-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "joopo-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "joopo-2026.3.14.tgz" },
        { filename: "joopo-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
