...
enum JoopoMLXTTSHelper {
    static func main() async {
        do {
            ...
            FileHandle.standardError.write(Data("joopo-mlx-tts: \(error)\n".utf8))
            exit(1)
        }
    }
    ...
    private enum Usage: Error, CustomStringConvertible {
        ...
        var description: String {
            switch self {
            case .requested:
                "usage: joopo-mlx-tts --text <text> --output <wav> [--model <hf-repo>] [--language <id>] [--voice <name>]"
