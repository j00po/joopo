...
final class WakeWordGateTests: XCTestCase {
    ...
    func testMatchHandlesMultiWordTriggers() {
        let transcript = "hey joopo do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey joopo"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    func testMatchPrefersMostSpecificTriggerWhenOverlapping() {
        ...
        let config = WakeWordGateConfig(triggers: ["joopo", "hey joopo"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
        XCTAssertEqual(match?.trigger, "hey joopo")
    }
