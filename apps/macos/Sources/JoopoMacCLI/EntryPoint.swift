...
struct JoopoMacCLI {
    static func main() async {
        ...
        default:
            fputs("joopo-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      joopo-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      joopo-mac discover [--timeout <ms>] [--json] [--include-local]
      joopo-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      joopo-mac connect
      joopo-mac connect --url ws://127.0.0.1:18789 --json
      joopo-mac discover --timeout 3000 --json
      joopo-mac wizard --mode local
    """)
}
