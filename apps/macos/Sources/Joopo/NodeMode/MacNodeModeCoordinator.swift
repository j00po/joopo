...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.joopo", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = JoopoConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: JoopoNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: JoopoLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        var caps: [String] = [JoopoCapability.canvas.rawValue, JoopoCapability.screen.rawValue]
        if browserControlEnabled, connectionMode == .local {
            caps.append(JoopoCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(JoopoCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(JoopoCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: JoopoConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: JoopoLocationMode(rawValue: rawLocationMode) ?? .off,
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            JoopoSystemCommand.which.rawValue,
            JoopoSystemCommand.run.rawValue,
            JoopoSystemCommand.execApprovalsGet.rawValue,
            JoopoSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(JoopoCapability.browser.rawValue) {
            commands.append(JoopoBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(JoopoCapability.camera.rawValue) {
        ...
        if capsSet.contains(JoopoCapability.location.rawValue) {
