...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case JoopoBrowserCommand.proxy.rawValue:
            ...
            case JoopoLocationCommand.get.rawValue:
            ...
            case JoopoSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case JoopoSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case JoopoSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case JoopoSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case JoopoSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case JoopoCanvasA2UICommand.push.rawValue,
             JoopoCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard JoopoConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: JoopoNodeError(
    ...
    private static func resolveA2UIHostUrl(from raw: String?) -> String? {
        ...
        return baseUrl.appendingPathComponent("__joopo__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(JoopoSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(JoopoSystemWhichParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> JoopoLocationMode {
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: JoopoNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: JoopoCanvasSnapshotFormat,
        maxWidth: Int?,
