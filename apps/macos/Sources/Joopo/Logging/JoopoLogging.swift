...
enum JoopoLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = JoopoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = JoopoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        JoopoLogging.bootstrapIfNeeded()
        let label = JoopoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct JoopoOSLogHandler: AppLogLevelBackedHandler {
...
struct JoopoFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = JoopoLogging.parseLabel(self.label)
