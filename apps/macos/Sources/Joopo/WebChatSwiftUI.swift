...
private let webChatSwiftLogger = Logger(subsystem: "ai.joopo", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "joopo.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: JoopoChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> JoopoChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(JoopoChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            JoopoChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return JoopoChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> JoopoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                JoopoGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    JoopoSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: JoopoProtocol.ModelChoice) -> JoopoChatModelChoice {
        JoopoChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<JoopoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any JoopoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = JoopoChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: JoopoChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<JoopoChatView>) -> NSViewController
    {
