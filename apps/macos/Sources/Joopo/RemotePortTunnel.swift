...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.joopo", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        guard let sshKey = JoopoConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = JoopoConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.joopo.remote.tunnel.port", qos: .utility)
            do {
