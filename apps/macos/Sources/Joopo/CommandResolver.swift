...
enum CommandResolver {
    private static let projectRootDefaultsKey = "joopo.gatewayProjectRootPath"
    private static let helperName = "joopo"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let joopoEntry = root.appendingPathComponent("joopo.mjs").path
        if FileManager().isReadableFile(atPath: joopoEntry) { return joopoEntry }
        let binEntry = root.appendingPathComponent("bin/joopo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/joopo")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let joopoPaths = self.joopoManagedPaths(home: home)
        if !joopoPaths.isEmpty {
            extras.insert(contentsOf: joopoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + joopoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func joopoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".joopo"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func joopoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectJoopoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("joopo.mjs").path,
            root.appendingPathComponent("bin/joopo.js").path,
        ]
    ...
    static func hasAnyJoopoInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.joopoExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let joopoPath = self.projectJoopoExecutable(projectRoot: root) {
            return [joopoPath, subcommand] + extraArgs
        }
        if let joopoPath = self.joopoExecutable(searchPaths: searchPaths) {
            return [joopoPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "joopo", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            joopo entrypoint missing (looked for dist/index.js or joopo.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real joopo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/joopo"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/joopo.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/joopo.mjs"
            node "$PRJ/joopo.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/joopo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/joopo.js"
            node "$PRJ/bin/joopo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent joopo"
          pnpm --silent joopo \(quotedArgs);
        else
          echo "joopo CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? JoopoConfigFile.loadDict()
