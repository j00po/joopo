...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        ...
        case .tokenRequired:
            "Paste the token configured on the gateway host. "
                + "On the gateway host, run `joopo config get gateway.auth.token`. "
                + "If the gateway uses an environment variable instead, use `JOOPO_GATEWAY_TOKEN`."
        case .tokenMismatch:
            "Check `gateway.auth.token` or `JOOPO_GATEWAY_TOKEN` on the gateway host and try again."
        case .gatewayTokenNotConfigured:
            "This gateway is set to token auth, but no `gateway.auth.token` is configured on the gateway host. "
                ...
                + "set `JOOPO_GATEWAY_TOKEN` before starting the gateway."
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired Joopo client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. "
                + "Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired Joopo client. "
                + "In your Joopo chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .tokenRequired, .gatewayTokenNotConfigured:
            "No token yet? Generate one on the gateway host with "
                + "`joopo doctor --generate-gateway-token`, then set it as `gateway.auth.token`."
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired Joopo client yet, "
                + "approve the pending request on the gateway host with `joopo devices approve`."
    ...
    var statusMessage: String {
        ...
        case .tokenMismatch:
            "Gateway token mismatch. Check gateway.auth.token or JOOPO_GATEWAY_TOKEN on the gateway host."
        case .gatewayTokenNotConfigured:
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired Joopo client, "
