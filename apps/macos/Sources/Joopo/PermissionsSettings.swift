    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so Joopo can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = JoopoLocationMode.off.rawValue
    ...
    private var locationMode: JoopoLocationMode {
        JoopoLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: JoopoLocationMode) async -> Bool {
