...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.joopo.mac`) even if the shipped
    ...
    {
        if environment["JOOPO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "joopo.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "joopo.nixMode") { return true }
