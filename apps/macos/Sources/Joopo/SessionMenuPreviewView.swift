...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.joopo", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> JoopoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: JoopoSessionPreviewEntry,
    ...
    private static func cache(payload: JoopoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: JoopoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [JoopoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [JoopoKit.AnyCodable]) -> [JoopoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(JoopoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: JoopoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: JoopoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: JoopoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: JoopoChatMessage) -> String? {
