...
enum DebugActions {
    private static let verboseDefaultsKey = "joopo.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "joopo.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = JoopoPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Joopo", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = JoopoPaths.configURL
        guard
