...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["JOOPO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/joopo")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("joopo-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("joopo-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/joopo/ (rolling or stdout), or nil if none exist.
