...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: JoopoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.joopo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: JoopoKit.AnyCodable]()) {
                $0[$1.key] = JoopoKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: JoopoProtocol.AnyCodable?) -> [String: JoopoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: JoopoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: JoopoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: JoopoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("joopo.control.heartbeat")
    static let controlAgentEvent = Notification.Name("joopo.control.agent")
}
