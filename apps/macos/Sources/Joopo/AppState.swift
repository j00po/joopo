...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = JoopoConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            JoopoConfigFile.canonicalHostForComparison(host) ==
                JoopoConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = JoopoConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: JoopoConfigFile.loadDict(),
            draft: .init(
        ...
        JoopoConfigFile.saveDict(synced.root)
    }
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/joopo"
        state.remoteCliPath = ""
        return state
    }
}
