...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["JOOPO_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "joopo-gateway",
                executablePath: "/tmp/joopo-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 joopo-gateway @ /tmp/joopo-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
