...
@Suite(.serialized)
struct JoopoConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("joopo-config-\(UUID().uuidString)")
            .appendingPathComponent("joopo.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["JOOPO_CONFIG_PATH": override]) {
            #expect(JoopoConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["JOOPO_CONFIG_PATH": override]) {
            ...
            #expect(JoopoConfigFile.remoteGatewayPort() == 19999)
            #expect(JoopoConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(JoopoConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(JoopoConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(JoopoConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(JoopoConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["JOOPO_CONFIG_PATH": override]) {
            ...
            JoopoConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = JoopoConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["JOOPO_CONFIG_PATH": override]) {
            ...
            JoopoConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = JoopoConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["JOOPO_CONFIG_PATH": override]) {
            ...
            JoopoConfigFile.clearRemoteGatewayUrl()
            let root = JoopoConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("joopo-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "JOOPO_CONFIG_PATH": nil,
            "JOOPO_STATE_DIR": dir,
        ]) {
            #expect(JoopoConfigFile.stateDirURL().path == dir)
            #expect(JoopoConfigFile.url().path == "\(dir)/joopo.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("joopo-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("joopo.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "JOOPO_STATE_DIR": stateDir.path,
            "JOOPO_CONFIG_PATH": configPath.path,
        ]) {
            JoopoConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-joopo-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = JoopoConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try JoopoConfigFile.withTestingFileLock {
                JoopoConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = JoopoConfigFile.loadDict()
