...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: JoopoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: JoopoProtocol.AnyCodable] = [
            "presence": .init([JoopoProtocol.AnyCodable(entry)]),
        ]
        let payload = JoopoProtocol.AnyCodable(payloadMap)
