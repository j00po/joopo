...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(JoopoCapability.browser.rawValue))
        #expect(!commands.contains(JoopoBrowserCommand.proxy.rawValue))
        #expect(commands.contains(JoopoCanvasCommand.present.rawValue))
