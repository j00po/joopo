...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["joopo"]
        let text = "uh joopo"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "joopo")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["joopo", "hey joopo"]
        let text = "hey joopo"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey joopo")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey joopo do thing"
        let config = WakeWordGateConfig(triggers: ["joopo"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["joopo"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "joopo")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "joopo tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["joopo", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["joopo", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "joopo")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["joopo"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "joopo", start: 0.2, duration: 0.1, range: transcript.range(of: "joopo")),
