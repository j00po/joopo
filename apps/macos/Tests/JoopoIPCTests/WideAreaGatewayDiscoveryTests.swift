...
import Foundation
import Testing
@testable import JoopoDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("JOOPO_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("JOOPO_WIDE_AREA_DOMAIN", "joopo.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("JOOPO_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("JOOPO_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._joopo-gw._tcp.joopo.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.joopo.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Joopo)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/joopo/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Joopo)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/joopo/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._joopo-gw._tcp.joopo.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
