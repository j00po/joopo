...
@MainActor
enum JoopoAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class JoopoAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.joopo.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.joopo.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.joopo.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? JoopoAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "joopo.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "joopo.watch.promptId"
    static let sessionKeyKey = "joopo.watch.sessionKey"
    static let actionPrimaryIDKey = "joopo.watch.action.primary.id"
    static let actionPrimaryLabelKey = "joopo.watch.action.primary.label"
    static let actionSecondaryIDKey = "joopo.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "joopo.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "joopo.watch.action.primary"
    static let actionSecondaryIdentifier = "joopo.watch.action.secondary"
    static let actionIdentifierPrefix = "joopo.watch.action."
    static let actionIDKeyPrefix = "joopo.watch.action.id."
    static let actionLabelKeyPrefix = "joopo.watch.action.label."
    static let categoryPrefix = "joopo.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: JoopoWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> JoopoWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return JoopoWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Joopo" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [JoopoWatchAction]) -> [UNNotificationAction] {
...
struct JoopoApp: App {
    ...
    @UIApplicationDelegateAdaptor(JoopoAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        JoopoAppModelRegistry.appModel = appModel
...
extension JoopoApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Joopo: installing uncaught exception handler")
