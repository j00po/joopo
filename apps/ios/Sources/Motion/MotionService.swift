...
final class MotionService: MotionServicing {
    func activities(params: JoopoMotionActivityParams) async throws -> JoopoMotionActivityPayload {
        ...
        let mapped: [JoopoMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return JoopoMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = JoopoPedometerPayload(
