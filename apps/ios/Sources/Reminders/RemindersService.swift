...
final class RemindersService: RemindersServicing {
    func list(params: JoopoRemindersListParams) async throws -> JoopoRemindersListPayload {
        ...
        let payload: [JoopoReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return JoopoRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: JoopoRemindersAddParams) async throws -> JoopoRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = JoopoReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return JoopoRemindersAddPayload(reminder: payload)
    }
