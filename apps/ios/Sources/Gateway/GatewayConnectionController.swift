...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [JoopoCapability.canvas.rawValue, JoopoCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(JoopoCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(JoopoCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = JoopoLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(JoopoCapability.location.rawValue) }

        caps.append(JoopoCapability.device.rawValue)
        caps.append(JoopoCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(JoopoCapability.watch.rawValue)
        }
        caps.append(JoopoCapability.photos.rawValue)
        caps.append(JoopoCapability.contacts.rawValue)
        caps.append(JoopoCapability.calendar.rawValue)
        caps.append(JoopoCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(JoopoCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            JoopoScreenCommand.record.rawValue,
            JoopoSystemCommand.notify.rawValue,
            JoopoChatCommand.push.rawValue,
        ...
        if caps.contains(JoopoCapability.camera.rawValue) {
            commands.append(JoopoCameraCommand.list.rawValue)
            commands.append(JoopoCameraCommand.snap.rawValue)
            commands.append(JoopoCameraCommand.clip.rawValue)
        }
        if caps.contains(JoopoCapability.location.rawValue) {
            commands.append(JoopoLocationCommand.get.rawValue)
        }
        if caps.contains(JoopoCapability.device.rawValue) {
            commands.append(JoopoDeviceCommand.status.rawValue)
            commands.append(JoopoDeviceCommand.info.rawValue)
        }
        if caps.contains(JoopoCapability.watch.rawValue) {
            commands.append(JoopoWatchCommand.status.rawValue)
            commands.append(JoopoWatchCommand.notify.rawValue)
        }
        if caps.contains(JoopoCapability.photos.rawValue) {
            commands.append(JoopoPhotosCommand.latest.rawValue)
        }
        if caps.contains(JoopoCapability.contacts.rawValue) {
            commands.append(JoopoContactsCommand.search.rawValue)
            commands.append(JoopoContactsCommand.add.rawValue)
        }
        if caps.contains(JoopoCapability.calendar.rawValue) {
            commands.append(JoopoCalendarCommand.events.rawValue)
            commands.append(JoopoCalendarCommand.add.rawValue)
        }
        if caps.contains(JoopoCapability.reminders.rawValue) {
            commands.append(JoopoRemindersCommand.list.rawValue)
            commands.append(JoopoRemindersCommand.add.rawValue)
        }
        if caps.contains(JoopoCapability.motion.rawValue) {
            commands.append(JoopoMotionCommand.activity.rawValue)
            commands.append(JoopoMotionCommand.pedometer.rawValue)
        }
