...
enum WatchMessagingPayloadCodec {
    ...
    static func encodeNotificationPayload(
        id: String,
        params: JoopoWatchNotifyParams) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": JoopoWatchPayloadType.notify.rawValue,
            "id": id,
            "title": params.title,
            "body": params.body,
            "priority": params.priority?.rawValue ?? JoopoNotificationPriority.active.rawValue,
            "sentAtMs": self.nowMs(),
        ]
    ...
    static func encodeExecApprovalItem(_ item: JoopoWatchExecApprovalItem) -> [String: Any] {
        var payload: [String: Any] = [
            "id": item.id,
    ...
    static func encodeExecApprovalPromptPayload(
        _ message: JoopoWatchExecApprovalPromptMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": JoopoWatchPayloadType.execApprovalPrompt.rawValue,
    ...
    static func encodeExecApprovalResolvedPayload(
        _ message: JoopoWatchExecApprovalResolvedMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": JoopoWatchPayloadType.execApprovalResolved.rawValue,
    ...
    static func encodeExecApprovalExpiredPayload(
        _ message: JoopoWatchExecApprovalExpiredMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": JoopoWatchPayloadType.execApprovalExpired.rawValue,
            "approvalId": message.approvalId,
    ...
    static func encodeExecApprovalSnapshotPayload(
        _ message: JoopoWatchExecApprovalSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": JoopoWatchPayloadType.execApprovalSnapshot.rawValue,
    ...
    {
        guard (payload["type"] as? String) == JoopoWatchPayloadType.reply.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == JoopoWatchPayloadType.execApprovalResolve.rawValue else {
            return nil
        }
        guard let approvalId = nonEmpty(payload["approvalId"] as? String),
              let rawDecision = nonEmpty(payload["decision"] as? String),
              let decision = JoopoWatchExecApprovalDecision(rawValue: rawDecision)
        else {
            return nil
        }
        let replyId = self.nonEmpty(payload["replyId"] as? String) ?? UUID().uuidString
    ...
    {
        guard (payload["type"] as? String) == JoopoWatchPayloadType.execApprovalSnapshotRequest.rawValue else {
            return nil
        }
