...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "joopo.exec-approval"
    static let reviewActionIdentifier = "joopo.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.joopoPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.joopoPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func joopoPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
