...
struct IOSGatewayChatTransport: JoopoChatTransport {
    private static let logger = Logger(subsystem: "ai.joopo", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> JoopoChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(JoopoChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> JoopoChatHistoryPayload {
        ...
        return try JSONDecoder().decode(JoopoChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [JoopoChatAttachmentPayload]) async throws -> JoopoChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [JoopoChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(JoopoChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(JoopoGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: JoopoGatewayHealthOK.self))?.ok ?? true
