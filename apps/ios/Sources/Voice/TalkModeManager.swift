...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__JOOPO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<JoopoTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.joopo", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = JoopoTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> JoopoTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return JoopoTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> JoopoTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> JoopoTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return JoopoTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> JoopoTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = JoopoTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: JoopoTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    guard let chatEvent = try? GatewayPayloadDecoding.decode(
                        payload,
                        as: JoopoChatEventPayload.self)
                    else {
                        continue
                    }
                    guard chatEvent.runId == runId else { continue }
                    if let text = JoopoChatEventText.assistantText(from: chatEvent) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: JoopoAgentEventPayload.self)
            else {
                continue
            }
