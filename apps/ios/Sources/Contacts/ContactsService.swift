...
final class ContactsService: ContactsServicing {
    ...
    func search(params: JoopoContactsSearchParams) async throws -> JoopoContactsSearchPayload {
        ...
        return JoopoContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: JoopoContactsAddParams) async throws -> JoopoContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return JoopoContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return JoopoContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> JoopoContactPayload {
        JoopoContactPayload(
