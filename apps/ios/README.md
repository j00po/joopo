# Joopo iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Joopo Gateway as a `role: node`.
...
- Beta release uses canonical `ai.joopo.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `JoopoPushTransport=relay`, `JoopoPushDistribution=official`, and `JoopoPushAPNsEnvironment=production`.
...
- `JOOPO_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export JOOPO_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Joopo-<version>.ipa`
   - `apps/ios/build/beta/Joopo-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Joopo.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `JoopoPushTransport=direct` and `JoopoPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `JOOPO_APNS_TEAM_ID`, `JOOPO_APNS_KEY_ID`, and either `JOOPO_APNS_PRIVATE_KEY_P8` or `JOOPO_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.joopo/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `JOOPO_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `JoopoPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `joopo.json`. `JOOPO_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Joopo official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Joopo node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
