...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.joopo.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Joopo", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Joopo gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "JoopoShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Joopo is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("joopo-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
