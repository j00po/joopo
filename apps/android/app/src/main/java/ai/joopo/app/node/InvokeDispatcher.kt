...
import ai.joopo.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    return when (command) {
      ...
      JoopoCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      JoopoCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      JoopoCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      JoopoCanvasA2UICommand.Push.rawValue, JoopoCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      JoopoCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      JoopoCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      JoopoCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      JoopoLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      JoopoDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      JoopoDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      JoopoDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      JoopoDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      JoopoNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      JoopoNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      JoopoSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      JoopoTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      JoopoTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      JoopoTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      JoopoTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.joopo.app.protocol.JoopoPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      JoopoContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      JoopoContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      JoopoCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      JoopoCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      JoopoMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      JoopoMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      JoopoSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      JoopoSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      JoopoCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
