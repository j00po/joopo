...
private const val NOTIFICATION_CHANNEL_BASE_ID = "joopo.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Joopo Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Joopo Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Joopo Active")
      }
