...
internal fun joopoAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Joopo Android gateway connection failure.
    ...
    - tell me whether `joopo devices list` should show a pending pairing request
    - if more signal is needed, ask for `joopo qr --json`, `joopo devices list`, and `joopo nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${joopoAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Joopo gateway diagnostics", report))
