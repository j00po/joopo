...
val androidStoreFile = providers.gradleProperty("JOOPO_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("JOOPO_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("JOOPO_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("JOOPO_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set JOOPO_ANDROID_STORE_FILE, " +
      "JOOPO_ANDROID_STORE_PASSWORD, JOOPO_ANDROID_KEY_ALIAS, and " +
      "JOOPO_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/JoopoKit/Sources/JoopoKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.joopo.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "joopo-$versionName-$buildType.apk"
          } else {
            "joopo-$versionName-$flavorName-$buildType.apk"
          }
