...
@MainActor
public struct JoopoChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: JoopoChatViewModel
    ...
    private var visibleMessages: [JoopoChatMessage] {
        let base: [JoopoChatMessage]
    ...
    private func mergeToolResults(in messages: [JoopoChatMessage]) -> [JoopoChatMessage] {
    ...
    private func isToolResultMessage(_ message: JoopoChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: JoopoChatMessage) -> Bool {
    ...
    private func primaryText(in message: JoopoChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: JoopoChatMessage) -> Bool {
    ...
    private func toolCalls(in message: JoopoChatMessage) -> [JoopoChatMessageContent] {
    ...
    private func inlineToolResults(in message: JoopoChatMessage) -> [JoopoChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: JoopoChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: JoopoChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(JoopoChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
