...
struct ChatMessageBubble: View {
    let message: JoopoChatMessage
    let style: JoopoChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? JoopoChatTheme.userText : JoopoChatTheme.assistantText
    ...
    private var inlineAttachments: [JoopoChatMessageContent] {
    ...
    private var toolCalls: [JoopoChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [JoopoChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? JoopoChatTheme.userBubble
        }
        if self.style == .onboarding {
            return JoopoChatTheme.onboardingAssistantBubble
        }
        return JoopoChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return JoopoChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: JoopoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? JoopoChatTheme.userText : JoopoChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: JoopoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(JoopoChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [JoopoChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: JoopoChatTheme.assistantText)
            }
        }
    }
}
