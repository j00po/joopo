...
private let chatUILogger = Logger(subsystem: "ai.joopo", category: "JoopoChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class JoopoChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [JoopoChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [JoopoChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [JoopoChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [JoopoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [JoopoChatPendingToolCall] = []
    public private(set) var sessions: [JoopoChatSessionEntry] = []
    private let transport: any JoopoChatTransport
    private var sessionDefaults: JoopoChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: JoopoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any JoopoChatTransport,
    ...
    public var sessionChoices: [JoopoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [JoopoChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [JoopoChatThinkingLevelOption] = [
        JoopoChatThinkingLevelOption(id: "off", label: "off"),
        JoopoChatThinkingLevelOption(id: "minimal", label: "minimal"),
        JoopoChatThinkingLevelOption(id: "low", label: "low"),
        JoopoChatThinkingLevelOption(id: "medium", label: "medium"),
        JoopoChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: JoopoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [JoopoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: JoopoChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: JoopoChatMessage) -> JoopoChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> JoopoChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return JoopoChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: JoopoChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: JoopoChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: JoopoChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [JoopoChatMessage],
        incoming: [JoopoChatMessage]) -> [JoopoChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [JoopoChatMessage]) -> [JoopoChatMessage] {
        var result: [JoopoChatMessage] = []
    ...
    private static func dedupeKey(for message: JoopoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [JoopoChatMessageContent] = [
            JoopoChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> JoopoChatAttachmentPayload in
            JoopoChatAttachmentPayload(
                type: att.type,
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: JoopoChatSessionEntry?) -> [JoopoChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: JoopoChatSessionEntry,
        defaults: JoopoChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [JoopoChatThinkingLevelOption]?) -> [JoopoChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return JoopoChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [JoopoChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return JoopoChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [JoopoChatThinkingLevelOption],
        current: String) -> [JoopoChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [JoopoChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [JoopoChatThinkingLevelOption]) -> [JoopoChatThinkingLevelOption]
    {
        var result: [JoopoChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> JoopoChatSessionEntry {
        JoopoChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = JoopoChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: JoopoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: JoopoSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: JoopoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: JoopoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = JoopoChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> JoopoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
