...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> JoopoChatHistoryPayload
{
    JoopoChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> JoopoChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> JoopoChatThinkingLevelOption {
    JoopoChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> JoopoChatSessionEntry
{
    JoopoChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> JoopoChatModelChoice {
    JoopoChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [JoopoChatHistoryPayload],
    sessionsResponses: [JoopoChatSessionsListResponse] = [],
    modelResponses: [[JoopoChatModelChoice]] = [],
    ...
    -> (TestChatTransport, JoopoChatViewModel)
{
...
private func sendUserMessage(_ vm: JoopoChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, JoopoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [JoopoChatHistoryPayload]
    private let sessionsResponses: [JoopoChatSessionsListResponse]
    private let modelResponses: [[JoopoChatModelChoice]]
    ...
    private let stream: AsyncStream<JoopoChatTransportEvent>
    private let continuation: AsyncStream<JoopoChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<JoopoChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> JoopoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? JoopoChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [JoopoChatAttachmentPayload]) async throws -> JoopoChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return JoopoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> JoopoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? JoopoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: JoopoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func appendsExternalSessionUserMessageForActiveSession() async throws {
        ...
        transport.emit(
            .sessionMessage(
                JoopoSessionMessageEventPayload(
                    sessionKey: "agent:main:main",
                    message: JoopoChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = JoopoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = JoopoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: JoopoChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(JoopoChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == JoopoChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = JoopoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: JoopoChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = JoopoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = JoopoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = JoopoChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func decodesGatewayThinkingMetadataFromSessionList() throws {
        ...
        let decoded = try JSONDecoder().decode(
            JoopoChatSessionsListResponse.self,
            from: Data(json.utf8))
...
"""]],
        ...
        let vm = await MainActor.run { JoopoChatViewModel(sessionKey: "main", transport: transport) }
