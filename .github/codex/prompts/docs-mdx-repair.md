# Joopo Docs MDX Repair Agent

You are repairing generated Joopo documentation after a fast MDX validation failure.
...
1. Read `.joopo-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".joopo-sync/mdx/${LOCALE}.json"`.
