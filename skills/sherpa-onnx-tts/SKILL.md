...
1. Download the runtime for your OS (extracts into `$JOOPO_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.joopo/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$JOOPO_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.joopo/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Joopo config file (`$JOOPO_CONFIG_PATH`, default `~/.joopo/joopo.json`):
