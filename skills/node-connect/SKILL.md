---
name: node-connect
description: Diagnose Joopo Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Joopo is advertising that route, then fix pairing/auth.
...
- whether `joopo devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `joopo qr --json`. It uses the same setup-code payload Android scans.

```bash
joopo config get gateway.mode
joopo config get gateway.bind
joopo config get gateway.tailscale.mode
joopo config get gateway.remote.url
joopo config get gateway.auth.mode
joopo config get gateway.auth.allowTailscale
joopo config get plugins.entries.device-pair.config.publicUrl
joopo qr --json
joopo devices list
joopo nodes status
```

If this Joopo instance is pointed at a remote gateway, also run:

```bash
joopo qr --remote --json
```
...
If `joopo qr --json` says `Gateway is only bound to loopback`:
...
- `joopo devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run joopo qr again, rescan, then approve the pending device pairing.`
