---
name: healthcheck
description: Audit and harden hosts running Joopo for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# Joopo Host Hardening

## Overview

Assess and harden the host running Joopo, then align it to a user-defined risk tolerance without breaking access. Use Joopo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Joopo changes the host firewall, SSH, or OS updates; it does not.
...
5. Joopo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Joopo can access sensitive data.
...
### 2) Run Joopo security audits (read-only)

As part of the default read-only checks, run `joopo security audit --deep`. Only offer alternatives if the user requests them:

1. `joopo security audit` (faster, non-probing)
2. `joopo security audit --json` (structured output)

Offer to apply Joopo safe defaults (numbered):

1. `joopo security audit --fix`

Be explicit that `--fix` only tightens Joopo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Joopo version/update status (read-only)

As part of the default read-only checks, run `joopo update status`.
...
- Credential hygiene notes (location of Joopo creds, prefer disk encryption)
...
After Joopo install or first hardening pass, run at least one baseline audit and version check:

- `joopo security audit`
- `joopo security audit --deep`
- `joopo update status`

Ongoing monitoring is recommended. Use the Joopo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `joopo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `joopo update status`
...
Before creating, `joopo cron list` and match on exact `name`. If found, `joopo cron edit <id> ...`.
If not found, `joopo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `joopo update status` (preferred for source checkouts and channels)
2. `npm view joopo version` (published npm version)

## Joopo command accuracy

Use only supported commands and flags:

- `joopo security audit [--deep] [--fix] [--json]`
- `joopo status` / `joopo status --deep`
- `joopo health --json`
- `joopo update status`
- `joopo cron add|list|runs|run`

Do not invent CLI flags or imply Joopo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Joopo compaction:
