...
export function shouldEnableJoopoCompileCache(params: {
...
export function resolveJoopoCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
export type JoopoCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
};

type JoopoCompileCacheRespawnRuntime = {
  spawn: typeof spawn;
...
export function buildJoopoCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
...
}): JoopoCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
  if (!isSourceCheckoutInstallRoot(params.installRoot)) {
    return undefined;
  }
  if (env.JOOPO_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return undefined;
  }
...
export function respawnWithoutJoopoCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runJoopoCompileCacheRespawnPlan(plan);
  return true;
}

export function runJoopoCompileCacheRespawnPlan(
  plan: JoopoCompileCacheRespawnPlan,
  runtime: JoopoCompileCacheRespawnRuntime = {
    spawn,
  ...
  child.once("error", (error) => {
    clearSignalExitTimer();
    runtime.writeError(
      `[joopo] Failed to respawn CLI without compile cache: ${
...
export function enableJoopoCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableJoopoCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveJoopoCompileCacheDirectory(params));
  } catch {
