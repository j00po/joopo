...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses JOOPO_HOME for tilde expansion", () => {
    vi.stubEnv("JOOPO_HOME", "/srv/joopo-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/joopo-home", "cron", "jobs.json"));
  });
});
