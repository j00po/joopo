...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["JOOPO_CONTAINER_HINT", "JOOPO_PROFILE"]);
    delete process.env.JOOPO_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:joopo|joopo) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
