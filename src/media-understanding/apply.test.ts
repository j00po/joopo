...
const TEMP_MEDIA_PREFIX = "joopo-media-";
...
function createGroqAudioConfig(): JoopoConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): JoopoConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): JoopoConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: JoopoConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredJoopoTmpDir();
