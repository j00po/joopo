...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-tts-status-"));
});
...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  process.env.JOOPO_STATE_DIR = path.join(home, ".joopo");
  try {
    await run(home);
  } finally {
    restoreEnv("HOME", previousHome);
    restoreEnv("USERPROFILE", previousUserProfile);
    restoreEnv("JOOPO_HOME", previousJoopoHome);
    restoreEnv("JOOPO_STATE_DIR", previousStateDir);
  }
}
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".joopo", "settings", "tts.json");
  ...
  it("derives the default prefs path from JOOPO_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".joopo-dev");
      ...
      vi.stubEnv("JOOPO_CONFIG_PATH", path.join(stateDir, "joopo.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
