...
export function hasConfiguredInternalHooks(config: JoopoConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: JoopoConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
