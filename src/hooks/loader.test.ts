...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["JOOPO_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    it("detects configured internal hook surfaces", () => {
      expect(hasConfiguredInternalHooks({} satisfies JoopoConfig)).toBe(false);
      expect(
    ...
    const expectNoCommandHookRegistration = async (cfg: JoopoConfig) => {
    ...
    it("skips hook discovery until internal hooks are configured", async () => {
      for (const cfg of [
        {} satisfies JoopoConfig,
        { hooks: {} } satisfies JoopoConfig,
        { hooks: { internal: {} } } satisfies JoopoConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
