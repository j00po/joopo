...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "joopo-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "joopo-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "joopo-bundled");
      const plugin = makeHookEntry("shared", "joopo-plugin");
      const managed = makeHookEntry("shared", "joopo-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("joopo-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "joopo-managed");
      const workspace = makeHookEntry("shared", "joopo-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "joopo-workspace");
      const second = makeHookEntry("shared", "joopo-workspace");
      second.hook.handlerPath = "/tmp/joopo-workspace/shared/handler-2.js";
