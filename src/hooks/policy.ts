import type { JoopoConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "joopo-bundled": {
    ...
    canOverride: ["joopo-bundled"],
    canBeOverriddenBy: ["joopo-managed", "joopo-plugin"],
  },
  "joopo-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["joopo-bundled", "joopo-plugin"],
    canBeOverriddenBy: ["joopo-managed"],
  },
  "joopo-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["joopo-bundled", "joopo-managed", "joopo-plugin"],
    canBeOverriddenBy: ["joopo-managed"],
  },
  "joopo-workspace": {
    precedence: 40,
    ...
    canOverride: ["joopo-workspace"],
    canBeOverriddenBy: ["joopo-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "joopo-plugin") {
    return { enabled: true };
  }
