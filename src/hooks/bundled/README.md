...
This directory contains hooks that ship with Joopo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.joopo/workspace`)
...
homepage: https://docs.joopo.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.joopo/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.joopo/joopo.json`:
...
2. Restart gateway: `pkill -9 -f 'joopo.*gateway' && pnpm joopo gateway`
3. Enable the hook: `joopo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.joopo.ai/automation/hooks
