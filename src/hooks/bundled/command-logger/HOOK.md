...
homepage: https://docs.joopo.ai/automation/hooks#command-logger
metadata:
  {
    "joopo":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.joopo/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.joopo/logs/commands.log ~/.joopo/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/joopo`:
   ```
   /home/username/.joopo/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
