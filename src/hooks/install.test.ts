...
const fixtureRoot = path.join(process.cwd(), ".tmp", `joopo-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@joopo/test-hooks",
      version: "0.0.1",
      joopo: { hooks: params.hooks },
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    joopo: { hooks: [`./hooks/${params.hookName}`] },
  });
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "joopo.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "joopo.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@joopo/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@joopo/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@joopo/test-hooks@0.0.2-beta.1",
      name: "@joopo/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@joopo/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@joopo/test-hooks@beta"');
    }
  });
});
