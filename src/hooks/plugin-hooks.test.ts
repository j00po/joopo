...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "joopo-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.JOOPO_BUNDLED_HOOKS_DIR;
    process.env.JOOPO_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.JOOPO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.JOOPO_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".joopo", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): JoopoConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("joopo-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Joopo hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".joopo", "extensions", "claude-bundle");
