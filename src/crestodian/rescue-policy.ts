...
function resolveAgentEntry(cfg: JoopoConfig, agentId?: string) {
  if (!agentId) {
    return undefined;
  }
...
function resolveScopedExecConfig(cfg: JoopoConfig, agentId?: string) {
...
function isYoloHostPosture(cfg: JoopoConfig, agentId?: string): boolean {
...
export function resolveCrestodianRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Crestodian rescue is blocked because Joopo sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  ...
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Crestodian rescue only accepts commands from an Joopo owner.",
    };
  }
