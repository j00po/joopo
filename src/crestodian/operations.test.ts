...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/joopo.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.joopo.ai",
      sourceUrl: "https://github.com/joopo/joopo",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @joopo/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@joopo/demo",
    });
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/joopo.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "joopo-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expect(plan).toMatchObject({
      applied: false,
      message: "Plan: uninstall plugin joopo-demo. Say yes to apply.",
    });
    expect(runPluginUninstall).not.toHaveBeenCalled();

    await expect(
      executeCrestodianOperation({ kind: "plugin-uninstall", pluginId: "joopo-demo" }, runtime, {
        approved: true,
        deps: { runPluginUninstall },
        auditDetails: { rescue: true },
      }),
    ...
    expect(runPluginUninstall).toHaveBeenCalledWith("joopo-demo", expect.any(Object));
    expect(lines.join("\n")).toContain("[crestodian] done: plugin.uninstall");
    const auditPath = path.join(tempDir, "audit", "crestodian.jsonl");
    const audit = JSON.parse((await fs.readFile(auditPath, "utf8")).trim());
    expect(audit).toMatchObject({
      operation: "plugin.uninstall",
      summary: "Uninstalled plugin joopo-demo",
      details: {
        rescue: true,
        pluginId: "joopo-demo",
      },
