...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => JoopoConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): JoopoConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): JoopoConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
