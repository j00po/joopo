...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, Joopo's ring-zero setup helper.",
  "Turn the user's request into exactly one safe Joopo Crestodian command.",
  ...
  "Use the provided Joopo docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `Joopo docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `Joopo source: ${
