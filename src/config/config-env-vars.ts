...
function collectConfigEnvVarsByTarget(cfg?: JoopoConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: JoopoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: JoopoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.JOOPO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
