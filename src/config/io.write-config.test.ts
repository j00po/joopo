...
import type { ConfigFileSnapshot, JoopoConfig } from "./types.joopo.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "joopo-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { JOOPO_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("logs health-state write failures through public config reads", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      const healthPath = path.join(home, ".joopo", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      const pluginDir = path.join(home, ".joopo", "plugins", "demo");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      const pluginDir = path.join(home, ".joopo", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      const unwritableStatePath = path.join(home, ".joopo");
  ...
  it("keeps writes inside an JOOPO_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".joopo", "joopo.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "joopo.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "joopo.json"), "utf-8"),
      ) as {
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          JOOPO_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            $schema: "https://joopo.ai/config.json",
            gateway: { mode: "local" },
          },
          null,
          2,
      ...
      expect(persisted.$schema).toBe("https://joopo.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/joopo-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as JoopoConfig;
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      const includePath = path.join(home, ".joopo", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://joopo.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/joopo-test-required-plugin",
          source: "/tmp/joopo-test-required-plugin/index.ts",
          manifestPath: "/tmp/joopo-test-required-plugin/joopo.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.JOOPO_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
  ...
  it("notifies in-process reloaders with resolved source config when persisted env refs are restored", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      const previousConfigPath = process.env.JOOPO_CONFIG_PATH;
      const previousGatewayToken = process.env.JOOPO_GATEWAY_TOKEN;
      process.env.JOOPO_CONFIG_PATH = configPath;
      process.env.JOOPO_GATEWAY_TOKEN = "gateway-token-runtime";
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${JOOPO_GATEWAY_TOKEN}" },
            },
      ...
      try {
        ...
        if (previousGatewayToken === undefined) {
          ...
          process.env.JOOPO_GATEWAY_TOKEN = previousGatewayToken;
        }
      }
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/joopo-test-demo",
          source: "/tmp/joopo-test-demo/index.ts",
          manifestPath: "/tmp/joopo-test-demo/joopo.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        const postWriteSnapshot = await createConfigIO({
          env: { JOOPO_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
          homedir: () => home,
          logger: silentLogger,
  ...
  it("skipPluginValidation bypasses plugin schema rejection on writeConfigFile (#76800)", async () => {
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/joopo-test-strict-plugin",
            source: "/tmp/joopo-test-strict-plugin/index.ts",
            manifestPath: "/tmp/joopo-test-strict-plugin/joopo.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await expect(
          writeConfigFile({ agents: { list: "not-array" } } as unknown as JoopoConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/joopo-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "joopo-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as JoopoConfig;
      expect(persisted.logging?.file).toBe("~/joopo-upgrade-survivor/gateway.jsonl");
