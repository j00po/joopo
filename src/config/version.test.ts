...
import {
  compareJoopoVersions,
  isSameJoopoStableFamily,
  parseJoopoVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseJoopoVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseJoopoVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseJoopoVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseJoopoVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseJoopoVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseJoopoVersion("2026.3")).toBeNull();
    expect(parseJoopoVersion("latest")).toBeNull();
  });
});

describe("compareJoopoVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareJoopoVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareJoopoVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareJoopoVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareJoopoVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareJoopoVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareJoopoVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameJoopoStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameJoopoStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameJoopoStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameJoopoStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameJoopoStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
