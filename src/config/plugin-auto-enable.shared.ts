...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: JoopoConfig): boolean {
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: JoopoConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: JoopoConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: JoopoConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: JoopoConfig, plugin: PluginManifestRecord): boolean {
...
function collectConfiguredChannelIds(cfg: JoopoConfig, env: NodeJS.ProcessEnv): string[] {
...
function hasConfiguredWebSearchPluginEntry(cfg: JoopoConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: JoopoConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: JoopoConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: JoopoConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: JoopoConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: JoopoConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: JoopoConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: JoopoConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: JoopoConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: JoopoConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: JoopoConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: JoopoConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: JoopoConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: JoopoConfig): boolean {
  return (
...
function hasPluginEntries(cfg: JoopoConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: JoopoConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: JoopoConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: JoopoConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: JoopoConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: JoopoConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: JoopoConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
