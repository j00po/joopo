...
import type { ConfigFileSnapshot } from "./types.joopo.js";
...
describe("redactConfigSnapshot", () => {
          ...
          plugins: [
            {
              ...
              manifestPath: "/private/plugin/root/joopo.plugin.json",
            },
          ],
          diagnostics: [],
        },
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/joopo/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/joopo/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/joopo/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/joopo/nickserv-password.txt");
