...
import type { JoopoConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function coerceConfig(value: unknown): JoopoConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as JoopoConfig;
}
...
function stampConfigVersion(cfg: JoopoConfig): JoopoConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: JoopoConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      `Config was last written by a newer Joopo (${touched}); current version is ${VERSION}. ` +
        `If the gateway is healthy, check for CLI/runtime skew from stale PATH or global wrappers: ` +
        `command -v joopo; readlink -f "$(command -v joopo)"; joopo --version; joopo gateway status --deep.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as JoopoConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: JoopoConfig): JoopoConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run joopo plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function loadConfig(): JoopoConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as JoopoConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
      ...
      const loadValidationPluginMetadataSnapshot = (config: JoopoConfig) => {
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    let fallbackSourceConfig: JoopoConfig = {};
  ...
  async function readBestEffortConfig(): Promise<JoopoConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<JoopoConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
  ...
  ): Promise<{ persistedHash: string; persistedConfig: JoopoConfig }> {
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as JoopoConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as JoopoConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as JoopoConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.JOOPO_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.JOOPO_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `JOOPO_CONFIG_PATH` (and friends) are expected to work even
...
export function projectConfigOntoRuntimeSourceSnapshot(config: JoopoConfig): JoopoConfig {
...
export function loadConfig(): JoopoConfig {
  ...
  // reparsing joopo.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): JoopoConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<JoopoConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<JoopoConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: JoopoConfig = nextCfg;
  try {
