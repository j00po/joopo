...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.joopo/joopo.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".joopo", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.joopo/joopo.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".joopo", "joopo.json"));
    });
  });

  it("uses JOOPO_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { JOOPO_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".joopo", "joopo.json"));
    });
  });

  it("honors explicit JOOPO_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".joopo", 20002, "custom.json");
      const io = createIoForHome(home, { JOOPO_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("hints at stale wrappers when config was written by a newer Joopo", async () => {
    await withTempHome(async (home) => {
      const configPath = path.join(home, ".joopo", "joopo.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
      ...
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("command -v joopo"));
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("joopo --version"));
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("joopo gateway status --deep"),
      );
    });
  });
