...
import { withTempHome as withTempHomeBase } from "joopo/plugin-sdk/test-env";
import { resetPluginLoaderTestStateForTest } from "../plugins/loader.test-fixtures.js";
import { clearPluginSetupRegistryCache } from "../plugins/setup-registry.js";
import { resetConfigRuntimeState, type JoopoConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        JOOPO_PLUGIN_CATALOG_PATHS: undefined,
        JOOPO_MPM_CATALOG_PATHS: undefined,
        JOOPO_LOAD_SHELL_ENV: undefined,
        JOOPO_DEFER_SHELL_ENV_FALLBACK: undefined,
        JOOPO_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeJoopoConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.JOOPO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected JOOPO_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeJoopoConfig(home, config);
