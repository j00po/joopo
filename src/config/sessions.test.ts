...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ JOOPO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-joopo",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-joopo#general");
  });
  ...
  it("derives session transcripts dir from JOOPO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { JOOPO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when JOOPO_STATE_DIR differs from stored paths", () => {
