...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "joopo-web-search": {
                source: "npm",
                spec: "@ollama/joopo-web-search",
                installPath: "/tmp/joopo-web-search",
                resolvedName: "@ollama/joopo-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "joopo-web-search": {
                source: "npm",
                spec: "@ollama/joopo-web-search@0.2.2",
                installPath: "/tmp/joopo-web-search",
                resolvedName: "@ollama/joopo-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("joopo config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('joopo config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/joopo.json",
        expectedConfigPath: "/tmp/joopo.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: JoopoConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://joopo.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
