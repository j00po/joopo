...
export const ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV =
  "JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS";
...
type FutureConfigGuardParams = {
  ...
  config?: Pick<JoopoConfig, "meta"> | null;
  currentVersion?: string;
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this Joopo binary (${currentVersion}) is older than the config last written by Joopo ${touchedVersion}.`,
    hints: [
      "Run the newer joopo binary on PATH, or reinstall the intended gateway service from the newer install.",
