...
/**
 * Nix mode detection: When JOOPO_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.JOOPO_NIX_MODE === "1";
}

export const isNixMode = resolveIsNixMode();

// Support the remaining legacy pre-rebrand state dir.
const LEGACY_STATE_DIRNAMES = [".joopobot"] as const;
const NEW_STATE_DIRNAME = ".joopo";
const CONFIG_FILENAME = "joopo.json";
const LEGACY_CONFIG_FILENAMES = ["joopobot.json"] as const;

function resolveDefaultHomeDir(): string {
  return resolveRequiredHomeDir(process.env, os.homedir);
}

/** Build a homedir thunk that respects JOOPO_HOME for the given env. */
 ...
 * Can be overridden via JOOPO_STATE_DIR.
 * Default: ~/.joopo
 */
export function resolveStateDir(
  ...
  const override = env.JOOPO_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.JOOPO_TEST_FAST === "1") {
    return newDir;
  }
 ...
 * outside the config directory. Set via `JOOPO_INCLUDE_ROOTS` as a
 ...
 * directory containing `joopo.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.JOOPO_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via JOOPO_CONFIG_PATH.
 * Default: ~/.joopo/joopo.json (or $JOOPO_STATE_DIR/joopo.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.JOOPO_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.JOOPO_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.JOOPO_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const joopoStateDir = env.JOOPO_STATE_DIR?.trim();
  if (joopoStateDir) {
    const resolved = resolveUserPath(joopoStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/joopo-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `joopo-${uid}` : "joopo";
 ...
 *
 * Precedence:
 * - `JOOPO_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.JOOPO_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.JOOPO_GATEWAY_PORT?.trim();
