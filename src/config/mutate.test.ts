...
import type { ConfigFileSnapshot, JoopoConfig } from "./types.js";
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/joopo.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "joopo-config-mutate-" });
  const originalNixMode = process.env.JOOPO_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.JOOPO_NIX_MODE = originalNixMode;
    }
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("Joopo Nix overview: https://docs.joopo.ai/install/nix");
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".joopo", "config", "plugins.json5");
