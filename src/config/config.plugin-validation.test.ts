...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "joopo.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      JOOPO_STATE_DIR: path.join(suiteHome, ".joopo"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-config-plugin-validation-"));
  ...
  it("reports catalog install hints for missing configured official external plugins", () => {
    ...
    const message =
      "plugin not installed: brave — install the official external plugin with: joopo plugins install @joopo/brave-plugin";
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run joopo doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
  ...
  it("warns when plugins.allow contains a channel id without a plugin manifest (#76872)", () => {
    ...
    expect(res.warnings ?? []).toContainEqual({
      path: "plugins.allow",
      message:
        "plugin not installed: discord — install the official external plugin with: joopo plugins install @joopo/discord",
    });
  });

  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const installedPluginIndexPath = path.join(suiteHome, ".joopo", "plugins", "installs.json");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run joopo doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          JOOPO_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
    expect(res.ok).toBe(true);
  ...
  it("surfaces invalid Codex native plugin marketplaces as config diagnostics", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          JOOPO_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
        },
      },
    );
