...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.joopo) || !isRecord(entry.joopo.channel)) {
      continue;
    }
    const channel = entry.joopo.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: JoopoConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: JoopoConfig, pluginId: string) => boolean;
