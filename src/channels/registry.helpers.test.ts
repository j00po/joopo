...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://joopo.ai");
  });
  ...
  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    ...
    startupRegistry.channels = [
      {
        pluginId: "joopo-weixin",
        plugin: { id: "joopo-weixin", meta: { aliases: ["weixin"] } },
        source: "test",
      },
    ] as never;
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["joopo-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("joopo-weixin");
  });
