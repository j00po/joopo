...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: JoopoConfig, policy: ChannelAccessPolicy) => JoopoConfig;
  resolveAllowlist: (params: { cfg: JoopoConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: JoopoConfig; resolved: TResolved }) => JoopoConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: JoopoConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: JoopoConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: JoopoConfig, policy: ChannelAccessPolicy): JoopoConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: JoopoConfig, policy: ChannelAccessPolicy): JoopoConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: JoopoConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: JoopoConfig; resolved: string[] }) => {
      calls.push("apply");
