...
import type { JoopoPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["JOOPO_PLUGIN_CATALOG_PATHS", "JOOPO_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveJoopoPackageRootSync({ cwd: process.cwd() }),
    resolveJoopoPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<JoopoPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
