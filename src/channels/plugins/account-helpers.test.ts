...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): JoopoConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as JoopoConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: JoopoConfig) => string[];
  input: JoopoConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: JoopoConfig, expected: string) {
