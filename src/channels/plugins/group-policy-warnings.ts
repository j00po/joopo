...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: JoopoConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: JoopoConfig }>(
  collector: WarningCollector<{ cfg: JoopoConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: JoopoConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: JoopoConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: JoopoConfig },
>(
  projectCfg: (cfg: JoopoConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: JoopoConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: JoopoConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: JoopoConfig },
>(params: {
  providerConfigPresent: (cfg: JoopoConfig) => boolean;
