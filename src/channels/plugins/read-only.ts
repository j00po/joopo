...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed joopo package there.
...
function getChannelConfigRecord(cfg: JoopoConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: JoopoConfig, channelId: string): string[] {
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadJoopoPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
