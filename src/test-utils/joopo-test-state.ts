...
type JoopoTestStateLayout = "home" | "state-only" | "split";

type JoopoTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type JoopoTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: JoopoTestStateLayout;
  scenario?: JoopoTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "joopo-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "joopo.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "joopo.json"),
...
function scenarioConfig(options: JoopoTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "joopo-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: JoopoTestStateOptions): Record<string, string | undefined> {
...
export async function createJoopoTestState(
  options: JoopoTestStateOptions = {},
): Promise<JoopoTestState> {
...
export async function withJoopoTestState<T>(
  options: JoopoTestStateOptions,
  fn: (state: JoopoTestState) => Promise<T>,
): Promise<T> {
  const state = await createJoopoTestState(options);
  try {
