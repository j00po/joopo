...
const pluginDocs = [
  path.join(DOCS_ROOT, "tools", "joopohub.md"),
...
describe("JoopoHub plugin docs", () => {
  it("keeps the canonical plugin-publish snippets contract-valid", async () => {
    ...
    const pluginManifest = JSON.parse(
      await fs.readFile(
        path.join(DOCS_ROOT, "snippets", "plugin-publish", "minimal-joopo.plugin.json"),
        "utf8",
      ),
  ...
  it("does not tell plugin authors to use bare joopohub publish", async () => {
    for (const docPath of pluginDocs) {
      const markdown = await fs.readFile(docPath, "utf8");
      expect(markdown).not.toMatch(/(^|[\s`])joopohub publish\b/);
    }
  });
  ...
  it("keeps the canonical package snippet embedded in the primary plugin docs", async () => {
    ...
    expect(extractNamedJsonBlock(sdkSetup, "joopo-joopohub-package.json")).toEqual(snippet);
  });
});
