...
describe("usage-format", () => {
  const originalAgentDir = process.env.JOOPO_AGENT_DIR;
  const originalStateDir = process.env.JOOPO_STATE_DIR;
  let agentDir: string;
  let stateDir: string;

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.JOOPO_AGENT_DIR = originalAgentDir;
    }
  ...
  it("prefers models.json pricing over joopo config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to joopo config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
