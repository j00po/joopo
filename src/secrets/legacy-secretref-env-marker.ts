...
function toCandidate(
  ...
  defaults: NonNullable<JoopoConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: JoopoConfig): {
  config: JoopoConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as JoopoConfig & Record<string, unknown>;
