...
let compiledSecretTargetRegistryState: {
  ...
  joopoCompiledSecretTargets: CompiledTargetRegistryEntry[];
  joopoTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreJoopoTargetState: {
  knownTargetIds: Set<string>;
  joopoCompiledSecretTargets: CompiledTargetRegistryEntry[];
  joopoTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelJoopoTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const joopoCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "joopo.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    joopoCompiledSecretTargets,
    joopoTargetsById: buildConfigTargetIdIndex(joopoCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreJoopoTargetState() {
  if (compiledCoreJoopoTargetState) {
    return compiledCoreJoopoTargetState;
  }
  const joopoCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "joopo.json")
    .map(compileTargetRegistryEntry);
  compiledCoreJoopoTargetState = {
    knownTargetIds: new Set(joopoCompiledSecretTargets.map((entry) => entry.id)),
    joopoCompiledSecretTargets,
    joopoTargetsById: buildConfigTargetIdIndex(joopoCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(joopoCompiledSecretTargets),
  };
  return compiledCoreJoopoTargetState;
}

function getCompiledChannelJoopoTargets(
  channelId: string,
  ...
  if (compiledChannelJoopoTargets.has(normalizedChannelId)) {
    return compiledChannelJoopoTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "joopo.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelJoopoTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreJoopoTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreJoopoTargetState().joopoCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelJoopoTargets(explicitChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().joopoCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreJoopoTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreJoopoTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.joopoCompiledSecretTargets,
    entriesById: registryState.joopoTargetsById,
  });
