...
function beginSecretsRuntimeIsolationForTest(): SecretsRuntimeEnvSnapshot {
  ...
  delete process.env.JOOPO_VERSION;
  return envSnapshot;
}
...
describe("secrets runtime snapshot core lanes", () => {
  ...
  it("resolves env-backed auth profile SecretRefs", async () => {
    ...
    expect(snapshot.warnings.map((warning) => warning.path)).toEqual(
      expect.arrayContaining([
        "/tmp/joopo-agent-main.auth-profiles.openai:default.key",
        "/tmp/joopo-agent-main.auth-profiles.github-copilot:default.token",
      ]),
    );
  ...
  it("activates runtime snapshots for ensureAuthProfileStore", async () => {
    ...
    expect(
      ensureAuthProfileStore("/tmp/joopo-agent-main").profiles["openai:default"],
    ).toMatchObject({
      type: "api_key",
      key: "sk-runtime",
    });
