...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: JoopoConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "joopo.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.JOOPO_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_JOOPO_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.JOOPO_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.JOOPO_BUNDLED_PLUGINS_DIR ??= "extensions";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.JOOPO_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function collectJoopoCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "joopo.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_JOOPO_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForJoopoTarget(
  config: JoopoConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
async function expectJoopoCoverageEntriesResolved(
  label: string,
  entries: readonly SecretRegistryEntry[],
): Promise<void> {
  for (const batch of buildCoverageBatches(entries)) {
    logCoverageBatch(label, batch);
    const config = {} as JoopoConfig;
    const env: Record<string, string> = {};
    for (const [index, entry] of batch.entries()) {
      const envId = `JOOPO_SECRET_TARGET_${entry.id}`;
      ...
      applyConfigForJoopoTarget(config, entry, envId, wildcardToken);
    }
...
describe("secrets runtime target coverage", () => {
  ...
  it(
    "handles every core and channel joopo.json registry target when configured as active",
    async () => {
      await expectJoopoCoverageEntriesResolved(
        "joopo.json core",
        collectJoopoCoverageEntries({ includePluginEntries: false }),
      );
    },
  ...
  it(
    "handles every plugin joopo.json registry target when configured as active",
    async () => {
      await expectJoopoCoverageEntriesResolved(
        "joopo.json plugins",
        collectJoopoCoverageEntries({ includePluginEntries: true }),
      );
    },
    RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
  );
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `JOOPO_AUTH_SECRET_TARGET_${entry.id}`;
      ...
      const snapshot = await prepareAuthCoverageSnapshot({
        ...
        env,
        agentDirs: ["/tmp/joopo-agent-main"],
