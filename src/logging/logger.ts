...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredJoopoTmpDir() : POSIX_JOOPO_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "joopo.log")
    : `${POSIX_JOOPO_TMP_DIR}/joopo.log`;
}
...
type LoggerConfigLoader = () => JoopoConfig["logging"] | undefined;
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: JoopoConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.JOOPO_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[joopo] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
