...
const logPathTracker = createSuiteLogPathTracker("joopo-log-redaction-");
const originalConfigPath = process.env.JOOPO_CONFIG_PATH;
const originalHome = process.env.HOME;
const originalTestFileLog = process.env.JOOPO_TEST_FILE_LOG;
...
describe("file log redaction", () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    expect(loggerTest.resolveActiveLogFile("~/custom-joopo.log")).toBe(
      path.join(home, "custom-joopo.log"),
    );
  });
