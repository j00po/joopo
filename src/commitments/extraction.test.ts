...
describe("commitment extraction", () => {
  ...
  async function createConfig(): Promise<JoopoConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-commitments-"));
    tmpDirs.push(tmpDir);
    vi.stubEnv("JOOPO_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
  ...
  it("rejects disabled, low-confidence, and non-future candidates", () => {
    const cfg: JoopoConfig = { commitments: { enabled: true } };
