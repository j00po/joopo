...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[joopo proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[joopo proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`joopo proxy coverage\` for details.`,
  );
}
