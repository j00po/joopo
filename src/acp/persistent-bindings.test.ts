...
type ConfiguredBinding = NonNullable<JoopoConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: JoopoConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/joopo" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
