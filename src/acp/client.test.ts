...
const createTempDir = () => tempDirs.make("joopo-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets JOOPO_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "joopo",
    });

    expect(env.JOOPO_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("joopo");
  });

  it("overrides pre-existing JOOPO_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      JOOPO_SHELL: "wrong",
    });
    expect(env.JOOPO_SHELL).toBe("acp-client");
  ...
  it("preserves JOOPO_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        JOOPO_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["JOOPO_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Joopo bridge", () => {
    ...
    expect(env.JOOPO_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.JOOPO_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("JOOPO_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "joopo", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "joopo", "dist", "entry.js");
    const shimPath = path.join(dir, "joopo.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\joopo\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/joopo-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/joopo-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/joopo-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
