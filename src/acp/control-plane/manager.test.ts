...
async function withAcpManagerTaskStateDir(run: (root: string) => Promise<void>): Promise<void> {
  await withTempDir({ prefix: "joopo-acp-manager-task-" }, async (root) => {
...
describe("AcpSessionManager", () => {
  ...
  it("preserves token-streamed ACP progress boundaries in parented task summaries", async () => {
    await withAcpManagerTaskStateDir(async () => {
      ...
      expect(findTaskByRunId("direct-parented-korean-path-run")).toMatchObject({
        ...
        status: "succeeded",
        progressSummary: "현재 작업 디렉토리는 /home/bykim0119/.joopo/workspace 입니다",
      });
    });
  }, 300_000);
  ...
  it("treats unsupported close controls as recoverable during discard cleanup", async () => {
    ...
    hoisted.readAcpSessionEntryMock.mockReturnValue({
      sessionKey: "agent:joopo:acp:session-1",
      storeSessionKey: "agent:joopo:acp:session-1",
      acp: readySessionMeta({
        agent: "joopo",
      }),
    });
