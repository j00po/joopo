...
export type { JoopoConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: JoopoConfig, ...args: TArgs) => JoopoConfig;
  applyConfig: (cfg: JoopoConfig, ...args: TArgs) => JoopoConfig;
};
...
>(params: {
  ...
  applyPreset: (cfg: JoopoConfig, preset: TParams) => JoopoConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: JoopoConfig): {
  next: JoopoConfig;
  changed: boolean;
} {
