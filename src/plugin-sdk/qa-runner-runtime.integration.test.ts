...
const ORIGINAL_ENV = {
  ...
  JOOPO_CONFIG_PATH: process.env.JOOPO_CONFIG_PATH,
  JOOPO_TEST_FAST: process.env.JOOPO_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("joopo-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@joopo/qa-linked",
        type: "module",
        joopo: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@joopo/qa-linked",
          },
        },
      }),
      "utf8",
