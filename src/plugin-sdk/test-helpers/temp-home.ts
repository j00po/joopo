...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked JOOPO_HOME.
  delete process.env.JOOPO_HOME;
  process.env.JOOPO_STATE_DIR = path.join(base, ".joopo");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "joopo-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".joopo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
