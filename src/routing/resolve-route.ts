...
function listAgents(cfg: JoopoConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: JoopoConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<JoopoConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: JoopoConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: JoopoConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: JoopoConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<JoopoConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  JoopoConfig,
  {
    bindingsRef: JoopoConfig["bindings"];
    agentsRef: JoopoConfig["agents"];
    sessionRef: JoopoConfig["session"];
...
function resolveRouteCacheForConfig(cfg: JoopoConfig): Map<string, ResolvedAgentRoute> {
