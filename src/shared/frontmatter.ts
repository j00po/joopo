...
export function resolveJoopoManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type JoopoManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveJoopoManifestRequires(
  metadataObj: Record<string, unknown>,
): JoopoManifestRequires | undefined {
  const requiresRaw =
...
export function resolveJoopoManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveJoopoManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedJoopoManifestInstallBase = {
...
export function parseJoopoManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedJoopoManifestInstallBase | undefined {
  ...
  const spec: ParsedJoopoManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyJoopoManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedJoopoManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
