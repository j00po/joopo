...
describe("shared/node-match", () => {
  ...
  it("prefers a unique current Joopo client over a legacy joopobot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=joopo-macos.*node=third-mac.*client=joopo-macos/,
    );
  });
