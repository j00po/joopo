...
const CANVAS_HOST_PATH = "/__joopo__/canvas";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredJoopoTmpDir(), "web-media-core-"));
...
describe("loadWebMedia", () => {
  ...
  it("resolves home-relative local media paths through allowed local roots", async () => {
    vi.stubEnv("JOOPO_HOME", fixtureRoot);
    try {
