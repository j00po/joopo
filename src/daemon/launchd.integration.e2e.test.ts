...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `joopo-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      JOOPO_LAUNCHD_LABEL: `ai.joopo.launchd-int-${testId}`,
      JOOPO_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
