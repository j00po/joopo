...
export const JOOPO_WRAPPER_ENV_KEY = "JOOPO_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/joopo -> .pnpm/joopo@X.Y.Z/...
...
export async function resolveJoopoWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${JOOPO_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveJoopoWrapperPath(params.wrapperPath);
  if (wrapperPath) {
