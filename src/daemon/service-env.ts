...
function readServiceProxyEnvironment(
  ...
  const proxyUrl = normalizeOptionalString(env.JOOPO_PROXY_URL);
  return proxyUrl ? { JOOPO_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const wrapperPath = normalizeOptionalString(env.JOOPO_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    JOOPO_PROFILE: profile,
    JOOPO_WRAPPER: wrapperPath,
    JOOPO_GATEWAY_PORT: String(port),
    JOOPO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    JOOPO_SYSTEMD_UNIT: systemdUnit,
    JOOPO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    JOOPO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    JOOPO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    JOOPO_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.JOOPO_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.JOOPO_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    JOOPO_GATEWAY_TOKEN: gatewayToken,
    JOOPO_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    JOOPO_STATE_DIR: sharedEnv.stateDir,
    JOOPO_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.JOOPO_STATE_DIR;
