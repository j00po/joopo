...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.JOOPO_GATEWAY_PORT).toBe("18789");
    expect(env.JOOPO_GATEWAY_TOKEN).toBeUndefined();
    expect(env.JOOPO_SERVICE_MARKER).toBe("joopo");
    expect(env.JOOPO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.JOOPO_SERVICE_VERSION).toBe("string");
    expect(env.JOOPO_SYSTEMD_UNIT).toBe("joopo-gateway.service");
    expect(env.JOOPO_WINDOWS_TASK_NAME).toBe("Joopo Gateway");
    if (process.platform === "darwin") {
      expect(env.JOOPO_LAUNCHD_LABEL).toBe("ai.joopo.gateway");
    }
  });

  it("passes through JOOPO_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        JOOPO_WRAPPER: " /usr/local/bin/joopo-doppler ",
      },
      port: 18789,
    });

    expect(env.JOOPO_WRAPPER).toBe("/usr/local/bin/joopo-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".joopo", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", JOOPO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.JOOPO_SYSTEMD_UNIT).toBe("joopo-gateway-work.service");
    expect(env.JOOPO_WINDOWS_TASK_NAME).toBe("Joopo Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.JOOPO_LAUNCHD_LABEL).toBe("ai.joopo.work");
    }
  });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.JOOPO_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through JOOPO_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", JOOPO_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.JOOPO_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through JOOPO_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", JOOPO_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.JOOPO_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits JOOPO_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        JOOPO_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".joopo"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", JOOPO_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".joopo-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", JOOPO_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".joopo"));
  });

  it("uses JOOPO_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", JOOPO_STATE_DIR: "/var/lib/joopo" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/joopo"));
  });

  it("expands ~ in JOOPO_STATE_DIR", () => {
    const env = { HOME: "/Users/test", JOOPO_STATE_DIR: "~/joopo-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/joopo-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { JOOPO_STATE_DIR: "C:\\State\\joopo" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\joopo");
  });
});
