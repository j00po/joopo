...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.JOOPO_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.JOOPO_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.JOOPO_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  if (!isLaunchAgentServiceEnvironment(params)) {
    return;
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    JOOPO_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
