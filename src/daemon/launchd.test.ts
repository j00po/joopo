...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.joopo.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", JOOPO_PROFILE: "default" },
    });
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.joopo/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.joopo/service-env/ai.joopo.gateway.env";
    const wrapperPath = "/Users/test/.joopo/service-env/ai.joopo.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.joopo/service-env")).toBe(0o700);
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.joopo.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.joopo.gateway`;
  ...
  it("stops LaunchAgent with disable+stop when --disable is passed", async () => {
    ...
    expect(state.launchctlCalls).toContainEqual(["stop", "ai.joopo.gateway"]);
  ...
  it("treats already-unloaded services as successfully stopped without bootout fallback (--disable)", async () => {
    ...
    expect(state.launchctlCalls).toContainEqual([
      "disable",
      `${typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501"}/ai.joopo.gateway`,
    ]);
    expect(state.launchctlCalls.some((call) => call[0] === "bootout")).toBe(false);
    expect(output).toContain("Stopped LaunchAgent");
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.joopo.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when JOOPO_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.joopo.gateway.plist",
    },
    {
      name: "uses profile-specific label when JOOPO_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", JOOPO_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.joopo.jbphoenix.plist",
    },
    {
      name: "prefers JOOPO_LAUNCHD_LABEL over JOOPO_PROFILE",
      env: {
        HOME: "/Users/test",
        JOOPO_PROFILE: "jbphoenix",
        JOOPO_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from JOOPO_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        JOOPO_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty JOOPO_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        JOOPO_PROFILE: "myprofile",
        JOOPO_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.joopo.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        JOOPO_LAUNCHD_LABEL: "../evil/label",
      }),
