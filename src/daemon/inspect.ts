...
const EXTRA_MARKERS = ["joopobot", "joopo"] as const;
...
export function renderGatewayServiceCleanupHints(
  ...
  const profile = env.JOOPO_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = normalizeLowercaseStringOrEmpty(content);
  const markerKeys = ["joopo_service_marker"];
  const kindKeys = ["joopo_service_kind"];
...
function isJoopoGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "joopo") {
    return false;
  }
  return label.startsWith("ai.joopo.");
}

function isJoopoGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("joopo-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isJoopoGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  const defaultName = normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName());
  return normalized === defaultName || normalized.startsWith("joopo gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("joopobot");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "joopo"
        ? "joopo"
        : executionMarker === "joopobot" || legacyLabel || detectMarker(contents) === "joopobot"
          ? "joopobot"
          : null;
    if (!marker) {
      continue;
    ...
    if (marker === "joopo" && isJoopoGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "joopo" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedJoopo?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedJoopo ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedJoopo &&
      marker === "joopo" &&
      isJoopoGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isJoopoGatewayTaskName(name)) {
        continue;
      }
