...
async function resolveJoopoPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveJoopoPackageRoot(entrypointReal) : undefined;
