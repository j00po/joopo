...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds JOOPO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `joopo gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service JOOPO_GATEWAY_TOKEN does not match gateway.auth.token in joopo.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.JOOPO_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.JOOPO_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `joopo gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
