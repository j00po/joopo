...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.JOOPO_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.JOOPO_PROFILE);
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** Joopo-managed keys that must not be preserved from an old env file; stale file values
  ...
  // Joopo-managed keys (identified by inlineManagedKeys) are excluded: a stale
