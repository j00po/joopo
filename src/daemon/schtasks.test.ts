...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when JOOPO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".joopo", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when JOOPO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", JOOPO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".joopo-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers JOOPO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        JOOPO_PROFILE: "rescue",
        JOOPO_STATE_DIR: "C:\\State\\joopo",
      },
      expected: path.join("C:\\State\\joopo", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", JOOPO_PROFILE: "default" },
      expected: path.join("/home/test", ".joopo", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Joopo Gateway",
          "cd /d C:\\Projects\\joopo",
          "set NODE_ENV=production",
          "set JOOPO_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\joopo",
          environment: {
            NODE_ENV: "production",
            JOOPO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\joopo\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\joopo\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Joopo Share\\node.exe" "\\\\fileserver\\Joopo Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Joopo Share\\node.exe",
            "\\\\fileserver\\Joopo Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from JOOPO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ JOOPO_STATE_DIR: path.join(tmpDir, "custom-state") }),
