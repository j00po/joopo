...
import { getRuntimeConfig, type JoopoConfig } from "../config/config.js";
...
const JOOPO_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../joopo.mjs", import.meta.url));
const JOOPO_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const JOOPO_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const JOOPO_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(JOOPO_DIST_ENTRY_JS_PATH) || existsSync(JOOPO_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(JOOPO_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? JOOPO_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? JOOPO_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run joopo devices list",
      pairingHint:
        "Pairing required. Run `joopo devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__joopo_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "joopo tui";
    header.setText(
      theme.header(
    ...
    void drainAndStopTuiSafely(tui)
      .catch((err) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`joopo tui shutdown failed: ${String(err)}\n`);
          } catch {
