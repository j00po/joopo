...
export const JOOPO_NODE_OPTIONS_READY = "JOOPO_NODE_OPTIONS_READY";
export const JOOPO_NODE_EXTRA_CA_CERTS_READY = "JOOPO_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.JOOPO_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[JOOPO_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[JOOPO_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv) &&
    !isTruthyEnvValue(env[JOOPO_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[JOOPO_NODE_OPTIONS_READY] = "1";
