...
import {
  ...
  JOOPO_NODE_EXTRA_CA_CERTS_READY,
  JOOPO_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(plan?.env[JOOPO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[JOOPO_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      expect(plan).not.toBeNull();
      expect(plan?.argv).toEqual(["joopo", command]);
      expect(plan?.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(plan?.env[JOOPO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(plan?.env[JOOPO_NODE_OPTIONS_READY]).toBeUndefined();
    },
  );
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "joopo", "status"],
        env: {
          [JOOPO_NODE_EXTRA_CA_CERTS_READY]: "1",
          [JOOPO_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/joopo", "status"],
    ...
    expect(plan?.argv).toEqual([EXPERIMENTAL_WARNING_FLAG, "/usr/local/bin/joopo", "status"]);
  });
});
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/joopo/dist/entry.js", "status"],
        env: { JOOPO_NODE_OPTIONS_READY: "1" },
      },
      {
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/joopo/dist/entry.js", "tui"],
          env: {},
        },
        {
          spawn: spawn as unknown as typeof import("node:child_process").spawn,
