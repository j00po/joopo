...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<JoopoConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<JoopoConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: JoopoConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: JoopoConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: JoopoConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: JoopoConfig, result: JoopoConfig): JoopoConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: JoopoConfig = {
    ...result,
    tools: {
      ...result.tools,
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with joopo configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Joopo will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.joopo.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Joopo will store a reference instead of the API key.",
