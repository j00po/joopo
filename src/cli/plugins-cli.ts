...
function reportMissingPlugin(id: string) {
  defaultRuntime.error(
    `Plugin not found: ${id}. Run \`joopo plugins list\` to see installed plugins.`,
  );
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Joopo plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.joopo.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as JoopoConfig;
      ...
      let next: JoopoConfig = enableResult.config;
  ...
  plugins
    ...
    .action(async (opts: PluginRegistryOptions) => {
      ...
      if (inspection.refreshReasons.length > 0) {
        ...
        lines.push(
          `${theme.muted("Repair:")} ${theme.command("joopo plugins registry --refresh")}`,
        );
      }
    ...
    .action(async () => {
      ...
      if (shadowed.length > 0) {
        ...
        for (const diag of shadowed) {
          ...
          lines.push("  repair:");
          lines.push("    joopo plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
          lines.push("    edit or remove the config-selected plugin source");
          lines.push("    joopo plugins registry --refresh");
          lines.push("    joopo gateway restart --force");
        }
      }
      ...
      const docs = formatDocsLink("/plugin", "docs.joopo.ai/plugin");
      lines.push("");
