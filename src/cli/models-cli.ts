...
function rejectAgentScopedModelWrite(command: Command, commandName: "set" | "set-image"): void {
  ...
  throw new Error(
    `\`joopo models ${commandName}\` does not support \`--agent\`; it only updates global model defaults. Remove \`--agent\` or use agent config to set a per-agent model override.`,
  );
}
...
export function registerModelsCli(program: Command) {
  const models = program
    ...
    .option(
      "--agent <id>",
      "Agent id to inspect (overrides JOOPO_AGENT_DIR/PI_CODING_AGENT_DIR)",
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/models", "docs.joopo.ai/cli/models")}\n`,
    );
