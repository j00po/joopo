...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      JOOPO_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { JOOPO_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { JOOPO_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-daemon-cli-"));
    ...
    process.env.JOOPO_STATE_DIR = tmpDir;
    process.env.JOOPO_CONFIG_PATH = path.join(tmpDir, "joopo.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        JOOPO_PROFILE: "dev",
        JOOPO_STATE_DIR: "/tmp/joopo-daemon-state",
        JOOPO_CONFIG_PATH: "/tmp/joopo-daemon-state/joopo.json",
        JOOPO_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.joopo.gateway.plist",
    });
