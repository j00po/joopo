...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["joopo.bash", "joopo.fish", "joopo.ps1", "joopo.zsh"]),
    );
