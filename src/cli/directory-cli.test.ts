...
describe("registerDirectoryCli", () => {
  ...
  it("reports unsupported directory capability instead of continuing setup for installed plugins", async () => {
    mocks.resolveInstallableChannelPlugin.mockResolvedValue({
      cfg: { channels: { "joopo-weixin": {} } },
      channelId: "joopo-weixin",
      plugin: {
        id: "joopo-weixin",
      },
      configChanged: false,
    ...
    await expect(
      program.parseAsync(["directory", "peers", "list", "--channel", "joopo-weixin"], {
        from: "user",
      }),
    ...
    expect(mocks.resolveInstallableChannelPlugin).toHaveBeenCalledWith(
      expect.objectContaining({
        rawChannel: "joopo-weixin",
        allowInstall: true,
      }),
    );
    expect(mocks.replaceConfigFile).not.toHaveBeenCalled();
    expect(runtimeState.defaultRuntime.error).toHaveBeenCalledWith(
      expect.stringContaining("Channel joopo-weixin does not support directory peers"),
    );
  });
});
