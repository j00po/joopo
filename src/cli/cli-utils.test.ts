...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "joopo", "-V"] },
    { argv: ["node", "joopo", "tui"] },
    { argv: ["node", "joopo", "terminal"] },
    { argv: ["node", "joopo", "chat"] },
    { argv: ["node", "joopo", "gateway"] },
    { argv: ["node", "joopo", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "joopo", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "joopo", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "joopo", "gateway", "status"] },
    { argv: ["node", "joopo", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "joopo", "--help"] },
    { argv: ["node", "joopo", "gateway"] },
    { argv: ["node", "joopo", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "joopo.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("joopo.internal");
    } finally {
      log.mockRestore();
    }
  });
