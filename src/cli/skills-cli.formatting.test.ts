...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-bundled-skills-test-"));
    process.env.JOOPO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
