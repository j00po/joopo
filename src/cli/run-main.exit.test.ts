...
vi.mock("../infra/path-env.js", () => ({
  ensureJoopoCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.JOOPO_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.JOOPO_HIDE_BANNER;
  });
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "joopo", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "joopo", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "joopo", "agent", "--local"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "joopo", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "joopo", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "joopo", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "joopo", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.JOOPO_HIDE_BANNER = "1";

    await runCli(["node", "joopo", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "joopo", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "joopo", "browser", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "joopo", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "joopo", "gateway", "run"]],
    ["bare gateway runtime", ["node", "joopo", "gateway"]],
    ["node runtime", ["node", "joopo", "node", "run"]],
    ["local agent runtime", ["node", "joopo", "agent", "--local"]],
    ["provider inference", ["node", "joopo", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "joopo", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "joopo", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "joopo", "skills", "search", "browser"]],
    ["update command", ["node", "joopo", "update", "check"]],
    ["channel probe", ["node", "joopo", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "joopo", "channels", "capabilities"]],
    ["directory plugin command", ["node", "joopo", "directory", "peers", "list"]],
    ["message plugin command", ["node", "joopo", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "joopo", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "joopo", "--help"]],
    ["root version", ["node", "joopo", "--version"]],
    ["gateway help", ["node", "joopo", "gateway", "--help"]],
    ["gateway run help", ["node", "joopo", "gateway", "run", "--help"]],
    ["status", ["node", "joopo", "status"]],
    ["health", ["node", "joopo", "health"]],
    ["gateway status", ["node", "joopo", "gateway", "status"]],
    ["gateway health", ["node", "joopo", "gateway", "health"]],
    ["remote agent control-plane", ["node", "joopo", "agent", "run"]],
    ["chat control-plane", ["node", "joopo", "chat"]],
    ["terminal control-plane", ["node", "joopo", "terminal"]],
    ["config", ["node", "joopo", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "joopo", "channels"]],
    ["completion", ["node", "joopo", "completion", "zsh"]],
    ["debug proxy cli", ["node", "joopo", "proxy", "start"]],
    ["agents list", ["node", "joopo", "agents", "list"]],
    ["models list", ["node", "joopo", "models", "list"]],
    ["models status without live probe", ["node", "joopo", "models", "status"]],
    ["skills check", ["node", "joopo", "skills", "check"]],
    ["skills info", ["node", "joopo", "skills", "info", "weather"]],
    ["skills list", ["node", "joopo", "skills", "list"]],
    ["tasks list", ["node", "joopo", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "joopo", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "joopo", "tools", "effective"]],
    ["migrate", ["node", "joopo", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "joopo", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "joopo", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "joopo", "foo"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "joopo", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "joopo", "auth", "--help"]],
    ["tool", ["node", "joopo", "tool", "image_generate"]],
    ["tools", ["node", "joopo", "tools", "effective"]],
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "joopo", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "joopo", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "joopo", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "joopo", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "joopo", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "joopo", "--container", "demo", "status"]);
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    await runCli(["node", "joopo", "doctor", "--help"]);
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        "[joopo] Non-fatal uncaught exception (continuing):",
