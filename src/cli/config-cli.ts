...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "joopo config set secrets.providers.vault --provider-source file --provider-path /etc/joopo/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("joopo config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("joopo doctor")}\` ${message}`;
}
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("joopo plugins install <spec>")}`,
    `  ${formatCliCommand("joopo plugins update <plugin-id>")}`,
    `  ${formatCliCommand("joopo plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
async function runConfigOperations(params: {
  ...
  const nextConfig = next as JoopoConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "joopo.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.joopo.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing joopo.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing joopo.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for joopo.json")
    .action(async () => {
