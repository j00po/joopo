...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside Joopo.";
...
function getAuthProfileIdsForProvider(cfg: JoopoConfig, providerId: string): string[] {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.joopo.ai/cli/infer")}\n`,
    );
