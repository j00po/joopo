...
const serveJoopoChannelMcp = mocks.serveJoopoChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveJoopoChannelMcp: mocks.serveJoopoChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("joopo-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("joopo-cli-mcp-home-", async () => {
      ...
      expect(serveJoopoChannelMcp).toHaveBeenCalledWith({
