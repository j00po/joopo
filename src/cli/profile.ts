...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.joopo${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.JOOPO_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.JOOPO_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.JOOPO_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.JOOPO_CONFIG_PATH)) {
    env.JOOPO_CONFIG_PATH = path.join(stateDir, "joopo.json");
  }

  if (profile === "dev" && !env.JOOPO_GATEWAY_PORT?.trim()) {
    env.JOOPO_GATEWAY_PORT = "19001";
  }
}
