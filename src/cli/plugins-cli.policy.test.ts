...
describe("plugins cli policy mutations", () => {
  ...
  it("refreshes the persisted plugin registry after enabling a plugin", async () => {
    const sourceConfig = {} as JoopoConfig;
    const enabledConfig = {
      plugins: {
        entries: {
          alpha: { enabled: true },
  ...
  it("refreshes the persisted plugin registry after disabling a plugin", async () => {
    ...
    const nextConfig = writeConfigFile.mock.calls[0]?.[0] as JoopoConfig;
  ...
  it.each(["enable", "disable"] as const)(
    ...
    async (command) => {
      ...
      expect(runtimeErrors).toContain(
        "Plugin not found: missing-plugin. Run `joopo plugins list` to see installed plugins.",
      );
      expect(enablePluginInConfig).not.toHaveBeenCalled();
      expect(writeConfigFile).not.toHaveBeenCalled();
