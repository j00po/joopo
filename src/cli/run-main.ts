...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.JOOPO_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.JOOPO_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.JOOPO_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<JoopoConfig> | null = null;
  const readBestEffortCliConfig = async (): Promise<JoopoConfig> => {
  ...
  if (shouldStartProxyForCli(normalizedArgv)) {
    ...
    if (unownedPrimary) {
      throw new Error(
        `Unknown command: joopo ${unownedPrimary}. No built-in command or plugin CLI metadata owns "${unownedPrimary}".`,
      );
    }
  ...
  try {
    ...
    if (shouldRunBareRootCrestodian) {
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `joopo crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        console.error("[joopo] Uncaught exception:", formatUncaughtError(error));
        for (const message of runFatalErrorHooks({ reason: "uncaught_exception", error })) {
          console.error("[joopo]", message);
        }
