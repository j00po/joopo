import { bundledPluginRootAt, repoInstallSpec } from "joopo/plugin-sdk/test-fixtures";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { JoopoConfig } from "../config/config.js";
import type { ConfigFileSnapshot } from "../config/types.joopo.js";
import {
...
vi.mock("../commands/doctor/shared/channel-doctor.js", () => ({
  collectChannelDoctorStaleConfigMutations: (cfg: JoopoConfig) =>
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as JoopoConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as JoopoConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const discordNpmRequest = {
    rawSpec: "@joopo/discord",
    normalizedSpec: "@joopo/discord",
  ...
  beforeEach(() => {
    ...
    collectChannelDoctorStaleConfigMutationsMock.mockImplementation(async (cfg: JoopoConfig) => [
      {
        config: cfg,
        changes: [],
      },
  ...
  it("returns the source config and base hash when the snapshot is valid", async () => {
    const cfg = { plugins: { entries: { discord: { enabled: true } } } } as JoopoConfig;
  ...
  it("does not run stale Discord cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as JoopoConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        valid: true,
        sourceConfig: cfg,
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    const snapshotCfg = { plugins: {} } as JoopoConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `joopo doctor --fix` before installing plugins.");
  });
  ...
  it("throws when invalid snapshot parsed is empty", async () => {
    ...
    await expect(loadConfigForInstall(discordNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `joopo doctor` to repair it.",
    );
  });
