...
const loadConfig = vi.fn<() => JoopoConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      JOOPO_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { JOOPO_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("JOOPO_CONTAINER_HINT", "joopo-demo-container");

    await runServiceRestart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => [
        "Restart the container or the service that manages it for joopo-demo-container.",
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for joopo-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { JOOPO_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("JOOPO_GATEWAY_TOKEN", "env-token");
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["joopo", "gateway"],
      environment: { JOOPO_SERVICE_VERSION: "2026.4.24" },
    });
    const repairLoadedService = vi.fn(async () => ({
      result: "started" as const,
      message: "Gateway service definition repaired and started.",
      warnings: ["service was installed by Joopo 2026.4.24, current CLI is 2026.5.2"],
      loaded: true,
    }));
    ...
    expect(payload.warnings?.[0]).toContain("service was installed by Joopo");
    expect(payload.service?.loaded).toBe(true);
  });
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["joopo gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["joopo gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["joopo gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "joopo gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
