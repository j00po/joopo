...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining(formatCliCommand("joopo gateway restart")),
    );
  });
  ...
  it("prints CLI and gateway versions with stale wrapper guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/joopo",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      expect.stringContaining("CLI version: 2026.4.23 (/usr/local/bin/joopo)"),
    );
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/joopo-cli/joopo.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/joopo-daemon/joopo.json",
            exists: true,
            valid: true,
