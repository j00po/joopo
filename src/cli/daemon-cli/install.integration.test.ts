...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("joopo-daemon-install-int-");
    configPath = path.join(tempHome, "joopo.json");
    process.env.HOME = tempHome;
    process.env.JOOPO_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer Joopo", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.JOOPO_GATEWAY_TOKEN).toBeUndefined();
  });
});
