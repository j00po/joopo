...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: { wrapperPath?: string; env?: Record<string, string | undefined> }) => {
    ...
    if (params?.wrapperPath || params?.env?.JOOPO_WRAPPER) {
      environment.JOOPO_WRAPPER = params.wrapperPath ?? params.env?.JOOPO_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/joopo.json" },
  })),
}));
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveJoopoWrapperPath: async (value: string | undefined) => value?.trim() || undefined,
}));
...
function mockResolvedGatewayTokenSecretRef() {
  resolveSecretInputRefMock.mockReturnValue({
    ref: { source: "env", provider: "default", id: "JOOPO_GATEWAY_TOKEN" },
  });
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:JOOPO_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${JOOPO_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds JOOPO_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["joopo", "gateway", "run"],
      environment: {
        JOOPO_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(actionState.warnings).toContain(
      "Gateway service JOOPO_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["joopo", "gateway", "run"],
      environment: {
        JOOPO_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/joopo-doppler", "gateway", "run"],
      environment: {
    ...
    expect(buildGatewayInstallPlanMock).toHaveBeenCalledWith(
      expect.objectContaining({
        wrapperPath: "/usr/local/bin/joopo-doppler",
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/joopo-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service JOOPO_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when JOOPO_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["joopo", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["joopo", "gateway", "run"],
      environment: {
        JOOPO_STATE_DIR: "/tmp/joopo-doctor-manual",
        JOOPO_CONFIG_PATH: "/tmp/joopo-doctor-manual/joopo.json",
        JOOPO_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.JOOPO_STATE_DIR).toBeUndefined();
      expect(env.JOOPO_CONFIG_PATH).toBeUndefined();
