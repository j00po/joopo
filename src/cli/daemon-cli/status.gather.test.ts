...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    JOOPO_STATE_DIR: "/tmp/joopo-daemon",
    JOOPO_CONFIG_PATH: "/tmp/joopo-daemon/joopo.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.JOOPO_STATE_DIR ?? "/tmp/joopo-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.JOOPO_CONFIG_PATH ?? `${stateDir}/joopo.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/joopo-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.JOOPO_STATE_DIR = "/tmp/joopo-cli";
    process.env.JOOPO_CONFIG_PATH = "/tmp/joopo-cli/joopo.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/joopo-daemon/joopo.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/joopo-cli/joopo.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.JOOPO_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.JOOPO_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-status-config-"));
    const configPath = path.join(tmp, "joopo.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.JOOPO_STATE_DIR = tmp;
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.JOOPO_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "joopo-gateway" }],
        hints: [],
      },
      healthy: false,
