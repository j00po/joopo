...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("joopo doctor")}" (or "${formatCliCommand("joopo doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("joopo gateway install --force")}\` from the same --profile / JOOPO_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      ...
      defaultRuntime.error(
        warnText(
          `Fix: check for stale PATH/global wrappers with \`command -v joopo\`, \`readlink -f "$(command -v joopo)"\`, and \`joopo --version\`; reinstall the gateway service from the intended binary if needed.`,
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("joopo gateway restart")} and re-check with ${formatCliCommand("joopo gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.JOOPO_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("joopo gateway install")}`),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.JOOPO_PROFILE);
      defaultRuntime.error(
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("joopo status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.joopo.ai/troubleshooting`);
}
