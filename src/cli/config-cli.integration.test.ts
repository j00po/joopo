...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["JOOPO_CONFIG_PATH", "JOOPO_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  it("accepts plugin hook conversation-access policy via config set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-config-cli-plugin-hooks-"));
    const configPath = path.join(tempDir, "joopo.json");
    const envSnapshot = captureEnv(["JOOPO_CONFIG_PATH", "JOOPO_TEST_FAST"]);
    try {
      fs.writeFileSync(
      ...
      await runConfigSet({
        path: "plugins.entries.joopo-mem0.hooks.allowConversationAccess",
        value: "true",
        cliOptions: {},
      ...
      expect(afterWrite.plugins?.entries?.["joopo-mem0"]?.hooks).toEqual({
  ...
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-config-cli-int-"));
    const configPath = path.join(tempDir, "joopo.json");
    const batchPath = path.join(tempDir, "batch.json");
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "joopo.json");
    const envSnapshot = captureEnv([
      "JOOPO_CONFIG_PATH",
      "JOOPO_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("joopo-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("joopo-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,
