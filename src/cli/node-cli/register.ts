    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "joopo node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["joopo node status", "Check node host service status."],
          ["joopo node install", "Install the node host service."],
          ["joopo node start", "Start the installed node host service."],
          ["joopo node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.joopo.ai/cli/node")}\n`,
    );
