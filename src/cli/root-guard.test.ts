...
describe("assertNotRoot", () => {
  ...
  it("does not exit when uid is 0 and JOOPO_ALLOW_ROOT=1", () => {
    process.getuid = () => 0;
    assertNotRoot({ JOOPO_ALLOW_ROOT: "1" });
    expect(exitSpy).not.toHaveBeenCalled();
  });

  it("does not exit when uid is 0 and JOOPO_CLI_CONTAINER_BYPASS=1 with container hint", () => {
    process.getuid = () => 0;
    assertNotRoot({ JOOPO_CLI_CONTAINER_BYPASS: "1", JOOPO_CONTAINER_HINT: "my-container" });
    expect(exitSpy).not.toHaveBeenCalled();
  });

  it("exits when uid is 0 and JOOPO_CLI_CONTAINER_BYPASS=1 without container hint", () => {
    process.getuid = () => 0;
    assertNotRoot({ JOOPO_CLI_CONTAINER_BYPASS: "1" });
    expect(exitSpy).toHaveBeenCalledWith(1);
  });
  ...
  it("does not exit when euid is 0 but JOOPO_ALLOW_ROOT=1", () => {
    process.getuid = () => 1000;
    process.geteuid = () => 0;
    assertNotRoot({ JOOPO_ALLOW_ROOT: "1" });
    expect(exitSpy).not.toHaveBeenCalled();
  ...
  it("error message mentions JOOPO_ALLOW_ROOT", () => {
    process.getuid = () => 0;
    assertNotRoot({});
    const output = stderrSpy.mock.calls.map((c) => String(c[0])).join("");
    expect(output).toContain("JOOPO_ALLOW_ROOT");
  });
