...
import { getRuntimeConfig, readConfigFileSnapshot, type JoopoConfig } from "../config/config.js";
...
function isConfiguredAuthPlugin(plugin: ChannelPlugin, cfg: JoopoConfig): boolean {
  const key = plugin.id;
...
function resolveConfiguredAuthChannelInput(cfg: JoopoConfig, mode: ChannelAuthMode): string {
