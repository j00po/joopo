...
export function registerMigrateCommand(program: Command) {
  const migrate = program
    ...
    .option("--no-backup", "Skip the pre-migration Joopo backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo migrate list", "Show available migration providers."],
          ["joopo migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["joopo migrate hermes --dry-run", "Preview Hermes migration only."],
          [
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing Joopo state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
