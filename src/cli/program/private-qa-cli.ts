...
export function isPrivateQaCliEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.JOOPO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function resolvePrivateQaSourceModuleSpecifier(params?: {
  ...
  argv1?: string;
  moduleUrl?: string;
  resolvePackageRootSync?: typeof resolveJoopoPackageRootSync;
  ...
  const resolvePackageRootSync = params?.resolvePackageRootSync ?? resolveJoopoPackageRootSync;
...
export function loadPrivateQaCliModule(params?: {
  ...
  if (!specifier) {
    throw new Error("Private QA CLI is only available from an Joopo source checkout.");
  }
