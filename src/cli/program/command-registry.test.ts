...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "joopo", "doctor"]);
  ...
  it("narrows to the primary command when command help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "joopo", "doctor", "--help"]);
  ...
  it("keeps all placeholders for root help", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "joopo", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "joopo", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "joopo", "status"], async () => {
      await program.parseAsync(["node", "joopo", "status"]);
    });
