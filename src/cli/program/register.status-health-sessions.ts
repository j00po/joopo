...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo status", "Show channel health + session summary."],
          ["joopo status --all", "Full diagnosis (read-only)."],
          ["joopo status --json", "Machine-readable output."],
          ["joopo status --usage", "Show model provider usage/quota snapshots."],
          [
            "joopo status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["joopo status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.joopo.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.joopo.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo sessions", "List all sessions."],
          ["joopo sessions --agent work", "List sessions for one agent."],
          ["joopo sessions --all-agents", "Aggregate sessions across agents."],
          ["joopo sessions --active 120", "Only last 2 hours."],
          ["joopo sessions --limit 25", "Show the newest 25 sessions."],
          ["joopo sessions --json", "Machine-readable output."],
          ["joopo sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.joopo.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "joopo sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          [
            "joopo sessions cleanup --dry-run --fix-dm-scope",
            "Preview stale direct-DM rows after returning dmScope to main.",
          ],
          ["joopo sessions cleanup --enforce", "Apply maintenance now."],
          ["joopo sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["joopo sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "joopo sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .joopo/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo commitments", "List pending inferred follow-ups."],
          ["joopo commitments --all", "List all inferred follow-ups."],
          ["joopo commitments --agent work", "List one agent's inferred follow-ups."],
          ["joopo commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
