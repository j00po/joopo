...
${formatHelpExamples([
  ['joopo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['joopo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'joopo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'joopo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['joopo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'joopo agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.joopo.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.joopo.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['joopo agents set-identity --agent main --name "Joopo" --emoji "🦞"', "Set name + emoji."],
  ["joopo agents set-identity --agent main --avatar avatars/joopo.png", "Set avatar path."],
  [
    "joopo agents set-identity --workspace ~/.joopo/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "joopo agents set-identity --identity-file ~/.joopo/workspace/IDENTITY.md --agent main",
