...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.JOOPO_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.JOOPO_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.JOOPO_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.JOOPO_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.JOOPO_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.JOOPO_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "joopo", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "joopo"]);

    const names = program.commands.map((cmd) => cmd.name());
    expect(names).toContain("acp");
    expect(names).toContain("gateway");
    expect(names).toContain("joopobot");
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "joopo", "nodes", "list"], "joopo");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "joopo", "infer"], "joopo");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "joopo", "acp", "--help"], "joopo");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "joopo", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "joopo", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("joopo");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "joopo", "completion", "--write-state"];
    const program = new Command().name("joopo");
  ...
  it("keeps plugin CLI registrations available for the plugins command root", async () => {
    process.argv = ["node", "joopo", "plugins"];
    const program = new Command().name("joopo");
