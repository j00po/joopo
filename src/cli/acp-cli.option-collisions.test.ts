...
describe("acp cli option collisions", () => {
  ...
  it.each([
    ...
    await withTempSecretFiles("joopo-acp-cli-", files, async ({ tokenFile, passwordFile }) => {
  ...
  it("trims token file path before reading", async () => {
    await withTempSecretFiles("joopo-acp-cli-", { token: "tok_file\n" }, async (files) => {
  ...
  it("reports missing token-file read errors", async () => {
    await parseAcp(["--token-file", "/tmp/joopo-acp-missing-token.txt"]);
