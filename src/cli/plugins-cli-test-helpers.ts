...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as JoopoConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: JoopoConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/joopo-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: JoopoConfig) =>
    invokeMock<
      [JoopoConfig],
...
export function resetPluginsCliTestState() {
  ...
  enablePluginInConfig.mockImplementation(((cfg: JoopoConfig, pluginId: string) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })) as (...args: unknown[]) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: JoopoConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: JoopoConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
