...
import type { JoopoConfig, ConfigFileSnapshot } from "../config/types.joopo.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: JoopoConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
vi.mock("../infra/joopo-root.js", () => ({
  resolveJoopoPackageRoot: vi.fn(),
  resolveJoopoPackageRootSync: vi.fn(() => process.cwd()),
}));

vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.JOOPO_NIX_MODE === "1") {
      throw new Error(
        [
          ...
          "Agent-first Nix setup: https://github.com/joopo/nix-joopo#quick-start",
          "Joopo Nix overview: https://docs.joopo.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/joopo-config",
  };
});
...
const { resolveJoopoPackageRoot } = await import("../infra/joopo-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/joopo-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as JoopoConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/joopo-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveJoopoPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("joopo-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("joopo-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveJoopoPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("joopo");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["joopo", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/joopo-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "joopo-gateway" }],
      hints: [],
    });
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("joopo-completion-timeout");
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      expect.any(String),
      [path.join(root, "joopo.mjs"), "completion", "--write-state"],
      expect.objectContaining({
        env: expect.objectContaining({
          JOOPO_COMPLETION_SKIP_PLUGIN_COMMANDS: "1",
        }),
        timeout: 30_000,
      }),
    );
  ...
  it("refuses mutating updates in Nix mode before update side effects", async () => {
    await withEnvAsync({ JOOPO_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("JOOPO_NIX_MODE=1");
    });
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("joopo-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logs.some((line) => line.includes("joopo completion --write-state"))).toBe(true);
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          ...
          JOOPO_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.JOOPO_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.JOOPO_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.JOOPO_SERVICE_KIND).toBeUndefined();
  });
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@joopo/demo@1.0.0",
        installPath: "/tmp/joopo-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.JOOPO_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      if (!recordsPath) {
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          JOOPO_UPDATE_POST_CORE: "1",
          JOOPO_UPDATE_POST_CORE_CHANNEL: "dev",
          JOOPO_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
        }),
      }),
    );
    expect(replaceConfigFile).not.toHaveBeenCalled();
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("joopo-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        JOOPO_UPDATE_POST_CORE: "1",
        JOOPO_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("joopo-post-core-result");
    ...
    await withEnvAsync(
      {
        JOOPO_UPDATE_POST_CORE: "1",
        JOOPO_UPDATE_POST_CORE_CHANNEL: "stable",
        JOOPO_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("joopo-post-core-records");
    ...
    await withEnvAsync(
      {
        JOOPO_UPDATE_POST_CORE: "1",
        JOOPO_UPDATE_POST_CORE_CHANNEL: "stable",
        JOOPO_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as JoopoConfig,
      sourceConfig: { update: { channel: "stable" } } as JoopoConfig,
      runtimeConfig: { update: { channel: "stable" } } as JoopoConfig,
      config: { update: { channel: "stable" } } as JoopoConfig,
      hash: "stable-hash",
    });
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @joopo/demo@1.0.0"
    ...
    expect(jsonOutput?.postUpdate?.plugins?.warnings?.[0]).toMatchObject({
      pluginId: "demo",
      guidance: [
        "Run joopo doctor --fix to attempt automatic repair.",
        "Run joopo plugins inspect demo --runtime --json for details.",
      ],
    });
    expect(jsonOutput?.postUpdate?.plugins?.warnings?.[0]?.reason).toContain(
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("joopo-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run joopo doctor --fix to attempt automatic repair.");
    expect(logs).toContain("Run joopo plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; Joopo will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.JOOPO_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
              ...
              warnings: [
                {
                  ...
                  message:
                    'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run joopo doctor --fix to attempt automatic repair. Run joopo plugins inspect demo --runtime --json for details.',
                  guidance: [
                    "Run joopo doctor --fix to attempt automatic repair.",
                    "Run joopo plugins inspect demo --runtime --json for details.",
                  ],
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "joopo", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as JoopoConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "joopo@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("warns but still runs package updates when disk space looks low", async () => {
    ...
    expectPackageInstallSpec("joopo@latest");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("joopo-update"));
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["joopo", "gateway", "run"],
      environment: {
        JOOPO_SERVICE_MARKER: "joopo",
        JOOPO_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors.join("\n")).toContain(
      "joopo update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g joopo` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:joopo/joopo#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("joopo-update"));
        await updateCommand({ yes: true, tag: "github:joopo/joopo#main" });
      },
      expectedSpec: "github:joopo/joopo#main",
    },
    {
      name: "JOOPO_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("joopo-update"));
        await withEnvAsync(
          { JOOPO_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/joopo-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/joopo-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "joopo");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "joopo", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("joopo-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "joopo", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "joopo");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "joopo", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("joopo-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "joopo");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "joopo", version: "2026.4.21" }),
      "utf-8",
    );
  ...
  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("joopo-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "joopo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("joopo-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "joopo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "joopo", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("joopo-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "joopo");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "joopo", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "joopo");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("joopo-update");
    const localAppData = createCaseDir("joopo-localappdata");
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("joopo-update");
    const gitRoot = path.join(tempDir, "..", "joopo");
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `joopo update`.",
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  it.each([
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("joopo-updated-root");
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            JOOPO_CONFIG_PATH: "./config/joopo.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            JOOPO_STATE_DIR: path.resolve("./state"),
            JOOPO_CONFIG_PATH: path.resolve("./config/joopo.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            JOOPO_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ JOOPO_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.JOOPO_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("joopo-update-wizard");
    await withEnvAsync({ JOOPO_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/joopo as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ JOOPO_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "joopo"));
    });
