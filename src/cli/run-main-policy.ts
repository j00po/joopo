...
export function shouldUseRootHelpFastPath(
  ...
  const invocation = resolveCliArgvInvocation(argv);
  return (
    env.JOOPO_DISABLE_CLI_STARTUP_HELP_FAST_PATH !== "1" &&
...
export function shouldUseBrowserHelpFastPath(
  ...
  if (env.JOOPO_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
  const invocation = resolveCliArgvInvocation(argv);
  return (
...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (config?.plugins?.entries?.[parentPluginId]?.enabled === false) {
      return (
        `The \`joopo ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (
      ...
      return (
        `The \`joopo ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`joopo plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`joopo ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
