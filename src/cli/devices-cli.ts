...
function buildFallbackStateMismatchError(details: ConnectPairingRequiredDetails): Error {
  return new Error(
    [
      ...
      "The running gateway is probably using a different JOOPO_PROFILE or JOOPO_STATE_DIR than this CLI.",
...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["joopo", "devices", "approve", requestId];
      ...
      .action(async (requestId: string | undefined, opts: DevicesRpcOpts) => {
        ...
        if (usingImplicitSelection) {
          ...
          switch (approval.kind) {
            ...
            case "re-approval":
              defaultRuntime.log(
                "  Note:   Already paired. Approval-bound device details changed, so Joopo created a fresh request instead of silently reusing the old approval.",
              );
              break;
            case "new-pairing":
