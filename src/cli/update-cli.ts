...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Joopo and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["joopo update", "Update a source checkout (git)"],
        ["joopo update --channel beta", "Switch to beta channel (git + npm)"],
        ["joopo update --channel dev", "Switch to dev channel (git + npm)"],
        ["joopo update --tag beta", "One-off update to a dist-tag or version"],
        ["joopo update --tag main", "One-off package install from GitHub main"],
        ["joopo update --dry-run", "Preview actions without changing anything"],
        ["joopo update --no-restart", "Update without restarting the service"],
        ["joopo update --json", "Output result as JSON"],
        ["joopo update --yes", "Non-interactive (accept downgrade prompts)"],
        ["joopo update wizard", "Interactive update wizard"],
        ["joopo --update", "Shorthand for joopo update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run joopo update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.joopo.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo update status", "Show channel + version status."],
          ["joopo update status --json", "JSON output."],
          ["joopo update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.joopo.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
