...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.JOOPO_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or JOOPO_GATEWAY_PASSWORD.",
  );
}
...
async function maybeLogPendingControlUiBuild(cfg: JoopoConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("joopo setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("joopo onboard --mode local")}\` or \`${formatCliCommand("joopo setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.JOOPO_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.JOOPO_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.JOOPO_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.JOOPO_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (futureStartupBlock && process.env.JOOPO_SERVICE_MARKER?.trim()) {
  ...
  if (process.env.JOOPO_SERVICE_MARKER?.trim()) {
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or JOOPO_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set JOOPO_GATEWAY_TOKEN or JOOPO_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or JOOPO_GATEWAY_TOKEN/JOOPO_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("joopo gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: JOOPO_GATEWAY_TOKEN env if set)",
    )
