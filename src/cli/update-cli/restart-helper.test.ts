...
vi.mock("node:child_process", async () => {
  const { mockNodeBuiltinModule } = await import("joopo/plugin-sdk/test-node-mocks");
...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-JoopoScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-JoopoSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "joopo restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-JoopoListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-JoopoSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        JOOPO_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'joopo-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses JOOPO_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        JOOPO_PROFILE: "default",
        JOOPO_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("joopo-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        JOOPO_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        JOOPO_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped joopo gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart joopo-gateway.service");
      expect(calls).toContain("--user is-active --quiet joopo-gateway.service");
      expect(calls).toContain("is-active --quiet joopo-gateway.service");
      expect(calls).not.toContain("--user restart joopo-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.joopo.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.joopo.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.joopo/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.joopo/logs/gateway-restart.log' 2>&1");
    ...
    it("uses JOOPO_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/joopo-state/logs' 2>/dev/null && : >>'/tmp/joopo-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/joopo-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("joopo restart attempt source=update target=ai.joopo.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.joopo.gateway");
      expect(log).toContain("joopo restart failed source=update status=42");
      expect(log).not.toContain("joopo restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        JOOPO_LAUNCHD_LABEL: "ai.joopo.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        JOOPO_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.joopo.$(echo injected)");
      expect(log).not.toContain("target=ai.joopo.injected");
    });

    it("uses JOOPO_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        JOOPO_PROFILE: "default",
        JOOPO_LAUNCHD_LABEL: "com.custom.joopo",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.joopo'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Joopo Gateway'");
      expect(content).toContain("function Invoke-JoopoSchtasksWithTimeout");
      expect(content).toContain("function Get-JoopoScheduledTaskState");
      expect(content).toContain("function Invoke-JoopoStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("joopo restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".joopo\\gateway.cmd"',
      );
      expect(content).toContain("joopo restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses JOOPO_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        JOOPO_PROFILE: "default",
        JOOPO_WINDOWS_TASK_NAME: "Joopo Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'Joopo Gateway (custom)'");
      expect(content).toContain("Get-JoopoScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-JoopoSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-JoopoStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        JOOPO_PROFILE: "production",
      });
      expect(content).toContain("joopo-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        JOOPO_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.joopo.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'Joopo Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        JOOPO_LAUNCHD_LABEL: "ai.joopo.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.joopo.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
