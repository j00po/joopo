...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.JOOPO_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.JOOPO_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.JOOPO_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.JOOPO_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.JOOPO_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.JOOPO_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `joopo-restart-${timestamp}.sh`;
...
exec 3>&2
${logSetup}
printf '[%s] joopo restart attempt source=update target=%s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  if systemctl --user restart '${escaped}'; then
    status=0
    printf '[%s] joopo restart done source=update\\n' "$(date -u +%FT%TZ)" >&2
  else
    status=$?
    printf '[%s] joopo restart failed source=update status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
  fi
elif systemctl is-active --quiet '${escaped}' || systemctl is-enabled --quiet '${escaped}'; then
  status=78
  printf '[%s] system-scoped joopo gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
# Self-cleanup (log is retained under the Joopo state logs directory).
rm -f "$0"
exit "$status"
`;
      ...
      filename = `joopo-restart-${timestamp}.cmd`;
...
setlocal
set "JOOPO_RESTART_SCRIPT=%~f0"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:JOOPO_RESTART_SCRIPT; $s=Get-Content -Raw -LiteralPath $p; $m='# POWERSHELL'; $i=$s.IndexOf($m); if ($i -lt 0) { exit 1 }; Invoke-Expression $s.Substring($i)"
...
try {
  ...
  Add-Content -LiteralPath $logPath -Value "[$(Get-Date -Format o)] joopo restart log initialized"
} catch {
...
function Join-JoopoProcessArguments {
...
function Invoke-JoopoSchtasksWithTimeout {
  param(
    [string[]]$Arguments,
    [int]$TimeoutSeconds
  )
  ...
  try {
    ...
    $startInfo.Arguments = Join-JoopoProcessArguments -Arguments $Arguments
    ...
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      ...
      Write-RestartLog "joopo restart schtasks timeout source=update args=$($Arguments -join ' ')"
      return 124
    }
    ...
    Write-RestartLog "joopo restart schtasks failed source=update args=$($Arguments -join ' ') error=$($_.Exception.Message)"
    return 1
  }
}

function Get-JoopoScheduledTaskState {
...
function Get-JoopoListenerPids {
  param([int]$Port)
  $listenerPids = @()
...
function Invoke-JoopoStartupLauncher {
  $launcherPath = Join-Path $env:USERPROFILE ".joopo\\gateway.cmd"
  if (-not (Test-Path -LiteralPath $launcherPath)) {
    Write-RestartLog "joopo restart startup launcher missing source=update path=$launcherPath"
    return 1
  }

  try {
    Start-Process -FilePath $launcherPath -WindowStyle Hidden | Out-Null
    Write-RestartLog "joopo restart launched startup fallback source=update path=$launcherPath"
    return 0
  } catch {
    Write-RestartLog "joopo restart startup fallback failed source=update error=$($_.Exception.Message)"
    return 1
  }
}
...
$port = ${port}
Write-RestartLog "joopo restart attempt source=update target=$taskName"

$taskState = Get-JoopoScheduledTaskState -TaskName $taskName
if ($taskState -eq "Running") {
  $endStatus = Invoke-JoopoSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10
  if ($endStatus -ne 0) {
    Write-RestartLog "joopo restart schtasks end did not complete cleanly source=update status=$endStatus"
  }
} else {
  Write-RestartLog "joopo restart skipped schtasks end source=update state=$taskState"
}

for ($attempt = 1; $attempt -le 10; $attempt++) {
  $listeners = @(Get-JoopoListenerPids -Port $port)
  ...
  if ($attempt -eq 10) {
    foreach ($listenerPid in $listeners) {
      try {
        ...
        Write-RestartLog "joopo restart killed stale listener source=update pid=$listenerPid"
      } catch {
        Write-RestartLog "joopo restart failed to kill stale listener source=update pid=$listenerPid error=$($_.Exception.Message)"
      }
    }
    break
  }
...
$status = Invoke-JoopoSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30
if ($status -ne 0) {
  $status = Invoke-JoopoStartupLauncher
}
if ($status -eq 0) {
  Write-RestartLog "joopo restart done source=update"
} else {
  Write-RestartLog "joopo restart failed source=update status=$status"
}
