...
const STEP_LABELS: Record<string, string> = {
  ...
  "joopo doctor entry": "Checking doctor entrypoint",
  "joopo doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g joopo@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g joopo@latest --omit=optional");
  }
