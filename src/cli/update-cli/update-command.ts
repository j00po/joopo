...
const POST_CORE_UPDATE_ENV = "JOOPO_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "JOOPO_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "JOOPO_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "JOOPO_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "JOOPO_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_RESULT_POLL_MS = 100;
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "JOOPO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
...
const POST_UPDATE_PLUGIN_REPAIR_GUIDANCE = "Run joopo doctor --fix to attempt automatic repair.";
...
const UPDATE_QUIPS = [
  ...
  "Reborn from npm with a smaller blast radius. Stronger now.",
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run joopo plugins inspect ${pluginId} --runtime --json for details.`;
}
...
function formatPostUpdateGatewayRecoveryInstructions(result: UpdateRunResult): string[] {
  const lines = [
    `Recovery: run \`${replaceCliName(formatCliCommand("joopo gateway restart"), CLI_NAME)}\`; if macOS reports the LaunchAgent is installed but not loaded, run \`${replaceCliName(formatCliCommand("joopo gateway install --force"), CLI_NAME)}\` from the logged-in user session, then rerun \`${replaceCliName(formatCliCommand("joopo gateway status --deep"), CLI_NAME)}\`.`,
  ];
  const beforeVersion = normalizeOptionalString(result.before?.version);
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall Joopo ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("joopo gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `joopo update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("joopo update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
function isRunningInsideGatewayService(
  ...
  if (env.JOOPO_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.JOOPO_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `Node ${process.versions.node ?? "unknown"} is too old for joopo@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    "Upgrade Node to 22.16+ or Node 24, then rerun `joopo update`.",
    "Bare `npm i -g joopo` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g joopo@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.JOOPO_SERVICE_MARKER;
  delete resolvedEnv.JOOPO_SERVICE_KIND;
  return resolvedEnv;
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("joopo completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (expectedGatewayVersion: string | undefined) => {
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("joopo gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.JOOPO_UPDATE_IN_PROGRESS = "1";
        ...
        try {
          ...
          delete process.env.JOOPO_UPDATE_IN_PROGRESS;
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("joopo gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (isPackageManagerUpdateMode(params.result.mode)) {
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("joopo doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("joopo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("joopo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-update-post-core-"));
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  const devTargetRef =
    channel === "dev" ? process.env.JOOPO_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Joopo..."));
    defaultRuntime.log("");
  }
    ...
    if (shouldBlockPackageUpdateFromGatewayServiceEnv({ prePackageServiceStop })) {
      ...
      defaultRuntime.error(
        [
          ...
          "That path replaces the active Joopo dist tree while the live gateway may still lazy-load old chunks.",
          `Run \`${replaceCliName(formatCliCommand("joopo update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`,
        ].join("\n"),
      );
      defaultRuntime.exit(1);
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `joopo update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Joopo install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("joopo doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("joopo gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g joopo@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g joopo@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
