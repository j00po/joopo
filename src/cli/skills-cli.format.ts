...
export function formatSkillInfo(
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`joopo config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$JOOPO_CONFIG_PATH")} ${theme.muted("(default: ~/.joopo/joopo.json)")}`,
    );
  }
