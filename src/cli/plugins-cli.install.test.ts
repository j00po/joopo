...
import { installedPluginRoot } from "joopo/plugin-sdk/test-fixtures";
...
const CLI_STATE_ROOT = "/tmp/joopo-state";
const ORIGINAL_JOOPO_STATE_DIR = process.env.JOOPO_STATE_DIR;
const ORIGINAL_JOOPO_NIX_MODE = process.env.JOOPO_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/joopo-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.JOOPO_STATE_DIR = PROFILE_STATE_ROOT;
  return path.join(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): JoopoConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): JoopoConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@joopo/${pluginId}`,
    npmTarballName: `joopo-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@joopo/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@joopo/${pluginId}@1.2.3`,
...
function createPathHookPackInstalledConfig(tmpRoot: string): JoopoConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): JoopoConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_JOOPO_STATE_DIR === undefined) {
      delete process.env.JOOPO_STATE_DIR;
    } else {
      process.env.JOOPO_STATE_DIR = ORIGINAL_JOOPO_STATE_DIR;
    }
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `joopo doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("does not persist incomplete config entries for config-gated bundled installs", async () => {
    ...
    const writtenConfig = writeConfigFile.mock.calls.at(-1)?.[0] as JoopoConfig;
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing joopo.extensions",
      code: "missing_joopo_extensions",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-joopo-plugin@2026.4.23",
    });
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/joopo-demo-1.2.3.tgz";
    ...
    expect(writePersistedInstalledPluginIndexInstallRecords).toHaveBeenCalledWith({
      demo: expect.objectContaining({
        ...
        npmTarballName: "joopo-demo-1.2.3.tgz",
      }),
    });
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@joopo/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@joopo/discord"]);
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-joopo-plugin@latest"]);
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as JoopoConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the active profile extensions dir to local path installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-local-plugin-"));
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `joopo plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-local-hook-pack-"));
    const cfg = {} as JoopoConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("does not fall back to npm when explicit JoopoHub rejects a real package", async () => {
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "joopo skills install demo" instead.');
  });
