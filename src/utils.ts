...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".joopo");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.JOOPO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$JOOPO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via JOOPO_STATE_DIR.
