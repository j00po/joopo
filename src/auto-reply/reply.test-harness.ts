...
type HomeEnvSnapshot = {
  ...
  JOOPO_STATE_DIR: string | undefined;
  JOOPO_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    JOOPO_AGENT_DIR: process.env.JOOPO_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".joopo", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.JOOPO_STATE_DIR = path.join(home, ".joopo");
    process.env.JOOPO_AGENT_DIR = path.join(home, ".joopo", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".joopo", "agent");
