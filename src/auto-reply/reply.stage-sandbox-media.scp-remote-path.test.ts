...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "joopo"),
    sessionKey,
    remoteCacheDir: join(home, ".joopo", "media", "remote-cache", slugifySessionKey(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("joopo-triggers-", async (home) => {
