...
const hoisted = await vi.hoisted(async () => {
  ...
  return {
    ...
    exportTrajectoryBundleMock: vi.fn(() => ({
      outputDir: "/tmp/workspace/.joopo/trajectory-exports/joopo-trajectory-session",
      manifest: {
        eventCount: 7,
        runtimeEventCount: 3,
    ...
    resolveDefaultTrajectoryExportDirMock: vi.fn(
      () => "/tmp/workspace/.joopo/trajectory-exports/joopo-trajectory-session",
    ),
    accessMock: vi.fn(
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-export-command-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "joopo sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryReply", () => {
  ...
  it("builds a trajectory bundle from the target session", async () => {
    ...
    expect(hoisted.exportTrajectoryBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionId: "session-1",
        sessionKey: "agent:target:session",
        workspaceDir: expect.stringContaining("joopo-export-command-"),
      }),
    );
  });
  ...
  it("keeps user-named output paths inside the workspace trajectory export directory", async () => {
    ...
    expect(hoisted.exportTrajectoryBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({
        outputDir: path.join(params.workspaceDir, ".joopo", "trajectory-exports", "my-bundle"),
      }),
    );
  });
  ...
  it("rejects output paths redirected by a symlinked exports directory", async () => {
    ...
    fs.mkdirSync(path.join(workspaceDir, ".joopo"), { recursive: true });
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".joopo", "trajectory-exports"));
  ...
  it("rejects symlinked state directories before creating export folders", async () => {
    ...
    const workspaceDir = makeTempDir();
    const outsideDir = makeTempDir();
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".joopo"));
    const params = makeParams(workspaceDir);
    params.command.commandBodyNormalized = "/export-trajectory my-bundle";
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.joopo.ai/tools/trajectory");
    ...
    expect(command).not.toContain("joopo sessions export-trajectory");
    const request = readEncodedRequestFromCommand(command);
    expect(request).toMatchObject({
      sessionKey: "agent:target:session",
      workspace: expect.stringContaining("joopo-export-command-"),
    });
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("joopo sessions export-trajectory");
