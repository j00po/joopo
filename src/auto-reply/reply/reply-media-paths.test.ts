...
describe("createReplyMediaPathNormalizer", () => {
  ...
  it("stages absolute workspace media paths so the PR scenario now works", async () => {
    const absolutePath = "/Users/peter/.joopo/workspace/exports/images/chart.png";
    const normalize = createReplyMediaPathNormalizer({
      cfg: { agents: { defaults: { mediaMaxMb: 8 } } },
      sessionKey: "session-key",
      workspaceDir: "/Users/peter/.joopo/workspace",
    });
  ...
  it("keeps managed generated media under the shared media root", async () => {
    vi.stubEnv("JOOPO_STATE_DIR", "/Users/peter/.joopo");
    const normalize = createReplyMediaPathNormalizer({
      cfg: {},
      sessionKey: "session-key",
      workspaceDir: "/tmp/agent-workspace",
    ...
    const result = await normalize({
      mediaUrls: ["/Users/peter/.joopo/media/tool-image-generation/generated.png"],
    });

    expect(result).toMatchObject({
      mediaUrl: "/Users/peter/.joopo/media/tool-image-generation/generated.png",
      mediaUrls: ["/Users/peter/.joopo/media/tool-image-generation/generated.png"],
    });
    expect(resolveOutboundAttachmentFromUrl).not.toHaveBeenCalled();
  });
  ...
  it("keeps managed outbound media under the shared media root with sandbox mapping", async () => {
    ...
    const result = await normalize({
      mediaUrls: ["/Users/peter/.joopo/media/outbound/generated.png"],
    });

    expect(result).toMatchObject({
      mediaUrl: "/Users/peter/.joopo/media/outbound/generated.png",
      mediaUrls: ["/Users/peter/.joopo/media/outbound/generated.png"],
    });
    expect(resolveOutboundAttachmentFromUrl).not.toHaveBeenCalled();
  });
  ...
  it("drops managed outbound media symlinks escaping the shared media root without sandbox mapping", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-reply-media-state-"));
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-reply-media-outside-"));
