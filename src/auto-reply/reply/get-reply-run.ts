...
type AgentDefaults = NonNullable<JoopoConfig["agents"]>["defaults"];
...
type RunPreparedReplyParams = {
  ...
  sessionCfg: JoopoConfig["session"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.JOOPO_TEST_FAST === "1",
  });
  ...
  const transcriptBodyBase = isHeartbeat
    ...
    : isBareSessionReset
      ? softResetTail || `[Joopo session ${startupAction}]`
      : hasUserBody
        ? baseBodyFinal
