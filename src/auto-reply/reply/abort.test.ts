...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop joopo!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@joopo_bot", { botUsername: "joopo_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@joopo_bot", { botUsername: "joopo_bot" })).toBe(true);
