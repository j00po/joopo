...
import { buildCurrentJoopoCliCommand } from "./commands-joopo-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.joopo.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "joopo gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentJoopoCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Joopo session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Joopo session file.",
    ) === true
  );
}
