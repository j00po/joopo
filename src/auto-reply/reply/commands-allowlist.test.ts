...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-allowlist-config-"));
  const configPath = path.join(dir, "joopo.json");
  const previous = process.env.JOOPO_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.JOOPO_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<JoopoConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: JoopoConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
