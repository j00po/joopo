...
function buildDiagnosticsParams(
  ...
  return {
    cfg: { commands: { text: true } } as JoopoConfig,
    ctx: {
      Provider: "whatsapp",
      Surface: "whatsapp",
      CommandSource: "text",
      ...
      return (
        options.execResult ?? {
          ...
          details: {
            ...
            command: "joopo gateway diagnostics export --json",
            cwd: "/tmp",
          },
        }
      );
...
describe("diagnostics command", () => {
  it("requests Gateway diagnostics approval without a duplicate pending chat reply", async () => {
    ...
    ).toContain("https://docs.joopo.ai/gateway/diagnostics");
    ...
    expect(command).not.toBe("joopo gateway diagnostics export --json");
  });
  ...
  it("falls back to a visible reply when approval cannot be queued", async () => {
    ...
    expect(result?.reply?.text).toContain("https://docs.joopo.ai/gateway/diagnostics");
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, Joopo session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
