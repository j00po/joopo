...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: joopo doctor --non-interactive"),
  ...
  triggerJoopoRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerJoopoRestart: mocks.triggerJoopoRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerJoopoRestart.mockReset();
    mocks.triggerJoopoRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerJoopoRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerJoopoRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerJoopoRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
