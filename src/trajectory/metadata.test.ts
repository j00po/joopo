...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/joopo.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.joopo/joopo.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          JOOPO_STATE_DIR: "/Users/tester/.joopo",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$JOOPO_STATE_DIR/joopo.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/joopo.js");
