...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: JoopoConfig) => ChannelHealthMonitor | null;
  onCronRestart?: () => void;
};
...
> & {
  ...
  initialConfig: JoopoConfig;
  initialCompareConfig?: JoopoConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: JoopoConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: JoopoConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (JOOPO_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (JOOPO_SKIP_CHANNELS=1 or JOOPO_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: JoopoConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
