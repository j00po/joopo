  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-joopo-account-id": "work",
        "x-joopo-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-joopo-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.joopo?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.joopo?.headers?.Authorization).toBe(
      "Bearer ${JOOPO_MCP_TOKEN}",
    );
    expect(config.mcpServers?.joopo?.headers?.["x-joopo-message-channel"]).toBe(
      "${JOOPO_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.joopo?.headers?.["x-joopo-sender-is-owner"]).toBeUndefined();
  });
});
