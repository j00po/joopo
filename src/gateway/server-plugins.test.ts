...
const loadJoopoPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadLogger(): {
  ...
  const call = loadJoopoPlugins.mock.calls.at(-1)?.[0] as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadJoopoPlugins.mockReturnValue(createRegistry([]));
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", () => {
    ...
    loadJoopoPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", () => {
    ...
    loadJoopoPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", () => {
    ...
    expect(loadJoopoPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.joopo.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", () => {
    ...
    loadJoopoPlugins.mockReturnValue(pluginRegistry);
