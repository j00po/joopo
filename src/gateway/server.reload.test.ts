...
const hoisted = vi.hoisted(() => {
  ...
  const resolveJoopoPackageRootSync = vi.fn((_params: unknown) => "/package");
...
vi.mock("../infra/joopo-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/joopo-root.js")>();
  return {
    ...actual,
    resolveJoopoPackageRootSync: hoisted.resolveJoopoPackageRootSync,
  };
});
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    prevSkipChannels = process.env.JOOPO_SKIP_CHANNELS;
    prevSkipGmail = process.env.JOOPO_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.JOOPO_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.JOOPO_SKIP_CHANNELS = "0";
    ...
    hoisted.resolveJoopoPackageRootSync.mockClear();
    hoisted.resolveJoopoPackageRootSync.mockReturnValue("/package");
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.JOOPO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.JOOPO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.JOOPO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.JOOPO_SKIP_PROVIDERS;
    } else {
      process.env.JOOPO_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("JOOPO_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ JOOPO_TEST_MINIMAL_GATEWAY: undefined }, async () =>
...
`,
    ...
    const previousGatewayTokenEnv = process.env.JOOPO_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.JOOPO_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
