...
import type { ConfigFileSnapshot, JoopoConfig } from "../config/types.joopo.js";
...
function asPluginInstallConfig(records: PluginInstallRecords): JoopoConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: JoopoConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: JoopoConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: JoopoConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
