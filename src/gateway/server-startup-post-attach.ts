...
import type { loadJoopoPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "JOOPO_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: JoopoConfig): GatewayMemoryStartupPolicy {
...
function resolveRestartSentinelPathFast(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.JOOPO_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.JOOPO_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".joopo");
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadJoopoPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { ensureJoopoModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureJoopoModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: JoopoConfig;
  pluginRegistry: ReturnType<typeof loadJoopoPlugins>;
  ...
  await measureStartup(params.startupTrace, "sidecars.channels", async () => {
    if (!skipChannels) {
      ...
      params.logChannels.info(
        "skipping channel start (JOOPO_SKIP_CHANNELS=1 or JOOPO_SKIP_PROVIDERS=1)",
      );
    }
  });
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: JoopoConfig;
    pluginRegistry: ReturnType<typeof loadJoopoPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
