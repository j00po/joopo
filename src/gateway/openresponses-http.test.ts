...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("JOOPO_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "joopo", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `joopo` or `joopo/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "joopo", input: "hi" },
        { "x-joopo-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "joopo/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "joopo/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "joopo", input: "hi" },
        { "x-joopo-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-joopo-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats write-scoped HTTP callers as non-owner and admin-scoped callers as owner", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "joopo", input: "hi" },
      { "x-joopo-scopes": "operator.admin, operator.write" },
    );
