...
export function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("joopo doctor --fix")}" to repair, then retry.`
    : "";
...
function hasActiveGatewayAuthSecretRef(config: JoopoConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: JoopoConfig): JoopoConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: JoopoConfig): void {
