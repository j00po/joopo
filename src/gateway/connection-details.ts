...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: JoopoConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.JOOPO_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.JOOPO_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set JOOPO_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: joopo doctor --fix",
