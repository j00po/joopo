...
export function attachJoopoTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__joopo && typeof record.__joopo === "object" && !Array.isArray(record.__joopo)
      ? (record.__joopo as Record<string, unknown>)
      : {};
  return {
    ...record,
    __joopo: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __joopo: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachJoopoTranscriptMeta(message, { seq: firstSeq + index }),
  );
...
function parsedSessionEntryToMessage(parsed: unknown, seq: number): unknown {
  ...
  if (entry.message) {
    return attachJoopoTranscriptMeta(entry.message, {
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "joopo" && model === "delivery-mirror";
