...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("joopo.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("joopo.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `joopo-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.JOOPO_CONFIG_PATH = path.join(root, "joopo.json");
}
