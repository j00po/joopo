...
function gatewayTokenConfig(config: JoopoConfig): JoopoConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): JoopoConfig {
  return value as JoopoConfig;
}

function buildSnapshot(config: JoopoConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/joopo-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: JoopoConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.JOOPO_SKIP_CHANNELS;
  const previousSkipProviders = process.env.JOOPO_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.JOOPO_SKIP_CHANNELS;
    } else {
      process.env.JOOPO_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.JOOPO_SKIP_PROVIDERS;
    } else {
      process.env.JOOPO_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
