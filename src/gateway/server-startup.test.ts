...
const ensureJoopoModelsJsonMock = vi.fn<
  (
    config: unknown,
    agentDir: unknown,
    options?: unknown,
...
vi.mock("../agents/models-config.js", () => ({
  ensureJoopoModelsJson: (config: unknown, agentDir: unknown, options?: unknown) =>
    ensureJoopoModelsJsonMock(config, agentDir, options),
}));
...
describe("gateway startup primary model warmup", () => {
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureJoopoModelsJsonMock).toHaveBeenCalledWith(
      cfg,
      "/tmp/agent",
  ...
  it("honors the startup model prewarm skip env", () => {
    ...
    expect(
      shouldSkipStartupModelPrewarm({
        JOOPO_SKIP_STARTUP_MODEL_PREWARM: "true",
      }),
    ).toBe(true);
  });
  ...
  it("warns when scoped models.json preparation fails", async () => {
    ensureJoopoModelsJsonMock.mockRejectedValueOnce(new Error("models write failed"));
    const warn = vi.fn();
