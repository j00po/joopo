...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("joopo doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.JOOPO_PROXY_ACTIVE;
    delete process.env.JOOPO_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// to private addresses only with JOOPO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with JOOPO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.JOOPO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://joopo-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      JOOPO_STATE_DIR: "/tmp/joopo-client-service-state",
