...
import type { ConfigFileSnapshot, ModelDefinitionConfig, JoopoConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: JoopoConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: JoopoConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: JoopoConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/joopo-startup-recovery.json";
const validConfig = {
  gateway: {
    mode: "local",
  },
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires Joopo >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
      legacyIssues: [],
    });
