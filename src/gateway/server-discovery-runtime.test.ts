...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn(() => "joopo.internal."),
  writeWideAreaGatewayZone: vi.fn(async () => ({
    changed: true,
    zonePath: "/tmp/joopo.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (Joopo)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/joopo"),
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    ...
    process.env.JOOPO_SSH_PORT = "2222";
  ...
  it("continues startup when a local discovery service never settles", async () => {
    ...
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.JOOPO_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "10";
  ...
  it("skips local discovery services for truthy JOOPO_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.JOOPO_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "joopo.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
