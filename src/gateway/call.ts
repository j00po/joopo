...
function loadGatewayConfig(): JoopoConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: JoopoConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.JOOPO_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.JOOPO_TEST_HANDSHAKE_TIMEOUT_MS);
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.JOOPO_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as JoopoConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `joopo doctor` for diagnostics.";
  }
  return message;
}
