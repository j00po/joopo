...
let lastCreateJoopoToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/joopo-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateJoopoToolsContext?.agentTo,
          agentThreadId: lastCreateJoopoToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createJoopoTools: (ctx: Record<string, unknown>) => {
      lastCreateJoopoToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateJoopoToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-joopo-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-joopo-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateJoopoToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateJoopoToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateJoopoToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateJoopoToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateJoopoToolsContext?.pluginToolAllowlist).toEqual(
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(lastCreateJoopoToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateJoopoToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateJoopoToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-joopo-message-to": "channel:24514",
        "x-joopo-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
