...
import { ensureJoopoCliOnPath } from "../infra/path-env.js";
...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.JOOPO_GATEWAY_STARTUP_TRACE);
  let timelineConfig: JoopoConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.JOOPO_DIAGNOSTICS_EVENT_LOOP);
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`joopo config set gateway.auth.mode token\` and \`joopo config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed Joopo config and rebuild.",
    "For the first-party Nix flow, see https://github.com/joopo/nix-joopo#quick-start and https://docs.joopo.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing joopo.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.JOOPO_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.JOOPO_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "JOOPO_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "JOOPO_RAW_STREAM_PATH",
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: JoopoConfig) =>
  ...
  const getReadiness = createReadinessChecker({
    ...
    shouldSkipChannelReadiness: () =>
      isTruthyEnvValue(process.env.JOOPO_SKIP_CHANNELS) ||
      isTruthyEnvValue(process.env.JOOPO_SKIP_PROVIDERS),
  });
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: JoopoConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
