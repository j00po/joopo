...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.JOOPO_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.JOOPO_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: JoopoConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-joopo-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-joopo-account-id")),
