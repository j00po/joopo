...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "joopo-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".joopo", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["JOOPO_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "joopo");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.JOOPO_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".joopo");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "joopo-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-wizard-home-"));
      const configPath = path.join(tempHome, ".joopo", "joopo.json");
      process.env.HOME = tempHome;
      process.env.JOOPO_STATE_DIR = path.join(tempHome, ".joopo");
      process.env.JOOPO_CONFIG_PATH = configPath;
      process.env.JOOPO_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".joopo", "joopo.json");
      const bundledPluginsDir = path.join(tempHome, "joopo-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.JOOPO_STATE_DIR = path.join(tempHome, ".joopo");
      process.env.JOOPO_CONFIG_PATH = configPath;
      process.env.JOOPO_SKIP_CHANNELS = "1";
      ...
      process.env.JOOPO_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.JOOPO_SKIP_PROVIDERS = "1";
      process.env.JOOPO_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.JOOPO_TEST_MINIMAL_GATEWAY = "1";
