export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Joopo session status reports the active model as:",
] as const;
...
export const EXPECTED_CODEX_STATUS_COMMAND_TEXT = [
  ...
  "Joopo status shown above.",
  "Status shown above.",
] as const;

export function isExpectedCodexStatusCommandText(text: string): boolean {
  const normalized = text.toLowerCase();
  const mentionsJoopoStatus =
    normalized.includes("joopo is running on") ||
    /joopo\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("joopo status:") ||
  ...
  return (
    ...
    (mentionsJoopoStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from joopo status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live joopo config shows") ||
