...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.JOOPO_CONFIG_PATH) {
    configPaths.add(process.env.JOOPO_CONFIG_PATH);
  }
  if (process.env.JOOPO_STATE_DIR) {
    configPaths.add(path.join(process.env.JOOPO_STATE_DIR, "joopo.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.JOOPO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "joopo-test-no-bundled-extensions")
    : "joopo-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".joopo-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".joopo-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".joopo-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>joopo-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__joopoTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__joopoTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["JOOPO_GATEWAY_TOKEN"]);
  const prev = process.env.JOOPO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.JOOPO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.JOOPO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
