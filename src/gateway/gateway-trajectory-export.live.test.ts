...
const CODEX_HARNESS_LIVE = process.env.JOOPO_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.JOOPO_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["JOOPO_TRAJECTORY", "JOOPO_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-joopo-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.JOOPO_TRAJECTORY = "1";
      process.env.JOOPO_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".joopo", "trajectory-exports", "bundle");
