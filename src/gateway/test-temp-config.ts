...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "joopo-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.JOOPO_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as JoopoConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "joopo-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.JOOPO_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
