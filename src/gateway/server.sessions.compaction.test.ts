...
test("sessions.patch preserves nested model ids under provider overrides", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-gw-sessions-nested-"));
  ...
  await withEnvAsync({ JOOPO_CONFIG_PATH: undefined }, async () => {
    ...
    await withEnvAsync({ JOOPO_CONFIG_PATH: configPath }, async () => {
