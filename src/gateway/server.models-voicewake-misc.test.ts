...
describe("gateway server models + voicewake", () => {
  const listModels = async (params?: { view?: "default" | "configured" | "all" }) =>
    withEnvAsync({ JOOPO_DISABLE_BUNDLED_PLUGINS: "1" }, async () =>
      params
  ...
  const withModelsConfig = async <T>(config: unknown, run: () => Promise<T>): Promise<T> => {
    ...
    if (!configPath) {
      throw new Error("Missing JOOPO_CONFIG_PATH");
    }
  ...
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("joopo-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["joopo", "claude", "computer"]);
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".joopo", "settings", "voicewake.json"), "utf8"),
  ...
  test("voicewake.routing.get/set persists and broadcasts", { timeout: 60_000 }, async () => {
    await withTempHome(async (homeDir) => {
      ...
      const onDisk = JSON.parse(
        await fs.readFile(
          path.join(homeDir, ".joopo", "settings", "voicewake-routing.json"),
          "utf8",
        ),
      ) as { routes?: unknown };
