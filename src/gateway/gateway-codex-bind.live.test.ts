...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.JOOPO_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const joopoDir = path.join(params.homeDir, ".joopo");
  await fs.mkdir(joopoDir, { recursive: true });
  await fs.writeFile(
    path.join(joopoDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.JOOPO_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-live-codex-bind-"));
      const tempHome = path.join(tempRoot, "home");
      const stateDir = path.join(tempRoot, "state");
      const workspace = path.join(tempRoot, "workspace");
      const configPath = path.join(tempRoot, "joopo.json");
      ...
      const bindModel =
        process.env.JOOPO_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("JOOPO_CONFIG_PATH", previous.configPath);
        restoreEnvVar("JOOPO_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("JOOPO_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("JOOPO_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("JOOPO_SKIP_CRON", previous.skipCron);
        restoreEnvVar("JOOPO_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("JOOPO_STATE_DIR", previous.stateDir);
      }
    },
