...
const { createJoopoTools } = await import("../agents/joopo-tools.js");
...
type SessionSendTool = ReturnType<typeof createJoopoTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createJoopoTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["JOOPO_GATEWAY_PORT", "JOOPO_GATEWAY_TOKEN"]);
  ...
  process.env.JOOPO_GATEWAY_PORT = String(gatewayPort);
  process.env.JOOPO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("JOOPO_CONFIG_PATH missing in gateway test environment");
      }
