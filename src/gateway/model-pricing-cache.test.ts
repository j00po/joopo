...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listJoopoPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listJoopoPluginManifestMetadata: (
      params?: Parameters<typeof actual.listJoopoPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listJoopoPluginManifestMetadata(params);
      return actual.listJoopoPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listJoopoPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as JoopoConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listJoopoPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/joopo.plugin.json",
    ...overrides,
  };
}
