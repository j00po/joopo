...
const hoisted = vi.hoisted(() => {
  ...
  const resolveDefaultAgentDir = vi.fn(() => "/tmp/joopo-state/agents/default/agent");
  ...
  const ensureJoopoModelsJson = vi.fn(async () => undefined);
  return {
    startPluginServices,
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/joopo-state",
    resolveConfigPath: vi.fn(() => "/tmp/joopo-state/joopo.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/joopo-state"),
  };
});
...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/joopo-workspace"),
...
vi.mock("../agents/models-config.js", () => ({
  ensureJoopoModelsJson: hoisted.ensureJoopoModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("JOOPO_SKIP_CHANNELS", "0");
    vi.stubEnv("JOOPO_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureJoopoModelsJson.mockReset();
    hoisted.ensureJoopoModelsJson.mockResolvedValue(undefined);
  });
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-home-"));
    try {
      const joopoHome = path.join(osHome, "joopo-home");
      const stateDirFromHome = path.join(joopoHome, ".joopo");
      ...
      expect(
        __testing.hasRestartSentinelFileFast({
          HOME: osHome,
          JOOPO_HOME: "~/joopo-home",
        } as NodeJS.ProcessEnv),
      ).toBe(true);

      const backslashStateDir = path.resolve(`${osHome}\\joopo-state`);
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/joopo-workspace",
        deps: {} as never,
  ...
  it("prewarms models.json in the configured default agent dir", async () => {
    ...
    hoisted.resolveDefaultAgentDir.mockReturnValue("/tmp/joopo-state/agents/ops/agent");
    ...
    expect(hoisted.ensureJoopoModelsJson).toHaveBeenCalledWith(
      cfg,
      "/tmp/joopo-state/agents/ops/agent",
  ...
  it("starts channels without waiting for primary model prewarm completion", async () => {
    await withEnvAsync(
      { JOOPO_SKIP_CHANNELS: undefined, JOOPO_SKIP_PROVIDERS: undefined },
      async () => {
