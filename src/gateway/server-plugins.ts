...
import { clearActivatedPluginRuntimeState, loadJoopoPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "joopo.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "joopo.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: JoopoConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.joopo.ai/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadJoopoPlugins({
    config: resolvedConfig,
