...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("joopo-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __joopo?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__joopo?.kind).toBe("compaction");
    expect(marker.__joopo?.id).toBe("comp-1");
  ...
  test("reads recent messages from the transcript tail without loading the whole file", () => {
    ...
    expect(out).toEqual([
      expect.objectContaining({
        role: "user",
        content: "recent",
        __joopo: expect.objectContaining({ seq: 3 }),
      }),
      expect.objectContaining({
        role: "assistant",
        content: "latest",
        __joopo: expect.objectContaining({ seq: 4 }),
      }),
    ]);
  });
  ...
  test("reads active tree branch asynchronously without SessionManager.open", async () => {
    ...
    try {
      ...
      expect(messages[2]).toMatchObject({
        __joopo: expect.objectContaining({ id: "user-2", seq: 3 }),
      });
      expect(sessionManagerOpenSpy).not.toHaveBeenCalled();
      expect(readFileSpy).not.toHaveBeenCalled();
    } finally {
    ...
    try {
      ...
      expect(out).toEqual([
        expect.objectContaining({
          ...
          __joopo: expect.objectContaining({ seq: 1 }),
        }),
        expect.objectContaining({
          role: "assistant",
          content: [{ type: "text", text: "clean answer" }],
          __joopo: expect.objectContaining({ seq: 2 }),
        }),
      ]);
  ...
  test.each([
    ...
    ({ sessionId, sessionFileParts, wrongStorePathParts, message }) => {
      ...
      expect((out[0] as { __joopo?: { seq?: number } }).__joopo?.seq).toBe(1);
    },
  );
  ...
  test("keeps compaction markers when reading only the active SessionManager branch", () => {
    ...
    expect(
      out.map((message) => ({
        role: (message as { role?: string }).role,
        content: (message as { content?: unknown }).content,
        kind: (message as { __joopo?: { kind?: string } }).__joopo?.kind,
      })),
    ).toEqual([
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("joopo-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  registerTempSessionStore("joopo-session-usage-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses JOOPO_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/joopo-home"), ".joopo", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test.each([
    {
      storePath: "/tmp/joopo/agents/main/sessions/sessions.json",
      sessionFile: "/tmp/joopo/agents/ops/sessions/sess-safe.jsonl",
    },
    {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/joopo/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/joopo/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
  ...
  test("prefers the current sessionId transcript before a stale sessionFile candidate", () => {
    ...
    const candidates = resolveSessionTranscriptCandidates(
      ...
      storePath,
      "/tmp/joopo/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl",
    );

    expect(candidates[0]).toBe(
      path.resolve("/tmp/joopo/agents/main/sessions/11111111-1111-4111-8111-111111111111.jsonl"),
    );
    expect(candidates).toContain(
      path.resolve("/tmp/joopo/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl"),
    );
  });

  test("keeps explicit custom sessionFile ahead of synthesized fallback", () => {
    const storePath = "/tmp/joopo/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/joopo/agents/main/sessions/custom-transcript.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps custom topic-like transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/joopo/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/joopo/agents/main/sessions/custom-topic-notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps forked transcript paths ahead of synthesized fallback", () => {
    ...
    const sessionFile =
      "/tmp/joopo/agents/main/sessions/2026-03-23T16-30-00-000Z_11111111-1111-4111-8111-111111111111.jsonl";
  ...
  test("keeps timestamped custom transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/joopo/agents/main/sessions/sessions.json";
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const sessionFile = "/tmp/joopo/agents/main/sessions/2026-03-23T16-30-00-000Z_notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile);
  ...
  test("still treats generated topic transcripts from another session as stale", () => {
    ...
    const staleSessionFile =
      "/tmp/joopo/agents/main/sessions/22222222-2222-4222-8222-222222222222-topic-thread.jsonl";
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("joopo-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });

  beforeAll(() => {
    vi.stubEnv("JOOPO_HOME", tmpDir);
  });
...
describe("oversized transcript line guards", () => {
  ...
  registerTempSessionStore("joopo-session-fs-oversized-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
