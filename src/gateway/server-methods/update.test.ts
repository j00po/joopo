...
vi.mock("../../infra/joopo-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/joopo-root.js")>(
    "../../infra/joopo-root.js",
  );
  return {
    ...actual,
    resolveJoopoPackageRoot: async () => "/tmp/joopo",
  };
});
...
describe("update.run restart scheduling", () => {
  ...
  it("forces an immediate restart after successful package-manager updates", async () => {
    resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
      kind: "global",
      mode: "npm",
      root: "/tmp/joopo-global",
      packageRoot: "/tmp/joopo-global",
    });
