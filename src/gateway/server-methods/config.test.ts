...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/joopo.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/joopo.json"],
    });
  });
  ...
  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/joopo.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/joopo.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.JOOPO_CONFIG_PATH = "/tmp/config $(touch pwned).json";
  ...
  it("returns a generic error and logs details when the opener fails", async () => {
    process.env.JOOPO_CONFIG_PATH = "/tmp/config.json";
    execFileMock.mockImplementation((...args: unknown[]) => {
