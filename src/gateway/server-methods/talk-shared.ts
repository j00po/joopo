...
function getVoiceCallRealtimeConfig(config: JoopoConfig): {
  provider?: string;
...
export function getVoiceCallStreamingConfig(config: JoopoConfig): {
  provider?: string;
...
export function buildTalkRealtimeConfig(config: JoopoConfig, requestedProvider?: string) {
...
export function buildTalkTranscriptionConfig(config: JoopoConfig, requestedProvider?: string) {
...
export function buildRealtimeInstructions(): string {
  return `You are Joopo's realtime voice interface. Keep spoken replies concise. If the user asks for code, repository state, tools, files, current Joopo context, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`;
}
