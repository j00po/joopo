...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([JOOPO_MODEL_ID, JOOPO_DEFAULT_MODEL_ID]);
  ids.add(`joopo/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`joopo/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== JOOPO_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
