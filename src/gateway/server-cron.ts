...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.JOOPO_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: JoopoConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: JoopoConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: JoopoConfig, requestedAgentId: string) => {
