...
export function warnLegacyJoopoEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but Joopo only reads JOOPO_* names now.`,
      "Rename them by replacing the legacy prefix with JOOPO_; the old names are ignored.",
    ].join("\n"),
    { code: "JOOPO_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

export function resetLegacyJoopoEnvWarningForTest(): void {
  warned = false;
}
