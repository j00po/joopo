...
describe("gateway auth", () => {
  ...
  it("resolves token/password from JOOPO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          JOOPO_GATEWAY_TOKEN: "env-token",
          JOOPO_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides JOOPO_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        JOOPO_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
