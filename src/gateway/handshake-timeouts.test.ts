...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers JOOPO_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        JOOPO_HANDSHAKE_TIMEOUT_MS: "75",
        JOOPO_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        JOOPO_HANDSHAKE_TIMEOUT_MS: "",
        JOOPO_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("resolves preauth handshake timeout with env over config over default", () => {
    expect(
      resolvePreauthHandshakeTimeoutMs({
        env: { JOOPO_HANDSHAKE_TIMEOUT_MS: "75000" },
    ...
    expect(
      resolvePreauthHandshakeTimeoutMs({
        env: { JOOPO_HANDSHAKE_TIMEOUT_MS: "garbage" },
        configuredTimeoutMs: 30_000,
      }),
    ).toBe(30_000);
  ...
  test("resolves preauth handshake timeout from the test-only env before config", () => {
    expect(
      resolvePreauthHandshakeTimeoutMs({
        env: { VITEST: "1", JOOPO_TEST_HANDSHAKE_TIMEOUT_MS: "50" },
        configuredTimeoutMs: 30_000,
      }),
    ).toBe(50);
  });
  ...
  test("ignores invalid handshake timeout overrides and falls back safely", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        JOOPO_HANDSHAKE_TIMEOUT_MS: "abc",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        JOOPO_HANDSHAKE_TIMEOUT_MS: "-1",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        JOOPO_HANDSHAKE_TIMEOUT_MS: "0",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
  ...
  test("getConnectChallengeTimeoutMsFromEnv reads JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS;
    const originalHandshake = process.env.JOOPO_HANDSHAKE_TIMEOUT_MS;
    try {
      process.env.JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      process.env.JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS = "";
      process.env.JOOPO_HANDSHAKE_TIMEOUT_MS = "30000";
      expect(resolveConnectChallengeTimeoutMs()).toBe(30_000);
    } finally {
      if (original === undefined) {
        delete process.env.JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.JOOPO_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
      if (originalHandshake === undefined) {
        delete process.env.JOOPO_HANDSHAKE_TIMEOUT_MS;
      } else {
        process.env.JOOPO_HANDSHAKE_TIMEOUT_MS = originalHandshake;
      }
    }
  });
