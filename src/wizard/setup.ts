...
async function writeWizardConfigFile(config: JoopoConfig): Promise<JoopoConfig> {
...
export async function runSetupWizard(
  ...
  await prompter.intro("Joopo setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readSetupConfigFileSnapshot();
  let baseConfig: JoopoConfig = snapshot.valid
    ? snapshot.exists
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("joopo doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("joopo doctor")}`,
        `Inspect: ${formatCliCommand("joopo plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("joopo configure")}.`;
  ...
  let localGatewayToken = process.env.JOOPO_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.JOOPO_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: JoopoConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
