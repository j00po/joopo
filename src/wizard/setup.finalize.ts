...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: JoopoConfig =
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.joopo.ai/gateway/health",
            "https://docs.joopo.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("joopo gateway run")}`,
          `Or rerun with: ${formatCliCommand("joopo onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("joopo onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.joopo.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
    ...
    if (gatewayProbe.ok) {
      await prompter.note(
        [
          ...
          "Stored in: $JOOPO_CONFIG_PATH (default: ~/.joopo/joopo.json) under gateway.auth.token, or in JOOPO_GATEWAY_TOKEN.",
          `View token: ${formatCliCommand("joopo config get gateway.auth.token")}`,
          `Generate token: ${formatCliCommand("joopo doctor --generate-gateway-token")}`,
          "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
          `Open the dashboard anytime: ${formatCliCommand("joopo dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Joopo."
            : "Copy/paste this URL in a browser on this machine to control Joopo.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("joopo dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.joopo.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.joopo.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("joopo configure --section web")}`,
          "",
          "Docs: https://docs.joopo.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("joopo configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.joopo.ai/tools/web"}`,
          "Docs: https://docs.joopo.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("joopo configure --section web")}`,
          "",
          "Docs: https://docs.joopo.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://joopo.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Joopo."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Joopo.",
  );
