...
import type { GatewayBindMode, GatewayTailscaleMode, JoopoConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.JOOPO_GATEWAY_TOKEN)) ||
        randomToken();
