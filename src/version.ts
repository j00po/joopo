...
declare const __JOOPO_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "joopo";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["JOOPO_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["JOOPO_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["JOOPO_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.JOOPO_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Joopo version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __JOOPO_VERSION__ === "string" ? __JOOPO_VERSION__ : undefined,
  bundledVersion: process.env.JOOPO_BUNDLED_VERSION,
});
