...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## Joopo CLI Quick Reference");
    ...
    expect(prompt).toContain("joopo gateway restart");
    expect(prompt).toContain("Do not chain `joopo gateway stop`");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__joopo__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List Joopo agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Joopo docs: /tmp/joopo/docs");
    expect(prompt).toContain(
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/joopo",
      docsPath: "/tmp/joopo/docs",
      sourcePath: "/tmp/joopo",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Joopo docs: /tmp/joopo/docs");
    expect(prompt).toContain("Local source: /tmp/joopo");
    expect(prompt).toContain(
      "For Joopo behavior, commands, config, or architecture: consult local docs first.",
    );
    expect(prompt).toContain(
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Joopo docs: https://docs.joopo.ai");
    expect(prompt).toContain("Source: https://github.com/joopo/joopo");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/joopo",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/joopo/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/joopo/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Joopo Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/joopo. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSupplement", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/joopo/BOOTSTRAP.md");
  ...
  it("adds pending bootstrap guidance and BOOTSTRAP.md contents for override prompts", () => {
    ...
    expect(supplement).toContain("## /tmp/joopo/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`joopo ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Joopo subagents");
