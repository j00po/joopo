...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.joopo/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "joopo doctor" and "joopo approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
