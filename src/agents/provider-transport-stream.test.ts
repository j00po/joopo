...
describe("provider transport stream contracts", () => {
    ...
    const cases = [
      {
        ...
        alias: "joopo-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "joopo-openai-completions-transport",
      },
      {
        ...
        alias: "joopo-azure-openai-responses-transport",
      },
      {
        ...
        alias: "joopo-anthropic-messages-transport",
      },
      {
        ...
        alias: "joopo-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Joopo transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createJoopoTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("keeps Codex defaults on the Joopo transport until PI preserves attribution", () => {
