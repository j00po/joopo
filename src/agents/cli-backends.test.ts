...
function createClaudeCliOverrideConfig(config: CliBackendConfig): JoopoConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Joopo's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__joopo__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__joopo__*");
  ...
  it("keeps Claude permission mode unset when Joopo exec policy is not YOLO", () => {
    const resolved = resolveCliBackendConfig("claude-cli", {
