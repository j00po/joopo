...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/joopo/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./joopo-plugin-tools.js", () => ({
  resolveJoopoPluginToolsForOptions: () => [],
}));

vi.mock("./joopo-tools.nodes-workspace-guard.js", () => ({
...
describe("createJoopoTools TTS config wiring", () => {
  beforeEach(() => {
  ...
  it("passes the resolved shared config into the tts tool", async () => {
    ...
    const { __testing, createJoopoTools } = await import("./joopo-tools.js");
...
describe("createJoopoTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });
