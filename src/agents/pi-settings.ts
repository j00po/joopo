...
export function resolveCompactionReserveTokensFloor(cfg?: JoopoConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: JoopoConfig): AgentCompactionMode {
 ...
 * before the provider call) when Joopo or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (joopo#75799).
