...
function createDefaultSpawnConfig(): JoopoConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: JoopoConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects Joopo config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
    ...
    expect(result).toHaveProperty("error", expect.stringContaining("Joopo config agent"));
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps Joopo ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
