...
const JOOPO_CLAUDE_PLUGIN_NAME = "joopo-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredJoopoTmpDir(), "joopo-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, JOOPO_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: JOOPO_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Joopo skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
