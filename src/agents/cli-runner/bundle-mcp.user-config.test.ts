...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from Joopo config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "joopo-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates Joopo transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "joopo-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "joopo-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "joopo-cli-bundle-mcp-user-servers-loopback-",
    );
      ...
      additionalConfig: {
        mcpServers: {
          joopo: {
            ...
            headers: { Authorization: "Bearer ${JOOPO_MCP_TOKEN}" },
          },
        },
      },
    });
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "joopo-cli-bundle-mcp-user-servers-replace-",
    );
