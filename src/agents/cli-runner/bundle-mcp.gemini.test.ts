...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/joopo-bundle-mcp-gemini",
    ...
    expect(prepared.env?.JOOPO_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["joopo"]);
    expect(raw.mcpServers?.joopo?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.joopo?.headers?.Authorization).toBe("Bearer loopback-token-123");
