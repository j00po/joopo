...
const LIVE = isLiveTestEnabled(["JOOPO_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "joopo-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("joopo_live_probe", "Joopo Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Joopo-configured streamable-http server", async () => {
    const geminiCommand = process.env.JOOPO_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("joopoLiveProbe");
