...
const prepareDeps = {
  ...
  createMcpLoopbackServerConfig,
  resolveJoopoReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveJoopoReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveJoopoReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ...
          JOOPO_MCP_AGENT_ID: sessionAgentId ?? "",
          JOOPO_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          JOOPO_MCP_SESSION_KEY: params.sessionKey ?? "",
          JOOPO_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  let joopoHistoryMessages: unknown[] | undefined;
  const loadJoopoHistoryMessages = async () => {
    joopoHistoryMessages ??= await loadCliSessionHistoryMessages({
      sessionId: params.sessionId,
  ...
  const joopoReferences = await prepareDeps.resolveJoopoReferencePaths({
    workspaceDir,
  ...
  try {
    const hookResult = await resolvePromptBuildHookResult({
      ...
      messages: await loadJoopoHistoryMessages(),
      hookCtx: {
        runId: params.runId,
        agentId: sessionAgentId,
        sessionKey: params.sessionKey,
  ...
  const joopoHistoryPrompt = reusableCliSession.sessionId
    ? undefined
    : buildCliSessionHistoryPrompt({
  ...
  return {
    ...
    ...(joopoHistoryPrompt ? { joopoHistoryPrompt } : {}),
    heartbeatPrompt,
    authEpoch,
    authEpochVersion: CLI_AUTH_EPOCH_VERSION,
    extraSystemPromptHash,
