...
const CLI_BACKEND_PRESERVE_ENV = "JOOPO_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.JOOPO_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.JOOPO_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.JOOPO_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.JOOPO_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.JOOPO_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.JOOPO_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.joopoHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.joopoHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (
            env.JOOPO_MCP_TOKEN ||
            env.JOOPO_MCP_SESSION_KEY ||
            env.JOOPO_MCP_SENDER_IS_OWNER
          ) {
