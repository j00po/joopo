...
function resolveAnthropicOpusThinking(cfg: JoopoConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: JoopoConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<JoopoConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as JoopoConfig,
...
describe("model-selection", () => {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", async () => {
      const warnLogs = createWarnLogCapture("joopo-model-selection-test");
      try {
        const cfg: Partial<JoopoConfig> = {
          agents: {
            defaults: {
