...
describe("provider attribution", () => {
  it("resolves the canonical Joopo product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Joopo runtime wrapper.",
      product: "Joopo",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { JOOPO_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { JOOPO_VERSION: "2026.3.22" })).toEqual({
      originator: "joopo",
      version: "2026.3.22",
      "User-Agent": "joopo/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { JOOPO_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ JOOPO_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
