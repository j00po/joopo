...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: JoopoConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: JoopoConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: JoopoConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: JoopoConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: JoopoConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgInheritDefaultsWithoutAgentModel: JoopoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: JoopoConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: JoopoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: JoopoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result).toMatchObject({ workspace: "~/joopo" });
  });

  it("uses JOOPO_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "joopo-home");
    vi.stubEnv("JOOPO_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as JoopoConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".joopo", "workspace"));
  });

  it("uses JOOPO_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "joopo-home");
    vi.stubEnv("JOOPO_HOME", home);
    // Clear state dir so it falls back to JOOPO_HOME
    vi.stubEnv("JOOPO_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as JoopoConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".joopo", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/joopo-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/joopo-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-agent-scope-"));
