...
import { resolveJoopoMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"joopo":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveJoopoMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("joopo-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["JOOPO_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("JOOPO_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Joopo-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["JOOPO_STATE_DIR", "JOOPO_CONFIG_PATH"]);
    try {
      process.env.JOOPO_STATE_DIR = "/tmp/untrusted-state";
      process.env.JOOPO_CONFIG_PATH = "/tmp/untrusted-config/joopo.json";
