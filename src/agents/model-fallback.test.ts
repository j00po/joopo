...
function makeFallbacksOnlyCfg(): JoopoConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): JoopoConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "joopo-auth-suite-mock");
...
// https://github.com/joopo/joopo/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("joopo-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
