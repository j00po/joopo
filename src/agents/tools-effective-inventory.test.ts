...
import type { createJoopoCodingTools } from "./pi-tools.js";
...
const effectiveInventoryState = vi.hoisted(() => ({
  ...
  createToolsMock: vi.fn<typeof createJoopoCodingTools>(
    (_options) =>
      [
        mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
        mockTool({ name: "docs_lookup", label: "Docs Lookup", description: "Search docs" }),
...
vi.mock("./pi-tools.js", () => ({
  createJoopoCodingTools: (options?: Parameters<typeof createJoopoCodingTools>[0]) =>
...
async function loadHarness(options?: {
  ...
  effectiveInventoryState.createToolsMock =
    ...
    vi.fn<typeof createJoopoCodingTools>((_options) => effectiveInventoryState.tools);
  return {
...
describe("resolveEffectiveToolInventory", () => {
  ...
  beforeEach(() => {
    ...
    effectiveInventoryState.createToolsMock = vi.fn<typeof createJoopoCodingTools>(
  ...
  it("passes resolved model compat into effective tool creation", async () => {
    const createToolsMock = vi.fn<typeof createJoopoCodingTools>(() => [
      mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
    ]);
    const { resolveEffectiveToolInventory } = await loadHarness({
      createToolsMock,
