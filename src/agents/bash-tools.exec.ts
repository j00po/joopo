...
function stripJoopoPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "joopo") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "joopo"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "joopo"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "joopo") {
      return argv.slice(idx);
    }
  }
  return argv;
...
function parseJoopoChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const joopoArgv = stripJoopoPackageRunner(argv);
  return (
    normalizeCommandBaseName(joopoArgv[0]) === "joopo" &&
    (joopoArgv[1] === "channels" || joopoArgv[1] === "channel") &&
    joopoArgv[2] === "login"
  );
}
...
function rejectUnsafeControlShellCommand(command: string): void {
  ...
  for (const candidate of candidates) {
    ...
    if (parseJoopoChannelsLoginShellCommand(candidate)) {
      throw new Error(
        [
          "exec cannot run interactive Joopo channel login commands.",
          "Run `joopo channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
        ].join(" "),
      );
    }
  }
