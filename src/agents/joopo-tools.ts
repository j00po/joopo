...
} from "./joopo-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./joopo-tools.nodes-workspace-guard.js";
import {
  collectPresentJoopoTools,
  isUpdatePlanToolEnabledForJoopoTools,
} from "./joopo-tools.registration.js";
import {
  type HookContext,
...
const defaultJoopoToolsDeps: JoopoToolsDeps = {
  callGateway,
};

let joopoToolsDeps: JoopoToolsDeps = defaultJoopoToolsDeps;

export function createJoopoTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? joopoToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("joopo-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("joopo-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("joopo-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("joopo-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("joopo-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("joopo-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("joopo-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("joopo-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("joopo-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("joopo-tools:nodes-tool");
  const embedded = isEmbeddedMode();
  const effectiveCallGateway = embedded
    ? createEmbeddedCallGateway()
    : joopoToolsDeps.callGateway;
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentJoopoTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
    ...
    ...collectPresentJoopoTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: joopoToolsDeps.callGateway,
          }),
    ...
    ...collectPresentJoopoTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("joopo-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("joopo-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("joopo-tools:tool-hooks");
...
export const __testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<JoopoToolsDeps>) {
    joopoToolsDeps = overrides
      ? {
          ...defaultJoopoToolsDeps,
          ...overrides,
        }
      : defaultJoopoToolsDeps;
  },
