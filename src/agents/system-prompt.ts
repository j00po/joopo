...
function buildWebchatCanvasSection(params: { isMinimal: boolean; runtimeChannel?: string }) {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__joopo__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__joopo__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__joopo__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Joopo handles all routing internally.",
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Joopo docs: ${docsPath}` : "Joopo docs: https://docs.joopo.ai",
    ...
    sourcePath
      ? "If docs are incomplete or stale, inspect the local Joopo source code before answering."
      : "If docs are incomplete or stale, review the Joopo source on GitHub before answering.",
    "When diagnosing issues, run `joopo status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Joopo process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Joopo agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Joopo agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Joopo.";
  }
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside Joopo.",
      "",
      "## Tooling",
      ...
      toolLines.length > 0
        ...
        : [
            ...
            "- browser: control Joopo's dedicated browser",
            "- canvas: present/eval/snapshot the Canvas",
            "- nodes: list/describe/notify/camera/screen on paired nodes",
            "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
      ...
      ...safetySection,
      "## Joopo CLI Quick Reference",
      "Joopo is controlled via subcommands. Do not invent commands.",
      ...
      "- joopo gateway start",
      "- joopo gateway stop",
      "Do not chain `joopo gateway stop` and `joopo gateway start` as a restart substitute.",
      "If unsure, ask the user to run `joopo help` (or `joopo gateway --help`) and paste the output.",
      "",
      ...skillsSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## Joopo Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, Joopo pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by Joopo and included below in Project Context.",
      "",
