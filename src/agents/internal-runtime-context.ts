export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_JOOPO_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_JOOPO_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[JOOPO_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[JOOPO_INTERNAL_CONTEXT_END]]";

export const JOOPO_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
export const JOOPO_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "Joopo runtime context for the immediately preceding user message.";
export const JOOPO_RUNTIME_EVENT_HEADER = "Joopo runtime event.";
export const JOOPO_RUNTIME_CONTEXT_CUSTOM_TYPE = "joopo.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["Joopo runtime context (internal):", JOOPO_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === JOOPO_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === JOOPO_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === JOOPO_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${JOOPO_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${JOOPO_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${JOOPO_RUNTIME_EVENT_HEADER}\n${JOOPO_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isJoopoRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === JOOPO_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isJoopoRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isJoopoRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  return messages.filter(
    (message, index) => !isJoopoRuntimeContextCustomMessage(message) || index > lastUserIndex,
  );
}
