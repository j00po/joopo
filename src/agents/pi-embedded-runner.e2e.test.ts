...
const ensureJoopoModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureJoopoModelsJson: (...args: Parameters<typeof ensureJoopoModelsJsonMock>) =>
        ensureJoopoModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("joopo-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureJoopoModelsJsonMock.mockReset();
  ensureJoopoModelsJsonMock.mockResolvedValue({ wrote: false });
