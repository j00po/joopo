...
function resolveAttachmentLimits(config: JoopoConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".joopo", "attachments");
  const relDir = path.posix.join(".joopo", "attachments", attachmentId);
