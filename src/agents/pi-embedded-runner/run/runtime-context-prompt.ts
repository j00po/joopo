...
import {
  ...
  JOOPO_RUNTIME_CONTEXT_CUSTOM_TYPE,
  JOOPO_RUNTIME_CONTEXT_NOTICE,
  JOOPO_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentTurnPromptContext } from "./params.js";
export { JOOPO_RUNTIME_CONTEXT_CUSTOM_TYPE };

const JOOPO_RUNTIME_EVENT_USER_PROMPT = "Continue the Joopo runtime event.";
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt) {
    return runtimeContext
      ? {
          prompt: JOOPO_RUNTIME_EVENT_USER_PROMPT,
          runtimeContext,
          runtimeOnly: true,
...
export async function queueRuntimeContextForNextTurn(params: {
  ...
  await params.session.sendCustomMessage(
    {
      customType: JOOPO_RUNTIME_CONTEXT_CUSTOM_TYPE,
      content: runtimeContext,
      display: false,
      details: { source: "joopo-runtime-context" },
    },
    { deliverAs: "nextTurn" },
  );
}
