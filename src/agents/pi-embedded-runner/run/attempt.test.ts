...
describe("normalizeMessagesForLlmBoundary", () => {
  ...
  it("keeps only safe blocked metadata at the LLM boundary", () => {
    ...
    expect(output[0]).toHaveProperty("__joopo.beforeAgentRunBlocked");
    expect(output[0]).not.toHaveProperty("__joopo.beforeAgentRunBlocked.reason");
    expect(JSON.stringify(output)).not.toContain("secret prompt");
    expect(JSON.stringify(output)).not.toContain("matched secret prompt");
    expect(input[0]).toHaveProperty("__joopo");
  });
});
    ...
    expect(seenContext.messages).toEqual([
      ...
      },
      {
        ...
        content: [
          {
            type: "text",
            text: "[joopo] missing tool result in session history; inserted synthetic error result for transcript repair.",
          },
        ],
        isError: true,
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    ...
    const legacy = buildAfterTurnRuntimeContext({
      attempt: {
        ...
        agentAccountId: "acct-1",
        authProfileId: "openai:p1",
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as JoopoConfig,
        skillsSnapshot: undefined,
        senderIsOwner: true,
        provider: "openai-codex",
        modelId: "gpt-5.4",
