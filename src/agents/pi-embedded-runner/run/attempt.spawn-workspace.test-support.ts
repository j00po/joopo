...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createJoopoCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createJoopoCodingToolsMock = vi.fn(() => []);
...
vi.mock("../../docs-path.js", () => ({
  resolveJoopoReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../pi-tools.js", () => ({
  createJoopoCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createJoopoCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("joopo.cache-ttl", data),
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createJoopoCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.JOOPO_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.JOOPO_TRAJECTORY = "0";
  }
  try {
    return await (
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.JOOPO_TRAJECTORY;
    } else {
      process.env.JOOPO_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
