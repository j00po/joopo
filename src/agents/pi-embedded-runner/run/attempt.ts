...
import { resolveJoopoReferencePaths } from "../../docs-path.js";
...
import { createJoopoCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
import {
  ...
  try {
    ...
    const toolsRaw = !toolConstructionPlan.constructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-joopo-coding-tools");
    ...
    const joopoReferences = await resolveJoopoReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    const attemptSystemPrompt = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        docsPath: joopoReferences.docsPath ?? undefined,
        sourcePath: joopoReferences.sourcePath ?? undefined,
        ttsHint,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Joopo
      // compaction overrides applied in createPreparedEmbeddedPiSettingsManager — same
      // rehydration also restores Pi's auto-compaction (joopo#75799), so re-apply
      // both guards.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Joopo-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Joopo agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        // double-compaction. See: https://github.com/joopo/joopo/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("joopo:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/joopo/joopo/issues/8643
