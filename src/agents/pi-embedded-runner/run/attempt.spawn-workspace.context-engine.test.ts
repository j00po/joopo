...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  it("sends transcriptPrompt visibly and queues runtime context as hidden custom context", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        prompt: [
          "visible ask",
          "",
          "<<<BEGIN_JOOPO_INTERNAL_CONTEXT>>>",
          "secret runtime context",
          "<<<END_JOOPO_INTERNAL_CONTEXT>>>",
        ].join("\n"),
    ...
    expect(seen.messages).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          role: "custom",
          customType: "joopo.runtime-context",
          display: false,
          content:
            "<<<BEGIN_JOOPO_INTERNAL_CONTEXT>>>\nsecret runtime context\n<<<END_JOOPO_INTERNAL_CONTEXT>>>",
        }),
    ...
    expect(seen.systemPrompt).not.toContain("JOOPO_INTERNAL_CONTEXT");
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("JOOPO_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.joopo/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.joopo/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.joopo/skills/smaug",
              source: "joopo-workspace",
              sourceInfo: {
                path: "/Users/alice/.joopo/skills/smaug/SKILL.md",
                source: "joopo-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.joopo/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/joopo-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/joopo-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context when system prompt override is configured", async () => {
    ...
    hoisted.createJoopoCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/joopo-override-workspace/BOOTSTRAP.md",
    ...
    expect(systemPrompt).toContain("## /tmp/joopo-override-workspace/BOOTSTRAP.md");
    expect(systemPrompt).toContain("Ask who I am.");
  });

  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/joopo-hook-workspace";
  ...
  it("adds explicit reply context to the current model input without exposing generic runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("JOOPO_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the Joopo runtime event.");
    expect(result.finalPromptText).toBe("Continue the Joopo runtime event.");
    ...
    const trajectoryEvents = (
      ...
      .map((line) => JSON.parse(line) as TrajectoryEvent);
    const contextCompiled = trajectoryEvents.find((event) => event.type === "context.compiled");
    expect(contextCompiled?.data?.prompt).toBe("Continue the Joopo runtime event.");
