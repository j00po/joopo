...
function joopoTranscriptAssistant(model: "delivery-mirror" | "gateway-injected"): AgentMessage {
  return {
    role: "assistant",
...
describe("normalizeAssistantReplayContent", () => {
  ...
  it("filters joopo delivery-mirror and gateway-injected assistant messages from replay", () => {
    const messages = [
      userMessage("hello"),
      joopoTranscriptAssistant("delivery-mirror"),
      bedrockAssistant([{ type: "text", text: "real reply" }]),
      joopoTranscriptAssistant("gateway-injected"),
    ];
    const out = normalizeAssistantReplayContent(messages);
  ...
  it("preserves a trailing real model reply whose only content happens to be the sentinel text (jooposweeper review on #77287)", () => {
