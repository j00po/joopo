...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("joopo-bundled-");
  const workspaceDir = await createTempDir("joopo-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: JoopoConfig) {
