...
async function compactEmbeddedPiSessionDirectOnce(
  ...
  await ensureJoopoModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const toolsRaw = createJoopoCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createJoopoCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Joopo-managed registrations so custom tools survive startup.
