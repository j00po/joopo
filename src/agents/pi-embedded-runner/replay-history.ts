...
// `provider:"joopo"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_JOOPO_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyJoopoAssistant(message: AgentMessage): boolean {
...
export function normalizeAssistantReplayContent(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (isTranscriptOnlyJoopoAssistant(message)) {
