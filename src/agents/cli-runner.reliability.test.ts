...
const hookRunnerGlobalStateKey = Symbol.for("joopo.plugins.hook-runner-global-state");
...
function createSessionFile(params?: { history?: Array<{ role: "user"; content: string }> }) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-cli-hooks-"));
  vi.stubEnv("JOOPO_STATE_DIR", dir);
...
function buildPreparedContext(params?: {
  ...
  return {
    ...
    ...(params?.joopoHistoryPrompt
      ? { joopoHistoryPrompt: params.joopoHistoryPrompt }
      : {}),
    authEpochVersion: 2,
  };
}
...
describe("runCliAgent reliability", () => {
  ...
  it("seeds fresh CLI sessions from the Joopo transcript", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        joopoHistoryPrompt:
          "Continue this conversation using the Joopo transcript below.\n\nUser: earlier ask\n\nAssistant: earlier answer\n\n<next_user_message>\nhi\n</next_user_message>",
      }),
    );
  ...
  it("keeps resumed CLI sessions on native resume history", async () => {
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        ...
        joopoHistoryPrompt: "User: earlier ask",
      }),
    );
  ...
  it("blocks CLI runs before llm_input and model execution when before_agent_run blocks", async () => {
    ...
    try {
      ...
      expect(blockedLine.message.__joopo.beforeAgentRunBlocked).toMatchObject({
        blockedBy: "policy-plugin",
      });
      expect(blockedLine.message.__joopo.beforeAgentRunBlocked).not.toHaveProperty("reason");
      expect(Object.hasOwn(blockedLine.message.__joopo, "beforeAgentRunBlocked")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("builds fresh-session history reseed prompts from hook-mutated prompts", async () => {
    ...
    try {
      ...
      expect(context.joopoHistoryPrompt).toContain("Compaction summary: compacted earlier ask");
      expect(context.joopoHistoryPrompt).toContain("hook context");
      expect(context.joopoHistoryPrompt).toContain("current ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
