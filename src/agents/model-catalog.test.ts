...
let ensureJoopoModelsJsonMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    ensureJoopoModelsJsonMock = vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false });
    vi.doMock("./models-config.js", () => ({
      ensureJoopoModelsJson: ensureJoopoModelsJsonMock,
    }));
    vi.doMock("./agent-scope.js", () => ({
      resolveDefaultAgentDir: () => "/tmp/joopo",
    }));
  ...
  beforeEach(() => {
    ...
    ensureJoopoModelsJsonMock.mockClear();
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as JoopoConfig });
    ...
    const second = await loadModelCatalog({ config: {} as JoopoConfig });
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as JoopoConfig });
  ...
  it("does not prepare models.json or import provider discovery when loading fallback catalog in read-only mode", async () => {
    ...
    expect(ensureJoopoModelsJsonMock).not.toHaveBeenCalled();
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as JoopoConfig, readOnly: true });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as JoopoConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/joopo-model-catalog-env" } as NodeJS.ProcessEnv;
