import { listJoopoPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
import {
...
const JOOPO_ATTRIBUTION_PRODUCT = "Joopo";
const JOOPO_ATTRIBUTION_ORIGINATOR = "joopo";
...
function formatJoopoUserAgent(version: string): string {
  return `${JOOPO_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listJoopoPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: JOOPO_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: JOOPO_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatJoopoUserAgent(identity.version),
    },
  };
}
