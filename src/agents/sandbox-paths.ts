...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const joopoTmpDir = path.resolve(resolvePreferredJoopoTmpDir());
  if (!isPathInside(joopoTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: joopoTmpDir });
  return resolved;
}
