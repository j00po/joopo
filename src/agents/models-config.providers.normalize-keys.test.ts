...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<JoopoConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-agent-"));
    try {
      const providers: NonNullable<NonNullable<JoopoConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("replaces resolved env var value with env var name to prevent plaintext persistence", async () => {
    ...
    const env = {
      ...
      JOOPO_SKIP_PROVIDERS: undefined,
      JOOPO_TEST_MINIMAL_GATEWAY: undefined,
    };
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<JoopoConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<JoopoConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
