...
const GOOGLE_LIVE_SINGLE_KEY = "JOOPO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "JOOPO_LIVE_ANTHROPIC_KEY",
    listVar: "JOOPO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "JOOPO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `JOOPO_LIVE_${base}_KEY`;
