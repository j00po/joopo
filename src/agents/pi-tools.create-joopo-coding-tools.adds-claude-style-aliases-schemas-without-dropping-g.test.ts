...
import { createJoopoReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createJoopoCodingTools read behavior", () => {
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "joopo-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createJoopoReadTool(
      baseRead as unknown as Parameters<typeof createJoopoReadTool>[0],
    );
