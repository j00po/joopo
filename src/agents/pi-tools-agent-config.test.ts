...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: JoopoConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<JoopoConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<JoopoConfig["agents"]>["list"]>[number]["tools"];
  }): JoopoConfig {
    return {
      tools: params.tools,
      agents: {
