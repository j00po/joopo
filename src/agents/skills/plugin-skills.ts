 ...
 * plugin skills directory (~/.joopo/plugin-skills/) so the agent SDK can
 * discover them at the conventional file-system path.
 *
 * The plugin-skills directory is fully owned by Joopo — every entry is
...
function publishPluginSkills(skillDirs: string[], opts?: { pluginSkillsDir?: string }): void {
  ...
  // Plugin skill symlinks are owned by Joopo and publish at extra-dir
  ...
  // The plugin-skills directory is fully owned by Joopo: every entry is a
