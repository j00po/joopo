    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "joopo.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "joopo.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "joopo.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("joopo-");
  const acpxRoot = await tempDirs.make("joopo-acpx-plugin-");
  const helperRoot = await tempDirs.make("joopo-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("joopo-");
  const pluginRoot = await tempDirs.make("joopo-plugin-");
  const outsideDir = await tempDirs.make("joopo-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("joopo-");
    const pluginRoot = await tempDirs.make("joopo-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("joopo-");
    const pluginRoot = await tempDirs.make("joopo-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
