...
import { planJoopoModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: JoopoConfig): {
  config: JoopoConfig;
  sourceConfigForSecrets: JoopoConfig;
} {
...
export async function ensureJoopoModelsJson(
  config?: JoopoConfig,
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planJoopoModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
