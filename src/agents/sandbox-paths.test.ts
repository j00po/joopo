...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-managed-media-"));
...
async function withOutsideHardlinkInJoopoTmp<T>(
  params: {
    joopoTmpDir: string;
  ...
  const hardlinkPath = path.join(params.joopoTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.joopoTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.joopoTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.joopoTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const joopoTmpDir = resolvePreferredJoopoTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Joopo tmp root",
      media: path.join(joopoTmpDir, "image.png"),
      expected: path.join(joopoTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Joopo tmp root",
      media: pathToFileURL(path.join(joopoTmpDir, "photo.png")).href,
      expected: path.join(joopoTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Joopo tmp root",
      media: path.join(joopoTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(joopoTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(joopoTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside joopo tmp root",
      media: path.join(os.tmpdir(), "outside-joopo", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Joopo tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(joopoTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(joopoTmpDir, { recursive: true });
      const symlinkPath = path.join(joopoTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Joopo tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInJoopoTmp(
      {
        joopoTmpDir,
  ...
  it("rejects symlinked Joopo tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInJoopoTmp(
