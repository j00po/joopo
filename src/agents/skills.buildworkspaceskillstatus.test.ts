...
async function createTempWorkspaceDir() {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-skill-status-"));
...
function makeEntry(params: {
  ...
  return {
    skill: createFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "joopo-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("honors legacy joopobot skill metadata requirements and install hints", async () => {
    ...
    await writeSkill({
      ...
      metadata:
        '{"joopobot":{"requires":{"bins":["fakebin"]},"install":[{"id":"brew","kind":"brew","formula":"fakebin","bins":["fakebin"],"label":"Install fakebin"}]}}',
    });
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-bundled-"));
    ...
    await withEnvAsync({ JOOPO_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("joopo-workspace");
