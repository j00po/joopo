...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "joopo-openai-responses-transport",
  "openai-codex-responses": "joopo-openai-responses-transport",
  "openai-completions": "joopo-openai-completions-transport",
  "azure-openai-responses": "joopo-azure-openai-responses-transport",
  "anthropic-messages": "joopo-anthropic-messages-transport",
  "google-generative-ai": "joopo-google-generative-ai-transport",
};
...
export function createJoopoTransportStreamFnForModel(
  model: Model<Api>,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Joopo's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // native HTTP streams preserve the same Joopo request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
