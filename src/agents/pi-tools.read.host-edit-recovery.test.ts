...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when JOOPO_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const joopoHome = path.join(tmpDir, "joopo-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(joopoHome, { recursive: true });
    ...
    process.env.JOOPO_HOME = joopoHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(joopoHome, "demo.txt"))).rejects.toMatchObject({
        code: "ENOENT",
      });
    } finally {
