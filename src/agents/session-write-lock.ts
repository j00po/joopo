...
const CLEANUP_STATE_KEY = Symbol.for("joopo.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("joopo.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("joopo.session-write-lock");
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.JOOPO_TEST_SESSION_LOCK_WATCHDOG === "1";
}
