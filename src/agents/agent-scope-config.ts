...
export function listAgentEntries(cfg: JoopoConfig): AgentEntry[] {
...
export function listAgentIds(cfg: JoopoConfig): string[] {
...
export function resolveDefaultAgentId(cfg: JoopoConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: JoopoConfig, agentId: string): AgentEntry | undefined {
