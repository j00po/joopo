...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-cli-bundle-mcp-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.JOOPO_HOME;
      delete process.env.JOOPO_STATE_DIR;
      process.env.JOOPO_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
      ...
      const pluginRoot = path.join(tempHome, ".joopo", "extensions", "bundle-probe");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-cli-live-cleanup-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.JOOPO_HOME;
      delete process.env.JOOPO_STATE_DIR;
