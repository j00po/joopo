...
vi.mock("./embedded-pi-mcp.js", async () => {
  ...
  return {
    loadEmbeddedPiMcpConfig: (params: {
      ...
      const pluginRoot = path.join(params.workspaceDir, ".joopo", "extensions", "claude-bundle");
...
async function createWorkspaceBundle(params: {
  ...
  const pluginRoot = path.join(params.workspaceDir, ".joopo", "extensions", pluginId);
...
describe("loadEnabledBundlePiSettingsSnapshot", () => {
  it("reuses a compatible plugin metadata snapshot without loading a fresh one", async () => {
    const workspaceDir = await tempDirs.make("joopo-workspace-");
