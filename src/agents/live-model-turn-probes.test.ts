...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "JOOPO_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { JOOPO_LIVE_MODEL_IMAGE_PROBE: "false" },
        "JOOPO_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { JOOPO_LIVE_MODEL_IMAGE_PROBE: "1" },
        "JOOPO_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
