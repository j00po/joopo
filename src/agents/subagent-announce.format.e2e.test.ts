...
let configOverride: JoopoConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: JoopoConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/joopo/joopo/issues/31298
    previousFastTestEnv = process.env.JOOPO_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.JOOPO_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    resetAnnounceQueuesForTests();
    // JOOPO_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Joopo runtime context (internal):");
