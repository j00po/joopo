...
const originalRuntime = process.env.JOOPO_AGENT_RUNTIME;

afterEach(() => {
  clearAgentHarnesses();
  piRunAttempt.mockClear();
  if (originalRuntime == null) {
    delete process.env.JOOPO_AGENT_RUNTIME;
  } else {
    process.env.JOOPO_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: JoopoConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
describe("selectAgentHarness", () => {
  ...
  it("keeps an existing session pinned to its plugin harness even when env now forces PI", () => {
    process.env.JOOPO_AGENT_RUNTIME = "pi";
    registerFailingCodexHarness();
