        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by Joopo",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveJoopoCliExecutable();
  const argv =
    executable === "joopo"
      ? ["joopo"]
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `joopo-native-hook-relays-${uid}`);
}
...
async function runNativeHookRelayPreToolUse(params: {
  ...
  // Joopo plugin returns adjusted params here, we intentionally ignore them.
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("joopo:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `joopo-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveJoopoCliExecutable(): string {
  const envPath = process.env.JOOPO_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "joopo.mjs"),
  ...
  throw new Error("Cannot resolve Joopo CLI executable path for native hook relay");
}
