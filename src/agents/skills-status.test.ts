...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "joopo-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@joopo/missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "JOOPO_TEST_MISSING_SKILL_KEY",
            requires: { env: ["JOOPO_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "joopo-bundled" }),
      ],
    });
    ...
    expect(byName.get("needs-bin")).toMatchObject({
      ...
      install: [
        {
          kind: "node",
          label: "Install @joopo/missing-skill-bin (pnpm)",
          bins: [missingBin],
        },
      ],
    });
    expect(byName.get("needs-env")).toMatchObject({
      eligible: false,
      primaryEnv: "JOOPO_TEST_MISSING_SKILL_KEY",
      missing: { env: ["JOOPO_TEST_MISSING_SKILL_KEY"] },
    });
