...
function resolveHeartbeatConfigForAgent(params: {
  cfg: JoopoConfig;
  agentId: string;
}): NonNullable<NonNullable<JoopoConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Joopo config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Joopo config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: JoopoConfig, agentId: string): boolean {
