...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousJoopoAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousJoopoAgentDir = process.env.JOOPO_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousJoopoAgentDir === undefined) {
      delete process.env.JOOPO_AGENT_DIR;
    } else {
      process.env.JOOPO_AGENT_DIR = previousJoopoAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "JOOPO_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: JoopoConfig = {
  models: {
    providers: {
      "custom-proxy": {
