...
import { markJoopoExecEnv } from "../../infra/joopo-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Joopo will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "joopo.sandbox=1");
  args.push("--label", `joopo.sessionKey=${params.scopeKey}`);
  args.push("--label", `joopo.createdAtMs=${createdAtMs}`);
  args.push("--label", `joopo.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `joopo.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markJoopoExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "joopo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`joopo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`joopo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("joopo sandbox recreate --all");
}
