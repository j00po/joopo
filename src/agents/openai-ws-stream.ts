 ...
 *  - Falls back to the Joopo HTTP transport if the WebSocket connection fails
...
import { createJoopoTransportStreamFnForModel } from "./provider-transport-stream.js";
import {
...
const defaultOpenAIWsStreamDeps: OpenAIWsStreamDeps = {
  ...
  // WebSocket auto-mode HTTP fallback must keep the Joopo transport path so
  // degraded sessions do not leak cache-boundary markers or lose strict tools.
  createHttpFallbackStreamFn: (model) => createJoopoTransportStreamFnForModel(model),
...
function resolveWsSessionPoolConfig(env: NodeJS.ProcessEnv = process.env): {
  ...
  const enabled =
    env.JOOPO_OPENAI_WS_POOL === "1" || env.JOOPO_OPENAI_WS_SESSION_POOL === "1";
  const rawIdleMs = Number(env.JOOPO_OPENAI_WS_SESSION_POOL_IDLE_MS);
...
function resolveNativeOpenAITransportTurnState(params: {
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-joopo-turn-id": turnId,
      "x-joopo-turn-attempt": attempt,
    },
    metadata: {
      joopo_session_id: sessionHeaders["x-joopo-session-id"] ?? "",
      joopo_turn_id: turnId,
      joopo_turn_attempt: attempt,
      joopo_transport: params.transport,
    },
  };
}
 ...
 * Joopo HTTP transport when available, or the standard `streamSimple` path.
 *
