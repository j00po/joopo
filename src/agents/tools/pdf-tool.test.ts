...
function withPdfModel(primary: string): JoopoConfig {
  return {
    agents: { defaults: { pdfModel: { primary } } },
  } as JoopoConfig;
}

function withDefaultModel(primary: string): JoopoConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureJoopoModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
        const tool = requirePdfTool(
  ...
  it("uses native PDF path without eager extraction", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      ...
      expect(modelsConfig.ensureJoopoModelsJson).toHaveBeenCalledWith(
        expect.objectContaining({
