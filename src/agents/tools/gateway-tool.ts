...
import { isJoopoOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `joopo security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as JoopoConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as JoopoConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isJoopoOwnerOnlyCoreToolName("gateway"),
    description:
