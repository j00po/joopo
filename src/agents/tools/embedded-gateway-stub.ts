...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: JoopoConfig }) => string;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: JoopoConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: JoopoConfig) => {
    storePath: string;
    store: unknown;
  };
