...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    joopo: process.env.JOOPO_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.joopo === undefined) {
      delete process.env.JOOPO_GATEWAY_TOKEN;
    } else {
      process.env.JOOPO_GATEWAY_TOKEN = envSnapshot.joopo;
    }
  });
  ...
  it("uses JOOPO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.JOOPO_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
