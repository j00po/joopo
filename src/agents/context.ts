...
function isLikelyJoopoCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "joopo" ||
    entryBasename === "joopo.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Joopo CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureJoopoModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyJoopoCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): JoopoConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureJoopoModelsJson(cfg);
    } catch {
