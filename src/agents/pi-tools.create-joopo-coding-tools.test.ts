...
function expectNoSubagentControlTools(tools: ReturnType<typeof createJoopoCodingTools>) {
...
type JoopoCodingTool = ReturnType<typeof createJoopoCodingTools>[number];

function requireTool(tools: JoopoCodingTool[], name: string): JoopoCodingTool {
...
function requireToolExecute(tool: JoopoCodingTool): NonNullable<JoopoCodingTool["execute"]> {
  if (!tool.execute) {
...
describe("createJoopoCodingTools", () => {
  const testConfig: JoopoConfig = {};

  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createJoopoCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    const createJoopoToolsMock = vi.mocked(createJoopoTools);
    createJoopoToolsMock.mockClear();
    ...
    expect(createJoopoToolsMock).toHaveBeenCalledWith(
  ...
  it("skips unrelated tool families when construction is planned from a narrow allowlist", () => {
    ...
    expect(createJoopoToolsMock).not.toHaveBeenCalled();
  ...
  it("passes plugin suppression into Joopo tool construction plans", () => {
    const createJoopoToolsMock = vi.mocked(createJoopoTools);
    createJoopoToolsMock.mockClear();
  ...
  it("keeps plugin-only construction off the Joopo core factory", () => {
    const createJoopoToolsMock = vi.mocked(createJoopoTools);
    createJoopoToolsMock.mockClear();
  ...
  it("passes explicit denylist entries to Joopo tool factory planning", () => {
    const createJoopoToolsMock = vi.mocked(createJoopoTools);
    createJoopoToolsMock.mockClear();
  ...
  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createJoopoCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createJoopoCodingTools({
      config: testConfig,
    ...
    const codexTools = createJoopoCodingTools({
      config: testConfig,
    ...
    const disabledConfig: JoopoConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createJoopoCodingTools({
    ...
    const anthropicTools = createJoopoCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: JoopoConfig = {
      tools: {
        exec: {
    ...
    const allowed = createJoopoCodingTools({
    ...
    const denied = createJoopoCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createJoopoCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createJoopoCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createJoopoCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createJoopoCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createJoopoCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createJoopoCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createJoopoCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(codingTools.some((tool) => tool.name === "message")).toBe(false);

    const cronTools = createJoopoCodingTools({
      config: { tools: { profile: "coding" } },
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createJoopoCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createJoopoCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createJoopoCodingTools({
      modelProvider: "xai",
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createJoopoCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-read-"));
    try {
      ...
      const contents = "Hello from joopo read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "joopo-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "joopo-workspace"),
    ...
    const tools = createJoopoCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-canonical-"));
    try {
      const tools = createJoopoCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-legacy-alias-"));
    try {
      const tools = createJoopoCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-structured-write-"));
    try {
      const tools = createJoopoCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-structured-edit-"));
    try {
