...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("joopo-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("joopo-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("joopo-tilde-test-mkdir-");
  ...
  it("ignores JOOPO_HOME for write operations", async () => {
    const joopoHome = await createTempDir("joopo-home-override-", os.tmpdir());
    const dir = await createTempDir("joopo-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("JOOPO_HOME", joopoHome);

    createHostWorkspaceWriteTool(joopoHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(joopoHome, path.basename(testFile)))).rejects.toMatchObject(
      {
        code: "ENOENT",
      },
    );
  ...
  it("ignores JOOPO_HOME for mkdir operations", async () => {
    const joopoHome = await createTempDir("joopo-home-override-", os.tmpdir());
    const dir = await createTempDir("joopo-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(joopoHome, path.basename(newDir)))).rejects.toMatchObject({
      code: "ENOENT",
    });
  });

  it("ignores JOOPO_HOME for readFile operations", async () => {
    const joopoHome = await createTempDir("joopo-home-override-", os.tmpdir());
    const dir = await createTempDir("joopo-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(joopoHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores JOOPO_HOME for access operations", async () => {
    const joopoHome = await createTempDir("joopo-home-override-", os.tmpdir());
    const dir = await createTempDir("joopo-tilde-test-edit-");
