...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.joopo_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.joopo_transport).toBe("websocket");
    const turnId = requireValue(firstPayload.metadata?.joopo_turn_id, "turn id missing");
    expect(turnId).not.toBe("");
    expect(secondPayload.metadata?.joopo_turn_id).toBe(turnId);
    expect(firstPayload.metadata?.joopo_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.joopo_turn_attempt).toBe("2");
  });
  ...
  it("does not attach native OpenAI session headers or metadata for custom responses endpoints", async () => {
    ...
    expect(payload.metadata?.joopo_session_id).toBeUndefined();
    expect(payload.metadata?.joopo_transport).toBeUndefined();
  ...
  it("keeps the default websocket HTTP fallback on the Joopo transport", () => {
    expect(
  ...
  it("pools cleanly released sessions behind the explicit pool flag", async () => {
    ...
    try {
      releaseWsSession("registry-test", {
        allowPool: true,
        env: {
          JOOPO_OPENAI_WS_POOL: "1",
          JOOPO_OPENAI_WS_SESSION_POOL_IDLE_MS: "1000",
