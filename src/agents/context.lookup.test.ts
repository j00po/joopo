...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    ensureJoopoModelsJson: vi.fn(async () => {}),
...
vi.mock("./models-config.runtime.js", () => ({
  ensureJoopoModelsJson: contextTestState.ensureJoopoModelsJson,
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureJoopoModelsJson.mockClear();
  return { ensureJoopoModelsJson: contextTestState.ensureJoopoModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real joopo startup commands that need model metadata", async () => {
    const { shouldEagerWarmContextWindowCache } = await importContextModule();

    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "chat"])).toBe(true);
    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "chat", "--help"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "joopo", "matrix", "encryption", "help"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "help", "matrix"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "joopo", "browser", "status", "--help"]),
    ).toBe(false);
    expect(
    ...
    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "logs", "--limit", "5"])).toBe(
      false,
    );
    expect(
      shouldEagerWarmContextWindowCache(["node", "joopo", "memory", "search", "--json"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "message", "read"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "status", "--json"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "joopo", "sessions", "--json"])).toBe(
      false,
    );
    expect(
  ...
  it("skips model normalization during warmup but preserves provider-owned context metadata", async () => {
    ...
    expect(contextTestState.discoverModels).toHaveBeenCalledWith(
      expect.anything(),
      expect.stringMatching(/\/\.joopo\/agents\/main\/agent$/),
