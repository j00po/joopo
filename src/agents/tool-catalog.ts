...
function buildCoreToolGroupMap() {
  ...
  const joopoTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInJoopoGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:joopo": joopoTools,
