...
function resolveExecConfig(params: { cfg?: JoopoConfig; agentId?: string }) {
...
export type JoopoCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeJoopoTools: boolean;
  includePluginTools: boolean;
};

export function createJoopoCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep Joopo web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: JoopoCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeJoopoTools: includeCoreTools,
  ...
  const includeJoopoTools = includeCoreTools && toolConstructionPlan.includeJoopoTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createJoopoReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
  ...
  const pluginToolsOnly =
    includeJoopoTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("joopo-tools");
