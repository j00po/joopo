...
import { resolveOptionalMediaToolFactoryPlan } from "./joopo-tools.media-factory-plan.js";
import * as pdfModelConfigModule from "./tools/pdf-tool.model-config.js";

type CreateJoopoToolsOptions = Parameters<
  typeof import("./joopo-tools.js").createJoopoTools
>[0];

async function createJoopoToolsForTest(options?: CreateJoopoToolsOptions) {
  const { createJoopoTools } = await import("./joopo-tools.js");
  return createJoopoTools(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/joopo.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): JoopoConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: JoopoConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createJoopoToolsForTest({
      config,
      agentDir: "/tmp/joopo-agent-main",
