...
import type {
  ...
  JoopoPluginHttpRouteParams,
  JoopoPluginHookOptions,
  JoopoPluginNodeHostCommand,
  JoopoPluginNodeInvokePolicy,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@joopo/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@joopo/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("joopo.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<JoopoPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<JoopoPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<JoopoPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: JoopoPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: JoopoPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: JoopoPluginHookOptions | undefined,
    config: JoopoPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: JoopoPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: JoopoPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const registrationCapabilities = resolvePluginRegistrationCapabilities(mode);
    const normalized =
      typeof (registration as JoopoPluginChannelRegistration).plugin === "object"
        ? (registration as JoopoPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is JoopoPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: JoopoPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: JoopoPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: JoopoPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: JoopoPluginNodeInvokePolicy,
  ...
  const registerService = (record: PluginRecord, service: JoopoPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: JoopoPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<JoopoPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<JoopoPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as JoopoConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
