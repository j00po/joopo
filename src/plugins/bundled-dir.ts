...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "joopo-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "JOOPO_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.JOOPO_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "joopo.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveJoopoPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveJoopoPackageRootSync({ moduleUrl: import.meta.url });
...
export function resolveSourceCheckoutDependencyDiagnostic(
  ...
  for (const packageRoot of resolvePackageRootsForBundledPlugins()) {
    ...
    return {
      source: packageRoot,
      message:
        "Joopo source checkout detected without pnpm workspace dependencies; run `pnpm install` from the repo root so bundled plugins can load package-local dependencies.",
    };
  }
  return null;
}
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveJoopoPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.JOOPO_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.JOOPO_BUNDLED_PLUGINS_DIR ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.JOOPO_BUNDLED_PLUGINS_DIR?.trim();
