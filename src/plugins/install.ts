...
import {
  linkJoopoPeerDependencies,
  relinkJoopoPeerDependenciesInManagedNpmRoot,
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing joopo.extensions; update the plugin package to include joopo.extensions (for example ["./dist/index.js"]). See https://docs.joopo.ai/help/troubleshooting#plugin-install-fails-with-missing-joopo-extensions';
...
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_joopo-pack-archives";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_JOOPO_EXTENSIONS: "missing_joopo_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_JOOPO_EXTENSIONS: "empty_joopo_extensions",
  INVALID_JOOPO_EXTENSIONS: "invalid_joopo_extensions",
...
function ensureJoopoExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_JOOPO_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json joopo.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_JOOPO_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareJoopoReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@joopo/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official Joopo package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official Joopo package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official Joopo install.`,
  );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  try {
    await relinkJoopoPeerDependenciesInManagedNpmRoot({
      npmRoot: params.npmRoot,
...
async function installPluginFromManagedNpmRoot(
  ...
  if (params.packageName !== "joopo") {
    const repairedJoopoPeer = await repairManagedNpmRootJoopoPeer({
      npmRoot,
      timeoutMs,
      logger,
    });
    if (repairedJoopoPeer) {
      logger.info?.(`Repaired stale joopo peer dependency in ${npmRoot}`);
    }
  }
  await upsertManagedNpmRootDependency({
    ...
    managedOverrides: await readJoopoManagedNpmRootOverrides(),
  });
  ...
  if (params.packageName !== "joopo") {
    ...
    if (repairedJoopoPeer) {
      logger.info?.(`Repaired stale joopo peer dependency in ${npmRoot} after npm install`);
    }
  }
  await relinkJoopoPeerDependenciesInManagedNpmRoot({ npmRoot, logger });
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "joopo security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureJoopoExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  const extensionsResult = ensureJoopoExtensions({
    manifest,
  });
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid joopo.plugin.json: ${ocManifestResult.error}`,
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json joopo.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Joopo >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set JOOPO_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Joopo >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Joopo and retry.`,
  ...
  if (!extensionValidation.ok) {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_JOOPO_EXTENSIONS,
    };
  }
...
async function scanAndLinkInstalledPackage(params: {
  ...
  await linkJoopoPeerDependencies({
