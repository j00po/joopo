...
import type { JoopoPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("joopo-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "joopo.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  packageManifest?: JoopoPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "joopo.plugin.json");
  const linkedManifest = path.join(rootDir, "joopo.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-prometheus": {
          source: "npm",
          installPath: dir,
          resolvedName: "@joopo/diagnostics-prometheus",
  ...
  it("reports non-bundled providerAuthEnvVars as deprecated compat metadata", () => {
    ...
    expect(registry.diagnostics).toContainEqual(
      expect.objectContaining({
        ...
        source: path.join(dir, "joopo.plugin.json"),
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "joopo.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { JOOPO_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Joopo >=2026.3.22, but this host is 2026.3.21",
      expectWarn: true,
    },
    {
      name: "skips plugins whose beta minHostVersion is newer than the current host",
      minHostVersion: ">=2026.5.1-beta.1",
      env: { JOOPO_VERSION: "2026.4.30" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Joopo >=2026.5.1-beta.1, but this host is 2026.4.30",
      expectWarn: true,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | joopo.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { JOOPO_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("accepts legacy bare minHostVersion metadata for recorded installed globals", () => {
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("joopo.install.minHostVersion must use"),
      ),
    ).toBe(false);
  });
  ...
  it("does not runtime-gate bundled source plugins by install minHostVersion", () => {
    ...
    expect(registry.diagnostics.some((diag) => diag.message.includes("requires Joopo"))).toBe(
      false,
    );
  });
  ...
  it("suppresses duplicate warning when global candidates come from the same package artifact", () => {
    ...
    const manifest = { id: "opik-joopo", configSchema: { type: "object" } };
  ...
  it("resolves load-path manifests from the current env home", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        JOOPO_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeB,
        JOOPO_HOME: undefined,
        JOOPO_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
  ...
  it("resolves manifests against the current host version", () => {
    ...
    const olderHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        JOOPO_VERSION: "2026.3.21",
      }),
    });
    const newerHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        JOOPO_VERSION: "2026.3.22",
      }),
    });
