...
vi.mock("./loader.js", () => ({
  loadJoopoPlugins: (...args: unknown[]) => loadJoopoPluginsMock(...args),
}));
...
function setPluginLoadResult(overrides: Partial<ReturnType<typeof createPluginLoadResult>>) {
  ...
  loadJoopoPluginsMock.mockReturnValue(result);
      ...
      snapshot: createInstalledPluginIndexSnapshot([
        {
          ...
          manifestPath: "/tmp/provider-env-plugin/joopo.plugin.json",
