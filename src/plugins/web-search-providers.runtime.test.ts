...
let loadJoopoPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/joopo.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/joopo.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadJoopoPluginsMock).toHaveBeenCalledTimes(count);
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadJoopoPluginsMock = vi
      .spyOn(loaderModule, "loadJoopoPlugins")
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ JOOPO_HOME: "/tmp/joopo-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.JOOPO_HOME = "/tmp/joopo-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
  ...
  it("does not reuse snapshot provider loads across host Vitest env changes", () => {
    ...
    expect(loadJoopoPluginsMock).toHaveBeenCalledTimes(2);
  });
