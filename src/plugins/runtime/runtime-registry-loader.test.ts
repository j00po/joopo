...
const mocks = vi.hoisted(() => ({
  loadJoopoPlugins: vi.fn<typeof import("../loader.js").loadJoopoPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadJoopoPlugins: (...args: Parameters<typeof mocks.loadJoopoPlugins>) =>
    mocks.loadJoopoPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadJoopoPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadJoopoPlugins>) => mocks.loadJoopoPlugins(...args),
    );
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadJoopoPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadJoopoPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
