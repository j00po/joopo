...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as JoopoConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as JoopoConfig;
    const load = vi.fn(async (config?: JoopoConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: JoopoConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
