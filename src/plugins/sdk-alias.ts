...
function hasTrustedJoopoRootIndicator(params: {
  packageRoot: string;
  ...
  const hasJoopoBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("joopo")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.joopo === "string");
  const hasJoopoEntrypoint = fs.existsSync(path.join(params.packageRoot, "joopo.mjs"));
  return hasCliEntryExport || hasJoopoBin || hasJoopoEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedJoopoRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedJoopoRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveJoopoPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedJoopoRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveJoopoPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedJoopoRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["joopo/plugin-sdk", "@joopo/plugin-sdk"] as const;
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@joopo/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.JOOPO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `joopo/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "joopo/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.joopoPackageRoot, "extensions"),
    path.join(params.joopoPackageRoot, "dist", "extensions"),
    path.join(params.joopoPackageRoot, "dist-runtime", "extensions"),
