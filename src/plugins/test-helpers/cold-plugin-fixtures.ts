...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/joopo-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "joopo.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): JoopoConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    JOOPO_HOME: path.join(homeDir, "home"),
    JOOPO_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    JOOPO_DISABLE_PERSISTED_PLUGIN_REGISTRY:
