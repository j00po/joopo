...
export type ConfigScopedRuntimeCache<T> = WeakMap<JoopoConfig, Map<string, T>>;

export type ConfigScopedPromiseLoader<T> = {
  load(config?: JoopoConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: JoopoConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<JoopoConfig, Promise<T>>();

  const createPromise = (config?: JoopoConfig): Promise<T> => {
  ...
  return {
    async load(config?: JoopoConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<JoopoConfig, Promise<T>>();
    },
  };
}
