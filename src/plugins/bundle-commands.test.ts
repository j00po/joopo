...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".joopo", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "JOOPO_HOME", "JOOPO_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("joopo-bundle-commands-home-");
      const workspaceDir = await createTempDir("joopo-bundle-commands-workspace-");
