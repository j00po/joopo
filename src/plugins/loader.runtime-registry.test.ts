...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadJoopoPlugins(loadOptions);

      loadJoopoPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadJoopoPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadJoopoPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadJoopoPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadJoopoPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadJoopoPlugins(loadOptions)).not.toBe(registry);
  });
});
