...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `joopo-${label}-`));
  tempDirs.push(dir);
  return dir;
}
...
describe("installPluginFromNpmSpec e2e", () => {
  it("scrubs root joopo materialized by required npm peers", async () => {
    ...
    const registry = await startStaticRegistry([
      ...
      { packageName: "joopo", latest: "2026.0.0", versions: joopoVersions },
    ]);
    ...
    expect(rawLock.packages?.["node_modules/joopo"]).toMatchObject({
      peer: true,
      version: "2026.0.0",
    });
    ...
    expect(lock.packages?.["node_modules/joopo"]).toBeUndefined();
    await expect(fs.lstat(path.join(npmRoot, "node_modules", "joopo"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "joopo"))
  ...
  it("omits peers when installing beside an existing host-peer plugin", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { joopo: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { joopo: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { joopo: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(npmRoot, "node_modules", codexName, "node_modules", "joopo"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(npmRoot, "node_modules", opikName, "node_modules", "joopo"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("relinks managed npm sibling joopo peers after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "joopo");
    ...
    expect(manifest.dependencies?.joopo).toBeUndefined();
    const lock = JSON.parse(await fs.readFile(path.join(npmRoot, "package-lock.json"), "utf8")) as {
      packages?: Record<string, unknown>;
    };
    expect(lock.packages?.["node_modules/joopo"]).toBeUndefined();
