...
function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listJoopoPluginManifestMetadata", () => {
  afterEach(() => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "joopo.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "joopo.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    writeJson(path.join(home, ".joopo", "plugins", "installs.json"), {
      plugins: [{ rootDir: path.join(staleBundledRoot, "openai"), origin: "bundled" }],
    });

    const records = listJoopoPluginManifestMetadata({
      JOOPO_HOME: home,
