...
function collectConfiguredPluginEntryIds(config: JoopoConfig): string[] {
...
function resolveRegister(mod: JoopoPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: JoopoPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as JoopoPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: JoopoPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Joopo ignored the plugin setup runtime entry. Remove setup-api/joopo.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
