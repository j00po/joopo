...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/joopo.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: JoopoConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: JoopoConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: JoopoConfig) => {
      if (Object.prototype.hasOwnProperty.call(config, "channels")) {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    ...
    expectStartupPluginIdsCase({
      ...
      env: {
        JOOPO_STATE_DIR: "/tmp/joopo-with-persisted-demo-channel",
      } as NodeJS.ProcessEnv,
      expected: ["browser", "memory-core"],
    });
  });
