...
import type {
  JoopoPluginHookOptions,
  JoopoPluginToolContext,
  JoopoPluginToolFactory,
  JoopoPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type JoopoPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<JoopoConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Joopo uses this to expose
 ...
 *
 * Runs after Joopo resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Joopo auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Joopo wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Joopo builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Joopo no
...
/**
 * Provider-owned stream wrapper hook after Joopo applies its generic
 ...
 *
 * Return `true` or `false` to override Joopo's built-in provider cache TTL
 ...
 *
 * Runs only after Joopo exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Joopo should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Joopo loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Joopo falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Joopo calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Joopo performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Joopo registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Joopo wrappers.
   *
   ...
   *
   * Called after Joopo resolves the raw configured credential but before the
   ...
   * hint than Joopo's generic auth-store guidance.
   */
   ...
   * Joopo's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Joopo should surface a provider-specific hint for
   ...
   * merging. Joopo deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Joopo stores the
   ...
   * without replacing the full Joopo-owned system prompt.
   */
   ...
   *
   * Runs after Joopo's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Joopo's default fallback policy.
   */
   ...
   * rewrites after Joopo has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Joopo handles
   ...
   *
   * Joopo uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `joopo doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `joopo doctor` should offer.
   *
   ...
   *
   * Joopo calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Joopo's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Joopo falls back to that stored profile.
   */
...
export type JoopoPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral Joopo session id when available. */
  sessionId?: string;
  /** Transcript file for this Joopo session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Joopo session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Joopo configuration */
  config: JoopoConfig;
 ...
 */
export type JoopoPluginCommandDefinition = {
...
export type JoopoPluginHttpRouteAuth = "gateway" | "plugin";
export type JoopoPluginHttpRouteMatch = "exact" | "prefix";
export type JoopoPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type JoopoPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type JoopoPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type JoopoPluginHttpRouteParams = {
  path: string;
...
export type JoopoPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type JoopoPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `joopo` program. For nested
...
export type JoopoPluginCliRegistrar = (ctx: JoopoPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Joopo to keep a plugin command lazy-loaded while still
 ...
 */
export type JoopoPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type JoopoPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `joopo nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `joopo nodes`, so a descriptor named
   * `"camera"` exposes `joopo nodes camera`.
   */
  descriptors?: JoopoPluginCliCommandDescriptor[];
};

export type JoopoPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type JoopoPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type JoopoPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type JoopoPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type JoopoPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: JoopoPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type JoopoPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: JoopoPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<JoopoPluginNodeInvokeTransportResult>;
};

export type JoopoPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type JoopoPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: JoopoPluginNodeInvokePolicyContext,
  ) => Promise<JoopoPluginNodeInvokePolicyResult> | JoopoPluginNodeInvokePolicyResult;
};

export type JoopoPluginSecurityAuditContext = {
  config: JoopoConfig;
...
export type JoopoPluginSecurityAuditCollector = (
  ctx: JoopoPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type JoopoGatewayDiscoveryAdvertiseContext = {
...
export type JoopoGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: JoopoGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type JoopoPluginServiceContext = {
  config: JoopoConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type JoopoPluginService = {
  id: string;
  start: (ctx: JoopoPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: JoopoPluginServiceContext) => void | Promise<void>;
};

export type JoopoPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type JoopoPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `joopo.plugin.json` via
  ...
  configSchema?: JoopoPluginConfigSchema;
  reload?: JoopoPluginReloadRegistration;
  nodeHostCommands?: JoopoPluginNodeHostCommand[];
  securityAuditCollectors?: JoopoPluginSecurityAuditCollector[];
  register?: (api: JoopoPluginApi) => void;
  activate?: (api: JoopoPluginApi) => void;
};

export type JoopoPluginModule = JoopoPluginDefinition | ((api: JoopoPluginApi) => void);
...
export type PluginConfigMigration = (config: JoopoConfig) =>
  | {
      config: JoopoConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `joopo migrate`. */
...
export type JoopoPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | JoopoPluginToolFactory,
    opts?: JoopoPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: JoopoPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: JoopoPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: JoopoPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: JoopoPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: JoopoPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, Joopo
  ...
  /**
   * Register a plugin-owned node feature command group under `joopo nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: JoopoPluginCliRegistrar,
    opts?: JoopoPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: JoopoPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: JoopoPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: JoopoPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: JoopoPluginSecurityAuditCollector) => void;
  registerService: (service: JoopoPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: JoopoGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `joopo migrate` (migration capability). */
   ...
   */
  registerCommand: (command: JoopoPluginCommandDefinition) => void;
