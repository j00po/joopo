...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@joopo/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@joopo/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@joopo/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@joopo/google-meet:dangerous-exec:src/node-host.ts",
  "@joopo/google-meet:dangerous-exec:src/realtime.ts",
  "@joopo/voice-call:dangerous-exec:src/tunnel.ts",
  "@joopo/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@joopo/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@joopo/acpx:dangerous-exec:dist/service-<hash>.js",
  "@joopo/codex:dangerous-exec:dist/client-<hash>.js",
  "@joopo/google-meet:dangerous-exec:dist/index.js",
  "@joopo/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "joopo-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return readdirSync("extensions", { withFileTypes: true })
    ...
    .flatMap((entry) => {
      ...
      let packageJson: {
        name?: unknown;
        joopo?: { release?: { publishToNpm?: unknown } };
      };
      try {
