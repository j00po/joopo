...
function buildScanFailureBlockReason(params: { error: string; targetLabel: string }) {
  return `${params.targetLabel} blocked: code safety scan failed (${params.error}). Run "joopo security audit --deep" for details.`;
}
...
function isPackageRootJoopoPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "joopo") ||
    (segments.length === 3 &&
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootJoopoPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedJoopoPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootJoopoPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostJoopoRootRealPath(): Promise<string | null> {
  const hostRoot = resolveJoopoPackageRootSync({
...
function isTrustedHostJoopoPath(params: {
  resolvedTargetPath: string;
  trustedHostJoopoRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostJoopoRootRealPath !== null &&
    isPathInside(params.trustedHostJoopoRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    ...
    if (
      ...
      isTrustedHostJoopoPath({
        resolvedTargetPath,
        trustedHostJoopoRootRealPath: params.trustedHostJoopoRootRealPath,
      })
    ) {
      return {};
    }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostJoopoRootRealPath = await resolveTrustedHostJoopoRootRealPath();
...
export async function scanBundleInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Bundle "{target}" has {count} suspicious code pattern(s). Run "joopo security audit --deep" for details.`,
...
export async function scanPackageInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Plugin "{target}" has {count} suspicious code pattern(s). Run "joopo security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanFileInstallSourceRuntime(
  ...
  const builtinScan = await scanFileTarget({
    ...
    suspiciousMessage: `Plugin file "{target}" has {count} suspicious code pattern(s). Run "joopo security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanSkillInstallSourceRuntime(params: {
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    path: params.sourceDir,
    suspiciousMessage:
      'Skill "{target}" has {count} suspicious code pattern(s). Run "joopo security audit --deep" for details.',
