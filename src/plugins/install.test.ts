...
vi.mock("../infra/joopo-root.js", () => ({
  resolveJoopoPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("joopo-plugin-install");
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    joopo?: { install?: Record<string, unknown> };
  };
  manifest.joopo = {
    ...manifest.joopo,
    install: {
      ...manifest.joopo?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "joopo.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "joopo.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(vi.mocked(runCommandWithTimeout)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: expect.stringContaining(".joopo-install-stage-"),
      }),
    );
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@joopo/voice-call" });
  ...
  it("rejects native plugin zip archives without joopo.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid joopo.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@joopo/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without joopo.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@joopo/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("joopo.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_JOOPO_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when joopo.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@joopo/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing joopo.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_JOOPO_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without joopo.extensions");
  });

  it("rejects package installs when joopo.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_JOOPO_EXTENSIONS);
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_JOOPO_EXTENSIONS);
      expect(result.error).toContain("joopo.runtimeExtensions[0]");
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        joopo: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("still scans declared package entrypoints when they live under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        joopo: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/joopo points at the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-joopo");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"joopo"}\n');
      vi.mocked(resolveJoopoPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "joopo-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(hostRoot, path.join(nodeModulesDir, "joopo"), "junction");
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/.bin/joopo points inside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-joopo");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"joopo"}\n');
      const hostBin = path.join(hostRoot, "joopo.mjs");
      fs.writeFileSync(hostBin, "#!/usr/bin/env node\n");
      vi.mocked(resolveJoopoPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "joopo-bin-peer-plugin");

      const binDir = path.join(pluginDir, "node_modules", ".bin");
      fs.mkdirSync(binDir, { recursive: true });
      fs.symlinkSync(hostBin, path.join(binDir, "joopo"), "file");
  ...
  it.runIf(process.platform !== "win32")(
    "fails package installs when node_modules/joopo points outside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-joopo");
      const spoofedRoot = path.join(tmpDir, "spoofed-joopo");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.mkdirSync(spoofedRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"joopo"}\n');
      fs.writeFileSync(path.join(spoofedRoot, "package.json"), '{"name":"joopo"}\n');
      vi.mocked(resolveJoopoPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "spoofed-joopo-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(spoofedRoot, path.join(nodeModulesDir, "joopo"), "junction");
      ...
      if (!result.ok) {
        expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED);
        expect(result.error).toContain("node_modules/joopo");
      }
    },
  );

  it.runIf(process.platform !== "win32")(
    "fails package installs for nested or non-exact joopo node_modules symlinks",
    async () => {
      const cases = [
        {
          pluginName: "nested-joopo-peer-plugin",
          relativePath: path.join("node_modules", "vendor", "node_modules", "joopo"),
        },
        {
          pluginName: "uppercase-joopo-peer-plugin",
          relativePath: path.join("node_modules", "Joopo"),
        },
        {
          pluginName: "trailing-space-joopo-peer-plugin",
          relativePath: path.join("node_modules", "joopo "),
        },
      ] as const;
  ...
  it("fails package installs when manifest traversal exceeds the directory cap", async () => {
    vi.stubEnv("JOOPO_INSTALL_SCAN_MAX_DIRECTORIES", "4");
  ...
  it("fails package installs when manifest traversal exceeds the depth cap", async () => {
    vi.stubEnv("JOOPO_INSTALL_SCAN_MAX_DEPTH", "2");
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveJoopoPackageRootSync).mockReturnValue(process.cwd());
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        joopo: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.joopo).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires Joopo >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json joopo.install.minHostVersion"],
    },
    {
  ...
  it("uses joopo.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@joopo/cognee-joopo"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@joopo/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkJoopoPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveJoopoPackageRootSync);
  ...
  it("creates a node_modules/joopo symlink when peerDependencies declares joopo", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { joopo: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "joopo");
  ...
  it("keeps the joopo peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { joopo: "*" }, { "is-number": "7.0.0" });
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "joopo");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "joopo");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("warns and skips when resolveJoopoPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expect(result.ok).toBe(true);
    expect(warnings.some((w) => w.includes("Could not locate joopo package root"))).toBe(true);
  });
});
