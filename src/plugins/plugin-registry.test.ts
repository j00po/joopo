...
function makeTempDir() {
  return makeTrackedTempDir("joopo-plugin-registry", tempDirs);
}

function hermeticEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    JOOPO_BUNDLED_PLUGINS_DIR: undefined,
    JOOPO_VERSION: "2026.4.25",
    VITEST: "true",
    ...overrides,
  };
}
...
function createIndex(
  ...
  return {
    ...
    plugins: [
      {
        pluginId,
        manifestPath: path.join(pluginRoot, "joopo.plugin.json"),
...
describe("plugin registry facade", () => {
  ...
  it("keeps absolute and home plugin API paths user-resolved", () => {
    ...
    expect(resolvePluginPath("~/joopo/plugin.txt", pluginRoot)).toBe(
      resolvePluginPath("~/joopo/plugin.txt", undefined),
    );
  });
  ...
  it("resolves contribution owners from a plugin lookup table without rereading manifests", () => {
    ...
    fs.unlinkSync(path.join(rootDir, "joopo.plugin.json"));
      ...
      plugins: [
        {
          ...
          manifestPath: path.join(rootDir, "joopo.plugin.json"),
          source: path.join(rootDir, "index.ts"),
          rootDir,
        },
      ],
  ...
  it("reads the persisted registry before deriving from discovered candidates", async () => {
    ...
    fs.writeFileSync(
      path.join(persistedRootDir, "joopo.plugin.json"),
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(persistedRootDir, "joopo.plugin.json"),
            manifestHash: hashFile(path.join(persistedRootDir, "joopo.plugin.json")),
  ...
  it("falls back to the derived registry when persisted bundled roots point at another checkout", async () => {
    ...
    await writePersistedInstalledPluginIndex(
      createIndex("persisted", {
        plugins: [
          {
            ...createIndex("persisted").plugins[0],
            manifestPath: path.join(staleBundledRootDir, "joopo.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ JOOPO_BUNDLED_PLUGINS_DIR: rootDir }),
    });
  ...
  it("derives fresh config-scoped registries when the persisted registry is missing", () => {
    ...
    createCandidate(rootDir);
    const env = hermeticEnv({ JOOPO_BUNDLED_PLUGINS_DIR: bundledRoot });
    ...
    const manifestReadsAfterFirst = readFileSyncSpy.mock.calls.filter((call) =>
      String(call[0]).endsWith("joopo.plugin.json"),
    ).length;
