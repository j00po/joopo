...
import { listJoopoPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  JOOPO_DISABLE_BUNDLED_PLUGINS: process.env.JOOPO_DISABLE_BUNDLED_PLUGINS,
  JOOPO_BUNDLED_PLUGINS_DIR: process.env.JOOPO_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/joopo.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reflects manifest edits and state-dir changes on the next lookup", () => {
    ...
    process.env.JOOPO_STATE_DIR = stateDirA;
    process.env.JOOPO_HOME = undefined;
    process.env.JOOPO_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.JOOPO_STATE_DIR = stateDirB;
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listJoopoPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listJoopoPluginManifestMetadata(process.env)).toHaveLength(1);
