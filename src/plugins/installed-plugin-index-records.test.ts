...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@joopo/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@joopo/plugin-twitch",
        },
      },
      {
        stateDir,
