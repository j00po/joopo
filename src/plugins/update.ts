...
import {
  ...
  installedPackageNeedsJoopoPeerLinkRepair,
...
import { linkJoopoPeerDependencies } from "./plugin-peer-link.js";
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareJoopoReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function migratePluginConfigId(cfg: JoopoConfig, fromId: string, toId: string): JoopoConfig {
  if (fromId === toId) {
    return cfg;
  }
...
function disablePluginConfigEntry(config: JoopoConfig, pluginId: string): JoopoConfig {
...
async function repairJoopoPeerLinksForNpmInstalls(params: {
  config: JoopoConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair joopo peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsJoopoPeerLinkRepair(installPath)) {
      continue;
    }

    const peerDependencies = readInstalledPackagePeerDependencies(installPath);
    if (!Object.hasOwn(peerDependencies, "joopo")) {
      continue;
    }
    ...
    try {
      ...
      repaired = !installedPackageNeedsJoopoPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair joopo peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (pluginId: string, message: string) => {
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; Joopo will continue without it. ` +
        message;
    ...
    if (!params.dryRun && record.source === "npm" && currentVersion) {
      ...
      if (metadataResult.ok) {
        if (
          ...
          !installedPackageNeedsJoopoPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairJoopoPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
