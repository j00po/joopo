...
/**
 * Symlink the host joopo package for plugins that declare it as a peer.
 ...
 */
export async function linkJoopoPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<void> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "joopo");
  if (peers.length === 0) {
    return;
  }
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate joopo package root to symlink peerDependencies; plugin may fail to resolve joopo at runtime.",
    );
    return;
  }
...
export async function relinkJoopoPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
