...
import { bundledPluginRootAt } from "joopo/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-joopo",
    targetDir: params?.targetDir ?? "/tmp/opik-joopo",
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createJoopoPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { joopo: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "joopo");
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@joopo/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["joopo-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/joopo-codex-app-server",
    version: params.version,
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-joopo",
        spec: "@opik/opik-joopo",
        integrity: "sha512-old",
        installPath: "/tmp/opik-joopo",
      }),
      pluginIds: ["opik-joopo"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-joopo",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        ...
        spec: "joopo-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/joopo-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "joopo-codex-app-server",
        targetDir: "/tmp/joopo-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "joopo-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        spec: "@joopo/acpx",
        installPath,
        resolvedName: "@joopo/acpx",
        resolvedSpec: "@joopo/acpx@2026.5.2-beta.1",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@joopo/acpx",
          version: "2026.5.2",
          resolvedSpec: "@joopo/acpx@2026.5.2",
        },
      }),
    );
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@joopo/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@joopo/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@joopo/acpx",
        resolvedSpec: "@joopo/acpx@2026.5.3",
  ...
  it("repairs missing joopo peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@joopo/codex",
      version: "2026.5.3",
      peerDependencies: { joopo: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@joopo/codex",
      version: "2026.5.3",
  ...
  it("skips unchanged npm plugins when the joopo peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@joopo/codex",
      version: "2026.5.3",
      peerDependencies: { joopo: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "joopo"), { recursive: true });
    mockNpmViewMetadata({
      name: "@joopo/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs joopo peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@joopo/brave-plugin" },
      { pluginId: "codex", packageName: "@joopo/codex" },
      { pluginId: "discord", packageName: "@joopo/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createJoopoPeerLinkFixtures(plugins);
  ...
  it("repairs sibling joopo peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@joopo/brave-plugin" },
      { pluginId: "codex", packageName: "@joopo/codex" },
      { pluginId: "discord", packageName: "@joopo/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@joopo/codex",
            version: "2026.5.4",
            resolvedSpec: "@joopo/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling joopo peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@joopo/brave-plugin" },
      { pluginId: "codex", packageName: "@joopo/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@joopo/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@joopo/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@joopo/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toContainEqual(
      expect.stringContaining('Could not repair joopo peer link for "broken"'),
    );
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".joopo", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.joopo/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-joopo@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-joopo@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-joopo" (@opik/opik-joopo@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-joopo: aborted: npm package integrity drift detected for @opik/opik-joopo@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @joopo/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@joopo/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @joopo/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "joopo-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "joopo-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "joopo-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "joopo-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "joopo-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "joopo-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "joopo-codex-app-server",
      });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for joopo-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "joopo-codex-app-server",
          ...
          npmResolution: {
            name: "joopo-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "joopo-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update joopo-codex-app-server: npm package not found for joopo-codex-app-server.",
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "joopo-codex-app-server@rc",
      }),
      pluginIds: ["joopo-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@joopo/voice-call",
      targetDir: "/tmp/joopo-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@joopo/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@joopo/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@joopo/voice-call");
    expect(result.config.plugins?.entries?.["@joopo/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@joopo/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@joopo/voice-call",
      installPath: "/tmp/joopo-voice-call",
      version: "0.0.2",
    });
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@joopo/context-engine",
      targetDir: "/tmp/joopo-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@joopo/context-engine");
    expect(result.config.plugins?.installs?.["@joopo/context-engine"]).toMatchObject({
      source: "npm",
      spec: "@joopo/context-engine",
      installPath: "/tmp/joopo-context-engine",
      version: "0.0.2",
    });
  ...
  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/joopo/extensions/demo";
    const extensionsDir = "/var/joopo/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/joopo-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        version: "2.0.0",
        npmResolution: {
          name: "@joopo/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@joopo/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(result.config.plugins?.installs?.["legacy-chat"]).toMatchObject({
      ...
      version: "2.0.0",
      resolvedName: "@joopo/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/joopo-plugins/voice-call",
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/joopo-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(result.config.plugins?.installs?.["default-chat"]).toMatchObject({
      source: "npm",
      spec: "@joopo/default-chat",
      installPath: "/tmp/joopo-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@joopo/legacy-chat@1.2.3",
              resolvedSpec: "@joopo/legacy-chat@1.2.3",
              installPath: "/tmp/joopo-plugins/legacy-chat",
            },
          },
        },
