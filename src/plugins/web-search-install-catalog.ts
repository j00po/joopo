...
function getConfigPath(config: JoopoConfig | undefined, path: string): unknown {
...
function setConfigPath(target: JoopoConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: JoopoConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: JoopoConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: JoopoConfig) =>
