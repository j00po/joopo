...
import { normalizeJoopoVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeJoopoVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeJoopoVersionBase(plugin.version) ??
    plugin.version
  );
}
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "joopo";
