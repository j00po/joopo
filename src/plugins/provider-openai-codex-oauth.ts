...
const openAICodexOAuthOriginator = "joopo";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Joopo process and then retry `joopo models auth login --provider openai-codex`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://docs.joopo.ai/start/faq", "OAuth help");
