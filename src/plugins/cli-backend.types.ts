...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Joopo consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Joopo should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Joopo's bundle
   ...
   *
   * Use this when the backend should consume a canonical Joopo auth profile
   ...
   * `profile-only` treats the selected Joopo auth profile as the sole auth
   ...
   * as static config, such as mapping Joopo thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Joopo's tool
