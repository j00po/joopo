...
function resolveConfiguredDefaultModelPrimary(cfg: JoopoConfig): string | undefined {
...
async function applyDefaultModelFromAuthChoice(params: {
  config: JoopoConfig;
  configBeforeProviderAuth?: JoopoConfig;
  selectedModel: string;
  selectedModelDisplay?: string;
  preserveExistingDefaultModel: boolean | undefined;
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  workspaceDir?: string;
  runSelectedModelHook: (config: JoopoConfig) => Promise<void>;
}): Promise<JoopoConfig> {
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: JoopoConfig; defaultModel?: string }> {
...
export async function applyAuthChoiceLoadedPluginProvider(
  ...
  const resolveScopedRuntimeProviders = (config: JoopoConfig): ProviderPlugin[] =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
...
export async function applyAuthChoicePluginProvider(
  ...
  if (!provider) {
    await params.prompter.note(
      `${options.label} auth plugin is not available. Install or enable the plugin, then rerun onboarding. If this started after an update, run "joopo doctor --fix" first.`,
      options.label,
    );
    return { config: nextConfig };
  }
