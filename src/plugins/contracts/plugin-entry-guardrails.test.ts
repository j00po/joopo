...
function analyzeSourceModule(params: { filePath: string; source: string }): {
  ...
  for (const match of source.matchAll(STATIC_FROM_IMPORT_RE)) {
    ...
    if (specifier === "joopo/plugin-sdk/core" && importsDefinePluginEntry(importClause)) {
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          joopo?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.joopo?.extensions) ? pkg.joopo.extensions : [];
        if (
          extensions.some(
  ...
  it("follows relative import edges while scanning guarded contract graphs", () => {
    expect(
      analyzeSourceModule({
        ...
        import { y } from "joopo/plugin-sdk/testing";
      `,
  ...
  it("detects aliased definePluginEntry imports from core", () => {
    expect(
      analyzeSourceModule({
        ...
        source: `
          import { definePluginEntry as dpe } from "joopo/plugin-sdk/core";
          import { somethingElse } from "joopo/plugin-sdk/core";
        `,
      }).importsDefinePluginEntryFromCore,
    ).toBe(true);
  });
