...
} from "joopo/plugin-sdk/plugin-test-runtime";
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "joopo/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "joopo/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolveWorkspacePackagePublicModuleUrl } from "../../plugin-sdk/test-helpers/public-surface-loader.js";

type TtsRuntimeModule = typeof import("joopo/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("joopo/plugin-sdk/speech-core");
...
function asLegacyTtsConfig(value: unknown): JoopoConfig {
  return value as JoopoConfig;
}

function asLegacyJoopoConfig(value: Record<string, unknown>): JoopoConfig {
  return value as unknown as JoopoConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): JoopoConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("joopo/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: JoopoConfig): ResolvedTtsConfig {
  const rawConfig =
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: JoopoConfig = asLegacyJoopoConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.JOOPO_TTS_PREFS;
      process.env.JOOPO_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.JOOPO_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: JoopoConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
