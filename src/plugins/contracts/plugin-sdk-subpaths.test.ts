...
import type {
  ...
  JoopoPluginApi as CoreJoopoPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "joopo/plugin-sdk/core";
import * as providerEntrySdk from "joopo/plugin-sdk/provider-entry";
...
import type {
  ...
  JoopoPluginApi as SharedJoopoPluginApi,
...
import type { JoopoPluginApi } from "../types.js";
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_JOOPO_BROWSER_COLOR",
      "DEFAULT_JOOPO_BROWSER_ENABLED",
      "DEFAULT_JOOPO_BROWSER_PROFILE_NAME",
      "DEFAULT_UPLOAD_DIR",
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps the deprecated channel-runtime shim unused in repo imports", () => {
    const matches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']joopo\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreJoopoPluginApi>().toMatchTypeOf<JoopoPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreJoopoPluginApi>().toMatchTypeOf<SharedJoopoPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("joopo/plugin-sdk/core");
    const channelActionsSdk = await importResolvedPluginSdkSubpath(
      "joopo/plugin-sdk/channel-actions",
    );
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "joopo/plugin-sdk/global-singleton",
    );
    const textRuntimeSdk = await importResolvedPluginSdkSubpath("joopo/plugin-sdk/text-runtime");
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("joopo/plugin-sdk/plugin-entry");
    const channelLifecycleSdk = await importResolvedPluginSdkSubpath(
      "joopo/plugin-sdk/channel-lifecycle",
    );
    const channelPairingSdk = await importResolvedPluginSdkSubpath(
      "joopo/plugin-sdk/channel-pairing",
    );
    const channelReplyPipelineSdk = await importResolvedPluginSdkSubpath(
      "joopo/plugin-sdk/channel-reply-pipeline",
    );
    const representativeModules = [];
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`joopo/plugin-sdk/${id}`));
    }
  ...
  it("keeps the Zalouser command-auth compatibility facade importable", async () => {
    const zalouserSdk = await importResolvedPluginSdkSubpath("joopo/plugin-sdk/zalouser");
    const commandAuthSdk = await importResolvedPluginSdkSubpath("joopo/plugin-sdk/command-auth");
