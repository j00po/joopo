  ...
  return fs
    ...
    .filter((pluginDir) => {
      ...
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        joopo?: JoopoPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.joopo?.extensions).length > 0;
    })
    .map(
      (pluginDir) =>
        JSON.parse(
          fs.readFileSync(path.join(pluginDir, "joopo.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
