...
const suiteTempRootTracker = createSuiteTempRootTracker("joopo-plugin-install-npm-spec");
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootJoopo) {
      lockPackages["node_modules/joopo"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalJoopoPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      fs.rmSync(path.join(packageDir, "node_modules", "joopo"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (argv[0] === "npm" && argv[1] === "install") {
        ...
        prunePluginLocalJoopoPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          ...
          if (pkg.materializesRootJoopo) {
            const joopoRoot = path.join(npmRoot, "node_modules", "joopo");
            fs.mkdirSync(joopoRoot, { recursive: true });
            fs.writeFileSync(
              path.join(joopoRoot, "package.json"),
              JSON.stringify({ name: "joopo", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "joopo") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "joopo"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    const archivePath = path.join(stateDir, "joopo-pack-demo-1.2.3.tgz");
    fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

    mockNpmViewAndInstallMany([
      {
        packageName: "@joopo/pack-demo",
        version: "1.2.3",
        pluginId: "pack-demo",
        npmRoot,
    ...
    expect(result.targetDir).toBe(path.join(npmRoot, "node_modules", "@joopo/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@joopo/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("joopo-pack-demo-1.2.3.tgz");
    ...
    const dependencySpec = managedManifest.dependencies?.["@joopo/pack-demo"];
    expect(dependencySpec).toMatch(/^file:\.\/_joopo-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_joopo-pack-archives"))).toBe(false);
    expect(runCommandWithTimeoutMock.mock.calls).toHaveLength(1);
  });

  it("installs npm plugins into .joopo/npm", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    ...
    expect(result.targetDir).toBe(path.join(npmRoot, "node_modules", "@joopo/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@joopo/voice-call@0.0.1");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed joopo peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { joopo: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs
          .lstatSync(path.join(npmRoot, "node_modules", "peer-plugin", "node_modules", "joopo"))
          .isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/joopo");
      }
      expect(
        fs
          .lstatSync(path.join(npmRoot, "node_modules", "peer-plugin", "node_modules", "joopo"))
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root joopo materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(result.ok).toBe(true);
      expect(fs.existsSync(path.join(npmRoot, "node_modules", "joopo"))).toBe(false);
      ...
      expect(
        fs
          .lstatSync(
            path.join(npmRoot, "node_modules", "required-peer-plugin", "node_modules", "joopo"),
          )
          .isSymbolicLink(),
      ).toBe(true);
    },
  ...
  it("repairs stale managed joopo root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    fs.mkdirSync(path.join(npmRoot, "node_modules", "joopo"), { recursive: true });
    fs.writeFileSync(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/joopo": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/joopo/-/joopo-2026.5.4.tgz",
            },
          },
          dependencies: {
            joopo: {
    ...
    mockNpmViewAndInstall({
      spec: "@joopo/discord@beta",
      packageName: "@joopo/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { joopo: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("joopo");
    expect(manifest.dependencies).toMatchObject({
      "@joopo/discord": "2026.5.5-beta.1",
    });
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmRoot, "package-lock.json"), "utf8"),
    ) as {
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "joopo");
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@joopo/voice-call@0.0.1"))) {
        return successfulSpawn(
          JSON.stringify({
    ...
    await expect(
      fs.promises.access(path.join(npmRoot, "node_modules", "joopo")),
    ).rejects.toThrow();
  });
  ...
  it.each(officialLaunchPluginCases)(
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Joopo package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = path.join(npmRoot, "node_modules", "@joopo", "voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@joopo", "voice-call"))).toBe(true);
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@joopo", "whatsapp"))).toBe(true);
  });
  ...
  it("handles prerelease npm specs correctly", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@joopo/voice-call@beta"');
    }
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@joopo/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @joopo/voice-call@0.0.1");
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@joopo/voice-call@2026.5.3-1");
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@joopo/voice-call@0.0.2-beta.1",
        packageName: "@joopo/voice-call",
        version: "0.0.2-beta.1",
        pluginId: "voice-call",
        npmRoot: prereleaseOnlyNpmRoot,
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@joopo/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @joopo/voice-call@0.0.2-beta.1",
    );
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@joopo/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoRoot({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
    });
