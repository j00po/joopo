...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("joopo-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("joopo-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("joopo-marketplace-test-", async (homeDir) => {
      const joopoHome = path.join(homeDir, "joopo-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, JOOPO_HOME: joopoHome },
