...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "joopo", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".joopo", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".joopo", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "joopo-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".joopo", "extensions"),
      },
    });
  });
});
