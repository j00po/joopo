...
const discoverJoopoPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverJoopoPlugins: (...args: unknown[]) => discoverJoopoPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverJoopoPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
  ...
  loadPluginManifestMock.mockImplementation((rootDir: string) =>
    rootDir in manifestIds
      ...
      : {
          ...
          manifestPath: `${rootDir}/joopo.plugin.json`,
        },
  );
}
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@joopo/${params.pluginId}`,
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverJoopoPluginsMock.mockReset();
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    const env = { HOME: "/tmp/joopo-home" } as NodeJS.ProcessEnv;
    ...
    expect(discoverJoopoPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverJoopoPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("marks bundled sources that require plugin config before activation", () => {
    setBundledDiscoveryCandidates([
      createBundledCandidate({
        ...
        packageName: "@joopo/memory-lancedb",
      }),
    ]);
    setBundledManifestIdsByRoot({
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@joopo/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});
