...
import { clearPluginLoaderCache, loadJoopoPlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadJoopoPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadJoopoPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "joopo-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.JOOPO_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "joopo.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".joopo", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ JOOPO_STATE_DIR: stateDir, ...params.env }, () =>
    loadJoopoPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.JOOPO_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
