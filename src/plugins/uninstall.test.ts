...
type PluginConfig = NonNullable<JoopoConfig["plugins"]>;
...
function createSinglePluginWithEmptySlotsConfig(): JoopoConfig {
...
function createSingleNpmInstallConfig(installPath: string): JoopoConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "joopo-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as JoopoConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/joopo/plugin",
            "/missing/joopo/plugin",
          ),
        },
        loadPaths: ["/missing/joopo/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@joopo", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("joopo-kitchen-sink-fixture"),
        installs: {
          "joopo-kitchen-sink-fixture": {
            source: "npm",
            spec: "@joopo/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
      pluginId: "joopo-kitchen-sink-fixture",
      deleteFiles: true,
      extensionsDir,
    });
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@joopo/kitchen-sink",
      },
    });
  ...
  it("repairs remaining npm plugin joopo peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    await expect(fs.access(path.join(npmRoot, "node_modules", "joopo"))).rejects.toThrow();
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "joopo-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-joopo-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    ...
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@joopo", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@joopo/kitchen-sink@latest",
          installPath,
        },
        extensionsDir,
      }),
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@joopo/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@joopo", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("joopo-kitchen-sink-fixture", extensionsDir));
  });
