...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "joopo-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): JoopoConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: JoopoConfig, pluginId: string): Record<string, unknown> {
