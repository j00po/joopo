...
const originalBundledDir = process.env.JOOPO_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
...
function createJoopoRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "joopo" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@joopo/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.JOOPO_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "joopo.mjs"),
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
  if (originalDisableBundledPlugins === undefined) {
    delete process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.JOOPO_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "joopo-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "joopo-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "joopo-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "joopo-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    const repoRoot = createJoopoRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "joopo.mjs");
    ...
    process.env.JOOPO_DISABLE_BUNDLED_PLUGINS = "1";
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by JOOPO_HOME", () => {
    const homeA = makeRepoRoot("joopo-bundled-dir-home-a-");
    const homeB = makeRepoRoot("joopo-bundled-dir-home-b-");
    ...
    const bundledA = resolveBundledPluginsDir({ ...envBase, JOOPO_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, JOOPO_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "joopo.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("joopo-bundled-dir-vitest-override-reject-");
    ...
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("joopo-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("joopo-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.JOOPO_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("joopo-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("joopo-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "joopo.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.JOOPO_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createJoopoRoot({
      prefix: "joopo-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createJoopoRoot({
          prefix: "joopo-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createJoopoRoot({
          prefix: "joopo-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "joopo.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createJoopoRoot({
          prefix: "joopo-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
