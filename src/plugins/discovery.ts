...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @joopo/voice-call -> voice-call).
...
export function discoverJoopoPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  const scopedResult = tracePluginLifecyclePhase(
    ...
    () => {
      ...
      for (const extraPath of extra) {
        ...
        if (bundledAlias) {
          result.diagnostics.push({
            level: "warn",
            source: trimmed,
            message: `ignored plugins.load.paths entry that points at Joopo's ${bundledAlias.kind} bundled plugin directory; remove this redundant path or run joopo doctor --fix`,
          });
          continue;
        }
        discoverFromPath({
      ...
      if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
        // Keep workspace auto-discovery constrained to the Joopo extensions root.
