...
export const PLUGIN_MANIFEST_FILENAME = "joopo.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `joopo qa`, for example `matrix`. */
  commandName: string;
...
// package.json "joopo" metadata (used for setup/catalog)
...
export type JoopoPackageStartup = {
  /**
...
export type JoopoPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type JoopoPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: JoopoPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
...
} & Partial<Record<ManifestKey, JoopoPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): JoopoPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
