...
import { discoverJoopoPlugins, type PluginCandidate } from "./discovery.js";
...
function createGuardedPluginRegistrationApi(api: JoopoPluginApi): {
  api: JoopoPluginApi;
  close: () => void;
} {
  let closed = false;
...
function runPluginRegisterSync(
  register: NonNullable<JoopoPluginDefinition["register"]>,
  api: Parameters<NonNullable<JoopoPluginDefinition["register"]>>[0],
): void {
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadJoopoPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadJoopoPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: JoopoPluginDefinition;
  register?: JoopoPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as JoopoPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as JoopoPluginDefinition;
...
export function loadJoopoPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into Joopo yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: JoopoPluginModule | null = null;
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as JoopoPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: JoopoPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as JoopoPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'joopo plugins list' for details.`,
        );
      }
    }
...
export async function loadJoopoPluginCliRegistry(
  options: PluginLoadOptions = {},
