...
type LoadJoopoPlugins = typeof import("./loader.js").loadJoopoPlugins;
...
const loadJoopoPluginsMock = vi.fn<LoadJoopoPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/joopo.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadJoopoPluginsMock.mock.calls.at(-1)?.[0];
  if (!call) {
    throw new Error("expected Joopo plugin setup loader to be called");
  }
...
function createAutoEnabledProviderConfig() {
  const rawConfig: JoopoConfig = {
    plugins: {},
  };
  const autoEnabledConfig: JoopoConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadJoopoPlugins: (...args: Parameters<LoadJoopoPlugins>) =>
        loadJoopoPluginsMock(...args),
