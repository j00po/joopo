...
vi.mock("./channel-bridge.js", () => ({
  JoopoChannelBridge: class MockJoopoChannelBridge {
...
describe("serveJoopoChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveJoopoChannelMcp } = await import("./channel-server.js");

    const servePromise = serveJoopoChannelMcp({ verbose: false });
