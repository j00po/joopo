...
export class JoopoChannelBridge {
  ...
  constructor(
    private readonly cfg: JoopoConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`joopo mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `joopo mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
