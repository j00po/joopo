/**
 * Standalone MCP server that exposes Joopo plugin-registered tools
...
function resolvePluginToolPolicy(config: JoopoConfig): {
...
function resolveTools(config: JoopoConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "joopo-plugin-tools", tools });
}
