...
import { readRootJsonObjectSync } from "@joopo/fs-safe/json";
...
export function installedPackageNeedsJoopoPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "joopo"));
    return false;
  } catch (error) {
