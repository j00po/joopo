...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "joopo-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".joopo");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".joopo", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      JOOPO_STATE_DIR: path.join(home, ".joopo"),
      ...env,
    };
