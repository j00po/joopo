...
const JOOPO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "joopo.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.JOOPO_DIAGNOSTICS)) &&
    typeof env.JOOPO_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.JOOPO_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: JOOPO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.JOOPO_DIAGNOSTICS_RUN_ID ? { runId: env.JOOPO_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.JOOPO_DIAGNOSTICS_ENV ? { envName: env.JOOPO_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.JOOPO_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
