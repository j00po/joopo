...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: JoopoConfig | null = null;
  ...
  get(key: string, cfg: JoopoConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: JoopoConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: JoopoConfig): void {
