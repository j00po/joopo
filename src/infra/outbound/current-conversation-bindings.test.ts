...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.JOOPO_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-current-bindings-"));
    process.env.JOOPO_STATE_DIR = testStateDir;
