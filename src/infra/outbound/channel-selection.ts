...
function isConfiguredChannel(cfg: JoopoConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: joopo doctor --fix, or install individually: ${installCommands}.`;
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: JoopoConfig): Promise<boolean> {
