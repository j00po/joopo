...
const matrixChunkConfig: JoopoConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as JoopoConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredJoopoTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: JoopoConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as JoopoConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as JoopoConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as JoopoConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    const cfg = { channels: { line: {} } } as JoopoConfig;
    await deliverOutboundPayloads({
      cfg,
      channel: "line",
      to: "U123",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".joopo", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });

  it("includes Joopo tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as JoopoConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as JoopoConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for joopo/joopo#70386: if a reconnect or startup drain
