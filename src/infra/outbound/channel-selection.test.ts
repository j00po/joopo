  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          installSpec: `@joopo/${channelId}`,
          installCommand: `joopo plugins install @joopo/${channelId}`,
          doctorFixCommand: "joopo doctor --fix",
          repairHint: `Install the official external plugin with: joopo plugins install @joopo/${channelId}, or run: joopo doctor --fix.`,
        }
      : null,
}));
...
describe("resolveMessageChannelSelection", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedMessage:
        "Channel is unavailable: feishu. Install the official external plugin with: joopo plugins install @joopo/feishu, or run: joopo doctor --fix.",
    },
    {
      params: { cfg: {} as never },
    ...
    },
    {
      ...
      expectedMessage:
        "Channel is required (no available channels detected). Configured official external channel WhatsApp is missing its plugin. Install the official external plugin with: joopo plugins install @joopo/whatsapp, or run: joopo doctor --fix.",
    },
    {
      setup: () => {
        mocks.listChannelPlugins.mockReturnValue([
