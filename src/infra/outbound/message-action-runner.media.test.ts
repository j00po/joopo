...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Joopo tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideJoopo = path.join(os.tmpdir(), "outside-joopo", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
