...
vi.mock("./joopo-root.js", async () => {
  const actual = await vi.importActual<typeof import("./joopo-root.js")>("./joopo-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "joopo-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv([
      "JOOPO_NO_AUTO_UPDATE",
      "JOOPO_STATE_DIR",
      "NODE_ENV",
      "VITEST",
    ]);
    process.env.JOOPO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveJoopoPackageRoot } = await import("./joopo-root.js"));
    ...
    vi.mocked(resolveJoopoPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveJoopoPackageRoot).mockResolvedValue("/opt/joopo");
  ...
  it("honors JOOPO_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.JOOPO_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    expect(runAutoUpdate).not.toHaveBeenCalled();
    expect(log.info).toHaveBeenCalledWith(
      "auto-update disabled by JOOPO_NO_AUTO_UPDATE",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/joopo/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
