...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "joopo.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "joopo.plugin.json");
...
function statusCommandSpawn() {
  return [process.execPath, "joopo.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "joopo.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "joopo.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "joopo.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "joopo.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          JOOPO_FORCE_BUILD: "1",
          JOOPO_RUNNER_LOG: "1",
          JOOPO_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[joopo]");
      expect(spawnCalls.at(-1)?.args).toEqual(["joopo.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.JOOPO_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "joopo.mjs" ? childStderr : "",
        });
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          JOOPO_RUNNER_LOG: "0",
          JOOPO_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          JOOPO_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched Joopo child when requested", async () => {
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          JOOPO_RUNNER_LOG: "0",
          JOOPO_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=joopo-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["joopo.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.JOOPO_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
      expect(fsSync.existsSync(profileDir)).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched Joopo child when requested", async () => {
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "joopo.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["joopo.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.JOOPO_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
      ...
      expect(runRuntimePostBuild).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            JOOPO_BUILD_PRIVATE_QA: "1",
            JOOPO_ENABLE_PRIVATE_QA_CLI: "1",
          }),
        }),
      );
    });
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"joopo":{"extensions":["./index.ts"]}}\n',
      ...
      await expect(
        Promise.all([
          runStatusCommand({
            ...
            spawnSync,
            env: {
              JOOPO_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
            },
            runRuntimePostBuild,
          }),
          runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active joopo child and returns 143", async () => {
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("skips rebuilding when extension package metadata is newer than the build stamp", async () => {
    await withTempDir({ prefix: "joopo-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","joopo":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","joopo":{"extensions":["./stale.js"]}}\n',
        },
