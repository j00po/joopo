...
import { resolveJoopoPackageRoot } from "./joopo-root.js";
...
function resolveAutoUpdatePolicy(cfg: JoopoConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: JoopoConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.JOOPO_NO_AUTO_UPDATE);
  ...
  const root = await resolveJoopoPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("joopo update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by JOOPO_NO_AUTO_UPDATE", {
