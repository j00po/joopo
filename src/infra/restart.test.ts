...
let triggerJoopoRestart: typeof import("./restart.js").triggerJoopoRestart;
...
beforeAll(async () => {
  ...
  ({ triggerJoopoRestart } = await import("./restart.js"));
});
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-joopo/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "cjoopo", `p${stalePidB}`, "cjoopo-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerJoopoRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    ...
    const result = triggerJoopoRestart();
    ...
    expect(result).toEqual({
      ...
      tried: [
        ...
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.joopo.gateway.plist`,
      ],
    });
  });
  ...
  it("continues when launchctl bootstrap reports the service is already loaded", () => {
    ...
    expect(result).toEqual({
      ...
      tried: [
        `launchctl kickstart -k gui/${uid}/ai.joopo.gateway`,
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.joopo.gateway.plist`,
        `launchctl kickstart gui/${uid}/ai.joopo.gateway`,
      ],
    });
  });
});
