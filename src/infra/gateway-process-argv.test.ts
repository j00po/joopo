...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Joopo\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/joopo/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/joopo/joopo.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/joopo/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/joopo/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the joopo executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\joopo.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/joopo-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/joopo-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\joopo-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/joopo/custom.js", "gateway"])).toBe(false);
