...
import { bundledPluginFile } from "joopo/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "joopo.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "joopo-watch-node-" }, async (cwd) => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            PATH: "/usr/bin",
            JOOPO_WATCH_MODE: "1",
            JOOPO_WATCH_SESSION: "1700000000000-4242",
            JOOPO_NO_RESPAWN: "1",
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          env: expect.not.objectContaining({
            JOOPO_TRACE_SYNC_IO: expect.any(String),
          }),
        }),
      );
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { JOOPO_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    const runPromise = runWatch({
      ...
      createWatcher,
      env: {
        LAUNCH_JOB_LABEL: "ai.joopo.gateway",
        PATH: "/usr/bin",
      },
      lockDisabled: true,
      process: fakeProcess,
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/joopo/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/joopo/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[joopo] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[joopo] Invalid package config: /tmp/joopo/.pnpm/chokidar/package.json"],
        ["[joopo] This usually means a file in node_modules is empty or truncated."],
        ["[joopo] Recommended recovery:"],
        ["[joopo]   rm -rf node_modules"],
        ["[joopo]   pnpm store prune"],
        ["[joopo]   pnpm install"],
        [""],
        ["[joopo] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "joopo-watch-node-lock-" }, async (cwd) => {
