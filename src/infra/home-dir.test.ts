...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers JOOPO_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for JOOPO_HOME",
      env: { JOOPO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when JOOPO_HOME is tilde-only and no fallback home exists",
      env: { JOOPO_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores JOOPO_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          JOOPO_HOME: "/srv/joopo-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/joopo-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/joopo-home " },
      expected: "/srv/joopo-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/joopo-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of JOOPO_HOME", () => {
    expect(
