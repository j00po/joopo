  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set JOOPO_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.JOOPO_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
