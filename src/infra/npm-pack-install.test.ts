...
vi.mock("./install-source-utils.js", async () => {
  ...
  return {
    ...
    withTempDir: vi.fn(async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
      return await fn("/tmp/joopo-npm-pack-install-test");
    }),
...
describe("installFromNpmSpecArchive", () => {
  const baseSpec = "@joopo/test@1.0.0";
  const baseArchivePath = "/tmp/joopo-test.tgz";
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    expect(withTempDir).toHaveBeenCalledWith("joopo-test-", expect.any(Function));
  });
  ...
  it("returns resolution metadata and installer result on success", async () => {
    mockPackedSuccess({ name: "@joopo/test", version: "1.0.0" });
    ...
    expect(okResult.npmResolution.resolvedSpec).toBe("@joopo/test@1.0.0");
    expect(Date.parse(okResult.npmResolution.resolvedAt)).not.toBeNaN();
    expect(installFromArchive).toHaveBeenCalledWith({ archivePath: "/tmp/joopo-test.tgz" });
  });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @joopo/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and aborts on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @joopo/test@1.0.0: expected sha512-old, got sha512-new",
    );
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("rejects prerelease resolutions unless explicitly requested", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ...
      archivePath: baseArchivePath,
      metadata: {
        resolvedSpec: "@joopo/test@latest",
...
describe("installFromNpmSpecArchiveWithInstaller", () => {
  ...
  it("passes archive path and installer params to installFromArchive", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/joopo-plugin.tgz",
      metadata: {
        resolvedSpec: "@joopo/voice-call@1.0.0",
        integrity: "sha512-same",
      },
    });
