...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevJoopoHome = process.env.JOOPO_HOME;
    const prevHome = process.env.HOME;
    process.env.JOOPO_HOME = "/srv/joopo-home";
    process.env.HOME = "/home/other";
    const joopoHome = path.join(path.resolve("/srv/joopo-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", joopoHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevJoopoHome === undefined) {
        delete process.env.JOOPO_HOME;
      } else {
        process.env.JOOPO_HOME = prevJoopoHome;
      }
