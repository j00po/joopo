...
import { bundledDistPluginFile } from "joopo/plugin-sdk/test-fixtures";
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "joopo", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["JOOPO_UPDATE_PACKAGE_SPEC"]);
    process.env.JOOPO_UPDATE_PACKAGE_SPEC = "file:/tmp/joopo.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "joopo", tag: "latest" })).toBe(
      "file:/tmp/joopo.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "joopo",
        tag: "beta",
        env: { JOOPO_UPDATE_PACKAGE_SPEC: "joopo@next" },
      }),
    ).toBe("joopo@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "joopo"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "joopo", tag: "main" })).toBe(
      JOOPO_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "joopo",
        tag: "github:joopo/joopo#feature/my-branch",
      }),
    ).toBe("github:joopo/joopo#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "joopo",
        tag: "https://example.com/joopo-main.tgz",
      }),
    ).toBe("https://example.com/joopo-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    ...
    try {
      await withTempDir({ prefix: "joopo-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:joopo/joopo#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/joopo-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/joopo-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:joopo/joopo#main")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "joopo-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "joopo");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "joopo"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "joopo"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "joopo"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "joopo"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    ...
    try {
      await withTempDir({ prefix: "joopo-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "joopo@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "joopo@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "joopo@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "joopo@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "joopo-update-npm-missing-bin-" }, async (base) => {
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    try {
      await withTempDir({ prefix: "joopo-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "joopo");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "joopo-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "joopo");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "joopo-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "joopo@file+..+pack+joopo-2026.5.6.tgz",
        "node_modules",
        "joopo",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "joopo"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "joopo-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "joopo");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "joopo"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "joopo@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "joopo@latest",
    ...
    expect(globalInstallArgs("pnpm", "joopo@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "joopo@latest",
    ]);
    expect(globalInstallArgs("bun", "joopo@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "joopo@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "joopo@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "joopo@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "joopo@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "joopo@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "joopo@latest"]);
    expect(globalInstallArgs("pnpm", "joopo@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "joopo@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "joopo@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/joopo/lib/node_modules")).toEqual({
      prefix: "/opt/joopo",
      globalRoot: "/opt/joopo/lib/node_modules",
      binDir: "/opt/joopo/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "joopo-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".joopo-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".joopo-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".joopo-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "joopo"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".joopo-123", ".joopo-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "joopo"));
      const markerFileStat = await fs.stat(path.join(root, ".joopo-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "joopo-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "joopo-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".joopo-install-stage", ".joopo-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.joopo-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.joopo-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "joopo-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "joopo");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "joopo-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "joopo-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "joopo-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "joopo-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "matrix", "@joopo/matrix");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "joopo-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "matrix", "@joopo/matrix");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "joopo-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@joopo/qa-lab");
        await writePackageDistInventory(packageRoot);
