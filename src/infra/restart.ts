...
export function emitGatewayRestart(reasonOverride?: string): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerJoopoRestart() uses schtasks to restart the gateway.
...
export function triggerJoopoRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.JOOPO_SYSTEMD_UNIT,
      process.env.JOOPO_PROFILE,
    );
  ...
  const label =
    process.env.JOOPO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.JOOPO_PROFILE);
