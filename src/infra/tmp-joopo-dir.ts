...
export const POSIX_JOOPO_TMP_DIR = "/tmp/joopo";
...
export type ResolvePreferredJoopoTmpDirOptions = {
...
export function resolvePreferredJoopoTmpDir(
  options: ResolvePreferredJoopoTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "joopo" : `joopo-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[joopo] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Joopo temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Joopo temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_JOOPO_TMP_DIR;
