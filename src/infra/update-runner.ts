...
const DEFAULT_PACKAGE_NAME = "joopo";
...
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "joopo-update-preflight-";
...
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "JOOPO_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["joopo", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "joopo.mjs");
      ...
      const doctorStep = await runStep(
        step("joopo doctor", doctorArgv, gitRoot, {
