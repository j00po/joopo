...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: JoopoConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: JoopoConfig) {
...
function resolveHeartbeatAgents(cfg: JoopoConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: JoopoConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: JoopoConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: JoopoConfig, heartbeat?: HeartbeatConfig) {
...
function usesCodexHarness(params: {
  ...
  const runtimeId =
    ...
    normalizeHeartbeatRuntimeId(process.env.JOOPO_AGENT_RUNTIME) ||
...
function resolveHeartbeatAckMaxChars(cfg: JoopoConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: JoopoConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: JoopoConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: JoopoConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
