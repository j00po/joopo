...
describe("tmp-joopo-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__joopoTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_JOOPO_TMP_DIR, resolvePreferredJoopoTmpDir } from "./src/infra/tmp-joopo-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_JOOPO_TMP_DIR,
            resolverType: typeof resolvePreferredJoopoTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-joopo-dir-browser-entry.ts",
      },
      write: false,
    });
