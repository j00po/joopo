...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/joopo-state", "devices")).toEqual({
      dir: path.join("/tmp/joopo-state", "devices"),
      pendingPath: path.join("/tmp/joopo-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/joopo-state", "devices", "paired.json"),
    });
  });
