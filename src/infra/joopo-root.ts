...
import { joopoRootFs, joopoRootFsSync } from "./joopo-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["joopo"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await joopoRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(joopoRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = joopoRootFsSync.realpathSync(normalized);
...
export async function resolveJoopoPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveJoopoPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
...
export const __testing = {
  clearJoopoPackageRootCaches(): void {
