...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.JOOPO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*joopo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "joopo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "joopo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "joopo-gw");
  ...
  records.push(`_joopo-gw._tcp IN PTR ${instanceLabel}._joopo-gw._tcp`);
  records.push(`${instanceLabel}._joopo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._joopo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; joopo-content-hash: ${contentHash}\n${contentBody}`;
}
