...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@joopo/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@joopo/matrix", id: "matrix", expected: true },
    { packageName: "@joopo/matrix", id: "@joopo/matrix", expected: true },
    { packageName: "@joopo/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@joopo/matrix", id: " ", expected: false },
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "joopo-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "joopo-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "joopo-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "joopo-install-safe-" }, async (parentDir) => {
