...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "JOOPO_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "joopo", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("joopo channels login --channel whatsapp");
  });
