...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "joopo.internal", expected: "joopo.internal." },
    { value: "joopo.internal.", expected: "joopo.internal." },
    { value: "  joopo.internal  ", expected: "joopo.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { JOOPO_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { JOOPO_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("joopo.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "joopo.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN joopo.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_joopo-gw._tcp IN PTR studio-london._joopo-gw._tcp`,
      `studio-london._joopo-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Joopo)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/joopo`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN joopo.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._joopo-gw._tcp IN TXT`,
        `displayName=Mac Studio (Joopo)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("joopo.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("joopo.internal."),
