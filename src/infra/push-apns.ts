...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Joopo to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "joopo.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.JOOPO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.JOOPO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set JOOPO_APNS_TEAM_ID and JOOPO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.JOOPO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.JOOPO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.JOOPO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set JOOPO_APNS_PRIVATE_KEY_P8 or JOOPO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading JOOPO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
