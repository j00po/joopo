...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): JoopoConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): JoopoConfig {
  return {
