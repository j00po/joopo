...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.JOOPO_CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_AUTH_TOKEN;
    delete process.env.JOOPO_CLAWHUB_CONFIG_PATH;
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    await withTempDir({ prefix: "joopo-clawhub-config-" }, async (configRoot) => {
      const configPath = path.join(configRoot, "clawhub", "config.json");
      process.env.JOOPO_CLAWHUB_CONFIG_PATH = configPath;
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    await withTempDir({ prefix: "joopo-clawdhub-config-" }, async (configRoot) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "joopo-clawhub-home-" }, async (fakeHome) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "joopo-clawhub-home-" }, async (fakeHome) => {
        await withTempDir({ prefix: "joopo-clawhub-xdg-" }, async (xdgRoot) => {
  ...
  it("injects resolved auth token into ClawHub requests", async () => {
    process.env.JOOPO_CLAWHUB_TOKEN = "env-token-123";
  ...
  it("annotates 429 errors with the reset hint and a sign-in hint when unauthenticated", async () => {
    process.env.JOOPO_CLAWHUB_CONFIG_PATH = path.join(os.tmpdir(), "joopo-no-clawhub-config");
    await expect(
      searchClawHubSkills({
        query: "calendar",
        fetchImpl: async () =>
