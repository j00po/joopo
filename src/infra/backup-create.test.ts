...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "joopo-backup-2026-01-01",
    archivePath: "/tmp/joopo-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/joopo-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.joopo",
        "Skipped 1 path:",
        "- workspace: ~/Projects/joopo (covered by ~/.joopo)",
        "Created /tmp/joopo-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.joopo/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.joopo/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.joopo/config.json",
        "- credentials: ~/.joopo/oauth",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/joopo.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.joopo\\");

    expect(filter(String.raw`C:\Users\me\.joopo\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.joopo\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withJoopoTestState(
      {
        layout: "state-only",
        prefix: "joopo-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "joopo.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(
          entries.some((entry) => entry.endsWith("/state/extensions/demo/joopo.plugin.json")),
        ).toBe(true);
