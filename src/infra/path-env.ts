...
function candidateBinDirs(opts: EnsureJoopoPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Joopo process is on.
  try {
  ...
  // Bundled macOS app: `joopo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "joopo");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.JOOPO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "joopo"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `joopo`
...
/**
 * Best-effort PATH bootstrap so skills that require the `joopo` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureJoopoCliOnPath(opts: EnsureJoopoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.JOOPO_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.JOOPO_PATH_BOOTSTRAPPED = "1";
