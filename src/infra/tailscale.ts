...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.JOOPO_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Joopo. You can keep running the web gateway without it: \`${formatCliCommand("joopo gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
