...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__joopo_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "joopo-root");
...
function setPackageRoot(root: string, name = "joopo") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./joopo-root.js").resolveJoopoPackageRootSync,
  asyncResolver: typeof import("./joopo-root.js").resolveJoopoPackageRoot,
  opts: Parameters<typeof import("./joopo-root.js").resolveJoopoPackageRootSync>[0],
...
vi.mock("./joopo-root.fs.runtime.js", () => ({
  joopoRootFsSync: mockFsModule(),
  joopoRootFs: mockFsPromisesModule(),
}));

describe("resolveJoopoPackageRoot", () => {
  let resolveJoopoPackageRoot: typeof import("./joopo-root.js").resolveJoopoPackageRoot;
  let resolveJoopoPackageRootSync: typeof import("./joopo-root.js").resolveJoopoPackageRootSync;
  let clearJoopoPackageRootCaches: typeof import("./joopo-root.js").__testing.clearJoopoPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveJoopoPackageRoot,
      resolveJoopoPackageRootSync,
      __testing: { clearJoopoPackageRootCaches },
    } = await import("./joopo-root.js"));
  });

  beforeEach(() => {
    clearJoopoPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "joopo");
        const pkgRoot = path.join(project, "node_modules", "joopo");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "joopo");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "joopo.mjs")));
    ...
    {
      name: "falls through from a non-joopo moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-joopo");
    ...
    {
      name: "returns null for non-joopo package roots",
      setup: () => {
        const pkgRoot = fx("not-joopo");
        setPackageRoot(pkgRoot, "not-joopo");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "joopo.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "joopo");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
