...
import { resolveJoopoPackageRootSync } from "./joopo-root.js";
...
type ManagedNpmRootJoopoMetadata = {
...
function buildManagedJoopoMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
}): ManagedNpmRootJoopoMetadata | undefined {
  const metadata: ManagedNpmRootJoopoMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
export async function readJoopoManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.joopo)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const joopoMetadata = buildManagedJoopoMetadata({
    current: manifest.joopo,
    managedOverrideKeys,
  });
...
export async function repairManagedNpmRootJoopoPeer(params: {
  npmRoot: string;
  timeoutMs?: number;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "joopo" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasJoopoPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "joopo"));
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall joopo" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall joopo" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootJoopoPeer({ npmRoot: params.npmRoot });
  return true;
}

async function managedNpmRootLockfileHasJoopoPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "joopo" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/joopo" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "joopo" in parsed.dependencies;
  } catch (err) {
...
async function scrubManagedNpmRootJoopoPeer(params: { npmRoot: string }): Promise<void> {
  const manifestPath = path.join(params.npmRoot, "package.json");
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("joopo" in dependencies) {
    const { joopo: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("joopo" in dependencies) {
          delete dependencies.joopo;
          parsed.packages[""] = { ...rootPackage, dependencies };
          lockChanged = true;
        }
      }
      if ("node_modules/joopo" in parsed.packages) {
        delete parsed.packages["node_modules/joopo"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "joopo" in parsed.dependencies) {
  ...
  const joopoPackageDir = path.join(params.npmRoot, "node_modules", "joopo");
  if (await pathExists(joopoPackageDir)) {
    await fs.rm(joopoPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["joopo", "joopo.cmd", "joopo.ps1"].map((binName) =>
