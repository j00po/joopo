  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.joopo.proxy.validation",
        "apns-topic": "ai.joopo.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "Joopo APNs proxy validation" } }));
    });
  } finally {
