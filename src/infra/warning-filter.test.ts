...
const warningFilterKey = Symbol.for("joopo.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "JOOPO_TEST_WARNING" });
      emitWarning(
        Object.assign(new Error("The punycode module is deprecated."), {
          name: "DeprecationWarning",
          code: "DEP0040",
        }),
        { type: "Warning", code: "JOOPO_VISIBLE_OVERRIDE" },
      );
      await flushWarnings();
      expect(seenWarnings).toContainEqual(
        expect.objectContaining({
          code: "JOOPO_TEST_WARNING",
          name: "Warning",
