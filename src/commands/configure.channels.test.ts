...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirm).toHaveBeenCalledWith(
    expect.objectContaining({
      message: `Delete ${label} configuration from ~/.joopo/joopo.json?`,
    }),
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from joopo.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from joopo.json", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram configuration from ~/.joopo/joopo.json?",
      }),
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete done configuration from ~/.joopo/joopo.json?",
      }),
    );
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram\\nBot configuration from ~/.joopo/joopo.json?",
      }),
    );
    expect(note).toHaveBeenCalledWith(
