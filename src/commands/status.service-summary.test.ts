...
describe("readServiceStatusSummary", () => {
  it("marks Joopo-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["joopo", "gateway", "run"] })),
    ...
    expect(summary.managedByJoopo).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByJoopo).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.JOOPO_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.JOOPO_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["joopo", "gateway", "run", "--port", "18789"],
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "joopo-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "joopo-gateway.service");
      ...
      expect(summary.layout).toMatchObject({
        sourcePath: serviceFile,
        sourcePathReal: path.join(realRoot, "joopo-gateway.service"),
        entrypoint,
