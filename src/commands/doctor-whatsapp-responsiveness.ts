...
const LOCAL_TUI_CMD_RE =
  /(?:^|\s)(?:joopo-tui|joopo\s+tui|joopo\s+chat|joopo\s+terminal)(?:\s|$)/;
...
function hasWhatsappEnabled(cfg: JoopoConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("joopo doctor --fix")}.`,
      );
    }
  }
