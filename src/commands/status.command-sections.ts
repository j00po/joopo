...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("joopo security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("joopo security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.joopo.ai/faq",
    "Troubleshooting: https://docs.joopo.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("joopo status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("joopo logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("joopo node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("joopo status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("joopo gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`joopo devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("joopo devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("joopo devices list")}`),
  ];
}
