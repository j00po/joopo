...
export function resetCleanupCommandMocks() {
  ...
  resolveCleanupPlanFromDisk.mockReturnValue({
    stateDir: "/tmp/.joopo",
    configPath: "/tmp/.joopo/joopo.json",
    oauthDir: "/tmp/.joopo/credentials",
    configInsideState: true,
    oauthInsideState: true,
    workspaceDirs: ["/tmp/.joopo/workspace"],
  });
  removePath.mockResolvedValue({ ok: true });
  listAgentSessionDirs.mockResolvedValue(["/tmp/.joopo/agents/main/sessions"]);
