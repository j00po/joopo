...
const secretRefKeyMock = vi.hoisted(() => vi.fn(() => "env:default:JOOPO_GATEWAY_TOKEN"));
...
describe("resolveGatewayInstallToken", () => {
  ...
  it("validates SecretRef token but does not persist resolved plaintext", async () => {
    const tokenRef = { source: "env", provider: "default", id: "JOOPO_GATEWAY_TOKEN" };
    resolveSecretInputRefMock.mockReturnValue({ ref: tokenRef });
    resolveSecretRefValuesMock.mockResolvedValue(
      new Map([["env:default:JOOPO_GATEWAY_TOKEN", "resolved-token"]]),
    );
    ...
    const result = await resolveGatewayInstallToken({
      ...
      env: { JOOPO_GATEWAY_TOKEN: "resolved-token" } as NodeJS.ProcessEnv,
    });
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(result.unavailableReason).toContain("joopo config set gateway.auth.mode token");
    expect(result.unavailableReason).toContain("joopo config set gateway.auth.mode password");
  ...
  it("passes the install env through to gateway auth resolution", async () => {
    const env = {
      JOOPO_GATEWAY_PASSWORD: "dotenv-password", // pragma: allowlist secret
