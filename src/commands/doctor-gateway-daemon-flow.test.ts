...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.joopo.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.joopo.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        JOOPO_STATE_DIR: "/tmp/joopo-service",
        JOOPO_CONFIG_PATH: "/tmp/joopo-service/joopo.json",
      },
    });
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining("joopo gateway install"),
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system Joopo gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "joopo-gateway.service",
        detail: "unit: /etc/systemd/system/joopo-gateway.service",
        scope: "system",
        marker: "joopo",
        legacy: false,
      },
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining("System-level Joopo gateway service detected"),
      "Gateway",
    );
  });
