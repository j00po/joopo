...
async function writeDefaultConfigFile(config: JoopoConfig): Promise<void> {
...
async function readConfigFileRaw(configPath: string): Promise<{
  ...
  try {
    ...
    return { exists: true, parsed: (parsed ?? {}) as JoopoConfig };
  } catch {
...
export async function setupCommand(
  ...
  runtime.log(
    `Next: run ${formatCliCommand("joopo configure")} to choose models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`For full first-run onboarding, run ${formatCliCommand("joopo setup --wizard")}.`);
}
