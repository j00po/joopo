...
import type { JoopoConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: JoopoConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: JoopoConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Joopo default (full)";
  const defaultRequestedAskSource = "Joopo default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("joopo approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: JoopoConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: JoopoConfig): string[] {
...
export async function noteSecurityWarnings(cfg: JoopoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("joopo security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.joopo/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("joopo approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.joopo.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("joopo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("joopo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("joopo doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("joopo config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('joopo config set session.dmScope "per-channel-peer"') +
