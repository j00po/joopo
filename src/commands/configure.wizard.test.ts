...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    promptRemoteGatewayConfig: vi.fn(async (cfg: JoopoConfig) => ({
      ...cfg,
      gateway: { mode: "remote", remote: { url: "wss://gateway.example.test" } },
    })),
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: JoopoConfig }) =>
      Boolean(config.auth?.profiles?.["openai-codex:default"]),
    ),
    setupChannels: vi.fn(async (cfg: JoopoConfig) => cfg),
  };
});
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.joopo/workspace",
  applyWizardMetadata: (cfg: JoopoConfig) => cfg,
...
function setupBaseWizardState(config: JoopoConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: JoopoConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: JoopoConfig) =>
  ...
  it("still supports keyless web search providers through the shared setup flow", async () => {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      createSearchProviderOption({
        ...
        docsUrl: "https://docs.joopo.ai/tools/web",
        credentialPath: "",
      }),
    ]);
    mocks.setupSearch.mockImplementation(async (cfg: JoopoConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(retryCall.nextConfig).toMatchObject({
      agents: {
        defaults: {
          workspace: expect.stringContaining("/.joopo/workspace"),
        },
      },
      plugins: {
        entries: {
