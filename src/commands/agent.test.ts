...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".joopo", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<JoopoConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<JoopoConfig["channels"]>["telegram"]>>,
...
beforeEach(() => {
  ...
  configIoMocks.readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as JoopoConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand", () => {
  it("enables the Codex runtime plugin for one-shot OpenAI model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      expect(pluginRegistryMocks.ensurePluginRegistryLoaded).toHaveBeenCalledWith({
        ...
        workspaceDir: path.join(home, "joopo"),
        onlyPluginIds: ["codex"],
      });
  ...
  it("uses no-tools plain prompt mode for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand(
        {
          message: "Reply with exactly JOOPO-MODEL-OK",
          agentId: "main",
          model: "openrouter/auto",
          modelRun: true,
          promptMode: "none",
    ...
    await withTempHome(async (home) => {
      ...
      expect(callArgs).toEqual(
        expect.objectContaining({
          provider: "openrouter",
          model: "openrouter/auto",
          prompt: "Reply with exactly JOOPO-MODEL-OK",
          modelRun: true,
          promptMode: "none",
          disableTools: true,
        }),
