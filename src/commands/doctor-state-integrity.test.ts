...
type EnvSnapshot = {
  HOME?: string;
  JOOPO_HOME?: string;
  JOOPO_STATE_DIR?: string;
  JOOPO_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    JOOPO_HOME: process.env.JOOPO_HOME,
    JOOPO_STATE_DIR: process.env.JOOPO_STATE_DIR,
    JOOPO_OAUTH_DIR: process.env.JOOPO_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: JoopoConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("JOOPO_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: JoopoConfig) {
...
async function runStateIntegrityText(cfg: JoopoConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.JOOPO_HOME = tempHome;
    process.env.JOOPO_STATE_DIR = path.join(tempHome, ".joopo");
    delete process.env.JOOPO_OAUTH_DIR;
    fs.mkdirSync(process.env.JOOPO_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when JOOPO_OAUTH_DIR is explicitly configured", async () => {
    process.env.JOOPO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: JoopoConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("joopo tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.JOOPO_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.JOOPO_HOME = symlinkHome;
        process.env.JOOPO_STATE_DIR = path.join(symlinkHome, ".joopo");
  ...
  it("prints joopo-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: JoopoConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/joopo sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/joopo sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /joopo sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-tui-pointer-clear-"));
    try {
