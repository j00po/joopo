...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/joopo-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/joopo-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/joopo-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/joopo-dev/joopo.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/joopo-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/joopo-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/joopo-dev/joopo.json");
    expect(payload.auth.storePath).toBe("/tmp/joopo-agent/auth-profiles.json");
  ...
  it("honors JOOPO_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    const previous = process.env.JOOPO_AGENT_DIR;
    process.env.JOOPO_AGENT_DIR = "/tmp/joopo-isolated-agent";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.JOOPO_AGENT_DIR = previous;
      }
    }

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/joopo-isolated-agent");
    const payload = JSON.parse(String((localRuntime.log as Mock).mock.calls[0]?.[0]));
    expect(payload.agentDir).toBe("/tmp/joopo-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/joopo-isolated-agent/auth-profiles.json");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/joopo-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/joopo-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/joopo-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/joopo-agent/workspace"
          ? {
