...
vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: JoopoConfig, agentId: string) => `/tmp/joopo/agents/${agentId}`,
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as JoopoConfig);
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state file: /tmp/joopo/agents/main/auth-state.json",
      "Profiles: (none)",
    ]);
  });
});
