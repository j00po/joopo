...
export async function loadValidConfigOrThrow(): Promise<JoopoConfig> {
...
export async function updateConfig(
  mutator: (cfg: JoopoConfig) => JoopoConfig,
): Promise<JoopoConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: JoopoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: JoopoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("joopo agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
