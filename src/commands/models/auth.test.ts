...
vi.mock("../../plugins/provider-auth-choice-helpers.js", () => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: JoopoConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: JoopoConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: JoopoConfig;
  let lastUpdatedConfig: JoopoConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/joopo/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/joopo/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/joopo/workspace");
    mocks.isRemoteEnvironment.mockReturnValue(false);
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: JoopoConfig) => JoopoConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/joopo/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: JoopoConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/joopo/agents/coder" : "/tmp/joopo/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: JoopoConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/joopo/workspaces/coder" : "/tmp/joopo/workspace",
    );
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/joopo/agents/main", {
      externalCli: {
        mode: "scoped",
    ...
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai-codex:user@example.com",
      agentDir: "/tmp/joopo/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with joopo configure --section web (recommended mode: cached). Docs: https://docs.joopo.ai/tools/web",
    );
  });
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(runProviderAuth).toHaveBeenCalledWith(
      expect.objectContaining({
        agentDir: "/tmp/joopo/agents/coder",
        workspaceDir: "/tmp/joopo/workspaces/coder",
      }),
    );
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith(
      expect.objectContaining({ agentDir: "/tmp/joopo/agents/coder" }),
    );
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(mocks.resolvePluginProviders).toHaveBeenCalledWith(
      expect.objectContaining({
        config: {},
        workspaceDir: "/tmp/joopo/workspace",
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/joopo/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/joopo/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `joopo plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic setup-token auth is supported in Joopo.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Joopo prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic staff told us this Joopo path is allowed again.",
    );
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "joopo agents list" to see configured agents.',
    );
