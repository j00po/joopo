...
export const resolveJoopoPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadJoopoPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/joopo-root.js", () => ({
  ...
  resolveJoopoPackageRootSync: vi.fn(() => "/tmp/joopo"),
}));
...
beforeEach(() => {
  ...
  resolveJoopoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.JOOPO_STATE_DIR;
  originalUpdateInProgress = process.env.JOOPO_UPDATE_IN_PROGRESS;
  process.env.JOOPO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-doctor-state-"));
