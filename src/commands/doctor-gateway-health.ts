...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      ...
      'Fix: run `joopo gateway status --deep`; check `command -v joopo`, `readlink -f "$(command -v joopo)"`, and `joopo --version`; reinstall the gateway service from the intended binary if needed.',
    ].join("\n"),
