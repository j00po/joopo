...
export async function noteLegacyWhatsAppCrontabHealthCheck(
  ...
  note(
    [
      ...
      "`~/.joopo/bin/ensure-whatsapp.sh` is not maintained by current Joopo and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
      `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("joopo channels status --probe")}, ${formatCliCommand("joopo doctor")}, and ${formatCliCommand("joopo gateway status")} for current health checks.`,
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  note(
    [
      ...
      ...previewLines,
      `Repair with ${formatCliCommand("joopo doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
