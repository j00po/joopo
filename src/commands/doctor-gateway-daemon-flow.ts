...
function renderBlockingSystemGatewayServices(services: ExtraGatewayService[]): string {
  return [
    "System-level Joopo gateway service detected while the user gateway service is not installed.",
    ...services.map((svc) => `- ${svc.label} (${svc.detail})`),
    "Joopo will not install a second user-level gateway service automatically.",
    "Run `joopo gateway status --deep` or `joopo doctor --deep` to inspect duplicate services.",
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (!loaded) {
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (!install) {
        note(
          `Run ${formatCliCommand("joopo gateway install")} when you want to install the gateway service.`,
          "Gateway",
        );
      }
      if (install) {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.JOOPO_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("joopo gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
