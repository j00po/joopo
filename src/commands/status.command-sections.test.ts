...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:joopo security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:joopo security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:joopo status --all",
      "  Need to debug live? cmd:joopo logs --follow",
      "  Fix reachability first: cmd:joopo gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:joopo devices approve req-123)",
      "muted(Fallback: cmd:joopo devices approve --latest)",
      "muted(Inspect: cmd:joopo devices list)",
    ]);
  });
