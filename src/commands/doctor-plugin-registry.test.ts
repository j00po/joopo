...
function makeTempDir() {
  return makeTrackedTempDir("joopo-doctor-plugin-registry", tempDirs);
}
...
function createBundledCandidate(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "joopo.plugin.json"),
    JSON.stringify({
      id: params.id,
      name: params.id,
      configSchema: { type: "object" },
...
describe("maybeRepairPluginRegistryState", () => {
  ...
  it("warns about stale managed npm packages that shadow bundled plugins", async () => {
    ...
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("@joopo/google-meet@2026.5.2");
    expect(
      fs.existsSync(path.join(stateDir, "npm", "node_modules", "@joopo", "google-meet")),
    ).toBe(true);
  });
  ...
  it("removes stale managed npm packages from the package lock during repair", async () => {
    ...
    expect(packageLock.packages).not.toHaveProperty("node_modules/@joopo/google-meet");
    expect(packageLock.dependencies).not.toHaveProperty("@joopo/google-meet");
