...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => "/tmp/joopo-migrate-command-test",
}));
  ...
  const items: MigrationPlan["items"] = [
    {
      ...
      target: "/tmp/joopo/workspace/skills/alpha",
      details: {
        skillName: "alpha",
    ...
    },
    {
      ...
      target: "/tmp/joopo/workspace/skills/beta",
      details: {
        skillName: "beta",
...
describe("migrateApplyCommand", () => {
  ...
  beforeEach(async () => {
    await fs.rm("/tmp/joopo-migrate-command-test", { force: true, recursive: true });
    ...
    mocks.backupCreateCommand.mockResolvedValue({ archivePath: "/tmp/joopo-backup.tgz" });
  });
  ...
  it("creates a verified backup before applying a conflict-free migration", async () => {
    ...
    expect(mocks.provider.apply).toHaveBeenCalledWith(
      expect.objectContaining({
        backupPath: "/tmp/joopo-backup.tgz",
        reportDir: expect.stringContaining("/migration/hermes/"),
      }),
      planned,
    );
    expect(result.backupPath).toBe("/tmp/joopo-backup.tgz");
  });
