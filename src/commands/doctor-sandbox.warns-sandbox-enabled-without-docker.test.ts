...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): JoopoConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/joopo/sandbox/containers.json",
        shardedDir: "/tmp/joopo/sandbox/containers",
        exists: true,
        valid: true,
        entries: 2,
      },
    ...
    expect(note).toHaveBeenCalledWith(expect.stringContaining("joopo doctor --fix"), "Sandbox");
  });
