...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("joopo doctor --fix")} or ${formatCliCommand("joopo memory status --fix")}`
    : `Verify: ${formatCliCommand("joopo memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: JoopoConfig): Promise<void> {
  try {
 ...
 * Runs as part of `joopo doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("joopo config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("joopo config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("joopo memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`joopo config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    if (isKeyOptionalMemoryProvider(resolved.provider)) {
      ...
      // not warn. A skipped probe means the user ran `joopo doctor` without
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("joopo configure --section model")}`,
        `- To disable: ${formatCliCommand("joopo config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("joopo memory status --deep")}`,
      ].join("\n"),
      "Memory search",
