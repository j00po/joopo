...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.JOOPO_GATEWAY_TOKEN);
  ...
  // plaintext > ambient JOOPO_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like JOOPO_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }
      ...
      // branch, an ambient JOOPO_GATEWAY_TOKEN (or randomToken() fallback)
