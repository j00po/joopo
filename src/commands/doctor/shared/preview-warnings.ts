...
function hasChannels(cfg: JoopoConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: JoopoConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: JoopoConfig): boolean {
...
function hasExplicitChannelPluginBlockerConfig(cfg: JoopoConfig): boolean {
...
function hasConfiguredSafeBins(cfg: JoopoConfig): boolean {
...
function collectMessageToolUnavailableTargets(cfg: JoopoConfig): string[] {
...
function resolveGroupVisibleReplyProvenance(cfg: JoopoConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: JoopoConfig): string[] {
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    if (groupPolicy.provenance === "default") {
      warnings.push(
        `- messages.groupChat.visibleReplies defaults to "message_tool", but the message tool is unavailable for ${targetSummary}; Joopo falls back to automatic group/channel replies to avoid silent responses. Enable the message tool or set messages.groupChat.visibleReplies explicitly.`,
      );
    } else {
      warnings.push(
        `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${targetSummary}; Joopo falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
      );
    }
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Joopo falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
