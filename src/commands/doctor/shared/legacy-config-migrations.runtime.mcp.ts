...
const MCP_SERVER_TYPE_RULE: LegacyConfigRule = {
  path: ["mcp", "servers"],
  message:
    'mcp.servers entries use Joopo transport names; CLI-native type aliases are legacy here. Run "joopo doctor --fix".',
  match: (value) =>
    isRecord(value) &&
...
export const LEGACY_CONFIG_MIGRATIONS_RUNTIME_MCP: LegacyConfigMigrationSpec[] = [
  defineLegacyConfigMigration({
    ...
    describe: "Move CLI-native MCP server type aliases to Joopo transport",
    ...
    apply: (raw, changes) => {
      ...
      for (const [serverName, rawServer] of Object.entries(servers)) {
        ...
        const alias = resolveJoopoMcpTransportAlias(rawServer.type);
