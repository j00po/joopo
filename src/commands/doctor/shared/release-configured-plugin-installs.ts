...
import { compareJoopoVersions } from "../../../config/version.js";
...
function isPluginsGloballyDisabled(cfg: JoopoConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: JoopoConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: JoopoConfig): string[] {
...
function isPluginEntryDisabled(cfg: JoopoConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: JoopoConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: JoopoConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: JoopoConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: JoopoConfig): string[] {
...
function collectConfiguredProviderIds(cfg: JoopoConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: JoopoConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: JoopoConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: JoopoConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: JoopoConfig): string[] {
...
function addEligiblePluginId(cfg: JoopoConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareJoopoVersions(
  ...
  const touchedComparedToRelease = compareJoopoVersions(params.touchedVersion, releaseVersion);
