...
describe("configured plugin install release step", () => {
  ...
  it("does not stamp config during update-time deferred install repair", async () => {
    mocks.repairMissingPluginInstallsForIds.mockResolvedValue({
      changes: [
        'Skipped package-manager repair for configured plugin "codex" during package update; rerun "joopo doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    ...
    const result = await maybeRunConfiguredPluginInstallReleaseStep({
      ...
      currentVersion: "2026.5.2-beta.1",
      touchedVersion: "2026.5.1",
      env: { JOOPO_UPDATE_IN_PROGRESS: "1" },
    });
