...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<JoopoConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as JoopoConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: JoopoConfig, changes: string[]): JoopoConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as JoopoConfig["talk"]) ?? {};
