...
async function makeTempRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-doctor-codex-assets-"));
  tempRoots.add(root);
  return root;
}
...
function codexConfig(): JoopoConfig {
  return {
    plugins: {
      entries: {
        codex: { enabled: true },
...
describe("collectCodexNativeAssetWarnings", () => {
  it("points users at explicit Codex migration instead of auto-copying native assets", async () => {
    ...
    expect(warnings[0]).toContain("joopo migrate codex --dry-run");
