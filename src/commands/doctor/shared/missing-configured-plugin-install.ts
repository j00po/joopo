...
function addConfiguredAgentRuntimePluginIds(
  ...
  addConfiguredPluginId(ids, env?.JOOPO_AGENT_RUNTIME);
...
function collectConfiguredPluginIds(cfg: JoopoConfig, env?: NodeJS.ProcessEnv): Set<string> {
...
function collectBlockedPluginIds(cfg: JoopoConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: JoopoConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
async function repairMissingPluginInstalls(params: {
  ...
  if (isUpdatePackageDoctorPass(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      changes.push(
        `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "joopo doctor --fix" after the update completes.`,
      );
    }
  }
