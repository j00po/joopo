...
function resolveRuntime(params: {
  ...
  return (
    normalizeString(params.env?.JOOPO_AGENT_RUNTIME) ??
...
function collectConfigModelRefs(cfg: JoopoConfig, env?: NodeJS.ProcessEnv): CodexRouteHit[] {
...
function rewriteConfigModelRefs(params: {
  ...
  env?: NodeJS.ProcessEnv;
  runtime: CodexRepairRuntime;
}): { cfg: JoopoConfig; changes: CodexRouteHit[] } {
...
export function collectCodexRouteWarnings(params: {
  ...
  return [
    [
      ...
      '- Run `joopo doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*` with `agentRuntime.id: "codex"`.',
    ].join("\n"),
  ];
}
...
export function maybeRepairCodexRoutes(params: {
  ...
  codexRuntimeReady?: boolean;
}): { cfg: JoopoConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg, params.env);
  if (hits.length === 0) {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `joopo doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
