...
function hasConfiguredChannels(cfg: JoopoConfig): boolean {
...
export function normalizeLegacyCommandsConfig(
  ...
  return {
    ...cfg,
    commands: commands as JoopoConfig["commands"],
  };
}
...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as JoopoConfig["browser"],
  };
}
...
type ModelProviderEntry = Partial<
  NonNullable<NonNullable<JoopoConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<JoopoConfig["models"]>>;
type ModelDefinitionEntry = NonNullable<ModelProviderEntry["models"]>[number];
type AgentRuntimePolicyPatch = NonNullable<
  NonNullable<NonNullable<JoopoConfig["agents"]>["defaults"]>["agentRuntime"]
>;
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  return changed
    ? {
        ...cfg,
        agents: nextAgents as JoopoConfig["agents"],
      }
    : cfg;
}
...
export function normalizeLegacyOpenAICodexModelsAddMetadata(
  ...
  return {
    ...
    models: {
      ...rawModels,
      providers: nextProviders as NonNullable<JoopoConfig["models"]>["providers"],
    },
  };
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<JoopoConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as JoopoConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<JoopoConfig["tools"]>["media"],
    },
  };
}
