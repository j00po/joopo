...
function isCodexRuntimeConfigured(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
  if (normalizeString(env.JOOPO_AGENT_RUNTIME) === "codex") {
    return true;
  }
...
function isCodexPluginConfigured(cfg: JoopoConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetWarnings(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode Joopo agents use isolated per-agent Codex homes.",
      ...
      "- Run `joopo migrate codex --dry-run` to inventory them. Applying that migration copies skills into the current Joopo agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
