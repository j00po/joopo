...
export async function channelsListCommand(
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `joopo channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `joopo status` or `joopo models list`.",
    ),
  );
