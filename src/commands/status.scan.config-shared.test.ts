...
describe("status.scan.config-shared", () => {
  beforeEach(() => {
    ...
    mocks.resolveConfigPath.mockReturnValue(
      `/tmp/joopo-status-scan-config-shared-missing-${process.pid}.json`,
    );
  });
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readBestEffortConfig,
      resolveConfig,
      env: { VITEST: "true", JOOPO_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      expect.stringContaining("JOOPO_GATEWAY_TOKEN overrides gateway.auth.token"),
    ]);
  });

  it("does not add a status diagnostic when config uses JOOPO_GATEWAY_TOKEN", async () => {
    const sourceConfig = {
