...
import {
  JOOPO_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveJoopoWrapperPath,
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperPath = await resolveJoopoWrapperPath(
    params.wrapperPath ?? params.env[JOOPO_WRAPPER_ENV_KEY],
  );
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [JOOPO_WRAPPER_ENV_KEY]: wrapperPath }
    : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.JOOPO_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("joopo gateway install")}\` after fixing the error.`;
}
