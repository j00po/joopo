...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.JOOPO_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: JoopoConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: JoopoConfig) => {
...
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.JOOPO_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<JoopoConfig> {
