...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: JoopoConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => JoopoConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: JoopoConfig, sourceConfig: JoopoConfig) => void>(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "joopo-agent-" });
}

function mockConfig(home: string, storePath: string): JoopoConfig {
  const cfg = {
    agents: {
      defaults: {
