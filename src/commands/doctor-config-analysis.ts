...
import { JoopoSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: JoopoConfig): {
  config: JoopoConfig;
  removed: string[];
} {
  const parsed = JoopoSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: JoopoConfig): void {
