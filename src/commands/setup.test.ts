...
function createSetupDeps(home: string) {
  ...
  return {
    ...
    ensureAgentWorkspace: vi.fn(async (params?: { dir?: string }) => ({
      dir: params?.dir ?? path.join(home, ".joopo", "workspace"),
    })),
    ...
    mkdir: vi.fn(async () => {}),
    resolveSessionTranscriptsDir: vi.fn(() => path.join(home, ".joopo", "sessions")),
...
describe("setupCommand", () => {
  it("writes gateway.mode=local on first run", async () => {
    await withTempHome(async (home) => {
      ...
      const workspace = path.join(home, ".joopo", "workspace");
  ...
  it("explains that plain setup only initializes local files", async () => {
    await withTempHome(async (home) => {
      ...
      expect(logs).toContain("joopo configure");
      expect(logs).toContain("joopo setup --wizard");
    });
  });
