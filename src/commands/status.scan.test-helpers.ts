...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/joopo-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = JoopoConfig>(
  overrides: T = {} as T,
): JoopoConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): JoopoConfig {
