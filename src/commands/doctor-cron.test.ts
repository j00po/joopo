...
async function makeTempStorePath() {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-doctor-cron-"));
...
function createCronConfig(storePath: string): JoopoConfig {
  return {
    cron: {
      store: storePath,
...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("rewrites stale managed dreaming jobs to the isolated agentTurn shape", async () => {
    ...
    expect(job).toMatchObject({
      ...
      payload: {
        kind: "agentTurn",
        message: "__joopo_memory_core_short_term_promotion_dream__",
        lightContext: true,
      },
      delivery: { mode: "none" },
    });
    ...
    await noteLegacyWhatsAppCrontabHealthCheck({
      ...
      readCrontab: async () => ({
        stdout: [
          ...
          "*/5 * * * * ~/.joopo/bin/ensure-whatsapp.sh >> ~/.joopo/logs/whatsapp-health.log 2>&1",
          "0 9 * * * /usr/bin/true",
          "",
        ].join("\n"),
      }),
