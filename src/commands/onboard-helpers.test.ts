...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".joopo-work");
    const defaultStateDir = path.join(homeDir, ".joopo");
    const profileConfigPath = path.join(profileStateDir, "joopo.json");
    ...
    vi.stubEnv("JOOPO_HOME", homeDir);
    vi.stubEnv("JOOPO_PROFILE", "work");
    vi.stubEnv("JOOPO_STATE_DIR", profileStateDir);
    vi.stubEnv("JOOPO_CONFIG_PATH", profileConfigPath);
