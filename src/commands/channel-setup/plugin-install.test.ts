...
import { bundledPluginRoot, bundledPluginRootAt } from "joopo/plugin-sdk/test-fixtures";
...
const discoverJoopoPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverJoopoPlugins: (args: unknown) => discoverJoopoPlugins(args),
}));

import fs from "node:fs";
import type { ChannelPluginCatalogEntry } from "../../channels/plugins/catalog.js";
import type { JoopoConfig } from "../../config/config.js";
import { loadJoopoPlugins } from "../../plugins/loader.js";
...
const bundledChatNpmSpec = "@joopo/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/joopo", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/joopo-test/${id}`,
    source: `/tmp/joopo-test/${id}/index.ts`,
    manifestPath: `/tmp/joopo-test/${id}/joopo.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadJoopoPlugins).toHaveBeenCalledWith(
    expect.not.objectContaining({
      onlyPluginIds: [params.pluginId],
    }),
  );
  const firstLoadCall = vi.mocked(loadJoopoPlugins).mock.calls[0]?.[0] as
...
beforeEach(() => {
  ...
  discoverJoopoPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: JoopoConfig = { update: { channel } };
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: JoopoConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/joopo-ledger-channel";
    process.env.JOOPO_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ...
      npmResolution: {
        name: "@joopo/wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@joopo/wecom@2026.5.4-beta.1",
      },
    });
    ...
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "@joopo/wecom@beta",
        expectedPluginId: "wecom-joopo-plugin",
      }),
    );
    expect(result.cfg.plugins?.installs?.["wecom-joopo-plugin"]?.spec).toBe("@joopo/wecom");
  });
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: bundledPluginRootAt("/opt/joopo", "bundled-chat"),
          }),
        ]),
      }),
    );
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-joopo-plugin");
  });
