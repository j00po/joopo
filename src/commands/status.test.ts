...
beforeAll(() => {
  envSnapshot = captureEnv(["JOOPO_PROFILE"]);
  process.env.JOOPO_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByJoopo: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/joopo/pnpm-lock.yaml",
        markerPath: "/tmp/joopo/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.JOOPO_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.joopo.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.joopo.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.JOOPO_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/joopo-root.js", () => ({
  resolveJoopoPackageRoot: vi.fn().mockResolvedValue("/tmp/joopo"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("joopo status --all") ||
          line.includes("joopo --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("joopo --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("JOOPO_GATEWAY_TOKEN", "abcd1234", async () => {
