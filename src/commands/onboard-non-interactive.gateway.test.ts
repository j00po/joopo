...
const testConfigStore = new Map<string, JoopoConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.JOOPO_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.JOOPO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("JOOPO_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "joopo.json");
}
...
function readTestConfig<T = JoopoConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  replaceConfigFile: async ({ nextConfig }: { nextConfig: JoopoConfig }) => {
    testConfigStore.set(resolveTestConfigPath(), nextConfig);
  },
  resolveGatewayPort: (cfg: JoopoConfig) => cfg.gateway?.port ?? 18789,
}));
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/joopo-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "joopo"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("joopo-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "joopo");
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `joopo gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `joopo gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.JOOPO_STATE_DIR = stateDir;
      process.env.JOOPO_CONFIG_PATH = path.join(stateDir, "joopo.json");
