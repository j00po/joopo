...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.JOOPO_GATEWAY_TOKEN;
    prevPassword = process.env.JOOPO_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.JOOPO_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.JOOPO_GATEWAY_PASSWORD;
    } else {
      process.env.JOOPO_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".joopo"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".joopo", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as JoopoConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.JOOPO_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as JoopoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when JOOPO_GATEWAY_TOKEN env overrides gateway.auth.token config (#74271)", async () => {
    process.env.JOOPO_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("JOOPO_GATEWAY_TOKEN overrides");
    expect(message).toContain("env-first precedence");
  });

  it("does not warn when only env token is set without config token", async () => {
    process.env.JOOPO_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as JoopoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("JOOPO_GATEWAY_TOKEN overrides");
  });

  it("does not warn when config token uses JOOPO_GATEWAY_TOKEN SecretRef", async () => {
    process.env.JOOPO_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as JoopoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as JoopoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as JoopoConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("joopo approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as JoopoConfig);
    ...
    expect(message).toContain("Run: joopo security audit --deep");
  });
