...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("joopo security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("joopo status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("joopo tasks maintenance --apply")}`,
