...
function collectConfiguredChannelIds(cfg: JoopoConfig): string[] {
  const channels =
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: JoopoConfig = baseCfg;
  let candidate = structuredClone(baseCfg);
  let pendingChanges = false;
  let fixHints: string[] = [];
  const doctorFixCommand = formatCliCommand("joopo doctor --fix");
  ...
  if (hasLegacyInternalHookHandlers(snapshot.parsed)) {
    note(
      [
        ...
        "- joopo doctor --fix does not rewrite this shape automatically.",
      ].join("\n"),
      "Legacy config keys detected",
    );
  }
