...
const SEARCH_TOOL = "https://docs.joopo.ai/mcp.SearchJoopo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.joopo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('joopo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.joopo.ai/");
      runtime.log(`Search: ${formatCliCommand('joopo docs "your query"')}`);
    }
    return;
  }
