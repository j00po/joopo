...
import { replaceConfigFile, type JoopoConfig } from "../config/config.js";
...
import { JOOPO_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function buildGatewayServiceRepairEnv(
  ...
  const wrapperPath = command?.environment?.[JOOPO_WRAPPER_ENV_KEY]?.trim();
  if (!wrapperPath || Object.hasOwn(process.env, JOOPO_WRAPPER_ENV_KEY)) {
    return process.env;
  }
  return {
    ...process.env,
    [JOOPO_WRAPPER_ENV_KEY]: wrapperPath,
  };
}
...
function resolveGatewayServiceWrapperPath(
  ...
  return normalizeOptionalString(command?.environment?.[JOOPO_WRAPPER_ENV_KEY]) ?? null;
}
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "joopo-gateway.service";
}
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${JOOPO_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  if (serviceLayout?.entrypointSourceCheckout) {
    note(
      [
        ...
        "Run `joopo doctor --fix` from the intended package install, or reinstall the gateway service with `joopo gateway install --force`.",
      ].join("\n"),
      "Gateway service config",
    );
  }
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service JOOPO_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (serviceRewriteBlocked) {
    note(
      "Gateway service is running; leaving supervisor metadata unchanged. Stop the service first or use `joopo gateway install --force` when you want to replace the active launcher.",
      "Gateway service config",
    );
    return;
  }
  ...
  if (!repair) {
    note(
      "Run `joopo gateway install --force` when you want to replace the gateway service definition.",
      "Gateway service config",
    );
    return;
  }
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Joopo gateway next.");
      }
    }
  }
