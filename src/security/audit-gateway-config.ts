...
type CollectDangerousConfigFlags = (cfg: JoopoConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.JOOPO_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.JOOPO_GATEWAY_PASSWORD);
