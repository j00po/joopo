...
function summarizeGroupPolicy(cfg: JoopoConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: JoopoConfig): boolean {
...
function isBrowserEnabled(cfg: JoopoConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: JoopoConfig): SecurityAuditFinding[] {
