...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: JoopoConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when JOOPO_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: JoopoConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from JOOPO_GATEWAY_TOKEN", () => {
    const cfg: JoopoConfig = {
