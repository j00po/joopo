...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: JoopoConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: JoopoConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: JoopoConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): JoopoConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: JoopoConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
