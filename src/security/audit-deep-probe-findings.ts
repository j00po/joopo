...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("joopo status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("joopo security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set JOOPO_GATEWAY_TOKEN/JOOPO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("joopo security audit --deep")}".`,
    });
  }
  return findings;
}
