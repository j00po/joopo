...
```bash
export JOOPO_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.joopo/joopo.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env JOOPO_PARALLELS_DISCORD_TOKEN \
...
- Use the `joopo` wrapper for guest `message send/read`; `node joopo.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/joopo-parallels-smoke.*`
...
- host inbound nonce appears in `joopo message read` output
