---
name: joopo-ghsa-maintainer
description: Inspect, patch, validate, publish, or confirm Joopo GHSA security advisories and private-fork state.
---

# Joopo GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `joopo-release-maintainer`.
...
```bash
fork=$(gh api /repos/joopo/joopo/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/joopo/joopo/security-advisories/<GHSA> \
