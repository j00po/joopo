---
name: crabbox
description: Use Crabbox for Joopo remote Linux validation. Default to Blacksmith Testbox; includes direct Blacksmith and owned AWS/Hetzner fallback notes when Crabbox fails.
---

# Crabbox

Use Crabbox when Joopo needs remote Linux proof for broad tests, CI-parity
...
- Joopo scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- Do not treat inherited shell env as operator intent. In particular,
  `JOOPO_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `JOOPO_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. Joopo broad
...
pnpm crabbox:run -- --provider blacksmith-testbox \
  ...
  --timing-json \
  --shell -- \
  "env CI=1 NODE_OPTIONS=--max-old-space-size=4096 JOOPO_TEST_PROJECTS_PARALLEL=6 JOOPO_VITEST_MAX_WORKERS=1 JOOPO_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
```
...
- Testbox queued/capacity pressure: do not convert a broad changed gate or full
  suite into local `JOOPO_LOCAL_CHECK_MODE=throttled pnpm ...`. Leave the
...
blacksmith testbox run --id <tbx_id> "env CI=1 NODE_OPTIONS=--max-old-space-size=4096 JOOPO_TEST_PROJECTS_PARALLEL=6 JOOPO_VITEST_MAX_WORKERS=1 JOOPO_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
blacksmith testbox stop --id <tbx_id>
```
...
```sh
blacksmith testbox run --id <tbx_id> "env CI=1 NODE_OPTIONS=--max-old-space-size=4096 JOOPO_TEST_PROJECTS_PARALLEL=6 JOOPO_VITEST_MAX_WORKERS=1 JOOPO_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test"
```

Auth fallback, only when `blacksmith` says auth is missing:

```sh
blacksmith auth login --non-interactive --organization joopo
```
...
```sh
blacksmith testbox warmup ci-check-testbox.yml --ref main --idle-timeout 90
blacksmith testbox run --id <tbx_id> "env CI=1 NODE_OPTIONS=--max-old-space-size=4096 JOOPO_TEST_PROJECTS_PARALLEL=6 JOOPO_VITEST_MAX_WORKERS=1 pnpm test:changed"
blacksmith testbox stop --id <tbx_id>
```
...
pnpm crabbox:run -- --id <cbx_id-or-slug> --timing-json --shell -- "env NODE_OPTIONS=--max-old-space-size=4096 JOOPO_TEST_PROJECTS_PARALLEL=6 JOOPO_VITEST_MAX_WORKERS=1 JOOPO_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
...
```sh
brew install joopo/tap/crabbox
crabbox login --url https://crabbox.joopo.ai --provider aws
```
...
- If broker auth is missing, run `crabbox login --url https://crabbox.joopo.ai --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal Joopo validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered Joopo proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.joopo.ai --provider aws --token-stdin`.
...
for owned-cloud lanes. Do not let that config override the Joopo default
...
Do not add Joopo-specific setup to Crabbox itself. Put repo setup in the
