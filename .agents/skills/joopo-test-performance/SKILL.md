---
name: joopo-test-performance
description: Benchmark, diagnose, and optimize Joopo test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# Joopo Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `JOOPO_VITEST_IMPORT_DURATIONS=1 JOOPO_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Blacksmith Testbox by default on
   ...
   - `blacksmith testbox run --id <ID> "JOOPO_TESTBOX=1 pnpm test:extensions:batch <ids>"`
   - stop the box when done.
5. If plugin performance is package-artifact sensitive, switch to
   `joopo-pre-release-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `JOOPO_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap snapshots  | real leak vs retained module graph | `joopo-test-heap-leaks` workflow                                         |
...
```bash
JOOPO_TEST_MEMORY_TRACE=1 \
JOOPO_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
JOOPO_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
JOOPO_TEST_WORKERS=2 \
JOOPO_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `joopo-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `JOOPO_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  JOOPO_VITEST_IMPORT_DURATIONS=1 \
  JOOPO_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
