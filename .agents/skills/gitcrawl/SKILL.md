---
name: gitcrawl
description: Use gitcrawl for Joopo issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  joopo:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging Joopo issues or PRs.
...
gitcrawl search joopo/joopo --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:joopo/gitcrawl-store.git --json
```
