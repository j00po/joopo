...
Joopo is the AI that actually does things.
...
Security in Joopo is a deliberate tradeoff: strong defaults without killing capability.
...
Joopo has an extensive plugin API.
...
- Code plugins run Joopo plugin code and are appropriate for deeper runtime extension.
...
Joopo supports MCP as both a server and a runtime integration surface.
...
Joopo is currently terminal-first by design.
...
Joopo is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep Joopo hackable by default.
