...
import "../app.ts";
import type { JoopoApp } from "../app.ts";
...
const mountedApps = new Set<JoopoApp>();

function collectMountedApps() {
  return new Set<JoopoApp>([
    ...mountedApps,
    ...document.querySelectorAll<JoopoApp>("joopo-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<JoopoApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<JoopoApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("joopo-app") as JoopoApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window.__JOOPO_CONTROL_UI_BASE_PATH__ = undefined;
