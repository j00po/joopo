...
const SKIP_DELETE_CONFIRM_KEY = "joopo:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__joopo__|media|api\/chat\/media\/outgoing)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__joopo__/assistant-media?${params.toString()}`;
}
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (requesterSessionKey) {
        headers.set("x-joopo-requester-session-key", requesterSessionKey);
      }
