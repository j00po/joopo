...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://joopo.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://joopo.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://joopo.ai/abc-123", baseHref)).toBe(
      "blob:https://joopo.ai/abc-123",
    );
  });
