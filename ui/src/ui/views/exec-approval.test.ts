...
import { type JoopoModalDialog } from "../components/modal-dialog.ts";
...
async function getRenderedDialog() {
  const modal = container.querySelector<JoopoModalDialog>("joopo-modal-dialog");
...
describe("approval and confirmation modals", () => {
  ...
  it("renders exec approval as a labelled modal", async () => {
    ...
    expect(dialog.getAttribute("aria-labelledby")).toBe("joopo-modal-dialog-label");
    expect(dialog.getAttribute("aria-describedby")).toBe("joopo-modal-dialog-description");
    expect(modal.shadowRoot?.querySelector("#joopo-modal-dialog-label")?.textContent).toBe(
      "Exec approval needed",
    );
    expect(
      modal.shadowRoot?.querySelector("#joopo-modal-dialog-description")?.textContent,
  ...
  it("uses the shared modal primitive for gateway URL confirmation and cancels on Escape", async () => {
    ...
    render(
      renderGatewayUrlConfirmation({
        pendingGatewayUrl: "wss://gateway.example/joopo",
    ...
    const { dialog } = await getRenderedDialog();
    expect(container.querySelector("joopo-modal-dialog")).not.toBeNull();
