...
describe("JoopoApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    const { JoopoApp } = await import("./app.ts");
    const app = Object.create(JoopoApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await JoopoApp.prototype.toggleRealtimeTalk.call(app as never);
