  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __JOOPO_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__JOOPO_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__JOOPO_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", () => {
    ...
    setControlUiBasePath(" /joopo/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/joopo"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", () => {
    setTestLocation({
      ...
      pathname: "/apps/joopo/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/joopo"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", () => {
    ...
    sessionStorage.setItem("joopo.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "joopo.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/joopo",
    ...
    const scopedKey = "joopo.control.settings.v1:wss://gateway.example:8443/joopo";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      borderRadius: 50,
      sessionsByGateway: {
        "wss://gateway.example:8443/joopo": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", () => {
    ...
    const scopedKey = `joopo.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("falls back to claw when persisted custom theme data is invalid", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `joopo.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        theme: "custom",
        themeMode: "dark",
  ...
  it("caps persisted session scopes to the most recent gateways", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `joopo.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("persists local user identity separately from gateway settings", () => {
    ...
    expect(JSON.parse(localStorage.getItem("joopo.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "🦞",
    });
  });
  ...
  it("removes the persisted local user identity when cleared", () => {
    ...
    expect(localStorage.getItem("joopo.control.user.v1")).toBeNull();
  });
});
