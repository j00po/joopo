...
declare global {
  interface Window {
    __JOOPO_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class JoopoApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(this.client, this.sessionKey, {
      ...
      onTranscript: (entry) => {
        this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "Joopo"}: ${entry.text}`;
      },
    });
...
if (!customElements.get("joopo-app")) {
  customElements.define("joopo-app", JoopoApp);
}
