...
DOCKER_BUILD_EXTENSIONS="${JOOPO_DOCKER_BUILD_EXTENSIONS:-${JOOPO_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "JOOPO_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${JOOPO_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because JOOPO_DOCKER_BUILD_ON_MISSING/JOOPO_TESTBOX allows fallback."
fi
