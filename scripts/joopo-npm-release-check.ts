...
const EXPECTED_REPOSITORY_URL = "https://github.com/joopo/joopo";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@joopo/fs-safe";
...
const skipPackValidationEnv = "JOOPO_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "joopo") {
    errors.push(`package.json name must be "joopo"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.joopo !== "joopo.mjs") {
    errors.push(
      `package.json bin.joopo must be "joopo.mjs"; found "${pkg.bin?.joopo ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`joopo-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `joopo-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
