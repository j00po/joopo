...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.JOOPO_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@joopo/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "joopo.plugin.json");
    ...
    if (packageJson.joopo && "extensions" in packageJson.joopo) {
      packageJson.joopo = {
        ...packageJson.joopo,
        extensions: rewritePackageExtensions(packageJson.joopo.extensions),
        ...(typeof packageJson.joopo.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.joopo.setupEntry) }
          : {}),
      };
    }
