...
joopo_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${JOOPO_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "JOOPO_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
joopo_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.joopo"
  ...
  export JOOPO_STATE_DIR="$dest_dir"
  export JOOPO_CONFIG_PATH="$dest_dir/joopo.json"
}

joopo_live_prepare_staged_config() {
  if [ ! -f "${JOOPO_CONFIG_PATH:-}" ]; then
    return 0
  fi
