...
function liveDockerScriptCommand(script, envPrefix = "", options = {}) {
  ...
  const skipBuild = options.skipBuild === false ? "" : "JOOPO_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c 'harness="\${JOOPO_DOCKER_E2E_TRUSTED_HARNESS_DIR:-}"; if [ -z "$harness" ]; then if [ -d .release-harness/scripts ]; then harness=.release-harness; else harness=.; fi; fi; JOOPO_LIVE_DOCKER_REPO_ROOT="\${JOOPO_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `JOOPO_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} JOOPO_BUNDLED_PLUGIN_SWEEP_INDEX=${index} JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
export const mainLanes = [
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "JOOPO_IMAGE=joopo:local-live-gateway JOOPO_DOCKER_BUILD_EXTENSIONS=matrix JOOPO_LIVE_GATEWAY_PROVIDERS=claude-cli,codex-cli,google-gemini-cli",
      { skipBuild: false },
    ),
    {
      providers: ["claude-cli", "codex-cli", "google-gemini-cli"],
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "JOOPO_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "JOOPO_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
  ...
  serviceLane("onboard", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane(
    "npm-onboard-channel-agent",
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-discord-channel-agent",
    "JOOPO_NPM_ONBOARD_CHANNEL=discord JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-slack-channel-agent",
    "JOOPO_NPM_ONBOARD_CHANNEL=slack JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  serviceLane("gateway-network", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  serviceLane(
    "agents-delete-shared-workspace",
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  ...
  lane("crestodian-rescue", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-rescue", {
    stateScenario: "empty",
  }),
  ...
  npmLane(
    ...
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
    {
  ...
  npmLane(
    ...
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
    {
      stateScenario: "upgrade-survivor",
  ...
  npmLane("update-migration", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration", {
    stateScenario: "upgrade-survivor",
    timeoutMs: 30 * 60 * 1000,
    weight: 3,
  }),
  lane("plugins", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  npmLane("plugin-update", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
...
export const tailLanes = [
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "JOOPO_LIVE_CODEX_BIND=1 JOOPO_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts",
    ),
    {
      cacheKey: "codex-harness",
      provider: "codex-cli",
  ...
  liveLane(
    ...
    "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-codex-npm-plugin",
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "JOOPO_LIVE_CLI_BACKEND_AUTH=api-key JOOPO_LIVE_CLI_BACKEND_MODEL=codex-cli/gpt-5.4",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "JOOPO_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "JOOPO_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "JOOPO_LIVE_ACP_BIND_AGENT=droid JOOPO_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "JOOPO_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "JOOPO_LIVE_ACP_BIND_AGENT=opencode JOOPO_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const releasePathPackageInstallOpenAiLanes = [
  npmLane(
    "install-e2e-openai",
    "JOOPO_INSTALL_TAG=beta JOOPO_E2E_MODELS=openai JOOPO_INSTALL_E2E_IMAGE=joopo-install-e2e-openai:local pnpm test:install:e2e",
    {
      resources: ["service"],
      weight: 3,
    },
...
const releasePathPackageInstallAnthropicLanes = [
  npmLane(
    ...
    "JOOPO_INSTALL_TAG=beta JOOPO_E2E_MODELS=anthropic JOOPO_INSTALL_E2E_IMAGE=joopo-install-e2e-anthropic:local pnpm test:install:e2e",
    {
      resources: ["service"],
      weight: 3,
    },
...
const primaryReleasePathChunks = {
  core: [
    lane("qr", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr"),
    serviceLane("onboard", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
      stateScenario: "empty",
      weight: 2,
    }),
...
function openWebUILane() {
  return liveLane("openwebui", "JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    needsLiveImage: false,
    provider: "openai",
    resources: ["service"],
...
export function releasePathChunkLanes(chunk, options = {}) {
  ...
  if (!base) {
    throw new Error(
      `JOOPO_DOCKER_ALL_CHUNK must be one of: ${[
