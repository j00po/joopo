...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.joopo?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.joopo?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
export async function collectClawHubJoopoOwnerErrors(params: {
  ...
  await Promise.all(
    params.plugins.map(async (plugin) => {
      ...
      if (response.status === 404) {
        errors.push(
          `${plugin.packageName}: ClawHub package row must already exist under @${requiredOwnerHandle} before Joopo release publish.`,
        );
        return;
      }
      if (!response.ok) {
