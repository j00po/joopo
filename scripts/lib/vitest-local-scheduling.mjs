...
function isSystemThrottleDisabled(env) {
  const normalized = env.JOOPO_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.JOOPO_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.JOOPO_VITEST_MAX_WORKERS ?? env.JOOPO_TEST_WORKERS);
  if (override !== null) {
