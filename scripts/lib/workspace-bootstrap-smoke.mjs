...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "joopo-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "joopo.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".joopo", "workspace");
