...
const JOOPO_PACKAGE_SPEC_RE =
  /^joopo@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;
...
export function validateJoopoPackageSpec(spec: string) {
  if (!JOOPO_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `Package spec must be joopo@main, joopo@alpha, joopo@beta, joopo@latest, or an exact Joopo release version; got: ${spec}`,
    );
  }
  return spec;
}
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    JOOPO_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...(params.packageTgz ? { JOOPO_NPM_TELEGRAM_PACKAGE_TGZ: params.packageTgz } : {}),
    JOOPO_NPM_TELEGRAM_PACKAGE_LABEL: `${params.spec} (${params.version})`,
    JOOPO_NPM_TELEGRAM_PROVIDER_MODE: params.providerMode,
    JOOPO_NPM_TELEGRAM_SCENARIOS: params.scenarios.join(","),
    JOOPO_NPM_TELEGRAM_OUTPUT_DIR: params.rawOutputDir,
    JOOPO_NPM_TELEGRAM_FAST: params.baseEnv.JOOPO_NPM_TELEGRAM_FAST ?? "1",
    JOOPO_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    JOOPO_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    JOOPO_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    JOOPO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
...
export async function assertHarnessRoot(harnessRoot: string) {
  ...
  try {
    ...
    throw new Error(`Missing Joopo Telegram npm harness: ${scriptPath}`);
  }
}
...
export async function resolveMainVersion(harnessRoot: string) {
  ...
  if (typeof packageJson.version !== "string" || packageJson.version.trim().length === 0) {
    throw new Error("Joopo package.json must contain a non-empty version.");
  }
