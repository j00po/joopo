...
export function isPublishablePluginPackage(packageJson) {
  return packageJson.joopo?.release?.publishToNpm === true;
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "joopo" || id.startsWith("joopo/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "joopo.plugin.json")) {
    merged.add("joopo.plugin.json");
  }
...
function normalizeJoopoPeerRange(value) {
...
function resolveJoopoPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeJoopoPeerRange(packageJson.joopo?.compat?.pluginApi) ||
    normalizeJoopoPeerRange(packageJson.peerDependencies?.joopo) ||
    normalizeJoopoPeerRange(packageJson.joopo?.build?.joopoVersion) ||
    normalizeJoopoPeerRange(rootPackageJson?.version) ||
    normalizeJoopoPeerRange(packageJson.version)
  );
}

export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const joopoPeerRange = resolveJoopoPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!joopoPeerRange) {
    throw new Error(
      `cannot infer joopo peerDependency range for ${plan.pluginDir}; set joopo.compat.pluginApi or package version`,
    );
  }
  ...
  const existingJoopoMeta = getRecord(existingPeerDependenciesMeta.joopo);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      joopo: joopoPeerRange,
    },
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.joopo?.extensions)
      ? packageJson.joopo.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.joopo?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.joopo.setupEntry)
      : undefined,
  };
  return {
    ...plan,
