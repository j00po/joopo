...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.joopo?.extensions)
    ? packageJson.joopo.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.joopo?.setupEntry === "string" &&
    packageJson.joopo.setupEntry.trim().length > 0
      ? packageJson.joopo.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "joopo.plugin.json"));
    }
    if (hasPackageJson) {
