...
import { parseReleaseVersion } from "../joopo-npm-release-check.ts";
...
export const JOOPO_PLUGIN_NPM_REPOSITORY_URL = "https://github.com/joopo/joopo";
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.joopo?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.joopo?.extensions ?? [];

  if (!packageName.startsWith("@joopo/")) {
    errors.push(
      `package name must start with "@joopo/"; found "${packageName || "<missing>"}".`,
    );
  }
  if (packageJson.private === true) {
    errors.push("package.json private must not be true.");
  }
  if (repositoryUrl !== JOOPO_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${JOOPO_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("joopo.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("joopo.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("joopo.install.npmSpec must be a non-empty string for publishable plugins.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.joopo?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.joopo?.install?.npmSpec),
    });
  }
...
function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "joopo-plugin-npm-view-"));
