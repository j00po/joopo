...
function usage() {
  return `Usage:
  node scripts/lib/joopo-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/joopo-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/joopo-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "joopo-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "joopo-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'JOOPO_TEST_STATE_JSON'`,
    json,
    "JOOPO_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const configPath = path.join(stateDir, "joopo.json");
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `joopo-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `joopo-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'JOOPO_TEST_STATE_TMP_ROOT="${JOOPO_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    "export JOOPO_TEST_STATE_TMP_ROOT",
    'mkdir -p "$JOOPO_TEST_STATE_TMP_ROOT"',
    `JOOPO_TEST_STATE_HOME="$(mktemp -d "$JOOPO_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$JOOPO_TEST_STATE_HOME"',
    'export USERPROFILE="$JOOPO_TEST_STATE_HOME"',
    'export JOOPO_HOME="$JOOPO_TEST_STATE_HOME"',
    'export JOOPO_STATE_DIR="$JOOPO_TEST_STATE_HOME/.joopo"',
    'export JOOPO_CONFIG_PATH="$JOOPO_STATE_DIR/joopo.json"',
    'export JOOPO_TEST_WORKSPACE_DIR="$JOOPO_TEST_STATE_HOME/workspace"',
    'mkdir -p "$JOOPO_STATE_DIR" "$JOOPO_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$JOOPO_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `joopo_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown Joopo test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      JOOPO_TEST_STATE_HOME="$raw_label"
      mkdir -p "$JOOPO_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${JOOPO_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      mkdir -p "$tmp_root"
      JOOPO_TEST_STATE_HOME="$(mktemp -d "$tmp_root/joopo-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$JOOPO_TEST_STATE_HOME"
  export USERPROFILE="$JOOPO_TEST_STATE_HOME"
  ...
  export JOOPO_CONFIG_PATH="$JOOPO_STATE_DIR/joopo.json"
  export JOOPO_TEST_WORKSPACE_DIR="$JOOPO_TEST_STATE_HOME/workspace"
  unset JOOPO_AGENT_DIR
  unset PI_CODING_AGENT_DIR
  unset JOOPO_SERVICE_REPAIR_POLICY
  mkdir -p "$JOOPO_STATE_DIR" "$JOOPO_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$JOOPO_CONFIG_PATH" <<'JOOPO_TEST_STATE_JSON'
{}
JOOPO_TEST_STATE_JSON
      ;;
    update-stable)
...
JOOPO_TEST_STATE_JSON
      ;;
    external-service)
      export JOOPO_SERVICE_REPAIR_POLICY="external"
      cat > "$JOOPO_CONFIG_PATH" <<'JOOPO_TEST_STATE_JSON'
{}
JOOPO_TEST_STATE_JSON
      ;;
