...
import { collectClawHubJoopoOwnerErrors } from "./lib/plugin-clawhub-release.ts";
...
export async function runClawHubOwnerPreflight(argv: string[]) {
  ...
  const errors = await collectClawHubJoopoOwnerErrors({ plugins: candidates });
  if (errors.length > 0) {
    throw new Error(
      `ClawHub Joopo package ownership preflight failed:\n${errors.map((error) => `- ${error}`).join("\n")}`,
    );
  }

  console.log(`ClawHub Joopo owner preflight passed for ${candidates.length} candidate(s).`);
}
