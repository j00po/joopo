#!/usr/bin/env bash
# Rootless Joopo in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-joopo-podman.sh launch        # Start gateway
#   ./scripts/run-joopo-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   joopo --container joopo dashboard --no-open
#   joopo --container joopo channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      JOOPO_GATEWAY_TOKEN|JOOPO_PODMAN_CONTAINER|JOOPO_PODMAN_IMAGE|JOOPO_IMAGE|JOOPO_PODMAN_PULL|JOOPO_PODMAN_GATEWAY_HOST_PORT|JOOPO_GATEWAY_PORT|JOOPO_PODMAN_BRIDGE_HOST_PORT|JOOPO_BRIDGE_PORT|JOOPO_GATEWAY_BIND|JOOPO_PODMAN_USERNS|JOOPO_BIND_MOUNT_OPTIONS|JOOPO_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-joopo-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${JOOPO_CONFIG_DIR:-$EFFECTIVE_HOME/.joopo}"
ENV_FILE="${JOOPO_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${JOOPO_CONFIG_DIR:-$EFFECTIVE_HOME/.joopo}"
ENV_FILE="${JOOPO_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${JOOPO_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${JOOPO_PODMAN_CONTAINER:-joopo}"
JOOPO_IMAGE="${JOOPO_PODMAN_IMAGE:-${JOOPO_IMAGE:-joopo:local}}"
PODMAN_PULL="${JOOPO_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${JOOPO_PODMAN_GATEWAY_HOST_PORT:-${JOOPO_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${JOOPO_PODMAN_BRIDGE_HOST_PORT:-${JOOPO_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${JOOPO_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$JOOPO_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  JOOPO_CONTAINER="" JOOPO_CONFIG_DIR="$config_dir" \
    joopo config get gateway.bind 2>/dev/null || true
}
...
# JOOPO_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${JOOPO_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v joopo >/dev/null 2>&1; then
    echo "Warning: joopo not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    JOOPO_CONTAINER="" JOOPO_CONFIG_DIR="$config_dir" \
      joopo config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    JOOPO_CONTAINER="" JOOPO_CONFIG_DIR="$config_dir" \
      joopo config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  JOOPO_CONTAINER="" JOOPO_CONFIG_DIR="$config_dir" \
    joopo config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${JOOPO_GATEWAY_TOKEN:-}" ]]; then
  export JOOPO_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "JOOPO_GATEWAY_TOKEN" "$JOOPO_GATEWAY_TOKEN"
  echo "Generated JOOPO_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/joopo.json"
...
PODMAN_USERNS="${JOOPO_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported JOOPO_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (JOOPO_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${JOOPO_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${JOOPO_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$JOOPO_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.joopo/.npm \
    -e JOOPO_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.joopo:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.joopo/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$JOOPO_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME joopo dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME joopo devices approve --latest  # if pairing required"
