...
BASE_IMAGE="${BASE_IMAGE:-joopo-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-joopo-sandbox-common:bookworm-slim}"
...
JOOPO_DOCKER_BUILD_USE_BUILDX="${JOOPO_DOCKER_BUILD_USE_BUILDX:-0}"
JOOPO_DOCKER_BUILD_CACHE_FROM="${JOOPO_DOCKER_BUILD_CACHE_FROM:-}"
JOOPO_DOCKER_BUILD_CACHE_TO="${JOOPO_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=joopo.sandbox=1)
NOTE
