...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${JOOPO_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with JOOPO_VITEST_MAX_WORKERS=$JOOPO_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env JOOPO_VITEST_MAX_WORKERS="$JOOPO_VITEST_MAX_WORKERS" pnpm test
      else
