...
- regenerates apps/ios/Joopo.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${JOOPO_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid JOOPO_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid JOOPO_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid JOOPO_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid JOOPO_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing JOOPO_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(JOOPO_URL_SLASH)$(JOOPO_URL_SLASH)#g'
)"
...
JOOPO_CODE_SIGN_STYLE = Automatic
JOOPO_DEVELOPMENT_TEAM = ${TEAM_ID}
JOOPO_IOS_SELECTED_TEAM = ${TEAM_ID}
JOOPO_APP_BUNDLE_ID = ai.joopo.client
JOOPO_SHARE_BUNDLE_ID = ai.joopo.client.share
JOOPO_ACTIVITY_WIDGET_BUNDLE_ID = ai.joopo.client.activitywidget
JOOPO_WATCH_APP_BUNDLE_ID = ai.joopo.client.watchkitapp
JOOPO_WATCH_EXTENSION_BUNDLE_ID = ai.joopo.client.watchkitapp.extension
JOOPO_APP_PROFILE =
JOOPO_SHARE_PROFILE =
JOOPO_PUSH_TRANSPORT = relay
JOOPO_PUSH_DISTRIBUTION = official
JOOPO_URL_SLASH = /
JOOPO_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
JOOPO_PUSH_APNS_ENVIRONMENT = production
EOF
