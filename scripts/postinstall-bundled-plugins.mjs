...
const DISABLE_POSTINSTALL_ENV = "JOOPO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "JOOPO_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallJoopoHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.JOOPO_HOME?.trim();
  return override ? pathResolve(resolvePostinstallTildePath(override, osHome)) : osHome;
}

function resolvePostinstallUserPath(input, joopoHome) {
  return pathResolve(resolvePostinstallTildePath(input, joopoHome));
}
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.joopo-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const joopoHome = resolvePostinstallJoopoHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  if (stateOverride) {
    addStateRoot(resolvePostinstallUserPath(stateOverride, joopoHome));
  }
  const configPath = env?.JOOPO_CONFIG_PATH?.trim();
  if (configPath) {
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.joopo-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneJoopoCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune Joopo compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
