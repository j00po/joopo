...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/joopo/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Joopo Re-Auth" -c "After getting key, run: ssh $SERVER '~/joopo/scripts/mobile-reauth.sh'" --id joopo-auth
        ;;
    *)
