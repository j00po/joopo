...
function parseArgs(argv) {
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("JOOPO_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("JOOPO_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
  ...
  const envIds = normalizeCsv(process.env.JOOPO_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function joopoCommand(repoRoot, args) {
  return {
...
function sourceJoopoCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    JOOPO_LOG_DIR: path.join(runRoot, "logs"),
    JOOPO_QA_SUITE_PROGRESS: process.env.JOOPO_QA_SUITE_PROGRESS ?? "1",
    ...
    for (const { phase, args } of commands) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...joopoCommand(params.repoRoot, ["plugins", ...args]),
    ...
    for (const alias of plugin.cliCommandAliases) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...joopoCommand(params.repoRoot, [command, "--help"]),
...
function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = runMeasuredCommand({
      ...
      ...sourceJoopoCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-plugin-gauntlet-"));
