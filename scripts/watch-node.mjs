...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.JOOPO_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[joopo] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[joopo] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[joopo] This usually means a file in node_modules is empty or truncated.");
  console.error("[joopo] Recommended recovery:");
  console.error("[joopo]   rm -rf node_modules");
  console.error("[joopo]   pnpm store prune");
  console.error("[joopo]   pnpm install");
  console.error("");
  console.error("[joopo] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.JOOPO_WATCH_MODE = "1";
  childEnv.JOOPO_WATCH_SESSION = watchSession;
  ...
  childEnv.JOOPO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.JOOPO_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `joopo doctor --fix --non-interactive` once.",
        deps,
      );
