...
# Build and bundle Joopo into a minimal .app we can open.
# Outputs to dist/Joopo.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Joopo.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Joopo"
MLX_TTS_HELPER_PRODUCT="joopo-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.joopo.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/joopo/joopo/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Joopo/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :JoopoBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :JoopoGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Joopo"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Joopo"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Joopo"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Joopo" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Joopo/Resources/Joopo.icns" "$APP_ROOT/Contents/Resources/Joopo.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Joopo/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying JoopoKit resources"
JOOPOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/JoopoKit_JoopoKit.bundle"
if [ -d "$JOOPOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/JoopoKit_JoopoKit.bundle"
  cp -R "$JOOPOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/JoopoKit_JoopoKit.bundle"
else
  echo "WARN: JoopoKit resource bundle not found at $JOOPOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Joopo"
killall -q Joopo 2>/dev/null || true
