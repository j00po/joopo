...
function ensureJoopoExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "joopo");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "joopo.plugin.json") ||
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureJoopoExtensionAlias({ repoRoot, distExtensionsRoot });
