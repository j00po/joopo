...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.JOOPO_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestNoOutputTimeoutMs(env = process.env) {
  return parsePositiveInt(env.JOOPO_VITEST_NO_OUTPUT_TIMEOUT_MS);
}
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.JOOPO_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.JOOPO_VITEST_MAX_WORKERS ?? env.JOOPO_TEST_WORKERS);
}
