...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.joopo?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.joopo?.channel?.order;
      const labelRaw = parsed.joopo?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".joopo-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "joopo-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "joopo-build",
...
function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "joopo", "browser", "--help"]);`,
  ...
  const result = spawnSync(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", inlineModule],
    {
      ...
      env: {
        ...renderContext.env,
        JOOPO_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      },
