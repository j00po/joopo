...
function usage() {
  return "Usage: node scripts/check-joopo-package-tarball.mjs <joopo.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`Joopo package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.JOOPO_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-joopo-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Joopo package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Joopo package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Joopo package tarball integrity passed.");
