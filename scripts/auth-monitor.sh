...
#
# Suggested cron: */30 * * * * /home/admin/joopo/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Joopo notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., joopo-alerts)
...
STATE_FILE="$HOME/.joopo/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Joopo if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use joopo
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Joopo to $NOTIFY_PHONE..."
            joopo send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Joopo is down. Run: ssh l36 '~/joopo/scripts/mobile-reauth.sh'" "urgent"
    exit 1
