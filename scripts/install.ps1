# Joopo Installer for Windows (PowerShell)
# Usage: iwr -useb https://joopo.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://joopo.ai/install.ps1))) -NoOnboard

param(
    [ValidateSet("npm", "git")]
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\joopo",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Joopo Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Joopo.$NC" -Level info
    Write-Host ""
}
...
function Get-NpmWorkingDirectory {
    $workingDirectory = Join-Path ([System.IO.Path]::GetTempPath()) "joopo-installer"
    New-Item -ItemType Directory -Path $workingDirectory -Force | Out-Null
    return $workingDirectory
}

function Install-JoopoNpm {
    param([string]$Target = "latest")

    $installSpec = Resolve-PackageInstallSpec -Target $Target
    
    Write-Host "Installing Joopo ($installSpec)..." -Level info
    ...
    try {
        ...
        Write-Host "Joopo installed" -Level success
        return $true
    } catch {
...
function Install-JoopoGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Joopo from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/joopo/joopo.git $RepoDir 2>&1
    } elseif ($Update) {
...
@echo off
node "$entryPath" %*
"@ | Out-File -FilePath "$wrapperDir\joopo.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:joopo/joopo#main"
    }
    if (Test-ExplicitPackageInstallSpec -Target $trimmed) {
        return $trimmed
    }
    return "joopo@$trimmed"
}
...
function Complete-Install {
    ...
    throw "Joopo installation failed with exit code $($script:InstallExitCode)."
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Joopo from git to $GitDir" -Level info
        } else {
            try {
                npm uninstall -g joopo 2>$null | Out-Null
            } catch { }
            if (!(Install-JoopoGit -RepoDir $GitDir -Update:(-not $NoGitUpdate))) {
                return (Fail-Install)
            }
        }
    } else {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Joopo via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            $gitWrapper = "$env:USERPROFILE\.local\bin\joopo.cmd"
            if (Test-Path $gitWrapper) {
                Remove-Item -Force $gitWrapper
                Write-Host "Removed git wrapper (switching to npm)" -Level info
            }
            if (!(Install-JoopoNpm -Target $Tag)) {
                return (Fail-Install)
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'joopo onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Joopo installed successfully!" -Level success
    return $true
}
