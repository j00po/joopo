...
const (
	...
	envDocsI18nPromptTimeout    = "JOOPO_DOCS_I18N_PROMPT_TIMEOUT"
	envDocsI18nCommandWaitDelay = "JOOPO_DOCS_I18N_COMMAND_WAIT_DELAY"
	envDocsI18nCodexExecutable  = "JOOPO_DOCS_I18N_CODEX_EXECUTABLE"
)
...
func runCodexExecPrompt(ctx context.Context, req codexPromptRequest) (string, error) {
	outputFile, err := os.CreateTemp("", "joopo-docs-i18n-codex-*.txt")
	if err != nil {
		return "", err
	}
...
func isolatedCodexHomeBase() (string, error) {
	...
	base := filepath.Join(cacheDir, "joopo-docs-i18n")
