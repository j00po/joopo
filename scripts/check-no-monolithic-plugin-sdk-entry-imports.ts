...
import { discoverJoopoPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']joopo\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']joopo\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']joopo\/plugin-sdk\/channel-runtime["']/,
    label: "joopo/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']joopo\/plugin-sdk\/config-runtime["']/,
    label: "joopo/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']joopo\/plugin-sdk\/infra-runtime["']/,
    label: "joopo/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic joopo/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy joopo/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused joopo/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
