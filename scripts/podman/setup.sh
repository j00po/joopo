#!/usr/bin/env bash
# One-time host setup for rootless Joopo in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.joopo by default, and uses the repo-local
# launch script at ./scripts/run-joopo-podman.sh.
#
...
#   Or set JOOPO_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-joopo-podman.sh launch
#   ./scripts/run-joopo-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start joopo.service
set -euo pipefail

REPO_PATH="${JOOPO_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-joopo-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/joopo.container.in"
JOOPO_USER="$(id -un)"
JOOPO_HOME="${HOME:-}"
JOOPO_CONFIG_DIR="${JOOPO_CONFIG_DIR:-}"
JOOPO_WORKSPACE_DIR="${JOOPO_WORKSPACE_DIR:-}"
JOOPO_IMAGE="${JOOPO_PODMAN_IMAGE:-${JOOPO_IMAGE:-joopo:local}}"
JOOPO_CONTAINER_NAME="${JOOPO_PODMAN_CONTAINER:-joopo}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${JOOPO_PODMAN_GATEWAY_HOST_PORT:-${JOOPO_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate JOOPO_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${JOOPO_PODMAN_QUADLET:-}" ]]; then
  case "${JOOPO_PODMAN_QUADLET,,}" in
...
if [[ "$JOOPO_IMAGE" == "joopo:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set JOOPO_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$JOOPO_HOME" ]]; then
  JOOPO_HOME="$(resolve_user_home "$JOOPO_USER")"
fi
if [[ -z "$JOOPO_HOME" ]]; then
  echo "Unable to resolve HOME for user $JOOPO_USER." >&2
  exit 1
fi
if [[ -z "$JOOPO_CONFIG_DIR" ]]; then
  JOOPO_CONFIG_DIR="$JOOPO_HOME/.joopo"
fi
if [[ -z "$JOOPO_WORKSPACE_DIR" ]]; then
  JOOPO_WORKSPACE_DIR="$JOOPO_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$JOOPO_HOME"
validate_mount_source_path "config directory" "$JOOPO_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$JOOPO_WORKSPACE_DIR"
validate_container_name "$JOOPO_CONTAINER_NAME"
validate_image_name "$JOOPO_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$JOOPO_CONFIG_DIR" "$JOOPO_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$JOOPO_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$JOOPO_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${JOOPO_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "JOOPO_DOCKER_APT_PACKAGES=${JOOPO_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${JOOPO_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "JOOPO_EXTENSIONS=${JOOPO_EXTENSIONS}")
fi
if [[ -n "${JOOPO_INSTALL_BROWSER:-}" ]]; then
  BUILD_ARGS+=(--build-arg "JOOPO_INSTALL_BROWSER=${JOOPO_INSTALL_BROWSER}")
fi

if [[ "$JOOPO_IMAGE" == "joopo:local" ]]; then
  echo "Building image $JOOPO_IMAGE ..."
  podman build -t "$JOOPO_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$JOOPO_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $JOOPO_IMAGE"
  else
    echo "Pulling image $JOOPO_IMAGE ..."
    podman pull "$JOOPO_IMAGE"
  fi
fi

ENV_FILE="$JOOPO_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated JOOPO_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "JOOPO_PODMAN_CONTAINER" "$JOOPO_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "JOOPO_PODMAN_IMAGE" "$JOOPO_IMAGE"

CONFIG_JSON="$JOOPO_CONFIG_DIR/joopo.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$JOOPO_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/joopo.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  JOOPO_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$JOOPO_HOME")"
  JOOPO_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$JOOPO_CONFIG_DIR")"
  JOOPO_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$JOOPO_WORKSPACE_DIR")"
  JOOPO_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$JOOPO_IMAGE")"
  JOOPO_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$JOOPO_CONTAINER_NAME")"
  sed \
    -e "s|{{JOOPO_HOME}}|$JOOPO_HOME_ESCAPED|g" \
    -e "s|{{JOOPO_CONFIG_DIR}}|$JOOPO_CONFIG_ESCAPED|g" \
    -e "s|{{JOOPO_WORKSPACE_DIR}}|$JOOPO_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$JOOPO_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$JOOPO_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start joopo.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start joopo.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-joopo-podman.sh launch"
echo "  ./scripts/run-joopo-podman.sh launch setup"
echo "  joopo --container $JOOPO_CONTAINER_NAME dashboard --no-open"
