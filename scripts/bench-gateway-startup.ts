...
function printUsage(): void {
  console.log(`Joopo Gateway startup benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "joopo.json");
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "joopo-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "joopo-bench",
    npm_config_update_notifier: "false",
    JOOPO_CONFIG: configPath,
    JOOPO_CONFIG_PATH: configPath,
    JOOPO_GATEWAY_STARTUP_TRACE: "1",
    JOOPO_HOME: root,
    JOOPO_LOCAL_CHECK: "0",
    JOOPO_NO_RESPAWN: "1",
    JOOPO_STATE_DIR: path.join(root, "state"),
    JOOPO_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
  return env;
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "joopo-gateway-bench-"));
  ...
  const childArgs = [
    ...(options.cpuProfDir
      ? [
          ...
          options.cpuProfDir,
          "--cpu-prof-name",
          `joopo-gateway-${options.benchCase.id}-${options.sampleIndex}-${Date.now()}.cpuprofile`,
        ]
      : []),
    options.entry,
    "gateway",
