...
function toCatalogInstall(value, packageName) {
  ...
  const joopohubSpec = trimString(install.joopohubSpec);
  ...
  return {
    ...
    ...(npmSpec ? { npmSpec } : {}),
    ...(defaultChoice === "joopohub" || defaultChoice === "npm" || defaultChoice === "local"
      ? { defaultChoice }
      : {}),
...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.joopo) ? packageJson.joopo : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.joopo?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.joopo?.channel?.id) || left.name;
    const rightId = trimString(right.joopo?.channel?.id) || right.name;
