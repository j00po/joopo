...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/joopo-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
for attempt in $(seq 1 "${JOOPO_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${JOOPO_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${JOOPO_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${JOOPO_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
