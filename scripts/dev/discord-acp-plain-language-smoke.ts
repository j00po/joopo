...
type DriverMode = "token" | "webhook" | "joopo";
...
function resolveStateDir(): string {
  const override = process.env.JOOPO_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.JOOPO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".joopo");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver joopo] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Joopo spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|joopo> Driver transport mode (default: token)\n" +
    ...
    "  --joopo-bin <path>        Joopo CLI binary for driver=joopo (default: joopo)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  JOOPO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  JOOPO_DISCORD_SMOKE_DRIVER\n" +
    "  JOOPO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  JOOPO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  JOOPO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  JOOPO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  JOOPO_DISCORD_SMOKE_AGENT\n" +
    "  JOOPO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  JOOPO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  JOOPO_DISCORD_SMOKE_POLL_MS\n" +
    "  JOOPO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  JOOPO_DISCORD_SMOKE_JOOPO_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.JOOPO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.JOOPO_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "joopo"
        ? "joopo"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.JOOPO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.JOOPO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.JOOPO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.JOOPO_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.JOOPO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.JOOPO_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.JOOPO_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.JOOPO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.JOOPO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.JOOPO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const joopoBin =
    resolveArg("--joopo-bin") || process.env.JOOPO_DISCORD_SMOKE_JOOPO_BIN || "joopo";
...
async function joopoCliJson<T>(params: { joopoBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.joopoBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`joopo ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithJoopo(params: {
  joopoBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await joopoCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    joopoBin: params.joopoBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "joopo") {
    return await readMessagesWithJoopo({
      joopoBin: params.args.joopoBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `joopo-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("joopo message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "joopo"
            ? await readMessagesWithJoopo({
                joopoBin: args.joopoBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Joopo.`,
        diagnostics: {
          bindingCandidates: [
            {
