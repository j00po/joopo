...
function usage() {
  return [
    "Usage: pnpm rtt <joopo@spec> [--package-tgz PATH] [--provider mock-openai|live-frontier] [--runs N] [--samples N] [--sample-timeout-ms N] [--timeout-ms N] [--harness-root PATH] [--output PATH]",
    "",
    "Examples:",
    "  pnpm rtt joopo@main --package-tgz .artifacts/package/joopo.tgz",
    "  pnpm rtt joopo@beta",
    "  pnpm rtt joopo@2026.4.30",
    "  pnpm rtt joopo@latest --provider live-frontier",
  ].join("\n");
}
...
function parseArgs(argv: string[]) {
  ...
  let harnessRoot = "~/Developer/joopobot";
  let output = "runs";
  ...
  return {
    spec: validateJoopoPackageSpec(spec),
    options: {
      packageTgz,
      providerMode,
      runs,
...
async function main() {
  ...
  if (spec === "joopo@main" && !options.packageTgz) {
    throw new Error("joopo@main requires --package-tgz.");
  }
  const version =
    spec === "joopo@main"
