...
const DEFAULT_JOOPO_TESTBOX_CLAIM_TTL_MINUTES = 12 * 60;
const TESTBOX_ID_PATTERN = /^tbx_[a-z0-9]+$/u;
const JOOPO_TESTBOX_CLAIM_FILE = "joopo-runner.json";
...
function resolveBlacksmithTestboxStateDir({ env = process.env, homeDir } = {}) {
  if (env.JOOPO_BLACKSMITH_TESTBOX_STATE_DIR) {
    return env.JOOPO_BLACKSMITH_TESTBOX_STATE_DIR;
  }
...
function resolveJoopoTestboxClaimPath({ testboxId, env = process.env, homeDir } = {}) {
  const stateDir = resolveBlacksmithTestboxStateDir({ env, homeDir });
  return path.join(stateDir, testboxId, JOOPO_TESTBOX_CLAIM_FILE);
}

export function evaluateJoopoTestboxClaim({
  testboxId,
  cwd,
  env = process.env,
  exists = fs.existsSync,
  ...
  const claimPath = resolveJoopoTestboxClaimPath({ testboxId, env, homeDir });
  const expectedRepoRoot = path.resolve(cwd || process.cwd());
  const maxAgeMinutes = parsePositiveInteger(
    env.JOOPO_TESTBOX_CLAIM_TTL_MINUTES,
    DEFAULT_JOOPO_TESTBOX_CLAIM_TTL_MINUTES,
  );
  const problems = [];

  if (!exists(claimPath)) {
    problems.push(
      `Joopo Testbox claim missing for ${testboxId}: expected ${claimPath}. ` +
  ...
  try {
    ...
    problems.push(`Joopo Testbox claim is unreadable for ${testboxId}: ${error.message}`);
  }

  const claimedRepoRoot = claim?.repoRoot ? path.resolve(claim.repoRoot) : "";
  if (!claimedRepoRoot) {
    problems.push(`Joopo Testbox claim is missing repoRoot for ${testboxId}: ${claimPath}`);
  } else if (claimedRepoRoot !== expectedRepoRoot) {
    problems.push(
      `Joopo Testbox claim repo mismatch for ${testboxId}: claimed ${claimedRepoRoot}, current ${expectedRepoRoot}. ` +
  ...
  if (!Number.isFinite(claimedAtMs)) {
    problems.push(`Joopo Testbox claim is missing claimedAt for ${testboxId}: ${claimPath}`);
  } else {
    const ageMinutes = Math.floor((now().getTime() - claimedAtMs) / 60000);
    if (ageMinutes > maxAgeMinutes) {
      problems.push(
        `Joopo Testbox claim is stale for ${testboxId}: ${ageMinutes}m old, limit ${maxAgeMinutes}m. ` +
...
export function writeJoopoTestboxClaim({
  testboxId,
  cwd,
  env = process.env,
  homeDir,
