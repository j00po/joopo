...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.JOOPO_VITEST_MAX_WORKERS || env.JOOPO_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      JOOPO_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.JOOPO_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.JOOPO_TEST_PROJECTS_PARALLEL &&
        !baseEnv.JOOPO_VITEST_MAX_WORKERS &&
        !baseEnv.JOOPO_TEST_WORKERS &&
