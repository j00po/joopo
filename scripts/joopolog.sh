...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "joopolog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
joopolog - Joopo Logging Utility

USAGE:
    joopolog [OPTIONS]

DESCRIPTION:
    View Joopo logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Joopo logs flow through the macOS unified log (subsystem: ai.joopo).
...
QUICK START:
    joopolog -n 100             Show last 100 lines from all components
    joopolog -f                 Follow logs in real-time
    joopolog -e                 Show only errors
    joopolog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    joopolog                   Show last 50 lines from past 5 minutes (default)
    joopolog -f                Stream logs continuously
    joopolog -n 100            Show last 100 lines
    joopolog -e                Show only recent errors
    joopolog -l 30m -n 200     Show last 200 lines from past 30 minutes
    joopolog -c ServerManager  Show recent ServerManager logs
    joopolog -s "fail"         Search for "fail" in recent logs
    joopolog --server -e       Show recent server errors
    joopolog -f -d             Stream debug logs continuously
