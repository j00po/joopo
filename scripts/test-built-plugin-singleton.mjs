...
const { clearPluginCommands, getPluginCommandSpecs, loadJoopoPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadJoopoPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "joopo.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'joopo/plugin-sdk';",
...
const registry = loadJoopoPlugins({
  ...
  env: {
    ...process.env,
    JOOPO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
