...
export function runTestboxSyncSanity({
  ...
  const result = evaluateTestboxSyncSanity({
    ...
    deletionThreshold: parsePositiveInteger(
      env.JOOPO_TESTBOX_DELETION_THRESHOLD,
      DEFAULT_DELETION_THRESHOLD,
    ),
    allowMassDeletions: parseBooleanEnv(env.JOOPO_TESTBOX_ALLOW_MASS_DELETIONS),
  });
  ...
  if (keyResult.checked) {
    stdout.write(`Testbox local key and Joopo claim ok: ${keyResult.testboxId}\n`);
  }
  stdout.write(
