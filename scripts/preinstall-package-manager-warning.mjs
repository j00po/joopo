...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[joopo] warning: detected ${packageManager} for install lifecycle.`,
    "[joopo] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[joopo] prefer: corepack pnpm install",
  ].join("\n");
}
