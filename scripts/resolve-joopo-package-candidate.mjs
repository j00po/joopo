...
const DEFAULT_OUTPUT_NAME = "joopo-current.tgz";
export const JOOPO_PACKAGE_SPEC_RE =
  /^joopo@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-joopo-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateJoopoPackageSpec(spec) {
  if (!JOOPO_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be joopo@alpha, joopo@beta, joopo@latest, or an exact Joopo release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Joopo branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `joopo-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^joopo-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no Joopo tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-joopo-for-docker.mjs",
        "--source-dir",
      ...
      validateJoopoPackageSpec(options.packageSpec);
  ...
  console.error(`Checking Joopo package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-joopo-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `Joopo package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "joopo") {
    throw new Error(`package candidate must be named "joopo"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
