#!/usr/bin/env node
// Builds the Joopo package artifact used by Docker E2E.
...
async function newestJoopoTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Joopo tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building Joopo package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building Joopo Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing Joopo package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing Joopo package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestJoopoTarball(outputDir, packOutput);
  ...
  console.error("==> Checking Joopo package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-joopo-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
