#!/bin/bash
# Setup Joopo Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'joopo-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Joopo message: Send warning via Joopo itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/joopo-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/joopo-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/joopo-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now joopo-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/JoopoAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/JoopoAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Joopo*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (JoopoAuth) shows status and opens auth URL if needed."
echo "The full widget (JoopoAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status joopo-auth-monitor.timer"
echo ""
echo "Quick commands:"
