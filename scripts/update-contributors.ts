...
const REPO = "joopo/joopo";
const PER_LINE = 10;
const AVATAR_PROBE_SIZE = 40;
const AVATAR_SIZE = 48;
const CLAWTRIBUTORS_START = "<!-- contributors:start -->";
const CLAWTRIBUTORS_END = "<!-- contributors:end -->";
const CLAWTRIBUTORS_HIDDEN_START = "<!-- contributors:hidden:start";
const CLAWTRIBUTORS_HIDDEN_END = "contributors:hidden:end -->";
...
const readmeWithoutMeta = hiddenRange
  ...
  : currentReadme;
const range = findContributorsRange(readmeWithoutMeta);
...
async function probeDefaultGitHubAvatar(login: string): Promise<boolean> {
  try {
    const response = await fetch(`https://github.com/${login}.png?size=${AVATAR_PROBE_SIZE}`, {
      headers: { "user-agent": "joopo-contributors" },
...
function parseReadmeEntries(
  ...
  const range = findContributorsRange(content);
  if (!range) {
    return [];
  }
