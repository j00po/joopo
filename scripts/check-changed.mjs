...
export function shouldDelegateChangedCheckToTestbox(argv = [], env = process.env) {
  if (!isTruthyEnvFlag(env.JOOPO_TESTBOX)) {
    return false;
  }
  if (isTruthyEnvFlag(env.JOOPO_TESTBOX_REMOTE_RUN)) {
    return false;
  }
...
export async function runChangedCheckViaTestbox(argv = [], env = process.env) {
  console.error(
    "[check:changed] JOOPO_TESTBOX=1 set; delegating to Blacksmith Testbox via `pnpm testbox:run`.",
  );
