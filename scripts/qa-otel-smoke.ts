...
function joopoEntryArgs(): string[] {
...
function spawnJoopo(args: string[], env: NodeJS.ProcessEnv): ChildProcess {
  return spawn(process.execPath, [...joopoEntryArgs(), ...args], {
    env,
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "joopo-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.JOOPO_QA_SUITE_PROGRESS = env.JOOPO_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function assertSmoke(params: {
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("joopo.content."));
  ...
  const modelSpans = params.spans.filter((span) => span.name === "joopo.model.call");
  const modelErrorSpans = modelSpans.filter((span) => {
    const serialized = JSON.stringify(span.attributes);
    return (
      Object.hasOwn(span.attributes, "error.type") ||
      Object.hasOwn(span.attributes, "joopo.errorCategory") ||
      serialized.includes("StreamAbandoned")
    );
  });
  if (modelSpans.length === 0) {
    failures.push("no joopo.model.call span was exported");
  }
...
async function main() {
  ...
  let childExitCode = 1;
  try {
    const child = spawnJoopo(buildQaArgs(options), buildQaEnv(port));
