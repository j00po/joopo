...
REMOTE_URL="${JOOPO_REF_REMOTE:-https://github.com/joopo/joopo.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-joopo-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Joopo branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Joopo ref: ${REF}" >&2
exit 1
