...
if joopo_live_truthy "${JOOPO_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  JOOPO_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${JOOPO_LIVE_PROVIDERS:-}" || -n "${JOOPO_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      joopo_live_collect_auth_dirs_from_csv "${JOOPO_LIVE_PROVIDERS:-}"
      joopo_live_collect_auth_dirs_from_csv "${JOOPO_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      joopo_live_collect_auth_files_from_csv "${JOOPO_LIVE_PROVIDERS:-}"
      joopo_live_collect_auth_files_from_csv "${JOOPO_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${JOOPO_DOCKER_PROFILE_ENV_ONLY:-0}"
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e JOOPO_LIVE_TEST=1 \
  -e JOOPO_LIVE_MODELS="${JOOPO_LIVE_MODELS:-modern}" \
  -e JOOPO_LIVE_PROVIDERS="${JOOPO_LIVE_PROVIDERS:-}" \
  -e JOOPO_LIVE_MAX_MODELS="${JOOPO_LIVE_MAX_MODELS:-12}" \
  -e JOOPO_LIVE_MODEL_TIMEOUT_MS="${JOOPO_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e JOOPO_LIVE_REQUIRE_PROFILE_KEYS="${JOOPO_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e JOOPO_LIVE_GATEWAY_MODELS="${JOOPO_LIVE_GATEWAY_MODELS:-}" \
  -e JOOPO_LIVE_GATEWAY_PROVIDERS="${JOOPO_LIVE_GATEWAY_PROVIDERS:-}" \
  -e JOOPO_LIVE_GATEWAY_MAX_MODELS="${JOOPO_LIVE_GATEWAY_MAX_MODELS:-}" \
