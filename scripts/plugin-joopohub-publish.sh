...
if [[ "${mode}" != "--dry-run" && "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/plugin-joopohub-publish.sh [--dry-run|--publish] <package-dir>" >&2
  exit 2
fi
...
if ! command -v joopohub >/dev/null 2>&1; then
  echo "joopohub CLI is required on PATH" >&2
  exit 1
fi
...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-joopo/joopo}}"
source_commit="${SOURCE_COMMIT:-$(git rev-parse HEAD)}"
source_ref="${SOURCE_REF:-$(git symbolic-ref -q HEAD || true)}"
joopohub_workdir="${JOOPOHUB_WORKDIR:-${JOOPOHUB_WORKDIR:-$(pwd)}}"
...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/joopo-joopohub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${JOOPO_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${JOOPO_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
echo "Resolved JoopoHub workdir: ${joopohub_workdir}"
echo "Publish auth: GitHub Actions OIDC via JoopoHub short-lived token"

printf 'Pack command: JOOPOHUB_WORKDIR=%q' "${joopohub_workdir}"
printf ' %q' "${pack_cmd[@]}"
printf '\n'
...
JOOPOHUB_WORKDIR="${joopohub_workdir}" \
...
try {
  ...
  console.error(`joopohub package pack did not return JSON: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
}
if (!parsed || typeof parsed.path !== "string" || parsed.path.trim() === "") {
  console.error("joopohub package pack output did not include a tarball path.");
  process.exit(1);
}
...
if [[ "${mode}" == "--dry-run" ]]; then
  JOOPOHUB_WORKDIR="${joopohub_workdir}" "${publish_cmd[@]}" --dry-run
  exit 0
fi

publish_log="${pack_dir}/publish.log"
for attempt in $(seq 1 "${JOOPO_JOOPOHUB_PUBLISH_ATTEMPTS:-8}"); do
  if JOOPOHUB_WORKDIR="${joopohub_workdir}" "${publish_cmd[@]}" > >(tee "${publish_log}") 2>&1; then
    exit 0
  fi
  if ! grep -Eqi "rate limit|too many requests|\\b429\\b" "${publish_log}"; then
    exit 1
  fi
  echo "JoopoHub publish hit a rate limit; retrying (${attempt}/${JOOPO_JOOPOHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${JOOPO_JOOPOHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "JoopoHub publish failed after ${JOOPO_JOOPOHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
