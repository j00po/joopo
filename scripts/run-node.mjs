...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.JOOPO_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.JOOPO_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.JOOPO_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by JOOPO_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by JOOPO_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "JOOPO_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "JOOPO_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "JOOPO_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "JOOPO_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "JOOPO_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "JOOPO_RUN_NODE_BUILD_LOCK_STALE_MS";
...
const logRunner = (message, deps) => {
  if (deps.env.JOOPO_RUNNER_LOG === "0") {
    return;
  }
  const line = `[joopo] ${message}\n`;
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `joopo-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.JOOPO_TRACE_SYNC_IO === "1") {
...
const runJoopo = async (deps) => {
  const diagnosticArgs = resolveRunNodeDiagnosticArgs(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "joopo.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[joopo] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.JOOPO_BUILD_PRIVATE_QA = "1";
    deps.env.JOOPO_ENABLE_PRIVATE_QA_CLI = "1";
  }
