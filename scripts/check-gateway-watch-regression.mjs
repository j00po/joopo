...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  JOOPO_SKIP_ACPX_RUNTIME: "1",
  JOOPO_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$JOOPO_WATCH_PID_FILE"',
    'mkdir -p "$JOOPO_HOME/.joopo"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$JOOPO_HOME/.joopo/joopo.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    JOOPO_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    JOOPO_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-gateway-watch-"));
