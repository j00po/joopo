...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "joopo/plugin-sdk/agent-runtime-test-contracts",
  "joopo/plugin-sdk/plugin-test-contracts",
  "joopo/plugin-sdk/provider-test-contracts",
] as const;
...
function resolveInstalledBinaryPath(prefixDir: string): string {
  return process.platform === "win32"
    ? join(prefixDir, "joopo.cmd")
    : join(prefixDir, "bin", "joopo");
}
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".joopo", "joopo.json");
  mkdirSync(join(homeDir, ".joopo"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-joopo-release-check",
  });

  writePackedBundledPluginActivationConfig(homeDir);
  execFileSync(
    process.execPath,
    [join(packageRoot, "joopo.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
  ...
  execFileSync(process.execPath, [join(packageRoot, "joopo.mjs"), "plugins", "doctor"], {
    cwd: packageRoot,
    stdio: "inherit",
    env,
  });
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "joopo-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "joopo");
    ...
    execFileSync(
      process.execPath,
      [join(packageRoot, "joopo.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.joopo-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.joopo-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from joopo/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("joopo/plugin-sdk/".length);
