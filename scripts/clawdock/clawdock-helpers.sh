...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/joopo"
      return 1
    fi
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^JOOPO_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d joopo-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart joopo-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f joopo-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec joopo-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm joopo-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build joopo-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d joopo-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build joopo-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "JOOPO_GATEWAY_TOKEN=$token" joopo-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" joopo-gateway \
    bash -c './joopo.mjs config set gateway.remote.token "$TOKEN" && ./joopo.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec joopo-gateway \
    bash -c "./joopo.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart joopo-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm joopo-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec joopo-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (joopo alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to joopo project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.joopo)${_CLR_RESET}"
