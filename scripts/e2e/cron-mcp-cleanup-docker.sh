...
IMAGE_NAME="$(docker_e2e_resolve_image "joopo-cron-mcp-cleanup-e2e" JOOPO_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="joopo-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t joopo-cron-mcp-client-log.XXXXXX)"
...
JOOPO_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 cron-mcp-cleanup empty)"
...
docker_e2e_run_with_harness \
  ...
  -e "GW_TOKEN=$TOKEN" \
  -e "JOOPO_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    export SUCCESS_MARKER=JOOPO_CRON_MCP_CLEANUP_OK
    export MOCK_REQUEST_LOG=/tmp/joopo-cron-mock-openai-requests.jsonl
    export JOOPO_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:\$MOCK_PORT/v1\"
    mock_pid=\"\$(joopo_e2e_start_mock_openai \"\$MOCK_PORT\" /tmp/cron-mcp-cleanup-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
      joopo_e2e_stop_process \"\${gateway_pid:-}\"
    ...
    gateway_pid=\"\$(joopo_e2e_start_gateway \"\$entry\" $PORT /tmp/cron-mcp-cleanup-gateway.log)\"
    joopo_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/cron-mcp-cleanup-gateway.log 300
