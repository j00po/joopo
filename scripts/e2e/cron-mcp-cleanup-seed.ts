...
import { applyDockerOpenAiProviderConfig, type JoopoConfig } from "./docker-openai-seed.ts";
...
process.title = "joopo-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.JOOPO_STATE_DIR?.trim() || path.join(os.homedir(), ".joopo");
  const configPath =
    process.env.JOOPO_CONFIG_PATH?.trim() || path.join(stateDir, "joopo.json");
