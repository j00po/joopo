...
async function main() {
  ...
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "joopo-crestodian-first-run-")));
  ...
  assert(
    shouldStartCrestodianForBareRoot(["node", "joopo"]),
    "bare joopo invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "joopo", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
