...
const PLUGIN_SPEC =
  process.env.JOOPO_KITCHEN_SINK_NPM_SPEC || "npm:@joopo/kitchen-sink@latest";
const PLUGIN_ID = process.env.JOOPO_KITCHEN_SINK_PLUGIN_ID || "joopo-kitchen-sink-fixture";
...
const READY_TIMEOUT_MS = readPositiveInt(process.env.JOOPO_KITCHEN_SINK_RPC_READY_MS, 240000);
const COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.JOOPO_KITCHEN_SINK_RPC_COMMAND_MS,
  180000,
);
const RPC_TIMEOUT_MS = readPositiveInt(process.env.JOOPO_KITCHEN_SINK_RPC_CALL_MS, 60000);
const MAX_RSS_MIB = readPositiveInt(process.env.JOOPO_KITCHEN_SINK_MAX_RSS_MIB, 2048);
...
function resolveJoopoRunner() {
  if (process.env.JOOPO_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.JOOPO_ENTRY],
      label: process.env.JOOPO_ENTRY,
    };
  }
  ...
  return { command: "pnpm", baseArgs: ["joopo"], label: "pnpm joopo" };
}

function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      JOOPO_CONFIG_PATH: path.join(stateDir, "joopo.json"),
      JOOPO_NO_ONBOARD: "1",
      JOOPO_SKIP_PROVIDERS: "0",
      JOOPO_KITCHEN_SINK_PERSONALITY:
        process.env.JOOPO_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runJoopo(runner, args, env, options = {}) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = await callGateway({
    config: readJson(options.env.JOOPO_CONFIG_PATH),
    configPath: options.env.JOOPO_CONFIG_PATH,
...
function configureKitchenSink(env, port) {
  const configPath = env.JOOPO_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.JOOPO_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function main() {
  const runner = resolveJoopoRunner();
  const port = readPositiveInt(process.env.JOOPO_KITCHEN_SINK_RPC_PORT, 19173);
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");

  console.log(`Kitchen Sink RPC walk using ${PLUGIN_SPEC} via ${runner.label}`);
  await runJoopo(runner, ["plugins", "install", PLUGIN_SPEC], env, { timeoutMs: 240000 });
  configureKitchenSink(env, port);
  await runJoopo(runner, ["plugins", "enable", PLUGIN_ID], env, { timeoutMs: 60000 });
  const inspect = parseJsonOutput(
    (await runJoopo(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env))
      .stdout,
  );
