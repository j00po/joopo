...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsRes?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "joopo/default") ?? modelIds.find((id) => id === "joopo");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing joopo model: ${JSON.stringify(modelIds)}`;
  } else if (modelsRes) {
...
if (!targetModel) {
  throw new Error(
    `joopo model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
