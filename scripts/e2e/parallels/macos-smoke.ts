...
const guestJoopo = "/opt/homebrew/bin/joopo";
const guestJoopoEntry = "/opt/homebrew/lib/node_modules/joopo/joopo.mjs";
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://joopo.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://joopo.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("joopo-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("joopo-parallels-macos-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("joopo-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.JOOPO_PARALLELS_MACOS_AGENT_TIMEOUT_S || 2700),
      () => this.verifyTurn(),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("joopo-main-upgrade.tgz"),
      );
      ...
      await this.phase(
        "upgrade.update-dev",
        Number(process.env.JOOPO_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S || 1800),
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'joopo.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'joopo-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'joopo.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g joopo >/dev/null 2>&1 || true
rm -rf "$HOME/.joopo"
...
curl -fsSL ${shellQuote(this.options.installUrl)} -o /tmp/joopo-install.sh
bash /tmp/joopo-install.sh --version ${shellQuote(this.installVersion)}
${guestJoopo} --version`,
    );
  }
...
${guestJoopo} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestJoopo, "--version"]);
...
check_path "$root/joopo"
check_path "$root/joopo/extensions"
if [ -d "$root/joopo/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/joopo/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/joopo-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/joopo`)}
export PATH=${shellQuote(`/tmp/joopo-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".joopo", "joopo.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 JOOPO_DISABLE_BUNDLED_PLUGINS=1 ${guestNode} ${guestJoopoEntry} update --channel dev --yes --json
${guestNode} ${guestJoopoEntry} --version
${guestNode} ${guestJoopoEntry} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestExec([guestNode, guestJoopoEntry, "update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'joopo.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'joopo-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'joopo.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} JOOPO_HOME=${shellQuote(home)} JOOPO_STATE_DIR=${shellQuote(`${home}/.joopo`)} JOOPO_CONFIG_PATH=${shellQuote(
        `${home}/.joopo/joopo.json`,
      )} ${guestNode} ${guestJoopoEntry} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/joopo-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestJoopo, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestJoopo(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestJoopo(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`joopo ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/joopo-dashboard-smoke.html 2>/dev/null; then
    grep -F '<title>Joopo Control</title>' /tmp/joopo-dashboard-smoke.html >/dev/null &&
      grep -F '<joopo-app></joopo-app>' /tmp/joopo-dashboard-smoke.html >/dev/null &&
      exit 0
  fi
  sleep 1
done
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestExec([guestNode, guestJoopoEntry, "models", "set", this.auth.modelId]);
...
rm -f "$provider_config_batch"`);
    ...
    this.guestExec([guestNode, guestJoopoEntry, "config", "set", "tools.profile", "minimal"]);
    this.guestSh(
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestNode} ${guestJoopoEntry} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    ...
    const matches = [...log.matchAll(/Joopo\s+([0-9][^\s]*)/gi)];
