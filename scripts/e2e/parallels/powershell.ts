...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'joopo-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-Joopo config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:JOOPO_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.joopo\\joopo.json' }
$env:JOOPO_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:JOOPO_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'joopo-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.JOOPO_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.JOOPO_PARALLELS_AGENT_CONFIG_PATCH || "{}");
...
Remove-Item Env:JOOPO_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:JOOPO_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsJoopoResolver = String.raw`function Resolve-JoopoCommand {
  if ($script:JoopoResolvedCommand) { return $script:JoopoResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\joopo.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\joopo.ps1'
  }
  foreach ($name in @('joopo.cmd', 'joopo.ps1', 'joopo')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'joopo.cmd'
    $shimCandidates += Join-Path $npmPrefix 'joopo.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:JoopoResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:JoopoResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
    $entryCandidates += Join-Path $env:APPDATA 'npm\node_modules\joopo\joopo.mjs'
  }
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\joopo\joopo.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:JoopoResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:JoopoResolvedCommand
    }
  }
  throw 'joopo command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Joopo {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $JoopoArgs)
  $command = Resolve-JoopoCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @JoopoArgs
    } else {
      & $command.Path @JoopoArgs
    }
  } finally {
