...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://joopo.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-joopo-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Joopo\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-joopo-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://joopo.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run joopo update --channel dev.
...
class WindowsSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("joopo-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("joopo-parallels-windows-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("joopo-main-fresh.tgz"));
    ...
    await this.phase(
      ...
      Number(process.env.JOOPO_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700),
      () => this.verifyTurn(),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase(
      "upgrade.update-dev",
      Number(process.env.JOOPO_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S || 1200),
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsJoopoResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanJoopo: boolean): void {
    const cleanScript = cleanJoopo
      ? "npm.cmd uninstall -g joopo --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "joopo --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-Joopo --version");
...
Invoke-Joopo onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "joopo onboard failed with exit code $LASTEXITCODE" }`,
      720_000,
    );
  }
...
${windowsScopedEnvFunction}
$script:JoopoUpdateExit = 0
Invoke-WithScopedEnv @{ JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; JOOPO_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-Joopo update --channel dev --yes --json
  $script:JoopoUpdateExit = $LASTEXITCODE
}
if ($script:JoopoUpdateExit -ne 0) { throw "joopo update failed with exit code $script:JoopoUpdateExit" }
Invoke-Joopo --version
Invoke-Joopo update status --json`,
      { timeoutMs: Number(process.env.JOOPO_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S || 1200) * 1000 },
    );
  }
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    const recoveryAfter =
      Number(process.env.JOOPO_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S || 180) * 1000;
    const start = Date.now();
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-Joopo gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= recoveryAfter) {
        ...
        this.guestPowerShell("Invoke-Joopo gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-Joopo gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-Joopo gateway status ${suffix}`);
  }
...
if (-not $agentOk) { throw 'joopo agent finished without OK response' }`,
      Number(process.env.JOOPO_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700) * 1000,
    );
  }
