...
class LinuxSmoke {
  ...
  private disableBonjour = parseBoolEnv(process.env.JOOPO_PARALLELS_LINUX_DISABLE_BONJOUR);
  private hostIp = "";
  private hostPort = 0;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("joopo-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("joopo-parallels-linux-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("joopo-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.JOOPO_PARALLELS_LINUX_AGENT_TIMEOUT_S || 1500),
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("joopo-main-upgrade.tgz"),
    );
...
printf 'preflight.npmRoot=%s\n' "$(npm root -g 2>/dev/null || true)"`);
  ...
  private installLatestRelease(): void {
    this.guestExec(["curl", "-fsSL", this.options.installUrl, "-o", "/tmp/joopo-install.sh"]);
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "JOOPO_NO_ONBOARD=1",
        "bash",
        "/tmp/joopo-install.sh",
        "--version",
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["joopo", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.joopo/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@joopo/test-bad-plugin","version":"1.0.0","joopo":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/joopo.plugin.json" <<'JSON'
...
config_path = Path("/root/.joopo/joopo.json")
...
PY`);
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " JOOPO_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "joopo gateway run" >/dev/null 2>&1 || true
rm -f /tmp/joopo-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env JOOPO_HOME=/root JOOPO_STATE_DIR=/root/.joopo JOOPO_CONFIG_PATH=/root/.joopo/joopo.json${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["joopo", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["joopo", "gateway", "status", "--deep", "--require-rpc"]
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.joopo/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.joopo/test-bad-plugin`);
  }

  private verifyLocalTurn(): void {
    this.guestExec(["joopo", "models", "set", this.auth.modelId]);
...
JSON
joopo config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["joopo", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} joopo agent --local --agent main --session-id "$session_id" --message ${shellQuote(
