...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\joopo\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
...
for attempt in 1 2; do
  ...
  rm -f "$HOME/.joopo/agents/main/sessions/$session_id.jsonl"
  output_file="$(mktemp)"
  set +e
...
if [ "$agent_ok" != true ]; then
  echo "joopo agent finished without OK response" >&2
  exit 1
fi`;
}

function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  return `$script:JoopoUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ JOOPO_DISABLE_BUNDLED_PLUGINS = '1'; JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1' } {
  Invoke-Joopo update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:JoopoUpdateExit = $LASTEXITCODE
}
$updateExit = $script:JoopoUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-JoopoGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-Joopo gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-Joopo gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-Joopo gateway restart *>&1 | Out-Host
...
$sessionPath = Join-Path $env:USERPROFILE '.joopo\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $sessionsDir = Join-Path $env:USERPROFILE '.joopo\\agents\\main\\sessions'
  $sessionPath = Join-Path $sessionsDir "$sessionId.jsonl"
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-Joopo agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking minimal --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
if (-not $agentOk) { throw 'joopo agent finished without OK response' }`;
}
...
path = Path.home() / ".joopo" / "joopo.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_joopo_gateway_processes() {
  JOOPO_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/joopo gateway stop || true
  pkill -f 'joopo.*gateway' >/dev/null 2>&1 || true
}
start_joopo_gateway() {
  if /opt/homebrew/bin/joopo gateway restart; then
    return
  fi
  pkill -f 'joopo.*gateway' >/dev/null 2>&1 || true
  rm -f /tmp/joopo-parallels-macos-gateway.log
  nohup env JOOPO_HOME="$HOME" JOOPO_STATE_DIR="$HOME/.joopo" JOOPO_CONFIG_PATH="$HOME/.joopo/joopo.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} /opt/homebrew/bin/joopo gateway run --bind loopback --port 18789 --force >/tmp/joopo-parallels-macos-gateway.log 2>&1 </dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if /opt/homebrew/bin/joopo gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/joopo-parallels-macos-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_joopo_gateway_processes
JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 JOOPO_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/joopo update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("/opt/homebrew/bin/joopo", input.expectedNeedle)}
start_joopo_gateway
wait_for_gateway
/opt/homebrew/bin/joopo models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("/opt/homebrew/bin/joopo", input.auth.modelId, "macos")}
/opt/homebrew/bin/joopo config set agents.defaults.skipBootstrap true --strict-json
/opt/homebrew/bin/joopo config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("/opt/homebrew/bin/joopo", input, "parallels-npm-update-macos")}`;
}
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.joopo\\joopo.json'
...
function Stop-JoopoGatewayProcesses {
  Invoke-Joopo gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'joopo.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "joopo update failed with exit code $updateExit" }
  Write-Host "joopo update returned a stale post-swap module import; continuing to post-update health checks"
}
...
const configPath = path.join(process.env.HOME || "/root", ".joopo", "joopo.json");
...
stop_joopo_gateway_processes() {
  JOOPO_DISABLE_BUNDLED_PLUGINS=1 joopo gateway stop || true
  pkill -f 'joopo.*gateway' >/dev/null 2>&1 || true
}
start_joopo_gateway() {
  pkill -f "joopo gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/joopo-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env JOOPO_HOME=/root JOOPO_STATE_DIR=/root/.joopo JOOPO_CONFIG_PATH=/root/.joopo/joopo.json JOOPO_DISABLE_BONJOUR=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
    )} joopo gateway run --bind loopback --port 18789 --force >/tmp/joopo-parallels-linux-gateway.log 2>&1`,
  )} >/dev/null 2>&1 < /dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if joopo gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/joopo-parallels-linux-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_joopo_gateway_processes
JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 JOOPO_DISABLE_BUNDLED_PLUGINS=1 joopo update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("joopo", input.expectedNeedle)}
start_joopo_gateway
wait_for_gateway
joopo models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("joopo", input.auth.modelId, "linux")}
joopo config set agents.defaults.skipBootstrap true --strict-json
joopo config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("joopo", input, "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-Joopo --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "joopo --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
...
while ($true) {
  ...
  if ((Get-Date) -ge $versionDeadline) {
    if ($LASTEXITCODE -ne 0) { throw "joopo --version failed with exit code $LASTEXITCODE" }
    throw ${mismatch}
  }
  Start-Sleep -Seconds 2
}`;
