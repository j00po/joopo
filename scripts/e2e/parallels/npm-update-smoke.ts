...
const updateTimeoutSeconds = Number(process.env.JOOPO_PARALLELS_NPM_UPDATE_TIMEOUT_S || 1200);

function usage(): string {
  return `Usage: bash scripts/e2e/parallels-npm-update-smoke.sh [options]

Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: joopo@latest
  --update-target <target>    Target passed to guest 'joopo update --tag'.
...
class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("joopo-parallels-npm-update.");
    this.tgzDir = await makeTempDir("joopo-parallels-npm-update-tgz.");
    try {
      this.latestVersion = resolveLatestVersion();
      this.packageSpec = this.options.packageSpec || `joopo@${this.latestVersion}`;
      ...
      say(`Run same-guest joopo update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          JOOPO_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveJoopoRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("joopo@") ? target : `joopo@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      macosVm,
      script,
      "joopo-parallels-npm-update-macos",
    );
  ...
  private async guestLinux(
    ...
    const scriptPath = this.writeGuestScript(
      this.linuxVm,
      script,
      "joopo-parallels-npm-update-linux",
    );
    try {
      const status = await this.runStreamingToJobLog(
        "prlctl",
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveJoopoRegistryVersion(this.packageSpec);
    const target = resolveJoopoRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to joopo@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private configurePublishedTargets(): void {
    if (this.options.betaValidation) {
      const version = resolveJoopoRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `joopo@${version}`;
      say(`Beta validation target: joopo@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveJoopoRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveJoopoRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `joopo@${version}` : this.options.freshTargetSpec;
    }
  }
