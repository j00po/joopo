...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `joopo-parallels-${nonce}`;
...
if (Test-Path $logPath) {
  ...
  if ($bytes.Length -gt $offset) {
    "__JOOPO_LOG_OFFSET__:$($bytes.Length)"
...
if (Test-Path $donePath) {
  ...
  "__JOOPO_BACKGROUND_EXIT__:$backgroundExit"
  '__JOOPO_BACKGROUND_DONE__'
  ...
  exit 0
    ...
    const offsetMatch = poll.stdout.match(/__JOOPO_LOG_OFFSET__:(\d+)/);
    if (offsetMatch) {
      lastLogOffset = Number(offsetMatch[1]);
    }
    if (poll.stdout.includes("__JOOPO_BACKGROUND_DONE__")) {
      const exitMatch = poll.stdout.match(/__JOOPO_BACKGROUND_EXIT__:(\S+)/);
...
export class LinuxGuest {
  ...
  bash(script: string): string {
    const scriptPath = `/tmp/joopo-parallels-${process.pid}-${Date.now()}.sh`;
    const write = run(
      "prlctl",
...
export class WindowsGuest {
  ...
  powershell(script: string, options: GuestExecOptions = {}): string {
    const scriptName = `joopo-parallels-${process.pid}-${Date.now()}.ps1`;
