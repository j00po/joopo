...
const packageName = "@joopo/kitchen-sink";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "joopo-joopohub-fixture-"));
  try {
    ...
    api.registerChannel({
      plugin: {
        ...
        meta: {
          ...
          blurb: "Kitchen sink JoopoHub fixture channel",
        },
...
`,
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          name: packageName,
          displayName: "Joopo Kitchen Sink",
          family: "code-plugin",
          runtimeId: pluginId,
          channel: "official",
          isOfficial: true,
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/joopo/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
if (!fixture || !portFile) {
  console.error("usage: joopohub-fixture-server.cjs <kitchen-sink-plugin|plugins> <port-file>");
  process.exit(1);
}
...
async function main() {
  ...
  zip.file("package/joopo.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "Joopo Kitchen Sink",
