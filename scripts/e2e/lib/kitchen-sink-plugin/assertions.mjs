...
function scanLogs() {
  const roots = ["/tmp", path.join(process.env.HOME, ".joopo")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /joopo-kitchen-sink-/u.test(path.basename(entry))) {
...
function assertJoopoHubExternalInstallContract(installPath) {
  ...
  if (!fs.lstatSync(joopoPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink joopo peer is not a symlink: ${joopoPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(joopoPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink joopo peer ${linkedHostRoot} to target ${hostRoot}`);
  }
