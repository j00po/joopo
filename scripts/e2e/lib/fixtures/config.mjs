...
function writeConfig(kind) {
  const configPath = requireArg(process.env.JOOPO_CONFIG_PATH, "JOOPO_CONFIG_PATH");
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.JOOPO_GATEWAY_TOKEN, "JOOPO_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.JOOPO_STATE_DIR, "joopo.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.JOOPO_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(
    process.env.JOOPO_CONFIG_BATCH_PATH,
    "JOOPO_CONFIG_BATCH_PATH",
  );
  writeJson(batchPath, [
    ...
    { path: "gateway.auth.token", value: process.env.JOOPO_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.JOOPO_OPENWEBUI_MODEL },
  ]);
}
