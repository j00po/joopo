...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.JOOPO_WORKSPACE_DIR || path.join(process.env.HOME, ".joopo", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.JOOPO_STATE_DIR, "JOOPO_STATE_DIR");
  ...
  writeJson(path.join(stateDir, "joopo.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.JOOPO_STATE_DIR, "joopo.json"))?.agents?.list ?? [];
