...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write joopo.json");
  }
  ...
  if (authRaw.includes("sk-joopo-npm-onboard-e2e")) {
...
function assertAgentTurn() {
  ...
  const output = fs.readFileSync("/tmp/joopo-agent.combined", "utf8");
