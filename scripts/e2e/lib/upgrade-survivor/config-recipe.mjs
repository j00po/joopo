    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/joopo-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.JOOPO_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
function runJoopo(step) {
  const result = spawnSync("joopo", step.argv, {
    encoding: "utf8",
    env: process.env,
  });
  return {
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runJoopo(adaptedStep);
