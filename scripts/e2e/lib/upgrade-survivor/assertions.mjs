...
function getScenario() {
  const scenario = process.env.JOOPO_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("JOOPO_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.JOOPO_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  const stateDir = requireEnv("JOOPO_STATE_DIR");
  const workspace = requireEnv("JOOPO_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".joopo", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".joopo-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.JOOPO_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `joopo-${version}-${plugin}`,
          ".joopo-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/joopo-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const stage = process.env.JOOPO_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-joopo");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`joopo-${version}-`))
      : [];
    assert(
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const npmRoot = path.join(requireEnv("JOOPO_STATE_DIR"), "npm", "node_modules");
  assert(
...
function assertConfiguredPluginInstalls() {
  ...
  assertExternalPluginInstall(records, "brave", "@joopo/brave-plugin");
