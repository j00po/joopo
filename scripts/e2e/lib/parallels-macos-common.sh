...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/joopo-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/joopo-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${JOOPO_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${JOOPO_SECRET_ENV_NAME:-}" ] && [ -n "${JOOPO_SECRET_FILE:-}" ] && [ -f "$JOOPO_SECRET_FILE" ]; then
  secret_value="$(cat "$JOOPO_SECRET_FILE")"
  rm -f "$JOOPO_SECRET_FILE"
  export "${JOOPO_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "JOOPO_SECRET_ENV_NAME=$api_key_env" \
      "JOOPO_SECRET_FILE=$secret_path" \
      "JOOPO_WRAPPER_FILE=$wrapper_path" \
