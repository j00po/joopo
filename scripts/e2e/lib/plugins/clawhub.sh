run_plugins_clawhub_scenario() {
  if [ "${JOOPO_PLUGINS_E2E_CLAWHUB:-1}" = "0" ]; then
    echo "Skipping ClawHub plugin install and uninstall (JOOPO_PLUGINS_E2E_CLAWHUB=0)."
  else
    echo "Testing ClawHub plugin install and uninstall..."
    CLAWHUB_PLUGIN_SPEC="${JOOPO_PLUGINS_E2E_CLAWHUB_SPEC:-clawhub:@joopo/kitchen-sink}"
    CLAWHUB_PLUGIN_ID="${JOOPO_PLUGINS_E2E_CLAWHUB_ID:-joopo-kitchen-sink-fixture}"
    ...
    if [[ "${JOOPO_PLUGINS_E2E_LIVE_CLAWHUB:-0}" = "1" ]]; then
      ...
      if [[ -n "${JOOPO_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]; then
        echo "Ignoring ambient ClawHub URL for fixture-mode plugin E2E; set JOOPO_PLUGINS_E2E_LIVE_CLAWHUB=1 for live ClawHub."
      fi
      unset JOOPO_CLAWHUB_URL CLAWHUB_URL
      clawhub_fixture_dir="$(mktemp -d "/tmp/joopo-clawhub-fixture.XXXXXX")"
      start_clawhub_fixture_server "$clawhub_fixture_dir"
    fi

    node scripts/e2e/lib/plugins/assertions.mjs clawhub-preflight

    run_logged install-clawhub node "$JOOPO_ENTRY" plugins install "$CLAWHUB_PLUGIN_SPEC"
    node "$JOOPO_ENTRY" plugins list --json >/tmp/plugins-clawhub-installed.json
    node "$JOOPO_ENTRY" plugins inspect "$CLAWHUB_PLUGIN_ID" --json >/tmp/plugins-clawhub-inspect.json

    node scripts/e2e/lib/plugins/assertions.mjs clawhub-installed

    node "$JOOPO_ENTRY" plugins update "$CLAWHUB_PLUGIN_ID" >/tmp/plugins-clawhub-update.log 2>&1
    node "$JOOPO_ENTRY" plugins list --json >/tmp/plugins-clawhub-updated.json
    node "$JOOPO_ENTRY" plugins inspect "$CLAWHUB_PLUGIN_ID" --json >/tmp/plugins-clawhub-updated-inspect.json

    node scripts/e2e/lib/plugins/assertions.mjs clawhub-updated

    run_logged uninstall-clawhub node "$JOOPO_ENTRY" plugins uninstall "$CLAWHUB_PLUGIN_SPEC" --force
    node "$JOOPO_ENTRY" plugins list --json >/tmp/plugins-clawhub-uninstalled.json
