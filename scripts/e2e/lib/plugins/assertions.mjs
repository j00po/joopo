...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.JOOPO_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".joopo", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Joopo plugin install/update/uninstall commands. Use `joopo plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".joopo", "git");
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@joopo/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@joopo/demo-plugin-npm") {
