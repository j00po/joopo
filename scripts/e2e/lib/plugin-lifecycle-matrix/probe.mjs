...
function joopoPath(...parts) {
  return path.join(home, ".joopo", ...parts);
}
...
function records() {
  const index = readJson(joopoPath("plugins", "installs.json"));
...
function config() {
  return readJson(process.env.JOOPO_CONFIG_PATH ?? joopoPath("joopo.json"));
}
