...
joopo_e2e_install_package /tmp/joopo-plugin-lifecycle-install.log "mounted Joopo package" /tmp/npm-prefix
...
pack_root="$(mktemp -d "/tmp/joopo-plugin-lifecycle-pack.XXXXXX")"
registry_root="$(mktemp -d "/tmp/joopo-plugin-lifecycle-registry.XXXXXX")"
