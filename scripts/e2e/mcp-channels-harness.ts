...
// against the Joopo npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/joopo-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      JOOPO_STATE_DIR: "/tmp/joopo-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[joopo mcp] ${String(chunk)}`);
  });
