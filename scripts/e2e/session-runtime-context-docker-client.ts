...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".joopo", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "joopo.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("JOOPO_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-session-runtime-context-"));
  process.env.HOME = root;
  process.env.JOOPO_STATE_DIR = path.join(root, ".joopo");
  process.env.JOOPO_CONFIG_PATH = path.join(process.env.JOOPO_STATE_DIR, "joopo.json");
  try {
    ...
    if (process.env.JOOPO_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
