...
EXTRA_MOUNTS="${JOOPO_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${JOOPO_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${JOOPO_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${JOOPO_DOCKER_SOCKET:-}"
TIMEZONE="${JOOPO_TZ:-}"
RAW_SKIP_ONBOARDING="${JOOPO_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
...
read_config_gateway_token() {
  local config_path="$JOOPO_CONFIG_DIR/joopo.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == JOOPO_GATEWAY_TOKEN=* ]]; then
      token="${line#JOOPO_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${JOOPO_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$JOOPO_GATEWAY_PORT" "$JOOPO_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$JOOPO_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$JOOPO_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network joopo-cli service because it
  ...
  run_prestart_gateway --entrypoint node joopo-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" joopo-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "JOOPO_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "JOOPO_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
JOOPO_CONFIG_DIR="${JOOPO_CONFIG_DIR:-$HOME/.joopo}"
JOOPO_WORKSPACE_DIR="${JOOPO_WORKSPACE_DIR:-$HOME/.joopo/workspace}"

validate_mount_path_value "JOOPO_CONFIG_DIR" "$JOOPO_CONFIG_DIR"
validate_mount_path_value "JOOPO_WORKSPACE_DIR" "$JOOPO_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "JOOPO_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "JOOPO_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "JOOPO_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "JOOPO_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "JOOPO_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "JOOPO_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$JOOPO_CONFIG_DIR"
mkdir -p "$JOOPO_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$JOOPO_CONFIG_DIR/identity"
mkdir -p "$JOOPO_CONFIG_DIR/agents/main/agent"
mkdir -p "$JOOPO_CONFIG_DIR/agents/main/sessions"

export JOOPO_CONFIG_DIR
export JOOPO_WORKSPACE_DIR
export JOOPO_GATEWAY_PORT="${JOOPO_GATEWAY_PORT:-18789}"
export JOOPO_BRIDGE_PORT="${JOOPO_BRIDGE_PORT:-18790}"
export JOOPO_GATEWAY_BIND="${JOOPO_GATEWAY_BIND:-lan}"
export JOOPO_DISABLE_BONJOUR="${JOOPO_DISABLE_BONJOUR:-}"
export JOOPO_IMAGE="$IMAGE_NAME"
export JOOPO_DOCKER_APT_PACKAGES="${JOOPO_DOCKER_APT_PACKAGES:-}"
export JOOPO_EXTENSIONS="${JOOPO_EXTENSIONS:-}"
export JOOPO_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export JOOPO_HOME_VOLUME="$HOME_VOLUME_NAME"
export JOOPO_ALLOW_INSECURE_PRIVATE_WS="${JOOPO_ALLOW_INSECURE_PRIVATE_WS:-}"
export JOOPO_SANDBOX="$SANDBOX_ENABLED"
export JOOPO_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export JOOPO_DOCKER_SETUP=1
export JOOPO_TZ="$TIMEZONE"
...
export JOOPO_OTEL_PRELOADED="${JOOPO_OTEL_PRELOADED:-}"
export JOOPO_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${JOOPO_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    JOOPO_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $JOOPO_CONFIG_DIR/joopo.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      JOOPO_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      JOOPO_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      JOOPO_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${JOOPO_CONFIG_DIR}:/home/node/.joopo"
    gateway_workspace_mount="${JOOPO_WORKSPACE_DIR}:/home/node/.joopo/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${JOOPO_INSTALL_DOCKER_CLI:-}" ]]; then
    export JOOPO_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  JOOPO_BRIDGE_PORT \
  JOOPO_GATEWAY_BIND \
  JOOPO_DISABLE_BONJOUR \
  JOOPO_GATEWAY_TOKEN \
  JOOPO_IMAGE \
  JOOPO_EXTRA_MOUNTS \
  JOOPO_HOME_VOLUME \
  JOOPO_DOCKER_APT_PACKAGES \
  JOOPO_EXTENSIONS \
  JOOPO_SANDBOX \
  JOOPO_DOCKER_SOCKET \
  DOCKER_GID \
  JOOPO_INSTALL_DOCKER_CLI \
  ...
  JOOPO_OTEL_PRELOADED \
  JOOPO_SKIP_ONBOARDING

if [[ "$IMAGE_NAME" == "joopo:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "JOOPO_DOCKER_APT_PACKAGES=${JOOPO_DOCKER_APT_PACKAGES}" \
    --build-arg "JOOPO_EXTENSIONS=${JOOPO_EXTENSIONS}" \
    --build-arg "JOOPO_INSTALL_DOCKER_CLI=${JOOPO_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Joopo metadata subdirectory
# (.joopo/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh joopo-gateway -c \
  'find /home/node/.joopo -xdev -exec chown node:node {} +; \
   [ -d /home/node/.joopo/workspace/.joopo ] && chown -R node:node /home/node/.joopo/workspace/.joopo || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (JOOPO_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from JOOPO_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $JOOPO_GATEWAY_BIND"
  if is_truthy_value "$JOOPO_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (JOOPO_DISABLE_BONJOUR=$JOOPO_DISABLE_BONJOUR)."
  elif [[ -z "$JOOPO_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (JOOPO_DISABLE_BONJOUR=$JOOPO_DISABLE_BONJOUR)."
  fi
  echo "Gateway token: $JOOPO_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm joopo-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm joopo-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm joopo-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.joopo.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d joopo-gateway

# --- Sandbox setup (opt-in via JOOPO_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$sandbox_dockerfile" ]]; then
    echo "Building sandbox image: joopo-sandbox:bookworm-slim"
    run_docker_build \
      -t "joopo-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker joopo-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg JOOPO_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (JOOPO_IMAGE=joopo:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: JOOPO_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Joopo config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.joopo.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate joopo-gateway
  fi
else
...
echo "Config: $JOOPO_CONFIG_DIR"
echo "Workspace: $JOOPO_WORKSPACE_DIR"
echo "Token: $JOOPO_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f joopo-gateway"
echo "  ${COMPOSE_HINT} exec joopo-gateway node dist/index.js health --token \"$JOOPO_GATEWAY_TOKEN\""
