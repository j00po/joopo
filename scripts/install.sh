...
# Joopo Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Joopo."
...
GUM_VERSION="${JOOPO_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://joopo.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$JOOPO_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.joopo.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_joopo_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.joopo-* "$npm_root"/joopo 2>/dev/null || true
}

extract_joopo_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_joopo_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/joopo" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/joopo"|"/usr/local/bin/joopo")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/joopo/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale joopo symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing joopo binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Joopo package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_joopo_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Joopo npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*joopo" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_joopo_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_joopo_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Joopo npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an joopo binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${JOOPO_TAGLINE_INDEX:-}" ]]; then
        if [[ "${JOOPO_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((JOOPO_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${JOOPO_NO_ONBOARD:-0}
NO_PROMPT=${JOOPO_NO_PROMPT:-0}
DRY_RUN=${JOOPO_DRY_RUN:-0}
INSTALL_METHOD=${JOOPO_INSTALL_METHOD:-}
JOOPO_VERSION=${JOOPO_VERSION:-latest}
USE_BETA=${JOOPO_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/joopo"
GIT_DIR=${JOOPO_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${JOOPO_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${JOOPO_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${JOOPO_VERBOSE:-0}"
VERIFY_INSTALL="${JOOPO_VERIFY_INSTALL:-0}"
JOOPO_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Joopo installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/joopo)
...
Environment variables:
  JOOPO_INSTALL_METHOD=git|npm
  JOOPO_VERSION=latest|next|main|<semver>|<spec>
  JOOPO_BETA=0|1
  JOOPO_GIT_DIR=...
  JOOPO_GIT_UPDATE=0|1
  JOOPO_NO_PROMPT=1
  JOOPO_VERIFY_INSTALL=1
  JOOPO_DRY_RUN=1
  JOOPO_NO_ONBOARD=1
  JOOPO_VERBOSE=1
  JOOPO_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Joopo checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Joopo source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_joopo_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"joopo"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://joopo.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Joopo updates; use npm i -g joopo@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_joopo_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/joopo" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/joopo" ]]; then
        ln -sf "$npm_root/joopo/dist/entry.js" "${npm_bin}/joopo"
        ui_info "Created joopo bin link at ${npm_bin}/joopo"
    fi
    return 0
}

# Check for existing Joopo installation
check_existing_joopo() {
    if [[ -n "$(type -P joopo 2>/dev/null || true)" ]]; then
        ui_info "Existing Joopo installation detected, upgrading"
        return 0
    fi
    return 1
}
...
collect_joopo_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${JOOPO_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_joopo_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/joopo"
        ...
        local version=""
        version="$(joopo_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_joopo_npm_root_candidates)
}

warn_duplicate_joopo_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_joopo_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Joopo global installs detected"
    echo "  Different Node/npm environments can run different Joopo versions."

    local active_node active_npm active_joopo
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_joopo="${JOOPO_BIN:-}"
    if [[ -z "$active_joopo" ]]; then
        active_joopo="$(type -P joopo 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active joopo: ${INFO}${active_joopo:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make joopo show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

joopo_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "joopo"
        return 0
...
warn_joopo_not_found() {
    ui_warn "Installed, but joopo is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t joopo 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named joopo; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_joopo_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P joopo 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/joopo" ]]; then
        echo "${npm_bin}/joopo"
        return 0
    fi
...
install_joopo_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/joopo/joopo.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Joopo from git checkout: ${repo_dir}"
    else
        ui_info "Installing Joopo from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Joopo" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Joopo" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/joopo" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/joopo"
    ui_success "Joopo wrapper installed to \$HOME/.local/bin/joopo"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view joopo dist-tags.beta 2>/dev/null || true)"
...
install_joopo() {
    local package_name="joopo"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            JOOPO_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="joopo"
        else
            JOOPO_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${JOOPO_VERSION}" ]]; then
        JOOPO_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${JOOPO_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${JOOPO_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Joopo v${resolved_version}"
    else
        ui_info "Installing Joopo (${JOOPO_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${JOOPO_VERSION}")"

    if ! install_joopo_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_joopo_paths
        install_joopo_npm "${install_spec}"
    fi

    if [[ "${JOOPO_VERSION}" == "latest" && "${package_name}" == "joopo" ]]; then
        if ! resolve_joopo_bin &> /dev/null; then
            ui_warn "npm install joopo@latest failed; retrying joopo@next"
            cleanup_npm_joopo_paths
            install_joopo_npm "joopo@next"
        fi
    fi

    ensure_joopo_bin_link || true

    ui_success "Joopo installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${JOOPO_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_joopo_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (joopo not on PATH yet)"
        warn_joopo_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${JOOPO_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.joopo/workspace-${profile}"
    else
        echo "${HOME}/.joopo/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        local user_claw
        user_claw="$(joopo_command_for_user "${JOOPO_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but joopo not on PATH; skipping onboarding"
        warn_joopo_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(joopo_command_for_user "$claw")"
...
if ! declare -F extract_joopo_semver >/dev/null 2>&1; then
...
resolve_joopo_version() {
    ...
    local claw="${JOOPO_BIN:-}"
    if [[ -z "$claw" ]] && command -v joopo &> /dev/null; then
        claw="$(command -v joopo)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_joopo_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/joopo/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/joopo/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: joopo not on PATH yet"
        warn_joopo_not_found
        return 1
    fi

    run_quiet_step "Checking Joopo version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: joopo gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_joopo_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Joopo checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set JOOPO_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_joopo_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/joopo" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/joopo"
    ...
    JOOPO_BIN="$(resolve_joopo_bin || true)"
    warn_duplicate_joopo_global_installs || true
    ...
    installed_version=$(resolve_joopo_version)
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/joopo"
        ui_kv "Update command" "joopo update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running joopo doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                JOOPO_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                JOOPO_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                JOOPO_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (joopo not on PATH yet)"
                    warn_joopo_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v joopo &> /dev/null; then
        local claw="${JOOPO_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_joopo_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (joopo daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if JOOPO_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: joopo daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${JOOPO_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
