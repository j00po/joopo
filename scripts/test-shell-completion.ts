 ...
 * `joopo update`. Use it to verify the completion installation flow
...
\`joopo update\`, \`joopo doctor\`, and \`joopo onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Joopo Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`joopo completion --install\` later to enable.`));
    return;
  }
