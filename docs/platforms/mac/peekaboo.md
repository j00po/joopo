...
read_when:
  - Hosting PeekabooBridge in Joopo.app
...
Joopo can host **PeekabooBridge** as a local, permission-aware UI automation
broker. This lets the `peekaboo` CLI drive UI automation while reusing the
macOS app's TCC permissions.

## What this is (and is not)

- **Host**: Joopo.app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate `joopo ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Joopo is a thin broker host.

## Relationship to Computer Use

Joopo has three desktop-control paths, and they intentionally stay separate:

- **PeekabooBridge host**: Joopo.app can host the local PeekabooBridge socket.
  The `peekaboo` CLI remains the client and uses Joopo.app's macOS
...
- **Codex Computer Use**: the bundled `codex` plugin prepares Codex app-server,
  ...
  Codex own native desktop-control tool calls during Codex-mode turns. Joopo
  does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Joopo can register TryCua's upstream
...
Use Peekaboo when you want the broad macOS automation surface and Joopo.app's
...
when you want the CUA driver exposed to any Joopo-managed runtime as a normal
MCP server.
...
When enabled, Joopo starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Joopo app TeamID).
...
- If `peekaboo` reports "bridge client is not authorized", ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or Joopo.app)
