...
service via the external `joopo` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.joopo.gateway`
  (or `ai.joopo.<profile>` when using `--profile`/`JOOPO_PROFILE`; legacy `com.joopo.*` is supported).
