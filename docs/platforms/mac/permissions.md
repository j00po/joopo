...
- Same path: run the app from a fixed location (for Joopo, `dist/Joopo.app`).
...
```bash
sudo tccutil reset Accessibility ai.joopo.mac
sudo tccutil reset ScreenCapture ai.joopo.mac
...
Workaround: move files into the Joopo workspace (`~/.joopo/workspace`) if you want to avoid per-folder grants.
