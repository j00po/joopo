...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `ai.joopo`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.joopo" AND category CONTAINS "voicewake"' --level info --style compact
  ```
