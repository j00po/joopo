...
read_when:
  - Installing Joopo on Windows
  - Choosing between native Windows and WSL2
  - Looking for Windows companion app status
title: "Windows"
---

Joopo supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `joopo --version`, `joopo doctor`, and `joopo plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
joopo agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `joopo onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `joopo onboard --non-interactive --install-daemon` and `joopo gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, Joopo falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, Joopo now aborts that path quickly and falls back instead of hanging forever
- Scheduled Tasks are still preferred when available because they provide better supervisor status

If you want the native CLI only, without gateway service install, use one of these:

```powershell
joopo onboard --non-interactive --skip-health
joopo gateway run
```
...
### 2) Install the Joopo gateway user service
...
```bash
systemctl --user is-enabled joopo-gateway.service
systemctl --user status joopo-gateway.service --no-pager
```
...
pnpm build
pnpm ui:build
pnpm joopo onboard --install-daemon
```
