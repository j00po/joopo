...
2. `npm i -g joopo@latest`
3. `joopo onboard --install-daemon`
...
Joopo installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `joopo gateway install` and
`joopo onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/joopo-gateway[-<profile>].service`:

```
[Unit]
Description=Joopo Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Joopo therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Joopo starts the child through a short
...
- Joopo-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `JOOPO_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
