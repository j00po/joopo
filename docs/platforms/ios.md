...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `joopo.internal.`), **or**
...
Run `joopo devices list` again before approval.
...
```bash
joopo nodes status
joopo gateway call node.list --params "{}"
```
...
- `JOOPO_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Joopo iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Joopo builds.
...
```bash
export JOOPO_APNS_TEAM_ID="TEAMID"
export JOOPO_APNS_KEY_ID="KEYID"
export JOOPO_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.joopo/credentials/apns
chmod 700 ~/.joopo/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.joopo/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.joopo/credentials/apns/AuthKey_KEYID.p8
export JOOPO_APNS_PRIVATE_KEY_PATH="$HOME/.joopo/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_joopo-gw._tcp` on `local.` and, when configured, the same
...
`joopo.internal.`) and Tailscale split DNS.
...
```bash
joopo nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__joopo__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__joopo__/canvas/` and `/__joopo__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Joopo node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Joopo by invoking node
...
```bash
joopo nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__joopo; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
joopo nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `joopo devices list` and approve manually.
