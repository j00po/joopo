---
summary: "Run Joopo on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Joopo
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Joopo Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Joopo on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Joopo bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/joopo-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `JOOPO_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `JOOPO_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/joopo-compile-cache`
...
For the standard `joopo onboard --install-daemon` path, edit the user unit:
...
`joopo-gateway.service` via `sudo systemctl edit joopo-gateway.service`.
