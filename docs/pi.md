---
summary: "Architecture of Joopo's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in Joopo
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

Joopo integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Joopo uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Joopo directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Joopo's local TUI mode)                                             |
...
├── pi-tools.ts                    # createJoopoCodingTools()
...
├── joopo-tools.ts              # Joopo-specific tools
...
Image injection is prompt-local: Joopo loads image refs from the current prompt and
...
2. **Custom Replacements**: Joopo replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Joopo Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Joopo's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System prompt construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Joopo CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Joopo wraps this with `guardSessionManager()` for tool result safety.
...
Joopo maintains an auth profile store with multiple API keys per provider:
...
Joopo loads custom pi extensions for specialized behavior:
...
Joopo also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Joopo Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Joopo tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.joopo/agents/<agentId>/sessions/` (or `$JOOPO_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `JOOPO_LIVE_TEST=1`)
