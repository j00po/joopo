---
summary: "Run Joopo with LM Studio"
read_when:
  - You want to run Joopo with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive Joopo setup.
...
```bash
joopo models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). Joopo
...
For unauthenticated LM Studio servers, omit the key; Joopo stores a local non-secret marker.
...
`usage` object, Joopo recovers token counts from llama.cpp-style
...
options, Joopo exposes the matching OpenAI-compatible `reasoning_effort`
...
on `/v1/chat/completions`; Joopo normalizes that binary discovery shape to
...
LM Studio supports just-in-time (JIT) model loading, where models are loaded on first request. Joopo preloads models through LM Studio's native load endpoint by default, which helps when JIT is disabled. To let LM Studio's JIT, idle TTL, and auto-evict behavior own model lifecycle, disable Joopo's preload step:
