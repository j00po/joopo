---
summary: "fal image and video generation setup in Joopo"
title: "Fal"
read_when:
  - You want to use fal image generation in Joopo
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate or video_generate
---

Joopo ships a bundled `fal` provider for hosted image and video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    joopo onboard --auth-choice fal-api-key
    ```
  </Step>
...
explicit transparent-background control in Joopo, so `background:
...
<Tip>
Use `joopo models list --provider fal` to see the full list of available fal
