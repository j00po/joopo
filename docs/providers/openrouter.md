---
summary: "Use OpenRouter's unified API to access many models in Joopo"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Joopo
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    joopo onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    joopo models set openrouter/<provider>/<model>
    ```
...
Joopo submits text-to-video and image-to-video jobs to OpenRouter, polls
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), Joopo also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------ |
| `HTTP-Referer`            | `https://joopo.ai`                                                                                  |
| `X-OpenRouter-Title`      | `Joopo`                                                                                             |
| `X-OpenRouter-Categories` | `cli-agent,cloud-agent,programming-app,creative-writing,writing-assistant,general-chat,personal-agent` |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Joopo
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Joopo sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Joopo uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Joopo maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. Joopo sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Joopo keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, Joopo forwards
