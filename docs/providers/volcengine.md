...
read_when:
  - You want to use Volcano Engine or Doubao models with Joopo
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    joopo onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    joopo models list --provider volcengine
    joopo models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, Joopo asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `joopo onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Joopo falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.joopo/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Joopo as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Joopo setup.
  </Card>
</CardGroup>
