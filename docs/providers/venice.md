---
summary: "Use Venice AI privacy-focused models in Joopo"
read_when:
  - You want privacy-focused inference in Joopo
...
<Steps>
  ...
  <Step title="Configure Joopo">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        joopo onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    joopo agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Joopo shows all available Venice models. Pick based on your needs:
...
```bash
joopo models set venice/kimi-k2-5
joopo models set venice/claude-opus-4-6
```

List all available models:

```bash
joopo models list --all --provider venice
```

You can also run `joopo configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Joopo fills the required DeepSeek V4
...
Joopo keeps that provider-specific replay fix separate from the native
...
Joopo ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
joopo agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
joopo agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
joopo agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
joopo agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
joopo agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `joopo models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
