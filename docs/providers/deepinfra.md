---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in Joopo"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in Joopo
title: "DeepInfra"
---
...
```bash
joopo onboard --deepinfra-api-key <key>
```
...
## Supported Joopo surfaces

The bundled plugin registers all DeepInfra surfaces that match current
Joopo provider contracts:

| Surface                  | Default model                      | Joopo config/tool                                     |
| ------------------------ | ---------------------------------- | -------------------------------------------------------- |
...
native model types. Joopo does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

Joopo dynamically discovers available DeepInfra models at startup. Use
