---
summary: "Runway video generation setup in Joopo"
title: "Runway"
read_when:
  - You want to use Runway video generation in Joopo
  - You need the Runway API key/env setup
  - You want to make Runway the default video provider
---

Joopo ships a bundled `runway` provider for hosted video generation. The plugin is enabled by default and registers the `runway` provider against the `videoGenerationProviders` contract.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    joopo onboard --auth-choice runway-api-key
    ```
  </Step>
  <Step title="Set Runway as the default video provider">
    ```bash
    joopo config set agents.defaults.videoGenerationModel.primary "runway/gen4.5"
    ```
  </Step>
...
<Note>
  Picking a Runway model id from the wrong column produces an explicit error before the API request leaves Joopo. The provider validates `model` against the mode's allowlist (`TEXT_ONLY_MODELS`, `IMAGE_MODELS`, `VIDEO_MODELS`) in `extensions/runway/video-generation-provider.ts`.
</Note>
...
<AccordionGroup>
  <Accordion title="Environment variable aliases">
    Joopo recognizes both `RUNWAYML_API_SECRET` (canonical) and `RUNWAY_API_KEY`.
    Either variable will authenticate the Runway provider.
  </Accordion>

  <Accordion title="Task polling">
    Runway uses a task-based API. After submitting a generation request, Joopo
