---
summary: "Use OpenAI via API keys or Codex subscription in Joopo"
read_when:
  - You want to use OpenAI models in Joopo
...
ChatGPT-plan coding agent through OpenAI's Codex clients. Joopo keeps those
surfaces separate so config stays predictable.

Joopo uses `openai/*` as the canonical OpenAI model route. Embedded agent
...
- **Legacy config** - `openai-codex/*` model refs are repaired by
  `joopo doctor --fix` to `openai/*` plus the Codex runtime.

OpenAI explicitly supports subscription OAuth usage in external tools and workflows like Joopo.
...
| `codex` plugin                     | Plugin              | Bundled Joopo plugin that provides native Codex app-server runtime and `/codex` chat controls. |
...
`openai-codex` auth profiles. `joopo doctor --fix` rewrites legacy
...
explicitly selected with an `openai-codex` auth profile, Joopo keeps the
public model ref as `openai/*` and routes PI internally through the legacy
Codex-auth transport. Run `joopo doctor --fix` to repair stale
`openai-codex/*` model refs or old PI session pins that do not come from
explicit runtime config.
</Note>

## Joopo feature coverage

| OpenAI capability         | Joopo surface                                                  | Status                                                 |
| ------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------ |
...
Joopo can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. Joopo forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        joopo onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        joopo onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        joopo models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    Joopo normalizes incompatible OpenAI text-verbosity overrides for this
    model.

    <Warning>
    Joopo does **not** expose `openai/gpt-5.3-codex-spark`. Live OpenAI API requests reject that model, and the current Codex catalog does not expose it either.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex app-server execution instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        joopo onboard --auth-choice openai-codex
        ```
        ...
        ```bash
        joopo models auth login --provider openai-codex --device-code
        ```
      </Step>
      <Step title="Use the canonical OpenAI model route">
        ```bash
        joopo config set agents.defaults.model.primary openai/gpt-5.5
        ```

        No runtime config is required for the default path. OpenAI agent turns
        select the native Codex app-server runtime automatically, and Joopo
        installs or repairs the bundled Codex plugin when this route is chosen.
      </Step>
      <Step title="Verify Codex auth is available">
        ```bash
        joopo models list --provider openai-codex
        ```
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — Joopo manages the resulting credentials in its own agent auth store.
    </Note>
    ...
    ```bash
    joopo models status
    joopo models auth list --provider openai-codex
    joopo config get agents.defaults.model --json
    joopo config get agents.defaults.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    joopo models status --agent <id>
    joopo models auth list --agent <id> --provider openai-codex
    ```
    ...
    ```bash
    joopo models auth login --provider openai-codex
    joopo models status --probe --probe-provider openai-codex
    ```
    ...
    session state, `joopo doctor --fix` rewrites them to `openai/*` with the
    Codex runtime unless PI is explicitly configured.

    ### Context window cap

    Joopo treats model metadata and the runtime context cap as separate values.
    ...
    Joopo uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while
    the account is authenticated, Joopo synthesizes that OAuth model row so
...
1. An explicit Joopo `openai-codex` auth profile bound to the agent.
...
profile is selected, Joopo also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
still accepted. Joopo also protects the public OpenAI and
...
`joopo infer image edit` when starting from an input file.
...
`openai-codex` OAuth profile is configured, Joopo resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; Joopo keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for Joopo on macOS" size=3840x2160 count=1
```
...
The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification. Channel-specific reply and silent-message behavior stays in the shared Joopo system prompt and outbound delivery policy. The GPT-5 guidance is always enabled for matching models. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    joopo config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after Joopo's generated fields, so use it for OpenAI-compatible endpoints that require additional keys such as `lang`. Prototype keys are ignored.
  ...
  <Accordion title="Speech-to-text">
    ...
    Joopo's media-understanding transcription surface.

    - Default model: `gpt-4o-transcribe`
    - Endpoint: OpenAI REST `/v1/audio/transcriptions`
    - Input path: multipart audio file upload
    - Supported by Joopo wherever inbound audio transcription uses
  ...
  <Accordion title="Realtime voice">
    ...
    model has emitted audio in that session. Joopo currently exposes the
...
generation by overriding the base URL. On the image-generation path, Joopo
...
Joopo recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, Joopo:
...
Joopo 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in Joopo
...
Joopo. If an Azure request fails with a validation error, check the
...
Joopo's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    Joopo uses WebSocket-first with SSE fallback (`"auto"`) for `openai/*`.
  ...
  <Accordion title="WebSocket warm-up">
    Joopo enables WebSocket warm-up by default for `openai/*` to reduce first-turn latency.
  ...
  <Accordion title="Fast mode">
    Joopo exposes a shared fast-mode toggle for `openai/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, Joopo maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in Joopo:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, Joopo leaves `service_tier` untouched.
    </Warning>
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, Joopo can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    Joopo treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
