...
Deepgram is a speech-to-text API. In Joopo it is used for inbound
...
For batch transcription, Joopo uploads the complete audio file to Deepgram
and injects the transcript into the reply pipeline (`{{Transcript}}` +
`[Audio]` block). For Voice Call streaming, Joopo forwards live G.711
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Joopo transcribes it
