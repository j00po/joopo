...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    joopo config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
...
<Tabs>
  <Tab title="Native Perplexity API (pplx-)">
    When your key starts with `pplx-`, Joopo uses the native Perplexity Search
  ...
  <Tab title="OpenRouter / Sonar (sk-or-)">
    When your key starts with `sk-or-`, Joopo routes through OpenRouter using
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    Joopo will detect the prefix and switch to the Sonar transport
    automatically.
