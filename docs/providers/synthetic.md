---
summary: "Use Synthetic's Anthropic-compatible API in Joopo"
read_when:
...
Joopo registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    joopo onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Joopo's Anthropic client appends `/v1` to the base URL automatically, so use
...
`joopo models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Joopo appends `/v1` automatically.
