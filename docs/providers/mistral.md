---
summary: "Use Mistral models and Voxtral transcription with Joopo"
read_when:
  - You want to use Mistral models in Joopo
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Joopo includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    joopo onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    joopo onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    joopo models list --provider mistral
    ```
  </Step>
</Steps>
...
Joopo currently ships this bundled Mistral catalog:
...
<Note>
Joopo defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    Joopo maps the session **thinking** level to Mistral's API:

    | Joopo thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
