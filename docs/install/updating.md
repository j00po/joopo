---
summary: "Updating Joopo safely (global install or source), plus rollback strategy"
read_when:
  - Updating Joopo
...
Keep Joopo up to date.

## Recommended: `joopo update`

The fastest way to update. It detects your install type (npm or git), fetches the latest version, runs `joopo doctor`, and restarts the gateway.
...
joopo update --dry-run   # preview without applying
```

`joopo update` does not accept `--verbose`. For update diagnostics, use
`--dry-run` to preview the planned actions, `--json` for structured results, or
`joopo update status --json` to inspect channel and availability state. The
...
state, config, credentials, and workspace in `~/.joopo`; it only changes
which Joopo code install the CLI and gateway use.
...
```bash
joopo update --channel dev --dry-run
joopo update --channel stable --dry-run
```
...
gateway is already installed, `joopo update` refreshes the service metadata
...
If `joopo update` fails after the npm package install phase, re-run the
...
```bash
curl -fsSL https://joopo.ai/install.sh | bash -s -- --install-method npm --version <version-or-dist-tag>
```

## Alternative: manual npm, pnpm, or bun

```bash
npm i -g joopo@latest
```

Prefer `joopo update` for supervised installs because it can coordinate the
...
When `joopo update` manages a global npm install, it installs the target into
...
Joopo retries once with `--omit=optional`. That retry helps hosts where native
optional dependencies cannot compile, while keeping the original failure visible
if the fallback also fails.

```bash
pnpm add -g joopo@latest
```

```bash
bun add -g joopo@latest
```
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    Joopo treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in Joopo-owned npm/git roots under the user config directory, and Gateway startup does not mutate the Joopo package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/joopo`. Joopo supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give Joopo write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:

    ```ini
    ReadWritePaths=/var/lib/joopo /home/joopo/.joopo /tmp
    ```

  </Accordion>
  <Accordion title="Disk-space preflight">
    Before package updates and explicit plugin installs, Joopo tries a best-effort disk-space check for the target volume. Low space produces a warning with the checked path, but does not block the update because filesystem quotas, snapshots, and network volumes can change after the check. The actual package-manager install and post-install verification remain authoritative.
  </Accordion>
</AccordionGroup>

## Auto-updater

The auto-updater is off by default. Enable it in `~/.joopo/joopo.json`:
...
from a package tree that has already been replaced. Shell `joopo update`
...
```bash
npm i -g joopo@<version>
joopo doctor
joopo gateway restart
```

<Tip>
`npm view joopo version` shows the current published version.
</Tip>
