---
summary: "Deploy Joopo on Render with Infrastructure-as-Code"
read_when:
  - Deploying Joopo to Render
...
Deploy Joopo on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/joopo/joopo)
...
repository configures everything needed to run Joopo:
...
services:
  - type: web
    ...
    envVars:
      - key: JOOPO_GATEWAY_PORT
        value: "8080"
      - key: JOOPO_STATE_DIR
        value: /data/.joopo
      - key: JOOPO_WORKSPACE_DIR
        value: /data/workspace
      - key: JOOPO_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Joopo state
...
`JOOPO_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Joopo repository, Render will not auto-deploy your Joopo. To update it, run a manual Blueprint sync from the dashboard.
...
For Joopo, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Joopo state plus any configured
...
- Missing `JOOPO_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `JOOPO_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `joopo backup create` in the Render shell.
