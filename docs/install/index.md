---
summary: "Install Joopo - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Joopo, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Joopo and Node kept under a local prefix such as
`~/.joopo`, without depending on a system-wide Node install:

```bash
curl -fsSL https://joopo.ai/install-cli.sh | bash
```
...
`joopo update --channel dev` and `joopo update --channel stable`. See
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g joopo@latest
```
...
Or skip the link and use `pnpm joopo ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:joopo/joopo#main
```
...
```bash
joopo --version      # confirm the CLI is available
joopo doctor         # check for config issues
joopo gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `joopo onboard --install-daemon` or `joopo gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Joopo on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Joopo completely.
  </Card>
</CardGroup>

## Troubleshooting: `joopo` not found

If the install succeeded but `joopo` is not found in your terminal:
