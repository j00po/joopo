---
summary: "Run Joopo Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Joopo running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Joopo Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Joopo 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Joopo Gateway in Docker
- Persist `~/.joopo` + `~/.joopo/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.joopo` state includes `joopo.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-joopo-project --name="Joopo Gateway"
    gcloud config set project my-joopo-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create joopo-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh joopo-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Joopo repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    JOOPO_IMAGE=joopo:latest
    JOOPO_GATEWAY_TOKEN=
    JOOPO_GATEWAY_BIND=lan
    JOOPO_GATEWAY_PORT=18789

    JOOPO_CONFIG_DIR=/home/$USER/.joopo
    JOOPO_WORKSPACE_DIR=/home/$USER/.joopo/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.joopo
    ```

    Set `JOOPO_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Joopo uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `JOOPO_GATEWAY_TOKEN`.
    ...
    services:
      joopo-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - JOOPO_GATEWAY_BIND=${JOOPO_GATEWAY_BIND}
          - JOOPO_GATEWAY_PORT=${JOOPO_GATEWAY_PORT}
          - JOOPO_GATEWAY_TOKEN=${JOOPO_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${JOOPO_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`JOOPO_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm joopo-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh joopo-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop joopo-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type joopo-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start joopo-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create joopo-deploy \
     --display-name="Joopo Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-joopo-project \
     --member="serviceAccount:joopo-deploy@my-joopo-project.iam.gserviceaccount.com" \
