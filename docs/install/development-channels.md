...
Joopo ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/joopo`, override with
  `JOOPO_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
joopo update --tag 2026.4.1-beta.1
...
joopo update --tag joopo@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `joopo update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Joopo prompts for confirmation (skip with `--yes`).
...
Preview what `joopo update` would do without making changes:

```bash
joopo update --dry-run
joopo update --channel beta --dry-run
joopo update --tag 2026.4.1-beta.1 --dry-run
joopo update --dry-run --json
```
...
When you switch channels with `joopo update`, Joopo also syncs plugin
sources:
