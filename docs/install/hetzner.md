---
summary: "Run Joopo Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want Joopo running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running Joopo in Docker on Hetzner or a similar provider
title: "Hetzner"
---

## Goal

Run a persistent Joopo Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Joopo 24/7 for ~$5", this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.joopo/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.joopo
    ```
  ...
  <Step title="Configure environment variables">
    ...
    JOOPO_CONFIG_DIR=/root/.joopo
    JOOPO_WORKSPACE_DIR=/root/.joopo/workspace
...
- Infrastructure: [joopo-terraform-hetzner](https://github.com/andreesg/joopo-terraform-hetzner)
- Docker config: [joopo-docker-config](https://github.com/andreesg/joopo-docker-config)
