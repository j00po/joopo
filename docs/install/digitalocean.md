---
summary: "Host Joopo on a DigitalOcean Droplet"
read_when:
  - Setting up Joopo on DigitalOcean
  - Looking for a simple paid VPS for Joopo
title: "DigitalOcean"
---

Run a persistent Joopo Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own Joopo state and services.
    adduser joopo
    usermod -aG sudo joopo
    loginctl enable-linger joopo

    su - joopo
    joopo --version
    ```

    Use the root shell only for system bootstrap. Run Joopo commands as the non-root `joopo` user so state lives under `/home/joopo/.joopo/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    joopo config set gateway.bind tailnet
    joopo gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `joopo backup create` is portable across hosts.
