---
summary: "Host Joopo on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Joopo on Oracle Cloud
  - Looking for free VPS hosting for Joopo
  - Want 24/7 Joopo on a small server
title: "Oracle Cloud"
---

Run a persistent Joopo Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname joopo
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=joopo
    ```

    From now on, connect via Tailscale: `ssh ubuntu@joopo`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    joopo config set gateway.bind loopback
    joopo config set gateway.auth.mode token
    joopo doctor --generate-gateway-token
    joopo config set gateway.tailscale.mode serve
    joopo config set gateway.trustedProxies '["127.0.0.1"]'

    systemctl --user restart joopo-gateway.service
    ```
  ...
  <Step title="Verify">
    ...
    ```
    https://joopo.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.joopo` to restrict credential file permissions.
- `joopo security audit` for an Joopo-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Joopo features work fine; a small number of native binaries need ARM builds:
...
- `~/.joopo/` — `joopo.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.joopo/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@joopo
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=joopo --reset` to re-authenticate.

**Gateway will not start** -- Run `joopo doctor --non-interactive` and check logs with `journalctl --user -u joopo-gateway.service -n 50`.
