---
summary: "Deploy Joopo Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Joopo on a Kubernetes cluster
  - You want to test Joopo in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running Joopo on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Joopo is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/joopo 18789:18789 -n joopo
...
```bash
kubectl get secret joopo-secrets -n joopo -o jsonpath='{.data.JOOPO_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: joopo (configurable via JOOPO_NAMESPACE)
├── Deployment/joopo        # Single pod, init container + gateway
├── Service/joopo           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/joopo-config  # joopo.json + AGENTS.md
└── Secret/joopo-secrets    # Gateway token + API keys
```
...
Edit `joopo.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret joopo-secrets -n joopo \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/joopo -n joopo
```

### Custom namespace

```bash
JOOPO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/joopo/joopo:latest # or pin to a specific version from https://github.com/joopo/joopo/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # joopo.json + AGENTS.md
