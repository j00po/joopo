...
| `clawdock-cli <command>`  | Run Joopo CLI commands in Docker           |
...
Use `clawdock-show-config` when you want to inspect the `.env` files and `joopo.json` quickly. It redacts `.env` values in its printed output.
