---
summary: "Shared Docker VM runtime steps for long-lived Joopo Gateway hosts"
read_when:
  - You are deploying Joopo on a cloud VM with Docker
...
```bash
docker compose exec joopo-gateway which gog
docker compose exec joopo-gateway which goplaces
docker compose exec joopo-gateway which wacli
```
...
```bash
docker compose logs -f joopo-gateway
```
