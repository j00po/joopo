---
summary: "Step-by-step Fly.io deployment for Joopo with persistent storage and HTTPS"
title: Fly.io
read_when:
  - Deploying Joopo on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

**Goal:** Joopo Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-joopo

    # Create a persistent volume (1GB is usually enough)
    fly volumes create joopo_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `JOOPO_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `JOOPO_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set JOOPO_GATEWAY_TOKEN=$(openssl rand -hex 32)
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require a valid gateway auth path. This Fly.io example uses `JOOPO_GATEWAY_TOKEN`, but `gateway.auth.password` or a correctly configured non-loopback `trusted-proxy` deployment also satisfy the requirement.
    - Treat these tokens like passwords.
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `joopo.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Create config file">
    ...
    **Note:** With `JOOPO_STATE_DIR=/data`, the config path is `/data/joopo.json`.

    **Note:** Replace `https://my-joopo.fly.dev` with your real Fly app
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-joopo.fly.dev/`

    Authenticate with the configured shared secret. This guide uses the gateway
    token from `JOOPO_GATEWAY_TOKEN`; if you switched to password auth, use
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `JOOPO_GATEWAY_PORT=3000`).
...
`--allow-unconfigured` only bypasses the startup guard. It does not create or repair `/data/joopo.json`, so make sure your real config exists and includes `gateway.mode="local"` when you want a normal local gateway start.

Verify the config exists:

```bash
fly ssh console --command "cat /data/joopo.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/joopo.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/joopo.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/joopo.json"
```
...
**Fix:** Ensure `JOOPO_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-joopo

# Release public IPs
fly ips release <public-ipv4> -a my-joopo
fly ips release <public-ipv6> -a my-joopo
...
fly ips allocate-v6 --private -a my-joopo
```
...
fly proxy 3000:3000 -a my-joopo
...
- Keep Joopo up to date: [Updating](/install/updating)
