---
summary: "Symptom first troubleshooting hub for Joopo"
read_when:
  - Joopo is not working and you need the fastest path to a fix
...
- `joopo status` → shows configured channels and no obvious auth errors.
- `joopo status --all` → full report is present and shareable.
- `joopo gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `joopo gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `joopo doctor` → no blocking config/service errors.
- `joopo channels status --probe` → reachable gateway returns live per-account
...
- `joopo logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in Joopo

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `joopo infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Joopo agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Joopo prompts crash the
...
## Plugin install fails with missing joopo extensions

If install fails with `package.json missing joopo.extensions`, the plugin package
is using an old shape that Joopo no longer accepts.

Fix in the plugin package:

1. Add `joopo.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `joopo plugins install <package>` again.

Example:

```json
{
  "name": "@joopo/my-plugin",
  "version": "1.2.3",
  "joopo": {
...
If `joopo doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run Joopo as
...
```mermaid
flowchart TD
  A[Joopo is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `joopo gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    joopo config get tools.exec.host
    joopo config get tools.exec.security
    joopo config get tools.exec.ask
    ...
    ```bash
    joopo config set tools.exec.host gateway
    joopo config set tools.exec.security full
    joopo config set tools.exec.ask off
    joopo gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `joopo` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `joopo browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
