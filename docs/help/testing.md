...
Joopo has three Vitest suites (unit/integration, e2e, live) and a small set
...
- [Matrix QA](/concepts/qa-matrix) - reference for `pnpm joopo qa matrix`.
...
- Linux VM-backed QA lane: `pnpm joopo qa suite --runner multipass --scenario channel-chat-baseline`
...
- Runtime performance reports: dispatch `Joopo Performance` with
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `JOOPO_LIVE_MODEL_FILE_PROBE=0` or
    `JOOPO_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Joopo Scheduled Live And E2E Checks` and manual
    `Joopo Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `Joopo Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/joopo-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server harness,
    ...
    `JOOPO_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when isolating other Codex
    app-server failures. For a focused sub-agent check, disable the other probes:
    `JOOPO_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 JOOPO_LIVE_CODEX_HARNESS_MCP_PROBE=0 JOOPO_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 JOOPO_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `JOOPO_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty Joopo state dir, routes bare `joopo` to
    ...
    `pnpm joopo qa suite --scenario crestodian-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `joopo models list --provider moonshot --json`, then run an isolated
  `joopo agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`media`, `e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `Joopo Release
...
`ghcr.io/joopo/joopo-live-media-runner:ubuntu-24.04`, which already has
`ffmpeg` and `ffprobe`. Docker live model/backend shards use the shared
`ghcr.io/joopo/joopo-live-test:<sha>` image built once per selected
commit, then pull it with `JOOPO_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab. It
    ...
    `joopo-testbox-env` helper is present.
...
- `pnpm joopo qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `JOOPO_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. It verifies hidden Joopo runtime context is persisted as a
    ...
    `joopo doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an Joopo package candidate in Docker, runs installed-package
  ...
  - The wrapper mounts only the `qa-lab` harness source from the checkout; the
    installed package owns `dist`, `joopo/plugin-sdk`, and bundled plugin
    runtime so the lane does not mix current checkout plugins into the package
    under test.
  - Defaults to `JOOPO_NPM_TELEGRAM_PACKAGE_SPEC=joopo@beta`; set
    `JOOPO_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/joopo-current.tgz` or
    `JOOPO_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead of
    installing from the registry.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm joopo qa telegram`. For CI/release automation, set
    `JOOPO_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `JOOPO_QA_CONVEX_SITE_URL` and the role secret. If
    `JOOPO_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
  ...
  - The wrapper validates Telegram or Convex credential env on the host before
    Docker build/install work. Set `JOOPO_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1`
    only when deliberately debugging pre-credential setup.
  - `JOOPO_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `JOOPO_QA_CREDENTIAL_ROLE` for this lane only.
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  the normalized `joopo-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/joopo/-/joopo-VERSION.tgz \
...
- `pnpm test:docker:plugins`
  - Packs and installs the current Joopo build in Docker, starts the Gateway
  ...
  - Also installs a known older npm baseline, enables Telegram before running
    `joopo update --tag <candidate>`, and verifies the candidate's
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests. Each
    ...
    the installed `joopo update` command in the same guest and verifies the
  ...
  - The OpenAI lane uses `openai/gpt-5.5` for the live agent-turn proof by
    ...
    `JOOPO_PARALLELS_OPENAI_MODEL` when deliberately validating another
    OpenAI model.
  ...
  - The script writes nested lane logs under `/tmp/joopo-parallels-npm-update.*`.
...
- `pnpm joopo qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `JOOPO_QA_TELEGRAM_GROUP_ID`, `JOOPO_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `JOOPO_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `JOOPO_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
  - Exits non-zero when any scenario fails. Use `--allow-failures` when you
    want artifacts without a failing exit code.
...
When `--credential-source convex` (or `JOOPO_QA_CREDENTIAL_SOURCE=convex`) is enabled for
`joopo qa telegram`, QA lab acquires an exclusive lease from a Convex-backed pool, heartbeats
...
- `JOOPO_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `JOOPO_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `JOOPO_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `JOOPO_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `JOOPO_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `JOOPO_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `JOOPO_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `JOOPO_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `JOOPO_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `JOOPO_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `JOOPO_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`JOOPO_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`JOOPO_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm joopo qa credentials doctor
pnpm joopo qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm joopo qa credentials list --kind telegram
pnpm joopo qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`JOOPO_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `JOOPO_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-joopo-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `JOOPO_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `JOOPO_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `JOOPO_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `JOOPO_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `JOOPO_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `JOOPO_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Joopo's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `JOOPO_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `JOOPO_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `JOOPO_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.joopo`.
- Set `JOOPO_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `JOOPO_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `JOOPO_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `JOOPO_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `JOOPO_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `JOOPO_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `JOOPO_LIVE_GATEWAY_SMOKE=1`,
  `JOOPO_LIVE_GATEWAY_MAX_MODELS=8`,
  `JOOPO_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `JOOPO_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
  explicitly want the larger exhaustive scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs Joopo once as an npm tarball through `scripts/package-joopo-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `JOOPO_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `JOOPO_DOCKER_ALL_LIVE_LIMIT=9`, `JOOPO_DOCKER_ALL_NPM_LIMIT=10`, and `JOOPO_DOCKER_ALL_SERVICE_LIMIT=7`; tune `JOOPO_DOCKER_ALL_WEIGHT_LIMIT` or `JOOPO_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Joopo E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `JOOPO_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Joopo tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `JOOPO_CURRENT_PACKAGE_TGZ=/path/to/joopo-*.tgz`, skip the host rebuild with `JOOPO_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `JOOPO_NPM_ONBOARD_CHANNEL=discord` or `JOOPO_NPM_ONBOARD_CHANNEL=slack`.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed Joopo tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
- Upgrade survivor smoke: `pnpm test:docker:upgrade-survivor` installs the packed Joopo tarball over a dirty old-user fixture with agents, channel config, plugin allowlists, stale plugin dependency state, and existing workspace/session files. It runs package update plus non-interactive doctor without live provider or channel keys, then starts a loopback Gateway and checks config/state preservation plus startup/status budgets.
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `joopo@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `JOOPO_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `joopo@2026.5.2 joopo@2026.4.23 joopo@2026.4.15`, and expand issue-shaped fixtures with `JOOPO_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external Joopo plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation expands the release-soak package gate to `last-stable-4 2026.4.23 2026.5.2 2026.4.15` plus `reported-issues`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `joopo infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `JOOPO_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/joopo-*.tgz`, skip the host build with `JOOPO_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `JOOPO_BUN_GLOBAL_SMOKE_DIST_IMAGE=joopo-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `JOOPO_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `JOOPO_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `JOOPO_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `JOOPO_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=joopo-dockerfile-smoke:local JOOPO_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, git moving refs, JoopoHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `JOOPO_PLUGINS_E2E_JOOPOHUB=0` to skip the JoopoHub block, or override the default kitchen-sink package/runtime pair with `JOOPO_PLUGINS_E2E_JOOPOHUB_SPEC` and `JOOPO_PLUGINS_E2E_JOOPOHUB_ID`. Without `JOOPO_JOOPOHUB_URL`/`JOOPOHUB_URL`, the test uses a hermetic local JoopoHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed Joopo tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
- Config reload metadata smoke: `pnpm test:docker:config-reload` (script: `scripts/e2e/config-reload-source-docker.sh`)
- Plugins: `pnpm test:docker:plugins` covers install/update smoke for local path, `file:`, npm registry with hoisted dependencies, git moving refs, JoopoHub fixtures, marketplace updates, and Claude-bundle enable/inspect. `pnpm test:docker:plugin-update` covers unchanged update behavior for installed plugins. `pnpm test:docker:plugin-lifecycle-matrix` covers resource-tracked npm plugin install, enable, disable, upgrade, downgrade, and missing-code uninstall.

To prebuild and reuse the shared functional image manually:

```bash
JOOPO_DOCKER_E2E_IMAGE=joopo-docker-e2e-functional:local pnpm test:docker:e2e-build
JOOPO_DOCKER_E2E_IMAGE=joopo-docker-e2e-functional:local JOOPO_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `JOOPO_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `JOOPO_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__joopo_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `JOOPO_SKIP_CHANNELS=1` so gateway live probes do not start
...
`JOOPO_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Joopo gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `joopo/default`, then sends a
...
This lane expects a usable live model key, and `JOOPO_PROFILE_FILE`
...
container, starts a second container that spawns `joopo mcp serve`, then
...
- `JOOPO_CONFIG_DIR=...` (default: `~/.joopo`) mounted to `/home/node/.joopo`
- `JOOPO_WORKSPACE_DIR=...` (default: `~/.joopo/workspace`) mounted to `/home/node/.joopo/workspace`
- `JOOPO_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `JOOPO_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `JOOPO_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `JOOPO_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/joopo/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `JOOPO_LIVE_PROVIDERS` / `JOOPO_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `JOOPO_DOCKER_AUTH_DIRS=all`, `JOOPO_DOCKER_AUTH_DIRS=none`, or a comma list like `JOOPO_DOCKER_AUTH_DIRS=.claude,.codex`
- `JOOPO_LIVE_GATEWAY_MODELS=...` / `JOOPO_LIVE_MODELS=...` to narrow the run
- `JOOPO_LIVE_GATEWAY_PROVIDERS=...` / `JOOPO_LIVE_PROVIDERS=...` to filter providers in-container
- `JOOPO_SKIP_DOCKER_BUILD=1` to reuse an existing `joopo:local-live` image for reruns that do not need a rebuild
- `JOOPO_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `JOOPO_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `JOOPO_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
