...
This is handy when you need to toggle obscure settings without editing `joopo.json`.
...
Use `JOOPO_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
JOOPO_PLUGIN_LIFECYCLE_TRACE=1 joopo plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm joopo ...`
...
`JOOPO_RUN_NODE_CPU_PROF_DIR`:

```bash
JOOPO_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm joopo status
```
...
```bash
JOOPO_TRACE_SYNC_IO=1 pnpm joopo gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `JOOPO_TRACE_SYNC_IO=1` when you explicitly want Node
...
`joopo-gateway-watch-main` (or a profile/port-specific variant such as
`joopo-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t joopo-gateway-watch-main
```
...
# or
JOOPO_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
JOOPO_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`JOOPO_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`JOOPO_PROFILE`, `JOOPO_CONFIG_PATH`, `JOOPO_STATE_DIR`,
`JOOPO_GATEWAY_PORT`, and `JOOPO_SKIP_CHANNELS` into the pane. Put
...
`joopo doctor --fix --non-interactive` once and restarts the Gateway child.
Use `JOOPO_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `joopo.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.joopo-dev` and
...
```bash
pnpm gateway:dev
JOOPO_PROFILE=dev joopo tui
```

If you don't have a global install yet, run the CLI via `pnpm joopo ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `JOOPO_PROFILE=dev`
   - `JOOPO_STATE_DIR=~/.joopo-dev`
   - `JOOPO_CONFIG_PATH=~/.joopo-dev/joopo.json`
   - `JOOPO_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`JOOPO_SKIP_CHANNELS=1`).
...
```bash
JOOPO_PROFILE=dev joopo gateway --dev --reset
```
...
## Raw stream logging (Joopo)

Joopo can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.joopo/logs/raw-stream.jsonl
```
...
```bash
JOOPO_RAW_STREAM=1
JOOPO_RAW_STREAM_PATH=~/.joopo/logs/raw-stream.jsonl
```

Default file:

`~/.joopo/logs/raw-stream.jsonl`
...
- If you need to use the built Joopo CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./joopo.mjs` or create a shell alias like `alias joopo-build="node $(pwd)/joopo.mjs"`
