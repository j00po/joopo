---
summary: "How Joopo validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing Joopo update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `joopo doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `joopo plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `joopo config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `joopo update --yes --json`, and requires the candidate update command to
...
JOOPO_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

JOOPO_UPGRADE_SURVIVOR_BASELINE_SPEC=joopo@latest \
JOOPO_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```
...
`JOOPO_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=npm`: validate `joopo@beta`, `joopo@latest`, or an exact
  published version.
...
`package_acceptance_package_spec=joopo@YYYY.M.D` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published Joopo
