...
read_when:
  ...
  - Cannot access docs.joopo.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Joopo **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/joopo/joopo/issues](https://github.com/joopo/joopo/issues)
    [https://github.com/joopo/joopo/pulls](https://github.com/joopo/joopo/pulls)
    ...
    - `joopo status`: quick snapshot of gateway/agent health + basic config.
    - `joopo models status`: checks provider auth + model availability.
    - `joopo doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `joopo status --all`, `joopo logs --follow`,
    `joopo gateway status`, `joopo health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Joopo">
    The repo recommends running from source and using onboarding:

    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash
    ...
    ```bash
    git clone https://github.com/joopo/joopo.git
    cd joopo
    pnpm install
    pnpm build
    pnpm ui:build
    ...
    If you don't have a global install yet, run it via `pnpm joopo onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `JOOPO_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `JOOPO_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `joopo doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `joopo gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `joopo gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Joopo now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Joopo on the new machine.
    2. Copy `$JOOPO_STATE_DIR` (default: `~/.joopo`) from the old machine.
    3. Copy your workspace (default: `~/.joopo/workspace`).
    4. Run `joopo doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.joopo/` (for example `~/.joopo/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/joopo/joopo/blob/main/CHANGELOG.md](https://github.com/joopo/joopo/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.joopo.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.joopo.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.joopo.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/joopo/joopo/tree/main/docs](https://github.com/joopo/joopo/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://joopo.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://joopo.ai/install.ps1](https://joopo.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://joopo.ai/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or joopo not recognized">
    ...
    **2) joopo is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Joopo, track/report it in:

    - [Issue #30640](https://github.com/joopo/joopo/issues/30640)
  ...
  <Accordion title="How do I install Joopo on Linux?">
  ...
  <Accordion title="How do I install Joopo on a VPS?">
  ...
  <Accordion title="Can I ask Joopo to update itself?">
    ...
    joopo update
    joopo update status
    joopo update --channel stable|beta|dev
    joopo update --tag <dist-tag|version>
    joopo update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    joopo update --yes --no-restart
    joopo gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `joopo onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Joopo with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Joopo, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Joopo**: Anthropic staff
      told us this usage is allowed again, and Joopo is treating `claude -p`
    ...
    Joopo also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Joopo-style Claude CLI usage is allowed again, so
    Joopo treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Joopo treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Joopo token path, but Joopo now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Joopo, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Joopo Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Joopo can keep replying while a provider is rate-limited.
  ...
  <Accordion title="Why does Joopo still mention openai-codex?">
    ...
    - `openai-codex/gpt-5.5` = legacy model route repaired by `joopo doctor --fix`
    ...
    `joopo models auth login --provider openai-codex`. Keep the model ref as
    `openai/gpt-5.5`; `openai-codex/*` model refs are legacy config that
    `joopo doctor --fix` rewrites.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Joopo can show the currently visible provider usage/quota windows in
    `joopo models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Joopo fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Joopo. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `joopo.json`.
    ...
    2. Enable the plugin: `joopo plugins enable google`
    3. Login: `joopo models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Joopo needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Joopo runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Joopo, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `joopo nodes status` / `joopo nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `joopo doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `joopo logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Joopo instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Joopo via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes. Use `joopo update --channel ...` when Joopo is already installed.
    This **does not delete your data** - it only changes the Joopo code install.
    Your state (`~/.joopo`) and workspace (`~/.joopo/workspace`) stay untouched.
    ...
    ```bash
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --install-method git
    curl -fsSL https://joopo.ai/install.sh | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Joopo-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Joopo on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Joopo is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Joopo in a VM and what are the requirements?">
