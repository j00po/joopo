---
summary: "Frequently asked questions about Joopo setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/joopo/joopo-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   joopo health --json
   joopo health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for Joopo?">
  ...
  <Accordion title="Can Joopo help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Joopo draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Joopo is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Joopo when you
    ...
    Showcase: [https://joopo.ai/showcase](https://joopo.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.joopo/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.joopo/joopo.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.joopo/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.joopo/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Joopo falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Joopo intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    joopo tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `JOOPO_SKIP_CRON` is not set.
    ...
    ```bash
    joopo cron run <jobId>
    joopo cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    ...
    joopo skills search --limit 20
    joopo skills install <skill-slug>
    joopo skills install <skill-slug> --version <version>
    joopo skills install <skill-slug> --force
  ...
  <Accordion title="Can Joopo run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.joopo.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.joopo/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "joopo": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `JOOPO_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `JOOPO_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Joopo validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Joopo memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Joopo also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Joopo auto-selects a provider when it
...
<AccordionGroup>
  <Accordion title="Is all data used with Joopo saved locally?">
    No - **Joopo's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.joopo` + your workspace directory).
  ...
  <Accordion title="Where does Joopo store its data?">
    Everything lives under `$JOOPO_STATE_DIR` (default: `~/.joopo`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$JOOPO_STATE_DIR/joopo.json`                             | Main config (JSON5)                                                |
    | `$JOOPO_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$JOOPO_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$JOOPO_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$JOOPO_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$JOOPO_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$JOOPO_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$JOOPO_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$JOOPO_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.joopo/agent/*` (migrated by `joopo doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.joopo/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.joopo`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
      Lowercase root `memory.md` is legacy repair input only; `joopo doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.joopo`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.joopo/skills`).

    Default workspace is `~/.joopo/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.joopo/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.joopo` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Joopo?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Joopo repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Joopo reads an optional **JSON5** config from `$JOOPO_CONFIG_PATH` (default: `~/.joopo/joopo.json`):

    ```
    $JOOPO_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.joopo/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `JOOPO_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Joopo enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and generates a runtime-only token for that startup, so **local WS clients must authenticate**. Configure `gateway.auth.token`, `gateway.auth.password`, `JOOPO_GATEWAY_TOKEN`, or `JOOPO_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `joopo doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Joopo.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `JOOPO_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `joopo configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Joopo auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.joopo/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Joopo protects many accidental clobbers:

    - Joopo-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Joopo-owned writes are rejected and saved as `joopo.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, Gateway fails closed or skips the reload; it does not rewrite `joopo.json`.
    - `joopo doctor --fix` owns repair and can restore last-known-good while saving the rejected file as `joopo.json.clobbered.*`.

    Recover:

    - Check `joopo logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `joopo.json.clobbered.*` or `joopo.json.rejected.*` beside the active config.
    - Run `joopo config validate` and `joopo doctor --fix`.
    - Copy only the intended keys back with `joopo config set` or `config.patch`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `joopo doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `joopo config set` for small changes.
    - Use `joopo configure` for interactive edits.
  ...
  <Accordion title="Can the Joopo browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       ```bash
       joopo devices list
       joopo devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `joopo gateway status`
    - Gateway health: `joopo status`
    - Channel health: `joopo channels status`
  ...
  <Accordion title="Can two Joopo instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `joopo agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    joopo agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Joopo load environment variables?">
    Joopo reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.joopo/.env` (aka `$JOOPO_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.joopo/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `JOOPO_LOAD_SHELL_ENV=1`, `JOOPO_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `joopo models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Joopo won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.joopo/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Joopo instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Joopo but keep it installed?">
    ...
    ```bash
    joopo onboard --install-daemon
    ```

    Notes:

    - Onboarding also offers **Reset** if it sees an existing config. See [Onboarding (CLI)](/start/wizard).
    - If you used profiles (`--profile` / `JOOPO_PROFILE`), reset each state dir (defaults are `~/.joopo-<profile>`).
    - Dev reset: `joopo gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Joopo skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Joopo runs on **your own account**, so if you're in the group, Joopo can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    joopo directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Joopo not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.joopo/agents/<agentId>/sessions/`.
    ...
    - Use `joopo doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > JOOPO_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does joopo gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `joopo gateway status` and trust these lines:
  ...
  <Accordion title='Why does joopo gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `JOOPO_STATE_DIR` mismatch).

    Fix:

    ```bash
    joopo gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Joopo enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `joopo gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Joopo in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `joopo gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `joopo dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `joopo doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `JOOPO_GATEWAY_TOKEN` or `gateway.auth.password` / `JOOPO_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `joopo devices list`
      - `joopo devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `joopo status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `JOOPO_CONFIG_PATH` (per-instance config)
    - `JOOPO_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `joopo --profile <name> ...` per instance (auto-creates `~/.joopo-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `joopo --profile <name> gateway install`.

    Profiles also suffix service names (`ai.joopo.<profile>`; legacy `com.joopo.*`, `joopo-gateway-<profile>.service`, `Joopo Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    joopo tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$JOOPO_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.joopo/logs/...`; profiles use `~/.joopo-<profile>/logs/...`)
    - Linux: `journalctl --user -u joopo-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Joopo Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `joopo gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Joopo?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `joopo gateway status`
    2. Is the Gateway healthy? `joopo status`
    3. Does the UI have the right token? `joopo dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    joopo channels status
    joopo channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Joopo already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: joopo gateway restart vs joopo gateway">
    - `joopo gateway restart`: restarts the **background service** (launchd/systemd).
    - `joopo gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `joopo gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Joopo assistant setup](/start/joopo) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    joopo message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Joopo to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `joopo pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `joopo pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `joopo doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Joopo only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    joopo pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    joopo pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Joopo blocks **cross-provider** messaging by default. If a tool call is bound
