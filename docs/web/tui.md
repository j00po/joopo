...
```bash
joopo tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `joopo chat` and `joopo terminal` are aliases for `joopo tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- `joopo` and `joopo crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- Turn delivery on:
  ...
  - or start with `joopo tui --deliver`
...
- Local shell commands receive `JOOPO_SHELL=tui-local` in their environment.
...
If `joopo config validate` is already failing, start with `joopo configure`
or `joopo doctor --fix` first. `joopo chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `joopo config set` or `joopo configure`, then rerun `!joopo config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!joopo doctor --fix`.

Tips:

- Prefer `joopo config set` or `joopo configure` over hand-editing `joopo.json`.
- `joopo docs "<query>"` searches the live docs index from the same machine.
- `joopo config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `JOOPO_THEME=light` before launching `joopo tui`.
- To force the original dark palette instead, set `JOOPO_THEME=dark`.
...
- Check the Gateway logs: `joopo logs --follow`.
- Confirm the agent can run: `joopo status` and `joopo models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `joopo agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `joopo.json`
