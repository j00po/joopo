...
- Re-open anytime: `joopo dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `joopo dashboard` still prints the
  clean URL and tells you to use the token from `JOOPO_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `JOOPO_GATEWAY_TOKEN`); `joopo dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `joopo dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `joopo dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `JOOPO_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `joopo status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `joopo config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `JOOPO_GATEWAY_TOKEN` in this shell, then rerun `joopo dashboard`
  - No shared secret configured: `joopo doctor --generate-gateway-token`
