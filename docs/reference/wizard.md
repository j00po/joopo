...
This is the full reference for `joopo onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.joopo/joopo.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `joopo doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Joopo now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.joopo/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.joopo/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.joopo/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$JOOPO_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.joopo/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `joopo pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `joopo configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `joopo health`.
    - Tip: `joopo status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
```bash
export JOOPO_GATEWAY_TOKEN="your-token"
joopo onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env JOOPO_GATEWAY_TOKEN
```
...
- Stores it under `~/.joopo/tools/signal-cli/<version>/`.
...
Typical fields in `~/.joopo/joopo.json`:
...
`joopo agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.joopo/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.joopo/agents/<agentId>/sessions/`.
