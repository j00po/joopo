---
summary: "Reference design for the public Joopo App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "Joopo App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public Joopo app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level Joopo App SDK wrapper
---

This page is the detailed API reference design for the public
[Joopo App SDK](/concepts/joopo-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@joopo/sdk` is the external app/client package for talking to the
  Gateway. `joopo/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `Joopo`, `Agent`, `Session`, `Run`,
...
- source: Joopo, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@joopo/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@joopo/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@joopo/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `joopo/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `Joopo`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [Joopo App SDK](/concepts/joopo-sdk)
