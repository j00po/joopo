...
Joopo has three public release lanes:
...
- Every stable Joopo release ships the npm package and macOS app together;
...
6. Run `Joopo NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For beta, tag `vYYYY.M.D-beta.N`, then run `Joopo Release Publish` from
   ...
   acceptance against the published `joopo@YYYY.M.D-beta.N` or
   `joopo@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `Joopo Release Publish`, reusing the successful preflight artifact via
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `Joopo Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `joopo@beta`, `joopo@latest`, or an exact release version; `source=ref`
...
- Run `Joopo Release Publish` for the mutating publish sequence after the
...
- Release checks now run in a separate manual workflow:
  `Joopo Release Checks`
- `Joopo Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `Joopo Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/joopo-cross-os-release-checks-reusable.yml` directly
...
- `Joopo Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an Joopo branch or release tag
- `Joopo NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `JOOPO_LIVE_TEST=1 JOOPO_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/joopo-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/joopo-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `JOOPO_NPM_TELEGRAM_PACKAGE_SPEC=joopo@YYYY.M.D-beta.N JOOPO_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex JOOPO_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `JOOPO_QA_TELEGRAM_*` env credentials directly.
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=joopo@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `Joopo Release Checks`, prepares a
...
`rerun_group=all` or when `npm_telegram_package_spec` is set. `Joopo Release
...
`Joopo Release Checks` uses the trusted workflow ref to resolve the target
...
The cross-OS OpenAI install smoke uses `JOOPO_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=joopo@YYYY.M.D-beta.N \
  -f npm_telegram_package_spec=joopo@YYYY.M.D-beta.N \
...
The Docker box lives in `Joopo Release Checks` through
`joopo-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `Joopo Release Checks`. It is the agentic
...
`scripts/resolve-joopo-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `joopo@beta`, `joopo@latest`, or an exact Joopo release
  version
...
`Joopo Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=joopo@2026.4.26
```
...
`Joopo Release Publish` is the normal mutating publish entrypoint. It
...
Joopo package must not be published.

## NPM workflow inputs

`Joopo NPM Release` accepts these operator-controlled inputs:
...
`Joopo Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Joopo NPM Release` preflight run id;
  required when `publish_joopo_npm=true`
- `npm_dist_tag`: npm target tag for the Joopo package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused repair work
- `plugins`: comma-separated `@joopo/*` package names when
  `plugin_publish_scope=selected`
- `publish_joopo_npm`: defaults to `true`; set `false` only when using the
  workflow as a plugin-only repair orchestrator

`Joopo Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an Joopo branch or
  release tag.
...
- For `Joopo NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `Joopo Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `Joopo NPM Release` with `preflight_only=true`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/joopo/joopo/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/joopo/joopo/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/joopo-npm-release.yml`](https://github.com/joopo/joopo/blob/main/.github/workflows/joopo-npm-release.yml)
- [`.github/workflows/joopo-release-checks.yml`](https://github.com/joopo/joopo/blob/main/.github/workflows/joopo-release-checks.yml)
- [`.github/workflows/joopo-cross-os-release-checks-reusable.yml`](https://github.com/joopo/joopo/blob/main/.github/workflows/joopo-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-joopo-package-candidate.mjs`](https://github.com/joopo/joopo/blob/main/scripts/resolve-joopo-package-candidate.mjs)
- [`scripts/joopo-npm-release-check.ts`](https://github.com/joopo/joopo/blob/main/scripts/joopo-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/joopo/joopo/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/joopo/joopo/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`joopo/maintainers/release/README.md`](https://github.com/joopo/maintainers/blob/main/release/README.md)
