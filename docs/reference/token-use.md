---
summary: "How Joopo builds prompt context and reports token usage + costs"
read_when:
...
Joopo tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Joopo assembles its own system prompt on every run. It includes:
...
For images, Joopo downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Joopo session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `joopo status --usage` and `joopo channels list` show
...
available; otherwise Joopo falls back to matching OAuth/API-key credentials
...
Joopo keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Joopo shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Joopo starts an
...
Provider prompt caching only applies within the cache TTL window. Joopo can
...
Anthropic's 1M context window is currently beta-gated. Joopo can inject the
...
Joopo skips the `context-1m-*` beta header because Anthropic currently
