...
Joopo manages sessions end-to-end across these areas:
...
Joopo is designed around a single **Gateway process** that owns session state.
...
Joopo persists sessions in two layers:
...
- Transcripts: `~/.joopo/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Joopo resolves these via `src/config/sessions.ts`.
...
Joopo no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `joopo doctor --fix` removes it from older configs.
...
In `mode: "warn"`, Joopo reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
joopo sessions cleanup --dry-run
joopo sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.joopo/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- **Parent fork policy** uses PI's active branch when creating a thread or subagent fork. If that branch is too large, Joopo starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `joopo doctor --fix`.
...
Joopo intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Joopo splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Joopo
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Joopo preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Joopo consumes the events, but Pi decides when to compact).

Joopo can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: Joopo still runs normal semantic compaction,
...
next model call, Joopo estimates the prompt pressure using the same preflight
...
Joopo also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Joopo bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, Joopo leaves it alone.
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  Joopo rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, Joopo falls back to built-in LLM summarization automatically.
...
Joopo supports "silent" turns for background tasks where the user should not see intermediate output.
...
- Joopo strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Joopo also suppresses **draft/typing streaming** when a
...
Joopo uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Joopo's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `joopo status`.
