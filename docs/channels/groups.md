...
Joopo treats group chats consistently across surfaces: Discord, iMessage, Matrix, Microsoft Teams, Signal, Slack, Telegram, WhatsApp, Zalo.

## Beginner intro (2 minutes)

Joopo "lives" on your own messaging accounts. There is no separate WhatsApp bot user. If **you** are in a group, Joopo can see that group and respond there.
...
Translation: allowlisted senders can trigger Joopo by mentioning it.
...
For group/channel rooms, Joopo defaults to `messages.groupChat.visibleReplies: "message_tool"`.
`joopo doctor --fix` writes this default into configured-channel configs that omit it.
...
If the message tool is unavailable under the active tool policy, Joopo falls
back to automatic visible replies instead of silently suppressing the response.
`joopo doctor` warns about this mismatch.
...
By default, Joopo prioritizes normal chat behavior and keeps context mostly as received. This means allowlists primarily decide who can trigger actions, not a universal redaction boundary for every quoted or historical snippet.
    ...
    list: [
      {
        id: "main",
        groupChat: {
          mentionPatterns: ["@joopo", "joopo", "\\+15555550123"],
          historyLimit: 50,
        },
      },
    ],
