...
Nextcloud Talk ships as a bundled plugin in current Joopo releases, so
...
```bash
joopo plugins install @joopo/nextcloud-talk
```
...
```bash
joopo plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Joopo" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
4. Configure Joopo:
   ...
   ```bash
   joopo channels add --channel nextcloud-talk \
...
- Approve via:
  - `joopo pairing list nextcloud-talk`
  - `joopo pairing approve nextcloud-talk <CODE>`
