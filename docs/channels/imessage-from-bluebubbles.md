...
3. Snapshot your config so you can roll back:
   ...
   ```bash
   cp ~/.joopo/joopo.json5 ~/.joopo/joopo.json5.bak
   ```
...
2. **Dry-run probe** — start the gateway and confirm both channels report healthy:
   ...
   joopo channels status --probe   # expect imessage.privateApi.available: true
   ```
...
| Threaded reply (`reply_to_guid`)                           | ✅                                  | ✅ (closes [#51892](https://github.com/joopo/joopo/issues/51892))              |
| Tapback (`react`)                                          | ✅                                  | ✅                                                                                   |
| Edit / unsend (macOS 13+ recipients)                       | ✅                                  | ✅                                                                                   |
| Send with screen effect                                    | ✅                                  | ✅ (closes part of [#9394](https://github.com/joopo/joopo/issues/9394))        |
...
3. Inbound traffic returns to BlueBubbles. Reply caches and ACP bindings on the iMessage side stay on disk under `~/.joopo/state/imessage/` and resume cleanly if you re-enable later.

The reply cache lives at `~/.joopo/state/imessage/reply-cache.jsonl` (mode `0600`, parent dir `0700`). It is safe to delete if you want a clean slate.
