...
<Note>
Requires Joopo 2026.4.25 or above. Run `joopo --version` to check. Upgrade with `joopo update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  joopo channels login --channel feishu
  ```
...
```bash
joopo pairing list feishu
joopo pairing approve feishu <CODE>
```
...
4. Check logs: `joopo logs --follow`
...
5. Ensure the gateway is running: `joopo gateway status`
6. Check logs: `joopo logs --follow`

### App Secret leaked

1. Reset the App Secret in Feishu Open Platform / Lark Developer
2. Update the value in your config
3. Restart the gateway: `joopo gateway restart`
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Joopo
...
conversion fails, Joopo falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, Joopo
hydrates it from Feishu before routing the turn. Normal group replies that
Joopo turns into threads keep using the reply root message ID (`om_*`) so the
