...
read_when:
  - You want to connect Joopo to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Joopo via the LINE Messaging API. The plugin runs as a webhook
...
```bash
joopo plugins install @joopo/line
```

Local checkout (when running from a git repo):

```bash
joopo plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Joopo applies strict pre-auth body limits and timeout before verification.
- Joopo processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
joopo pairing list line
joopo pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.joopo/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Joopo validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
