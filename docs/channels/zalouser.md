...
read_when:
  - Setting up Zalo Personal for Joopo
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Joopo.
...
Zalo Personal ships as a bundled plugin in current Joopo releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `joopo plugins install @joopo/zalouser`
- Pinned version: `joopo plugins install @joopo/zalouser@2026.5.2`
- Or from a source checkout: `joopo plugins install ./path/to/local/zalouser-plugin`
...
- `joopo pairing list zalouser`
- `joopo pairing approve zalouser <code>`
...
- On startup, Joopo resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, Joopo stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Joopo state. Example:
...
- Joopo sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Joopo sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `joopo channels logout --channel zalouser && joopo channels login --channel zalouser`
...
- The channel now runs fully in Joopo without external CLI binaries.
