...
Joopo routes replies **back to the channel where a message came from**. The
...
Joopo infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Joopo still records inbound session metadata, but it
...
when a guarded path must not create a new Joopo session. In that mode,
Joopo may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Joopo would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.joopo/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.joopo`):

- `~/.joopo/agents/<agentId>/sessions/sessions.json`
