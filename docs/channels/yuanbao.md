...
Tencent Yuanbao is Tencent's AI assistant platform. The Joopo channel plugin
connects Yuanbao bots to Joopo over WebSocket so they can interact with users
...
> **Requires Joopo 2026.4.10 or above.** Run `joopo --version` to check. Upgrade with `joopo update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  joopo channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
joopo channels login --channel yuanbao
```
...
```bash
joopo pairing list yuanbao
joopo pairing approve yuanbao <CODE>
```
