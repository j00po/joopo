...
Tlon is a decentralized messenger built on Urbit. Joopo connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Joopo releases, so normal packaged
...
```bash
joopo plugins install @joopo/tlon
```
...
```bash
joopo plugins install ./path/to/local/tlon-plugin
```
...
By default, Joopo blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `joopo message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Joopo replies in-thread.
