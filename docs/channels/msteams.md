...
Microsoft Teams ships as a bundled plugin in current Joopo releases, so no
...
```bash
joopo plugins install @joopo/msteams
```
...
```bash
joopo plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-joopo-bot --allow-anonymous
devtunnel port create my-joopo-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-joopo-bot
...
**4. Configure Joopo** using the credentials from the output:
...
- Talk to Joopo via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists - they can change. Joopo disables direct name matching by default; opt in explicitly with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Joopo resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.joopo/joopo.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `joopo-msteams` (must be unique) |
...
For production deployments, Joopo supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, Joopo uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, Joopo defaults to client secret authentication. Existing configurations continue to work without changes.
...
Joopo exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Joopo in Teams", full: "Joopo in Teams" },
...
Joopo handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Joopo only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/JoopoShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Joopo sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `joopo message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.joopo/msteams-polls.json`.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries yet (inspect the store file if needed).

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool or CLI. Joopo renders them as Teams Adaptive Cards from the generic presentation contract.
...
```bash
joopo message send --channel msteams \
...
joopo message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
joopo message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
joopo message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
joopo message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- **Ignore** the `groupId` query parameter for Joopo routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
