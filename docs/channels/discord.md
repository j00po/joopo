...
You will need to create a new application with a bot, add the bot to your server, and pair it to Joopo. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "Joopo".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your Joopo agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to Joopo in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with Joopo. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running Joopo before messaging your agent.
...
JSON5
joopo config patch --file ./discord.patch.json5 --dry-run
joopo config patch --file ./discord.patch.json5
joopo gateway
```

    If Joopo is already running as a background service, restart it via the Joopo Mac app or by stopping and restarting the `joopo gateway run` process.
    For managed service installs, run `joopo gateway install` from a shell where `DISCORD_BOT_TOKEN` is present, or store the variable in `~/.joopo/.env`, so the service can resolve the env SecretRef after restart.
  ...
  <Step title="Configure Joopo and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your Joopo agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
```
        ...
        For scripted or remote setup, write the same JSON5 block with `joopo config patch --file ./discord.patch.json5 --dry-run` and then rerun without `--dry-run`. Plaintext `token` values are supported. SecretRef values are also supported for `channels.discord.token` across env/file/exec providers. See [Secrets Management](/gateway/secrets).
...
```bash
joopo pairing list discord
joopo pairing approve discord <CODE>
```
...
If two enabled Discord accounts resolve to the same bot token, Joopo starts only one gateway monitor for that token. A config-sourced token wins over the default env fallback; otherwise the first enabled account wins and the duplicate account is reported disabled.
...
- Discord guild/channel metadata is added to the model prompt as untrusted
  ...
  back, Joopo strips the copied metadata from outbound replies and from
...
Discord forum and media channels only accept thread posts. Joopo supports two ways to create them:
...
- Use `joopo message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
joopo message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
joopo message thread create --channel discord --target channel:<forumId> \
...
Joopo supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- Joopo adds a trigger button automatically
...
<Tabs>
  <Tab title="DM policy">
    ...
    Legacy `channels.discord.dm.policy` and `channels.discord.dm.allowFrom` still read for compatibility. `joopo doctor --fix` migrates them to `dmPolicy` and `allowFrom` when it can do so without changing access.
...
```
    ...
    Enable the Discord Developer Portal **Server Members Intent** for the bot when using channel-audience access groups. DMs do not include guild member state, so Joopo resolves the member through Discord REST at authorization time.
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `joopo security audit` warns when name/tag entries are used
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces Joopo auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    Joopo can stream draft replies by sending a temporary message and editing it as text arrives. `channels.discord.streaming` takes `off` | `partial` | `block` | `progress` (default). `progress` keeps one editable status draft and updates it with tool progress until final delivery; the shared starter label is a rolling line, so it scrolls away like the rest once enough work appears. `streamMode` is a legacy runtime alias. Run `joopo doctor --fix` to rewrite persisted config to the canonical key.

    Set `channels.discord.streaming.mode` to `off` to disable Discord preview edits. If Discord block streaming is explicitly enabled, Joopo skips the preview stream to avoid double-streaming.
...
```
    ...
    Preview streaming is text-only; media replies fall back to normal delivery. When `block` streaming is explicitly enabled, Joopo skips the preview stream to avoid double-streaming.
...
```
    ...
    - Deprecated `spawnSubagentSessions`/`spawnAcpSessions` keys are migrated by `joopo doctor --fix`.
...
```
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while Joopo is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/joopo",
    },
  },
}
```
...
```
  ...
  <Accordion title="Approvals in Discord">
    ...
    When those buttons are present, they are the primary approval UX; Joopo
    ...
    If the Discord native approval runtime is not active, Joopo keeps the
    ...
    Joopo sends a same-chat fallback notice with the exact `/approve`
...
Joopo uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
- If `voice.autoJoin` has multiple entries for the same guild, Joopo joins the last configured channel for that guild.
...
- `voice.reconnectGraceMs` controls how long Joopo waits for a disconnected voice session to begin reconnecting before destroying it. Default: `15000`.
- Voice playback does not stop just because another user starts speaking. To avoid feedback loops, Joopo ignores new voice capture while TTS is playing; speak after playback finishes for the next turn.
- `voice.captureSilenceGraceMs` controls how long Joopo waits after Discord reports a speaker has stopped before finalizing that audio segment for STT. Default: `2500`; raise this if Discord splits normal pauses into choppy partial transcripts.
...
- Joopo also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
- If receive logs repeatedly show `DecryptionFailed(UnencryptedWhenPassthroughDisabled)` after updating, collect a dependency report and logs. The bundled `@discordjs/voice` line includes the upstream padding fix from discord.js PR #11449, which closed discord.js issue #11419.
- `The operation was aborted` receive events are expected when Joopo finalizes a captured speaker segment; they are verbose diagnostics, not warnings.
...
Discord voice messages show a waveform preview and require OGG/Opus audio. Joopo generates the waveform automatically, but needs `ffmpeg` and `ffprobe` on the gateway host to inspect and convert.

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord rejects text + voice message in the same payload).
- Any audio format is accepted; Joopo converts to OGG/Opus as needed.
...
```
  ...
  <Accordion title="Gateway metadata lookup timeout warnings">
    Joopo fetches Discord `/gateway/bot` metadata before connecting. Transient failures fall back to Discord's default gateway URL and are rate-limited in logs.
    ...
    - env fallback when config is unset: `JOOPO_DISCORD_GATEWAY_INFO_TIMEOUT_MS`
    - default: `30000` (30 seconds), max: `120000`

  </Accordion>

  <Accordion title="Gateway READY timeout restarts">
    Joopo waits for Discord's gateway `READY` event during startup and after runtime reconnects. Multi-account setups with startup staggering can need a longer startup READY window than the default.
    ...
    - startup env fallback when config is unset: `JOOPO_DISCORD_READY_TIMEOUT_MS`
    ...
    - runtime env fallback when config is unset: `JOOPO_DISCORD_RUNTIME_READY_TIMEOUT_MS`
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep Joopo current (`joopo update`) so the Discord voice receive recovery logic is present
...
- If command deploy/state is stale, restart gateway and re-check with `joopo channels status --probe`.
