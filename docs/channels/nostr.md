...
read_when:
  - You want Joopo to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Joopo to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Joopo releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`joopo onboard`) and `joopo channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
joopo plugins install @joopo/nostr
```
...
```bash
joopo plugins install --link <path-to-local-nostr-plugin>
```
...
joopo channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "joopo@example.com",
        lud16: "joopo@example.com",
      },
    },
  },
}
