...
On the first agent run, Joopo bootstraps the workspace (default
`~/.joopo/workspace`):
...
For embedded/local model runs, Joopo keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `joopo onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.joopo/workspace`).
</Note>
