---
summary: "Get Joopo installed and run your first chat in minutes."
read_when:
...
Install Joopo, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.joopo/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.joopo/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Joopo as a service account or want custom paths:

- `JOOPO_HOME` — home directory for internal path resolution
- `JOOPO_STATE_DIR` — override the state directory
- `JOOPO_CONFIG_PATH` — override the config file path
