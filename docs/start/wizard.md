...
CLI onboarding is the **recommended** way to set up Joopo on macOS,
...
`joopo dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`joopo configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.joopo/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `joopo doctor` first.
</Note>
...
- CLI command reference: [`joopo onboard`](/cli/onboard)
