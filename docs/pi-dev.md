...
A sane workflow for working on the Pi integration in Joopo.
...
```bash
JOOPO_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm joopo agent --message "Hello" --thinking low`
...
State lives under the Joopo state directory. Default is `~/.joopo`. If `JOOPO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `joopo.json` for config
