...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Joopo
...
Running Joopo via Node with `tsx` fails at startup with:

```
[joopo] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `joopo status` and `gateway:watch` worked.
