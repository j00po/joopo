...
Joopo can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Joopo tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
joopo agent --message "hi" --model codex-cli/gpt-5.5
```
...
gateway host, Joopo now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Joopo will
...
2. **Builds a system prompt** using the same Joopo prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   Joopo session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us Joopo-style Claude CLI usage is allowed again, so Joopo treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes Joopo's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so Joopo writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the Joopo skills snapshot
two ways: the compact Joopo skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Joopo would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Joopo to the
child process environment for the run.

Claude CLI also has its own noninteractive permission mode. Joopo maps that
...
`tools.exec.ask: "off"`), Joopo adds `--permission-mode bypassPermissions`.
...
The bundled Anthropic `claude-cli` backend also maps Joopo `/think` levels
...
Before Joopo can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
joopo models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, Joopo resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; Joopo clamps those settings to 64 MiB and 100,000
  lines.
...
- Joopo drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, Joopo lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), Joopo seeds
...
provider would start cold because Joopo's own session transcript is empty
for `claude-cli` runs.
...
Joopo will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Joopo appends the
...
- For Gemini CLI JSON output, Joopo reads reply text from `response` and
...
- `stats.cached` is normalized into Joopo `cacheRead`.
- If `stats.input` is missing, Joopo derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Joopo handles
...
CLI backends do **not** receive Joopo tool calls directly, but a backend can
...
If no MCP servers are enabled, Joopo still injects a strict config when a
...
- **No direct Joopo tool calls.** Joopo does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. Joopo sessions still work
  normally.
