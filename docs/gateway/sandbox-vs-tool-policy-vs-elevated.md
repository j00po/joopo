...
Joopo has three related (but different) controls:
...
Use the inspector to see what Joopo is _actually_ doing:
...
- Joopo validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:joopo`: all built-in Joopo tools (excludes provider plugins)
