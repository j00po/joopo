---
summary: "Schema-accurate configuration examples for common Joopo setups"
read_when:
  - Learning how to configure Joopo
...
```json5
{
  agent: { workspace: "~/.joopo/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.joopo/joopo.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.joopo/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/joopo" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.joopo/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
