...
read_when:
  - Generating or reviewing `joopo secrets apply` plans
...
`joopo secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `joopo.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `joopo secrets configure` or fix the target path to a supported shape above.
