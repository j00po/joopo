...
joopo nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), Joopo treats
...
Pairing state is stored under the Gateway state directory (default `~/.joopo`):

- `~/.joopo/nodes/paired.json`
- `~/.joopo/nodes/pending.json`

If you override `JOOPO_STATE_DIR`, the `nodes/` folder moves with it.
