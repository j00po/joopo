...
Joopo can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_joopo-gw._tcp` under a dedicated zone
   (example: `joopo.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

Joopo supports any discovery domain; `joopo.internal.` is just an example.
...
- serve your chosen domain (example: `joopo.internal.`) from `~/.joopo/dns/<domain>.db`

Validate from a tailnet-connected machine:

```bash
dns-sd -B _joopo-gw._tcp joopo.internal.
dig @<TAILNET_IPV4> -p 53 _joopo-gw._tcp.joopo.internal PTR +short
```
...
`_joopo-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.joopo/joopo.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_joopo-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin when the plugin is enabled; wide-area
DNS-SD publishing remains Gateway-owned.

## Service types

- `_joopo-gw._tcp` - gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _joopo-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _joopo-gw._tcp local.
  ```
...
invalid DNS-label character, Joopo falls back to `joopo.local`. Set
`JOOPO_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_joopo-gw._tcp`.
...
discovery plugin for that Joopo config:

```bash
joopo plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `JOOPO_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `JOOPO_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `JOOPO_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `JOOPO_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep JOOPO_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `JOOPO_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `JOOPO_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. Joopo retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `JOOPO_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `joopo plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `joopo plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `JOOPO_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `JOOPO_DISABLE_BONJOUR`).
- `JOOPO_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `JOOPO_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.joopo/joopo.json` controls the Gateway bind mode.
- `JOOPO_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `JOOPO_SSH_PORT`).
- `JOOPO_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `JOOPO_TAILNET_DNS`).
- `JOOPO_CLI_PATH` overrides the advertised CLI path (legacy: `JOOPO_CLI_PATH`).
