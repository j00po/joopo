---
summary: "Run multiple Joopo Gateways on one host (isolation, ports, and profiles)"
read_when:
...
joopo --profile rescue onboard
joopo --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `joopo --profile rescue onboard`:
...
`joopo --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.joopo/workspace-rescue`)
...
# extra gateway
joopo --profile ops setup
joopo --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
joopo --profile main setup
joopo --profile main gateway --port 18789
...
joopo --profile ops gateway install --port 19789
```
...
- `JOOPO_CONFIG_PATH` — per-instance config file
- `JOOPO_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `JOOPO_GATEWAY_PORT` / `--port`).
...
```bash
JOOPO_CONFIG_PATH=~/.joopo/main.json \
JOOPO_STATE_DIR=~/.joopo \
joopo gateway --port 18789

JOOPO_CONFIG_PATH=~/.joopo/rescue.json \
JOOPO_STATE_DIR=~/.joopo-rescue \
joopo gateway --port 19789
```
...
joopo --profile rescue gateway status --deep
joopo --profile rescue gateway probe
joopo status
joopo --profile rescue status
joopo --profile rescue browser status
```
