---
summary: "Expose Joopo diagnostics as Prometheus text metrics through the diagnostics-prometheus plugin"
title: "Prometheus metrics"
sidebarTitle: "Prometheus"
read_when:
  - You want Prometheus, Grafana, VictoriaMetrics, or another scraper to collect Joopo Gateway metrics
...
Joopo can expose diagnostics metrics through the official `diagnostics-prometheus` plugin. It listens to trusted internal diagnostics and renders a Prometheus text endpoint at:
...
<Steps>
  ...
  <Step title="Enable the plugin">
    <Tabs>
      ...
      <Tab title="CLI">
        ```bash
        joopo plugins enable diagnostics-prometheus
        ```
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Scrape the protected route">
    ...
    ```bash
    curl -H "Authorization: Bearer $JOOPO_GATEWAY_TOKEN" \
  ...
  <Step title="Wire Prometheus">
    ...
    scrape_configs:
      - job_name: joopo
        ...
        authorization:
          credentials_file: /etc/prometheus/joopo-gateway-token
        static_configs:
          - targets: ["joopo-gateway:18789"]
    ```
  </Step>
</Steps>
...
| `joopo_model_call_duration_seconds`        | histogram | `api`, `error_category`, `model`, `outcome`, `provider`, `transport`                      |
| `joopo_model_tokens_total`                 | counter   | `agent`, `channel`, `model`, `provider`, `token_type`                                     |
| `joopo_gen_ai_client_token_usage`          | histogram | `model`, `provider`, `token_type`                                                         |
| `joopo_model_cost_usd_total`               | counter   | `agent`, `channel`, `model`, `provider`                                                   |
| `joopo_tool_execution_total`               | counter   | `error_category`, `outcome`, `params_kind`, `tool`                                        |
| `joopo_tool_execution_duration_seconds`    | histogram | `error_category`, `outcome`, `params_kind`, `tool`                                        |
| `joopo_harness_run_total`                  | counter   | `channel`, `error_category`, `harness`, `model`, `outcome`, `phase`, `plugin`, `provider` |
| `joopo_harness_run_duration_seconds`       | histogram | `channel`, `error_category`, `harness`, `model`, `outcome`, `phase`, `plugin`, `provider` |
| `joopo_message_processed_total`            | counter   | `channel`, `outcome`, `reason`                                                            |
| `joopo_message_processed_duration_seconds` | histogram | `channel`, `outcome`, `reason`                                                            |
| `joopo_message_delivery_started_total`     | counter   | `channel`, `delivery_kind`                                                                |
| `joopo_message_delivery_total`             | counter   | `channel`, `delivery_kind`, `error_category`, `outcome`                                   |
| `joopo_message_delivery_duration_seconds`  | histogram | `channel`, `delivery_kind`, `error_category`, `outcome`                                   |
| `joopo_talk_event_total`                   | counter   | `brain`, `event_type`, `mode`, `provider`, `transport`                                    |
| `joopo_talk_event_duration_seconds`        | histogram | `brain`, `event_type`, `mode`, `provider`, `transport`                                    |
| `joopo_talk_audio_bytes`                   | histogram | `brain`, `event_type`, `mode`, `provider`, `transport`                                    |
| `joopo_queue_lane_size`                    | gauge     | `lane`                                                                                    |
| `joopo_queue_lane_wait_seconds`            | histogram | `lane`                                                                                    |
| `joopo_session_state_total`                | counter   | `reason`, `state`                                                                         |
| `joopo_session_queue_depth`                | gauge     | `state`                                                                                   |
| `joopo_session_recovery_total`             | counter   | `action`, `active_work_kind`, `state`, `status`                                           |
| `joopo_session_recovery_age_seconds`       | histogram | `action`, `active_work_kind`, `state`, `status`                                           |
| `joopo_memory_bytes`                       | gauge     | `kind`                                                                                    |
| `joopo_memory_rss_bytes`                   | histogram | none                                                                                      |
| `joopo_memory_pressure_total`              | counter   | `level`, `reason`                                                                         |
| `joopo_telemetry_exporter_total`           | counter   | `exporter`, `reason`, `signal`, `status`                                                  |
| `joopo_prometheus_series_dropped_total`    | counter   | none                                                                                      |
...
<AccordionGroup>
  <Accordion title="Bounded, low-cardinality labels">
    ...
    Label values are redacted and must match Joopo's low-cardinality character policy. Values that fail the policy are replaced with `unknown`, `other`, or `none`, depending on the metric.

  </Accordion>
  <Accordion title="Series cap and overflow accounting">
    The exporter caps retained time series in memory at **2048** series across counters, gauges, and histograms combined. New series beyond that cap are dropped, and `joopo_prometheus_series_dropped_total` increments by one each time.
...
sum by (provider) (rate(joopo_model_tokens_total[1m]))

# Spend (USD) over the last hour, by model
sum by (model) (increase(joopo_model_cost_usd_total[1h]))

# 95th percentile model run duration
histogram_quantile(
  0.95,
  sum by (le, provider, model)
    (rate(joopo_run_duration_seconds_bucket[5m]))
)

# Queue wait time SLO (95p under 2s)
histogram_quantile(
  0.95,
  sum by (le, lane) (rate(joopo_queue_lane_wait_seconds_bucket[5m]))
) < 2

# Dropped Prometheus series (cardinality alarm)
increase(joopo_prometheus_series_dropped_total[15m]) > 0
```

<Tip>
Prefer `gen_ai_client_token_usage` for cross-provider dashboards: it follows the OpenTelemetry GenAI semantic conventions and is consistent with metrics from non-Joopo GenAI services.
</Tip>

## Choosing between Prometheus and OpenTelemetry export

Joopo supports both surfaces independently. You can run either, both, or neither.
...
<Tabs>
  ...
  <Tab title="diagnostics-otel">
    - **Push** model: Joopo sends OTLP/HTTP to a collector or OTLP-compatible backend.
...
<AccordionGroup>
  <Accordion title="Empty response body">
    ...
    - Confirm the plugin is enabled and loaded with `joopo plugins list --enabled`.
  ...
  <Accordion title="`joopo_prometheus_series_dropped_total` is climbing">
