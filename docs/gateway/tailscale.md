...
Joopo can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Joopo requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this Joopo Serve/Funnel
mode. `off` means Joopo is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `JOOPO_GATEWAY_TOKEN` is set)
- `password` (shared secret via `JOOPO_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Joopo verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Joopo only treats a request as Serve when it arrives from loopback with
...
Prefer `JOOPO_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
joopo gateway --tailscale serve
joopo gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Joopo to undo `tailscale serve`
