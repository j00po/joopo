...
- Default rolling log file is under `/tmp/joopo/` (one file per day): `joopo-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.joopo/joopo.json`:
  - `logging.file`
  - `logging.level`
...
Joopo can mask sensitive tokens before log or transcript output leaves the
...
`joopo gateway` supports a per-gateway style switch:
