...
- `joopo gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `joopo doctor` reports no blocking config/service issues.
- `joopo channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.

## Split brain installs and newer config guard

Use this when a gateway service unexpectedly stops after an update, or logs show that one `joopo` binary is older than the version that last wrote `joopo.json`.

Joopo stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer Joopo, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which joopo
joopo --version
joopo gateway status --deep
joopo config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `joopo` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `joopo` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `JOOPO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
joopo logs --follow
joopo models status
joopo config get agents.defaults.models
```
...
- Joopo model runs fail only on normal agent turns
...
joopo infer model run --model <provider/model> --prompt "hi" --json
joopo logs --follow
```

Look for:

- direct tiny calls succeed, but Joopo runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Joopo retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but Joopo agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → Joopo transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    2. Set `compat.supportsTools: false` for models/backends that cannot handle Joopo's tool schema surface reliably.
    ...
    4. If tiny direct requests keep passing while Joopo agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
joopo status
joopo channels status --probe
joopo pairing list --channel <channel> [--account <id>]
joopo config get channels
joopo logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `joopo config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `joopo devices list` then `joopo devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `joopo devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `joopo devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
joopo logs --follow
joopo doctor
joopo gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `joopo onboard --mode local` / `joopo setup` to restamp the expected local-mode config. If you are running Joopo via Podman, the default config path is `~/.joopo/joopo.json`.
    ...
    - `System-level Joopo gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `JOOPO_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `joopo doctor --fix` or `joopo gateway install --force`, then restart the gateway service.
...
- A timestamped `joopo.json.rejected.*` file beside the active config
- A timestamped `joopo.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Joopo-owned write.
    - Gateway startup fails closed instead of rewriting `joopo.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Joopo-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `joopo doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(joopo config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Joopo-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Joopo-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `joopo doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `joopo config set` or `config.patch`.
    3. Run `joopo config validate` before restarting.
...
Use this when `joopo gateway probe` reaches something, but still prints a warning block.

```bash
joopo gateway probe
joopo gateway probe --json
joopo gateway probe --ssh user@gateway-host
```
...
joopo cron status
joopo cron list
joopo cron runs --id <jobId> --limit 20
joopo system heartbeat last
joopo logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Joopo skips the model call.
...
```bash
joopo nodes status
joopo nodes describe --node <idOrNameOrIp>
joopo approvals get --node <idOrNameOrIp>
joopo logs --follow
joopo status
```
...
joopo browser start --browser-profile joopo
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `joopo browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Joopo-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    joopo gateway status
    joopo config get gateway.mode
    joopo config get gateway.remote.url
    joopo config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    joopo config get gateway.bind
    joopo config get gateway.auth.mode
    joopo config get gateway.auth.token
    joopo gateway status
    joopo logs --follow
    ```
