...
Joopo supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `JOOPO_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Joopo runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Joopo validates the resolved target path.
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/Joopo QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/joopo"],
...
- Joopo resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__JOOPO_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `joopo.json` refs)
...
When reload-time activation fails after a healthy state, Joopo enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `joopo memory` remote-memory paths and `joopo qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `joopo status`, `joopo status --all`, `joopo channels status`, `joopo channels resolve`, `joopo security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `joopo secrets reload`.
...
<Steps>
  <Step title="Audit current state">
    ```bash
    joopo secrets audit --check
    ```
  </Step>
  <Step title="Configure SecretRefs">
    ```bash
    joopo secrets configure
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
<AccordionGroup>
  <Accordion title="secrets audit">
    ...
    - plaintext values at rest (`joopo.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `joopo.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `joopo secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `joopo.json` plus `auth-profiles.json` for one agent scope
    ...
    - `joopo secrets configure --providers-only`
    - `joopo secrets configure --skip-provider-setup`
    - `joopo secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    joopo secrets apply --from /tmp/joopo-secrets-plan.json
    joopo secrets apply --from /tmp/joopo-secrets-plan.json --allow-exec
    joopo secrets apply --from /tmp/joopo-secrets-plan.json --dry-run
    joopo secrets apply --from /tmp/joopo-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
Joopo intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
