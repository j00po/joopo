---
summary: "Use OpenShell as a managed sandbox backend for Joopo agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Joopo. Instead of running Docker
containers locally, Joopo delegates sandbox lifecycle to the `openshell` CLI,
...
- Joopo Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Joopo creates an OpenShell
...
- You edit files locally outside Joopo and want those changes visible in the
...
- When the sandbox is first created, Joopo seeds the remote workspace from
...
- Joopo does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Joopo after the initial seed, the remote sandbox does **not** see those changes. Use `joopo sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"joopo"`  | Sandbox source for first-time create                  |
...
1. Joopo calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Joopo calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `joopo sandbox` commands
