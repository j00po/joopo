...
Joopo can create a local diagnostics zip for bug reports. It combines
...
2. Joopo sends the diagnostics preamble and asks for one explicit exec
   approval. The approval runs `joopo gateway diagnostics export --json`.
   Do not approve diagnostics through an allow-all rule.
3. After approval, Joopo replies with a pasteable report containing the local
   bundle path, manifest summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Joopo does not
...
that the diagnostics flow was sent privately. If Joopo cannot find a private
owner route, the command fails closed and asks the owner to run it from a DM.

When the active Joopo session is using the native OpenAI Codex harness,
the same exec approval also covers an OpenAI feedback upload for the Codex
runtime threads Joopo knows about. That upload is separate from the local
...
the channels, Joopo session ids, Codex thread ids, and local resume commands
for the threads that were sent to OpenAI servers. If you deny or ignore the
approval, Joopo does not run the export, does not send Codex feedback, and
...
Persisted bundles live under `~/.joopo/logs/stability/` when events exist.

## Useful options

```bash
joopo gateway diagnostics export \
  --output joopo-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
