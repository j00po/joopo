...
- Use the macOS app's **Remote over SSH** mode (Settings → General → Joopo runs).
...
- `joopo health` and `joopo status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `joopo gateway status`, `joopo gateway health`, `joopo gateway probe`, and `joopo gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `JOOPO_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`JOOPO_GATEWAY_URL`) may use env credentials only (`JOOPO_GATEWAY_TOKEN` / `JOOPO_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `JOOPO_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `JOOPO_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `JOOPO_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `JOOPO_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `JOOPO_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `JOOPO_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
  break-glass. There is no `joopo.json` equivalent; this must be process
...
```bash
joopo config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.joopo.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.joopo.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.joopo.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.joopo.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.joopo.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.joopo.ssh-tunnel
```
