...
- If you install or update plugins (`joopo plugins install <package>`, `joopo plugins update <id>`), treat it like running untrusted code:
  ...
  - Gateway-backed skill dependency installs follow the same dangerous/suspicious split: built-in `critical` findings block unless the caller explicitly sets `dangerouslyForceUnsafeInstall`, while suspicious findings still warn only. `joopo skills install` remains the separate JoopoHub skill download/install flow.
