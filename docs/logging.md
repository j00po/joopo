...
read_when:
  - You need a beginner-friendly overview of Joopo logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Joopo has two main log surfaces:
...
Joopo keeps up to five numbered archives beside the active file, such as
`joopo-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.joopo/joopo.json`:

```json
{
  "logging": {
    "file": "/path/to/joopo.log"
  }
}
```
...
or times out before `logs.tail` answers, `joopo logs` falls back to the
...
```bash
joopo channels logs --channel whatsapp
```
...
Joopo preserves the original structured log arguments alongside these fields
...
`joopo gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
joopo gateway
joopo gateway --verbose --ws-log compact
joopo gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.joopo/joopo.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/joopo/joopo-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`JOOPO_LOG_LEVEL`** environment variable (e.g. `JOOPO_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `joopo.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `joopo --log-level debug gateway run`), which overrides the environment variable for that command.
...
Joopo writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
Joopo can redact sensitive tokens before they hit console output, file logs,
...
policy. Joopo still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `JOOPO_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `joopo doctor` first.
