...
read_when:
  - You want Joopo to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, Joopo can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. Joopo may check in afterward.
- You say you are exhausted. Joopo may ask later whether you slept.
- The agent says it will follow up after something changes. Joopo may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: Joopo remembers a
...
```bash
joopo config set commitments.enabled true
joopo config set commitments.maxPerDay 3
```

Equivalent `joopo.json`:
...
After an agent reply, Joopo may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, Joopo stores a commitment with:
...
Joopo never delivers an inferred commitment immediately after writing it.
...
joopo commitments dismiss cm_abc123
```

See [`joopo commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local Joopo state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
joopo config set commitments.enabled false
```
...
- Check `joopo commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`joopo commitments`](/cli/commitments)
