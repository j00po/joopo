...
- `/usage cost` in chats: local cost summary aggregated from Joopo session logs.
- CLI: `joopo status --usage` prints a full per-provider breakdown.
- CLI: `joopo channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Joopo treats
  ...
  quota, so Joopo inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Joopo prefers the
...
can supply plugin-specific usage auth logic; otherwise Joopo falls back to
