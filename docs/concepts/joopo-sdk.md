---
summary: "Public Joopo App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "Joopo App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Joopo
...
The **Joopo App SDK** is the public client API for apps outside the
Joopo process. Use `@joopo/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@joopo/sdk` talks to the Gateway from outside Joopo.
  `joopo/plugin-sdk/*` is only for plugins that run inside Joopo and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@joopo/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `Joopo`                | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `JoopoEvent`,
`JoopoEventType`, `GatewayEvent`, `JoopoTransport`,
...
`new Joopo({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `JoopoTransport`:
...
The SDK normalizes raw Gateway events into a stable `JoopoEvent` envelope:
...
Use the App SDK when code lives outside Joopo:
...
Use the Plugin SDK when code runs inside Joopo:
...
App SDK code should import from `@joopo/sdk`. Plugin code should import from
documented `joopo/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [Joopo App SDK API design](/reference/joopo-sdk-api-design)
