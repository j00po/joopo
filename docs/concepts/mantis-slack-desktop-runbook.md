...
Linux desktop, VNC rescue, Slack Web, a real Joopo gateway, screenshots,
...
- Mantis artifacts: owned by the Joopo run. They live under
...
- remote logs such as `slack-desktop-command.log`, `joopo-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, Joopo install/build or
...
remote status after Mantis has copied metadata proving that the Joopo gateway
