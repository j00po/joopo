...
`node-llama-cpp` runtime package next to Joopo, then point `local.modelPath`
at a GGUF file:
...
Joopo indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.joopo/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `joopo memory index --force`
...
**Memory search disabled?** Check `joopo memory status`. If no provider is
...
**Stale results?** Run `joopo memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Joopo falls back to in-process cosine similarity
automatically. `joopo memory status --deep` reports the local vector store
