...
[Honcho](https://honcho.dev) adds AI-native memory to Joopo. It persists
...
```bash
joopo plugins install @honcho-ai/joopo-honcho
joopo honcho setup
joopo gateway --force
```
...
Settings live under `plugins.entries["joopo-honcho"].config`:
    ...
    entries: {
      "joopo-honcho": {
        config: {
          ...
          workspaceId: "joopo", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `joopo honcho setup` detects and
...
```bash
joopo honcho setup                        # Configure API key and migrate files
joopo honcho status                       # Check connection status
joopo honcho ask <question>               # Query Honcho about the user
joopo honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/joopo-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Joopo integration guide](https://docs.honcho.dev/v3/guides/integrations/joopo)
- [Memory](/concepts/memory) -- Joopo memory overview
