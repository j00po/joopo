---
summary: "How Joopo remembers things across sessions"
...
Joopo remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.joopo/workspace`).
...
for that case. Joopo infers them in a hidden background pass, scopes them to
...
<Info>
Joopo auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Joopo
...
```bash
joopo memory rem-backfill --rollback
joopo memory rem-backfill --rollback-short-term
```

## CLI

```bash
joopo memory status          # Check index status and provider
joopo memory search "query"  # Search from the command line
joopo memory index --force   # Rebuild the index
```
