...
The private QA stack is meant to exercise Joopo in a more realistic,
...
Every QA flow runs under `pnpm joopo qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm joopo qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
...
```bash
pnpm joopo qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`joopo.run`, `joopo.harness.run`, `joopo.model.call`,
`joopo.context.assembled`, and `joopo.message.delivery` must be present;
model calls must not export `StreamAbandoned` on successful turns; raw diagnostic IDs and
`joopo.content.*` attributes must stay out of the trace. It writes
...
```bash
pnpm joopo qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `JOOPO_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, and Slack smoke lanes:

```bash
pnpm joopo qa telegram
pnpm joopo qa discord
pnpm joopo qa slack
```
...
```bash
pnpm joopo qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent Joopo Slack gateway
...
```bash
pnpm joopo qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `joopo infer image describe`
...
```bash
pnpm joopo qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Joopo
...
- `JOOPO_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `JOOPO_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `JOOPO_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `JOOPO_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
- `telegram-qa-observed-messages.json` - bodies redacted unless `JOOPO_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child Joopo gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `JOOPO_QA_DISCORD_GUILD_ID`
- `JOOPO_QA_DISCORD_CHANNEL_ID`
- `JOOPO_QA_DISCORD_DRIVER_BOT_TOKEN`
- `JOOPO_QA_DISCORD_SUT_BOT_TOKEN`
- `JOOPO_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `JOOPO_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `JOOPO_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `JOOPO_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child Joopo gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `JOOPO_QA_SLACK_CHANNEL_ID`
- `JOOPO_QA_SLACK_DRIVER_BOT_TOKEN`
- `JOOPO_QA_SLACK_SUT_BOT_TOKEN`
- `JOOPO_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `JOOPO_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
...
- `slack-qa-observed-messages.json` - bodies redacted unless `JOOPO_QA_SLACK_CAPTURE_CONTENT=1`.
...
{
  "display_information": {
    "name": "Joopo QA Driver",
    "description": "Test driver bot for Joopo QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Joopo QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
```json
{
  "display_information": {
    "name": "Joopo QA SUT",
    "description": "Joopo QA SUT connector for Joopo"
  },
  "features": {
    "bot_user": {
      "display_name": "Joopo QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#joopo-qa`) and invite both bots from inside the channel:

```
/invite @Joopo QA Driver
/invite @Joopo QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `JOOPO_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `JOOPO_QA_CONVEX_SITE_URL` and `JOOPO_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm joopo qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm joopo qa credentials list --kind slack --status all --json
```
...
- `mock-openai` is the scenario-aware Joopo mock. It remains the default
...
- how `joopo qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `joopo qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `joopo.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm joopo qa coverage` (add `--json` for machine-readable output).

For character and style checks, run the same scenario across multiple live model
refs and write a judged Markdown report:

```bash
pnpm joopo qa character-eval \
