---
summary: "Move one Joopo session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Joopo session.
...
Alice can message Joopo on Telegram and Discord:
...
Joopo keeps the current session context and changes the reply route:
...
4. Keep the same Joopo session, but receive future replies in Discord.
