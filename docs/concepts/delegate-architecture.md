---
summary: "Delegate architecture: running Joopo as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Joopo as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Joopo agent that:
...
Joopo's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Joopo's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.joopo/workspace-delegate",
  tools: {
...
- Cron run history: `~/.joopo/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.joopo/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Joopo's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
joopo agents add delegate
```

This creates:

- Workspace: `~/.joopo/workspace-delegate`
- State: `~/.joopo/agents/delegate/agent`
- Sessions: `~/.joopo/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.joopo/workspace" },
      {
        id: "delegate",
        workspace: "~/.joopo/workspace-delegate",
        tools: {
...
~/.joopo/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.joopo/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.joopo/workspace-org",
        agentDir: "~/.joopo/agents/org-assistant/agent",
...
recall view. Joopo redacts credential/token-like text, truncates long
