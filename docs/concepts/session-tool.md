...
Joopo gives agents tools to work across sessions, inspect status, and
...
After the target responds, Joopo can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `joopo-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Joopo
...
the completion origin only identifies a channel Joopo can still reuse the
