---
summary: "What the Joopo system prompt contains and how it is assembled"
read_when:
...
Joopo builds a custom system prompt for every agent run. The prompt is **Joopo-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Joopo and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full Joopo-owned prompt. The provider runtime can:
...
- **Joopo Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Joopo docs (repo or npm package) and when to read them.
...
Joopo keeps large stable content, including **Project Context**, above the
...
Joopo can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **Joopo
...
For channel auto-reply runs, Joopo can omit the generic **Silent Replies**
...
Joopo keeps committed prompt snapshots for the Codex runtime happy path under
...
Joopo developer instructions, turn-scoped collaboration-mode instructions
when Joopo provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after Joopo sends
...
through its own project-doc discovery. Joopo still resolves the remaining
...
occurs, Joopo can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, Joopo injects a compact **available skills list**
...
points to the local Joopo docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.joopo.ai](https://docs.joopo.ai).
