...
Paste this into `joopo.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Joopo can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Joopo cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. Joopo
...
5. Verify memory search itself works with `joopo memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Joopo auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `joopo memory status --deep` to inspect the memory-search backend
      and index health.
