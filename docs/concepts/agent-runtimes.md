---
summary: "How Joopo separates model providers, models, channels, and agent runtimes"
...
| ------------- | ------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How Joopo authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                              |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave Joopo.                            |
...
implements the `codex` runtime. Public config uses `agentRuntime.id`; `joopo
doctor --fix` rewrites older runtime-policy keys to that shape.

There are two runtime families:

- **Embedded harnesses** run inside Joopo's prepared agent loop. Today this
...
| Surface                                          | Joopo name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `joopo doctor --fix` owns legacy
...
That means Joopo selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Joopo session store
becomes Codex.
...
4. If legacy config still contains **`openai-codex/*` model refs**, repair it to
   `openai/<model>` with `joopo doctor --fix`.
...
| Surface                     | Joopo PI embedded                    | Codex app-server                                                            |
| --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Joopo through the PI embedded runner | Codex app-server                                                            |
| Canonical thread state      | Joopo transcript                     | Codex thread, plus Joopo transcript mirror                               |
| Joopo dynamic tools      | Native Joopo tool loop               | Bridged through the Codex adapter                                           |
| Native shell and file tools | PI/Joopo path                        | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Joopo context assembly        | Joopo projects assembled context into the Codex turn                     |
| Compaction                  | Joopo or selected context engine     | Codex-native compaction, with Joopo notifications and mirror maintenance |
| Channel delivery            | Joopo                                | Joopo                                                                    |

This ownership split is the main design rule:

- If Joopo owns the surface, Joopo can provide normal plugin hook behavior.
- If the native runtime owns the surface, Joopo needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Joopo should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Joopo chooses an embedded runtime after provider and model resolution:
...
2. `JOOPO_AGENT_RUNTIME=<id>` forces that runtime for new or reset sessions.
...
4. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
5. If no runtime claims a turn in `auto` mode, Joopo uses PI as the
...
Joopo routes PI internally through the legacy Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI PI session pins without
explicit config are repaired back to Codex.

If `joopo doctor` warns that the `codex` plugin is enabled while
`openai-codex/*` remains in config, treat that as legacy route state. Run
`joopo doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## Compatibility contract

When a runtime is not PI, it should document what Joopo surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Joopo can edit history or only mirror it.                                   |
| Do Joopo dynamic tools work?        | Messaging, sessions, cron, and Joopo-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Joopo-owned tools. |
