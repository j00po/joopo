...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.joopo/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.joopo/agents/<agentId>/sessions`.
...
a local profile, but Joopo does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.joopo/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.joopo` (or `JOOPO_STATE_DIR`)
- Workspace: `~/.joopo/workspace` (or `~/.joopo/workspace-<agentId>`)
- Agent dir: `~/.joopo/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.joopo/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Joopo runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.joopo/workspace` (or `~/.joopo/workspace-<profile>` when `JOOPO_PROFILE` is set).
- State defaults to `~/.joopo/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    joopo agents add coding
    joopo agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.joopo/agents/<agentId>`.
  ...
  <Step title="Create channel accounts">
    ...
    ```bash
    joopo channels login --channel whatsapp --account work
    ```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.joopo/workspace-alex" },
      { id: "mia", workspace: "~/.joopo/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Joopo upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Joopo falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.joopo/workspace-main" },
          { id: "coding", workspace: "~/.joopo/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.joopo/workspace-main" },
          { id: "alerts", workspace: "~/.joopo/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    joopo channels login --channel whatsapp --account personal
    joopo channels login --channel whatsapp --account biz
    ```

    `~/.joopo/joopo.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.joopo/workspace-home",
            agentDir: "~/.joopo/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.joopo/workspace-work",
            agentDir: "~/.joopo/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.joopo/credentials/whatsapp/personal
              // authDir: "~/.joopo/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.joopo/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.joopo/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.joopo/workspace-opus",
