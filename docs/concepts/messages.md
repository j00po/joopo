...
Joopo handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. Joopo keeps a
...
Joopo keeps that boundary explicit:
...
Joopo separates the **prompt body** from the **command body**:
...
Joopo can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, Joopo
strips the silent text but still delivers the media attachment.
Joopo resolves that behavior by conversation type:
...
Joopo also uses silent replies for internal runner failures that happen
