---
summary: "Use SOUL.md to give your Joopo agent an actual voice instead of generic assistant sludge"
read_when:
...
Joopo injects it on normal sessions, so it has real weight. If your agent
...
For Joopo, `SOUL.md` is that layer.
...
Path fixed for Joopo workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files Joopo injects into the system prompt.
  </Card>
