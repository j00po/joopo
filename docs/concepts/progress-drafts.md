...
read_when:
  ...
  - Explaining how Joopo updates one channel message while work is in progress
...
When progress drafts are enabled, Joopo creates one visible work-in-progress
...
That is usually enough. Joopo will pick an automatic one-word label, wait
...
Joopo sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from Joopo's built-in
...
Joopo uses the same formatter for progress drafts and `/verbose`:
...
Joopo truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, Joopo still suppresses the older standalone
...
When the final answer is ready, Joopo tries to keep the chat clean:

- If the draft can safely become the final answer, Joopo edits it in place.
- If the channel uses native progress streaming, Joopo finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, Joopo sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, Joopo keeps the
