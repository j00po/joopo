...
alongside Joopo. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Joopo falls back to the
...
Joopo creates a self-contained QMD home under
`~/.joopo/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Joopo creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so Joopo skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, Joopo retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, Joopo groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, Joopo falls back to the builtin SQLite engine.
  ...
  `joopo memory status` and one-shot CLI probes still recheck QMD directly.
...
Joopo keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Joopo checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Joopo searches all
...
Older QMD builds only accept one collection filter. When Joopo detects one
...
process, so you can tune QMD globally without adding new Joopo config:
...
collection under `~/.joopo/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Joopo logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Joopo
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Joopo still reports
...
with `joopo memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Joopo uses.

**Many QMD subprocesses during search?** Update QMD if possible. Joopo uses
...
`memory.qmd.searchMode = "search"`. Joopo treats that mode as lexical-only,
...
the next startup reconciliation. Joopo recreates stale managed collections
...
Joopo's builtin symlink rules. Keep temporary monorepo checkouts under
