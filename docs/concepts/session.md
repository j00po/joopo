---
summary: "How Joopo manages conversation sessions"
read_when:
...
Joopo organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `joopo security audit`.
...
Joopo automatically bounds session storage over time. By default, it runs
...
`joopo sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `joopo sessions cleanup --dry-run`.

## Inspecting sessions

- `joopo status` -- session store path and recent activity.
- `joopo sessions --json` -- all sessions (filter with `--active <minutes>`).
