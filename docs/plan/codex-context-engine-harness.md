...
summary: "Specification for making the bundled Codex app-server harness honor Joopo context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Joopo context-engine
...
Joopo-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Joopo-controlled code. The external
boundary is the Codex app-server protocol itself: Joopo can control what it
...
For Codex harness turns, Joopo should preserve this lifecycle:

1. Read the mirrored Joopo session transcript.
...
8. Mirror the Codex result back into the Joopo transcript.
...
11. Preserve Codex native compaction signals and Joopo compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Joopo should
not try to mutate the app-server's internal history except through supported
protocol calls.

Joopo's transcript mirror remains the source for Joopo features:
...
The context-engine interface returns Joopo `AgentMessage[]`, not a Codex
...
- Label it clearly as Joopo assembled context.
...
```text
Joopo assembled context for this turn:
...
inside Joopo and preserves context-engine semantics.
...
1. Joopo context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Joopo compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Joopo compaction should prefer the context engine's `compact()` result for
the Joopo transcript mirror and plugin state.
...
the Joopo session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Joopo session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Joopo compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Joopo core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Joopo/plugin state.
