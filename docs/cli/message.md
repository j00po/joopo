---
summary: "CLI reference for `joopo message` (send + channel actions)"
read_when:
...
```
joopo message <subcommand> [flags]
```
...
- `joopo message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, Joopo will attempt a live directory lookup when the provider supports it.
...
- `joopo message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Joopo does not force a `default` account SecretRef scope
...
```
joopo message send --channel discord \
...
```bash
joopo message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
joopo message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
joopo message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
joopo message poll --channel msteams \
...
```
joopo message react --channel slack \
...
```
joopo message react --channel signal \
...
```
joopo message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
joopo message send --channel telegram --target @mychat \
