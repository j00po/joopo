...
read_when:
  - You run joopo with no command and want to understand Crestodian
  - You need a configless-safe way to inspect or repair Joopo
...
Crestodian is Joopo's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `joopo` with no command starts Crestodian in an interactive terminal.
Running `joopo crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`joopo tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same Joopo reference discovery as regular agents. In a Git checkout,
...
[https://github.com/joopo/joopo](https://github.com/joopo/joopo), with explicit
...
joopo
joopo crestodian
joopo crestodian --json
joopo crestodian --message "models"
joopo crestodian --message "validate config"
joopo crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
joopo crestodian --message "set default model openai/gpt-5.5" --yes
joopo onboard --modern
```
...
config set-ref gateway.auth.token env JOOPO_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
- `joopo.json` is missing
- `joopo.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`joopo --help` and `joopo --version` still use the normal fast paths.
Noninteractive `joopo` exits with a short message instead of printing root
...
```text
~/.joopo/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`joopo onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `joopo onboard` still runs classic onboarding.
...
planner turn through Joopo's normal runtime paths. It first uses the
configured Joopo model. If no configured model is usable yet, it can fall
...
`joopo tui`, `joopo chat`, and `joopo terminal` still open the normal
...
Joopo: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
Joopo: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
Joopo: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `joopo` for agent handoff.
...
That lane starts with an empty state dir, routes bare `joopo` to Crestodian,
