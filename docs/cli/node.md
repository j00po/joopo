---
summary: "CLI reference for `joopo node` (headless node host)"
read_when:
...
`joopo node run` and `joopo node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `JOOPO_GATEWAY_TOKEN` / `JOOPO_GATEWAY_PASSWORD` are checked first.
...
network, set `JOOPO_ALLOW_INSECURE_PRIVATE_WS=1`. Without it, node startup
fails closed and asks you to use `wss://`, an SSH tunnel, or Tailscale.
This is a process-environment opt-in, not an `joopo.json` config key.
`joopo node install` persists it into the supervised node service when it is
...
```bash
joopo node install --host <gateway-host> --port 18789
```
...
joopo node stop
joopo node restart
joopo node uninstall
```

Use `joopo node run` for a foreground node host (no service).
...
- `joopo approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Joopo prepares a canonical `systemRunPlan`
