---
summary: "CLI reference for `joopo memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
joopo memory status
joopo memory status --deep
joopo memory status --fix
joopo memory index --force
joopo memory search "meeting notes"
joopo memory search --query "deployment" --max-results 20
joopo memory promote --limit 10 --min-score 0.75
joopo memory promote --apply
joopo memory promote --json --min-recall-count 0 --min-unique-queries 0
joopo memory promote-explain "router vlan"
joopo memory promote-explain "router vlan" --json
joopo memory rem-harness
joopo memory rem-harness --json
joopo memory status --json
joopo memory status --deep --index
joopo memory status --deep --index --verbose
joopo memory status --agent main
joopo memory index --agent main --verbose
```
...
```bash
joopo memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `joopo cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
joopo memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
joopo memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
