---
summary: "CLI reference for `joopo approvals` and `joopo exec-policy`"
read_when:
...
`joopo exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.joopo/exec-approvals.json`

Examples:

```bash
joopo exec-policy show
joopo exec-policy show --json

joopo exec-policy preset yolo
joopo exec-policy preset cautious --json

joopo exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `joopo exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `joopo approvals set --gateway`
or `joopo approvals set --node <id|name|ip>`.

## Common commands

```bash
joopo approvals get
joopo approvals get --node <id|name|ip>
joopo approvals get --gateway
```

`joopo approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
joopo approvals set --file ./exec-approvals.json
joopo approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
joopo approvals set --node <id|name|ip> --file ./exec-approvals.json
joopo approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Joopo policy aligned, also set:
...
```bash
joopo approvals allowlist add "~/Projects/**/bin/rg"
joopo approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
joopo approvals allowlist add --agent "*" "/usr/bin/uname"

joopo approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `joopo nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.joopo/exec-approvals.json`.
