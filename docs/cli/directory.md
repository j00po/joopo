---
summary: "CLI reference for `joopo directory` (self, peers, groups)"
read_when:
...
- `directory` is meant to help you find IDs you can paste into other commands (especially `joopo message send --target ...`).
...
```bash
joopo directory peers list --channel slack --query "U0"
joopo message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
joopo directory peers list --channel zalouser
joopo directory peers list --channel zalouser --query "name"
joopo directory peers list --channel zalouser --limit 50
```

## Groups

```bash
joopo directory groups list --channel zalouser
