---
summary: "CLI reference for `joopo daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `joopo daemon ...` in scripts
...
`joopo daemon ...` maps to the same service control surface as `joopo gateway ...` service commands.

## Usage

```bash
joopo daemon status
joopo daemon install
joopo daemon start
joopo daemon stop
joopo daemon restart
joopo daemon uninstall
```
...
Use [`joopo gateway`](/cli/gateway) for current docs and examples.
