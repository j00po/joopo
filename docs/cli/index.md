---
summary: "Joopo CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `joopo` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`joopo` is the main CLI entry point. Each core command has either a
...
```
joopo [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
Plugins can add additional top-level commands (for example `joopo voicecall`).
...
`joopo status --usage` and the Control UI surface provider usage/quota when
