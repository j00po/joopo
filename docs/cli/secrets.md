---
summary: "CLI reference for `joopo secrets` (reload, audit, configure, apply)"
read_when:
...
Use `joopo secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
joopo secrets reload
joopo secrets reload --json
joopo secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Joopo state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `joopo.json` refs)
...
joopo secrets audit
joopo secrets audit --check
joopo secrets audit --json
joopo secrets audit --allow-exec
```
...
```bash
joopo secrets configure
joopo secrets configure --plan-out /tmp/joopo-secrets-plan.json
joopo secrets configure --apply --yes
joopo secrets configure --providers-only
joopo secrets configure --skip-provider-setup
joopo secrets configure --agent ops
joopo secrets configure --json
```
...
- `configure` targets secret-bearing fields in `joopo.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Joopo fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
joopo secrets apply --from /tmp/joopo-secrets-plan.json --json
```
...
- `joopo.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.joopo/.env` known secret keys whose values were migrated
