---
summary: "CLI reference for `joopo onboard` (interactive onboarding)"
read_when:
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Joopo onboarding fits together.
  </Card>
...
joopo onboard --flow quickstart
joopo onboard --flow manual
joopo onboard --flow import
joopo onboard --import-from hermes --import-source ~/.hermes
joopo onboard --skip-bootstrap
joopo onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh Joopo setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `joopo onboard` keeps the classic onboarding flow.

For plaintext private-network `ws://` targets (trusted networks only), set
`JOOPO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
There is no `joopo.json` equivalent for this client-side transport
break-glass.
...
Joopo marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `joopo gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `joopo dashboard` (Control UI, no channel setup).
