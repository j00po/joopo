---
summary: "CLI reference for `joopo models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
joopo models status
joopo models list
joopo models set <model-or-alias>
joopo models scan
```

`joopo models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Joopo falls back to matching OAuth/API-key
...
the command uses `JOOPO_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For Codex OAuth troubleshooting, `joopo models status`,
`joopo models auth list --provider openai-codex`, and
`joopo config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, Joopo resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Joopo
...
By default Joopo tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `JOOPO_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Joopo could not resolve a probeable
...
```bash
joopo models auth add
joopo models auth list [--provider <id>] [--json]
joopo models auth login --provider <id>
joopo models auth setup-token --provider <id>
joopo models auth paste-token
```
...
`joopo plugins list` to see which providers are installed.
Use `joopo models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
joopo models auth login --provider openai-codex --set-default
...
- Anthropic note: Anthropic staff told us Joopo-style Claude CLI usage is allowed again, so Joopo treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Joopo token path, but Joopo now prefers Claude CLI reuse and `claude -p` when available.
