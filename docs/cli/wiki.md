---
summary: "CLI reference for `joopo wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `joopo wiki`
title: "Wiki"
---
...
Use `joopo wiki` when you want a compiled knowledge vault with:
...
joopo wiki doctor
joopo wiki init
joopo wiki ingest ./notes/alpha.md
joopo wiki compile
joopo wiki lint
joopo wiki search "alpha"
joopo wiki search "who should I ask about Teams?" --mode route-question
joopo wiki get entity.alpha --from 1 --lines 80

joopo wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

joopo wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

joopo wiki bridge import
joopo wiki unsafe-local import

joopo wiki obsidian status
joopo wiki obsidian search "alpha"
joopo wiki obsidian open syntheses/alpha-summary.md
joopo wiki obsidian command workspace:quick-switcher
joopo wiki obsidian daily
```
...
- `.joopo-wiki/cache/agent-digest.json`
- `.joopo-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `joopo memory search` when the
...
```bash
joopo wiki search "bgroux" --mode find-person
joopo wiki search "who knows Teams rollout?" --mode route-question
joopo wiki search "maintainer-whois" --mode source-evidence
joopo wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
joopo wiki get entity.alpha
joopo wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`joopo wiki` behavior is shaped by:
