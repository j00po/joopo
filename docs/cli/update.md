---
summary: "CLI reference for `joopo update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `joopo update` output or options
...
Safely update Joopo and switch between stable/beta/dev channels.
...
joopo update status
joopo update wizard
joopo update --channel beta
joopo update --channel dev
joopo update --tag beta
joopo update --tag main
joopo update --dry-run
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:joopo/joopo#main`.
...
`joopo update` does not have a `--verbose` flag. Use `--dry-run` to preview
the planned channel/tag/install/restart actions, `--json` for machine-readable
results, and `joopo update status --json` when you only need channel and
...
<Note>
In Nix mode (`JOOPO_NIX_MODE=1`), mutating `joopo update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-joopo, use the agent-first [Quick Start](https://github.com/joopo/nix-joopo#quick-start). `joopo update status` and `joopo update --dry-run` remain read-only.
</Note>
...
```bash
joopo update status
joopo update status --json
joopo update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), Joopo also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/joopo`, override with `JOOPO_GIT_DIR`),
...
For package-manager installs, `joopo update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: Joopo installs the new package into a temporary npm prefix, verifies
...
installed Joopo build while leaving full plugin-command completion rebuilds to
explicit `joopo completion --write-state` runs.
...
healthy. If the plist is installed but launchd is not supervising it, Joopo
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `JOOPO_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `joopo doctor` runs as the final safe-update check.
  </Step>
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `joopo update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying that you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin are reported as warnings after the core update succeeds. The JSON result keeps the top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `joopo doctor --fix` and `joopo plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `joopo doctor --fix` or `joopo update`.
...
`joopo --update` rewrites to `joopo update` (useful for shells and launcher scripts).

## Related

- `joopo doctor` (offers to run update first on git checkouts)
