---
summary: "CLI reference for `joopo agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
joopo agent --to +15555550123 --message "status update" --deliver
joopo agent --agent ops --message "Summarize logs"
joopo agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
joopo agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Joopo persists markers from the active source config snapshot, not from resolved runtime secret values.
