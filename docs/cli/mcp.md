---
summary: "Expose Joopo channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Joopo-backed channels
  - Running `joopo mcp serve`
  - Managing Joopo-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`joopo mcp` has two jobs:

- run Joopo as an MCP server with `joopo mcp serve`
- manage Joopo-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is Joopo acting as an MCP server
- `list` / `show` / `set` / `unset` is Joopo acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`joopo acp`](/cli/acp) when Joopo should host a coding harness session itself and route that runtime through ACP.

## Joopo as an MCP server

This is the `joopo mcp serve` path.

### When to use `serve`

Use `joopo mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Joopo-backed channel conversations
- you already have a local or remote Joopo Gateway with routed sessions
- you want one MCP server that works across Joopo's channel backends instead of running separate per-channel bridges

Use [`joopo acp`](/cli/acp) instead when Joopo should host the coding runtime itself and keep the agent session inside Joopo.

### How it works

`joopo mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Joopo Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `joopo mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Joopo Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `joopo agent` and `joopo infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Joopo (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Joopo already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    joopo mcp serve --url wss://gateway-host:18789 --token-file ~/.joopo/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    joopo mcp serve --url wss://gateway-host:18789 --password-file ~/.joopo/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    joopo mcp serve --verbose
    joopo mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Joopo equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Joopo channel configuration
...
Joopo ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `joopo mcp serve`
...
## Joopo as an MCP client registry

This is the `joopo mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Joopo over MCP. They manage Joopo-owned MCP server definitions under `mcp.servers` in Joopo config.

Those saved definitions are for runtimes that Joopo launches or configures later, such as embedded Pi and other runtime adapters. Joopo stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Joopo config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes Joopo `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

### Saved MCP server definitions

Joopo also stores a lightweight MCP server registry in config for surfaces that want Joopo-managed MCP definitions.

Commands:

- `joopo mcp list`
- `joopo mcp show [name]`
- `joopo mcp set <name> <json>`
- `joopo mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `joopo mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
joopo mcp list
joopo mcp show context7 --json
joopo mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
joopo mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
joopo mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Joopo uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |

Joopo config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `joopo mcp set` and repaired by `joopo doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
