...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Joopo Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`joopo acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Joopo channel
conversations instead of hosting an ACP harness session, use
[`joopo mcp serve`](/cli/mcp) instead.
...
- Joopo acts as an ACP server
- an IDE or ACP client connects to Joopo
- Joopo forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Joopo runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Joopo: use `joopo acp`
- Joopo should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Joopo gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
joopo acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
joopo acp --url wss://gateway-host:18789 --token-file ~/.joopo/gateway.token

# Attach to an existing session key
joopo acp --session agent:main:main

# Attach by label (must already exist)
joopo acp --session-label "support inbox"

# Reset the session key before the first prompt
joopo acp --session agent:main:main --reset-session
```
...
joopo acp client --server-args --url wss://gateway-host:18789 --token-file ~/.joopo/gateway.token

# Override the server command (default: joopo)
joopo acp client --server "node" --server-args joopo.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Joopo through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`joopo acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `joopo gateway call sessions.list` as the only ACP proof. That
...
it to drive an Joopo Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `joopo acp` over stdio.

Example config (persisted):

```bash
joopo config set gateway.remote.url wss://gateway-host:18789
joopo config set gateway.remote.token <token>
```
...
```bash
joopo acp --session agent:main:main
joopo acp --session agent:design:main
joopo acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Joopo plugin tools or selected
...
[Joopo tools MCP bridge](/tools/acp-agents-setup#joopo-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Joopo bot over ACP, use `acpx` with its built-in `joopo` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx joopo` at `joopo acp`.
3. Target the Joopo session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Joopo ACP session
acpx joopo exec "Summarize the active Joopo session state."

# Persistent named session for follow-up turns
acpx joopo sessions ensure --name codex-bridge
acpx joopo -s codex-bridge --cwd /path/to/repo \
  "Ask my Joopo work agent for recent context relevant to this repo."
```

If you want `acpx joopo` to target a specific Gateway and session key every
time, override the `joopo` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "joopo": {
      "command": "env JOOPO_HIDE_BANNER=1 JOOPO_SUPPRESS_NOTES=1 joopo acp --url ws://127.0.0.1:18789 --token-file ~/.joopo/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Joopo checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env JOOPO_HIDE_BANNER=1 JOOPO_SUPPRESS_NOTES=1 node joopo.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Joopo agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Joopo ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`JOOPO_GATEWAY_TOKEN`, `JOOPO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`JOOPO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `JOOPO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `joopo acp client` sets `JOOPO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `joopo`).
