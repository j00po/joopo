---
summary: "CLI reference for `joopo plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
joopo plugins search <query>
joopo plugins search <query> --limit 20
joopo plugins search <query> --json
joopo plugins install <path-or-spec>
joopo plugins inspect <id>
joopo plugins inspect <id> --runtime
joopo plugins inspect <id> --json
joopo plugins inspect --all
joopo plugins info <id>
joopo plugins enable <id>
joopo plugins disable <id>
joopo plugins registry
joopo plugins registry --refresh
joopo plugins uninstall <id>
joopo plugins doctor
joopo plugins update <id-or-npm-spec>
joopo plugins update --all
joopo plugins marketplace list <marketplace>
joopo plugins marketplace list <marketplace> --json
```

For slow install, inspect, uninstall, or registry-refresh investigation, run the
command with `JOOPO_PLUGIN_LIFECYCLE_TRACE=1`. The trace writes phase timings
to stderr and keeps JSON output parseable. See [Debugging](/help/debugging#plugin-lifecycle-trace).

<Note>
In Nix mode (`JOOPO_NIX_MODE=1`), plugin lifecycle mutators are disabled. Use the Nix source for this install instead of `plugins install`, `plugins update`, `plugins uninstall`, `plugins enable`, or `plugins disable`; for nix-joopo, use the agent-first [Quick Start](https://github.com/joopo/nix-joopo#quick-start).
</Note>

<Note>
Bundled plugins ship with Joopo. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.

Native Joopo plugins must ship `joopo.plugin.json` with an inline JSON Schema (`configSchema`, even if empty). Compatible bundles use their own bundle manifests instead.

`plugins list` shows `Format: joopo` or `Format: bundle`. Verbose list/info output also shows the bundle subtype (`codex`, `claude`, or `cursor`) plus detected bundle capabilities.
</Note>
...
joopo plugins install git:github.com/<owner>/<repo>@<ref>
joopo plugins install <package> --force              # overwrite existing install
joopo plugins install <package> --pin                # pin version
joopo plugins install <package> --dangerously-force-unsafe-install
joopo plugins install <path>                         # local path
joopo plugins install <plugin>@<marketplace>         # marketplace
joopo plugins install <plugin> --marketplace <name>  # marketplace (explicit)
joopo plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
```
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `joopo.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid during install, `plugins install` normally fails closed and tells you to run `joopo doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `joopo doctor --fix` can quarantine the invalid plugin entry. The only documented install-time exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `joopo.install.allowInvalidConfigRecovery`.
  ...
  <Accordion title="Hook packs and npm specs">
    ...
    Bare specs and `@latest` stay on the stable track. Joopo date-stamped correction versions such as `2026.5.3-1` are stable releases for this check. If npm resolves either of those to a prerelease, Joopo stops and asks you to opt in explicitly with a prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as `@1.2.3-beta.4`.

    If a bare install spec matches an official plugin id (for example `diffs`), Joopo installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer. That means manifest validation, dangerous-code scanning, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `joopo plugins update` can re-resolve the source later.

    After installing from git, use `joopo plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, execute that command directly through the Joopo root CLI, for example `joopo demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native Joopo plugin archives must contain a valid `joopo.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before Joopo writes install records.
...
```bash
joopo plugins install npm:joopo-codex-app-server
...
```bash
joopo plugins marketplace list <marketplace-name>
joopo plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
joopo plugins install <plugin-name> --marketplace <marketplace-name>
joopo plugins install <plugin-name> --marketplace <owner/repo>
joopo plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
joopo plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. Joopo accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, Joopo auto-detects:

- native Joopo plugins (`joopo.plugin.json`)
...
`/app/extensions/synology-chat`. Joopo will discover that mounted source
...
- `joopo plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `joopo doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `joopo gateway status --deep --require-rpc` confirms the reachable Gateway, service/process hints, config path, and RPC health.
...
```bash
joopo plugins install -l ./my-plugin
```
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to `plugins/installs.json` under the active Joopo state directory. Its top-level `installRecords` map is the durable source of install metadata, including records for broken or missing plugin manifests. The `plugins` array is the manifest-derived cold registry cache. The file includes a do-not-edit warning and is used by `joopo plugins update`, uninstall, diagnostics, and the cold plugin registry.

When Joopo sees shipped legacy `plugins.installs` records in config, runtime reads treat them as compatibility input without rewriting `joopo.json`. Explicit plugin writes and `joopo doctor --fix` move those records into the plugin index and remove the config key when config writes are allowed; if either write fails, the config records are kept so the install metadata is not lost.

### Uninstall

```bash
joopo plugins uninstall <id>
joopo plugins uninstall <id> --dry-run
joopo plugins uninstall <id> --keep-files
```

`uninstall` removes plugin records from `plugins.entries`, the persisted plugin index, plugin allow/deny list entries, and linked `plugins.load.paths` entries when applicable. Unless `--keep-files` is set, uninstall also removes the tracked managed install directory when it is inside Joopo's plugin extensions root. For active memory plugins, the memory slot resets to `memory-core`.
...
joopo plugins update <id-or-npm-spec>
joopo plugins update --all
joopo plugins update <id-or-npm-spec> --dry-run
joopo plugins update @joopo/voice-call
joopo plugins update joopo-codex-app-server --dangerously-force-unsafe-install
```
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, Joopo reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. Joopo resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
  ...
  <Accordion title="Version checks and integrity drift">
    Before a live npm update, Joopo checks the installed package version against the npm registry metadata. If the installed version and recorded artifact identity already match the resolved target, the update is skipped without downloading, reinstalling, or rewriting `joopo.json`.

    When a stored integrity hash exists and the fetched artifact hash changes, Joopo treats that as npm artifact drift. The interactive `joopo plugins update` command prints the expected and actual hashes and asks for confirmation before proceeding. Non-interactive update helpers fail closed unless the caller supplies an explicit continuation policy.
...
Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports missing plugin dependencies directly; installs and repairs stay in `joopo plugins install`, `joopo plugins update`, and `joopo doctor --fix`.

Plugin-owned CLI commands are usually installed as root `joopo` command groups, but plugins may also register nested commands under a core parent such as `joopo nodes`. After `inspect --runtime` shows a command under `cliCommands`, run it at the listed path; for example a plugin that registers `demo-git` can be verified with `joopo demo-git ping`.
...
For module-shape failures such as missing `register`/`activate` exports, rerun with `JOOPO_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in the diagnostic output.
...
joopo plugins registry
joopo plugins registry --refresh
joopo plugins registry --json
```

The local plugin registry is Joopo's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.

Use `plugins registry` to inspect whether the persisted registry is present, current, or stale. Use `--refresh` to rebuild it from the persisted plugin index, config policy, and manifest/package metadata. This is a repair path, not a runtime activation path.

`joopo doctor --fix` also repairs registry-adjacent managed npm drift: if an orphaned or recovered `@joopo/*` package under the managed plugin npm root shadows a bundled plugin, doctor removes that stale package and rebuilds the registry so startup validates against the bundled manifest.

<Warning>
`JOOPO_DISABLE_PERSISTED_PLUGIN_REGISTRY=1` is a deprecated break-glass compatibility switch for registry read failures. Prefer `plugins registry --refresh` or `joopo doctor --fix`; the env fallback is only for emergency startup recovery while the migration rolls out.
</Warning>

### Marketplace

```bash
joopo plugins marketplace list <source>
joopo plugins marketplace list <source> --json
```
