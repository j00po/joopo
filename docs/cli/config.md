---
summary: "CLI reference for `joopo config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `joopo.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `joopo configure`).

<Note>
When `JOOPO_NIX_MODE=1`, Joopo treats `joopo.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-joopo distribution, use [nix-joopo Quick Start](https://github.com/joopo/nix-joopo#quick-start) and set values under `programs.joopo.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `joopo config` without a subcommand.
</ParamField>
...
```bash
joopo config file
joopo config --section model
joopo config --section gateway --section daemon
joopo config schema
joopo config get browser.executablePath
...
joopo config set agents.defaults.heartbeat.every "2h"
joopo config set agents.list[0].tools.exec.node "node-id-or-name"
joopo config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
joopo config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
joopo config set secrets.providers.vaultfile --provider-source file --provider-path /etc/joopo/secrets.json --provider-mode json
joopo config patch --file ./joopo.patch.json5 --dry-run
joopo config unset plugins.entries.brave.config.webSearch.apiKey
joopo config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
joopo config validate
joopo config validate --json
```

### `config schema`

Print the generated JSON schema for `joopo.json` to stdout as JSON.
...
```bash
joopo config schema > joopo.schema.json
```
...
```bash
joopo config get agents.defaults.workspace
joopo config get agents.list[0].id
```

Use the agent list index to target a specific agent:

```bash
joopo config get agents.list
joopo config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
joopo config set agents.defaults.heartbeat.every "0m"
joopo config set gateway.port 19001 --strict-json
joopo config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
joopo config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
joopo config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`joopo config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    joopo config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    joopo config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    joopo config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/joopo-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ...
    ```bash
    joopo config set --batch-file ./config-set.batch.json --dry-run
    ```
...
```bash
joopo config patch --file ./joopo.patch.json5 --dry-run
joopo config patch --file ./joopo.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh joopo-host 'joopo config patch --stdin --dry-run' < ./joopo.patch.json5
ssh joopo-host 'joopo config patch --stdin' < ./joopo.patch.json5
```
...
```bash
joopo config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
joopo config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/joopo/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `joopo.json`.
...
`joopo config set` and other Joopo-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `joopo.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `joopo.json` layouts are unsupported for writes; use `JOOPO_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
joopo config set gateway.reload.mode hybrid --dry-run
joopo config set gateway.reload.mode hybrid
joopo config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `joopo.json`. Run `joopo doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `JOOPO_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `joopo config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `joopo configure` or `joopo doctor --fix`. `joopo chat` does not bypass the invalid-config guard.
</Note>
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `joopo config set` or `joopo configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `joopo config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `joopo doctor` or `joopo doctor --fix` for migration and repair help.
  </Step>
</Steps>
