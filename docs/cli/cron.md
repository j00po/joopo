---
summary: "CLI reference for `joopo cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `joopo cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`joopo cron list` and `joopo cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `joopo cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
`joopo cron run` returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use `joopo cron runs --id <job-id>` to follow the eventual outcome.

<Note>
`joopo cron run <job-id>` force-runs by default. Use `--due` to keep the older "only run if due" behavior.
</Note>
...
- When a job has `--model` but no fallback list is configured, Joopo passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
...
```bash
joopo cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
joopo cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
joopo cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
joopo cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
joopo cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
joopo cron list
joopo cron list --agent ops
joopo cron show <job-id>
joopo cron run <job-id>
joopo cron run <job-id> --due
joopo cron runs --id <job-id> --limit 50
```

`joopo cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.
...
```bash
joopo cron edit <job-id> --agent ops
joopo cron edit <job-id> --clear-agent
joopo cron edit <job-id> --session current
joopo cron edit <job-id> --session "session:daily-brief"
```

`joopo cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
joopo cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
joopo cron edit <job-id> --best-effort-deliver
joopo cron edit <job-id> --no-best-effort-deliver
joopo cron edit <job-id> --no-deliver
```
