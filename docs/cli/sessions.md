---
summary: "CLI reference for `joopo sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `joopo channels status --probe`,
`joopo status --deep`, or `joopo health --verbose` when you need live
channel connectivity.

`joopo sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
joopo sessions
joopo sessions --agent work
joopo sessions --all-agents
joopo sessions --active 120
joopo sessions --limit 25
joopo sessions --verbose
joopo sessions --json
```
...
```bash
joopo sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
joopo sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.joopo/trajectory-exports/` under the selected workspace.

`joopo sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`joopo sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.joopo/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.joopo/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
joopo sessions cleanup --agent work --dry-run
joopo sessions cleanup --all-agents --dry-run
joopo sessions cleanup --enforce
joopo sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
joopo sessions cleanup --dry-run --fix-dm-scope
joopo sessions cleanup --json
```

`joopo sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `joopo sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run logs (`cron/runs/<jobId>.jsonl`), which are managed by `cron.runLog.maxBytes` and `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`joopo sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.joopo/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.joopo/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
