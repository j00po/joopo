---
summary: "CLI reference for `joopo qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
joopo qr
joopo qr --setup-code-only
joopo qr --json
joopo qr --remote
joopo qr --url wss://gateway.example/ws
```
...
- With `--remote`, Joopo requires either `gateway.remote.url` or
