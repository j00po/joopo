---
summary: "Joopo Gateway CLI (`joopo gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Joopo's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `joopo gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How Joopo advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.joopo/joopo.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `joopo onboard --mode local` and `joopo setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `JOOPO_GATEWAY_TOKEN` for the process).
</ParamField>
...
joopo gateway restart --safe
joopo gateway restart --force
```

`joopo gateway restart --safe` asks the running Gateway to preflight active Joopo work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.
...
- Set `JOOPO_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `JOOPO_DIAGNOSTICS=timeline` with `JOOPO_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `JOOPO_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
joopo gateway health --url ws://127.0.0.1:18789
```
...
```bash
joopo gateway usage-cost
joopo gateway usage-cost --days 7
joopo gateway usage-cost --json
```
...
```bash
joopo gateway stability
joopo gateway stability --type payload.large
joopo gateway stability --bundle latest
joopo gateway stability --bundle latest --export
joopo gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, Joopo writes the same diagnostic snapshot to `~/.joopo/logs/stability/joopo-stability-*.json` when the recorder has events. Inspect the newest bundle with `joopo gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
```bash
joopo gateway diagnostics export
joopo gateway diagnostics export --output joopo-diagnostics.zip
joopo gateway diagnostics export --json
```
...
joopo gateway status --require-rpc
```
...
```bash
joopo gateway call status
joopo gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `joopo` or Node with those args.

```bash
cat > ~/.local/bin/joopo-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- joopo "$@"
EOF
chmod +x ~/.local/bin/joopo-doppler

joopo gateway install --wrapper ~/.local/bin/joopo-doppler --force
joopo gateway restart
```
...
`JOOPO_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
JOOPO_WRAPPER="$HOME/.local/bin/joopo-doppler" joopo gateway install --force
joopo doctor
```

To remove a persisted wrapper, clear `JOOPO_WRAPPER` while reinstalling:

```bash
JOOPO_WRAPPER= joopo gateway install --force
joopo gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active Joopo work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `JOOPO_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `JOOPO_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_joopo-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `joopo.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
joopo gateway discover --timeout 4000
joopo gateway discover --json | jq '.beacons[].wsUrl'
```
