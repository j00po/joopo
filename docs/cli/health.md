---
summary: "CLI reference for `joopo health` (gateway health snapshot via RPC)"
read_when:
...
joopo health
joopo health --json
joopo health --timeout 2500
joopo health --verbose
joopo health --debug
```

Notes:

- Default `joopo health` asks the running gateway for its health snapshot. When the
