---
summary: "CLI reference for `joopo devices` (device pairing + token rotation/revocation)"
read_when:
...
```
joopo devices remove <deviceId>
joopo devices remove <deviceId> --json
```

### `joopo devices clear --yes [--pending]`

Clear paired devices in bulk.

```
joopo devices clear --yes
joopo devices clear --yes --pending
joopo devices clear --yes --pending --json
```

### `joopo devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Joopo only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Joopo supersedes the previous pending entry and issues a new `requestId`. Run `joopo devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
Joopo keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `joopo devices list`
or use `joopo devices approve --latest` to preview the exact upgrade before
approving it.
...
```
joopo devices approve
joopo devices approve <requestId>
joopo devices approve --latest
```
...
### `joopo devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
joopo devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `joopo devices revoke --device <id> --role <role>`
...
```
joopo devices revoke --device <deviceId> --role node
```
...
```bash
joopo devices rotate --device <deviceId> --role operator
```
