---
summary: "CLI reference for `joopo configure` (interactive configuration prompts)"
read_when:
...
<Tip>
`joopo config` without a subcommand opens the same wizard. Use `joopo config get|set|unset` for non-interactive edits.
</Tip>

For web search, `joopo configure --section web` lets you choose a provider
...
joopo configure --section model --section channels
joopo configure --section gateway --section daemon
```
