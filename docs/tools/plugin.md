---
summary: "Install, configure, and manage Joopo plugins"
read_when:
...
Plugins extend Joopo with new capabilities: channels, model providers,
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    joopo plugins install npm-pack:./joopo-plugin-1.2.3.tgz
    ...
    joopo plugins install ./my-plugin.tgz
    ```
...
`joopo doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`joopo.install.allowInvalidConfigRecovery`.
During Gateway startup, invalid plugin config fails closed like any other invalid
config. Run `joopo doctor --fix` to quarantine the bad plugin config by
...
Run `joopo doctor --fix` to remove the stale channel/plugin entries; unknown
...
Gateway startup skips plugin discovery/load work and `joopo doctor` preserves
...
and custom load paths must still be installed through `joopo plugins install`.
Use `joopo plugins list --json` to see the static `dependencyStatus` for each
...
and config validation follows with `plugin present but blocked`, Joopo found
...
Joopo as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Joopo config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Joopo as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/joopo-config/npm
```

After fixing ownership, rerun `joopo doctor --fix` or
`joopo plugins registry --refresh` so the persisted plugin registry matches
the repaired files.

For npm installs, mutable selectors such as `latest` or a dist-tag are resolved
before installation and then pinned to the exact verified version in Joopo's
managed npm root. After npm finishes, Joopo verifies the installed
...
Managed npm roots also inherit Joopo's package-level npm `overrides`, so
security pins that protect the packaged host also apply to hoisted external
plugin dependencies.

Source checkouts are pnpm workspaces. If you clone Joopo to hack on bundled
plugins, run `pnpm install`; Joopo then loads bundled plugins from
`extensions/<id>` so edits and package-local dependencies are used directly.
Plain npm root installs are for packaged Joopo, not source checkout
development.
...
Both show up under `joopo plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
Native plugin npm packages must declare `joopo.extensions` in `package.json`.
...
npm packages installed into Joopo's managed plugin root.
...
Joopo needs at runtime. That is a plugin packaging issue, not a local config
...
Use `joopo.runtimeExtensions` when published runtime files do not live at the
...
publish `joopo.setupEntry`, use `joopo.runtimeSetupEntry` for its built
...
| Mattermost      | `@joopo/mattermost`     | [Mattermost](/channels/mattermost)         |
| Microsoft Teams | `@joopo/msteams`        | [Microsoft Teams](/channels/msteams)       |
| Nextcloud Talk  | `@joopo/nextcloud-talk` | [Nextcloud Talk](/channels/nextcloud-talk) |
| Nostr           | `@joopo/nostr`          | [Nostr](/channels/nostr)                   |
| Synology Chat   | `@joopo/synology-chat`  | [Synology Chat](/channels/synology-chat)   |
| Tlon            | `@joopo/tlon`           | [Tlon](/channels/tlon)                     |
| WhatsApp        | `@joopo/whatsapp`       | [WhatsApp](/channels/whatsapp)             |
| Zalo            | `@joopo/zalo`           | [Zalo](/channels/zalo)                     |
| Zalo Personal   | `@joopo/zalouser`       | [Zalo Personal](/plugins/zalouser)         |

### Core (shipped with Joopo)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` - bundled browser plugin for the browser tool, `joopo browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
to `plugins.allow` or remove `plugins.allow`; `joopo doctor` warns about this
shape.
...
`joopo plugins list` is a local plugin registry/config snapshot. An
...
`joopo gateway run` process, or use `joopo gateway restart` against the
running Gateway when the reload reports a failure.

<Accordion title="Plugin states: disabled vs missing vs invalid">
  - **Disabled**: plugin exists but enablement rules turned it off. Config is preserved.
  - **Missing**: config references a plugin id that discovery did not find.
  - **Invalid**: plugin exists but its config does not match the declared schema. Gateway startup skips only that plugin; `joopo doctor --fix` can quarantine the invalid entry by disabling it and removing its config payload.

</Accordion>

## Discovery and precedence

Joopo scans for plugins in this order (first match wins):

<Steps>
  <Step title="Config paths">
    `plugins.load.paths` - explicit file or directory paths. Paths that point
    back at Joopo's own packaged bundled plugin directories are ignored;
    run `joopo doctor --fix` to remove those stale aliases.
  </Step>

  <Step title="Workspace plugins">
    `\<workspace\>/.joopo/<plugin-root>/*.ts` and `\<workspace\>/.joopo/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global plugins">
    `~/.joopo/<plugin-root>/*.ts` and `~/.joopo/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Joopo. Many are enabled by default (model providers, speech).
...
`/app/extensions/synology-chat`, Joopo treats that mounted source directory
...
Set `JOOPO_DISABLE_BUNDLED_SOURCE_OVERLAYS=1` to force packaged dist bundles
...
- Run `joopo gateway status --deep --require-rpc` and confirm the active
...
- Restart the live Gateway after plugin install/config/code changes. In wrapper
  ...
  `joopo gateway run` process.
- Use `joopo plugins inspect <id> --runtime --json` to confirm hook registrations and
...
- For proof of the effective session model, use `joopo sessions` or the
...
```bash
joopo config set logging.level trace
joopo logs --follow
```
...
Joopo caches successful plugin tool factory results for repeated resolutions
...
- Run `joopo plugins list --enabled --verbose` to see every enabled plugin
  and origin.
- Run `joopo plugins inspect <id> --runtime --json` for each suspected plugin and
  compare `channels`, `channelConfigs`, `tools`, and diagnostics.
- Run `joopo plugins registry --refresh` after installing or removing
...
- If the duplicate is accidental, disable one side with
  ...
  install.
- If you explicitly enabled both plugins, Joopo keeps that request and
...
```bash
joopo plugins list                       # compact inventory
...
joopo plugins inspect --all             # fleet-wide table
joopo plugins info <id>                 # inspect alias
joopo plugins doctor                    # diagnostics
joopo plugins registry                  # inspect persisted registry state
joopo plugins registry --refresh        # rebuild persisted registry
...
joopo plugins install <spec> --force    # overwrite existing install
joopo plugins install <path>            # install from local path
joopo plugins install -l <path>         # link (no copy) for dev
joopo plugins install <plugin> --marketplace <source>
joopo plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
joopo plugins install <spec> --pin      # record exact resolved npm spec
joopo plugins install <spec> --dangerously-force-unsafe-install
joopo plugins update <id-or-npm-spec> # update one plugin
joopo plugins update <id-or-npm-spec> --dangerously-force-unsafe-install
joopo plugins update --all            # update all
joopo plugins uninstall <id>          # remove config and plugin index records
...
joopo plugins inspect <id> --runtime --json

# Run plugin-owned CLI commands directly from the Joopo root CLI.
joopo <plugin-command> --help
...
Bundled plugins ship with Joopo. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `joopo plugins enable <id>`.

`--force` overwrites an existing installed plugin or hook pack in place. Use
`joopo plugins update <id-or-npm-spec>` for routine upgrades of tracked npm
plugins. It is not supported with `--link`, which reuses the source path instead
of copying over a managed install target.

When `plugins.allow` is already set, `joopo plugins install` adds the
...
Joopo keeps a persisted local plugin registry as the cold read model for
...
the registry is missing, stale, or invalid, `joopo plugins registry
...
In Nix mode (`JOOPO_NIX_MODE=1`), plugin lifecycle mutators are disabled.
Manage plugin package selection and config through the Nix source for the
install instead; for nix-joopo, start with the agent-first
[Quick Start](https://github.com/joopo/nix-joopo#quick-start).
`joopo plugins update <id-or-npm-spec>` applies to tracked installs. Passing
...
`joopo plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`joopo plugins` CLI reference](/cli/plugins) for full details.
...
Joopo loads the entry object and calls `register(api)` during plugin
...
Joopo may evaluate the trusted plugin entry or channel plugin module to build
