...
read_when:
  - Looking for an overview of Joopo's media capabilities
...
Joopo generates images, videos, and music, understands inbound media
...
For async tools, Joopo submits the request to the provider, returns a task
...
Joopo wakes the agent with the generated media paths so it can tell the
user and, when required by source-delivery policy, relay the result through
the message tool. For message-tool-only group/channel routes, Joopo treats
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until Joopo has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in Joopo until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
