...
Joopo supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Joopo reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.joopo/.env`.

If you omit `baseUrl`, Joopo defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Joopo defaults to `kimi-k2.6`.
...
Joopo treats Kimi `web_search` as successful only after Moonshot returns
...
Joopo returns a structured `kimi_web_search_ungrounded` error instead of
