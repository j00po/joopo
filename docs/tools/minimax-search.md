...
Joopo supports MiniMax as a `web_search` provider through the MiniMax
...
Joopo also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Joopo resolves
...
- `query`
- `count` (Joopo trims the returned result list to the requested count)
