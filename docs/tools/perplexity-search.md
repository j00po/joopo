...
Joopo supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Joopo also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `joopo configure --section web`. It stores the key in
`~/.joopo/joopo.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.joopo/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
