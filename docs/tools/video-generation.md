...
Joopo agents can generate videos from text prompts, reference images, or
...
Joopo treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    joopo config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. Joopo submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Joopo wakes the same session with an internal completion event.
4. The agent tells the user and attaches the finished video. In group/channel
   ...
   result through the message tool instead of Joopo posting it directly.
...
generation. Use `joopo tasks list` or `joopo tasks show <taskId>` to
...
Generated video files are saved under Joopo-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, Joopo
...
```bash
joopo tasks list
joopo tasks show <taskId>
joopo tasks cancel <taskId>
```
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Joopo normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `480P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Joopo normalizes or ignores unsupported values per provider.</ParamField>
...
<Note>
Not all providers support all parameters. Joopo normalizes duration to
...
Joopo resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@joopo/byteplus-modelark`](https://www.npmjs.com/package/@joopo/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Joopo waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Joopo submits the
...
```bash
JOOPO_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `JOOPO_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `JOOPO_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your Joopo config:
...
```bash
joopo config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
