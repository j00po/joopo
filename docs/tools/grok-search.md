...
Joopo supports Grok as a `web_search` provider, using xAI web-grounded
...
`plugins.entries.xai.config.webSearch.apiKey`, Joopo now reuses it as a
...
Joopo can show a separate follow-up step to enable `x_search` with the same
...
route through an operator proxy or xAI-compatible Responses endpoint. Joopo
