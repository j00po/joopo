---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, Joopo ACP, OpenCode) through the ACP backend"
read_when:
...
let Joopo run external coding harnesses (for example Pi, Claude Code,
Cursor, Copilot, Droid, Joopo ACP, OpenCode, Gemini CLI, and other
...
directly to existing Joopo channel conversations, use
[`joopo mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ Joopo | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an Joopo Gateway session _as_ an ACP server for an editor or client                   | [`joopo acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to Joopo over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No Joopo tools, no ACP controls, no harness runtime                                                                                                                               |
...
```bash
joopo plugins install @joopo/acpx
joopo config set plugins.entries.acpx.enabled true
```
...
Joopo only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  <Accordion title="First-run gotchas">
    ...
    - Codex ACP runs with an isolated `CODEX_HOME`; Joopo copies only trusted project entries from the host Codex config and trusts the active workspace, leaving auth, notifications, and hooks on the host config.
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. Joopo owns routing,
...
Joopo plugin tools and built-in Joopo tools are **not** exposed to
...
| `joopo` | Joopo Gateway bridge through `joopo acp` | Lets an ACP-aware harness talk back to an Joopo Gateway session.                 |
| `pi`       | Pi/embedded Joopo runtime                   | Used for Joopo-native harness experiments.                                       |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but Joopo
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the Joopo session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from Joopo's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up Joopo-owned wrapper and adapter process trees after `close`, and reaps stale Joopo-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    Joopo dynamic tools still execute through Joopo, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, Joopo injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through Joopo approvals. Codex `Stop` hooks are relayed to
    Joopo `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    Joopo picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    Joopo config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an Joopo agent
    is configured with `runtime.type="acp"`, Joopo uses
...
plugin is enabled. Use **sub-agents** when you want Joopo-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Joopo native sub-agent runtime  |
...
1. Joopo ACP session control plane.
2. Official `@joopo/acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state Joopo routes to.
...
spawned ACP session - no child thread, same chat surface. Joopo keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; Joopo returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, Joopo inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by Joopo even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - Joopo binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, Joopo returns a clear
...
- Joopo ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, Joopo inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, Joopo uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, Joopo may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  normalize Joopo Codex refs such as `openai-codex/gpt-5.4` to Codex
  ...
  `session/set_model`; otherwise Joopo/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What Joopo sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. Joopo agents get Joopo's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_JOOPO_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in Joopo where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. Joopo converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see Joopo-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, Joopo uses an agent-to-agent (A2A) follow-up path
    ...
    Joopo skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an Joopo channel session key; Joopo still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Joopo session you are creating, so `mode: "session"` still requires `thread: true`.
...
- Joopo's sandbox policy does **not** wrap ACP harness execution.
- Joopo still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced Joopo-native work.
...
If no target resolves, Joopo returns a clear error
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, Joopo normalizes `openai-codex/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai-codex/gpt-5.4/high` to `reasoning_effort`. |
| `/acp set thinking <level>`  | runtime config key `thinking`        | For Codex ACP, Joopo sends the corresponding `reasoning_effort` where the adapter supports one.                                                                             |
...
aliases), the plugin-tools and Joopo-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | Joopo is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                       | Harness process finished but ACP session did not report completion.                                                    | Update Joopo; current acpx cleanup reaps Joopo-owned stale wrapper and adapter processes on close and Gateway startup.                                             |
| Harness sees `<<<BEGIN_JOOPO_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update Joopo and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
