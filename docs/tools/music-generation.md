...
For session-backed agent runs, Joopo starts music generation as a
...
completion agent writes only a private final reply, Joopo falls back to a
...
<Note>
Not all providers support all parameters. Joopo still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Joopo
...
- **Status lookup:** `joopo tasks list` or `joopo tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Joopo injects an internal completion event back
...
```bash
JOOPO_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
