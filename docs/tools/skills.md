...
Joopo uses **[AgentSkills](https://agentskills.io)-compatible** skill
folders to teach the agent how to use tools. Each skill is a directory
containing a `SKILL.md` with YAML frontmatter and instructions. Joopo
...
Joopo loads skills from these sources, **highest precedence first**:
...
Codex CLI's native `$CODEX_HOME/skills` directory is not one of these Joopo
...
Use `joopo migrate codex --dry-run` to inventory them and
`joopo migrate codex` to choose skill directories with an interactive
checkbox prompt before copying them into the current Joopo agent workspace.
...
`joopo.plugin.json` (paths relative to the plugin root). Plugin skills
...
`metadata.joopo.requires.config` on the plugin's config entry.
...
Joopo follows the AgentSkills spec for layout/intent. The parser used
...
<ParamField path="homepage" type="string">
  URL surfaced as "Website" in the macOS Skills UI. Also supported via `metadata.joopo.homepage`.
</ParamField>
<ParamField path="user-invocable" type="boolean" default="true">
  When `true`, the skill is exposed as a user slash command.
</ParamField>
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Joopo keeps the skill's instructions out of the agent's normal
...
Joopo filters skills at load time using `metadata` (single-line JSON):
...
<ParamField path="requires.config" type="string[]">
  List of `joopo.json` paths that must be truthy.
</ParamField>
...
If no `metadata.joopo` is present, the skill is always eligible (unless
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Joopo lists each entry so you can see the available artifacts.
    - Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
    - Node installs honor `skills.install.nodeManager` in `joopo.json` (default: npm; options: npm/pnpm/yarn/bun). This only affects skill installs; the Gateway runtime should still be Node - Bun is not recommended for WhatsApp/Telegram.
    - Gateway-backed installer selection is preference-driven: when install specs mix kinds, Joopo prefers Homebrew when `skills.install.preferBrew` is enabled and `brew` exists, then `uv`, then the configured node manager, then other fallbacks like `go` or `download`.
    - If every install spec is `download`, Joopo surfaces all download options instead of collapsing to one preferred installer.

  </Accordion>
  <Accordion title="Per-installer details">
    - **Go installs:** if `go` is missing and `brew` is available, the gateway installs Go via Homebrew first and sets `GOBIN` to Homebrew's `bin` when possible.
    - **Download installs:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.joopo/tools/<skillKey>`).
...
under `skills.entries` in `~/.joopo/joopo.json`:
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience for skills that declare `metadata.joopo.primaryEnv`. Supports plaintext or SecretRef.
</ParamField>
...
defines `metadata.joopo.skillKey`, use that key under `skills.entries`.

<Note>
For stock image generation/editing inside Joopo, use the core
...
When an agent run starts, Joopo:
...
For the bundled `claude-cli` backend, Joopo also materializes the same
...
Joopo still owns precedence, per-agent allowlists, gating, and
`skills.entries.*` env/API key injection. Other CLI backends use the
prompt catalog only.

## Snapshots and refresh

Joopo snapshots the eligible skills **when a session starts** and
...
session, Joopo refreshes the snapshot so visible skills stay aligned
with the current agent.

### Skills watcher

By default, Joopo watches skill folders and bumps the skills snapshot
...
Joopo can treat macOS-only skills as eligible when the required
...
probes, Joopo clears its cached bin matches so agents no longer see
skills that cannot currently run there.

## Token impact

When skills are eligible, Joopo injects a compact XML list of available
...
Joopo ships a baseline set of skills as **bundled skills** with the
install (npm package or Joopo.app). `~/.joopo/skills` exists for
