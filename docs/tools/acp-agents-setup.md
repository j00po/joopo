...
read_when:
  ...
  - Enabling the plugin-tools or Joopo-tools MCP bridge
...
When Joopo uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Joopo `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Joopo before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Joopo/acpx cannot force a model selection.
...
Packaged installs use the official `@joopo/acpx` runtime plugin for ACP.
...
```bash
joopo plugins install ./path/to/local/acpx-plugin
```
...
live probe. Set `JOOPO_ACPX_RUNTIME_STARTUP_PROBE=1` only when you need the
...
- `command` accepts an absolute path, relative path (resolved from the Joopo workspace), or command name.
...
When you install Joopo globally with `npm install -g joopo`, the acpx
...
normally and reports the missing dependency through `joopo acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Joopo plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Joopo plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
joopo config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `joopo-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Joopo
  plugins.
...
### Joopo tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Joopo tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
joopo config set plugins.entries.acpx.config.joopoToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `joopo-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Joopo tools. The initial server exposes `cron`.
...
```bash
joopo config set plugins.entries.acpx.config.timeoutSeconds 180
```
...
```bash
joopo config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Joopo exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
joopo config set plugins.entries.acpx.config.permissionMode approve-all
joopo config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Joopo defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
