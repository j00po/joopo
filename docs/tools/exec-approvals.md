...
host-local `ask: "always"` in `~/.joopo/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `joopo approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `joopo exec-policy show`                                      | Local-machine merged view.                                                             |
| `joopo exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `joopo` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Joopo also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.joopo/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, Joopo treats inline code-eval forms as approval-only
...
**both** policy layers - requested exec policy in Joopo config
...
- In YOLO mode, Joopo does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when Joopo's requested exec
...
```bash
joopo exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.joopo/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `joopo approvals set --gateway` or
`joopo approvals set --node <id|name|ip>`.
...
```bash
joopo approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `joopo exec-policy` does not synchronize node approvals.
- `joopo exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `joopo approvals --node ...`.
...
specific argument shape. Joopo evaluates the regular expression
...
entries instead of hand-editing the encoded value. If Joopo cannot
...
edit its local `~/.joopo/exec-approvals.json` directly.

CLI: `joopo approvals` supports gateway or node editing - see
...
When an async exec approval is denied, Joopo prevents the agent from
