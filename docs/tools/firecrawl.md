...
Joopo can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `joopo configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `joopo doctor --fix`.
...
when you run Firecrawl yourself. Joopo accepts `http://` only for loopback,
...
Joopo always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Joopo
