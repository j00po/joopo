...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai-codex` OAuth profile is configured, Joopo routes image
...
nearby geometry option instead of the exact requested one, Joopo remaps to
...
Joopo tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when Joopo can
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai-codex` OAuth profile is configured, Joopo reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Joopo
    ...
    Joopo forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible Joopo maps
    ...
    transparency-capable OpenAI image model. Joopo routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    Joopo forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Joopo-managed image attachments

    Joopo intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Joopo image generation" size=3840x2160 count=1
```
  </Tab>
...
`joopo infer image edit`; `--openai-background` remains as an
