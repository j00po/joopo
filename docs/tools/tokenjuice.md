...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Joopo
...
Today this applies to PI embedded runs and Joopo dynamic tools in the Codex
app-server harness. Tokenjuice hooks Joopo's tool-result middleware and
...
```bash
joopo config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
joopo plugins enable tokenjuice
```

Joopo already ships the plugin. There is no separate `plugins install`
or `tokenjuice install joopo` step.
...
```bash
joopo config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
joopo plugins disable tokenjuice
```
