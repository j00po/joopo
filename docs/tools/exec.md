...
Run shell commands in the workspace. `exec` is a mutating shell surface: commands can create, edit, or delete files wherever the selected host or sandbox filesystem permits. Disabling Joopo filesystem tools such as `write`, `edit`, or `apply_patch` does not make `exec` read-only.
...
- `gateway`/`node` approvals are controlled by `~/.joopo/exec-approvals.json`.
...
- `timeout` applies to foreground, background, `yieldMs`, gateway, sandbox, and node `system.run` execution. If omitted, Joopo uses `tools.exec.timeoutSec`; explicit `timeout: 0` disables the exec process timeout for that call.
...
- Joopo sets `JOOPO_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- `joopo channels login` is blocked from `exec` because it is an interactive channel-auth flow; run it in a terminal on the gateway host, or use the channel-native login tool from chat when one exists.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.joopo/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#yolo-mode-no-approval).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Joopo prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`joopo security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `joopo doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`joopo security audit` and `joopo doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
