---
summary: "Joopo browser control API, CLI reference, and scripting actions"
read_when:
  - Scripting or debugging the agent browser via the local control API
  - Looking for the `joopo browser` CLI reference
...
This page is the reference for the local control HTTP API, the `joopo browser`
...
that override because Joopo does not launch those browser processes.

If shared-secret gateway auth is configured, browser HTTP routes require auth too:

- `Authorization: Bearer <gateway token>`
- `x-joopo-password: <gateway password>` or HTTP Basic auth with that password
...
- Role-style accessibility snapshots (`--interactive`, `--compact`,
  ...
  action engine.
- Page screenshots for the managed `joopo` browser when a per-tab CDP
...
Joopo, then restart the gateway. For Docker, also install the Chromium
...
`JOOPO_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
joopo browser start
joopo browser start --headless # one-shot local managed headless launch
joopo browser stop            # also clears emulation on attach-only/remote CDP
joopo browser tabs
joopo browser tab             # shortcut for current tab
joopo browser tab new
...
joopo browser screenshot --ref 12        # or --ref e12
joopo browser screenshot --labels
joopo browser snapshot
joopo browser snapshot --format aria --limit 200
joopo browser snapshot --interactive --compact --depth 6
joopo browser snapshot --efficient
joopo browser snapshot --labels
joopo browser snapshot --urls
joopo browser snapshot --selector "#main" --interactive
joopo browser snapshot --frame "iframe#main" --interactive
joopo browser console --level error
joopo browser errors --clear
joopo browser requests --filter api --clear
joopo browser pdf
joopo browser responsebody "**/api" --max-chars 5000
```
...
joopo browser click 12 --double           # or e12 for role refs
joopo browser click-coords 120 340        # viewport coordinates
joopo browser type 23 "hello" --submit
joopo browser press Enter
joopo browser hover 44
joopo browser scrollintoview e12
joopo browser drag 10 11
joopo browser select 9 OptionA OptionB
joopo browser download e12 report.pdf
joopo browser waitfordownload report.pdf
joopo browser upload /tmp/joopo/uploads/file.pdf
joopo browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'
joopo browser dialog --accept
joopo browser wait --text "Done"
joopo browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"
joopo browser evaluate --fn '(el) => el.textContent' --ref 7
joopo browser highlight e12
joopo browser trace start
joopo browser trace stop
```
...
joopo browser storage local set theme dark
joopo browser storage session clear
joopo browser set offline on
joopo browser set headers --headers-json '{"X-Debug":"1"}'
joopo browser set credentials user pass            # --clear to remove
joopo browser set geo 37.7749 -122.4194 --origin "https://example.com"
joopo browser set media dark
joopo browser set timezone America/New_York
joopo browser set locale en-US
...
- Download, trace, and upload paths are constrained to Joopo temp roots: `/tmp/joopo{,/downloads,/uploads}` (fallback: `${os.tmpdir()}/joopo/...`).
- `upload` can also set file inputs directly via `--input-ref` or `--element`.

Stable tab ids and labels survive Chromium raw-target replacement when Joopo
...
- `--format aria`: accessibility tree with `axN` refs. When Playwright is available, Joopo binds refs with backend DOM ids to the live page so follow-up actions can use them; otherwise treat the output as inspection-only.
...
Joopo supports two "snapshot" styles:

- **AI snapshot (numeric refs)**: `joopo browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `joopo browser click 12`, `joopo browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright's `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `joopo browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `joopo browser click e12`, `joopo browser highlight e12`.
  - Internally, the ref is resolved via `getByRole(...)` (plus `nth()` for duplicates).
  - Add `--labels` to include a viewport screenshot with overlayed `e12` labels.
  - Add `--urls` when link text is ambiguous and the agent needs concrete
    navigation targets.

- **ARIA snapshot (ARIA refs like `ax12`)**: `joopo browser snapshot --format aria`
  - Output: the accessibility tree as structured nodes.
  - Actions: `joopo browser click ax12` works when the snapshot path can bind
...
- Wait for URL (globs supported by Playwright):
  - `joopo browser wait --url "**/dash"`
- Wait for load state:
  - `joopo browser wait --load networkidle`
- Wait for a JS predicate:
  - `joopo browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `joopo browser wait "#main"`
...
1. `joopo browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `joopo browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `joopo browser trace start`
   - reproduce the issue
   - `joopo browser trace stop` (prints `TRACE:<path>`)
...
```bash
joopo browser status --json
joopo browser snapshot --interactive --json
joopo browser requests --filter api --json
joopo browser cookies --json
```
...
- The joopo browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `joopo browser evaluate` and `wait --fn`
