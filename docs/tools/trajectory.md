---
summary: "Export redacted trajectory bundles for debugging an Joopo agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Joopo session
...
Trajectory capture is Joopo's per-session flight recorder. It records a
...
Joopo writes the bundle under the workspace:

```text
.joopo/trajectory-exports/joopo-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.joopo/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Joopo sends the
...
Trajectory capture is on by default for Joopo agent runs.
...
| `metadata.json`       | Joopo version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Joopo also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `JOOPO_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export JOOPO_TRAJECTORY_DIR=/var/lib/joopo/trajectories
```

When this variable is set, Joopo writes one JSONL file per session id in that
directory.
...
Set `JOOPO_TRAJECTORY=0` before starting Joopo:

```bash
export JOOPO_TRAJECTORY=0
```
...
Joopo redacts sensitive values before writing export files:
...
- confirm Joopo was started without `JOOPO_TRAJECTORY=0`
- check whether `JOOPO_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.joopo/trajectory-exports/`
