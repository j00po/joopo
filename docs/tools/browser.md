...
read_when:
  ...
  - Debugging why joopo is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Joopo-managed)"
---

Joopo can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `joopo` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **joopo** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`joopo`, `work`, `remote`, ...).
...
joopo browser --browser-profile joopo doctor --deep
joopo browser --browser-profile joopo status
...
If `joopo browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `joopo browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `joopo browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `joopo` vs `user`

- `joopo`: managed, isolated browser (no extension required).
- `user`: built-in Chrome MCP attach profile for your **real signed-in Chrome**
  session.

For agent browser tool calls:

- Default: use the isolated `joopo` browser.
...
Set `browser.defaultProfile: "joopo"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.joopo/joopo.json`.
...
```json5
{
  browser: {
    ...
    profiles: {
      joopo: { cdpPort: 18800, color: "#FF4500" },
      work: {
        cdpPort: 18801,
        color: "#0066CC",
        headless: true,
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). Overriding `gateway.port` or `JOOPO_GATEWAY_PORT` shifts the derived ports in the same family.
- Local `joopo` profiles auto-assign `cdpPort`/`cdpUrl`; set those only for remote CDP. `cdpUrl` defaults to the managed local CDP port when unset.
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Joopo pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Joopo-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
...
- `POST /start?headless=true` and `joopo browser start --headless` request a
  ...
  remote CDP profiles reject the override because Joopo does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `joopo browser status --json`
  reports `headlessSource` as `env`, `profile`, `config`,
  `request`, `linux-display-fallback`, or `default`.
- `JOOPO_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `JOOPO_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- Default profile is `joopo` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
Joopo uses it automatically. Set `browser.executablePath` to override
auto-detection. Top-level and per-profile `executablePath` values accept `~`
for your OS home directory:

```bash
joopo config set browser.executablePath "/usr/bin/google-chrome"
joopo config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
```
...
Per-profile `executablePath` only affects local managed profiles that Joopo
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Joopo will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local Joopo-managed browser profile.
- `headless` only affects local managed profiles that Joopo launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `joopo browser stop` stops the browser process that
  Joopo launched
- attach-only and remote CDP profiles: `joopo browser stop` closes the active
  ...
  though no browser process was launched by Joopo
...
Joopo preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Joopo can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Joopo treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Joopo can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Joopo
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and Joopo runs on the host, treat
...
Joopo process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-Joopo WebSocket base, such
...
an address Joopo cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, Joopo treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by Joopo.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Joopo accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  Joopo calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Joopo connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Joopo tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Joopo
  ...
  accepts it, Joopo falls back to that root as well. This lets a bare `ws://`
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-joopo-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Joopo
  generates a runtime-only gateway token for that startup. Configure
  `gateway.auth.token`, `gateway.auth.password`, `JOOPO_GATEWAY_TOKEN`, or
  `JOOPO_GATEWAY_PASSWORD` explicitly if clients need a stable secret across
  restarts.
- Joopo does **not** auto-generate that token when `gateway.auth.mode` is
...
Joopo supports multiple named profiles (routing configs). Profiles can be:

- **joopo-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `joopo` profile is auto-created if missing.
...
Joopo can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Joopo attaches.
...
```bash
joopo browser --browser-profile user start
joopo browser --browser-profile user status
joopo browser --browser-profile user tabs
joopo browser --browser-profile user snapshot --format ai
```
...
- `joopo doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `joopo` profile because it can
  act inside your signed-in browser session.
- Joopo does not launch the browser for this driver; it only attaches.
- Joopo uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, Joopo skips
...
Compared to the managed `joopo` profile, existing-session drivers are more constrained:
...
When launching locally, Joopo picks the first available:
...
control API** plus a matching `joopo browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means Joopo cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "joopo" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by joopo` when a
...
- For the local loopback `joopo` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Joopo's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (joopo, chrome, or remote CDP).
