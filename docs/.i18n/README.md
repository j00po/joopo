...
- repo: `joopo/docs`
- local checkout: `~/Projects/joopo-docs`

## Source of truth

- English docs are authored in `joopo/joopo`.
...
1. Edit English docs in `joopo/joopo`.
2. Push to `main`.
3. `joopo/joopo/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `joopo/docs`.
...
5. `joopo/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `joopo/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `joopo/docs` instead.
...
- Sync metadata is written to `.joopo-sync/source.json` in the publish repo.
- Source repo secret: `JOOPO_DOCS_SYNC_TOKEN`
- Publish repo secret: `JOOPO_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `joopo/docs` first.
