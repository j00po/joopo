---
summary: "Set up Codex Computer Use for Codex-mode Joopo agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Joopo agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Joopo
...
Use this page when Joopo is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Joopo.app and Peekaboo

Joopo.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Joopo.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Joopo agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Joopo node and exposes mobile
...
Joopo-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Joopo's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Joopo command:

```bash
cua-driver mcp-config --client joopo
```

or register the stdio server yourself:

```bash
joopo mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal Joopo MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Joopo
...
With this config, Joopo checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Joopo asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Joopo also tries to
...
plugin command surface is available. These are Joopo chat/runtime commands,
not `joopo codex ...` CLI subcommands:
...
Joopo uses the same app-server API that Codex itself exposes. The
...
During install, Joopo polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Joopo prefers
...
`computer-use` is registered, Joopo tries to add the standard bundled
...
Joopo reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Joopo says Computer Use
...
Joopo intentionally fails closed when `computerUse.enabled` is true. A
...
tool hook could not reach an active Joopo relay through the local bridge or
Gateway fallback. Start a fresh Joopo session with `/new` or `/reset`. If it
