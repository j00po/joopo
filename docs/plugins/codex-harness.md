---
summary: "Run Joopo embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
...
The bundled `codex` plugin lets Joopo run embedded agent turns through the
...
Joopo still owns chat channels, session files, model selection, tools,
...
to the Joopo `message` tool if the deployment has not explicitly configured
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable Joopo
...
Most users who want "Codex in Joopo" want this route: sign in with a
...
```bash
joopo models auth login --provider openai-codex
```
...
`joopo doctor --fix` rewrites to `openai/gpt-*` across primary models,
...
| --------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------- |
| Native embedded runtime           | `agentRuntime.id: "codex"`                          | Runs Joopo embedded agent turns through Codex app-server.                  |
...
| Native hook relay                 | Plugin hooks around Codex-native events             | Lets Joopo observe/block supported Codex-native tool/finalization events.  |
...
- replace direct OpenAI API-key surfaces such as images, embeddings, speech, or
  realtime
- convert `openai-codex/*` model refs without `joopo doctor --fix`
- make ACP/acpx the default Codex path
- hot-switch existing sessions that already recorded a PI runtime
- replace Joopo channel delivery, session files, auth-profile storage, or
  message routing
...
Native Codex turns keep Joopo plugin hooks as the public compatibility layer.
These are in-process Joopo hooks, not Codex `hooks.json` command hooks:
...
Joopo dynamic tool results after Joopo executes the tool and before the
result is returned to Codex. This is separate from the public
`tool_result_persist` plugin hook, which transforms Joopo-owned transcript
tool-result writes.
...
If any configured model route is still `openai-codex/*`, `joopo doctor --fix`
...
| `openai-codex/gpt-5.5`                            | Legacy route repaired by doctor          | You are on old config; run `joopo doctor --fix` to rewrite it. |
...
`joopo doctor` warns when configured model refs or persisted session route
state still use `openai-codex/*`. `joopo doctor --fix` rewrites those routes
to:
...
Joopo records the selected harness id on that session and keeps using it for
later turns in the same session id. Change `agentRuntime` config or
`JOOPO_AGENT_RUNTIME` when you want future sessions to use another harness;
...
`Runtime: Joopo Pi Default`, and the Codex app-server harness appears as
`Runtime: OpenAI Codex`.

## Requirements

- Joopo with the bundled `codex` plugin available.
...
- Codex auth available to the app-server process or to Joopo's Codex auth
  bridge. Local app-server launches use an Joopo-managed Codex home for each
...
Joopo on the protocol surface it has been tested against.

For live and Docker smoke tests, auth usually comes from the Codex CLI account
or an Joopo `openai-codex` auth profile. Local stdio app-server launches can
also fall back to `CODEX_API_KEY` / `OPENAI_API_KEY` when no account is present.

## Workspace bootstrap files

Codex handles `AGENTS.md` itself through native project-doc discovery. Joopo
...
For Joopo workspace parity, the Codex harness resolves the other bootstrap
...
that runtime is forced, Joopo still tries the Codex harness and fails closed
...
| "Repair old `openai-codex/*` config/session pins"      | `joopo doctor --fix`                          |
...
Joopo only advertises ACP spawn guidance to agents when ACP is enabled,
...
```bash
JOOPO_AGENT_RUNTIME=codex joopo gateway run
```

With Codex forced, Joopo fails early if the Codex plugin is disabled, the
...
Joopo session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the Joopo session binding for that thread
...
By default, the plugin starts Joopo's managed Codex binary locally with:
...
By default, Joopo starts local Codex harness sessions in YOLO mode:
...
disallows the implicit YOLO approval, reviewer, or sandbox value, Joopo
...
Stdio app-server launches inherit Joopo's process environment by default,
but Joopo owns the Codex app-server account bridge and sets both
`CODEX_HOME` and `HOME` to per-agent directories under that agent's Joopo
...
state scoped to the Joopo agent instead of leaking in from the operator's
personal Codex CLI home.

Joopo plugins and Joopo skill snapshots still flow through Joopo's own
plugin registry and skill loader. Personal Codex CLI assets do not. If you have
useful Codex CLI skills or plugins that should become part of an Joopo agent,
...
The Codex migration provider copies skills into the current Joopo agent
...
1. An explicit Joopo Codex auth profile for the agent.
2. The app-server's existing account in that agent's Codex home.
3. For local stdio app-server launches only, `CODEX_API_KEY`, then
   `OPENAI_API_KEY`, when no app-server account is present and OpenAI auth is
   still required.

When Joopo sees a ChatGPT subscription-style Codex auth profile, it removes
...
loading. In that mode, Joopo does not expose dynamic tools that duplicate
Codex-native workspace operations: `read`, `write`, `edit`, `apply_patch`,
`exec`, `process`, and `update_plan`. Remaining Joopo integration tools such
...
under the `joopo` namespace, keeping the initial model context smaller.
...
| -------------------------- | ---------------- | ----------------------------------------------------------------------------------------- |
| `codexDynamicToolsProfile` | `"native-first"` | Use `"joopo-compat"` to expose the full Joopo dynamic tool set to Codex app-server. |
| `codexDynamicToolsLoading` | `"searchable"`   | Use `"direct"` to put Joopo dynamic tools directly in the initial Codex tool context.  |
| `codexDynamicToolsExclude` | `[]`             | Additional Joopo dynamic tool names to omit from Codex app-server turns.               |
...
| `clearEnv`                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Joopo builds its inherited environment. `CODEX_HOME` and `HOME` are reserved for Joopo's per-agent Codex isolation on local launches. |
| `requestTimeoutMs`            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                          |
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after a turn-scoped Codex app-server request while Joopo waits for `turn/completed`. Raise this for slow post-tool or status-only synthesis phases.                                                                  |
...
Joopo-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: each Codex `item/tool/call` request must receive
an Joopo response within 30 seconds. On timeout, Joopo aborts the tool
...
After Joopo responds to a Codex turn-scoped app-server request, the harness
...
response, Joopo best-effort interrupts the Codex turn, records a diagnostic
timeout, and releases the Joopo session lane so follow-up chat messages are
...
- `JOOPO_CODEX_APP_SERVER_BIN`
- `JOOPO_CODEX_APP_SERVER_ARGS`
- `JOOPO_CODEX_APP_SERVER_MODE=yolo|guardian`
- `JOOPO_CODEX_APP_SERVER_APPROVAL_POLICY`
- `JOOPO_CODEX_APP_SERVER_SANDBOX`

`JOOPO_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`JOOPO_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`JOOPO_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
capabilities in the same Codex thread as the Joopo harness turn. Joopo
does not translate Codex plugins into synthetic `codex_plugin_*` Joopo
...
Codex plugins only work when the selected Joopo agent runtime is the native
...
Thread app config is computed when Joopo establishes a Codex harness session
...
Joopo reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. A plugin app is
exposed only when Joopo can map it back to the migrated plugin through stable
...
Plugin-owned app tools use Codex's native app configuration. Joopo injects a
restrictive `config.apps` patch for the Codex thread: `_default` is disabled and
only apps owned by enabled migrated plugins are enabled. Joopo sets
...
`open_world_enabled: true`; Joopo does not expose a separate plugin
open-world policy knob. Joopo does not maintain per-plugin destructive
tool-name deny lists. Tool approval mode is prompted by default for plugin
apps, because Joopo does not have an interactive app-elicitation UI in this
same-thread path.
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, Joopo returns a deterministic decline.
- When policy is `true`, Joopo auto-accepts only safe schemas it can map to
...
- `app_inventory_missing` or `app_inventory_stale`: app readiness came from an
  empty or stale cache; Joopo schedules an async refresh and excludes plugin
...
The short version: Joopo does not vendor the desktop-control app or execute
...
`cua-driver mcp` with `joopo mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'`.
...
When `computerUse.autoInstall` is true, Joopo can register the standard
...
Model switching stays Joopo-controlled. When an Joopo session is attached
...
generic and works on any channel that supports Joopo text commands.
...
- `/codex resume <thread-id>` attaches the current Joopo session to an existing Codex thread.
...
When Codex reports a usage-limit failure, Joopo includes the next
...
3. Copy the completed diagnostics reply into the bug report or support thread.
   It includes the local bundle path, privacy summary, Joopo session ids,
...
feedback upload for the currently attached thread without the full Joopo
...
Core Joopo also exposes owner-only `/diagnostics [note]` as the general
...
`joopo gateway diagnostics export --json` through explicit exec approval
every time. Do not approve diagnostics with an allow-all rule. After approval,
Joopo sends a pasteable report with the local bundle path and manifest
summary. When the active Joopo session is using the Codex harness, that
...
If `/diagnostics` is invoked by an owner in a group chat, Joopo keeps the
...
Joopo refuses the group request and asks the owner to run it from a DM.
...
Joopo session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent. If you deny or ignore the approval,
Joopo does not print those Codex ids. This upload does not replace the local
Gateway diagnostics export.

`/codex resume` writes the same sidecar binding file that the harness uses for
normal turns. On the next message, Joopo resumes that Codex thread, passes the
currently selected Joopo model into app-server, and keeps extended history
enabled.
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Joopo plugin hooks                 | Joopo                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | Joopo bundled plugins | Per-turn adapter behavior around Joopo dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Joopo does not use project or global Codex `hooks.json` files to route
Joopo plugin behavior. For the supported native tool and permission bridge,
Joopo injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
omits `PermissionRequest` so Codex's app-server reviewer and Joopo's approval
...
Codex-level controls; they are not exposed as Joopo plugin hooks in the v1
contract.

For Joopo dynamic tools, Joopo executes the tool after Codex asks for the
call, so Joopo fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
Joopo can mirror selected events, but it cannot rewrite the native Codex
...
notifications and Joopo adapter state, not native Codex hook commands.
Joopo's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke Joopo plugin hooks.

## V1 support contract

Codex mode is not PI with a different model call underneath. Codex owns more of
the native model loop, and Joopo adapts its plugin and session surfaces
...
| Joopo channel routing and delivery         | Supported                                                                            | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                           |
| Joopo dynamic tools                        | Supported                                                                            | Codex asks Joopo to execute these tools, so Joopo stays in the execution path.                                                                                                                       |
| Prompt and context plugins                    | Supported                                                                            | Joopo builds prompt overlays and projects context into the Codex turn before starting or resuming the thread.                                                                                           |
...
| Dynamic tool hooks                            | Supported                                                                            | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Joopo-owned dynamic tools.                                                                                                 |
...
| Native permission policy                      | Supported through Codex app-server approvals and the compatibility native hook relay | Codex app-server approval requests route through Joopo after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review. |
| App-server trajectory capture                 | Supported                                                                            | Joopo records the request it sent to app-server and the app-server notifications it receives.                                                                                                           |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Joopo does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Joopo owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Joopo-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Joopo observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current Joopo compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Joopo can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Tools, media, and compaction

The Codex harness changes the low-level embedded agent executor only.

Joopo still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal Joopo delivery path.

The native hook relay is intentionally generic, but the v1 support contract is
limited to the Codex-native tool and permission paths that Joopo tests. In
...
Codex hook event is an Joopo plugin surface until the runtime contract names
it.

For `PermissionRequest`, Joopo only returns explicit allow or deny decisions
...
Joopo remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through Joopo's plugin
...
default `messages.queue.mode: "steer"`, Joopo batches queued chat messages
...
Joopo uses the followup queue when the selected mode allows fallback. See
[Steering queue](/concepts/queue-steering).

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. Joopo keeps a transcript mirror for channel
...
reasoning or plan records when the app-server emits them. Today, Joopo only
...
Joopo is writing an Joopo-owned session transcript tool result.
...
**Joopo uses PI instead of Codex:** `agentRuntime.id: "auto"` can still use PI as the
...
stable `0.125.0` protocol floor is what Joopo tests.
