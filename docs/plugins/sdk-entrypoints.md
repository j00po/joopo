...
when Joopo loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Joopo will use a
...
**Import:** `joopo/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `JoopoPluginConfigSchema \| () => JoopoPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: JoopoPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `joopo.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Joopo resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `joopo/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: JoopoPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: JoopoPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Joopo may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Joopo
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `joopo nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; Joopo resolves it to the
...
Joopo loads this instead of the full entry when a channel is disabled,
...
- `joopo/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `joopo/plugin-sdk/channel-setup` for optional-install setup surfaces
- `joopo/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`joopo/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so Joopo can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Joopo to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Joopo rejects descriptor names outside
...
Joopo classifies loaded plugins by their registration behavior:
...
Use `joopo plugins inspect <id>` to see a plugin's shape.
