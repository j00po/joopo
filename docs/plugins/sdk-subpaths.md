...
The plugin SDK is exposed as a set of narrow subpaths under `joopo/plugin-sdk/`.
...
| `plugin-sdk/config-schema`                | `JoopoSchema`                                                                                                                                                             |
...
| `plugin-sdk/plugin-test-api`              | Minimal `JoopoPluginApi` mock builder for direct plugin registration unit tests                                                                                           |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `joopo.json` Zod schema export (`JoopoSchema`) |
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Bundled Joopo channel config schemas for maintained bundled plugins only |
    ...
    | `plugin-sdk/discord` | Deprecated Discord compatibility facade for published `@joopo/discord@2026.3.13` and tracked owner compatibility; new plugins should use generic channel SDK subpaths |
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-auth` | `createProviderApiKeyAuthMethod`, `ensureApiKeyFromOptionEnvOrPrompt`, `upsertAuthProfile`, `upsertApiKeyProfile`, `writeOAuthCredentials`, deprecated `resolveJoopoAgentDir` compatibility export |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/config-types` | Type-only config surface for plugin config shapes such as `JoopoConfig` and channel/provider config types |
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Joopo tool bridge helpers, runtime-plan tool policy helpers, terminal outcome classification, tool progress formatting/detail helpers, and attempt result utilities |
    ...
    | `plugin-sdk/agent-runtime` | Agent dir/identity/workspace helpers, including `resolveAgentDir`, `resolveDefaultAgentDir`, and deprecated `resolveJoopoAgentDir` compatibility export |
