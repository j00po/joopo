...
An **agent harness** is the low level executor for one prepared Joopo agent
...
runtime and the normal Joopo provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Joopo
  session transcript
...
Before a harness is selected, Joopo has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Joopo transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Joopo-owned
...
**Import:** `joopo/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "joopo/plugin-sdk/agent-harness";
...
Joopo chooses a harness after provider/model resolution:
...
2. `JOOPO_AGENT_RUNTIME=<id>` forces a registered harness with that id for
   sessions that are not already pinned.
3. `JOOPO_AGENT_RUNTIME=pi` forces the built-in PI harness.
4. `JOOPO_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
5. If no registered harness matches, Joopo uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, Joopo does not
...
Joopo. The harness then claims that provider in `supports(...)`.
...
- app-server request: Joopo sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs continue to use the
normal Joopo provider path unless you force the Codex harness with
...
Joopo requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Joopo only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Joopo to the newer tested stable line.
...
`joopo/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Joopo's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded Joopo
...
`joopo doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Joopo still owns the chat channel,
...
By default, Joopo runs embedded agents with Joopo Pi. In `auto` mode,
...
`JOOPO_AGENT_RUNTIME` still overrides the configured runtime.
...
Keep that binding explicitly associated with the Joopo session, and keep
mirroring user-visible assistant/tool output into the Joopo transcript.

The Joopo transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Joopo can
clear it when the owning Joopo session is reset.

## Tool and media results

Core constructs the Joopo tool list and passes it into the prepared attempt.
