---
summary: "Create your first Joopo plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
read_when:
  - You want to create a new Joopo plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Joopo
---
...
- For in-repo plugins: repository cloned and `pnpm install` done. Source
  checkout plugin development is pnpm-only because Joopo loads bundled
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Joopo to a messaging platform (Discord, IRC, etc.)
  </Card>
  <Card title="Provider plugin" icon="cpu" href="/plugins/sdk-provider-plugins">
    Add a model provider (LLM, proxy, or custom endpoint)
  </Card>
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Map a local AI CLI into Joopo's text fallback runner
  </Card>
...
`joopo/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json joopo.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Joopo",
      "contracts": {
        "tools": ["my_tool"]
      },
      "activation": {
...
plugins should prefer regular Joopo plugin hooks unless Joopo grows an
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, Joopo retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `joopo/plugin-sdk/error-runtime`
...
Joopo captures and caches the validated descriptor from the registered tool,
...
`api.registerTool(..., { optional: true })` so Joopo can avoid loading that
...
Plugins can add root `joopo` command groups with `api.registerCli`. Provide
`descriptors` for every top-level command root so Joopo can show and route
...
```bash
joopo plugins inspect demo-plugin --runtime --json
joopo demo-plugin ping
```

## Import conventions

Always import from focused `joopo/plugin-sdk/<subpath>` paths:
...
package-root seam instead of promoting it into `joopo/plugin-sdk/*`.

Some generated `joopo/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `joopo` metadata</Check>
<Check>**joopo.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [joopo/joopo](https://github.com/joopo/joopo/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Joopo X account [@joopo](https://x.com/joopo) for release announcements.
