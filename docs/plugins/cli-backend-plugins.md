...
read_when:
  ...
  - You need to map a third-party CLI into Joopo's text fallback runner
---

CLI backend plugins let Joopo call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Joopo at the plugin runtime module              |
| Manifest ownership   | `joopo.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `joopo.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json joopo.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Joopo",
    ...
    `cliBackends` is the runtime ownership list. It lets Joopo auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "joopo/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how Joopo should launch and parse the CLI:
...
| `modelAliases`                            | Map Joopo model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields Joopo reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific Joopo auth profile                |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into Joopo's loopback MCP tool bridge           |
...
CLI backends do not receive Joopo tools by default. If the CLI can consume an
...
"always-on"` so Joopo can fail closed when a caller requires no native tools.
...
```bash
joopo plugins inspect acme-cli --runtime --json
joopo agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `joopo.extensions` and built runtime entries for published packages</Check>
<Check>`joopo.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
