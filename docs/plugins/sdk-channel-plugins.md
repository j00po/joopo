---
summary: "Step-by-step guide to building a messaging channel plugin for Joopo"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Joopo to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Joopo to a
...
<Info>
  If you have not built any Joopo plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Joopo keeps one
...
`defineChannelMessageAdapter` from `joopo/plugin-sdk/channel-message`. The
...
receive/send lifecycle helpers on `joopo/plugin-sdk/channel-message`.
...
Use `joopo/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `joopo/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `joopo/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `joopo/plugin-sdk/approval-auth-runtime`
- `joopo/plugin-sdk/approval-client-runtime`
- `joopo/plugin-sdk/approval-delivery-runtime`
- `joopo/plugin-sdk/approval-gateway-runtime`
- `joopo/plugin-sdk/approval-handler-adapter-runtime`
- `joopo/plugin-sdk/approval-handler-runtime`
- `joopo/plugin-sdk/approval-native-runtime`
- `joopo/plugin-sdk/approval-reply-runtime`
- `joopo/plugin-sdk/channel-runtime-context`

Likewise, prefer `joopo/plugin-sdk/setup-runtime`,
`joopo/plugin-sdk/setup-adapter-runtime`,
`joopo/plugin-sdk/reply-runtime`,
`joopo/plugin-sdk/reply-dispatch-runtime`,
`joopo/plugin-sdk/reply-reference`, and
`joopo/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `joopo/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `joopo/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `joopo/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `joopo.setupEntry` in
...
- use the broader `joopo/plugin-sdk/setup` seam only when you also need the
...
- `joopo/plugin-sdk/account-core`,
  `joopo/plugin-sdk/account-id`,
  `joopo/plugin-sdk/account-resolution`, and
  `joopo/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `joopo/plugin-sdk/inbound-envelope` and
  `joopo/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `joopo/plugin-sdk/messaging-targets` for target parsing/matching
- `joopo/plugin-sdk/outbound-media` and
  `joopo/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `joopo/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `joopo/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `joopo/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `joopo/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `joopo/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `joopo/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
} from "joopo/plugin-sdk/channel-inbound";
...
`joopo/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`joopo/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#joopo-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/joopo-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "joopo": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Joopo to Acme Chat."
        }
      }
    }
    ```

    ```json joopo.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { JoopoConfig } from "joopo/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "joopo/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Joopo
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "joopo/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Joopo loads this instead of the full entry when the channel is disabled
    ...
    `joopo/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Joopo. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Joopo.
...
├── package.json              # joopo.channel metadata
├── joopo.plugin.json      # Manifest with config schema
