...
read_when:
  ...
  - You want a reference for all registration methods on JoopoPluginApi
...
<Note>
  This page is for plugin authors using `joopo/plugin-sdk/*` inside
  Joopo. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [Joopo App SDK](/concepts/joopo-sdk) and the `@joopo/sdk` package
  instead.
</Note>
...
prefer `joopo/plugin-sdk/channel-core`; keep `joopo/plugin-sdk/core` for
...
`joopo.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Joopo's
...
<Warning>
  ...
  `joopo/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`joopo/plugin-sdk/discord` and `joopo/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `JoopoPluginApi` object with these
methods:
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `joopo nodes` |
...
cannot register this middleware; keep normal Joopo plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. Joopo calls the
...
`joopo nodes canvas` explicit plugin-owned node features.
...
- User config still wins. Joopo merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Joopo thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `joopo/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep Joopo as the source of truth for due checks and execution.
...
| `api.config`             | `JoopoConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `joopo/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Joopo is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Joopo's plugin
...
- **`@joopo/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@joopo/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `joopo/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `joopo/plugin-sdk/speech`, `.../provider-model-shared`, or another
