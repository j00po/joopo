...
read_when:
  ...
  - You are defining plugin config schemas or package.json joopo metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`joopo.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `joopo` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/joopo-my-channel",
      "version": "1.0.0",
      "type": "module",
      "joopo": {
        "extensions": ["./index.ts"],
...
`joopo.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
When enabled, Joopo loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `joopo.plugin.json` in the package root. Joopo uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Joopo",
  "configSchema": {
    "type": "object",
...
The `setup-entry.ts` file is a lightweight alternative to `index.ts` that Joopo loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).
...
Bundled workspace channels that keep setup-safe exports in sidecar modules can use `defineBundledChannelSetupEntry(...)` from `joopo/plugin-sdk/channel-entry-contract` instead of `defineSetupPluginEntry(...)`. That bundled contract also supports an optional `runtime` export so setup-time runtime wiring can stay lightweight and explicit.

<AccordionGroup>
  <Accordion title="When Joopo uses setupEntry instead of the full entry">
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so Joopo can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "joopo/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `joopo.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `joopo onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "joopo/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `joopo/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `joopo/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `joopo/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "joopo/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/joopo-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="npm">
    ```bash
    joopo plugins install @myorg/joopo-my-plugin
    ```
  ...
  <Tab title="npm package spec">
    ...
    ```bash
    joopo plugins install npm:@myorg/joopo-my-plugin
    ```
...
```bash
joopo plugins install <package-name>
```

<Info>
For npm-sourced installs, `joopo plugins install` installs the package under `~/.joopo/npm` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>
