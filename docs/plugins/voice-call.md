...
read_when:
  - You want to place an outbound voice call from Joopo
...
Voice calls for Joopo via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        joopo plugins install @joopo/voice-call
        ```
      </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `joopo doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `joopo doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `joopo_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Joopo tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `joopo_agent_consult`.
...
configured Joopo agent without paying a full agent-consult round trip on
...
`joopo_agent_consult` still run the full Joopo agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call joopo_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `joopo doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
joopo voicecall call --to "+15555550123" --message "Hello from Joopo"
joopo voicecall start --to "+15555550123"   # alias for call
joopo voicecall continue --call-id <id> --message "Any questions?"
joopo voicecall speak --call-id <id> --message "One moment"
joopo voicecall dtmf --call-id <id> --digits "ww123456#"
joopo voicecall end --call-id <id>
joopo voicecall status --call-id <id>
joopo voicecall tail
joopo voicecall latency                      # summarize turn latency from logs
...
Provider signatures are checked against the public URL Joopo reconstructs
...
Use `joopo logs --follow` for the live phase trace. A healthy Twilio Meet
...
`joopo voicecall tail` still shows persisted call records; it is useful for
...
- `joopo logs --follow` shows realtime TwiML served, the realtime bridge
