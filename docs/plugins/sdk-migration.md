...
read_when:
  - You see the JOOPO_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the JOOPO_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before Joopo 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Joopo plugin
---

Joopo has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`joopo/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`joopo/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`joopo/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`joopo/extension-api`** - a bridge that gave plugins direct access to
...
Joopo does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`joopo/plugin-sdk/\<subpath\>`)
...
`joopo/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `joopo doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `joopo/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `JoopoConfig` | `joopo/plugin-sdk/config-types` |
    | Already-loaded config assertions and plugin-entry config lookup | `joopo/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `joopo/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `joopo/plugin-sdk/config-mutation` |
    | Session store helpers | `joopo/plugin-sdk/session-store-runtime` |
    | Markdown table config | `joopo/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `joopo/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `joopo/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `joopo/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `joopo/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `joopo/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "joopo/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "joopo/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "joopo/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "joopo/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "joopo/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "joopo/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `joopo/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `joopo/plugin-sdk/system-event-runtime` |
    | Heartbeat event and visibility helpers | `joopo/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `joopo/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `joopo/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `joopo/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `joopo/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `joopo/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `joopo/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `joopo/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `joopo/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `joopo/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `joopo/plugin-sdk/error-runtime` |
    | Transport readiness waits | `joopo/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `joopo/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `joopo/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `joopo/plugin-sdk/number-runtime` |
    | Process-local async lock | `joopo/plugin-sdk/async-lock-runtime` |
    | File locks | `joopo/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `joopo/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `JoopoSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | Joopo-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@joopo/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`joopo/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`joopo/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "joopo/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "joopo/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `joopo/plugin-sdk/channel-inbound` or
    `joopo/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `joopo/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `joopo/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `joopo/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `joopo/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Joopo no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Joopo downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="JoopoSchemaType alias → JoopoConfig">
    `JoopoSchemaType` re-exported from `joopo/plugin-sdk` is now a
    one-line alias for `JoopoConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { JoopoSchemaType } from "joopo/plugin-sdk";
    // After
    import type { JoopoConfig } from "joopo/plugin-sdk/config-schema";
    ```
...
```bash
JOOPO_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 joopo gateway run
JOOPO_SUPPRESS_EXTENSION_API_WARNING=1 joopo gateway run
```
