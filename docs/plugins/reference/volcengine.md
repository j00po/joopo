---
summary: "Adds Volcengine, Volcengine Plan model provider support to Joopo."
read_when:
  - You are installing, configuring, or auditing the volcengine plugin
title: "Volcengine plugin"
---

# Volcengine plugin

Adds Volcengine, Volcengine Plan model provider support to Joopo.

## Distribution

- Package: `@joopo/volcengine-provider`
- Install route: included in Joopo

## Surface

providers: volcengine, volcengine-plan; contracts: speechProviders

## Related docs

- [volcengine](/providers/volcengine)
