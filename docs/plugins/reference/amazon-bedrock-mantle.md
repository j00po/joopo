---
summary: "Adds Amazon Bedrock Mantle model provider support to Joopo."
read_when:
  - You are installing, configuring, or auditing the amazon-bedrock-mantle plugin
title: "Amazon Bedrock Mantle plugin"
---

# Amazon Bedrock Mantle plugin

Adds Amazon Bedrock Mantle model provider support to Joopo.

## Distribution

- Package: `@joopo/amazon-bedrock-mantle-provider`
- Install route: included in Joopo

## Surface

providers: amazon-bedrock-mantle

## Related docs

- [amazon-bedrock-mantle](/providers/bedrock-mantle)
