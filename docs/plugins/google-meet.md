...
read_when:
  - You want an Joopo agent to join a Google Meet call
  - You want an Joopo agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for Joopo — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured Joopo agent answers, and regular Joopo TTS speaks into Meet.
...
provider plus regular Joopo TTS. OpenAI is the default transcription
...
```bash
joopo googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `joopo googlemeet setup --json` for
...
```bash
joopo googlemeet setup --transport twilio
```
...
```bash
joopo googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
joopo googlemeet create --transport chrome-node --mode agent
```
...
```bash
joopo googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. Joopo uses the
  ...
  the Joopo Chrome profile on the node to already be signed in to Google.
...
Joopo's microphone/camera permission grant and avoid the Meet **Use
...
Use `joopo googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in Joopo browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by Joopo. For
...
You do **not** need a full Joopo Gateway or model API key inside a macOS VM
...
- Gateway host: Joopo Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: Joopo CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update Joopo in the VM, then enable the bundled plugin there:

```bash
joopo plugins enable google-meet
```

Start the node host in the VM:

```bash
joopo node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
JOOPO_ALLOW_INSECURE_PRIVATE_WS=1 \
  joopo node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
JOOPO_ALLOW_INSECURE_PRIVATE_WS=1 \
  joopo node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
joopo node restart
```

`JOOPO_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`joopo.json` setting. `joopo node install` stores it in the LaunchAgent
...
```bash
joopo googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, Joopo browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, Joopo auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `joopo node run` in the VM,
  approve pairing, and make sure `joopo plugins enable google-meet` and
  `joopo plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses Joopo
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. Joopo
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by Joopo; use separate virtual devices or Loopback-style routing
...
Joopo does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with Joopo, review BlackHole's
...
Chrome transport opens the Meet URL through Joopo browser control and joins
as the signed-in Joopo browser profile. On macOS, the plugin checks for
...
```bash
joopo googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
joopo googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local Joopo audio
...
meeting; Joopo does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Joopo agent. Be brief.",
...
joopo plugins list | grep -E 'google-meet|voice-call'
joopo googlemeet setup
```
...
`JOOPO_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes Joopo requests:
...
`meetings.space.readonly` lets Joopo resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets Joopo pass `SpaceConfig` settings such as
...
```bash
joopo googlemeet auth login --json
```
...
```bash
JOOPO_GOOGLE_MEET_CLIENT_ID="your-client-id" \
JOOPO_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
joopo googlemeet auth login --json
```
...
JOOPO_GOOGLE_MEET_CLIENT_ID="your-client-id" \
JOOPO_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
joopo googlemeet auth login --json --manual
```
...
support existed, rerun `joopo googlemeet auth login --json` so the refresh
...
```bash
joopo googlemeet doctor --oauth --json
```
...
```bash
joopo googlemeet doctor --oauth --create-space --json
joopo googlemeet create --no-join --json
```
...
```bash
joopo googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
joopo googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `joopo googlemeet auth login
...
- `JOOPO_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `JOOPO_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `JOOPO_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `JOOPO_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `JOOPO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `JOOPO_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `JOOPO_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
joopo googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
joopo googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
joopo googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
joopo googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
joopo googlemeet latest --today
joopo googlemeet calendar-events --today --json
joopo googlemeet artifacts --event "Weekly sync"
joopo googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
joopo googlemeet latest --meeting https://meet.google.com/abc-defg-hij
joopo googlemeet artifacts --conference-record conferenceRecords/abc123 --json
joopo googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
joopo googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
Joopo accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops Joopo's local/session
...
```bash
joopo googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
joopo googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
joopo googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
joopo googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
JOOPO_LIVE_TEST=1 \
JOOPO_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
joopo googlemeet setup --transport chrome-node --mode transcribe
joopo googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `JOOPO_LIVE_TEST=1` enables guarded live tests.
- `JOOPO_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `JOOPO_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `JOOPO_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `JOOPO_GOOGLE_MEET_CLIENT_SECRET`,
  `JOOPO_GOOGLE_MEET_ACCESS_TOKEN`, and
  `JOOPO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `JOOPO_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Joopo browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the Joopo browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, Joopo reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured Joopo TTS provider.
...
- `chrome.guestName: "Joopo Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through Joopo browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  Joopo agent in a per-meeting sub-agent session, and spoken back through the
  normal Joopo TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `joopo_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `joopo_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional Joopo agent id for
  `joopo_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `joopo_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured Joopo
agent produces the answer, and regular Joopo TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means Joopo did
...
transcripts are routed through the configured Joopo agent, and the answer is
spoken through the normal Joopo TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Joopo agent | Normal Joopo TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal Joopo tools, it can call `joopo_agent_consult`.

The consult tool runs the regular Joopo agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
Joopo sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated Joopo agent workspace, model defaults,
...
```bash
joopo googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `joopo logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
joopo plugins list | grep google-meet
joopo googlemeet setup
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; Joopo
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, Joopo
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `JOOPO_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `joopo googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the Joopo Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. Joopo should click **Use microphone** or, for
...
```bash
joopo googlemeet setup
joopo googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> Joopo agent -> TTS talk-back path,
...
run `joopo googlemeet status --json <session-id>` after participants speak
...
utterance but Joopo did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  Joopo. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
joopo googlemeet recover-tab
joopo googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
joopo voicecall smoke --to "+15555550123"
```
...
```bash
joopo voicecall smoke --to "+15555550123" --yes
```
...
- Run `joopo googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `joopo voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `joopo voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `joopo logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `joopo googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `joopo logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: Joopo owns the
