...
If the active memory plugin exposes shared recall artifacts, Joopo can search
...
currently exposing public bridge inputs yet. Run `joopo wiki doctor` first,
...
`joopo wiki status`, `joopo wiki doctor`, and `joopo wiki bridge
...
joopo wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
