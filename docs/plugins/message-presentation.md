...
Message presentation is Joopo's shared contract for rich outbound chat UI.
...
```bash
joopo message send --channel slack \
  --target channel:C123 \
...
```bash
joopo message send --channel telegram \
...
Use helpers from `joopo/plugin-sdk/interactive-runtime` when bridging older
code:
