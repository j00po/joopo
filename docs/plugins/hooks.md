...
Plugin hooks are in-process extension points for Joopo plugins. Use them
...
current plugin options; Joopo injects it per handler without mutating the
...
- Joopo strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, Joopo stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when Joopo can
...
identifier when Joopo can derive one from the session key or delivery
metadata.
...
Joopo continues; it does not cancel plugin-owned network work unless the
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Joopo can derive a
...
Codex native `Stop` hooks are relayed into this hook as Joopo
...
reach the next model turn exactly once. Joopo drains queued injections before
...
context when syncing external wake schedulers, and keep Joopo as the
