...
At startup, Joopo does roughly this:
...
The manifest is the control-plane source of truth. Joopo uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Joopo
...
Joopo does not cache plugin discovery results or direct manifest registry
...
Joopo still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Joopo calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Joopo no longer calls, such as
...
| --  | _(built-in model lookup)_         | Joopo tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on Joopo's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Joopo's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Joopo adds those
...
- Joopo only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Joopo plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-joopo-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-joopo-scopes` only when the header is explicitly present
  - if `x-joopo-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-joopo-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `joopo/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `joopo/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `joopo/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `joopo/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `joopo/plugin-sdk/config-schema` | Root `joopo.json` Zod schema (`JoopoSchema`) |
...
<Info>
`joopo/plugin-sdk/channel-runtime`, `joopo/plugin-sdk/config-runtime`,
and `joopo/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `joopo/plugin-sdk/*` subpaths. Never
...
`joopo/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Joopo writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Joopo's
...
- if both `catalog` and `discovery` are registered, Joopo uses `catalog`
...
- Read-only command paths such as `joopo status`, `joopo status --all`,
  `joopo channels status`, `joopo channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `joopo.extensions`:
...
Security guardrail: every `joopo.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `joopo plugins install` installs plugin dependencies with a
...
Optional: `joopo.setupEntry` can point at a lightweight setup-only module.
When Joopo needs setup surfaces for a disabled channel plugin, or
...
Optional: `joopo.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Joopo load the
...
Channel plugins can advertise setup/discovery metadata via `joopo.channel` and
install hints via `joopo.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@joopo/nextcloud-talk",
  "joopo": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `joopo.channel` fields beyond the minimal example:
...
Joopo can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.joopo/mpm/plugins.json`
- `~/.joopo/mpm/catalog.json`
- `~/.joopo/plugins/catalog.json`

Or point `JOOPO_PLUGIN_CATALOG_PATHS` (or `JOOPO_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "joopo": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `joopo.install` block. The
...
2. add typed plugin registration/runtime surfaces
   Extend `JoopoPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Joopo stays opinionated without becoming hardcoded to one
