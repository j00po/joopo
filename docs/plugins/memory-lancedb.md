...
Set `dimensions` for non-standard embedding models. Joopo knows the
...
`memory_forget` query path, and `joopo ltm search`. Auto-recall prefers the
...
```bash
joopo ltm list
joopo ltm search "project preferences"
joopo ltm stats
```

The plugin also extends `joopo memory` with a non-vector `query` subcommand
that runs against the LanceDB table directly:

```bash
joopo memory query --cols id,text,createdAt --limit 20
joopo memory query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.joopo/memory/lancedb`. Override the
path with `dbPath`:
...
Joopo treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Joopo and restart the
Gateway.
...
```bash
joopo ltm stats
joopo ltm search "recent preference"
```
