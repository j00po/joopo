...
read_when:
  - You are building an Joopo plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Joopo plugin manifest** only.
...
Joopo auto-detects those bundle layouts too, but they are not validated
against the `joopo.plugin.json` schema described here.

For compatible bundles, Joopo currently reads bundle metadata plus declared
...
Joopo runtime expectations.

Every native Joopo plugin **must** ship a `joopo.plugin.json` file in the
**plugin root**. Joopo uses this manifest to validate configuration
...
`joopo.plugin.json` is the metadata Joopo reads **before it loads your
...
- QA runner metadata the shared `joopo qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Joopo still reads this during the deprecation window.                                                    |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                          |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Joopo can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                               |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `joopo qa` host before plugin runtime loads.                                                                                                                                         |
...
Joopo reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Joopo can
...
If a tool has no `toolMetadata`, Joopo preserves the existing behavior and
...
Joopo reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Joopo falls back to `choiceId`.                                           |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Joopo
...
the shared `joopo qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `joopo qa`, for example `matrix`.       |
...
Joopo also includes `setup.providers[].envVars` in generic provider auth and
...
Joopo can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Joopo treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `joopo.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Joopo reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Joopo can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `joopo migrate`.             |
...
Joopo reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Joopo can still load the plugin, but
...
the same defaults through `package.json#joopo.channel.commands` alongside
...
When `channels.chat` is configured, Joopo considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Joopo disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Joopo
...
Use `modelSupport` when Joopo should infer your provider plugin from
...
Use `modelCatalog` when Joopo should know provider model metadata before
...
provider-filtered list uses an alias, Joopo can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Joopo model config compatibility.     |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Joopo
...
| Field                      | Type               | What it means                                                                                                     |
| -------------------------- | ------------------ | ----------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Joopo provider id, for example `z-ai` for a `zai` provider. |
...
### Joopo Provider Index

The Joopo Provider Index is Joopo-owned preview metadata for providers
...
4. Joopo Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `joopo doctor --fix` to
...
| File                | Use it for                                                                                                                    |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `joopo.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                      |
| `package.json`      | npm metadata, dependency installation, and the `joopo` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Joopo must know it before loading plugin code, put it in `joopo.plugin.json`
...
`joopo` block instead of `joopo.plugin.json`.
`joopo.bundle` and `joopo.bundle.json` are not Joopo plugin contracts;
native plugins must use `joopo.plugin.json` plus the supported
`package.json#joopo` fields below.

Important examples:

| Field                                                                              | What it means                                                                                                                                                                        |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `joopo.extensions`                                                                 | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `joopo.runtimeExtensions`                                                          | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `joopo.setupEntry`                                                                 | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `joopo.runtimeSetupEntry`                                                          | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `joopo.channel`                                                                    | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `joopo.channel.commands`                                                           | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `joopo.channel.configuredState`                                                    | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `joopo.channel.persistedAuthState`                                                 | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `joopo.install.joopohubSpec` / `joopo.install.npmSpec` / `joopo.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `joopo.install.defaultChoice`                                                      | Preferred install path when multiple install sources are available.                                                                                                                  |
| `joopo.install.minHostVersion`                                                     | Minimum supported Joopo host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                                |
| `joopo.install.expectedIntegrity`                                                  | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `joopo.install.allowInvalidConfigRecovery`                                         | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `joopo.startup.deferConfiguredChannelFullLoadUntilAfterListen`                     | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#joopo.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `joopo.plugin.json`.

`joopo.install.minHostVersion` is enforced during install and manifest
...
Official install-on-demand metadata should use `joopohubSpec` when the plugin is
published on JoopoHub; onboarding treats that as the preferred remote source and
records JoopoHub artifact facts after install. `npmSpec` remains the compatibility
fallback for packages that have not moved to JoopoHub yet.

Exact npm version pinning already lives in `npmSpec`, for example
`"npmSpec": "@wecom/wecom-joopo-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `joopo.setupEntry` when status, channel list,
...
entrypoint fields. For example, `joopo.runtimeExtensions` cannot make an
escaping `joopo.extensions` path loadable.

`joopo.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`joopo.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`joopo.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Joopo discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.

Precedence, highest to lowest:

1. **Config-selected** — a path explicitly pinned in `plugins.entries.<id>`
2. **Bundled** — plugins shipped with Joopo
3. **Global install** — plugins installed into the global Joopo plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `joopo.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Joopo plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `JoopoPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
