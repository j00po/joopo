---
summary: "How Joopo installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Joopo installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Joopo keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Joopo
package directory.
...
- SDK/core imports are peer or supplied Joopo imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Joopo-owned package roots

Joopo owns only the plugin lifecycle:
...
Joopo uses stable per-source roots:

- npm packages install under `~/.joopo/npm`
- git packages clone under `~/.joopo/git`
- local/path/archive installs are copied or referenced without dependency repair

npm installs run in the npm root with:

```bash
npm install --prefix ~/.joopo/npm <spec> --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`joopo plugins install npm-pack:<path.tgz>` uses that same managed npm root
for a local npm-pack tarball. Joopo reads the tarball's npm metadata, adds it
...
npm may hoist transitive dependencies to `~/.joopo/npm/node_modules` beside
the plugin package. Joopo scans the managed npm root before trusting the
...
Plugins that import `joopo/plugin-sdk/*` declare `joopo` as a peer
dependency. Joopo does not let npm install a separate registry copy of the
...
resolution/materialization for the shared root and Joopo reasserts
plugin-local `node_modules/joopo` links for installed packages that declare
...
Local plugins are treated as developer-controlled directories. Joopo does not
...
joopo plugins install <source>
joopo doctor --fix
```

`doctor --fix` can clean legacy Joopo-generated dependency state and recover
...
Older Joopo versions generated bundled-plugin dependency roots at startup or
...
`.joopo-runtime-deps*` manifests, generated plugin `node_modules`, install
