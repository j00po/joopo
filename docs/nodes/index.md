...
`joopo nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`joopo node run` or the
...
joopo devices reject <requestId>
joopo nodes status
...
`joopo devices list` before approving.
...
- `node.pair.*` (CLI: `joopo nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `joopo nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.joopo/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, Joopo also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If Joopo cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
joopo node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export JOOPO_GATEWAY_TOKEN="<gateway-token>"
joopo node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `joopo node run` supports token or password auth.
- Env vars are preferred: `JOOPO_GATEWAY_TOKEN` / `JOOPO_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `JOOPO_GATEWAY_*` env vars.

### Start a node host (service)

```bash
joopo node install --host <gateway-host> --port 18789 --display-name "Build Node"
joopo node start
joopo node restart
```
...
If the node retries with changed auth details, re-run `joopo devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `joopo node run` / `joopo node install` (persists in `~/.joopo/node.json` on the node).
- `joopo nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
joopo approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
joopo approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.joopo/exec-approvals.json`.
...
```bash
joopo config set tools.exec.host node
joopo config set tools.exec.security allowlist
joopo config set tools.exec.node "<id-or-name>"
```
...
```bash
joopo nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
joopo nodes canvas snapshot --node <idOrNameOrIp> --format png
joopo nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
joopo nodes canvas present --node <idOrNameOrIp> --target https://example.com
joopo nodes canvas hide --node <idOrNameOrIp>
joopo nodes canvas navigate https://example.com --node <idOrNameOrIp>
joopo nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
joopo nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
joopo nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
joopo nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
joopo nodes camera list --node <idOrNameOrIp>
joopo nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
joopo nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
joopo nodes camera clip --node <idOrNameOrIp> --duration 10s
joopo nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
joopo nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
joopo nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
joopo nodes location get --node <idOrNameOrIp>
joopo nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
joopo nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from Joopo"}'
```
...
```bash
joopo nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
joopo nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
joopo nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
joopo nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
joopo nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.joopo/exec-approvals.json`).
...
```bash
joopo config set tools.exec.node "node-id-or-name"
```
...
```bash
joopo config unset tools.exec.node
joopo config unset agents.list[0].tools.exec.node
```
...
Joopo can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
joopo node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.joopo/node.json`.
- Exec approvals are enforced locally via `~/.joopo/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `JOOPO_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `JOOPO_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `joopo nodes …` works against this Mac).
