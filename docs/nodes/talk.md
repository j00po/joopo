...
- Browser realtime Talk uses `talk.client.toolCall` for `joopo_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `joopo-mlx-tts` helper when present, or an executable on `PATH`. Set `JOOPO_MLX_TTS_BIN` to point at a custom helper binary during development.
