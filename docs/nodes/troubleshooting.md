...
```bash
joopo nodes describe --node <idOrNameOrIp>
joopo nodes canvas snapshot --node <idOrNameOrIp>
joopo logs --follow
```
...
joopo nodes status
joopo approvals get --node <idOrNameOrIp>
joopo approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`joopo approvals get --node ...`), not in the gateway pairing record.
