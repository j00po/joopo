...
Joopo supports **camera capture** for agent workflows:
...
```bash
joopo nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
joopo nodes camera snap --node <id> --facing front
joopo nodes camera clip --node <id> --duration 3000
joopo nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`joopo.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `joopo` CLI to invoke camera commands on the macOS node.

Examples:

```bash
joopo nodes camera list --node <id>            # list camera ids
joopo nodes camera snap --node <id>            # prints MEDIA:<path>
joopo nodes camera snap --node <id> --max-width 1280
joopo nodes camera snap --node <id> --delay-ms 2000
joopo nodes camera snap --node <id> --device-id <id>
joopo nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
joopo nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
joopo nodes camera clip --node <id> --device-id <id>
joopo nodes camera clip --node <id> --no-audio
```

Notes:

- `joopo nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
joopo nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
