...
Explicit copy flows, such as `joopo agents add`, use this portability policy:
...
has such a marker, `joopo doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but Joopo cannot resolve a probeable model
