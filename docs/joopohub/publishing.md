---
summary: "How JoopoHub publishing works for skills, plugins, owners, scopes, releases, and review."
read_when:
...
# Publishing on JoopoHub

JoopoHub publishing is owner-scoped: every publish targets a publisher, and the
server decides whether the signed-in user is allowed to publish there.

## Owners

An owner is a JoopoHub publisher handle, such as `@alice` or `@joopo`.
...
```text
https://joopohub.ai/<owner>/<slug>
```

Example:

```text
https://joopohub.ai/alice/review-helper
```
...
`@joopo/dronzer`, it can only be published as `@joopo`. If you publish as
...
2. The publish request is sent to JoopoHub with the selected owner.
3. The server validates owner permissions, package scope, package name, version,
   file limits, and source metadata.
4. JoopoHub stores the release and starts automated security checks.
...
If the package scope and selected owner do not match, JoopoHub rejects the
publish:

```text
Package scope "@joopo" must match selected owner "@vintageayu".
Publish as "@joopo" or rename this package to "@vintageayu/dronzer".
```
...
```sh
joopohub package transfer @opik/opik-joopo --to opik
```
...
This protects org namespaces. A package named `@joopo/dronzer` claims the
`@joopo` namespace, so only publishers with access to the `@joopo` owner
can publish it.
