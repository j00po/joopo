...
<p align="center">
    <picture>
        <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/joopo/joopo/main/docs/assets/joopo-logo-text-dark.svg">
        <img src="https://raw.githubusercontent.com/joopo/joopo/main/docs/assets/joopo-logo-text.svg" alt="Joopo" width="500">
    </picture>
</p>
...
**Joopo** is a _personal AI assistant_ you run on your own devices.
...
Model note: while many providers and models are supported, prefer a current flagship model from the provider you trust and already use. See [Onboarding](https://docs.joopo.ai/start/onboarding).
...
```bash
npm install -g joopo@latest
# or: pnpm add -g joopo@latest

joopo onboard --install-daemon
```

Joopo Onboard installs the Gateway daemon (launchd/systemd user service) so it stays running.

## Quick start (TL;DR)

Runtime: **Node 24 (recommended) or Node 22.16+**.

Full beginner guide (auth, pairing, channels): [Getting started](https://docs.joopo.ai/start/getting-started)

```bash
joopo onboard --install-daemon

joopo gateway --port 18789 --verbose

# Send a message
joopo message send --target +1234567890 --message "Hello from Joopo"
...
joopo agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.joopo.ai/install/updating) (and run `joopo doctor`).

Models config + CLI: [Models](https://docs.joopo.ai/concepts/models). Auth profile rotation + fallbacks: [Model failover](https://docs.joopo.ai/concepts/model-failover).

## Security defaults (DM access)

Joopo connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.joopo.ai/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:

- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dmPolicy="pairing"` / `channels.slack.dmPolicy="pairing"`; legacy: `channels.discord.dm.policy`, `channels.slack.dm.policy`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `joopo pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`).

Run `joopo doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.joopo.ai/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.joopo.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, IRC, Microsoft Teams, Matrix, Feishu, LINE, Mattermost, Nextcloud Talk, Nostr, Synology Chat, Tlon, Twitch, Zalo, Zalo Personal, WeChat, QQ, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.joopo.ai/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
- **[Voice Wake](https://docs.joopo.ai/nodes/voicewake) + [Talk Mode](https://docs.joopo.ai/nodes/talk)** — wake words on macOS/iOS and continuous voice on Android (ElevenLabs + system TTS fallback).
- **[Live Canvas](https://docs.joopo.ai/platforms/mac/canvas)** — agent-driven visual workspace with [A2UI](https://docs.joopo.ai/platforms/mac/canvas#canvas-a2ui).
- **[First-class tools](https://docs.joopo.ai/tools)** — browser, canvas, nodes, cron, sessions, and Discord/Slack actions.
- **[Companion apps](https://docs.joopo.ai/platforms/macos)** — macOS menu bar app + iOS/Android [nodes](https://docs.joopo.ai/nodes).
- **[Onboarding](https://docs.joopo.ai/start/wizard) + [skills](https://docs.joopo.ai/tools/skills)** — onboarding-driven setup with bundled/managed/workspace skills.
...
- Before exposing anything remotely, read [Security](https://docs.joopo.ai/gateway/security), [Sandboxing](https://docs.joopo.ai/gateway/sandboxing), and [Configuration](https://docs.joopo.ai/gateway/configuration).
...
- New here: [Getting started](https://docs.joopo.ai/start/getting-started), [Onboarding](https://docs.joopo.ai/start/wizard), [Updating](https://docs.joopo.ai/install/updating)
- Channel setup: [Channels index](https://docs.joopo.ai/channels), [WhatsApp](https://docs.joopo.ai/channels/whatsapp), [Telegram](https://docs.joopo.ai/channels/telegram), [Discord](https://docs.joopo.ai/channels/discord), [Slack](https://docs.joopo.ai/channels/slack)
- Apps + nodes: [macOS](https://docs.joopo.ai/platforms/macos), [iOS](https://docs.joopo.ai/platforms/ios), [Android](https://docs.joopo.ai/platforms/android), [Nodes](https://docs.joopo.ai/nodes)
- Config + security: [Configuration](https://docs.joopo.ai/gateway/configuration), [Security](https://docs.joopo.ai/gateway/security), [Sandboxing](https://docs.joopo.ai/gateway/sandboxing)
- Remote + web: [Gateway](https://docs.joopo.ai/gateway), [Remote access](https://docs.joopo.ai/gateway/remote), [Tailscale](https://docs.joopo.ai/gateway/tailscale), [Web surfaces](https://docs.joopo.ai/web)
- Tools + automation: [Tools](https://docs.joopo.ai/tools), [Skills](https://docs.joopo.ai/tools/skills), [Cron jobs](https://docs.joopo.ai/automation/cron-jobs), [Webhooks](https://docs.joopo.ai/automation/webhook), [Gmail Pub/Sub](https://docs.joopo.ai/automation/gmail-pubsub)
- Internals: [Architecture](https://docs.joopo.ai/concepts/architecture), [Agent](https://docs.joopo.ai/concepts/agent), [Session model](https://docs.joopo.ai/concepts/session), [Gateway protocol](https://docs.joopo.ai/reference/rpc)
- Troubleshooting: [Channel troubleshooting](https://docs.joopo.ai/channels/troubleshooting), [Logging](https://docs.joopo.ai/logging), [Docs home](https://docs.joopo.ai)
...
Note: signed builds required for macOS permissions to stick across rebuilds (see [macOS Permissions](https://docs.joopo.ai/platforms/mac/permissions)).

### iOS node (optional)

- Pairs as a node over the Gateway WebSocket (device pairing).
- Voice trigger forwarding + Canvas surface.
- Controlled via `joopo nodes …`.

Runbook: [iOS connect](https://docs.joopo.ai/platforms/ios).

### Android node (optional)

- Pairs as a WS node via device pairing (`joopo devices ...`).
- Exposes Connect/Chat/Voice tabs plus Canvas, Camera, Screen capture, and Android device command families.
- Runbook: [Android connect](https://docs.joopo.ai/platforms/android).
...
`pnpm joopo setup` writes the local config/workspace needed for `pnpm gateway:watch`. It is safe to re-run, but you normally only need it on first setup or after resetting local state. `pnpm gateway:watch` does not rebuild `dist/control-ui`, so rerun `pnpm ui:build` after `ui/` changes or use `pnpm ui:dev` when iterating on the Control UI. If you want this checkout to run onboarding directly, use `pnpm joopo onboard --install-daemon`.

Note: `pnpm joopo ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `joopo` binary, while `pnpm gateway:watch` rebuilds the runtime on demand during the dev loop.
...
Switch channels (git + npm): `joopo update --channel stable|beta|dev`.
Details: [Development channels](https://docs.joopo.ai/install/development-channels).

## Agent workspace + skills

- Workspace root: `~/.joopo/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.joopo/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.joopo/joopo.json` (model + defaults):
...
[Full configuration reference (all keys + examples).](https://docs.joopo.ai/gateway/configuration)
