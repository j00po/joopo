...
import { normalizeOptionalLowercaseString, sleep } from "joopo/plugin-sdk/text-runtime";
import { loadWebMedia } from "joopo/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, JoopoConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as JoopoConfig,
      channel: "msteams",
    });
