...
import { isRecord, normalizeOptionalString } from "joopo/plugin-sdk/text-runtime";
...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["joopoPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["joopo", "pollId"]) ??
    readNestedString(value, ["joopo", "poll", "id"]) ??
    readNestedString(value, ["data", "joopoPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "joopo", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "joopo poll vote",
          displayText: "Vote recorded",
          value: { joopoPollId: pollId, pollId },
        },
      },
    ],
  };
