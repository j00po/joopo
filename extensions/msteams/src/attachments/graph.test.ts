...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the Joopo User-Agent to guarded Graph attachment fetches", async () => {
  ...
  it("adds the Joopo User-Agent to Graph shares downloads for reference attachments", async () => {
    mockGraphMediaFetch({
      messageId: "msg-share",
      messageResponse: {
        body: {},
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ Joopo\/.+$/);
  });
});
