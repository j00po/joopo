...
const DEFAULT_SERVICE_NAME = "joopo";
...
const PRELOADED_OTEL_SDK_ENV = "JOOPO_OTEL_PRELOADED";
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "joopo.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "joopo.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "joopo.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("joopo.provider.request", {
    "joopo.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(attributes, "joopo.content.input_messages", event.inputMessages);
  }
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "joopo.content.output_messages",
      event.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "joopo.content.system_prompt", event.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "joopo.content.tool_input", event.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "joopo.content.tool_output", event.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `joopo.${key}`, eventAttributes[rawKey]);
  }
}
...
function addTraceAttributes(
  ...
  attributes["joopo.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["joopo.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["joopo.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["joopo.traceFlags"] = normalized.traceFlags;
  }
}

export function createDiagnosticsOtelService(): JoopoPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("joopo");
      const tracer = trace.getTracer("joopo");
      ...
      const tokensCounter = meter.createCounter("joopo.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("joopo.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("joopo.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("joopo.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("joopo.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("joopo.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("joopo.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("joopo.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("joopo.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageProcessedCounter = meter.createCounter("joopo.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("joopo.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "joopo.message.delivery.started",
        {
          unit: "1",
      ...
      const messageDeliveryDurationHistogram = meter.createHistogram(
        "joopo.message.delivery.duration_ms",
        {
          unit: "ms",
          description: "Outbound message delivery duration",
        },
      );
      const queueDepthHistogram = meter.createHistogram("joopo.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("joopo.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("joopo.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("joopo.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("joopo.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionStuckCounter = meter.createCounter("joopo.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("joopo.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const sessionRecoveryRequestedCounter = meter.createCounter(
        "joopo.session.recovery.requested",
        {
          unit: "1",
      ...
      const sessionRecoveryCompletedCounter = meter.createCounter(
        "joopo.session.recovery.completed",
        {
          unit: "1",
      ...
      const sessionRecoveryAgeHistogram = meter.createHistogram(
        "joopo.session.recovery.age_ms",
        {
          unit: "ms",
          description: "Age of sessions selected for recovery",
        },
      );
      const talkEventCounter = meter.createCounter("joopo.talk.event", {
        unit: "1",
        description: "Talk events emitted by type",
      });
      const talkEventDurationHistogram = meter.createHistogram("joopo.talk.event.duration_ms", {
        unit: "ms",
        description: "Talk event duration when reported",
      });
      const talkAudioBytesHistogram = meter.createHistogram("joopo.talk.audio.bytes", {
        unit: "By",
        description: "Talk audio frame byte lengths",
      });
      const runAttemptCounter = meter.createCounter("joopo.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("joopo.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const modelCallDurationHistogram = meter.createHistogram("joopo.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "joopo.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "joopo.model_call.response_bytes",
        {
          unit: "By",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "joopo.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const execProcessDurationHistogram = meter.createHistogram("joopo.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("joopo.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("joopo.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("joopo.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("joopo.memory.external_bytes", {
        unit: "By",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "joopo.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "joopo.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "joopo.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("joopo.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        const otelLogger = logProvider.getLogger("joopo");
        recordLogRecord = (evt, metadata) => {
          try {
            ...
            assignOtelLogAttribute(attributes, "joopo.log.level", logLevelName);
            if (evt.loggerName) {
              assignOtelLogAttribute(attributes, "joopo.logger", evt.loggerName);
            }
            if (evt.loggerParents?.length) {
              assignOtelLogAttribute(
                attributes,
                "joopo.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["joopo.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["joopo.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["joopo.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["joopo.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "joopo.tool.params.kind": summary.kind,
          ...("length" in summary ? { "joopo.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "joopo.channel": evt.channel ?? "unknown",
          "joopo.agent": lowCardinalityAttr(evt.agentId),
          "joopo.provider": evt.provider ?? "unknown",
          "joopo.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "joopo.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "joopo.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "joopo.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "joopo.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "joopo.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "joopo.token": "total" });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "joopo.tokens.input": usage.input ?? 0,
          "joopo.tokens.output": usage.output ?? 0,
          "joopo.tokens.cache_read": usage.cacheRead ?? 0,
          "joopo.tokens.cache_write": usage.cacheWrite ?? 0,
          "joopo.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("joopo.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "joopo.channel": evt.channel ?? "unknown",
          "joopo.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        const attrs = {
          "joopo.channel": lowCardinalityAttr(evt.channel),
          "joopo.webhook": lowCardinalityAttr(evt.updateType),
        };
        ...
        const span = spanWithDuration("joopo.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "joopo.error": redactedError,
        };
        const span = tracer.startSpan("joopo.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "joopo.channel": lowCardinalityAttr(evt.channel),
          "joopo.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "joopo.channel": lowCardinalityAttr(evt.channel),
          "joopo.outcome": evt.outcome ?? "unknown",
        };
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.reason) {
          spanAttrs["joopo.reason"] = lowCardinalityAttr(evt.reason, "unknown");
        }
        const span = spanWithDuration("joopo.message.processed", spanAttrs, evt.durationMs);
      ...
      const messageDeliveryAttrs = (
        ...
        "joopo.channel": lowCardinalityAttr(evt.channel),
        "joopo.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          ...
          {
            ...attrs,
            "joopo.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
          { endTimeMs: evt.ts },
        );
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...messageDeliveryAttrs(evt),
          "joopo.outcome": "error",
          "joopo.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("joopo.message.delivery", attrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("joopo.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "joopo.lane": evt.lane };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "joopo.state": evt.state };
        if (evt.reason) {
          attrs["joopo.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["joopo.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["joopo.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("joopo.session.stuck", { attributes: spanAttrs });
      ...
      const sessionRecoveryAttrs = (evt: SessionRecoveryDiagnosticEvent) => {
        ...
        if (evt.activeWorkKind) {
          attrs["joopo.active_work_kind"] = evt.activeWorkKind;
        }
        return attrs;
      };
      ...
      const recordSessionRecoveryRequested = (
        ...
        attrs["joopo.action"] = evt.allowActiveAbort ? "abort" : "recover";
      ...
      const recordSessionRecoveryCompleted = (
        ...
        const attrs = sessionRecoveryAttrs(evt);
        attrs["joopo.status"] = evt.status;
        attrs["joopo.action"] = lowCardinalityAttr(evt.action, "unknown");
        if (evt.outcomeReason) {
          attrs["joopo.reason"] = redactSensitiveText(evt.outcomeReason);
        }
      ...
      const talkEventAttrs = (evt: TalkDiagnosticEvent): Record<string, string> => ({
        "joopo.talk.brain": lowCardinalityAttr(evt.brain),
        "joopo.talk.event_type": lowCardinalityAttr(evt.talkEventType),
        "joopo.talk.mode": lowCardinalityAttr(evt.mode),
        "joopo.talk.provider": lowCardinalityAttr(evt.provider),
        "joopo.talk.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "joopo.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "joopo.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "joopo.loop.level": evt.level,
        "joopo.loop.action": evt.action,
        "joopo.loop.detector": evt.detector,
        "joopo.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "joopo.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("joopo.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "joopo.memory.level": evt.level,
          "joopo.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "joopo.memory.rss_bytes": evt.memory.rssBytes,
          "joopo.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "joopo.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "joopo.memory.external_bytes": evt.memory.externalBytes,
          "joopo.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "joopo.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "joopo.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "joopo.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("joopo.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "joopo.outcome": evt.outcome,
          "joopo.provider": evt.provider ?? "unknown",
          "joopo.model": evt.model ?? "unknown",
        };
        if (evt.channel) {
          attrs["joopo.channel"] = evt.channel;
        }
        if (evt.blockedBy) {
          attrs["joopo.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.blockedBy) {
          spanAttrs["joopo.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        if (evt.errorCategory) {
          spanAttrs["joopo.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("joopo.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "joopo.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "joopo.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "joopo.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "joopo.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "joopo.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "joopo.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("joopo.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["joopo.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["joopo.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["joopo.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["joopo.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["joopo.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("joopo.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "joopo.harness.phase": evt.phase,
          "joopo.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "joopo.harness.cleanup_failed": true } : {}),
        };
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("joopo.harness.run", spanAttrs, evt.durationMs, {
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "joopo.context.message_count": evt.messageCount,
          "joopo.context.history_text_chars": evt.historyTextChars,
          "joopo.context.history_image_blocks": evt.historyImageBlocks,
          "joopo.context.max_message_text_chars": evt.maxMessageTextChars,
          "joopo.context.system_prompt_chars": evt.systemPromptChars,
          "joopo.context.prompt_chars": evt.promptChars,
          "joopo.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["joopo.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["joopo.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("joopo.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const recordModelFailover = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "joopo.failover.reason": lowCardinalityAttr(evt.reason, "unknown"),
        };
        if (evt.fromProvider) {
          spanAttrs["joopo.provider"] = evt.fromProvider;
        }
        if (evt.fromModel) {
          spanAttrs["joopo.model"] = evt.fromModel;
        }
        if (evt.toProvider) {
          spanAttrs["joopo.failover.to_provider"] = evt.toProvider;
        }
        if (evt.toModel) {
          spanAttrs["joopo.failover.to_model"] = evt.toModel;
        }
        if (evt.lane) {
          spanAttrs["joopo.lane"] = lowCardinalityAttr(evt.lane, "unknown");
        }
        if (evt.suspended !== undefined) {
          spanAttrs["joopo.failover.suspended"] = evt.suspended;
        }
        if (evt.cascadeDepth !== undefined) {
          spanAttrs["joopo.failover.cascade_depth"] = evt.cascadeDepth;
        }
        const span = spanWithDuration("joopo.model.failover", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "joopo.provider": evt.provider,
        "joopo.model": evt.model,
        "joopo.api": lowCardinalityAttr(evt.api),
        "joopo.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["joopo.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["joopo.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("joopo.model.call", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordModelCallCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("joopo.model.call", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "joopo.errorCategory": errorType,
          ...(evt.failureKind
            ? { "joopo.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["joopo.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["joopo.api"] = evt.api;
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("joopo.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("joopo.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["joopo.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(
          spanAttrs,
          evt as unknown as Record<string, unknown>,
      ...
      const recordToolExecutionBlocked = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          "joopo.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("joopo.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "joopo.exec.target": evt.target,
          "joopo.exec.mode": evt.mode,
          "joopo.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["joopo.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "joopo.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["joopo.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["joopo.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["joopo.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("joopo.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "joopo.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "joopo.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "joopo.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "joopo.liveness.active": evt.active,
          "joopo.liveness.waiting": evt.waiting,
          "joopo.liveness.queued": evt.queued,
          "joopo.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "joopo.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "joopo.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "joopo.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "joopo.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "joopo.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "joopo.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "joopo.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("joopo.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordDiagnosticPhaseCompleted = (
        ...
        const spanAttrs: Record<string, string | number> = {
          "joopo.phase": lowCardinalityAttr(evt.name, "unknown"),
          ...(evt.cpuUserMs !== undefined ? { "joopo.phase.cpu_user_ms": evt.cpuUserMs } : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "joopo.phase.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "joopo.phase.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "joopo.phase.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        for (const [key, value] of Object.entries(evt.details ?? {})) {
          spanAttrs[`joopo.phase.detail.${key}`] =
            typeof value === "boolean" ? String(value) : value;
        }
        const span = spanWithDuration("joopo.diagnostic.phase", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "joopo.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "joopo.signal": evt.signal,
          "joopo.status": evt.status,
          ...(evt.reason ? { "joopo.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "joopo.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
