...
import type { AnyAgentTool } from "joopo/plugin-sdk/agent-runtime";
import type { PluginTrustedToolPolicyRegistration } from "joopo/plugin-sdk/core";
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-skill-workshop-test-"));
  tempDirs.push(dir);
  return dir;
}
