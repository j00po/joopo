...
import type { JoopoPluginApi, JoopoPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<JoopoPluginApi> = {}): JoopoPluginApi {
...
function fakeCtx(overrides: Partial<JoopoPluginToolContext> = {}): JoopoPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: JoopoPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
