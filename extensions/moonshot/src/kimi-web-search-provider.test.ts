...
describe("kimi web search provider", () => {
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    await withEnvAsync({ KIMI_API_KEY: undefined, MOONSHOT_API_KEY: undefined }, async () => {
      ...
      const result = await tool.execute({ query: "Joopo docs" });
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "Joopo is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result).toMatchObject({
        provider: "kimi",
        content: expect.stringContaining("Joopo is available on GitHub."),
        citations: [],
      });
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "Joopo", url: "https://github.com/joopo/joopo" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "Joopo is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result).toMatchObject({
        provider: "kimi",
        content: expect.stringContaining("Joopo is on GitHub."),
        citations: ["https://github.com/joopo/joopo"],
      });
      expect(result).not.toHaveProperty("error");
    });
  });
