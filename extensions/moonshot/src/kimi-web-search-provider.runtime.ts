import { createProviderHttpError } from "joopo/plugin-sdk/provider-http";
...
function resolveKimiBaseUrl(kimi?: KimiConfig, joopoConfig?: JoopoConfig): string {
  ...
  const moonshotBaseUrl = joopoConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: JoopoConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.joopo.ai/tools/kimi-search",
