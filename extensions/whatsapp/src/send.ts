...
import { generateSecureUuid } from "joopo/plugin-sdk/core";
import { redactIdentifier } from "joopo/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "joopo/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "joopo/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "joopo/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "joopo/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: JoopoConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`joopo channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: JoopoConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
