...
import { logInfo } from "joopo/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("joopo channels login")} and scan the QR again.`,
        ),
      );
