...
import type { ChannelSetupWizard } from "joopo/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "joopo/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<JoopoConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: JoopoConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: JoopoConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: JoopoConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: JoopoConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Joopo can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Joopo" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("joopo channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
