...
import { info, success } from "joopo/plugin-sdk/runtime-env";
import { getChildLogger } from "joopo/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "joopo/plugin-sdk/runtime-env";
import { replaceFileAtomic } from "joopo/plugin-sdk/security-runtime";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("joopo channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
