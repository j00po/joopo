...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "joopo-test-web-logout-"));
    previousOAuthDir = process.env.JOOPO_OAUTH_DIR;
    process.env.JOOPO_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.JOOPO_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the Joopo auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Joopo auth root", async () => {
