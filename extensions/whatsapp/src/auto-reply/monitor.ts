import { resolveAccountEntry } from "joopo/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "joopo/plugin-sdk/channel-inbound-debounce";
import { formatCliCommand } from "joopo/plugin-sdk/cli-runtime";
import { hasControlCommand } from "joopo/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "joopo/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "joopo/plugin-sdk/reply-history";
import { resolveAgentRoute } from "joopo/plugin-sdk/routing";
import { logVerbose } from "joopo/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "joopo/plugin-sdk/runtime-env";
import { getChildLogger } from "joopo/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "joopo/plugin-sdk/runtime-env";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("joopo channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("joopo channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("joopo channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("joopo channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
