...
import { sleep } from "joopo/plugin-sdk/text-runtime";
...
vi.mock("joopo/plugin-sdk/text-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/text-runtime")>(
    "joopo/plugin-sdk/text-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
