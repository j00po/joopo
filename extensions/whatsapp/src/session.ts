...
import { VERSION } from "joopo/plugin-sdk/cli-runtime";
import {
  resolveEnvHttpProxyUrl,
  shouldUseEnvHttpProxyForUrl,
} from "joopo/plugin-sdk/fetch-runtime";
import { danger, success } from "joopo/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "joopo/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "joopo/plugin-sdk/text-runtime";
import {
  readCredsJsonRaw,
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("joopo channels login")}`,
              ),
            );
          }
        }
