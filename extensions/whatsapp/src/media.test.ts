...
import { captureEnv } from "joopo/plugin-sdk/test-env";
import { mockPinnedHostnameResolution } from "joopo/plugin-sdk/test-env";
import { optimizeImageToPng } from "joopo/plugin-sdk/web-media";
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub JOOPO_STATE_DIR.
    // Also keep it outside the Joopo temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["JOOPO_STATE_DIR"]);
    process.env.JOOPO_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "joopo-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredJoopoTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/joopo` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredJoopoTmpDir();
  ...
  it("allows default Joopo state workspace and sandbox roots", async () => {
  ...
  it("rejects default Joopo state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
