...
import { formatEnvelopeTimestamp } from "joopo/plugin-sdk/channel-test-helpers";
import { normalizeLowercaseStringOrEmpty } from "joopo/plugin-sdk/string-coerce-runtime";
import { vi } from "vitest";
import type { MockBaileysSocket } from "../../../test/mocks/baileys.js";
import { createMockBaileys } from "../../../test/mocks/baileys.js";

// Use globalThis to store the mock config so it survives vi.mock hoisting
const CONFIG_KEY = Symbol.for("joopo:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("joopo:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[joopo]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/joopo-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("joopo:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("joopo:lastSocket")];
  if (typeof getter === "function") {
