import { createQueuedWizardPrompter } from "joopo/plugin-sdk/plugin-test-runtime";
import { DEFAULT_ACCOUNT_ID } from "joopo/plugin-sdk/routing";
  ...
  return {
    ...
    splitSetupEntries: (raw: string) =>
      raw
        ...
        .filter(Boolean),
    setSetupChannelEnabled: (cfg: JoopoConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<JoopoConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
