...
vi.mock("joopo/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/channel-pairing")>(
...
vi.mock("joopo/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/media-store")>(
    "joopo/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `joopo-inbound-media-${crypto.randomUUID()}`);
