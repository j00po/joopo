...
let resetLogger: typeof import("joopo/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("joopo/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: JoopoConfig = {
  channels: { whatsapp: {} },
};
...
vi.mock("joopo/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/media-runtime")>(
    "joopo/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("joopo/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `joopo-outbound-${crypto.randomUUID()}.log`);
