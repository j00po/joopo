...
function collectTelegramPollingRuntimeIssues(params: {
  ...
  const fix = `Run: ${formatCliCommand("joopo channels status --probe")} (or restart the gateway). Check the bot token, proxy/network settings, and logs if it persists.`;
...
function collectTelegramWebhookRuntimeIssues(params: {
  ...
  issues.push({
    ...
    fix: `Run: ${formatCliCommand("joopo channels status --probe")} (or restart the gateway). Check the webhook URL, secret, TLS/proxy reachability, and Telegram setWebhook logs if it persists.`,
  });
}
