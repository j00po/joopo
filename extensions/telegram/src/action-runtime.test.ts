...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): JoopoConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as JoopoConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): JoopoConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as JoopoConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: JoopoConfig, accountId = "media") {
    await handleTelegramAction(
