...
vi.mock("joopo/plugin-sdk/runtime-secret-resolution", () => {
  return {
...
describe("telegram doctor", () => {
  ...
  it("formats invalid allowFrom warnings", () => {
    ...
    expect(warnings[1]).toContain("joopo doctor --fix");
  });
  ...
  it("warns and repairs Telegram apiRoot values that include the bot endpoint", () => {
    ...
    expect(
      collectTelegramApiRootWarnings({ hits, doctorFixCommand: "joopo doctor --fix" }),
    ).toContain(
