...
function resolveDefaultAgentId(cfg: JoopoConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: JoopoConfig): string[] {
...
function listBoundAccountIds(cfg: JoopoConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: JoopoConfig, channelId: string): string | null {
...
export function resolveDefaultTelegramAccountSelection(cfg: JoopoConfig): {
  accountId: string;
  accountIds: string[];
