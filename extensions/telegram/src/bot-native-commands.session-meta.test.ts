...
type ResolveConfiguredBindingRouteFn =
  typeof import("joopo/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("joopo/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("joopo/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
vi.mock("joopo/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/conversation-runtime")>(
    "joopo/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/session-store-runtime")>(
    "joopo/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/command-auth-native")>(
    "joopo/plugin-sdk/command-auth-native",
  );
...
vi.mock("joopo/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/plugin-runtime")>(
    "joopo/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/joopo-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/joopo-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/joopo-sessions.json");
    expect(sendMessage).toHaveBeenCalledWith(
      100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/joopo-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as JoopoConfig,
    ...
    expect(sessionMocks.resolveAndPersistSessionFile).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        storePath: "/tmp/joopo-sessions/sessions.json",
        sessionsDir: "/tmp/joopo-sessions",
        fallbackSessionFile: path.resolve("/tmp/joopo-sessions", "sess-topic-topic-42.jsonl"),
      }),
    );
    expect(pluginRuntimeMocks.executePluginCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/joopo-sessions", "sess-topic-topic-42.jsonl"),
        messageThreadId: 42,
      }),
    );
  });
