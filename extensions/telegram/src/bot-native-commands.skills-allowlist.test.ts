...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "joopo/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("joopo-telegram-skills-");
    await writeSkill({
    ...
    listSkillCommandsForAgents.mockImplementation(
      ({ cfg, agentIds }: { cfg: JoopoConfig; agentIds?: string[] }) =>
