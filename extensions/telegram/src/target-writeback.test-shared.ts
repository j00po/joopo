...
vi.mock("joopo/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/config-mutation")>(
    "joopo/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/cron-store-runtime")>(
    "joopo/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeConfigFile).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ expectedConfigPath: "/tmp/joopo.json" }),
      );
