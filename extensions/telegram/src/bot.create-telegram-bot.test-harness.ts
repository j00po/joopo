...
import type { GetReplyOptions, MsgContext } from "joopo/plugin-sdk/reply-runtime";
...
type GetRuntimeConfigFn =
  typeof import("joopo/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("joopo/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("joopo/plugin-sdk/session-store-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/joopo-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("joopo/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: JoopoConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: JoopoConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: JoopoConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: JoopoConfig) => {
