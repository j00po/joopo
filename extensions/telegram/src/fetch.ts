...
} from "joopo/plugin-sdk/fetch-runtime";
import {
  captureHttpExchange,
  resolveEffectiveDebugProxyUrl,
} from "joopo/plugin-sdk/proxy-capture";
import { resolveRequestUrl } from "joopo/plugin-sdk/request-url";
...
// fix for the leak observed in joopo#68128 is the transport lifecycle that
...
function resolveJoopoProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.JOOPO_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveJoopoProxyUrlForTelegram() : undefined;
