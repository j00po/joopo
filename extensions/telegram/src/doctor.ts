...
function formatTelegramAccountConfigPath(cfg: JoopoConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: JoopoConfig): {
  config: JoopoConfig;
  changes: string[];
} {
...
async function repairTelegramConfig(params: { cfg: JoopoConfig }): Promise<{
  config: JoopoConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: JoopoConfig): Promise<{
  config: JoopoConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("joopo/plugin-sdk/runtime-secret-resolution");
