...
import { replaceFileAtomicSync } from "joopo/plugin-sdk/security-runtime";
import { resolveStorePath } from "joopo/plugin-sdk/session-store-runtime";

const TTL_MS = 24 * 60 * 60 * 1000;
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("joopo.telegramSentMessagesState");
...
function resolveSentMessageStorePath(cfg?: Pick<JoopoConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<JoopoConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<JoopoConfig, "session">): SentMessageStore {
