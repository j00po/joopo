...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<JoopoConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<JoopoConfig["channels"]>["telegram"]>>,
): JoopoConfig {
  return {
    channels: {
      telegram: {
