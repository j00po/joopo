...
import { parseExecApprovalCommandText } from "joopo/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "joopo/plugin-sdk/channel-config-helpers";
import { shouldDebounceTextInbound } from "joopo/plugin-sdk/channel-inbound";
import {
  createInboundDebouncer,
  resolveInboundDebounceMs,
} from "joopo/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "joopo/plugin-sdk/command-auth";
import { resolveCommandAuthorizedFromAuthorizers } from "joopo/plugin-sdk/command-auth-native";
import { buildCommandsMessagePaginated } from "joopo/plugin-sdk/command-status";
import { replaceConfigFile } from "joopo/plugin-sdk/config-mutation";
...
import { applyModelOverrideToSessionEntry } from "joopo/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "joopo/plugin-sdk/models-provider-runtime";
import { resolveAgentRoute } from "joopo/plugin-sdk/routing";
import { resolveThreadSessionKeys } from "joopo/plugin-sdk/routing";
import { danger, logVerbose, warn } from "joopo/plugin-sdk/runtime-env";
import {
  loadSessionStore,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in joopo.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
