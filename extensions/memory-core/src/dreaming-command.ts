...
import { resolveMemoryDreamingConfig } from "joopo/plugin-sdk/memory-core-host-status";
import type { JoopoPluginApi, PluginCommandContext } from "joopo/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "joopo/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: JoopoConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: JoopoConfig, enabled: boolean): JoopoConfig {
...
function formatStatus(cfg: JoopoConfig): string {
...
export async function handleDreamingCommand(api: JoopoPluginApi, ctx: PluginCommandContext) {
...
export function registerDreamingCommand(api: JoopoPluginApi): void {
  api.registerCommand({
    name: "dreaming",
