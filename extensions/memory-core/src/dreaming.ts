...
import { peekSystemEventEntries } from "joopo/plugin-sdk/system-event-runtime";
...
export function registerShortTermPromotionDreaming(api: JoopoPluginApi): void {
  ...
  const resolveCurrentConfig = (): JoopoConfig =>
    (api.runtime.config?.current?.() ?? api.config) as JoopoConfig;
