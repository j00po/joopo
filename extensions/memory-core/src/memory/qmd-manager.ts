...
import { withFileLock } from "joopo/plugin-sdk/file-lock";
import {
...
const QMD_UPDATE_QUEUE_KEY = Symbol.for("joopo.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Joopo search semantics match direct qmd search.
  return trimmed;
}
