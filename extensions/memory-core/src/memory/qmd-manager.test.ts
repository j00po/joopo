...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("joopo.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("joopo.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("joopo.qmdEmbedQueueTail");
...
vi.mock("joopo/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("joopo/plugin-sdk/memory-core-host-engine-foundation")
  >("joopo/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/file-lock")>(
    "joopo/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { JoopoConfig } from "joopo/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
