...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "joopo/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("joopo/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): JoopoConfig {
  return {
    agents: {
      defaults: {
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: JoopoConfig;
  secondCfg: JoopoConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("joopo.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-qmd-workspace-"));
