...
import type {
  MemorySearchConfig,
  JoopoConfig,
} from "joopo/plugin-sdk/memory-core-host-engine-foundation";
...
const { createdWatchers, memoryLoggerWarn, watchMock } = vi.hoisted(() => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("joopo.test.memoryWatchFactory")] =
    result.watchMock;
  return result;
});

vi.mock("joopo/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("joopo/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
} from "joopo/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  afterAll(() => {
    Reflect.deleteProperty(globalThis, Symbol.for("joopo.test.memoryWatchFactory"));
  });
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): JoopoConfig {
    const defaults: NonNullable<NonNullable<JoopoConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: JoopoConfig) {
