...
function resolveStoredDreaming(config: JoopoConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: JoopoConfig = {}) {
  const registered: { command?: JoopoPluginCommandDefinition } = {};
  let runtimeConfig: JoopoConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: JoopoConfig }) => {
        runtimeConfig = nextConfig;
      }),
      writeConfigFile: vi.fn(async (nextConfig: JoopoConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: JoopoPluginCommandDefinition) => {
