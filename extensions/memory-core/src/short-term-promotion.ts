...
import { formatMemoryDreamingDay } from "joopo/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "joopo/plugin-sdk/memory-host-events";
import { privateFileStore } from "joopo/plugin-sdk/security-runtime";
...
const PROMOTION_MARKER_PREFIX = "joopo-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*joopo-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*joopo-memory-promotion:([^\n]+?)\s*-->/gi);
