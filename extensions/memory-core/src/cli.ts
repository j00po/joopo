    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["joopo memory status", "Show index and provider status."],
          [
            "joopo memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["joopo memory status --deep", "Probe embedding provider readiness."],
          ["joopo memory index --force", "Force a full reindex."],
          ['joopo memory search "meeting notes"', "Quick search using positional query."],
          [
            'joopo memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `joopo memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "joopo memory rem-backfill --path ./memory",
          ...
          ["joopo memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.joopo.ai/cli/memory")}\n`,
    );
