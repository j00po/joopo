...
} from "joopo/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "joopo/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "joopo/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "joopo/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "joopo/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("joopo.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("joopo:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("joopo-dreaming-backfill-");
    ...
    expect(content.match(/joopo:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("joopo-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- joopo:dreaming:diary:start -->");
    expect(content).toContain("<!-- joopo:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- joopo:dreaming:diary:start -->");
    const end = content.indexOf("<!-- joopo:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- joopo:dreaming:light:start -->\n- Candidate: test\n<!-- joopo:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("joopo-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("joopo-dreaming-narrative-");
    const stateDir = await createTempWorkspace("joopo-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("joopo-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("joopo-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("joopo-dreaming-detach-");
    const logger = createMockLogger();
