...
import type { MemoryEmbeddingProbeResult } from "joopo/plugin-sdk/memory-core-host-engine-storage";
import { resolveMemoryRemDreamingConfig } from "joopo/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "joopo/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: JoopoConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredJoopoTmpDir(), "joopo-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: JoopoConfig): string {
...
function resolveAgent(cfg: JoopoConfig, agent?: string) {
...
function resolveAgentIds(cfg: JoopoConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: joopo memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredJoopoTmpDir(), "joopo-rem-backfill-"),
      );
      try {
