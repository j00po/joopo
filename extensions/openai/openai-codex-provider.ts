...
import { loginOpenAICodexOAuth } from "joopo/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "joopo/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "joopo/plugin-sdk/text-runtime";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.joopo.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `joopo models auth login --provider openai-codex` or `joopo configure`.";
}
