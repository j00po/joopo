...
vi.mock("joopo/plugin-sdk/provider-auth-runtime", () => ({
  resolveApiKeyForProvider: resolveApiKeyForProviderMock,
}));

vi.mock("joopo/plugin-sdk/provider-http", () => ({
...
vi.mock("joopo/plugin-sdk/logging-core", () => ({
...
describe("openai image generation provider", () => {
  ...
  it("allows loopback image requests for openai only inside the QA harness envelope", async () => {
    mockGeneratedPngResponse();
    vi.stubEnv("JOOPO_QA_ALLOW_LOCAL_IMAGE_PROVIDER", "1");
