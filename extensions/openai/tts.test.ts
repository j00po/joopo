...
} from "joopo/plugin-sdk/proxy-capture";
...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Joopo attribution headers to native OpenAI speech requests", async () => {
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.JOOPO_DEBUG_PROXY_ENABLED = "1";
      ...
      process.env.JOOPO_DEBUG_PROXY_SESSION_ID = "tts-session";
      ...
      store.upsertSession({
        ...
        sourceScope: "joopo",
        sourceProcess: "joopo",
        dbPath: process.env.JOOPO_DEBUG_PROXY_DB_PATH,
        blobDir: process.env.JOOPO_DEBUG_PROXY_BLOB_DIR,
      });
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.JOOPO_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.JOOPO_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.JOOPO_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
