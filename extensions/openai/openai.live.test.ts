...
import type { ResolvedTtsConfig } from "joopo/plugin-sdk/agent-runtime";
import type { JoopoConfig } from "joopo/plugin-sdk/config-types";
import { encodePngRgba, fillPixel } from "joopo/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "joopo/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "joopo/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.JOOPO_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.JOOPO_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.JOOPO_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.JOOPO_LIVE_TEST === "1";
...
function createLiveConfig(): JoopoConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "Joopo integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing Joopo OpenAI realtime transcription integration test OK.";
