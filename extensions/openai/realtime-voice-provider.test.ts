import { REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ } from "joopo/plugin-sdk/realtime-voice";
...
describe("buildOpenAIRealtimeVoiceProvider", () => {
  ...
  it("adds Joopo attribution headers to native realtime websocket requests", () => {
    vi.stubEnv("JOOPO_VERSION", "2026.3.22");
    const provider = buildOpenAIRealtimeVoiceProvider();
  ...
  it("returns browser-safe Joopo attribution headers for native WebRTC offers", async () => {
    vi.stubEnv("JOOPO_VERSION", "2026.3.22");
  ...
  it("resolves keychain OPENAI_API_KEY refs before creating browser sessions", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:joopo:OPENAI_REALTIME_BROWSER_TEST");
    ...
    expect(execFileSyncMock).toHaveBeenCalledWith(
      "/usr/bin/security",
      ["find-generic-password", "-s", "joopo", "-a", "OPENAI_REALTIME_BROWSER_TEST", "-w"],
      expect.objectContaining({
        encoding: "utf8",
        timeout: 5000,
      }),
  ...
  it("resolves and caches keychain OPENAI_API_KEY refs before creating bridges", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:joopo:OPENAI_REALTIME_BRIDGE_TEST");
  ...
  it("does not resolve keychain refs during configured checks", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:joopo:OPENAI_REALTIME_CONFIGURED_TEST");
    const provider = buildOpenAIRealtimeVoiceProvider();
  ...
  it("fails closed when keychain refs cannot be resolved", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:joopo:OPENAI_REALTIME_MISSING_TEST");
