...
vi.mock("joopo/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("joopo/plugin-sdk/provider-stream-family")>();
...
describe("buildOpenAIProvider", () => {
  ...
  it("owns Codex wrapper composition for responses payloads", () => {
    ...
    expect(result.options?.headers).toMatchObject({
      originator: "joopo",
      "User-Agent": expect.stringMatching(/^joopo\//u),
    });
