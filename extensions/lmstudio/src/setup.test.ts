import { CUSTOM_LOCAL_AUTH_MARKER } from "joopo/plugin-sdk/provider-auth";
import type { JoopoConfig } from "joopo/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "joopo/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "joopo/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "joopo/plugin-sdk/provider-setup";
import type { WizardPrompter } from "joopo/plugin-sdk/setup";
...
vi.mock("joopo/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("joopo/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("joopo/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("joopo/plugin-sdk/provider-auth");
  vi.doUnmock("joopo/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): JoopoConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as JoopoConfig),
    env: params?.env ?? {},
...
describe("lmstudio setup", () => {
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("JOOPO_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
