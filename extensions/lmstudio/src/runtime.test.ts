...
vi.mock("joopo/plugin-sdk/provider-auth-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("joopo/plugin-sdk/provider-auth-runtime")>();
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("joopo/plugin-sdk/provider-auth-runtime");
  vi.resetModules();
});
