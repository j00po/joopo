...
import { withAgentModelAliases } from "joopo/plugin-sdk/provider-onboard";
import {
...
import { WizardCancelledError, type WizardPrompter } from "joopo/plugin-sdk/setup";
import {
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.JOOPO_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: JoopoConfig): JoopoConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<JoopoConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
