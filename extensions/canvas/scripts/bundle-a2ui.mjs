...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.JOOPO_SPARSE_PROFILE || process.env.JOOPO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because JOOPO_A2UI_SKIP_MISSING=1 or JOOPO_SPARSE_PROFILE is set.",
      );
      return;
    }
