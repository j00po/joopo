...
vi.mock("joopo/plugin-sdk/channel-actions", async (importOriginal) => ({
  ...(await importOriginal<typeof import("joopo/plugin-sdk/channel-actions")>()),
...
describe("Canvas tool", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      tempRoot = await mkdtemp(path.join(os.tmpdir(), "joopo-canvas-tool-"));
