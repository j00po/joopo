...
import { themeContext } from "@joopo/a2ui-theme-context";
...
class JoopoA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.joopoA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["joopo:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.joopoCanvasA2UIAction ??
      globalThis.joopoCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.joopoCanvasA2UIAction) {
...
if (!customElements.get("joopo-a2ui-host")) {
  customElements.define("joopo-a2ui-host", JoopoA2UIHost);
}
