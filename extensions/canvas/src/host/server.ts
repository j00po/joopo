...
import { detectMime } from "joopo/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "joopo/plugin-sdk/runtime-env";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.joopoCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.joopoCanvasA2UIAction &&
        typeof window.joopoCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.joopoSendUserAction === "function";
  ...
  window.addEventListener("joopo:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Joopo node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.joopoSendUserAction === "function"
        ? window.joopoSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Joopo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
