...
import { escapeRegExp } from "joopo/plugin-sdk/text-runtime";
import { readRequestBodyWithLimit } from "joopo/plugin-sdk/webhook-ingress";
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this Joopo repo from source + docs before acting.`;
  }
