...
import { sleep } from "joopo/plugin-sdk/runtime-env";
import { appendRegularFile } from "joopo/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/joopo-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/joopo";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/joopo-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `joopo-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "JOOPO_CONFIG_PATH" &&
          key !== "JOOPO_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `JOOPO_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `JOOPO_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm joopo qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredJoopoTmpDir(), `${plan.vmName}-qa-suite-`),
  );
