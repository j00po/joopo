...
const CRABBOX_BIN_ENV = "JOOPO_MANTIS_CRABBOX_BIN";
const CRABBOX_PROVIDER_ENV = "JOOPO_MANTIS_CRABBOX_PROVIDER";
const CRABBOX_CLASS_ENV = "JOOPO_MANTIS_CRABBOX_CLASS";
const CRABBOX_LEASE_ID_ENV = "JOOPO_MANTIS_CRABBOX_LEASE_ID";
const CRABBOX_KEEP_ENV = "JOOPO_MANTIS_KEEP_VM";
const CRABBOX_IDLE_TIMEOUT_ENV = "JOOPO_MANTIS_CRABBOX_IDLE_TIMEOUT";
const CRABBOX_TTL_ENV = "JOOPO_MANTIS_CRABBOX_TTL";
const HYDRATE_MODE_ENV = "JOOPO_MANTIS_HYDRATE_MODE";
const SLACK_URL_ENV = "JOOPO_MANTIS_SLACK_URL";
const SLACK_CHANNEL_ID_ENV = "JOOPO_MANTIS_SLACK_CHANNEL_ID";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.JOOPO_LIVE_OPENAI_KEY) && trimToValue(next.OPENAI_API_KEY)) {
    next.JOOPO_LIVE_OPENAI_KEY = next.OPENAI_API_KEY;
  }
  if (!trimToValue(next.JOOPO_MANTIS_SLACK_BOT_TOKEN) && trimToValue(next.SLACK_BOT_TOKEN)) {
    next.JOOPO_MANTIS_SLACK_BOT_TOKEN = next.SLACK_BOT_TOKEN;
  }
  if (
    !trimToValue(next.JOOPO_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(next.JOOPO_QA_SLACK_SUT_BOT_TOKEN)
  ) {
    next.JOOPO_MANTIS_SLACK_BOT_TOKEN = next.JOOPO_QA_SLACK_SUT_BOT_TOKEN;
  }
  if (!trimToValue(next.JOOPO_MANTIS_SLACK_APP_TOKEN) && trimToValue(next.SLACK_APP_TOKEN)) {
    next.JOOPO_MANTIS_SLACK_APP_TOKEN = next.SLACK_APP_TOKEN;
  }
  if (
    !trimToValue(next.JOOPO_MANTIS_SLACK_APP_TOKEN) &&
    trimToValue(next.JOOPO_QA_SLACK_SUT_APP_TOKEN)
  ) {
    next.JOOPO_MANTIS_SLACK_APP_TOKEN = next.JOOPO_QA_SLACK_SUT_APP_TOKEN;
  }
  if (
    !trimToValue(next.JOOPO_MANTIS_SLACK_CHANNEL_ID) &&
    trimToValue(next.JOOPO_QA_SLACK_CHANNEL_ID)
  ) {
    next.JOOPO_MANTIS_SLACK_CHANNEL_ID = next.JOOPO_QA_SLACK_CHANNEL_ID;
  }
  return next;
}

function resolveSlackGatewayEnvPayload(env: NodeJS.ProcessEnv): SlackGatewayCredentialPayload {
  const channelId = trimToValue(env.JOOPO_QA_SLACK_CHANNEL_ID);
  const sutBotToken = trimToValue(env.JOOPO_QA_SLACK_SUT_BOT_TOKEN);
  const sutAppToken = trimToValue(env.JOOPO_QA_SLACK_SUT_APP_TOKEN);
  if (!channelId || !sutBotToken || !sutAppToken) {
    throw new Error(
      "Gateway setup requires JOOPO_QA_SLACK_CHANNEL_ID, JOOPO_QA_SLACK_SUT_BOT_TOKEN, and JOOPO_QA_SLACK_SUT_APP_TOKEN when using --credential-source env.",
    );
  }
  return {
    channelId,
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.JOOPO_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(params.env.JOOPO_MANTIS_SLACK_APP_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.JOOPO_MANTIS_SLACK_BOT_TOKEN = payload.sutBotToken;
  params.env.JOOPO_MANTIS_SLACK_APP_TOKEN = payload.sutAppToken;
  params.env.JOOPO_MANTIS_SLACK_CHANNEL_ID =
    trimToValue(params.env.JOOPO_MANTIS_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.JOOPO_QA_SLACK_CHANNEL_ID =
    trimToValue(params.env.JOOPO_QA_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.JOOPO_QA_SLACK_SUT_BOT_TOKEN =
    trimToValue(params.env.JOOPO_QA_SLACK_SUT_BOT_TOKEN) ?? payload.sutBotToken;
  params.env.JOOPO_QA_SLACK_SUT_APP_TOKEN =
    trimToValue(params.env.JOOPO_QA_SLACK_SUT_APP_TOKEN) ?? payload.sutAppToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
if [ -n "\${JOOPO_LIVE_OPENAI_KEY:-}" ] && [ -z "\${OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$JOOPO_LIVE_OPENAI_KEY"
fi
...
team_id="\${JOOPO_QA_SLACK_TEAM_ID:-}"
auth_test_token="\${JOOPO_QA_SLACK_SUT_BOT_TOKEN:-\${JOOPO_MANTIS_SLACK_BOT_TOKEN:-}}"
...
const token = process.env.JOOPO_QA_SLACK_SUT_BOT_TOKEN || process.env.JOOPO_MANTIS_SLACK_BOT_TOKEN;
...
if [ -z "$slack_url" ] && [ -n "$team_id" ] && [ -n "\${JOOPO_QA_SLACK_CHANNEL_ID:-}" ]; then
  slack_url="https://app.slack.com/client/$team_id/$JOOPO_QA_SLACK_CHANNEL_ID"
fi
profile="\${JOOPO_MANTIS_SLACK_BROWSER_PROFILE_DIR:-$HOME/.config/joopo-mantis/slack-chrome-profile}"
mkdir -p "$profile"
if [ "$setup_gateway" = "1" ]; then
  export SLACK_BOT_TOKEN="\${JOOPO_MANTIS_SLACK_BOT_TOKEN:-\${SLACK_BOT_TOKEN:-}}"
  export SLACK_APP_TOKEN="\${JOOPO_MANTIS_SLACK_APP_TOKEN:-\${SLACK_APP_TOKEN:-}}"
  if [ -z "$SLACK_BOT_TOKEN" ] || [ -z "$SLACK_APP_TOKEN" ]; then
    echo "Gateway setup requires JOOPO_MANTIS_SLACK_BOT_TOKEN and JOOPO_MANTIS_SLACK_APP_TOKEN." >&2
    exit 2
  fi
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export JOOPO_HOME="$HOME/.joopo-mantis/slack-joopo"
    mkdir -p "$JOOPO_HOME"
...
MANTIS_SLACK_PATCH
    pnpm joopo config patch --file "$out/slack.socket.patch.json5" --dry-run
    pnpm joopo config patch --file "$out/slack.socket.patch.json5"
    nohup pnpm joopo gateway run --dev --allow-unconfigured --port 38973 --cli-backend-logs </dev/null >"$out/joopo-gateway.log" 2>&1 &
    gateway_pid="$!"
    echo "$gateway_pid" >"$out/joopo-gateway.pid"
    sleep 12
    if ! kill -0 "$gateway_pid" >/dev/null 2>&1; then
      echo "Joopo gateway exited during startup." >&2
      wait "$gateway_pid" || true
      exit 1
    fi
    disown "$gateway_pid" >/dev/null 2>&1 || true
  else
    qa_args=(joopo qa slack --repo-root . --output-dir "$out/slack-qa" --provider-mode "$provider_mode" --model "$primary_model" --alt-model "$alternate_model" --credential-source "$credential_source" --credential-role "$credential_role")
    if [ "$fast_mode" = "1" ]; then
      qa_args+=(--fast)
    fi
...
cat >"$out/remote-metadata.json" <<MANTIS_REMOTE_METADATA
{
  ...
  "gatewayAlive": $(if [ "$setup_gateway" = "1" ] && [ -f "$out/joopo-gateway.pid" ] && kill -0 "$(cat "$out/joopo-gateway.pid")" >/dev/null 2>&1; then echo true; else echo false; fi),
  "gatewayPid": "$(if [ -f "$out/joopo-gateway.pid" ]; then cat "$out/joopo-gateway.pid"; fi)",
  "gatewayPort": 38973,
...
export async function runMantisSlackDesktopSmoke(
  ...
  const slackChannelId =
    ...
    trimToValue(env.JOOPO_QA_SLACK_CHANNEL_ID) ??
  ...
  const remoteOutputDir = `/tmp/joopo-mantis-slack-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Slack desktop gateway setup did not report a live Joopo gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
