...
const DEFAULT_BROWSER_URL = "https://joopo.ai";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "JOOPO_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "JOOPO_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/joopo-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.JOOPO_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/joopo-mantis-desktop-${startedAt
    .toISOString()
