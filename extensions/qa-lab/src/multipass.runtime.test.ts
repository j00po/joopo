...
describe("qa multipass runtime", () => {
  ...
  it("rejects repo-local symlink output directories that escape the repo root", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-multipass-"));
  ...
  it("reuses suite scenario semantics and resolves mounted artifact paths", () => {
    ...
    expect(plan.guestOutputDir).toBe("/workspace/joopo-host/.artifacts/qa-e2e/multipass-test");
  ...
  it("renders a guest script that runs the live qa suite by default", () => {
    ...
    expect(script).toContain("'pnpm' 'joopo' 'qa' 'suite' '--transport' 'qa-channel'");
    ...
    expect(script).toContain("/workspace/joopo-host/.artifacts/qa-e2e/multipass-test");
  });
  ...
  it("forwards live key list and numbered key env shapes", () => {
    vi.stubEnv("JOOPO_LIVE_ANTHROPIC_KEYS", "anthropic-a anthropic-b");
    ...
    expect(plan.forwardedEnv.JOOPO_LIVE_ANTHROPIC_KEYS).toBe("anthropic-a anthropic-b");
  ...
  it("skips stale CODEX_HOME values that do not exist on the host", () => {
    vi.stubEnv("CODEX_HOME", "/tmp/does-not-exist-joopo-codex-home");
    const plan = createQaMultipassPlan({
      repoRoot: process.cwd(),
      outputDir: path.join(process.cwd(), ".artifacts", "qa-e2e", "multipass-live-test"),
      providerMode: "live-frontier",
  ...
  it("falls back to os.homedir() when HOME is unset for CODEX_HOME discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-multipass-home-"));
    ...
    try {
      ...
      expect(plan.guestCodexHomePath).toBe("/workspace/joopo-codex-home");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("does not leave a temp guest transfer script behind when multipass is missing", async () => {
    ...
    const tempEntries = fs
      .readdirSync(resolvePreferredJoopoTmpDir())
