...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    JOOPO_HOME: params.homeDir,
    JOOPO_CONFIG_PATH: params.configPath,
    JOOPO_STATE_DIR: params.stateDir,
    JOOPO_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    JOOPO_SKIP_STARTUP_MODEL_PREWARM: "1",
    JOOPO_NO_RESPAWN: "1",
    JOOPO_TEST_FAST: "1",
    JOOPO_QA_PARENT_PID: String(process.pid),
    JOOPO_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    JOOPO_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { JOOPO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { JOOPO_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: JoopoConfig) => JoopoConfig;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredJoopoTmpDir(), "joopo-qa-suite-"),
  );
  ...
  const keepTemp = process.env.JOOPO_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
