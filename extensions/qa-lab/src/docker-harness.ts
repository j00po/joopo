...
const QA_LAB_UI_OVERLAY_DIR = "/opt/joopo-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        JOOPO_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      JOOPO_CONFIG_PATH: /tmp/joopo/joopo.json
      ...
      JOOPO_PROFILE: ""
    volumes:
      - ./state:/opt/joopo-scaffold:ro
      - ${repoMount}:/opt/joopo-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/joopo/workspace /tmp/joopo/state && cp /opt/joopo-scaffold/joopo.json /tmp/joopo/joopo.json && cp -R /opt/joopo-scaffold/seed-workspace/. /tmp/joopo/workspace/ && ln -snf /opt/joopo-repo /tmp/joopo/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t joopo:qa-local-prebaked --build-arg JOOPO_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "joopo:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "joopo.json"),
  ];
