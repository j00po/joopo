...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "joopo.json"),
    ...
    expect(compose).toContain('      - "http://joopo-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/joopo-qa-lab-ui"');
    expect(compose).toContain(":/opt/joopo-repo:ro");
    expect(compose).toContain("./state:/opt/joopo-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/joopo-scaffold/seed-workspace/. /tmp/joopo/workspace/",
    );
    expect(compose).toContain("JOOPO_CONFIG_PATH: /tmp/joopo/joopo.json");
    expect(compose).toContain("JOOPO_STATE_DIR: /tmp/joopo/state");
    expect(compose).toContain('JOOPO_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("JOOPO_GATEWAY_TOKEN=qa-token");
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/joopo",
        imageName: "joopo:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("joopo:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t joopo:qa-local-prebaked --build-arg JOOPO_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/joopo",
      ),
    ]);
  });
});
