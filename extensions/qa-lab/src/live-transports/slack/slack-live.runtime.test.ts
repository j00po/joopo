...
describe("Slack live QA runtime helpers", () => {
  it("resolves env credential payloads", () => {
    expect(
      __testing.resolveSlackQaRuntimeEnv({
        JOOPO_QA_SLACK_CHANNEL_ID: "C123456789",
        JOOPO_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        JOOPO_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        JOOPO_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      __testing.resolveSlackQaRuntimeEnv({
        JOOPO_QA_SLACK_CHANNEL_ID: "qa-channel",
        JOOPO_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        JOOPO_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        JOOPO_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("JOOPO_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "joopo-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing JOOPO_QA_CONVEX_SITE_URL");
