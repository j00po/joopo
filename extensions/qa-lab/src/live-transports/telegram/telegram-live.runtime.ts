...
import { fetchWithSsrFGuard } from "joopo/plugin-sdk/ssrf-runtime";
...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "JOOPO_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "JOOPO_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "JOOPO_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "JOOPO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "JOOPO_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("JOOPO_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "JOOPO_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "JOOPO_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledJoopoTelegramOnboardingPreflight(params: {
  joopoCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredJoopoTmpDir(), "joopo-npm-telegram-"),
  );
  const homeDir = path.join(tempRoot, "home");
  const stateDir = path.join(homeDir, ".joopo");
  ...
  const env = {
    ...
    JOOPO_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-joopo-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
      params.joopoCommand,
    ...
    await execFileAsync(params.joopoCommand, ["doctor", "--non-interactive"], { env });
  } finally {
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutJoopoCommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledJoopoTelegramOnboardingPreflight({
        joopoCommand: params.sutJoopoCommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutJoopoCommand
        ? {
            executablePath: params.sutJoopoCommand,
