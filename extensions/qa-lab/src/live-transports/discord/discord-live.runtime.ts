...
import {
  ...
  requestDiscord,
} from "@joopo/discord/api.js";
import { DEFAULT_EMOJIS } from "joopo/plugin-sdk/channel-feedback";
import type { JoopoConfig } from "joopo/plugin-sdk/config-types";
import { formatErrorMessage } from "joopo/plugin-sdk/error-runtime";
import { writeExternalFileWithinRoot } from "joopo/plugin-sdk/security-runtime";
...
const DISCORD_QA_CAPTURE_CONTENT_ENV = "JOOPO_QA_DISCORD_CAPTURE_CONTENT";
const DISCORD_QA_CAPTURE_UI_METADATA_ENV = "JOOPO_QA_DISCORD_CAPTURE_UI_METADATA";
const DISCORD_QA_KEEP_THREADS_ENV = "JOOPO_QA_DISCORD_KEEP_THREADS";
const QA_REDACT_PUBLIC_METADATA_ENV = "JOOPO_QA_REDACT_PUBLIC_METADATA";
const DISCORD_QA_ENV_KEYS = [
  ...
  "JOOPO_QA_DISCORD_SUT_APPLICATION_ID",
] as const;
...
function resolveDiscordQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): DiscordQaRuntimeEnv {
  const voiceChannelId = env.JOOPO_QA_DISCORD_VOICE_CHANNEL_ID?.trim();
  const runtimeEnv = {
    guildId: resolveEnvValue(env, "JOOPO_QA_DISCORD_GUILD_ID"),
    channelId: resolveEnvValue(env, "JOOPO_QA_DISCORD_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "JOOPO_QA_DISCORD_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "JOOPO_QA_DISCORD_SUT_BOT_TOKEN"),
    sutApplicationId: resolveEnvValue(env, "JOOPO_QA_DISCORD_SUT_APPLICATION_ID"),
    ...(voiceChannelId ? { voiceChannelId } : {}),
  };
  validateDiscordQaRuntimeEnv(runtimeEnv, "JOOPO_QA_DISCORD");
  return runtimeEnv;
}
...
async function resolveDiscordQaVoiceChannel(params: {
  ...
  if (!first) {
    throw new Error(
      "Discord voice auto-join scenario could not find a visible voice/stage channel for the SUT bot. Add voiceChannelId to the Convex discord credential payload or set JOOPO_QA_DISCORD_VOICE_CHANNEL_ID.",
    );
  }
  return first;
}
...
<body>
  <main>
    ...
    <section class="message">
      <div class="author">Joopo Discord SUT</div>
...
export async function runDiscordQaLive(params: {
  ...
  const publishedCleanupIssues = redactPublicMetadata
    ? cleanupIssues.map(() => "details redacted (JOOPO_QA_REDACT_PUBLIC_METADATA=1)")
    : cleanupIssues;
