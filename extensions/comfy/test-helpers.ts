...
export function buildComfyConfig(config: Record<string, unknown>): JoopoConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): JoopoConfig {
  return {
    models: {
      providers: {
        comfy: config,
