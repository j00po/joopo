import { createChannelPairingController } from "joopo/plugin-sdk/channel-pairing";
import {
...
import type { RuntimeEnv } from "joopo/plugin-sdk/runtime";
import {
...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as JoopoConfig);
  ...
  if (message.isGroup && commandGate.shouldBlock) {
    const { logInboundDrop } = await import("joopo/plugin-sdk/channel-inbound");
    logInboundDrop({
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as JoopoConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as JoopoConfig);
  ...
  const { dispatchChannelMessageReplyWithBase } =
    await import("joopo/plugin-sdk/channel-message");
