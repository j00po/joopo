...
import { createSendCfgThreadingRuntime } from "joopo/plugin-sdk/channel-test-helpers";
...
vi.mock("joopo/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("joopo/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("joopo/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("joopo/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("joopo/plugin-sdk/text-runtime");
  vi.resetModules();
});
