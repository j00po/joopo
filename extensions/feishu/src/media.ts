...
import type { MessageReceipt } from "joopo/plugin-sdk/channel-message";
import { mediaKindFromMime } from "joopo/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "joopo/plugin-sdk/media-runtime";
import { readRegularFile, writeExternalFileWithinRoot } from "joopo/plugin-sdk/security-runtime";
import {
...
async function downloadMessageResourceWithType(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "joopo-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredJoopoTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
