...
import { getSessionBindingService } from "joopo/plugin-sdk/conversation-runtime";
import {
...
} from "joopo/plugin-sdk/interactive-runtime";
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("joopo/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("joopo/plugin-sdk/config-mutation");
