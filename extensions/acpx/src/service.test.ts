...
const { cleanupJoopoOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupJoopoOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleJoopoOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleJoopoOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupJoopoOwnedAcpxProcessTree: cleanupJoopoOwnedAcpxProcessTreeMock,
  reapStaleJoopoOwnedAcpxOrphans: reapStaleJoopoOwnedAcpxOrphansMock,
}));
...
const previousEnv = {
  JOOPO_ACPX_RUNTIME_STARTUP_PROBE: process.env.JOOPO_ACPX_RUNTIME_STARTUP_PROBE,
  JOOPO_SKIP_ACPX_RUNTIME: process.env.JOOPO_SKIP_ACPX_RUNTIME,
  JOOPO_SKIP_ACPX_RUNTIME_PROBE: process.env.JOOPO_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupJoopoOwnedAcpxProcessTreeMock.mockClear();
  reapStaleJoopoOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("JOOPO_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("JOOPO_SKIP_ACPX_RUNTIME");
  restoreEnv("JOOPO_SKIP_ACPX_RUNTIME_PROBE");
...
function createServiceContext(workspaceDir: string) {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".joopo-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe("createAcpxRuntimeService", () => {
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupJoopoOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupJoopoOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Joopo-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    reapStaleJoopoOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupJoopoOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleJoopoOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Joopo-owned ACPX processes");
  ...
  it("can run the embedded runtime probe at startup when explicitly enabled", async () => {
    process.env.JOOPO_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime probe via env", async () => {
    process.env.JOOPO_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.JOOPO_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.JOOPO_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (JOOPO_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
