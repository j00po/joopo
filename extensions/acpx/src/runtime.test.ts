...
const DOCUMENTED_JOOPO_BRIDGE_COMMAND =
  "env JOOPO_HIDE_BANNER=1 JOOPO_SUPPRESS_NOTES=1 joopo acp --url ws://127.0.0.1:18789 --token-file ~/.joopo/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/joopo/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${JOOPO_ACPX_LEASE_ID_ARG} lease-close ${JOOPO_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "joopo" ? "joopo acp" : agentName),
        list: () => ["codex", "joopo"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes Joopo Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(__testing.isCodexAcpCommand("joopo acp")).toBe(false);
  });
  ...
  it("cleans up Joopo-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/joopo/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        joopoWrapperRoot: "/tmp/joopo/acpx",
      },
      {
        joopoProcessCleanup: {
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/joopo/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    const { runtime, delegate, wrappedStore } = makeRuntime(baseStore, {
      joopoGatewayInstanceId: "gateway-test",
      joopoProcessLeaseStore: leaseStore.store,
      joopoWrapperRoot: "/tmp/joopo/acpx",
      agentRegistry: {
    ...
    expect(launchCommands[0]).toContain("JOOPO_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("JOOPO_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]).toMatchObject({
      joopoGatewayInstanceId: "gateway-test",
      joopoLeaseId: leases[0]?.leaseId,
    });
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${JOOPO_ACPX_LEASE_ID_ARG} lease-old ${JOOPO_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${JOOPO_ACPX_LEASE_ID_ARG} lease-current ${JOOPO_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("routes joopo ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:joopo:acp:test",
      backend: "acpx",
  ...
  it("routes non-joopo sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for joopo sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:joopo:acp:test",
      backend: "acpx",
      runtimeSessionName: "joopo-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the joopo agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "joopo" ? "codex" : agentName),
        list: () => ["codex", "joopo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the joopo bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "joopo acp" : agentName),
        list: () => ["codex", "joopo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped joopo bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "joopo" ? DOCUMENTED_JOOPO_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "joopo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node joopo entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "joopo" ? "env JOOPO_HIDE_BANNER=1 node joopo.mjs acp" : agentName,
        list: () => ["codex", "joopo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:joopo:acp:test",
        agentCommand: DOCUMENTED_JOOPO_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:joopo:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:joopo:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to joopo bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
