...
import { formatPluginConfigIssue } from "joopo/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "joopo-plugin-tools";
const ACPX_JOOPO_TOOLS_MCP_SERVER_NAME = "joopo-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "joopo.plugin.json")) &&
...
function resolveAcpxPluginRootFromJoopoLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Joopo layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromJoopoLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveJoopoRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const joopoRoot = resolveJoopoRoot(pluginRoot);
  const distEntry = path.join(joopoRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(joopoRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveJoopoToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const joopoRoot = resolveJoopoRoot(pluginRoot);
  const distEntry = path.join(joopoRoot, "dist", "mcp", "joopo-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(joopoRoot, "src", "mcp", "joopo-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.joopoToolsMcpBridge && resolved[ACPX_JOOPO_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_JOOPO_TOOLS_MCP_SERVER_NAME} is reserved when joopoToolsMcpBridge=true`,
    );
  }
  ...
  if (params.joopoToolsMcpBridge) {
    resolved[ACPX_JOOPO_TOOLS_MCP_SERVER_NAME] = resolveJoopoToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
export function resolveAcpxPluginConfig(params: {
  ...
  const joopoToolsMcpBridge = normalized.joopoToolsMcpBridge === true;
