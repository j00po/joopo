...
import { readJsonFileWithFallback } from "joopo/plugin-sdk/json-store";
import {
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--joopo-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @joopo/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Joopo's isolated CODEX_HOME wrapper, but launch the plugin-local
...
${params.envSetup}
const joopoWrapperArgs = new Set([
  ${quoteCommandPart(JOOPO_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(JOOPO_GATEWAY_INSTANCE_ID_ARG)},
]);

function stripJoopoWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (joopoWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripJoopoWrapperArgs(process.argv.slice(2));
...
if (!command) {
  console.error("[joopo] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[joopo] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Joopo; fallback must not float to an unpatched newer release.
