...
const ENABLE_STARTUP_PROBE_ENV = "JOOPO_ACPX_RUNTIME_STARTUP_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        joopoGatewayInstanceId: params.gatewayInstanceId,
        joopoProcessLeaseStore: params.processLeaseStore,
        joopoWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: JoopoPluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleJoopoOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: JoopoPluginServiceContext): Promise<void> {
      if (process.env.JOOPO_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (JOOPO_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale Joopo-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
      ...
      if (!shouldRunStartupProbe() || process.env.JOOPO_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: JoopoPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
