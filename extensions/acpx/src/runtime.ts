...
type JoopoAcpxRuntimeOptions = AcpRuntimeOptions & {
  joopoWrapperRoot?: string;
  joopoGatewayInstanceId?: string;
  joopoProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  joopoProcessCleanup?: AcpxProcessCleanupDeps;
};
...
function readJoopoLeaseIdFromRecord(record: AcpLoadedSessionRecord): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { joopoLeaseId } = record as { joopoLeaseId?: unknown };
  return typeof joopoLeaseId === "string" ? joopoLeaseId.trim() || undefined : undefined;
}
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return {
        ...
        joopoLeaseId: lease.leaseId,
        joopoGatewayInstanceId: lease.gatewayInstanceId,
    ...
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = {
          ...
          joopoLeaseId: launch.leaseId,
          joopoGatewayInstanceId: launch.gatewayInstanceId,
        } as AcpSessionRecord;
      }
...
const JOOPO_BRIDGE_EXECUTABLE = "joopo";
const JOOPO_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_JOOPO_PREFIX = "openai-codex/";
...
function isJoopoBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === JOOPO_BRIDGE_EXECUTABLE) {
    return parts[1] === JOOPO_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^joopo(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === JOOPO_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_JOOPO_PREFIX)) {
    value = value.slice(CODEX_ACP_JOOPO_PREFIX.length);
  }
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isJoopoBridgeCommand(command);
}
...
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: JoopoAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { joopoProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = joopoProcessCleanup;
    this.wrapperRoot = options.joopoWrapperRoot;
    this.gatewayInstanceId = options.joopoGatewayInstanceId;
    this.processLeaseStore = options.joopoProcessLeaseStore;
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readJoopoLeaseIdFromRecord(record);
    ...
    await cleanupJoopoOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
      wrapperRoot: this.wrapperRoot,
      deps: this.processCleanupDeps,
