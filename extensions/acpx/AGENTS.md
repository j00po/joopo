...
The ACPX extension is a thin Joopo wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `joopo/acpx`, not in this extension.
...
Use this flow when Joopo needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `joopo/acpx` repo first.
2. In Joopo, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Joopo and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Joopo ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Joopo-specific glue, move it to `joopo/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
