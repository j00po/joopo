...
import { root as fsRoot } from "joopo/plugin-sdk/security-runtime";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- joopo:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- joopo:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- joopo:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- joopo:wiki:${group.dir}:index:end -->`,
