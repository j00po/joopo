...
vi.mock("joopo/plugin-sdk/memory-host-search", () => ({
  getActiveMemorySearchManager: getActiveMemorySearchManagerMock,
}));

vi.mock("joopo/plugin-sdk/memory-host-core", () => ({
  resolveDefaultAgentId: resolveDefaultAgentIdMock,
  resolveSessionAgentId: resolveSessionAgentIdMock,
}));

vi.mock("joopo/plugin-sdk/session-transcript-hit", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("joopo/plugin-sdk/session-transcript-hit")>();
  return {
    ...actual,
...
function createAppConfig(): JoopoConfig {
  return {
    agents: {
...
function createSessionVisibilityAppConfig(): JoopoConfig {
  return {
    agents: {
  ...
  loadCombinedSessionStoreForGatewayMock.mockReturnValue({
    ...
    store: {
      "agent:main:child-session": {
        ...
        updatedAt: 1,
        sessionFile: "/tmp/joopo/child-session.jsonl",
      },
      "agent:main:sibling-session": {
        sessionId: "sibling-session",
        updatedAt: 2,
        sessionFile: "/tmp/joopo/sibling-session.jsonl",
      },
    },
  });
}
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Related",
          "<!-- joopo:wiki:related:start -->",
          "### Related Pages",
          "- [Needle Person](entities/needle-person.md)",
          "<!-- joopo:wiki:related:end -->",
          "",
        ].join("\n"),
      }),
      "utf8",
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Agent Card",
          "- Maintainer lane: CEO; Microsoft-facing Joopo maintainer",
          "",
          "## AI Notes",
