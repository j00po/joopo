...
- Start from `joopo wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `joopo wiki obsidian status` before shelling out, then prefer the dedicated helpers like `joopo wiki obsidian search`, `joopo wiki obsidian open`, `joopo wiki obsidian command`, and `joopo wiki obsidian daily`.
