...
import { validateJsonSchemaValue, type JsonSchemaObject } from "joopo/plugin-sdk/config-schema";
import type { RealtimeTranscriptionProviderPlugin } from "joopo/plugin-sdk/realtime-transcription";
import type { RealtimeVoiceProviderPlugin } from "joopo/plugin-sdk/realtime-voice";
...
vi.mock("joopo/plugin-sdk/ssrf-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("joopo/plugin-sdk/ssrf-runtime")>();
  return {
    ...actual,
...
describe("google-meet plugin", () => {
  ...
  afterAll(() => {
    vi.doUnmock("joopo/plugin-sdk/ssrf-runtime");
  ...
  it("defaults to chrome agent mode with safe read-only tools", () => {
    ...
    expect(resolveGoogleMeetConfig({}).realtime.instructions).toContain("joopo_agent_consult");
  });
  ...
  it("declares advanced config metadata in the plugin entry and manifest", () => {
    const manifest = JSON.parse(
      readFileSync(new URL("./joopo.plugin.json", import.meta.url), "utf8"),
    ) as {
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    expect(
      resolveGoogleMeetConfigWithEnv(
        {},
        {
          JOOPO_GOOGLE_MEET_CLIENT_ID: "client-id",
          GOOGLE_MEET_CLIENT_SECRET: "client-secret",
          JOOPO_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
          GOOGLE_MEET_ACCESS_TOKEN: "access-token",
          JOOPO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
          GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
          JOOPO_GOOGLE_MEET_PREVIEW_ACK: "true",
        },
      ),
    ).toMatchObject({
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["joopo-audio-bridge", "capture"],
          audioOutputCommand: ["joopo-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "joopo-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "joopo-google-meet-tool-dry-run-"));
          ...
          if (request.path === "/act") {
            return {
              result: JSON.stringify({
                ...
                manualActionMessage: "Admit the Joopo browser participant in Google Meet.",
                title: "Meet",
                url: "https://meet.google.com/abc-defg-hij",
              }),
            };
  ...
  it("retries caption enable until the captions button is available", async () => {
    ...
    const captionsStateKey = "__joopoMeetCaptions";
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualActionMessage:
            "Sign in to Google in the Joopo browser profile, then retry the Meet join.",
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("joopo node run");
  });
