...
describe("google-meet CLI", () => {
  ...
  it("prints markdown artifact and attendance output", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "joopo-google-meet-artifacts-"));
  ...
  it("writes an export bundle", async () => {
    stubMeetArtifactsApi();
    const stdout = captureStdout();
    const tempDir = mkdtempSync(path.join(tmpdir(), "joopo-google-meet-export-"));
  ...
  it("includes artifact warnings in export summaries and manifests", async () => {
    ...
    const stdout = captureStdout();
    const tempDir = mkdtempSync(path.join(tmpdir(), "joopo-google-meet-export-warning-"));
  ...
  it("prints a dry-run export manifest without writing files", async () => {
    stubMeetArtifactsApi();
    const stdout = captureStdout();
    const parentDir = mkdtempSync(path.join(tmpdir(), "joopo-google-meet-export-dry-run-"));
  ...
  it("prints human-readable session doctor output", async () => {
    ...
    try {
            ...
            session: {
              ...
              chrome: {
                ...
                health: {
                  ...
                  lastCaptionText: "Can everyone hear Joopo?",
      ...
      expect(stdout.output()).toContain("last caption text: Alice: Can everyone hear Joopo?");
      ...
      await setupCli({
        ...
        runtime: {
          recoverCurrentTab: async () => ({
            ...
            message: "Admit the Joopo browser participant in Google Meet.",
          }),
        },
