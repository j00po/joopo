...
import { callGatewayFromCli } from "joopo/plugin-sdk/gateway-runtime";
import type { PluginRuntime } from "joopo/plugin-sdk/plugin-runtime";
import type { RuntimeLogger } from "joopo/plugin-sdk/plugin-runtime";
  ...
  if (result.code !== 0 || !outputMentionsBlackHole2ch(output)) {
    ...
    throw new Error(
      [
        ...
        "Install BlackHole 2ch and route Chrome input/output through the Joopo audio bridge.",
        hint,
      ]
        .filter(Boolean)
        .join(" "),
  ...
  > => {
    ...
    if (params.config.chrome.audioBridgeCommand) {
      if (params.mode === "agent") {
        throw new Error(
          "Chrome agent mode requires chrome.audioInputCommand and chrome.audioOutputCommand so Joopo can run STT and regular TTS directly.",
        );
      }
...
function meetStatusScript(params: {
  ...
  const captionState = (() => {
    ...
    const w = window;
    if (!inCall && !w.__joopoMeetCaptions) return undefined;
    if (!w.__joopoMeetCaptions) {
      w.__joopoMeetCaptions = {
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    ...
    manualActionMessage = "Sign in to Google in the Joopo browser profile, then retry the Meet join.";
    ...
    manualActionMessage = "Admit the Joopo browser participant in Google Meet, then retry speech.";
  } else if (permissionNeeded) {
    manualActionReason = "meet-permission-required";
    manualActionMessage = allowMicrophone
      ? "Allow microphone/camera/speaker permissions for Meet in the Joopo browser profile, then retry."
      : "Join without microphone/camera permissions in the Joopo browser profile, then retry.";
    ...
    manualActionMessage = allowMicrophone
      ? "Meet is showing the microphone choice. Click Use microphone in the Joopo browser profile, then retry."
      : "Meet is showing the microphone choice. Choose the no-microphone option in the Joopo browser profile, then retry.";
  }
  ...
  do {
    try {
      ...
      browser = {
        ...
        manualActionMessage:
          "Open the Joopo browser profile, finish Google Meet login, admission, or permission prompts, then retry.",
        notes: [
          ...permissionNotes,
