...
import type { PluginRuntime, RuntimeLogger } from "joopo/plugin-sdk/plugin-runtime";
  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking Joopo to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the Joopo Google Meet microphone before asking Joopo to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
