    ...
    await withStartedMatrixHarness(
      ...
      async ({ outputDir, result }) => {
        expect(calls).toEqual([
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml down --remove-orphans @/repo/joopo`,
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml up -d @/repo/joopo`,
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps --format json matrix-qa-homeserver @/repo/joopo`,
        ]);
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml restart matrix-qa-homeserver @/repo/joopo`,
        );
      },
    );
  });
  ...
  it("falls back to the container IP when the host port is unreachable", async () => {
    ...
    await withStartedMatrixHarness(
      ...
      ({ outputDir, result }) => {
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps -q matrix-qa-homeserver @/repo/joopo`,
        );
        expect(calls).toContain(
          "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} container-123 @/repo/joopo",
        );
      },
    );
  });
