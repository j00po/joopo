...
const {
  ...
  resolveMatrixQaJoopoCliEntryPath,
  runMatrixQaJoopoCli,
  startMatrixQaJoopoCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `joopo ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaJoopoCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaJoopoCli: vi.fn(),
  startMatrixQaJoopoCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaJoopoCli.mockReset();
    startMatrixQaJoopoCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Joopo Matrix QA run",
        scenarios: [
    ...
    try {
      ...
      await expect(
        runMatrixQaScenario(scenario, {
          ...
          gatewayRuntimeEnv: {
            JOOPO_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaJoopoCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaJoopoCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.JOOPO_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.JOOPO_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      await expect(
        runMatrixQaScenario(scenario, {
          ...
          driverPassword: "driver-password",
          gatewayRuntimeEnv: {
            JOOPO_CONFIG_PATH: "/tmp/gateway-config.json",
            JOOPO_STATE_DIR: "/tmp/gateway-state",
            PATH: process.env.PATH,
          },
          outputDir,
        }),
      ...
      expect(startMatrixQaJoopoCli).toHaveBeenCalledTimes(1);
      expect(startMatrixQaJoopoCli.mock.calls[0]?.[0].args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(startMatrixQaJoopoCli.mock.calls[0]?.[0].timeoutMs).toBe(16_000);
      ...
      expect(runMatrixQaJoopoCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaJoopoCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(runMatrixQaJoopoCli.mock.calls[0]?.[0].stdin).toBe("encoded-recovery-key\n");
      const cliEnv = startMatrixQaJoopoCli.mock.calls[0]?.[0].env;
      expect(cliEnv?.JOOPO_STATE_DIR).toContain("joopo-matrix-cli-qa-");
      expect(cliEnv?.JOOPO_CONFIG_PATH).toContain("joopo-matrix-cli-qa-");
      const configPath = String(cliEnv?.JOOPO_CONFIG_PATH);
      ...
      await expect(readdir(String(cliEnv?.JOOPO_STATE_DIR))).rejects.toThrow();
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaJoopoCli.mockImplementation(async ({ args, env }) => {
        if (env.JOOPO_CONFIG_PATH) {
      ...
      expect(runMatrixQaJoopoCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "Joopo Matrix QA CLI Account Add E2EE",
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Account Add Owner",
          registrationToken: "registration-token",
        }),
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaJoopoCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.JOOPO_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.JOOPO_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Encryption Setup Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Encryption Idempotent Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("joopo matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaJoopoCli.mockReturnValue({
      ...
      expect(startMatrixQaJoopoCli.mock.calls[0]?.[0].env.JOOPO_CONFIG_PATH).toContain(
        "joopo-matrix-e2ee-setup-qa-",
      );
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Encryption Failure Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Recovery Key Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Invalid Recovery Key Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaJoopoCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.JOOPO_CONFIG_PATH);
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Multi Account Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix CLI setup then gateway encrypted reply through the QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Setup Gateway",
          registrationToken: "registration-token",
        }),
      );
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Joopo Matrix QA CLI Setup Driver",
          registrationToken: "registration-token",
        }),
      );
      expect(createPrivateRoom).toHaveBeenCalledWith({
