import { spawn as startJoopoCliProcess } from "node:child_process";
...
import { redactSensitiveText } from "joopo/plugin-sdk/logging-core";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `joopo ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaJoopoCliEntryPath(cwd: string): string {
...
export function startMatrixQaJoopoCli(params: {
  ...
  const distEntryPath = resolveMatrixQaJoopoCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startJoopoCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
    env: params.env,
...
export async function runMatrixQaJoopoCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaJoopoCli(params).wait();
}
...
export async function createMatrixQaJoopoCliRuntime(params: {
  accountId: string;
  accessToken: string;
