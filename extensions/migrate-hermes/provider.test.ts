...
import { createCapturedPluginRegistration } from "joopo/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when JOOPO_HOME is set", () => {
    const previous = process.env.JOOPO_HOME;
    process.env.JOOPO_HOME = path.join(path.sep, "tmp", "joopo-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.JOOPO_HOME = previous;
      }
    }
  });
