...
function getChannelConfig(cfg: JoopoConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: JoopoConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: JoopoConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: JoopoConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Joopo uses to send replies back to Chat.",
      ],
