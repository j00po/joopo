/**
 * Synology Chat Channel Plugin for Joopo.
 *
...
import { waitUntilAbort } from "joopo/plugin-sdk/channel-lifecycle";
import {
...
import { createEmptyChannelDirectoryAdapter } from "joopo/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "joopo pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: JoopoConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: JoopoConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Joopo",
        order: 90,
      },
      capabilities: {
