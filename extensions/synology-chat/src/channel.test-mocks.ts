...
vi.mock("joopo/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("joopo/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("joopo/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("joopo/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/joopo/synology-chat-sessions.json"),
