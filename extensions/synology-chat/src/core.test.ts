...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: JoopoConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.JOOPO_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("Joopo");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.JOOPO_BOT_NAME = "TestBot";
