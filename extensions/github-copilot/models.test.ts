import { createProviderUsageFetch, makeResponse } from "joopo/plugin-sdk/test-env";
...
vi.mock("joopo/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("joopo/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/joopo-state",
}));

import type { ProviderResolveDynamicModelContext } from "joopo/plugin-sdk/core";
import { resolveCopilotForwardCompatModel } from "./models.js";

afterAll(() => {
  vi.doUnmock("@mariozechner/pi-ai/oauth");
  vi.doUnmock("joopo/plugin-sdk/provider-model-shared");
  vi.doUnmock("joopo/plugin-sdk/json-store");
  vi.doUnmock("joopo/plugin-sdk/state-paths");
  vi.resetModules();
});
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/joopo-state/credentials/github-copilot.token.json";
