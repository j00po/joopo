...
import { normalizeOptionalAccountId } from "joopo/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "joopo/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: JoopoConfig, requestedAccountId?: string): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: JOOPO_TWITCH_ACCESS_TOKEN",
...
export async function configureWithEnvToken(
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var JOOPO_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.JOOPO_TWITCH_ACCESS_TOKEN?.trim();
