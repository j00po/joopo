...
- Extension production code should import from `joopo/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `joopo.plugin.json` and the package
  `joopo` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `joopo/plugin-sdk/*`.
