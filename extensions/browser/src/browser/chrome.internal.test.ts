...
import {
  buildJoopoChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchJoopoChrome,
  resolveJoopoUserDataDir,
  stopJoopoChrome,
} from "./chrome.js";
 ...
 * not exercise: launchJoopoChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopJoopoChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildJoopoChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveJoopoUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveJoopoUserDataDir();
      expect(dir.endsWith(path.join("joopo", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveJoopoUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildJoopoChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildJoopoChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchJoopoChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchJoopoChrome(resolved, profile);
  ...
  describe("launchJoopoChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "joopo-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchJoopoChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchJoopoChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      ...
      const userDataDir = resolveJoopoUserDataDir(profile.name);
      ...
      try {
        const running = await launchJoopoChrome(
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchJoopoChrome(resolved, profile)).rejects.toThrow(
    ...
    it("uses the configured local launch timeout while waiting for CDP discovery", async () => {
      ...
      try {
        ...
        const rejection = expect(launchJoopoChrome(resolved, profile)).rejects.toThrow(
          /Failed to start Chrome CDP/,
        );
  ...
  describe("stopJoopoChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopJoopoChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopJoopoChrome>[0],
        1,
      );
  ...
  describe("launchJoopoChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "joopo-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveJoopoUserDataDir(profileName));
        ...
        const staged = resolveJoopoUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_JOOPO_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchJoopoChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchJoopoChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateJoopoProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateJoopoProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateJoopoProfile }, "decorateJoopoProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".joopo-profile-decorated") || s.endsWith("Preferences")) {
