...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.JOOPO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "joopo",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/joopo" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    joopo: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchJoopoChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveJoopoUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopJoopoChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.JOOPO_GATEWAY_PORT;
  process.env.JOOPO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.JOOPO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.JOOPO_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.JOOPO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.JOOPO_GATEWAY_PASSWORD;
  } else {
    process.env.JOOPO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
