...
vi.mock("joopo/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/ssrf-runtime")>(
    "joopo/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
describe("fetchBrowserJson loopback auth", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("JOOPO_GATEWAY_TOKEN", "loopback-token");
  ...
  it("preserves dispatcher timeout context without no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: ["Chrome CDP handshake timeout", "Restart the Joopo gateway"],
      omits: ["Can't reach the Joopo browser control service", "Do NOT retry the browser tool"],
    });
  });
  ...
  it("preserves dispatcher abort context without no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: ["operation aborted", "Restart the Joopo gateway"],
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=manual"),
      {
        contains: [
          ...
          "browser profile is external to Joopo",
          "Restarting the Joopo gateway will not launch it",
        ],
        omits: ["Restart the Joopo gateway", "Do NOT retry the browser tool"],
      },
    );
  });
  ...
  it("keeps restart-gateway guidance for managed local dispatcher timeouts", async () => {
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=joopo"),
      {
        contains: ["Chrome CDP handshake timeout", "Restart the Joopo gateway"],
        omits: ["browser profile is external to Joopo", "Do NOT retry the browser tool"],
      },
    );
  });
  ...
  it("keeps no-retry hint but not restart guidance for persistent external profile failures", async () => {
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=manual"),
      {
        ...
        omits: ["Restart the Joopo gateway"],
      },
    );
  });
  ...
  it("keeps no-retry hint for persistent dispatcher failures", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      ...
      omits: ["Can't reach the Joopo browser control service"],
    });
  });
  ...
  it("keeps absolute URL failures wrapped as reachability errors", async () => {
    ...
    await expectThrownBrowserFetchError(
      ...
      {
        contains: [
          "Can't reach the Joopo browser control service",
          "Do NOT retry the browser tool",
        ],
      },
    );
