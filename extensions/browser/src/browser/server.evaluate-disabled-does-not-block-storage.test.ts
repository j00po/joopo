...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        joopo: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.JOOPO_GATEWAY_PORT;
    process.env.JOOPO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.JOOPO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.JOOPO_GATEWAY_PASSWORD;
