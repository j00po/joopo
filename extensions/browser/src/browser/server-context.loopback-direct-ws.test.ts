...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const joopo = ctx.forProfile("joopo");

    const opened = await joopo.openTab("about:blank");
  ...
  it("uses an HTTP /json base for focus and close under strict SSRF", async () => {
    ...
    await joopo.focusTab("T1");
    await joopo.closeTab("T1");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await joopo.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await joopo.focusTab("T2");
    await joopo.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(joopo.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(joopo.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(joopo.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
