...
const { stopJoopoChromeMock } = vi.hoisted(() => ({
  stopJoopoChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopJoopoChrome: stopJoopoChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["joopo", "user"]);
    ...
    expect(stopMap.joopo).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopJoopoChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("joopo browser stop failed: Error: oops");
  });
});
