...
function localJoopoProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "joopo",
...
function createLocalJoopoResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localJoopoProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveJoopoUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-reset-"));
    const profileDir = path.join(tempRoot, "joopo");
    ...
    const ops = createLocalJoopoResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveJoopoUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-reset-no-own-"));
    const profileDir = path.join(tempRoot, "joopo");
    fs.mkdirSync(profileDir, { recursive: true });
