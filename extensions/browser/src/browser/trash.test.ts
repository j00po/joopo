...
const resolvePreferredJoopoTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/joopo"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "joopo-browser-")));
    ...
    resolvePreferredJoopoTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "joopo-demo");
