...
type StopChromeTarget = Parameters<typeof stopJoopoChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "joopo-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateJoopoProfile(userDataDir, { color: DEFAULT_JOOPO_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_JOOPO_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_JOOPO_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".joopo-profile-decorated"),
      "utf-8",
    );
  ...
  it("writes managed download prefs when a download dir is provided", async () => {
    ...
    decorateJoopoProfile(userDataDir, {
      color: DEFAULT_JOOPO_BROWSER_COLOR,
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateJoopoProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_JOOPO_BROWSER_PROFILE_NAME);
  });
...
describe("browser chrome helpers", () => {
  ...
  it("stopJoopoChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopJoopoChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopJoopoChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildJoopoChromeLaunchArgs({
      ...
      userDataDir: "/tmp/joopo-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/joopo-test-user-data");
  });
});
