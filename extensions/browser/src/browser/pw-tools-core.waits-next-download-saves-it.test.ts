...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredJoopoTmpDir: vi.fn(() => "/tmp/joopo"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-joopo-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-joopo-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredJoopoTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredJoopoTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredJoopoTmpDir.mockReturnValue("/tmp/joopo");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredJoopoTmpDir.mockReturnValue("/tmp/joopo-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "joopo-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "joopo-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredJoopoTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(res.path)).toBe(
      path.resolve(path.join(path.sep, "tmp", "joopo-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toBe(path.basename(res.path));
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "joopo-preferred", "downloads")}${path.sep}`),
    );
  });
