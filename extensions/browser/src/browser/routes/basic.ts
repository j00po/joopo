...
async function runBrowserLiveProbe(req: BrowserRequest, ctx: BrowserRouteContext) {
  ...
  try {
    ...
    return {
      ...
      status: "fail" as const,
      summary: String(err),
      fixHint: "Run joopo browser start, then retry with joopo browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "joopo" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched joopo profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post(
    ...
    asyncBrowserRoute(async (req, res) => {
      ...
      if (driver && driver !== "joopo" && driver !== "clawd" && driver !== "existing-session") {
        return jsonError(
          res,
          400,
          `unsupported profile driver "${driver}"; use "joopo", "clawd", or "existing-session"`,
        );
      }
