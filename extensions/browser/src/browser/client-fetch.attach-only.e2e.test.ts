...
import { clearRuntimeConfigSnapshot } from "joopo/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("joopo-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".joopo", "joopo.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to Joopo");
      expect(message).toContain("Restarting the Joopo gateway will not launch it");
      expect(message).not.toContain("Restart the Joopo gateway");
