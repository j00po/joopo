...
const mocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => JoopoConfig>(),
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<JoopoConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: JoopoConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): JoopoConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: JoopoConfig) {
