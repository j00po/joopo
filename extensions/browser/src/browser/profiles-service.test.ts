...
import { resolveJoopoUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn<(cfg: JoopoConfig) => Promise<void>>(async (_cfg) => {}),
}));
...
vi.mock("./chrome.js", () => ({
  resolveJoopoUserDataDir: vi.fn(() => "/tmp/joopo-test/joopo/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "joopo-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveJoopoUserDataDir).mockReturnValue(userDataDir);
