...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".joopo-profile-decorated");
}
 ...
 */
export function decorateJoopoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_JOOPO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_JOOPO_BROWSER_COLOR).toUpperCase();
