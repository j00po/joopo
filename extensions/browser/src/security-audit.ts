import type { JoopoPluginSecurityAuditContext } from "joopo/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "joopo/plugin-sdk/secret-input";
import { formatCliCommand } from "joopo/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "joopo/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: JoopoPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("joopo security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.JOOPO_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.JOOPO_GATEWAY_PASSWORD) ||
