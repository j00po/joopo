...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Joopo's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Joopo's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated Joopo-managed browser (`joopo`).",
...
export const browserPluginNodeHostCommands: JoopoPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: JoopoPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): JoopoPluginService {
  let service: JoopoPluginService | null = null;
...
export function registerBrowserPlugin(api: JoopoPluginApi) {
  api.registerTool(((ctx: JoopoPluginToolContext) =>
