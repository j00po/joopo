...
// a config block to ~/.joopo/joopo.json under
...
import { mutateConfigFile } from "joopo/plugin-sdk/config-mutation";
...
export async function persistAllowAlways(input: {
  ...
  await mutateConfigFile({
    ...
    mutate: (draft) => {
      // Plugin config is intentionally plugin-owned; the root JoopoConfig
