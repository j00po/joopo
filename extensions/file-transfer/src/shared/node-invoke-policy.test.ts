...
import type { JoopoPluginNodeInvokePolicyContext } from "joopo/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: JoopoPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<JoopoPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<JoopoPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
