...
import type {
  ...
  JoopoPluginNodeInvokePolicyContext,
  JoopoPluginNodeInvokePolicyResult,
...
}): Promise<JoopoPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: JoopoPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: JoopoPluginNodeInvokePolicyContext,
): Promise<JoopoPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): JoopoPluginNodeInvokePolicy {
  return {
    commands: COMMANDS,
