...
import { createSandboxTestContext } from "joopo/plugin-sdk/test-fixtures";
import {
...
const JOOPO_OPENSHELL_E2E = process.env.JOOPO_E2E_OPENSHELL === "1";
const JOOPO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const JOOPO_OPENSHELL_COMMAND =
  process.env.JOOPO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "joopo-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && JOOPO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: JOOPO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(JOOPO_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(JOOPO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-openshell-e2e-"));
      ...
      const gatewayName = `joopo-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `joopo-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/joopo-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: JOOPO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("joopo-openshell-e2e");
