...
import type { JoopoConfig, RuntimeEnv } from "./runtime-api.js";
...
function createRuntimeCore(cfg: JoopoConfig) {
  ...
  return {
    ...
    channel: {
      ...
      session: {
        resolveStorePath: () => "/tmp/joopo-test-sessions.json",
...
describe("mattermost inbound user posts", () => {
  ...
  it("pins direct-message main route updates to the configured owner", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const directConfig: JoopoConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
