...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__joopo_channel_id";
...
type InteractionCallbackConfig = Pick<JoopoConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "joopo-mattermost-interactions").update(botToken).digest("hex");
}
