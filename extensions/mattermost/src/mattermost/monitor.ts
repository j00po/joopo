...
import { resolveChannelStreamingPreviewToolProgress } from "joopo/plugin-sdk/channel-streaming";
import { isLoopbackHost } from "joopo/plugin-sdk/gateway-runtime";
import { createClaimableDedupe, type ClaimableDedupe } from "joopo/plugin-sdk/persistent-dedupe";
import { isReasoningReplyPayload } from "joopo/plugin-sdk/reply-payload";
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  const cfg = (opts.config ?? core.config.current()) as JoopoConfig;
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `Joopo ignored this action for ${decision.roomLabel}.`,
          },
        };
      },
