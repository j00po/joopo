...
import { DEFAULT_ACCOUNT_ID } from "joopo/plugin-sdk/setup";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import type { JoopoConfig, JoopoPluginApi } from "../runtime-api.js";
...
vi.mock("./setup.accounts.runtime.js", () => ({
  listMattermostAccountIds: vi.fn((cfg: JoopoConfig) => {
  ...
  it("offers env shortcut only for the default account when env is present and config is empty", () => {
    ...
    expect(
      mattermostSetupWizard.envShortcut?.isAvailable?.({
        cfg: { channels: { mattermost: {} } } as JoopoConfig,
        accountId: "default",
      } as never),
    ).toBe(true);
  ...
  it("keeps env shortcut as a no-op patch for the selected account", () => {
    expect(
      mattermostSetupWizard.envShortcut?.apply?.({
        cfg: { channels: { mattermost: { enabled: false } } } as JoopoConfig,
        accountId: "default",
      } as never),
    ).toEqual({
      channels: {
