import { formatErrorMessage } from "joopo/plugin-sdk/agent-harness-runtime";
import type {
...
const CODEX_CONVERSATION_GLOBAL_STATE = Symbol.for("joopo.codex.conversationBinding");
  ...
  const response: CodexThreadStartResponse = await client.request(
    "thread/start",
    {
      ...
      developerInstructions:
        "This Codex thread is bound to an Joopo conversation. Answer normally; Joopo will deliver your final response back to the conversation.",
    ...
    async (request): Promise<JsonValue | undefined> => {
      if (request.method === "item/tool/call") {
        return {
          contentItems: [
            {
              type: "inputText",
              text: "Joopo native Codex conversation binding does not expose dynamic Joopo tools yet.",
            },
          ],
          success: false,
        };
      ...
      if (
        ...
        return {
          decision: "decline",
          reason:
            "Joopo native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.",
        };
      }
