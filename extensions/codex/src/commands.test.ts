...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`Joopo session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`Joopo session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-codex-command-"));
  });
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this Joopo session to Codex thread thread-123.",
    });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this Joopo session yet.",
    });
  });
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("Joopo session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("Joopo session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("Joopo session key: `agent:main:discord:two`");
    expect(request.text).toContain("Joopo session id: `session-two`");
