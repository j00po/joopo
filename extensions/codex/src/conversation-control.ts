...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("joopo.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this Joopo session yet.");
  }
  return binding;
}
