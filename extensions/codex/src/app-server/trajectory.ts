...
import { resolveUserPath } from "joopo/plugin-sdk/agent-harness-runtime";
import type {
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.JOOPO_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.JOOPO_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
