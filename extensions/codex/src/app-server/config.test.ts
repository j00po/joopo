...
describe("Codex app-server config", () => {
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        JOOPO_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        JOOPO_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expect(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { JOOPO_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
    ).toEqual(
      expect.objectContaining({
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expect(
      resolveRuntimeForTest({
        ...
        env: { JOOPO_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
    ).toEqual(
      expect.objectContaining({
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          JOOPO_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expect(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          JOOPO_CODEX_COMPUTER_USE: "1",
          JOOPO_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          JOOPO_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          JOOPO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
    ).toEqual(
      expect.objectContaining({
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { JOOPO_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("ignores removed JOOPO_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { JOOPO_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../joopo.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
