...
import { resolveAgentDir } from "joopo/plugin-sdk/agent-runtime";
import { emitTrustedDiagnosticEvent } from "joopo/plugin-sdk/diagnostic-runtime";
...
type JoopoCodingToolsOptions = NonNullable<
  Parameters<(typeof import("joopo/plugin-sdk/agent-harness"))["createJoopoCodingTools"]>[0]
>;
type JoopoCodingToolsFactory =
  (typeof import("joopo/plugin-sdk/agent-harness"))["createJoopoCodingTools"];
...
let joopoCodingToolsFactoryForTests: JoopoCodingToolsFactory | undefined;
...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Joopo dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Joopo dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
function fingerprintCodexPluginAppCacheCredentials(
  ...
  hash.update("joopo:codex:plugin-app-cache-credentials:v1");
  hash.update("\0");
...
export async function runCodexAppServerAttempt(
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    if (isCurrentTurnNotification && notification.method === "error") {
      ...
      // Joopo hands a turn-scoped request result back to Codex. Once Codex
...
async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedDynamicToolResponse("Joopo dynamic tool call aborted before execution.");
  }
  ...
  const abortFromRun = () => {
    const message = "Joopo dynamic tool call aborted.";
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Joopo dynamic tool call finished."));
    }
  }
}
...
function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Joopo plugin approval prompt.
...
function buildCodexNativeHookRelayId(params: {
  ...
  const hash = createHash("sha256");
  hash.update("joopo:codex:native-hook-relay:v1");
  hash.update("\0");
...
function resolveJoopoCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<JoopoCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
    sessionKey: sandboxSessionKey,
    runSessionKey:
...
async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const createJoopoCodingTools =
    joopoCodingToolsFactoryForTests ??
    (await import("joopo/plugin-sdk/agent-harness")).createJoopoCodingTools;
  const sessionKeys = resolveJoopoCodingToolsSessionKeys(params, input.sandboxSessionKey);
  const allTools = createJoopoCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as JoopoCodingToolsOptions["modelCompat"])
        : undefined,
...
export const __testing = {
  ...
  resolveJoopoCodingToolsSessionKeys,
  shouldForceMessageTool,
  setJoopoCodingToolsFactoryForTests(factory: JoopoCodingToolsFactory): void {
    joopoCodingToolsFactoryForTests = factory;
  },
  resetJoopoCodingToolsFactoryForTests(): void {
    joopoCodingToolsFactoryForTests = undefined;
  },
