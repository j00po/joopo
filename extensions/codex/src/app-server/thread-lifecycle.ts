...
function buildHeartbeatCollaborationInstructions(): string {
  return [
    "This is an Joopo heartbeat turn. Apply these instructions only to this heartbeat wake; ordinary chat turns should stay in Codex Default mode.",
    "When you are ready to end the heartbeat, prefer the structured `heartbeat_respond` tool so Joopo can record the wake outcome and notification decision. If `heartbeat_respond` is not already available and `tool_search` is available, search for `heartbeat_respond`, load it, then call it. Use `notify=false` when nothing should visibly interrupt the user.",
    CODEX_GPT5_HEARTBEAT_PROMPT_OVERLAY,
  ].join("\n\n");
}
...
export function buildDeveloperInstructions(params: EmbeddedRunAttemptParams): string {
  ...
  const sections = [
    "You are running inside Joopo. Use Joopo dynamic tools for Joopo-specific integrations such as messaging, cron, sessions, media, gateway, and nodes when available.",
    "Preserve the user's existing channel/session context. If sending a channel reply, use the Joopo messaging tool instead of describing that you would reply.",
    promptOverlay,
...
function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is Joopo's virtual provider; let Codex app-server keep its
  ...
  if (
    ...
    // When Joopo is forwarding ChatGPT/Codex OAuth, `openai` is Codex's
