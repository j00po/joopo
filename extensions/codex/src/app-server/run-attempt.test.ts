...
} from "joopo/plugin-sdk/hook-runtime";
import { createMockPluginRegistry } from "joopo/plugin-sdk/plugin-test-runtime";
...
import { CODEX_JOOPO_DYNAMIC_TOOL_NAMESPACE } from "./dynamic-tools.js";
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/joopo-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt", () => {
  beforeEach(async () => {
    resetAgentEventsForTest();
    vi.stubEnv("JOOPO_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-codex-run-"));
  });

  afterEach(async () => {
    __testing.resetCodexAppServerClientFactoryForTests();
    __testing.resetJoopoCodingToolsFactoryForTests();
  ...
  it("allows Codex dynamic tool filtering to opt back into Joopo compatibility", () => {
    const tools = ["read", "exec", "message", "custom_tool"].map((name) => ({ name }));

    expect(
      __testing
        .applyCodexDynamicToolProfile(tools, {
          codexDynamicToolsProfile: "joopo-compat",
  ...
  it("starts Codex threads without duplicate Joopo workspace tools by default", async () => {
  ...
  it("starts Codex threads with searchable Joopo dynamic tools by default", async () => {
    __testing.setJoopoCodingToolsFactoryForTests(() => [
    ...
    expect(webSearch).toEqual(
      expect.objectContaining({
        namespace: CODEX_JOOPO_DYNAMIC_TOOL_NAMESPACE,
        deferLoading: true,
      }),
    );
  ...
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      __testing.resolveJoopoCodingToolsSessionKeys(
        params,
    ...
    expect(__testing.resolveJoopoCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("returns a failed dynamic tool response when an app-server tool call exceeds the deadline", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Joopo dynamic tool call timed out after 1ms while running tool message.",
        },
      ],
    });
  ...
  it("logs process poll timeout context separately from session idle", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Joopo dynamic tool call timed out after 1ms while waiting for process action=poll sessionId=rapid-crustacean. This is a tool RPC timeout, not a session idle timeout.",
        },
      ],
    });
      ...
      contentItems: [
        {
          ...
          text: expect.stringMatching(
            /^(Unknown Joopo tool: message|Action send requires a target\.)$/u,
          ),
        },
      ],
    });
  ...
  it("projects mirrored history when starting Codex without a native thread binding", async () => {
    ...
    params.prompt = "make the default webpage joopo";
    ...
    expect(inputText).toContain("Joopo assembled context for this turn:");
    ...
    expect(inputText).toContain("make the default webpage joopo");
  });

  it("passes Joopo bootstrap files through Codex developer instructions", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for joopo/joopo#67996: a throw inside the projector's
  ...
  it("builds resume and turn params from the currently selected Joopo model", () => {
    ...
    expect(buildTurnCollaborationMode(params)).toEqual({
      ...
      settings: {
        ...
        developer_instructions: expect.stringContaining(
          "This is an Joopo heartbeat turn. Apply these instructions only to this heartbeat wake",
        ),
      },
    });
