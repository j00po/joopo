import type { AnyAgentTool } from "joopo/plugin-sdk/agent-harness";
import { wrapToolWithBeforeToolCallHook } from "joopo/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installJoopoOwnedToolHooks,
  mediaToolResult,
  resetJoopoOwnedToolHooks,
  textToolResult,
...
describe("Joopo-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetJoopoOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installJoopoOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installJoopoOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installJoopoOwnedToolHooks();
