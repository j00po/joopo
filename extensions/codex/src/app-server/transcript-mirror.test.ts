...
import type { AgentMessage } from "joopo/plugin-sdk/agent-harness-runtime";
import {
...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-codex-transcript-"));
  tempDirs.push(dir);
...
describe("mirrorCodexAppServerTranscript", () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("joopo-codex-transcript-missing-dir-");
