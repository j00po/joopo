...
import { embeddedAgentLog, JOOPO_VERSION } from "joopo/plugin-sdk/agent-harness-runtime";
...
function defaultServerRequestResponse(
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "Joopo did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
  ...
  if (isCodexAppServerApprovalRequest(request.method)) {
    return {
      decision: "decline",
      reason: "Joopo codex app-server bridge does not grant native approvals yet.",
    };
  }
...
function timeoutServerRequestResponse(
  ...
  return {
    contentItems: [
      {
        type: "inputText",
        text: `Joopo dynamic tool call timed out after ${CODEX_DYNAMIC_TOOL_SERVER_REQUEST_TIMEOUT_MS}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): void {
  ...
  if (!detectedVersion) {
    throw new Error(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but Joopo could not determine the running Codex version. Update the configured Codex app-server binary, or remove custom command overrides to use the managed binary.`,
    );
  }
...
export function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Joopo, Codex Desktop, or an env override, so only the slash-delimited
