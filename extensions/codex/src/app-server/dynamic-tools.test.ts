...
import {
  CODEX_JOOPO_DYNAMIC_TOOL_NAMESPACE,
...
describe("createCodexDynamicToolBridge", () => {
  it("defers Joopo dynamic tools behind Codex tool search by default", () => {
  ...
  it("keeps config out of Codex tool-result contexts", async () => {
    const config = { session: { store: "/tmp/joopo-session-store.json" } };
    const registry = createEmptyPluginRegistry();
