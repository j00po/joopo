...
} from "joopo/plugin-sdk/migration";
import type {
  MigrationItem,
  MigrationPlan,
      ...
      createMigrationItem({
        ...
        reason: collides
          ...
          : targetExists && !params.overwrite
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: `Copy ${item.skill.sourceLabel} into this Joopo agent workspace.`,
        details: {
          skillName: item.name,
  ...
  for (const plugin of plugins) {
    if (
      ...
      items.push(
        createMigrationItem({
          ...
          message: `Install Codex plugin "${plugin.pluginName}" in the Joopo-managed Codex app-server runtime.`,
          details: {
            configKey,
    ...
    items.push(
      createMigrationManualItem({
        ...
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with joopo plugins install <path>.",
      }),
    );
  }
  return items;
...
function buildPluginConfigItem(
  ...
  return createMigrationItem({
    ...
    message:
      "Enable Joopo's Codex plugin integration and record migrated source-installed curated plugins.",
    details: {
...
export async function buildCodexMigrationPlan(
  ...
  const warnings = [
    ...
    ...(source.archivePaths.length > 0
      ? [
          "Codex config and hook files are archive-only. They are preserved in the migration report, not loaded into Joopo automatically.",
        ]
      : []),
  ];
  return {
    ...
    warnings,
    nextSteps: [
      "Run joopo doctor after applying the migration.",
      "Review skipped or auth-required Codex plugin/config/hook items before exposing them in Joopo sessions.",
    ],
    metadata: {
