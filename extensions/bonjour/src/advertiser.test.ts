...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.JOOPO_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const started = await startAdvertiser({
      ...
      cliPath: "/opt/homebrew/bin/joopo",
      minimal: false,
    });

    expect(createService).toHaveBeenCalledTimes(1);
    const [gatewayCall] = createService.mock.calls as Array<[Record<string, unknown>]>;
    expect(gatewayCall?.[0]?.type).toBe("joopo-gw");
  ...
  it("honors truthy JOOPO_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.JOOPO_DISABLE_BONJOUR = "true";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.JOOPO_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("logs ciao handler classifications at the bonjour caller", async () => {
    ...
    expect(
      handler?.(
        new Error(
          "Can't probe for a service which is announced already. Received announcing for service Joopo Gateway._joopo._tcp.local.",
        ),
      ),
    ).toBe(true);
    expect(logger.warn).toHaveBeenCalledWith(
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._joopo-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._joopo-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("Mac (Joopo)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
  ...
  it("falls back to joopo when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.JOOPO_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("joopo");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("joopo.local");
  ...
  it("truncates reported Kubernetes service name at the DNS label byte limit", async () => {
    ...
    expectDnsLabelByteLength(`${reportedHostname} (Joopo)`, 64);
  ...
  it("truncates multi-byte hostname within DNS label byte limit", async () => {
    // 21 CJK characters = 63 bytes in UTF-8, adding " (Joopo)" pushes over
  ...
  it("uses system hostname when JOOPO_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.JOOPO_MDNS_HOSTNAME;
