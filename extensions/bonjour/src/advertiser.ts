...
import type { PluginLogger } from "joopo/plugin-sdk/plugin-entry";
...
// See https://github.com/joopo/joopo/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.JOOPO_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Joopo"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Joopo";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Joopo\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.JOOPO_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "joopo";
    ...
    const recreateAdvertiser = async (reason: string, opts?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or JOOPO_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
