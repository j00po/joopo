...
import { canResolveEnvSecretRefInReadOnlyPath } from "joopo/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "joopo/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<JoopoConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: JoopoConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: JoopoConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: JoopoConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: JoopoConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: JoopoConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: JoopoConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: JoopoConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: JoopoConfig, override?: number): number {
