...
import { buildOauthProviderAuthResult } from "joopo/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "joopo/plugin-sdk/provider-auth-api-key";
import { buildProviderReplayFamilyHooks } from "joopo/plugin-sdk/provider-model-shared";
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "joopo/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "joopo/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: JoopoConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: JoopoConfig): string | undefined {
...
export function registerMinimaxProviders(api: JoopoPluginApi) {
