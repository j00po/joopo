...
import { resolveChannelTtsVoiceDelivery } from "joopo/plugin-sdk/channel-targets";
import type {
  JoopoConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { isVerbose, logVerbose } from "joopo/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredJoopoTmpDir } from "joopo/plugin-sdk/sandbox";
import { privateFileStoreSync } from "joopo/plugin-sdk/security-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.JOOPO_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: JoopoConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: JoopoConfig): TtsProvider {
  return sortSpeechProvidersForAutoSelection(cfg)[0]?.id ?? "";
}

function resolveTtsRuntimeConfig(cfg: JoopoConfig): JoopoConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: JoopoConfig): TtsProvider[] {
