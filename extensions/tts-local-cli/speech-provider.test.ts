...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "joopo/plugin-sdk/speech-core";
...
vi.mock("joopo/plugin-sdk/media-runtime", () => ({
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "joopo-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.JOOPO_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("joopo/plugin-sdk/media-runtime")>(
