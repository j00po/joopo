...
import type { MemoryEmbeddingProvider } from "joopo/plugin-sdk/memory-core-host-engine-embeddings";
import { resolveLivePluginConfigObject } from "joopo/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "joopo/plugin-sdk/runtime-env";
import {
  normalizeLowercaseStringOrEmpty,
  truncateUtf16Safe,
} from "joopo/plugin-sdk/text-runtime";
import { Type } from "typebox";
import { definePluginEntry, type JoopoPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("joopo/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("joopo/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("joopo/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("joopo/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("joopo/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("joopo/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as JoopoConfig;
...
function createEmbeddings(api: JoopoPluginApi, cfg: MemoryConfig): Embeddings {
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as JoopoConfig
          : undefined,
        "memory-lancedb",
