...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "joopo-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("joopo/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      const mockApi = {
        ...
        runtime: {
          ...
          agent: {
            resolveAgentDir: vi.fn(() => "/tmp/joopo-agent"),
          },
        },
        logger: {
          info: vi.fn(),
      ...
      vi.doUnmock("joopo/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
  ...
  test("runs auto-recall through the registered before_prompt_build hook", async () => {
    ...
    vi.resetModules();
    vi.doMock("joopo/plugin-sdk/runtime-env", () => ({
