import type { ChannelConfigUiHint } from "joopo/plugin-sdk/channel-core";
...
export const discordChannelConfigUiHints = {
  ...
  "streaming.progress.labels": {
    ...
    help: 'Candidate labels for streaming.progress.label="auto". Leave unset to use Joopo built-in progress labels.',
  },
  ...
  "eventQueue.listenerTimeout": {
    ...
    help: "Canonical Discord listener timeout control in ms for gateway normalization/enqueue handlers. Default is 120000 in Joopo; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.",
  },
  ...
  gatewayInfoTimeoutMs: {
    ...
    help: "Timeout for Discord /gateway/bot metadata lookup before falling back to the default gateway URL. Default is 30000; JOOPO_DISCORD_GATEWAY_INFO_TIMEOUT_MS can override when config is unset.",
  },
  gatewayReadyTimeoutMs: {
    label: "Discord Gateway READY Timeout (ms)",
    help: "Startup wait for the Discord gateway READY event before restarting the socket. Default is 15000; JOOPO_DISCORD_READY_TIMEOUT_MS can override when config is unset.",
  },
  gatewayRuntimeReadyTimeoutMs: {
    label: "Discord Gateway Runtime READY Timeout (ms)",
    help: "Runtime reconnect wait for the Discord gateway READY event before force-stopping the lifecycle. Default is 30000; JOOPO_DISCORD_RUNTIME_READY_TIMEOUT_MS can override when config is unset.",
  },
  "voice.enabled": {
  ...
  "voice.reconnectGraceMs": {
    ...
    help: "Grace period for a disconnected Discord voice session to enter Signalling or Connecting before Joopo destroys it. Default: 15000.",
  },
  "voice.captureSilenceGraceMs": {
    label: "Discord Voice Capture Silence Grace (ms)",
    help: "Silence window after Discord reports a speaker ended before Joopo finalizes the audio segment for transcription. Default: 2500.",
  },
  "voice.tts": {
