...
function createCfg(): JoopoConfig {
  return {
    channels: {
      discord: {
        enabled: true,
...
function resolveAccount(cfg: JoopoConfig, accountId = "default"): ResolvedDiscordAccount {
  return discordPlugin.config.resolveAccount(cfg, accountId);
}

function startDiscordAccount(cfg: JoopoConfig, accountId = "default") {
