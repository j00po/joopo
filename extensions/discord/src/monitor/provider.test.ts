...
let runtimeEnvModule: typeof import("joopo/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): JoopoConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("joopo/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("joopo/plugin-sdk/runtime-env");
  ...
  it("passes Joopo event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
