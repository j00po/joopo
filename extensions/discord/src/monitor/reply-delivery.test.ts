...
vi.mock("joopo/plugin-sdk/outbound-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/outbound-runtime")>(
    "joopo/plugin-sdk/outbound-runtime",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    await deliverDiscordReply({
      ...
      textLimit: 2000,
      replyToMode: "off",
      mediaLocalRoots: ["/tmp/joopo-media"],
    });
    ...
    expect(deliverOutboundPayloadsMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        replyToId: undefined,
        replyToMode: "off",
        mediaAccess: { localRoots: ["/tmp/joopo-media"] },
      }),
    );
  });
