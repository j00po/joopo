...
import { clearSessionStoreCacheForTest } from "joopo/plugin-sdk/session-store-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { ChannelType, type AutocompleteInteraction } from "../internal/discord.js";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("joopo/plugin-sdk/conversation-binding-runtime");
...
vi.mock("joopo/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("joopo/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("joopo/plugin-sdk/conversation-binding-runtime")>(
        "joopo/plugin-sdk/conversation-binding-runtime",
      ),
  );
});

vi.mock("joopo/plugin-sdk/agent-runtime", () => ({
  normalizeProviderId: (value: string) => value.trim().toLowerCase(),
  resolveDefaultModelForAgent: (params: { cfg: JoopoConfig }) => {
...
vi.mock("joopo/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `joopo-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("joopo/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("joopo/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("joopo/plugin-sdk/command-auth");
