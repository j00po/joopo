...
import * as commandRegistryModule from "joopo/plugin-sdk/command-auth";
import type { ChatCommandDefinition, CommandArgsParsing } from "joopo/plugin-sdk/command-auth";
import type { ModelsProviderData } from "joopo/plugin-sdk/command-auth";
import type { JoopoConfig } from "joopo/plugin-sdk/config-types";
import * as globalsModule from "joopo/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "joopo/plugin-sdk/session-store-runtime";
import * as commandTextModule from "joopo/plugin-sdk/text-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "joopo-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          {
            id: "pi",
            label: "Joopo Pi Default",
            description: "Use the built-in Joopo Pi runtime.",
          },
          {
            id: "codex",
            label: "codex",
