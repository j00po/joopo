...
function createThreadBinding(
  overrides?: Partial<import("joopo/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("joopo/plugin-sdk/conversation-runtime").SessionBindingRecord;
}

function createPreflightArgs(params: {
  cfg: import("joopo/plugin-sdk/config-types").JoopoConfig;
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("joopo/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello joopo from dm audio");
    ...
    expect(result?.preflightAudioTranscript).toBe("hello joopo from dm audio");
  });
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("joopo/plugin-sdk/config-types").JoopoConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "Joopo", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@joopo-bot> /new incident room",
      mentionedUsers: [{ id: "joopo-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey joopo");
    ...
    expect(result).not.toBeNull();
    expect(result?.wasMentioned).toBe(true);
    expect(result?.preflightAudioTranscript).toBe("hey joopo");
  });
  ...
  it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
    const conversationRuntime = await import("joopo/plugin-sdk/conversation-runtime");
