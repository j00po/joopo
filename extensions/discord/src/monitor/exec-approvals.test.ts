...
vi.mock("joopo/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("joopo/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<JoopoConfig["channels"]>["discord"]>["execApprovals"],
): JoopoConfig {
  return {
    channels: {
      discord: {
