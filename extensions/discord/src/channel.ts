...
import { sleepWithAbort } from "joopo/plugin-sdk/runtime-env";
import {
  createComputedAccountStatusAdapter,
  createDefaultChannelRuntimeState,
} from "joopo/plugin-sdk/status-helpers";
import { resolveTargetsWithOptionalToken } from "joopo/plugin-sdk/target-resolver-runtime";
...
function resolveDiscordStartupDelayMs(cfg: JoopoConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Joopo adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
