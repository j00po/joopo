...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: JoopoConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): JoopoConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("joopo/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/acp-runtime")>(
    "joopo/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/command-auth")>(
    "joopo/plugin-sdk/command-auth",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/reply-runtime")>(
...
vi.mock("joopo/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("joopo/plugin-sdk/native-command-config-runtime")
  >("joopo/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/runtime-env")>(
    "joopo/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/error-runtime")>(
    "joopo/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
