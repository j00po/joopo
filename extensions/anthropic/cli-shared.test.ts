...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it("maps Joopo thinking levels to Claude effort args", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from Joopo YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent Joopo exec policy", () => {
    expect(
      resolveClaudePermissionMode({
        backendId: "claude-cli",
        agentId: "safe-agent",
