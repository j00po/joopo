import { formatCliCommand, parseDurationMs } from "joopo/plugin-sdk/cli-runtime";
import type {
  JoopoPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type JoopoConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "joopo/plugin-sdk/provider-auth";
import {
  cloneFirstTemplateModel,
  isClaudeOpus47ModelId,
  type ProviderPlugin,
  resolveClaudeThinkingProfile,
} from "joopo/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "joopo/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("joopo models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("joopo models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("joopo doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: JoopoPluginApi): void {
