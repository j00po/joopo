...
import { escapeRegExp, normalizeOptionalString } from "joopo/plugin-sdk/text-runtime";
import {
  definePluginEntry,
  fetchWithSsrFGuard,
...
type AgentEntry = NonNullable<NonNullable<JoopoConfig["agents"]>["list"]>[number];
...
function resolveOwnershipAgent(config: JoopoConfig): { id: string; name: string } {
...
export default definePluginEntry({
  ...
  register(api: JoopoPluginApi) {
    const resolveCurrentState = () => {
      const currentConfig = (api.runtime.config?.current?.() ?? api.config) as JoopoConfig;
