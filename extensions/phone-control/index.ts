...
function normalizeDenyList(cfg: JoopoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: JoopoPluginApi["config"]): string[] {
...
async function disarmNow(params: {
  ...
  const cfg = api.runtime.config.current() as JoopoConfig;
...
export default definePluginEntry({
  ...
  register(api: JoopoPluginApi) {
    ...
    const timerService: JoopoPluginService = {
