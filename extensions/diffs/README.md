...
Read-only diff viewer plugin for **Joopo** agents.
...
Set plugin-wide defaults in `~/.joopo/joopo.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/joopo",
        },
      },
    },
  },
...
Before:
Joopo supports plugins.

After:
Joopo supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/joopo-diffs`).
