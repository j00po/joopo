...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("joopo-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/joopo/plugins/diffs/view/",
    );
    expect((result?.details as Record<string, unknown>).viewerUrl).toEqual(
      expect.stringContaining("https://example.com/joopo/plugins/diffs/view/"),
    );
  });
...
function createApi(pluginConfig?: Record<string, unknown>): JoopoPluginApi {
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: JoopoPluginToolContext = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
