...
import type { JsonSchemaObject } from "joopo/plugin-sdk/config-schema";
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/joopo/",
      }),
    ).toBe("https://example.com/joopo");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/joopo?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/joopo#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/joopo/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/joopo",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/joopo/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("joopoDiffsReady");
