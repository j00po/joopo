...
describe("DiffArtifactStore", () => {
  ...
  beforeEach(async () => {
    ...
    } = await createDiffStoreHarness("joopo-diffs-store-"));
  });
...
describe("createDiffsHttpHandler", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("joopo-diffs-http-"));
  });
  ...
  it("serves the shared viewer runtime asset", async () => {
    ...
    expect(handled).toBe(true);
    expect(res.statusCode).toBe(200);
    expect(String(res.body)).toContain("joopoDiffsReady");
  });
