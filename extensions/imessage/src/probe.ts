...
import type { BaseProbeResult } from "joopo/plugin-sdk/channel-contract";
import { runCommandWithTimeout } from "joopo/plugin-sdk/process-runtime";
...
import { detectBinary } from "joopo/plugin-sdk/setup";
...
export function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run Joopo on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
