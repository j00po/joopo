...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: JoopoConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: JoopoConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: JoopoConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run Joopo on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `joopo channels status --probe` to verify private API actions.",
    "Ensure Joopo has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: JoopoConfig; accountId?: string }) =>
