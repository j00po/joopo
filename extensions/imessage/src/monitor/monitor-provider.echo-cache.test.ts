...
describe("iMessage sent-message echo cache", () => {
  ...
  it("matches persisted echoes written by another process", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-imsg-echo-"));
    ...
    rememberPersistedIMessageEcho({
      ...
      text: "Joopo imsg live test",
      messageId: "guid-1",
    });

    expect(cache.has("acct:imessage:+1555", { text: "Joopo imsg live test" })).toBe(true);
    expect(cache.has("acct:imessage:+1666", { text: "Joopo imsg live test" })).toBe(false);
  ...
  it("writes sent-echoes.jsonl 0600 and parent dir 0700", () => {
    ...
    // mitigation.
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-imsg-echo-perm-"));
    tempDirs.push(stateDir);
    vi.stubEnv("JOOPO_STATE_DIR", stateDir);
  ...
  it("clamps pre-existing sent-echoes.jsonl from older 0644/0755 to 0600/0700", () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-imsg-echo-clamp-"));
    tempDirs.push(stateDir);
    vi.stubEnv("JOOPO_STATE_DIR", stateDir);
