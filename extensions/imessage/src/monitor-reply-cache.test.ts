...
// Isolate from any live ~/.joopo/imessage/reply-cache.jsonl that the
...
beforeAll(() => {
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-imsg-reply-cache-"));
  priorStateDir = process.env.JOOPO_STATE_DIR;
...
afterAll(() => {
  if (priorStateDir === undefined) {
    ...
    process.env.JOOPO_STATE_DIR = priorStateDir;
  }
...
beforeEach(() => {
  ...
  // _reset helper does this when JOOPO_STATE_DIR is set, but explicitly
...
describe("findLatestIMessageEntryForChat", () => {
  ...
  it("never crosses account boundaries", () => {
    ...
    expect(process.env.JOOPO_STATE_DIR).toBe(tempStateDir);
...
describe("hydrate-on-resolve (post-restart short-id persistence)", () => {
  it("hydrates the on-disk JSONL before resolving a short id whose mapping predates this run", () => {
    ...
    // JOOPO_STATE_DIR is set, so we have to keep the file ourselves
