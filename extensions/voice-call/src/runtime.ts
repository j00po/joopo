...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Joopo agent receiving delegated requests from a live phone voice bridge.",
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as JoopoConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as JoopoConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
