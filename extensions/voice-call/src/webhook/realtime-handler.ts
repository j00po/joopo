...
function buildForcedConsultSpeechPrompt(result: string): string {
  ...
  return [
    "Internal Joopo consult result is ready.",
        ...
        params.handler(
          {
            ...
            context:
              "The realtime provider produced a final user transcript without invoking joopo_agent_consult, so Joopo is forcing the consult because consultPolicy is always.",
          },
          params.callId,
          {},
        ),
      ...
      if (forcedConsult) {
        if (forcedConsult.completedAt) {
          submitFinalToolResult({
            ...
            message: "Joopo already delivered this consult result internally. Do not repeat it.",
          });
          return;
        }
