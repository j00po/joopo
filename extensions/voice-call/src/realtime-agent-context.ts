...
import { root } from "joopo/plugin-sdk/security-runtime";
...
function buildConsultPolicyGuidance(
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call joopo_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call joopo_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's Joopo-specific context.",
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "Joopo agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the Joopo agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call joopo_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as JoopoConfig,
      agentId,
