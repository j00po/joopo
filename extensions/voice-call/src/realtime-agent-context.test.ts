...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "joopo-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity, system prompt, and workspace context", async () => {
    ...
    expect(instructions).toContain("Joopo agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call joopo_agent_consult before answering requests");
