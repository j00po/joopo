...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<JoopoPluginApi["registerService"]>[0];
};
...
function setup(config: Record<string, unknown>): Registered {
  ...
  const api = createTestPluginApi({
    ...
    config: {},
    pluginConfig: config,
    runtime: { tts: { textToSpeechTelephony: vi.fn() } } as unknown as JoopoPluginApi["runtime"],
    logger: noopLogger,
...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("joopo.voice-call.runtime")];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("joopo.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("joopo.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("JOOPO_CLI", "1");
    process.argv = ["node", "joopo", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expect(noopLogger.warn).toHaveBeenCalledWith(
      expect.stringContaining('Run "joopo doctor --fix"'),
    );
  });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager.initiateCall).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "Joopo voice call smoke test.",
        mode: "notify",
      });
