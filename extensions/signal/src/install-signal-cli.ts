...
import { runPluginCommandWithTimeout } from "joopo/plugin-sdk/run-command";
import type { RuntimeEnv } from "joopo/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "joopo/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredJoopoTmpDir(), "joopo-signal-"));
