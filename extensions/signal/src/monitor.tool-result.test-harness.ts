import type { MockFn } from "joopo/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/joopo-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("joopo/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/system-event-runtime")>(
    "joopo/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/transport-ready-runtime", () => ({
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("joopo/plugin-sdk/reply-runtime"),
      import("joopo/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
