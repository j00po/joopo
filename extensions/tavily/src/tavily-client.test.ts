...
vi.mock("joopo/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: joopo", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "joopo" });
  });

  it("runTavilyExtract sends X-Client-Source: joopo", async () => {
