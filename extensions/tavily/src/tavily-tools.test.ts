...
import type { JoopoPluginApi } from "joopo/plugin-sdk/plugin-runtime";
...
function fakeApi(): JoopoPluginApi {
  return {
    config: {},
  } as JoopoPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.joopo.ai", "", "joopo.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.joopo.ai", "joopo.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<JoopoPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<JoopoPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "joopo" });
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as JoopoConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
