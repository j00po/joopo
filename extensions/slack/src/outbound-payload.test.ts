import { installChannelOutboundPayloadContractSuite } from "joopo/plugin-sdk/channel-contract-testing";
...
describe("slackOutbound sendPayload", () => {
  ...
  it("offsets presentation controls against native Slack blocks before standalone interactive controls", async () => {
        ...
        channelData: {
          slack: {
            blocks: [
              {
                type: "actions",
                block_id: "joopo_reply_buttons_1",
                elements: [],
              },
            ],
          },
    ...
    expect(sendMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        blocks: [
          expect.objectContaining({ block_id: "joopo_reply_buttons_1" }),
          expect.objectContaining({
            block_id: "joopo_reply_buttons_2",
            elements: [expect.objectContaining({ action_id: "joopo:reply_button:2:1" })],
          }),
          expect.objectContaining({
            block_id: "joopo_reply_buttons_3",
            elements: [expect.objectContaining({ action_id: "joopo:reply_button:3:1" })],
          }),
        ],
      }),
    );
