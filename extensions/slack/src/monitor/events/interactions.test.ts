...
vi.mock("joopo/plugin-sdk/system-event-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("joopo/plugin-sdk/system-event-runtime")>();
  return {
    ...actual,
...
vi.mock("joopo/plugin-sdk/approval-gateway-runtime", () => ({
  resolveApprovalOverGateway: (arg: unknown) => resolveApprovalOverGatewayMock(arg),
}));

vi.mock("joopo/plugin-sdk/security-runtime", () => ({
...
describe("registerSlackInteractionEvents", () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "joopo:verify" }],
            },
          ],
        },
      },
  ...
  it("registers a matcher that accepts plugin action ids beyond the Joopo prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher.test("joopo:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "joopo:reply_button" }],
            },
          ],
        },
      },
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"joopo:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "joopo:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "joopo:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "joopo:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "joopo:datetime" }],
            },
          ],
        },
      },
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:joopo:deploy_form",
      callbackId: "joopo:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("keeps no-channel modal events open when allowFrom is unset", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V444",
          callback_id: "joopo:routing_form",
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@joopo.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.joopo.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "joopo:long_richtext",
