...
const JOOPO_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
...
export function shouldSkipJoopoSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? JOOPO_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Joopo's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
  });
  app.use(async (args) => {
    if (shouldSkipJoopoSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
