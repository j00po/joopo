...
type SlackRouteBinding = NonNullable<JoopoConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  JoopoConfig,
  { bindingsRef: JoopoConfig["bindings"]; normalizedCfg: JoopoConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: JoopoConfig): JoopoConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as JoopoConfig)
    : cfg;
