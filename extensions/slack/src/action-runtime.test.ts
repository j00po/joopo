...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): JoopoConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as JoopoConfig;
  ...
  async function resolveReadToken(cfg: JoopoConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: JoopoConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/joopo-media/report.pdf",
    ...
    expect(result.content[0]).toEqual(
      expect.objectContaining({
        type: "text",
        text: expect.stringContaining("/tmp/joopo-media/report.pdf"),
      }),
    );
    ...
    expect(result.details).toEqual(
      expect.objectContaining({
        ...
        contentType: "application/pdf",
        media: {
          mediaUrl: "/tmp/joopo-media/report.pdf",
          contentType: "application/pdf",
        },
      }),
    );
