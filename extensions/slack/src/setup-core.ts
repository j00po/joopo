import { hasConfiguredSecretInput } from "joopo/plugin-sdk/secret-input";
import {
  ...
  type JoopoConfig,
} from "joopo/plugin-sdk/setup-runtime";
import { formatDocsLink } from "joopo/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: JoopoConfig, accountId: string): JoopoConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: JoopoConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: JoopoConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: JoopoConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: JoopoConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: JoopoConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: JoopoConfig; accountId: string }) =>
    ...
    disable: (cfg: JoopoConfig) => setSetupChannelEnabled(cfg, channel, false),
