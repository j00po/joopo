import { CURRENT_MESSAGE_MARKER } from "joopo/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "joopo/plugin-sdk/channel-test-helpers";
import { resetInboundDedupe } from "joopo/plugin-sdk/reply-dedupe";
import { HISTORY_CONTEXT_MARKER } from "joopo/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bjoopo\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("joopo: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("joopo: hello <@U2>");
  });
