...
import { buildModelAliasIndex, resolveModelRefFromString } from "joopo/plugin-sdk/agent-runtime";
import {
  type JsonSchemaObject,
  validateJsonSchemaValue,
} from "joopo/plugin-sdk/json-schema-runtime";
import { normalizeOptionalString } from "joopo/plugin-sdk/text-runtime";
import { Type } from "typebox";
import { resolvePreferredJoopoTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<JoopoPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<JoopoPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return !!level && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: JoopoPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via joopo.invoke.",
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<JoopoPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredJoopoTmpDir(), prefix: "joopo-llm-task-" },
        async ({ dir: tmpDir }) => {
