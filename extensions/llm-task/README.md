...
`joopo.invoke --each`) without adding new Joopo code per workflow.
...
This extension depends on Joopo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Joopo extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.joopo/extensions` as a standalone plugin directory.
