...
describe("qqbot local media path remapping", () => {
  ...
  function createJoopoTestRoot() {
    const actualHome = getHomeDir();
    const joopoDir = path.join(actualHome, ".joopo");
    fs.mkdirSync(joopoDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(joopoDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createJoopoTestRoot();
  ...
  it("allows structured payload files inside sibling Joopo media subdirectories", () => {
    ...
    // trust the shared `~/.joopo/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".joopo", "media", "outbound");
