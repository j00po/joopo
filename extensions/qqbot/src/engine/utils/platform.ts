 ...
 * silk-wasm availability checks — all without importing `joopo/plugin-sdk`.
...
/** Return a path under `~/.joopo/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".joopo", "qqbot", ...subPaths);
}

/** Return a path under `~/.joopo/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.joopo/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Joopo's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".joopo", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.joopo/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.joopo/media`, Joopo's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `joopo/src/media/local-roots.ts`). Using it as a
 ...
 */
function getJoopoMediaDir(): string {
  return path.join(getHomeDir(), ".joopo", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".joopo", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Joopo's shared `~/.joopo/media`
  ...
  const allowedRoots = [getJoopoMediaDir(), getQQBotMediaPath()];
