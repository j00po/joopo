...
let _joopoVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${_pluginVersion} (Node/${process.versions.node}; ${os.platform()}; Joopo/${_joopoVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; joopoVersion?: string }): void {
  if (options.pluginVersion) {
    _pluginVersion = options.pluginVersion;
  }
  if (options.joopoVersion) {
    _joopoVersion = options.joopoVersion;
  }
}

/** Update the Joopo framework version in the User-Agent (called after runtime injection). */
export function setJoopoVersion(version: string): void {
  if (version) {
    _joopoVersion = version;
  }
}
