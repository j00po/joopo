...
function makeRuntime(params: {
  ...
  return {
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/joopo/qqbot-sessions.json"),
    ...
    tts: {
      textToSpeech: vi.fn(async () => ({
        success: true,
        audioPath: "/tmp/joopo-qqbot/tts.wav",
        provider: "test-tts",
        outputFormat: "wav",
      })),
    },
...
describe("dispatchOutbound", () => {
  ...
  it("synthesizes plain audioAsVoice text as a QQ voice reply", async () => {
    ...
    expect(audioFileToSilkBase64Mock).toHaveBeenCalledWith("/tmp/joopo-qqbot/tts.wav");
