...
import { coerceSecretRef, normalizeSecretInputString } from "joopo/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: JoopoConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: JoopoConfig): string {
...
/** Apply account config updates back into the Joopo config object. */
