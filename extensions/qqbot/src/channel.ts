import { getExecApprovalReplyMetadata } from "joopo/plugin-sdk/approval-runtime";
import {
...
import { toGatewayAccount, writeJoopoConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped joopo.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // joopo.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeJoopoConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe joopo.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeJoopoConfigThroughRuntime(getQQBotRuntime(), nextCfg as JoopoConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as JoopoConfig, accountId);
