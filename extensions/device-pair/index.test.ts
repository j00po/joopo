...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredJoopoTmpDir: vi.fn(() => path.join(os.tmpdir(), "joopo-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredJoopoTmpDir: pluginApiMocks.resolvePreferredJoopoTmpDir,
...
function createApi(params?: {
  ...
  runtime?: JoopoPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as JoopoPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: JoopoPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredJoopoTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredJoopoTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Joopo iOS app:");
    ...
    expect(text).not.toContain("![Joopo pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Joopo iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://joopo.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://joopo.local:18789");
  });
