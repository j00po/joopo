...
import { createPluginSetupWizardStatus } from "joopo/plugin-sdk/plugin-test-runtime";
...
describe("zalouser setup plugin", () => {
  it("builds setup status without an initialized runtime", async () => {
    const stateDir = await mkdtemp(path.join(os.tmpdir(), "joopo-zalouser-setup-"));

    try {
      await withEnvAsync({ JOOPO_STATE_DIR: stateDir }, async () => {
        await expect(
