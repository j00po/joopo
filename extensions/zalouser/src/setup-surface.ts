...
function ensureZalouserPluginEnabled(cfg: JoopoConfig): JoopoConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("joopo directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
      ...
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("joopo directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
