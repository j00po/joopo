...
vi.mock("joopo/plugin-sdk/provider-transport-runtime", async (importOriginal) => ({
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for(
  "joopo.modelProviderRequestTransport",
);
...
describe("google transport stream", () => {
  ...
  afterAll(() => {
    vi.doUnmock("joopo/plugin-sdk/provider-transport-runtime");
    vi.resetModules();
  });
  ...
  it("refreshes authorized_user ADC before Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "joopo-google-vertex-adc-"));
  ...
  it("refreshes authorized_user ADC from the Windows APPDATA fallback for Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "joopo-google-vertex-appdata-adc-"));
