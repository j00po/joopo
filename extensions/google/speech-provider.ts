import { transcodeAudioBufferToOpus } from "joopo/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "joopo/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: JoopoConfig): string | undefined {
...
function isJoopoGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isJoopoGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
