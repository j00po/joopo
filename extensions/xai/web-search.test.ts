import { createTestWizardPrompter } from "joopo/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "joopo/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "joopo/plugin-sdk/runtime-env";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      await expect(maybeTool.execute({ query: "Joopo" })).resolves.toMatchObject({
  ...
  it("routes Grok web search through plugin webSearch.baseUrl", async () => {
    ...
    await tool?.execute({ query: "Joopo Grok proxy test" });
