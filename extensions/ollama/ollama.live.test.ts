...
const LIVE = process.env.JOOPO_LIVE_TEST === "1" && process.env.JOOPO_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.JOOPO_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.JOOPO_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.JOOPO_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.JOOPO_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.JOOPO_LIVE_OLLAMA_WEB_SEARCH !== "0";
...
async function withTempJoopoState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "joopo-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "joopo.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runJoopoCli(args: string[], env: NodeJS.ProcessEnv) {
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "joopo-ollama-cli-output-"));
  ...
  try {
    const result = spawnSync(process.execPath, ["joopo.mjs", ...args], {
      cwd: process.cwd(),
      env,
      timeout: 90_000,
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  return {
    ...
    JOOPO_LIVE_TEST: "1",
    JOOPO_LIVE_OLLAMA: "1",
    JOOPO_LIVE_OLLAMA_WEB_SEARCH: "0",
    JOOPO_STATE_DIR: path.join(root, "state"),
    JOOPO_CONFIG_PATH: path.join(root, "joopo.json"),
    JOOPO_NO_RESPAWN: "1",
    JOOPO_TEST_FAST: "1",
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without PI model discovery", async () => {
    await withTempJoopoState(async ({ root }) => {
      const result = await runJoopoCli(
        [
          "infer",
          "model",
          "run",
