...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): JoopoConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): JoopoConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    const result = await tool.execute({ query: "joopo docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://joopo.ai/docs" }],
    });
  ...
  it("tries the future local direct endpoint when the local proxy endpoint is missing", async () => {
    ...
    await expect(
      runOllamaWebSearch({ config: createOllamaConfig(), query: "joopo" }),
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest joopo release" })).rejects.toThrow(
      "ollama signin",
    );
  });
