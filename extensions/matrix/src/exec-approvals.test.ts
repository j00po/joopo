...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "joopo-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<JoopoConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<JoopoConfig["channels"]>["matrix"]>>,
): JoopoConfig {
  return {
    channels: {
      matrix: {
