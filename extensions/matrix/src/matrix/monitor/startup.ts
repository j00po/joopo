...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleJoopoDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Joopo devices detected for ${params.auth.userId}: ${deviceHealth.staleJoopoDevices.map((device) => device.deviceId).join(", ")}. Run 'joopo matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
