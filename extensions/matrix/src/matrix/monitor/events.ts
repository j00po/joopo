...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'joopo matrix verify status --verbose --account ${accountId}' and 'joopo matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Joopo runtime is using the same account. " +
    `Check 'joopo matrix verify status --verbose --account ${accountId}' and 'joopo matrix devices list --account ${accountId}'.`
  );
}
