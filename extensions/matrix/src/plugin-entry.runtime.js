...
function hasTrustedJoopoRootIndicator(packageRoot, packageJson) {
  ...
  const hasJoopoBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("joopo")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.joopo === "string");
  const hasJoopoEntrypoint = fs.existsSync(path.join(packageRoot, "joopo.mjs"));
  return hasCliEntryExport || hasJoopoBin || hasJoopoEntrypoint;
}

function findJoopoPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "joopo" && hasTrustedJoopoRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findJoopoPackageRoot(moduleDir);
  if (location) {
