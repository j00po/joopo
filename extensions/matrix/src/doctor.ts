...
} from "joopo/plugin-sdk/runtime-doctor";
import {
...
function hasConfiguredMatrixChannel(cfg: JoopoConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: JoopoConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: JoopoConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "joopo doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "joopo doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: JoopoConfig): Promise<string[]> {
...
export async function cleanStaleMatrixPluginConfig(cfg: JoopoConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
