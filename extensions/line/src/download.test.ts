...
vi.mock("joopo/plugin-sdk/media-store", () => ({
...
describe("downloadLineMedia", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("joopo/plugin-sdk/media-store");
    vi.resetModules();
  });
  ...
  beforeEach(() => {
    ...
    saveMediaBufferMock.mockImplementation(
      async (_buffer: Buffer, contentType?: string, subdir?: string) => ({
        path: `/home/user/.joopo/media/${subdir ?? "unknown"}/saved-media`,
        contentType,
      }),
    );
  });
  ...
  it("persists inbound media with the shared media store", async () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.joopo/media/inbound/saved-media",
