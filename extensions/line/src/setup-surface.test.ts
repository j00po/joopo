...
import type { WizardPrompter } from "joopo/plugin-sdk/plugin-test-runtime";
import { bundledPluginRoot } from "joopo/plugin-sdk/test-fixtures";
import ts from "typescript";
import { afterAll, afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { JoopoConfig, PluginRuntime, ResolvedLineAccount } from "../api.js";
...
function collectRuntimeApiPreExports(runtimeApiPath: string): string[] {
  ...
  const removedLineRuntimeSpecifier = ["joopo", "plugin-sdk", "line-runtime"].join("/");
...
describe("line setup wizard", () => {
  ...
  it("reports account-scoped config keys for named accounts", () => {
    expect(lineSetupWizard.dmPolicy?.resolveConfigKeys?.({} as JoopoConfig, "work")).toEqual({
