...
import { createMockIncomingRequest } from "joopo/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "joopo/plugin-sdk/webhook-request-guards";
...
vi.mock("joopo/plugin-sdk/reply-runtime", () => ({
...
vi.mock("joopo/plugin-sdk/channel-message", () => ({
  createChannelMessageReplyPipeline: vi.fn(() => ({})),
  hasFinalChannelMessageReplyDispatch: vi.fn(() => false),
}));

vi.mock("joopo/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("joopo/plugin-sdk/webhook-ingress")>(
    "joopo/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("joopo/plugin-sdk/reply-runtime");
    vi.doUnmock("joopo/plugin-sdk/runtime-env");
    vi.doUnmock("joopo/plugin-sdk/channel-message");
    vi.doUnmock("joopo/plugin-sdk/webhook-ingress");
