...
import { resolvePluginConfigObject } from "joopo/plugin-sdk/plugin-config-runtime";
import type { JoopoPluginApi, ProviderThinkingProfile } from "joopo/plugin-sdk/plugin-entry";
import {
...
} from "joopo/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Joopo's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Joopo identifies this as an Anthropic model via the ARN heuristic.
 ...
 * the Joopo config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: JoopoPluginApi): void {
