...
import { parseAgentSessionKey, parseThreadSessionSuffix } from "joopo/plugin-sdk/routing";
import { isPathInside, replaceFileAtomic } from "joopo/plugin-sdk/security-runtime";
import {
  resolveSessionStoreEntry,
  updateSessionStore,
} from "joopo/plugin-sdk/session-store-runtime";
import { tempWorkspace, resolvePreferredJoopoTmpDir } from "joopo/plugin-sdk/temp-path";
...
function resolvePersistentTranscriptBaseDir(api: JoopoPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: JoopoPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: JoopoConfig): boolean {
  ...
  register(api: JoopoPluginApi) {
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as JoopoConfig;
          if (action === "status") {
            return {
