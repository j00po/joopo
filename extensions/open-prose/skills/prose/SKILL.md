...
metadata: { "joopo": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Joopo `sessions_spawn`
- **File I/O** == Joopo `read`/`write`
- **Remote fetch** == Joopo `web_fetch` (or `exec` with curl when POST is required)
