...
export type JoopoOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: JoopoTransport;
};

function resolveGatewayUrl(options: JoopoOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Joopo Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Joopo Gateway yet`);
}
...
function readChatProjection(event: JoopoEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: JoopoEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: JoopoEvent): boolean {
  return (
...
export class Joopo {
  ...
  private readonly transport: JoopoTransport;
  private readonly normalizedEvents = new EventHub<JoopoEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, JoopoEvent[]>();
  private connected = false;
  private eventPumpPromise: Promise<void> | null = null;
  private eventPumpReady: Promise<void> | null = null;

  constructor(options: JoopoOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: JoopoEvent) => boolean): AsyncIterable<JoopoEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: JoopoEvent) => boolean,
  ): AsyncIterable<JoopoEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: JoopoEvent): JoopoEvent | undefined => {
    ...
    const matches = (event: JoopoEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: JoopoEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): JoopoEvent[] {
...
export class AgentsNamespace {
  constructor(private readonly client: Joopo) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<JoopoEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Joopo,
