...
import { EventHub, Joopo, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements JoopoTransport {
...
describe("Joopo SDK", () => {
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "Joopo Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.tasks.list()).rejects.toThrow(
      "oc.tasks.list is not supported by the current Joopo Gateway yet",
    );
    await expect(oc.tasks.get("task_123")).rejects.toThrow(
      "oc.tasks.get is not supported by the current Joopo Gateway yet",
    );
    await expect(oc.tasks.cancel("task_123")).rejects.toThrow(
      "oc.tasks.cancel is not supported by the current Joopo Gateway yet",
    );
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current Joopo Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current Joopo Gateway yet",
    );
    expect(transport.calls).toEqual([]);
  });
