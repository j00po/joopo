  ...
  it("loads explicit extensionPath without importing bundled sqlite-vec", async () => {
    ...
    await expect(
      loadSqliteVecExtension({
        db: db as never,
        extensionPath: "/opt/joopo/sqlite-vec.so",
      }),
    ).resolves.toEqual({ ok: true, extensionPath: "/opt/joopo/sqlite-vec.so" });
    expect(db.enableLoadExtension).toHaveBeenCalledWith(true);
    expect(db.loadExtension).toHaveBeenCalledWith("/opt/joopo/sqlite-vec.so");
  });
