...
type BuiltInProvider = "joopo" | "pi" | "http";
...
export const llmInvokeCommand = createLlmInvokeCommand({
  ...
  helpConfig: [
    ...
    "Built-in providers: joopo, pi, http.",
    "Joopo provider uses JOOPO_URL (JOOPOBOT_URL also supported) and JOOPO_TOKEN.",
  ...
  helpExamples: [
    ...
    "llm.invoke --provider joopo --model claude-3-sonnet --prompt 'Write summary'",
...
export const llmTaskInvokeCommand = createLlmInvokeCommand({
  ...
  defaultProvider: "joopo",
  description: "Backward-compatible alias for llm.invoke using the Joopo adapter",
  helpTitle: "llm_task.invoke — backward-compatible alias for llm.invoke using Joopo",
  helpConfig: [
    "Requires JOOPO_URL (or JOOPOBOT_URL) and optionally JOOPO_TOKEN.",
    "Use llm.invoke for new workflows and non-Joopo adapters.",
  ],
  helpExamples: [
    ...
    meta: {
      ...
      argsSchema: {
        ...
        properties: {
          provider: {
            type: "string",
            description: "LLM adapter provider (joopo, pi, http). Optional if auto-detected.",
          },
          token: {
            type: "string",
...
function resolveProvider(
  ...
  const explicit = String(args.provider ?? env.LOBSTER_LLM_PROVIDER ?? "")
    ...
    .toLowerCase();
  if (explicit) {
    if (explicit === "joopo" || explicit === "pi" || explicit === "http") {
      return explicit;
    }
  ...
  if (String(env.JOOPO_URL ?? env.JOOPOBOT_URL ?? "").trim()) return "joopo";
...
function resolveAdapter({
  ...
  if (provider === "joopo") {
    const joopoUrl = String(env.JOOPO_URL ?? env.JOOPOBOT_URL ?? "").trim();
    if (!joopoUrl) {
      throw new Error(`${config.name} requires JOOPO_URL (or JOOPOBOT_URL) for provider=joopo`);
    }
    const endpoint = new URL("/tools/invoke", joopoUrl);
    const token = String(args.token ?? env.JOOPO_TOKEN ?? env.JOOPOBOT_TOKEN ?? "").trim();
    return {
      provider,
      source: config.sourceForProvider?.(provider) ?? "joopo",
      async invoke({ payload }) {
        return invokeJoopoAdapter({ endpoint, token, payload });
      },
    };
  }
...
async function invokeJoopoAdapter({
  endpoint,
  token,
  payload,
}: {
  ...
  if (parsed && typeof parsed === "object" && !Array.isArray(parsed) && "ok" in parsed) {
    if (parsed.ok !== true) {
      ...
      throw new Error(`joopo adapter error: ${msg}`);
    }
