function createInvokeCommand(commandName: string) {
  return {
    name: commandName,
    meta: {
      description: "Call a local Joopo tool endpoint",
      argsSchema: {
        type: "object",
        properties: {
          url: {
            type: "string",
            description: "Joopo control URL (or JOOPO_URL / JOOPOBOT_URL)",
          },
          token: { type: "string", description: "Bearer token (or JOOPO_TOKEN / JOOPOBOT_TOKEN)" },
      ...
      sideEffects: ["calls_joopobot_tool"],
    },
    help() {
      return (
        `${commandName} — call a local Joopo tool endpoint\n\n` +
        `Usage:\n` +
        ...
        `Config:\n` +
        `  - Uses JOOPO_URL env var by default (or pass --url).\n` +
        `  - Backward compatible: JOOPOBOT_URL is also supported.\n` +
        `  - Optional Bearer token via JOOPO_TOKEN env var (or pass --token).\n` +
        `  - Backward compatible: JOOPOBOT_TOKEN is also supported.\n` +
    ...
    async run({ input, args, ctx }) {
      ...
      const url = String(args.url ?? ctx.env.JOOPO_URL ?? ctx.env.JOOPOBOT_URL ?? "").trim();
      if (!url) throw new Error(`${commandName} requires --url or JOOPO_URL`);
      ...
      const token = String(
        args.token ?? ctx.env.JOOPO_TOKEN ?? ctx.env.JOOPOBOT_TOKEN ?? "",
      ).trim();
      ...
      if (!each) {
        // Drain input: for now we don't stream input into joopobot calls.
...
export const joopoInvokeCommand = createInvokeCommand("joopo.invoke");
export const joopobotInvokeCommand = createInvokeCommand("joopobot.invoke");
