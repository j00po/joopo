...
function helpText() {
  return (
    `lobster — Joopo-native typed shell\n\n` +
    `Usage:\n` +
    `  lobster '<pipeline>'\n` +
    ...
    `Commands:\n` +
    `  exec, head, json, pick, table, where, approve, ask, joopo.invoke, llm.invoke, llm_task.invoke, state.get, state.set, diff.last, commands.list, workflows.list, workflows.run, graph\n`
  );
}
