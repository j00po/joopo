...
test("email.triage --llm uses llm_task.invoke to draft replies (and can emit drafts)", async () => {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      // Joopo tool router envelope -> llm-task tool envelope
      res.end(
        JSON.stringify({
          ok: true,
          result: {
  ...
  try {
    ...
    const res1 = await runPipeline({
      ...
      env: {
        ...process.env,
        JOOPOBOT_URL: `http://127.0.0.1:${port}`,
...
test("email.triage --llm honors JOOPO_URL (not just JOOPOBOT_URL)", async () => {
  const registry = createDefaultRegistry();
  const cacheDir = await mkdtemp(join(tmpdir(), "lobster-cache-"));
