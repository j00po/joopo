...
test("llm.invoke auto-detects Joopo provider and normalizes output", async () => {
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx(
        { JOOPO_URL: `http://localhost:${port}`, LOBSTER_CACHE_DIR: cacheDir },
        registry,
      ),
    } as any);
    ...
    assert.equal(items[0].source, "joopo");
