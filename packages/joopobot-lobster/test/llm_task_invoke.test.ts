...
test("llm_task.invoke posts to /tools/invoke (joopobot) and normalizes result", async () => {
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx(
        { LOBSTER_CACHE_DIR: cacheDir, JOOPOBOT_URL: `http://localhost:${port}` },
        registry,
      ),
    } as any);
    ...
    assert.equal(payload.source, "joopobot");
...
test("llm_task.invoke persists to run state so resume skips remote call", async () => {
  ...
  try {
    const first = await cmd.run({
      ...
      ctx: baseCtx({ ...ctxEnv, JOOPOBOT_URL: `http://localhost:${port}` }, registry),
    } as any);
    const firstItems = await collect(first.output!);
    assert.equal(firstItems[0].source, "joopobot");
...
test("llm_task.invoke reuses file cache when URL unavailable", async () => {
  ...
  const ctxEnv = { LOBSTER_CACHE_DIR: cacheDir, JOOPOBOT_URL: `http://localhost:${port}` };
...
test("llm_task.invoke uses JOOPOBOT_URL (/tools/invoke) without requiring --url/--model", async () => {
  const registry = createDefaultRegistry();
  const cmd = registry.get("llm_task.invoke");
  assert.ok(cmd);
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      // This is the Joopo tool router envelope.
      res.writeHead(200, { "content-type": "application/json" });
      res.end(
        JSON.stringify({
          ok: true,
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx(
        { JOOPOBOT_URL: `http://localhost:${port}`, LOBSTER_CACHE_DIR: cacheDir },
        registry,
      ),
    } as any);
    ...
    const items = await collect(result.output!);
    assert.equal(items.length, 1);
    assert.equal(items[0].source, "joopobot");
    assert.equal(items[0].cached, false);
    assert.equal(items[0].runId, "task_joopobot_1");
