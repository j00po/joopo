...
**Lobster is safe automation for Joopo — workflows that ask before acting.**

---

## What is Lobster?

Lobster is a workflow runtime for Joopo. It lets you define multi-step automations that:
...
Think of it as **IFTTT/Zapier for Joopo, but with human checkpoints**.

---

## The Problem Lobster Solves

### Today's workflow in Joopo
...
```
Joopo calls: lobster.run("email.triage")
...
## Why Lobster Makes Joopo Better
...
│   - Calls Joopo tools (gmail, trello, etc)  │
│   - Halts at approval checkpoints               │
│   - Returns structured result + resume token    │
└─────────────────────────────────────────────────┘
```

**Key insight:** Lobster doesn't replace Joopo. It's the execution layer that makes Joopo's automations safe and efficient.

- **Joopo** = the brain (understands what you want)
...
### Average Joopo users (invisible benefit)
...
> "Lobster is Zapier for Joopo, except it asks you before doing anything irreversible."

---

## Why Not Build This Into Joopo Core?

It could be. But the plugin architecture is intentional:

1. **Core stays small** — Joopo is already complex
...
Joopo explicitly provides a plugin boundary to enable this pattern. Lobster is the first proof that it works.
...
- Native to Joopo (uses the same tools you already have)
- Safe by default (approvals are a language primitive)
- Invisible when you want (Joopo uses it automatically)
...
- **Not trying to replace Joopo** — It makes Joopo better
- **Not managing your secrets** — Joopo handles auth, Lobster orchestrates
...
| Question         | Answer                                                              |
| ---------------- | ------------------------------------------------------------------- |
| What is it?      | Workflow runtime for Joopo                                          |
...
| Best analogy?    | Zapier for Joopo, but with approval checkpoints                     |
