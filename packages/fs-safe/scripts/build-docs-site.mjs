...
const repoBase = "https://github.com/joopo/fs-safe";
...
const installCmd = "pnpm add @joopo/fs-safe";
...
const sections = [
  ...
  [
    "Specialized",
    ["secret-file.md", "regular-file.md", "sidecar-lock.md", "pinned-open.md", "local-roots.md"],
  ],
  ...
  return {
    ts: [
      ...
      },
      {
        type: "keyword",
        pattern:
          /\b(?:abstract|any|as|async|await|boolean|break|case|catch|class|const|continue|declare|default|delete|do|else|enum|export|extends|finally|for|from|function|get|if|implements|import|in|infer|instanceof|interface|is|keyof|let|namespace|never|new|number|of|out|override|package|private|protected|public|readonly|return|satisfies|set|static|string|super|switch|symbol|this|throw|try|type|typeof|undefined|unique|unknown|var|void|while|yield)\b/,
      },
      { type: "boolean", pattern: /\b(?:true|false|null|undefined)\b/ },
      {
        type: "number",
        pattern:
          /\b(?:0[xX][\da-fA-F_]+|0[bB][01_]+|0[oO][0-7_]+|\d[\d_]*(?:\.\d+)?(?:[eE][+-]?\d+)?)n?\b/,
      },
    ...
    bash: [
      ...
      { type: "variable", pattern: /\$\{[^}]+\}|\$[A-Za-z_]\w*|\$[0-9*@#?!$-]/ },
      {
        type: "keyword",
        pattern:
          /\b(?:if|then|else|elif|fi|for|while|until|do|done|case|esac|in|function|return|exit|break|continue|export|local|readonly|set|unset|declare)\b/,
      },
      {
        type: "function",
        pattern:
          /(?<=^|[\s|;&(])(?:pnpm|npm|yarn|bun|node|git|cd|ls|mkdir|rm|cp|mv|cat|echo|grep|sed|awk|find|curl|wget|tar|zip|unzip)\b/,
      },
